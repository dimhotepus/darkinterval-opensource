...
BEGIN_VS_SHADER( DI_WorldVertexAlpha_DX90, 
	...
	SHADER_FALLBACK
	{
		...
		{
			return "DI_WorldVertexAlpha_DX80";
		}
		return 0;
	}
		...
		if( g_pHardwareConfig->SupportsVertexAndPixelShaders() && !UsingEditor( params ) )
		{
			if( g_pHardwareConfig->GetDXSupportLevel() < 90 )
			{
				...
				SHADOW_STATE
				{
					...
					pShaderShadow->SetVertexShader( "DI_WorldVertexAlpha_DX80", vshIndex.GetIndex() );

					pShaderShadow->SetPixelShader( "DI_WorldVertexAlpha_DX80" );
					FogToFogColor();
				}
			...
			else
			{
				...
				SHADOW_STATE
				{
					...
					worldvertexalpha_Static_Index vshIndex;
					pShaderShadow->SetVertexShader( "DI_WorldVertexAlpha_DX90", vshIndex.GetIndex() );
