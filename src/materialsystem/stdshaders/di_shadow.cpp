...
#include "di_shadow_ps20.inc"
#include "di_shadow_ps20b.inc"
#include "di_shadow_vs20.inc"

BEGIN_VS_SHADER_FLAGS( DI_Shadow_DX90, "Help for Shadow", SHADER_NOT_EDITABLE )
	...
	SHADER_DRAW
	{
		SHADOW_STATE
		{
			...
			DECLARE_STATIC_VERTEX_SHADER(di_shadow_vs20 );
			SET_STATIC_VERTEX_SHADER(di_shadow_vs20 );

			if( g_pHardwareConfig->SupportsPixelShaders_2_b() )
			{
				DECLARE_STATIC_PIXEL_SHADER(di_shadow_ps20b );
				SET_STATIC_PIXEL_SHADER(di_shadow_ps20b );
			}
			else
			{
				DECLARE_STATIC_PIXEL_SHADER(di_shadow_ps20 );
				SET_STATIC_PIXEL_SHADER(di_shadow_ps20 );
			}
		...
		DYNAMIC_STATE
		{
			...
			DECLARE_DYNAMIC_VERTEX_SHADER(di_shadow_vs20 );
			SET_DYNAMIC_VERTEX_SHADER_COMBO( DOWATERFOG,  fogIndex );
			SET_DYNAMIC_VERTEX_SHADER(di_shadow_vs20 );

			if( g_pHardwareConfig->SupportsPixelShaders_2_b() )
			{
				DECLARE_DYNAMIC_PIXEL_SHADER(di_shadow_ps20b );
				SET_DYNAMIC_PIXEL_SHADER_COMBO( PIXELFOGTYPE, pShaderAPI->GetPixelFogCombo() );
				SET_DYNAMIC_PIXEL_SHADER(di_shadow_ps20b );
			}
			else
			{
				DECLARE_DYNAMIC_PIXEL_SHADER(di_shadow_ps20 );
				SET_DYNAMIC_PIXEL_SHADER_COMBO( PIXELFOGTYPE, pShaderAPI->GetPixelFogCombo() );
				SET_DYNAMIC_PIXEL_SHADER(di_shadow_ps20 );
			}
