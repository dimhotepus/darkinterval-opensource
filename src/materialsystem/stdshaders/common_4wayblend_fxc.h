							 ...
							 out HALF4 vResultBase, out HALF4 vResultBase2, out HALF4 vResultBase3, out HALF4 vResultBase4, out HALF4 vResultBump )
{
	...
	vResultBase2 += vWeights.x * tex2D( base2, coords.zy * uvScales23.xy );
	vResultBase3 += vWeights.x * tex2D( base3, coords.zy * uvScales23.zw );
	vResultBase4 += vWeights.x * tex2D( base4, coords.zy * uvScales4.xy  );
	if ( bBump )
	{
	...
	vResultBase2 += vWeights.y * tex2D( base2, coords.xz * uvScales23.xy );
	vResultBase3 += vWeights.y * tex2D( base3, coords.xz * uvScales23.zw );
	vResultBase4 += vWeights.y * tex2D( base4, coords.xz * uvScales4.xy  );
	if ( bBump )
	{
	...
	vResultBase2 += vWeights.z * tex2D( base2, coords.xy * uvScales23.xy );
	vResultBase3 += vWeights.z * tex2D( base3, coords.xy * uvScales23.zw );
	vResultBase4 += vWeights.z * tex2D( base4, coords.xy * uvScales4.xy  );
	if ( bBump )
	{
	...
	vResultBase2 = tex2D( base2, coords.xy * uvScales23.xy );
	vResultBase3 = tex2D( base3, coords.xy * uvScales23.zw  );
	vResultBase4 = tex2D( base4, coords.xy * uvScales4.xy  );
	if ( bBump )
	{
...
HALF4 LightMapSample( sampler LightmapSampler, float2 vTexCoord )
{
	...
	{
		HALF4 sample = tex2D( LightmapSampler, vTexCoord.xy );
		return sample;
	}
	#else
	{
