...
#ifdef DARKINTERVAL
#include "di_skin_vs20.inc"
#include "di_skin_ps20b.inc"
#include "di_skin_vs30.inc"
#include "di_skin_ps30.inc"
#else
...
static ConVar mat_fullbright( "mat_fullbright", "0" );
static ConVar r_lightwarpidentity( "r_lightwarpidentity", "0" );
static ConVar r_rimlight( "r_rimlight", "1" );
							...
							CBasePerMaterialContextData **pContextDataPtr )
{
	...
	if( pShader->IsSnapshotting() )
	{
...
#ifdef DARKINTERVAL
			DECLARE_STATIC_VERTEX_SHADER( di_skin_vs20 );
#else
			DECLARE_STATIC_VERTEX_SHADER( skin_vs20 );
#endif
			SET_STATIC_VERTEX_SHADER_COMBO( USE_STATIC_CONTROL_FLOW, bUseStaticControlFlow );
#ifdef DARKINTERVAL
			SET_STATIC_VERTEX_SHADER( di_skin_vs20 );
#else
			SET_STATIC_VERTEX_SHADER( skin_vs20 );
#endif
			// Assume we're only going to get in here if we support 2b
			DECLARE_STATIC_PIXEL_SHADER( di_skin_ps20b );
...
#ifdef DARKINTERVAL
			SET_STATIC_PIXEL_SHADER( di_skin_ps20b );
#else
		...
		else
		{
...
#ifdef DARKINTERVAL
			DECLARE_STATIC_VERTEX_SHADER(di_skin_vs30 );
#else
			DECLARE_STATIC_VERTEX_SHADER( skin_vs30 );
#endif
			SET_STATIC_VERTEX_SHADER_COMBO( DECAL, bIsDecal );
#ifdef DARKINTERVAL
			SET_STATIC_VERTEX_SHADER(di_skin_vs30 );
			DECLARE_STATIC_PIXEL_SHADER(di_skin_ps30);
#else
			...
			SET_STATIC_PIXEL_SHADER_COMBO( BLENDTINTBYBASEALPHA, bBlendTintByBaseAlpha );
#ifdef DARKINTERVAL
			SET_STATIC_PIXEL_SHADER(di_skin_ps30 );
#else
	...
	else // not snapshotting -- begin dynamic state
	{
		...
		if ( !g_pHardwareConfig->HasFastVertexTextures() )
		{
			bool bUseStaticControlFlow = g_pHardwareConfig->SupportsStaticControlFlow();
#ifdef DARKINTERVAL
			DECLARE_DYNAMIC_VERTEX_SHADER(di_skin_vs20 );
#else
...
#ifdef DARKINTERVAL
			SET_DYNAMIC_VERTEX_SHADER(di_skin_vs20 );

			DECLARE_DYNAMIC_PIXEL_SHADER(di_skin_ps20b );
#else
...
#ifdef DARKINTERVAL
			SET_DYNAMIC_PIXEL_SHADER(di_skin_ps20b );
#else
		...
		else
		{
...
#ifdef DARKINTERVAL
			DECLARE_DYNAMIC_VERTEX_SHADER(di_skin_vs30 );
#else
			...
			SET_DYNAMIC_VERTEX_SHADER_COMBO( COMPRESSED_VERTS, (int)vertexCompression );
#ifdef DARKINTERVAL
			SET_DYNAMIC_VERTEX_SHADER(di_skin_vs30 );

			DECLARE_DYNAMIC_PIXEL_SHADER(di_skin_ps30 );
#else
			...
			SET_DYNAMIC_PIXEL_SHADER_COMBO( FLASHLIGHTSHADOWS, bFlashlightShadows );
#ifdef DARKINTERVAL
			SET_DYNAMIC_PIXEL_SHADER(di_skin_ps30 );
#else
