...
//
// Purpose: 
// DI implements Mapbase's DecalModulate fix for decals on models under flashlight.
// $Header: $
// $NoKeywords: $
//===========================================================================//
...
#ifdef DARKINTERVAL
#include "mapbase_decalmodulate_vs20.inc"
#include "mapbase_decalmodulate_ps20.inc"
#include "mapbase_decalmodulate_ps20b.inc"

#include "mapbase_decalmodulate_vs30.inc"
#include "mapbase_decalmodulate_ps30.inc"
#else
...
#endif
#ifdef DARKINTERVAL
DEFINE_FALLBACK_SHADER(DI_DecalModulate, DI_DecalModulate_DX90)

BEGIN_VS_SHADER(DI_DecalModulate_DX90,	"")
#else
...
SHADER_DRAW
{
	...
	SHADOW_STATE
	{
		...
		if (!g_pHardwareConfig->HasFastVertexTextures())
		{
			DECLARE_STATIC_VERTEX_SHADER(mapbase_decalmodulate_vs20);
...
#endif
			SET_STATIC_VERTEX_SHADER(mapbase_decalmodulate_vs20);

			if (g_pHardwareConfig->SupportsPixelShaders_2_b())
			{
				DECLARE_STATIC_PIXEL_SHADER(mapbase_decalmodulate_ps20b);
				SET_STATIC_PIXEL_SHADER_COMBO(VERTEXALPHA,  bHasVertexAlpha);
#ifdef MAPBASE
				SET_STATIC_PIXEL_SHADER_COMBO(FLASHLIGHT,  bHasFlashlight);
				SET_STATIC_PIXEL_SHADER_COMBO(FLASHLIGHTDEPTHFILTERMODE, nShadowFilterMode);
#endif
				SET_STATIC_PIXEL_SHADER(mapbase_decalmodulate_ps20b);
			}
			else
			{
				DECLARE_STATIC_PIXEL_SHADER(mapbase_decalmodulate_ps20);
				SET_STATIC_PIXEL_SHADER_COMBO(VERTEXALPHA,  bHasVertexAlpha);
#ifdef MAPBASE
				SET_STATIC_PIXEL_SHADER_COMBO(FLASHLIGHT, bHasFlashlight);
#endif
				SET_STATIC_PIXEL_SHADER(mapbase_decalmodulate_ps20);
			}
		}
		else
		{
			DECLARE_STATIC_VERTEX_SHADER(mapbase_decalmodulate_vs30);
			SET_STATIC_VERTEX_SHADER_COMBO(VERTEXCOLOR,  bHasVertexAlpha);
			SET_STATIC_VERTEX_SHADER_COMBO(LIGHTING_PREVIEW, false);
#ifdef MAPBASE
			SET_STATIC_VERTEX_SHADER_COMBO(FLASHLIGHT, bHasFlashlight);
#endif
			SET_STATIC_VERTEX_SHADER(mapbase_decalmodulate_vs30);

			DECLARE_STATIC_PIXEL_SHADER(mapbase_decalmodulate_ps30);
			SET_STATIC_PIXEL_SHADER_COMBO(VERTEXALPHA,  bHasVertexAlpha);
#ifdef MAPBASE
			SET_STATIC_PIXEL_SHADER_COMBO(FLASHLIGHT, bHasFlashlight);
			SET_STATIC_PIXEL_SHADER_COMBO(FLASHLIGHTDEPTHFILTERMODE, nShadowFilterMode);
#endif
			SET_STATIC_PIXEL_SHADER(mapbase_decalmodulate_ps30);
		}
	...
	DYNAMIC_STATE
	{
		...
		if (!g_pHardwareConfig->HasFastVertexTextures())
		{
			DECLARE_DYNAMIC_VERTEX_SHADER(mapbase_decalmodulate_vs20);
			...
			SET_DYNAMIC_VERTEX_SHADER(mapbase_decalmodulate_vs20);

			if (g_pHardwareConfig->SupportsPixelShaders_2_b())
			{
				DECLARE_DYNAMIC_PIXEL_SHADER(mapbase_decalmodulate_ps20b);
				SET_DYNAMIC_PIXEL_SHADER_COMBO(PIXELFOGTYPE, pShaderAPI->GetPixelFogCombo());
#ifdef MAPBASE
				SET_DYNAMIC_PIXEL_SHADER_COMBO(FLASHLIGHTSHADOWS, bFlashlightShadows);
#endif
				SET_DYNAMIC_PIXEL_SHADER(mapbase_decalmodulate_ps20b);
			}
			else
			{
				DECLARE_DYNAMIC_PIXEL_SHADER(mapbase_decalmodulate_ps20);
				SET_DYNAMIC_PIXEL_SHADER_COMBO(PIXELFOGTYPE, pShaderAPI->GetPixelFogCombo());
				SET_DYNAMIC_PIXEL_SHADER(mapbase_decalmodulate_ps20);
			}
		}
		else
		{
			...
			DECLARE_DYNAMIC_VERTEX_SHADER(mapbase_decalmodulate_vs30);
			SET_DYNAMIC_VERTEX_SHADER_COMBO(DOWATERFOG, fogType == MATERIAL_FOG_LINEAR_BELOW_FOG_Z);
			SET_DYNAMIC_VERTEX_SHADER_COMBO(SKINNING, pShaderAPI->GetCurrentNumBones() > 0);
			SET_DYNAMIC_VERTEX_SHADER_COMBO(COMPRESSED_VERTS, (int)vertexCompression);
			SET_DYNAMIC_VERTEX_SHADER_COMBO(MORPHING, pShaderAPI->IsHWMorphingEnabled());
			//				SET_DYNAMIC_VERTEX_SHADER_COMBO( TESSELLATION, 0 );             // JasonM TODO: set this appropriately when we care about decals on subds				
			SET_DYNAMIC_VERTEX_SHADER(mapbase_decalmodulate_vs30);

			DECLARE_DYNAMIC_PIXEL_SHADER(mapbase_decalmodulate_ps30);
			SET_DYNAMIC_PIXEL_SHADER_COMBO(PIXELFOGTYPE, pShaderAPI->GetPixelFogCombo());
#ifdef MAPBASE
			SET_DYNAMIC_PIXEL_SHADER_COMBO(FLASHLIGHTSHADOWS, bFlashlightShadows);
#endif
			SET_DYNAMIC_PIXEL_SHADER(mapbase_decalmodulate_ps30);
