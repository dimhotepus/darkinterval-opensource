...
BEGIN_VS_SHADER( DI_UnlitGeneric_DX90, "Help for UnlitGeneric" )
	...
	SHADER_DRAW
	{
		...
		if ((pShaderShadow == NULL) && (pShaderAPI != NULL) && (params[RECEIVEFLASHLIGHT]->GetIntValue() == 0) && pShaderAPI->InFlashlightMode()) // Not snapshotting && flashlight pass
		{
			Draw( false );
		}
		else
