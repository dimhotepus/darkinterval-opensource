...
#include "convar.h"
#ifdef DARKINTERVAL
#include "di_Refract_vs20.inc"
#include "di_Refract_ps20.inc"
#include "di_Refract_ps20b.inc"
#else
...
//	bool bMirrorAboutViewportEdges = IsX360() && (info.m_nMirrorAboutViewportEdges != -1) && (params[info.m_nMirrorAboutViewportEdges]->GetIntValue() != 0);
	...
	SHADOW_STATE
	{
...
#ifdef DARKINTERVAL
		DECLARE_STATIC_VERTEX_SHADER(di_refract_vs20 );
#else
...
#ifdef DARKINTERVAL
		SET_STATIC_VERTEX_SHADER(di_refract_vs20 );
#else
		...
		{
#ifdef DARKINTERVAL
			DECLARE_STATIC_PIXEL_SHADER(di_refract_ps20b );
#else
			DECLARE_STATIC_VERTEX_SHADER(refract_vs20);
#endif
...
#ifdef DARKINTERVAL
			SET_STATIC_PIXEL_SHADER(di_refract_ps20b );
#else
		...
		{
#ifdef DARKINTERVAL
			DECLARE_STATIC_PIXEL_SHADER(di_refract_ps20 );
#else
		...
		//	SET_STATIC_PIXEL_SHADER_COMBO( MIRRORABOUTVIEWPORTEDGES, bMirrorAboutViewportEdges);
			...
			SET_STATIC_PIXEL_SHADER_COMBO( NORMAL_DECODE_MODE, (int) nNormalDecodeMode );
#ifdef DARKINTERVAL
			SET_STATIC_PIXEL_SHADER(di_refract_ps20 );
#else
	...
	DYNAMIC_STATE
	{
...
#ifdef DARKINTERVAL
		DECLARE_DYNAMIC_VERTEX_SHADER(di_refract_vs20 );
#else
...
#ifdef DARKINTERVAL
		SET_DYNAMIC_VERTEX_SHADER(di_refract_vs20 );
#else
		...
		{
#ifdef DARKINTERVAL
			DECLARE_DYNAMIC_PIXEL_SHADER(di_refract_ps20b );
#else
...
#ifdef DARKINTERVAL
			SET_DYNAMIC_PIXEL_SHADER(di_refract_ps20b );
#else
		...
		{
#ifdef DARKINTERVAL
			DECLARE_DYNAMIC_PIXEL_SHADER(di_refract_ps20 );
#else
			DECLARE_DYNAMIC_PIXEL_SHADER(_refract_ps20);
#endif
			SET_DYNAMIC_PIXEL_SHADER_COMBO( PIXELFOGTYPE, pShaderAPI->GetPixelFogCombo() );
#ifdef DARKINTERVAL
			SET_DYNAMIC_PIXEL_SHADER(di_refract_ps20 );
#else
...
/*		int nViewportX, nViewportY, nViewportWidth, nViewportHeight;
		...
		pShaderAPI->GetViewport(nViewportX, nViewportY, nViewportWidth, nViewportHeight);
