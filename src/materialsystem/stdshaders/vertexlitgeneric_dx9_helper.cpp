...
/*^*/ // 		printf("\t\t[3] pShaderAPI && ( (! pContextData ) || ( pContextData->m_bMaterialVarsChanged ) )  TRUE \n");
			...
			if ( bHasFlashlight )
			{
				...
				float tweaks[4];
#ifdef DARKINTERVAL
				tweaks[0] = flashlightState.m_flShadowFilterSize / (float)flashlightState.m_nShadowMapResolution;
#else
...
//					SET_DYNAMIC_PIXEL_SHADER_COMBO( PIXELFOGTYPE, pShaderAPI->GetPixelFogCombo() );
			...
			else
			{
				pShader->SetHWMorphVertexShaderState(VERTEX_SHADER_SHADER_SPECIFIC_CONST_10,
					VERTEX_SHADER_SHADER_SPECIFIC_CONST_11, SHADER_VERTEXTEXTURE_SAMPLER0);
...
#ifdef DARKINTERVAL
				SET_DYNAMIC_VERTEX_SHADER_COMBO( DYNAMIC_LIGHT, true );
				SET_DYNAMIC_VERTEX_SHADER_COMBO( STATIC_LIGHT, true );
#else
