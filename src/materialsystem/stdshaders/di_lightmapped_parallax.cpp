...
ConVar r_parallaxmap_debug("r_parallaxmap_debug", "0");
static ConVar r_parallaxmap_lod("r_parallaxmap_lod", "-1");

BEGIN_VS_SHADER( DI_LightmappedParallax_DX90,	 "DI_LightmappedParallax_DX90" )
			...
			IShaderDynamicAPI *pShaderAPI, IShaderShadow* pShaderShadow, bool bBumpedEnvMap, bool hasFlashlight)
	{
		...
		bool bHasColor = params[COLOR]->IsDefined();
		...
		if (hasBaseTexture)
		{
			...
			//	vvParams[0] = params[PARALLAXMAPRANGESCALE]->GetFloatValue() * (useDotProductFix ? 0.01 : 1);
				...
				pShaderAPI->SetVertexShaderConstant(VERTEX_SHADER_SHADER_SPECIFIC_CONST_2, vParams2, 1);
				...
				pShaderAPI->SetPixelShaderConstant(9, color.Base());
				
				MaterialFogMode_t fogType = pShaderAPI->GetSceneFogMode();
				int fogIndex = (fogType == MATERIAL_FOG_LINEAR_BELOW_FOG_Z) ? 1 : 0;

			//	ConVarRef normalmapsConvar("mat_normalmaps", true);
				int iDebug = /*(normalmapsConvar.GetBool()) ? 2 :*/ (r_parallaxmap_debug.GetInt() ? r_parallaxmap_debug.GetInt() : clamp(params[PARALLAXDEBUG]->GetIntValue(), 0, 3));
				...
				SET_DYNAMIC_PIXEL_SHADER_COMBO(PARALLAXDEBUG, iDebug);
				SET_DYNAMIC_PIXEL_SHADER(di_lightmapped_parallax_ps30);

				float eyePos[4];
				pShaderAPI->GetWorldSpaceCameraPosition(eyePos);
				eyePos[3] = r_parallaxmap_lod.GetFloat();
