...
#ifdef DARKINTERVAL
#include "di_cloak_blended_pass_vs20.inc"
#include "di_cloak_blended_pass_ps20.inc"
#include "di_cloak_blended_pass_ps20b.inc"
#include "di_cloak_blended_pass_vs30.inc"
#include "di_cloak_blended_pass_ps30.inc"
#else
						  ...
						  IShaderShadow* pShaderShadow, CloakBlendedPassVars_t &info, VertexCompressionType_t vertexCompression )
{
	...
	SHADOW_STATE
	{
		...
		if ( !g_pHardwareConfig->HasFastVertexTextures() )
		{
			...
			{
#ifdef DARKINTERVAL
				DECLARE_STATIC_PIXEL_SHADER(di_cloak_blended_pass_ps20b );
#else
				DECLARE_STATIC_PIXEL_SHADER( cloak_blended_pass_ps20b );
#endif
				SET_STATIC_PIXEL_SHADER_COMBO( BUMPMAP, bBumpMapping ? 1 : 0 );
#ifdef DARKINTERVAL
				SET_STATIC_PIXEL_SHADER(di_cloak_blended_pass_ps20b );
#else
			...
			{
#ifdef DARKINTERVAL
				DECLARE_STATIC_PIXEL_SHADER(di_cloak_blended_pass_ps20 );
#else
				DECLARE_STATIC_PIXEL_SHADER( cloak_blended_pass_ps20 );
#endif
				SET_STATIC_PIXEL_SHADER_COMBO( BUMPMAP, bBumpMapping ? 1 : 0 );
#ifdef DARKINTERVAL
				SET_STATIC_PIXEL_SHADER(di_cloak_blended_pass_ps20 );
#else
		...
		else
		{
			...
			// Vertex Shader
#ifdef DARKINTERVAL
			DECLARE_STATIC_VERTEX_SHADER(di_cloak_blended_pass_vs30 );
#else
			DECLARE_STATIC_VERTEX_SHADER( cloak_blended_pass_vs30 );
#endif
			SET_STATIC_VERTEX_SHADER_COMBO( BUMPMAP, bBumpMapping ? 1 : 0 );
#ifdef DARKINTERVAL
			SET_STATIC_VERTEX_SHADER(di_cloak_blended_pass_vs30 );
#else
			SET_STATIC_VERTEX_SHADER( cloak_blended_pass_vs30 );
#endif
			// Pixel Shader
#ifdef DARKINTERVAL
			DECLARE_STATIC_PIXEL_SHADER(di_cloak_blended_pass_ps30 );
#else
			DECLARE_STATIC_PIXEL_SHADER( cloak_blended_pass_ps30 );
#endif
			SET_STATIC_PIXEL_SHADER_COMBO( BUMPMAP, bBumpMapping ? 1 : 0 );
#ifdef DARKINTERVAL
			SET_STATIC_PIXEL_SHADER(di_cloak_blended_pass_ps30 );
#else
...
#ifdef DARKINTERVAL
			DECLARE_DYNAMIC_VERTEX_SHADER(di_cloak_blended_pass_vs20 );
#else
...
#ifdef DARKINTERVAL
			SET_DYNAMIC_VERTEX_SHADER(di_cloak_blended_pass_vs20 );
#else
...
#ifdef DARKINTERVAL
			{
				DECLARE_DYNAMIC_PIXEL_SHADER(di_cloak_blended_pass_ps20b );
				SET_DYNAMIC_PIXEL_SHADER(di_cloak_blended_pass_ps20b );
			}
			else
			{
				DECLARE_DYNAMIC_PIXEL_SHADER(di_cloak_blended_pass_ps20 );
				SET_DYNAMIC_PIXEL_SHADER(di_cloak_blended_pass_ps20 );
			}
#else
			{
...
#define vsh_forgot_to_set_dynamic_MORPHING 0
			// Set Vertex Shader Combos
#ifdef DARKINTERVAL
			DECLARE_DYNAMIC_VERTEX_SHADER(di_cloak_blended_pass_vs30 );
#else
			...
			SET_DYNAMIC_VERTEX_SHADER_COMBO( COMPRESSED_VERTS, (int)vertexCompression );
#ifdef DARKINTERVAL
			SET_DYNAMIC_VERTEX_SHADER(di_cloak_blended_pass_vs30 );
#else
			...
			// Set Pixel Shader Combos
#ifdef DARKINTERVAL
			DECLARE_DYNAMIC_PIXEL_SHADER(di_cloak_blended_pass_ps30 );
			SET_DYNAMIC_PIXEL_SHADER(di_cloak_blended_pass_ps30 );
#else
