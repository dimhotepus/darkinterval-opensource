...
#include "flowmappedwatercheap_vs20.inc"
#include "flowmappedwatercheap_ps20.inc"
#include "flowmappedwatercheap_ps20b.inc"
#include "flowmappedwater_vs20.inc"
#include "flowmappedwater_ps20.inc"
#include "flowmappedwater_ps20b.inc"
...
BEGIN_VS_SHADER( DI_Water_DX90, 
		...
		IShaderDynamicAPI* pShaderAPI, bool bReflection, bool bRefraction ) 
	{
		...
		//	hasFlashlight = UsingFlashlight(params);
		//	if (!hasFlashlight && pShaderAPI) hasFlashlight = CShader_IsFlag2Set(params, MATERIAL_VAR2_USE_FLASHLIGHT);
		//	if (UsingFlashlight(params) || CShader_IsFlag2Set(params, MATERIAL_VAR2_USE_FLASHLIGHT)) Msg("using flashlight is set (checked flag)\n");
			if ( bRefraction )
			{
				// refract sampler
			...
			DECLARE_STATIC_VERTEX_SHADER( flowmappedwater_vs20 );
			...
			SET_STATIC_VERTEX_SHADER(flowmappedwater_vs20 );
			...
			{
				DECLARE_STATIC_PIXEL_SHADER(flowmappedwater_ps20b );
				...
				SET_STATIC_PIXEL_SHADER(flowmappedwater_ps20b );
			}
			else
			{
				DECLARE_STATIC_PIXEL_SHADER( flowmappedwater_ps20 );
				SET_STATIC_PIXEL_SHADER_COMBO( REFLECT,  bReflection );
				SET_STATIC_PIXEL_SHADER_COMBO( REFRACT,  bRefraction );
				SET_STATIC_PIXEL_SHADER_COMBO( ABOVEWATER,  params[ABOVEWATER]->GetIntValue() );
				SET_STATIC_PIXEL_SHADER_COMBO( MULTITEXTURE, bHasMultiTexture );
				...
				SET_STATIC_PIXEL_SHADER_COMBO( FORCEFRESNEL, bForceFresnel );
				SET_STATIC_PIXEL_SHADER( flowmappedwater_ps20 );
			}
		...
		DYNAMIC_STATE
		{
			...
			//hasFlashlight = UsingFlashlight(params);
			//if (!hasFlashlight) hasFlashlight = CShader_IsFlag2Set(params, MATERIAL_VAR2_USE_FLASHLIGHT);
			//if (UsingFlashlight(params) || CShader_IsFlag2Set(params, MATERIAL_VAR2_USE_FLASHLIGHT)) Msg("using flashlight is set (checked Using)\n");
			if( bRefraction )
			{
			...
			DECLARE_DYNAMIC_VERTEX_SHADER( flowmappedwater_vs20 );
			SET_DYNAMIC_VERTEX_SHADER(flowmappedwater_vs20 );
...
//			bool bUberlight = false;
			if( hasFlashlight )
			{
				...
				BindTexture(SHADER_SAMPLER13, flashlightState.m_pSpotlightTexture, flashlightState.m_nSpotlightTextureFrame);
			}

			if ( g_pHardwareConfig->SupportsPixelShaders_2_b() )
			{
				DECLARE_DYNAMIC_PIXEL_SHADER( flowmappedwater_ps20b );
				SET_DYNAMIC_PIXEL_SHADER_COMBO( FLASHLIGHTSHADOWS, hasFlashlight);
				SET_DYNAMIC_PIXEL_SHADER_COMBO( PIXELFOGTYPE, pShaderAPI->GetPixelFogCombo());
				SET_DYNAMIC_PIXEL_SHADER(flowmappedwater_ps20b );
			}
			else
			{
				DECLARE_DYNAMIC_PIXEL_SHADER(flowmappedwater_ps20 );
				SET_DYNAMIC_PIXEL_SHADER_COMBO(FLASHLIGHTSHADOWS, 0);
				SET_DYNAMIC_PIXEL_SHADER_COMBO(PIXELFOGTYPE, pShaderAPI->GetPixelFogCombo());
				SET_DYNAMIC_PIXEL_SHADER(flowmappedwater_ps20 );
			}
		                        ...
		                        IShaderDynamicAPI* pShaderAPI, bool bBlend, bool bRefraction )
	{
		...
		SHADOW_STATE
		{
			...
			DECLARE_STATIC_VERTEX_SHADER(flowmappedwatercheap_vs20 );
			SET_STATIC_VERTEX_SHADER_COMBO( BLEND,  bBlend && bRefraction );
			SET_STATIC_VERTEX_SHADER(flowmappedwatercheap_vs20 );

			if( g_pHardwareConfig->SupportsPixelShaders_2_b() )
			{
				DECLARE_STATIC_PIXEL_SHADER(flowmappedwatercheap_ps20b );
				...
				SET_STATIC_PIXEL_SHADER_COMBO( FLOWMAP, bHasFlowmap );
				SET_STATIC_PIXEL_SHADER_COMBO( FLOW_DEBUG, clamp( params[ FLOW_DEBUG ]->GetIntValue(), 0, 2 ) );
				SET_STATIC_PIXEL_SHADER(flowmappedwatercheap_ps20b );
			}
			else
			{
				DECLARE_STATIC_PIXEL_SHADER(flowmappedwatercheap_ps20 );
				SET_STATIC_PIXEL_SHADER_COMBO( FRESNEL,  params[NOFRESNEL]->GetIntValue() == 0 );
				SET_STATIC_PIXEL_SHADER_COMBO( BLEND,  bBlend );
				SET_STATIC_PIXEL_SHADER_COMBO( REFRACTALPHA,  bRefraction );
				SET_STATIC_PIXEL_SHADER_COMBO( HDRTYPE,  g_pHardwareConfig->GetHDRType() );
				...
				SET_STATIC_PIXEL_SHADER_COMBO( FLOWMAP, bHasFlowmap );
				SET_STATIC_PIXEL_SHADER_COMBO( FLOW_DEBUG, clamp( params[ FLOW_DEBUG ]->GetIntValue(), 0, 2 ) );
				SET_STATIC_PIXEL_SHADER(flowmappedwatercheap_ps20 );
			}
		...
		DYNAMIC_STATE
		{
			...
			DECLARE_DYNAMIC_VERTEX_SHADER( flowmappedwatercheap_vs20 );
			SET_DYNAMIC_VERTEX_SHADER(flowmappedwatercheap_vs20 );

			if( g_pHardwareConfig->SupportsPixelShaders_2_b() )
			{
				DECLARE_DYNAMIC_PIXEL_SHADER(flowmappedwatercheap_ps20b );
				SET_DYNAMIC_PIXEL_SHADER_COMBO( HDRENABLED,  IsHDREnabled() );
				SET_DYNAMIC_PIXEL_SHADER_COMBO(PIXELFOGTYPE, pShaderAPI->GetPixelFogCombo());
				SET_DYNAMIC_PIXEL_SHADER(flowmappedwatercheap_ps20b );
			}
			else
			{
				DECLARE_DYNAMIC_PIXEL_SHADER(flowmappedwatercheap_ps20 );
				SET_DYNAMIC_PIXEL_SHADER_COMBO( HDRENABLED,  IsHDREnabled() );
				SET_DYNAMIC_PIXEL_SHADER_COMBO(PIXELFOGTYPE, pShaderAPI->GetPixelFogCombo());
				SET_DYNAMIC_PIXEL_SHADER(flowmappedwatercheap_ps20 );
			}
		}
		Draw();
	}
