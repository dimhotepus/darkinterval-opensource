...
#ifdef DARKINTERVAL
#include "di_emissive_scroll_blended_pass_vs20.inc"
#include "di_emissive_scroll_blended_pass_ps20.inc"
#include "di_emissive_scroll_blended_pass_ps20b.inc"
#include "di_emissive_scroll_blended_pass_vs30.inc"
#include "di_emissive_scroll_blended_pass_ps30.inc"
#endif
ConVar r_emissiveblend_mode("r_emissiveblend_mode", "0", 0, "0 = default (SHADER_BLEND_ONE, SHADER_BLEND_ONE)\n1 = override using r_emissiveblend_srcmode and r_emissiveblend_destmode");
ConVar r_emissiveblend_srcmode("r_emissiveblend_srcmode", "0");
ConVar r_emissiveblend_dstmode("r_emissiveblend_dstmode", "0");
								   ...
								   IShaderShadow* pShaderShadow, EmissiveScrollBlendedPassVars_t &info, VertexCompressionType_t vertexCompression )
{
	SHADOW_STATE
	{
		...
		if ( !g_pHardwareConfig->HasFastVertexTextures() )
		{
			// Vertex Shader
#ifdef DARKINTERVAL
			DECLARE_STATIC_VERTEX_SHADER(di_emissive_scroll_blended_pass_vs20 );
			SET_STATIC_VERTEX_SHADER(di_emissive_scroll_blended_pass_vs20 );

			// Pixel Shader
			if( g_pHardwareConfig->SupportsPixelShaders_2_b() )
			{
				DECLARE_STATIC_PIXEL_SHADER(di_emissive_scroll_blended_pass_ps20b );
				SET_STATIC_PIXEL_SHADER(di_emissive_scroll_blended_pass_ps20b );
			}
			else
			{
				DECLARE_STATIC_PIXEL_SHADER(di_emissive_scroll_blended_pass_ps20 );
				SET_STATIC_PIXEL_SHADER(di_emissive_scroll_blended_pass_ps20 );
			}
		}
		else
		{
			...
			DECLARE_STATIC_VERTEX_SHADER(di_emissive_scroll_blended_pass_vs30 );
			SET_STATIC_VERTEX_SHADER(di_emissive_scroll_blended_pass_vs30 );

			DECLARE_STATIC_PIXEL_SHADER(di_emissive_scroll_blended_pass_ps30 );
			SET_STATIC_PIXEL_SHADER(di_emissive_scroll_blended_pass_ps30 );
		}
#else
		...
		// Blending
		if (!r_emissiveblend_mode.GetInt())
			pShader->EnableAlphaBlending( SHADER_BLEND_ONE, SHADER_BLEND_ONE );
		else
			pShader->EnableAlphaBlending(ShaderBlendFactor_t(r_emissiveblend_srcmode.GetInt()), ShaderBlendFactor_t(r_emissiveblend_dstmode.GetInt()));
	...
	DYNAMIC_STATE
	{
...
#ifdef DARKINTERVAL
			DECLARE_DYNAMIC_VERTEX_SHADER(di_emissive_scroll_blended_pass_vs20 );
#else
...
#ifdef DARKINTERVAL
			SET_DYNAMIC_VERTEX_SHADER(di_emissive_scroll_blended_pass_vs20 );
#else
...
#ifdef DARKINTERVAL
			{
				DECLARE_DYNAMIC_PIXEL_SHADER(di_emissive_scroll_blended_pass_ps20b );
				SET_DYNAMIC_PIXEL_SHADER(di_emissive_scroll_blended_pass_ps20b );
			}
			else
			{
				DECLARE_DYNAMIC_PIXEL_SHADER(di_emissive_scroll_blended_pass_ps20 );
				SET_DYNAMIC_PIXEL_SHADER(di_emissive_scroll_blended_pass_ps20 );
			}
#else
			{
		...
		else
		{
			...
			// Set Vertex Shader Combos
#ifdef DARKINTERVAL
			DECLARE_DYNAMIC_VERTEX_SHADER(di_emissive_scroll_blended_pass_vs30 );
#else
			...
			SET_DYNAMIC_VERTEX_SHADER_COMBO( COMPRESSED_VERTS, (int)vertexCompression );
#ifdef DARKINTERVAL
			SET_DYNAMIC_VERTEX_SHADER(di_emissive_scroll_blended_pass_vs30 );

			DECLARE_DYNAMIC_PIXEL_SHADER(di_emissive_scroll_blended_pass_ps30 );
			SET_DYNAMIC_PIXEL_SHADER(di_emissive_scroll_blended_pass_ps30 );
#else
