...
ConVar mat_fullbright( "mat_fullbright","0" );
ConVar my_mat_fullbright( "mat_fullbright","0" );
								 ...
								 )
{
	...
	if ( pShaderShadow || ( ! pContextData ) || pContextData->m_bMaterialVarsChanged  || hasFlashlight )
	{
...
#ifdef DARKINTERVAL
		bool hasDetailTexture = IsTextureSet(info.m_nDetail, params);
#else
...
#ifdef DARKINTERVAL
		// this fixes the bug where flashlight shone on di_lightmappedgeneric_dx90
		// faces with $normalmapalphaenvmask broke the shader and displayed matte lightmap
		// without albedo, bump or cubemap. 
		if (hasFlashlight) CLEAR_FLAGS(MATERIAL_VAR_NORMALMAPALPHAENVMAPMASK);
		else
		{
			if (hasBump && IsTextureSet(info.m_nEnvmap, params))
			{
				SET_FLAGS(MATERIAL_VAR_NORMALMAPALPHAENVMAPMASK); // fixme: is this a hack or relatively normal solution?
			}
		}
#endif
		...
		bool hasParallaxCorrection = params[info.m_nEnvmapParallaxObb1]->IsDefined()
			&& params[info.m_nEnvmap]->IsDefined();
#endif
		if ( hasFlashlight )				
		{
...
//		if( hasLightmap )
			...
			//	if (hasFlashlight) // this is only for 360, so, disable.
			//	{
...
#ifndef DARKINTERVAL // HL2 code, but unused anyway
...
#ifdef DARKINTERVAL																			// Parallax cubemaps
		...
		if ( pShaderAPI && pContextData->m_bMaterialVarsChanged )
		{
...
#else // barely ever used, but bump2 transform can be more useful
				params[ info.m_nBumpTransform2 ]->MatrixIsIdentity() &&
			...
			if ( ! pContextData->m_bVertexShaderFastPath )
			{
				...
				if( hasEnvmapMask )
				{
#ifndef DARKINTERVAL // barely ever used, but bump2 transform can be more useful
		...
		//	if (bParallaxMapping )
		//	{
		//		pContextData->m_SemiStaticCmdsOut.SetPixelShaderConstant4(20, GetFloatParam(info.m_nHeightScale, params), 0, 0, 0);
		//	}
#endif
			// texture binds
			if( hasBaseTexture )
			...
			if (hasFlashlight)
			{
				...
				pContextData->m_SemiStaticCmdsOut.BindTexture( pShader, SHADER_SAMPLER13, info.m_nFlashlightTexture, info.m_nFlashlightTextureFrame);

				if (pFlashlightDepthTexture && g_pConfig->ShadowDepthTexture())
				{
					pContextData->m_SemiStaticCmdsOut.BindTexture( pShader, SHADER_SAMPLER14, pFlashlightDepthTexture, -1);
				}
			}
			*/
#endif // DARKINTERVAL
