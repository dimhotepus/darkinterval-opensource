...
#include "di_aftershock_vs20.inc"
#include "di_aftershock_ps20.inc"
#include "di_aftershock_ps20b.inc"
					...
					IShaderShadow* pShaderShadow, AftershockVars_t &info, VertexCompressionType_t vertexCompression )
{
	...
	SHADOW_STATE
	{
		...
		// Vertex Shader
		DECLARE_STATIC_VERTEX_SHADER(di_aftershock_vs20 );
		SET_STATIC_VERTEX_SHADER(di_aftershock_vs20 );
	
		// Pixel Shader
		if( g_pHardwareConfig->SupportsPixelShaders_2_b() )
		{
			DECLARE_STATIC_PIXEL_SHADER(di_aftershock_ps20b );
			SET_STATIC_PIXEL_SHADER(di_aftershock_ps20b );
		}
		else
		{
			DECLARE_STATIC_PIXEL_SHADER(di_aftershock_ps20 );
			SET_STATIC_PIXEL_SHADER(di_aftershock_ps20 );
		}
	...
	DYNAMIC_STATE
	{
		...
		DECLARE_DYNAMIC_VERTEX_SHADER(di_aftershock_vs20 );
		SET_DYNAMIC_VERTEX_SHADER_COMBO( SKINNING, pShaderAPI->GetCurrentNumBones() > 0 );
		SET_DYNAMIC_VERTEX_SHADER_COMBO( COMPRESSED_VERTS, (int)vertexCompression );
		SET_DYNAMIC_VERTEX_SHADER(di_aftershock_vs20 );
		...
		if( g_pHardwareConfig->SupportsPixelShaders_2_b() )
		{
			DECLARE_DYNAMIC_PIXEL_SHADER(di_aftershock_ps20b );
			SET_DYNAMIC_PIXEL_SHADER(di_aftershock_ps20b );
		}
		else
		{
			DECLARE_DYNAMIC_PIXEL_SHADER(di_aftershock_ps20 );
			SET_DYNAMIC_PIXEL_SHADER(di_aftershock_ps20 );
		}
