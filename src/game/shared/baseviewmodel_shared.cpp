...
#ifdef DARKINTERVAL // some may want to disable the sway.
ConVar v_sway_enabled("v_sway_enabled", "1", FCVAR_ARCHIVE | FCVAR_REPLICATED);
#endif
//-----------------------------------------------------------------------------
// Purpose: 
//-----------------------------------------------------------------------------
...
void CBaseViewModel::CalcViewModelView( CBasePlayer *owner, const Vector& eyePosition, const QAngle& eyeAngles )
{
	...
	// Add lag
#ifdef DARKINTERVAL
	if( v_sway_enabled.GetBool() && m_sway_enabled_boolean)
#endif
	CalcViewModelLag( vmorigin, vmangles, vmangoriginal );

#ifndef DARKINTERVAL // todo: why was this check disabled in DI? because MP?
	#if defined( CLIENT_DLL )
	if ( !prediction->InPrediction() )
	{
...
static void RecvProxy_Weapon( const CRecvProxyData *pData, void *pStruct, void *pOut )
{
	...
	if ( pNewWeapon != pOldWeapon )
	{
		...
		pViewModel->m_flAnimTime = CURTIME;
	}
}
#endif
...
#ifdef DARKINTERVAL
	SendPropInt		(SENDINFO(m_sway_enabled_boolean)),
#endif
#else
...
#ifdef DARKINTERVAL
	RecvPropInt(RECVINFO(m_sway_enabled_boolean)),
#endif
#endif
END_NETWORK_TABLE()
...
#ifdef DARKINTERVAL
	DEFINE_PRED_FIELD(m_sway_enabled_boolean, FIELD_BOOLEAN, FTYPEDESC_INSENDTABLE ),
#endif
