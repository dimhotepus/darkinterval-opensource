...
{
#ifdef DARKINTERVAL // don't spew unless the dev wants it
	if (developer.GetInt() < 1) return false;
#endif
	// Reported recently?
...
#ifdef DARKINTERVAL // todo: purpose of this replacement?
...
#ifdef DARKINTERVAL
// Purpose: attempted workaround for Valve's bullshit. Namely, how they broke fading decals. We're gonna use fading particles instead.
void CBaseEntity::ParticleDecalTrace(trace_t *pTrace, char const *decalName)
{
	Assert(pTrace->m_pEnt);
	QAngle vAngles;
	Vector srcElevated = pTrace->endpos;
	AngleVectors(vAngles, &pTrace->endpos);
	DispatchParticleEffect(decalName, srcElevated, QAngle(0, 90, 0) );
}
#endif
//------------------------------------------------------------------------------
...
#ifdef DARKINTERVAL // better asserts
	AssertMsg(!m_pPhysicsObject, "Entity %s, pos %f %f %f, didn't delete its physics object before doing VPhysicsInitSetup()", m_iClassname, GetAbsOrigin().x, GetAbsOrigin().y, GetAbsOrigin().z );
#else
...
void CBaseEntity::InvalidatePhysicsRecursive( int nChangeFlags )
{
	...
	//
#ifdef DARKINTERVAL // as per comment above, let's try this...
	#ifdef CLIENT_DLL
...
void CBaseEntity::FireBullets( const FireBulletsInfo_t &info )
{
	...
	if( IsPlayer() )
	{
		...
		if( rumbleEffect != RUMBLE_INVALID )
		{
			...
			{
#ifdef DARKINTERVAL
				if( info.m_iShots == sk_plr_num_shotgun_pellets.GetInt() * 2) // fix me, this won't work with DI's own altfire
#else
				if (info.m_iShots == 12)
#endif
				{
	...
	for (int iShot = 0; iShot < info.m_iShots; iShot++)
	{
		...
		if( IsPlayer() && info.m_iShots > 1 && iShot % 2 )
		{
			...
			bool prtalTrc = UTIL_Portal_TraceRay_Bullets(pShootThroughPortal, rayBullet, MASK_SHOT, &traceFilter, &tr);
			if ( prtalTrc == false )//!UTIL_Portal_TraceRay_Bullets( pShootThroughPortal, rayBullet, MASK_SHOT, &traceFilter, &tr ) )
			{
		...
		else
		{
			...
			rayBullet.Init( info.m_vecSrc, vecEnd );
			pShootThroughPortal = UTIL_Portal_FirstAlongRay( rayBullet, fPortalFraction );
			bool prtalTrc2 = UTIL_Portal_TraceRay_Bullets(pShootThroughPortal, rayBullet, MASK_SHOT, &traceFilter, &tr);
			if (prtalTrc2 == false)//!UTIL_Portal_TraceRay_Bullets( pShootThroughPortal, rayBullet, MASK_SHOT, &traceFilter, &tr ) )
			{
				pShootThroughPortal = NULL;
			}
#else
	...
	// bullet's final direction can be changed by passing through a portal
		...
		if (tr.fraction != 1.0)
		{
			...
			//  Make sure to use the currect value if we hit a vehicle the player is currently driving.
			if ( iPlayerDamage )
			{
...
#ifdef DARKINTERVAL
		// DI REVISIT: this is a temp filter used to disable bumpmapped (expensive) splashes
		// caused by helicopter shooting on the water.
		// Instead only player shots can cause those.
		if ( nMinSplashSize > 4 || nMaxSplashSize > 8 )
		{
			DispatchEffect("gunshotsplash_cheap", data);
		}
		else
		{
...
#else // DI doesn't use this stuff. It's also been taken out of all damage related function calls.
...
{
#ifdef DARKINTERVAL // because in DI the player can hit ragdolls and make them bleed, this can determine their blood colour
	...
	for (int i = 0; i < count; i++)
	{
		...
		else if (pSurfaceData->game.material == CHAR_TEX_FLESH || pSurfaceData->game.material == CHAR_TEX_BLOODYFLESH)
			return BLOOD_COLOR_RED;
		else if (pSurfaceData->game.material == CHAR_TEX_ALIENFLESH)
...
#ifdef DARKINTERVAL
		AI_TraceLine( ptr->endpos, ptr->endpos + vecTraceDir * -flTraceDist, MASK_SOLID, this, COLLISION_GROUP_NONE, &Bloodtr);
#else
...
int CBaseEntity::GetWaterType() const
{
	...
	/*
	int out = 0;
	if ( m_nWaterType == WL_WaterType_Water)
	out = WL_WaterType_Water;
	if ( m_nWaterType == WL_WaterType_Slime)
	out = WL_WaterType_Slime;
	if ( m_nWaterType == WL_WaterType_Colloid)
	out = WL_WaterType_Colloid;
	return out;
	*/
#else
	int out = 0;
...
void CBaseEntity::SetWaterType( int nType )
{
	...
	m_nWaterType = nType;
	/*
	if ( nType & WL_WaterType_Water)
		m_nWaterType = WL_WaterType_Water;
	if ( nType & WL_WaterType_Slime)
		m_nWaterType = WL_WaterType_Slime;
	if (nType & WL_WaterType_Colloid)
		m_nWaterType = WL_WaterType_Colloid;
	*/
#endif
}
