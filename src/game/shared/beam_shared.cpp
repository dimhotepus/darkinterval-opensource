...
void CBeam::BeamDamage( trace_t *ptr )
{
	...
	if ( ptr->fraction != 1.0 && ptr->m_pEnt != NULL )
	{
		...
		if ( pHit )
		{
			...
			VectorNormalize( dir );
#ifdef DARKINTERVAL // this makes more sense in DI
			...
			CTakeDamageInfo info( this, this, m_flDamage * (CURTIME - m_flFireTime), nDamageType );
...
bool CBeam::OnPredictedEntityRemove( bool isbeingremoved, C_BaseEntity *predicted )
{
	...
	if ( !beam )
	{
		...
		Assert( 0 );
		// Delete right away since it's fluffed up
		return true;
	}
