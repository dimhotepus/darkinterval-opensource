...
struct Ammo_t 
{
	...
	int					nMaxSplashSize;
#ifdef DARKINTERVAL
	float				flArmorMitigation; // how much does this ammo punch through armour. Various classes either ignore damage below certain mitigation, or scale it down by that factor.
#endif
	int					nFlags;
...
#ifdef DARKINTERVAL
	float				ArmorMitigation(int nAmmoIndex);

	void				AddAmmoType(char const* name, int damageType, int tracerType, int plr_dmg, int npc_dmg, int carry, float physicsForceImpulse, int nFlags, int minSplashSize = 4, int maxSplashSize = 8, float flArmorMitigation = 1.0f);
	void				AddAmmoType(char const* name, int damageType, int tracerType, char const* plr_cvar, char const* npc_var, char const* carry_cvar, float physicsForceImpulse, int nFlags, int minSplashSize = 4, int maxSplashSize = 8, float flArmorMitigation = 1.0f);
#else
...
private:
#ifdef DARKINTERVAL
	bool				AddAmmoType(char const* name, int damageType, int tracerType, int nFlags, int minSplashSize, int maxSplashSize, float flArmorMitigation = 1.0f);
#else
