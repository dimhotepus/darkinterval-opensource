...
// - removed all MP/TF/ECON stuff
// - some naming notation changes
// $NoKeywords: $
//=============================================================================//
#include "cbase.h"
...
#ifdef DARKINTERVAL
ConVar v_mirrorviewmodel("v_mirrorviewmodel", "0");

ConVar item_blink("item_blink", "0", FCVAR_ARCHIVE|FCVAR_REPLICATED); // blinking items, previously an Xbox exclusive feature, is a setting in DI
#endif
CBaseCombatWeapon::CBaseCombatWeapon()
{
	...
	m_nViewModelIndex	= 0;
#ifdef DARKINTERVAL
	m_bFlipViewModel	= v_mirrorviewmodel.GetBool();
#else
...
#if !defined( CLIENT_DLL )
#ifdef DARKINTERVAL // DI allows to enable item blink to spot supplies more easily
...
#ifdef DARKINTERVAL
//-----------------------------------------------------------------------------
// Purpose: allow zoom to be defined by the weapon, because suit zoom and weapon
// zoom have been intertwined in the mod
//-----------------------------------------------------------------------------
int CBaseCombatWeapon::GetZoomType(void) const
{
	return GetWpnData().iZoomType;
}
#endif
//-----------------------------------------------------------------------------
// Purpose: 
...
void CBaseCombatWeapon::DisplayAltFireHudHint()
{
	...
	CFmtStr hint;
#ifdef DARKINTERVAL
	hint.sprintf( "#hudhint_%s", GetClassname() );
#else
	...
	m_flHudHintMinDisplayTime = CURTIME + MIN_HUDHINT_DISPLAY_TIME;
#endif//CLIENT_DLL
}
...
void CBaseCombatWeapon::Equip( CBaseCombatCharacter *pOwner )
{
	...
	m_flNextSecondaryAttack		= CURTIME;
	SetTouch( NULL );
	SetThink( NULL );
#if !defined( CLIENT_DLL )
...
bool CBaseCombatWeapon::ReloadOrSwitchWeapons( void )
{
	...
	if ( !HasAnyAmmo() && m_flNextPrimaryAttack < CURTIME && m_flNextSecondaryAttack < CURTIME )
	{
...
bool CBaseCombatWeapon::DefaultDeploy( char *szViewModel, char *szWeaponModel, int iActivity, char *szAnimExt )
{
	...
	if ( pOwner )
	{
		...
		pOwner->SetNextAttack( CURTIME + SequenceDuration() );
	}
	...
	m_bAltFireHudHintDisplayed = false;
	m_bReloadHudHintDisplayed = false;
	m_flHudHintPollTime = CURTIME + 5.0f;
...
{ 
	...
	else
	{
		...
		SetContextThink( &CBaseCombatWeapon::HideThink, CURTIME + flSequenceDuration, HIDEWEAPON_THINK_CONTEXT );
	}

	// if we were displaying a hud hint, squelch it.
	if (m_flHudHintMinDisplayTime && CURTIME < m_flHudHintMinDisplayTime)
	{
...
void CBaseCombatWeapon::ItemPreFrame( void )
{
...
#ifndef CLIENT_DLL
	{
		...
		// any second.
		if( (!m_bAltFireHudHintDisplayed || !m_bReloadHudHintDisplayed) && CURTIME > m_flHudHintMinDisplayTime && CURTIME > m_flHudHintPollTime && GetOwner() && GetOwner()->IsPlayer() )
		{
			CBasePlayer *pPlayer = (CBasePlayer*)(GetOwner());
#ifndef DARKINTERVAL // hl2 used this logic but DI doesn't think it applies.
...
#endif
				m_flHudHintPollTime = CURTIME + 2.0f;
		}
	}
#endif
}
...
void CBaseCombatWeapon::ItemPostFrame( void )
{
	...
	if ((pOwner->m_nButtons & IN_ATTACK2) && (m_flNextSecondaryAttack <= CURTIME))
	{
		if (UsesSecondaryAmmo() && pOwner->GetAmmoCount(m_iSecondaryAmmoType)<=0 )
		{
			if (m_flNextEmptySoundTime < CURTIME)
			{
				WeaponSound(EMPTY);
				m_flNextSecondaryAttack = m_flNextEmptySoundTime = CURTIME + 0.5;
			}
		}
		else if (pOwner->GetWaterLevel() == 3 && m_can_altfire_underwater_boolean == false)
		{
	...
	if ( !bFired && (pOwner->m_nButtons & IN_ATTACK) && (m_flNextPrimaryAttack <= CURTIME))
	{
...
void CBaseCombatWeapon::HandleFireOnEmpty()
{
	...
	else
	{
		...
		{
			WeaponSound(EMPTY);
			m_flNextEmptySoundTime = CURTIME + 0.5;
		}
		m_bFireOnEmpty = true;
	}
}
...
bool CBaseCombatWeapon::DefaultReload( int iClipSize1, int iClipSize2, int iActivity )
{
	...
	MDLCACHE_CRITICAL_SECTION();
	float flSequenceEndTime = CURTIME + SequenceDuration();
...
{
	return m_reloads_singly_boolean;
}
...
{
	if ( m_reloads_singly_boolean )
	{
		CBasePlayer *pOwner = ToBasePlayer( GetOwner() );
		if ( !pOwner )
			return;

		if ((m_bInReload) && (m_flNextPrimaryAttack <= CURTIME))
		{
...
#ifdef DARKINTERVAL // DI's godmode allows infinite ammo
...
#ifdef DARKINTERVAL // new HEV lines
		pPlayer->SetSuitUpdate("!HEVVOXNEW_AMMODEPLETED", true, 0);
#else
...
#ifdef DARKINTERVAL
/*
void CC_DISplatter(void)
{
	CBasePlayer* pPlayer = UTIL_GetCommandClient();
	if (pPlayer == NULL)
	{
		Msg("No player to splatter\n");
		return;
	}
	CBaseCombatWeapon *pWeapon = pPlayer->GetActiveWeapon();
	if (pWeapon == NULL)
	{
		Msg("No weapon to splatter\n");
		return;
	}

	Vector vSrc = pPlayer->EyeDirection3D() + Vector(4, 16, -24);
	Vector vEnd = vSrc + Vector(0, -8, 0);

	NDebugOverlay::Sphere(pPlayer->GetLocalOrigin(), vec3_angle, 64, 150, 120, 25, 150, true, 1.0f);
	NDebugOverlay::Sphere(vSrc, vec3_angle, 12, 150, 20, 20, 150, true, 1.0f);
	NDebugOverlay::Sphere(vEnd, vec3_angle, 12, 20, 20, 20, 150, true, 1.0f);

	trace_t tr;
	UTIL_TraceLine(vSrc, vEnd, MASK_SHOT_HULL, NULL, COLLISION_GROUP_PLAYER, &tr);
	if (tr.fraction != 1.0)
	{
		NDebugOverlay::Cross3D(vSrc, 4.0, 0, 255, 0, 0, 1.0f);
		Msg("Placing splatter decal\n");
		UTIL_DecalTrace(&tr, "Impact.AlienFlesh");
	}
}
static ConCommand di_splatter("di_splatter", CC_DISplatter, "Splatters the viewmodel with blood");
*/
#endif
//-----------------------------------------------------------------------------
...
bool CBaseCombatWeapon::SetIdealActivity( Activity ideal )
{
	...
	SetWeaponIdleTime( CURTIME + SequenceDuration() );
	return true;
}
...
{
	m_flUnlockTime = CURTIME + lockTime;
...
{
	return ( m_flUnlockTime > CURTIME && m_hLocker != pAsker );
}
...
// Purpose:
//-----------------------------------------------------------------------------
#ifndef DARKINTERVAL // DI doesn't use this stuff. It's also been taken out of all damage related function calls.
...
BEGIN_PREDICTION_DATA( CBaseCombatWeapon )
	...
	DEFINE_FIELD( m_can_fire_underwater_boolean, FIELD_BOOLEAN ),
	DEFINE_FIELD( m_can_altfire_underwater_boolean, FIELD_BOOLEAN ),
	...
	DEFINE_FIELD( m_reloads_singly_boolean, FIELD_BOOLEAN ),	
