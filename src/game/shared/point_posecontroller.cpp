...
void C_PoseController::OnDataChanged( DataUpdateType_t updateType )
{
	...
	if ( m_bOldPoseValueParity != m_bPoseValueParity )
	{
		...
		float fClientPoseValue = m_fCurrentPoseValue + m_PoseTransitionValue.Interp( CURTIME );
...
void C_PoseController::UpdateModulation( void )
{
	...
	case POSECONTROLLER_FMODTYPE_SINE:
		{
			float fCycleTime = m_fFModRate * ( CURTIME + m_fFModTimeOffset );
	...
	case POSECONTROLLER_FMODTYPE_SQUARE:
		{
			float fCycleTime = fabsf( m_fFModRate * 2.0f * ( CURTIME + m_fFModTimeOffset ) );
	...
	case POSECONTROLLER_FMODTYPE_TRIANGLE:
		{
			float fCycleTime = fabsf( m_fFModRate * 4.0f * ( CURTIME + m_fFModTimeOffset ) );
...
void C_PoseController::UpdatePoseCycle( float fCycleAmount )
{
	...
	float fNewPoseValue = m_fCurrentPoseValue + m_PoseTransitionValue.Interp( CURTIME );
