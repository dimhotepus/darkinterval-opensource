...
{
	Animate( m_flSpriteFramerate * (CURTIME - m_flLastTime) );
...
{
	float frametime = CURTIME - m_flLastTime;
...
float CSprite::GetRenderScale( void )
{
	...
	if ( ( m_flScaleTime == 0 ) || ( (m_flScaleTimeStart+m_flScaleTime) < CURTIME ) )
		return m_flSpriteScale;

	//Get our percentage
	float timeDelta = ( CURTIME - m_flScaleTimeStart ) / m_flScaleTime;
...
{
	//See if we're done scaling
	if ( ( m_flBrightnessTime == 0 ) || ( (m_flBrightnessTimeStart+m_flBrightnessTime) < CURTIME ) )
	{
		return m_nBrightness;
	}

	//Get our percentage
	float timeDelta = ( CURTIME - m_flBrightnessTimeStart ) / m_flBrightnessTime;
...
void CSprite::ClientThink( void )
{
	...
	if ( m_nBrightness != m_nDestBrightness )
	{
		...
		m_flBrightnessTimeStart = CURTIME;
	}
}
