...
void PrecacheFileWeaponInfoDatabase( IFileSystem *filesystem, const unsigned char *pICEKey )
{
	...
	if ( manifest->LoadFromFile( filesystem, "scripts/weapon_manifest.txt", "GAME" ) )
	{
		for ( KeyValues *sub = manifest->GetFirstSubKey(); sub != NULL ; sub = sub->GetNextKey() )
		{
			...
			{
			//	ConColorMsg(Color(255,25,255,200),"Extracting Weapon manifest entry\n");
				char fileBase[512];
...
{
//	ConColorMsg(Color(255, 25, 255, 200), "Extracting Weapon data from file for %s\n", MAKE_STRING(szWeaponName));
	if ( !phandle )
	{
		Assert( 0 );
		return false;
...
{
//	ConColorMsg(Color(255, 25, 255, 200), "Performing parsing pass for Weapon %s\n", MAKE_STRING(szWeaponName));
	...
	// Use the console buckets if hud_fastswitch is set to 2.
	ConVarRef hud_fastswitch("hud_fastswitch");
	Assert(hud_fastswitch.IsValid());
...
#ifdef DARKINTERVAL 
	iZoomType = pKeyValuesData->GetInt("zoom_type", ZOOM_SUIT);  // value of 1 coresponds with ZOOM_SUIT in hl2_player.cpp
#endif
}
