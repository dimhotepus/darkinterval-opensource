...
{
#ifdef DARKINTERVAL // god mode lets you have unlimited ammo.
	if (iCount <= 0)
		return;
#endif
...
void CCombatCharVisCache::FrameUpdatePreEntityThink()
{
	...
	for ( int i = 0; i < 8; ++i )
	{
		...
		if ( !entry.m_hEntity1.IsValid() || !entry.m_hEntity2.IsValid() || ( CURTIME - entry.m_flTime > 10.0f ) )
		{
...
int CCombatCharVisCache::LookupVisibility( const CBaseCombatCharacter *pChar1, CBaseCombatCharacter *pChar2 )
{
	...
	if ( iCache == m_VisCache.InvalidIndex() )
	{
		...
		m_VisCache[iCache].m_flTime = CURTIME - 2.0f * VIS_CACHE_ENTRY_LIFE;
	}
...
VisCacheResult_t CCombatCharVisCache::HasVisibility( int iCache ) const
{
	...
	if ( CURTIME - entry.m_flTime > VIS_CACHE_ENTRY_LIFE )
		return VISCACHE_UNKNOWN;
