...
float ApplyViewLocking( float flAngleRaw, float flAngleClamped, ViewLockData_t &lockData, RemapAngleRange_CurvePart_t eCurvePart )
{
		...
		( lockData.bLocked && ( eCurvePart == RemapAngleRange_CurvePart_Spline ) ) )
	{
		...
		lockData.flUnlockTime = CURTIME + lockData.flLockInterval;
		flAngleOut = flAngleRaw;
	}
	else
	{
		if ( ( lockData.bLocked ) && ( CURTIME > lockData.flUnlockTime ) )
		{
			lockData.bLocked = false;
			if ( lockData.flUnlockBlendInterval > 0 )
			{
				lockData.flUnlockTime = CURTIME;
			}
			else
			{
		...
		if ( !lockData.bLocked )
		{
			if ( lockData.flUnlockTime != 0 )
			{
				...
				float flBlend = RemapValClamped( CURTIME - lockData.flUnlockTime, 0, lockData.flUnlockBlendInterval, 0, 1 );
				//Msg( "BLEND %f\n", flBlend );
								...
								float *pFOV )
{
	...
	*pAbsAngles = pPlayer->EyeAngles();
	...
	if ( bRunningAnim && !pData->bWasRunningAnim )
	{
		...
		pData->flEnterExitStartTime = CURTIME;
	...
	if ( bRunningAnim || pData->bRunningEnterExit )
	{
		*pAbsAngles = vehicleEyeAngles;
		...
		frac = ( CURTIME - pData->flEnterExitStartTime ) / pData->flEnterExitDuration;
		frac = clamp( frac, 0.0f, 1.0f );

		flFracFOV = ( CURTIME - pData->flEnterExitStartTime ) / ( pData->flEnterExitDuration * 0.85f );
