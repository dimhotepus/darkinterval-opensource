...
#ifdef DARKINTERVAL 
const char *SURFACEPROP_MANIFEST_FILE = "scripts/physics/surfaceproperties_manifest.txt";
#else
...
void PhysFrictionEffect( Vector &vecPos, Vector vecVel, float energy, int surfaceProps, int surfacePropsHit )
{
	...
	//Metal sparks
#ifdef DARKINTERVAL
	if ( energy > MASS10_SPEED2ENERGY(35) ) // DI wants these effects to happen a bit more often and on more surfaces
#else
...
#endif
	{
		...
		if ( psurf->game.material == CHAR_TEX_METAL 
			|| psurf->game.material == CHAR_TEX_GRATE
#ifdef DARKINTERVAL
			|| psurf->game.material == CHAR_TEX_VENT
			|| psurf->game.material == CHAR_TEX_COMPUTER
			|| psurf->game.material == CHAR_TEX_CONCRETE
#endif
			)
		{	
			switch ( phit->game.material )
...
#ifdef DARKINTERVAL // fix for the incorrect handle
