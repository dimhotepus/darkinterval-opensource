...
int CSingleplayRules::Damage_GetTimeBased( void )
{
	int iDamage = ( 
		...
		| DMG_RADIATION 
		| DMG_DROWNRECOVER 
#ifndef DARKINTERVAL // so that it doesn't get stuck with white flashing
		| DMG_ACID
#endif
		| DMG_SLOWBURN );
	return iDamage;
...
#ifdef DARKINTERVAL // for DI's damage tile icons
		| DMG_SONIC | DMG_FREEZE
#endif
		);
...
#ifdef DARKINTERVAL // so these don't produce decals, otherwise it looks silly
		| DMG_NERVEGAS | DMG_BURN | DMG_SLOWBURN
#endif
		);
	return iDamage;
}
...
bool CSingleplayRules::Damage_ShowOnHUD( int iDmgType )
{
	...
	return ( ( iDmgType & ( DMG_POISON | DMG_ACID | DMG_DROWN | DMG_BURN | DMG_SLOWBURN | DMG_NERVEGAS | DMG_RADIATION | DMG_SHOCK 
#ifdef DARKINTERVAL
		| DMG_SONIC | DMG_FREEZE
#endif
		) ) != 0 );
...
bool CSingleplayRules::Damage_ShouldNotBleed( int iDmgType )
{
	...
	return ( ( iDmgType & ( DMG_POISON | DMG_ACID 
#ifdef DARKINTERVAL
		| DMG_NERVEGAS | DMG_BURN | DMG_SLOWBURN 
#endif
		) ) != 0 );
	...
	bool CSingleplayRules::FShouldSwitchWeapon( CBasePlayer *pPlayer, CBaseCombatWeapon *pWeapon )
	{
		...
		if ( ( pWeapon != pPlayer->GetActiveWeapon() ) && ( pWeapon->GetWeight() <= pPlayer->GetActiveWeapon()->GetWeight() ) )
			return false;
#ifdef DARKINTERVAL
		// always switch to certain scripted weapons if we didn't have them before or they are unique
		if ((pWeapon->ClassMatches("weapon_oicw")) && !pPlayer->Weapon_OwnsThisType("weapon_oicw"))
			return true;
		if ((pWeapon->ClassMatches("weapon_rpg")) && !pPlayer->Weapon_OwnsThisType("weapon_rpg"))
			return true;
		if ((pWeapon->ClassMatches("weapon_immolator")))
			return true;
		if ((pWeapon->ClassMatches("weapon_tau")))
			return true;
		if ((pWeapon->ClassMatches("weapon_guardgun")))
			return true;
		if ((pWeapon->ClassMatches("weapon_physcannon")))
			return true;
		return false; // do not want autoswitching in general
#else
		return true;
#endif
	}
