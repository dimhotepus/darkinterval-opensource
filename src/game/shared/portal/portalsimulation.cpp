...
#include "world.h"
#include "hl2_Player.h" //TODO: Move any portal mod specific code to callback functions or something
...
static ConVar sv_dump_portalsimulator_collision( "sv_dump_portalsimulator_collision", "0", FCVAR_REPLICATED ); //whether to actually dump out the data now that the possibility exists
...
#ifdef PORTAL_COMPILE_SIMULATORS_EMBED_GUID
...
#ifdef DEBUG_PORTAL_COLLISION_ENVIRONMENTS
static ConVar sv_debug_dumpportalhole_nextcheck( "sv_debug_dumpportalhole_nextcheck", "0", FCVAR_REPLICATED );
#endif
	...
	{	
		...
		if( m_pLinkedPortal && m_pLinkedPortal->m_InternalData.Simulation.pPhysicsEnvironment )
		{
			...
			if( pClone )
			{
				...
				CHL2_Player *pPlayer = (CHL2_Player *)GetPlayerHoldingEntity( pEntity );

				if ( !pPlayer && pEntity->IsPlayer() )
				{
					pPlayer = (CHL2_Player *)pEntity;
				}
