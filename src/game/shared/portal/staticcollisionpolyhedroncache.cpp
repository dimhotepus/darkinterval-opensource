...
void CStaticCollisionPolyhedronCache::Update( void )
{
	...
	const size_t workSpaceSize = 4 * 1024 * 1024; //4MB. Fairly arbitrary size for a workspace. Brushes usually use 1-3MB in the end. Static props usually use about half as much as brushes.
	...
	//brushes
	{
		...
		while( enginetrace->GetBrushInfo( iBrush, &Planes, NULL ) )
		{
			...
			if( iPlaneCount > 400 )
			{
				// we'll have to get more memory to transform this brush
			...
			do
			{
				...
				if( pCollide != NULL )
				{
					...
					for( int i = 0; i != pCollide->solidCount; ++i )
					{
						...
						//		AssertMsg( pConvex != NULL, "Conversion from Convex to Polyhedron was unreversable at %f %f %f", 
						//			pTempPolyhedron->Center().x, 
						//			pTempPolyhedron->Center().y, 
						//			pTempPolyhedron->Center().z );
								if( pConvex )
								{
