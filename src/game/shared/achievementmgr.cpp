...
// - removed all TF/MP stuff
// - removed cheat checks
// - removed cheat flags from achievement commands (assume players are only competing with themselves)
// - replaced cc_achievement_debug checks with developer checks
// - removed "HPE" comments, too blocky
//=============================================================================
...
#ifndef DARKINTERVAL // just use developer 1 for this
...
#ifndef DARKINTERVAL // as a singleplayer experience, we don't care if players had cheats on. They're only competing with themselves.
#ifdef CLIENT_DLL
...
void CAchievementMgr::Update( float frametime )
{
	...
	{
		if ( m_vecThinkListeners[i].m_flThinkTime < CURTIME )
		{
			...
			if ( m_vecThinkListeners[i].pAchievement->IsAchieved() || m_vecThinkListeners[i].m_flThinkTime < CURTIME )
			{
...
void CAchievementMgr::AwardAchievement( int iAchievementID )
{
    ...
    // Necessary for sorting achievements by award time
...
{
#ifndef DARKINTERVAL // DI can award achievements without Steam, as it's all internal.
	...
	if (sv_nostats.GetBool())
	{
		...
		const float fNotificationCooldown = 120.0f;
		static float fNextNotification = 0.0f;
		if (CURTIME >= fNextNotification)
		{
			Msg("Achievements and stats disabled: sv_nostats is set.\n");
			fNextNotification = CURTIME + fNotificationCooldown;
		}
	...
	return true;
#endif
#ifndef DARKINTERVAL // again, we don't care if cheats were on. 
...
#ifdef DARKINTERVAL
	bool bVictimIsPlayerEnemy = true;
#else
...
void CAchievementMgr::SetAchievementThink( CBaseAchievement *pAchievement, float flThinkTime )
{
	...
	for ( int i = 0; i < iCount; i++ )
	{
		if ( m_vecThinkListeners[i].pAchievement == pAchievement )
		{
			...
			m_vecThinkListeners[i].m_flThinkTime = CURTIME + flThinkTime;
			return;
		}
	}
	...
	m_vecThinkListeners[iIdx].m_flThinkTime = CURTIME + flThinkTime;
}
