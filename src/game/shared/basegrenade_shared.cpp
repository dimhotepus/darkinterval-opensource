...
void CBaseGrenade::Explode( trace_t *pTrace, int bitsDamageType )
{
	...
	if ( pTrace->fraction != 1.0 )
	{
		...
		*/
		...
		if ((contents & MASK_WATER) == false)
		{
			CEffectData	data;
			data.m_nEntIndex = entindex();
			data.m_vOrigin = GetAbsOrigin();
			data.m_flScale = m_DmgRadius * 2.0f;
			DispatchEffect("ThumperDust", data);
		}
		// todo: can do some refract shockwave particle, or a shockwave bubble when in water
#endif
	}
	else
	{
...
void CBaseGrenade::BounceTouch( CBaseEntity *pOther )
{
	...
	if ( (pOther->m_takedamage != DAMAGE_NO) && (m_flNextAttack < CURTIME && GetAbsVelocity().Length() > 100))
	{
		if (m_hThrower)
		{
#if !defined( CLIENT_DLL )
		...
		m_flNextAttack = CURTIME + 1.0; // debounce
	}
	...
	if ( !m_bHasWarnedAI
#ifndef DARKINTERVAL // makes AI a bit more reactive to grenades, not waiting for them to slow down
		&& vecTestVelocity.Length() <= 60
#endif
		)
	{
...
#if !defined( CLIENT_DLL )
#ifdef DARKINTERVAL // make grenade danger sounds larger
		CSoundEnt::InsertSound ( SOUND_DANGER, GetAbsOrigin(), m_flDamage * 2, 0.3, this );
#else
...
void CBaseGrenade ::TumbleThink( void )
{
	...
	//
	// Emit a danger sound two seconds before exploding.
	//
#ifdef DARKINTERVAL
	if (m_flDetonateTime - 2 < CURTIME)
#else
	if (m_flDetonateTime - 1 < CURTIME)
#endif
	{
#if !defined( CLIENT_DLL )
		CSoundEnt::InsertSound ( SOUND_DANGER, GetAbsOrigin() + GetAbsVelocity() * (m_flDetonateTime - CURTIME), 400, 0.1, this );
#endif
	}

	if (m_flDetonateTime <= CURTIME)
	{
		SetThink( &CBaseGrenade::Detonate );
	}
...
#ifdef DARKINTERVAL // for the shockwave, unused now
//	m_iSpriteTexture = PrecacheModel("custom_uni/shockwave_refract.vmt");
#endif
}
