...
void CSpriteTrail::UpdateTrail( void )
{
	...
	if ( ( pLast == NULL ) || ( pLast->m_vecScreenPos.DistToSqr( screenPos ) > 4.0f ) )
	{
		...
		pNewPoint->m_flDieTime	= CURTIME + m_flLifeTime;
	...
	m_flUpdateTime = CURTIME + ( m_flLifeTime / (float) MAX_SPRITE_TRAIL_POINTS );
}
...
int CSpriteTrail::DrawModel( int flags )
{
	...
	currentPoint.m_flDieTime = CURTIME + m_flLifeTime;
	...
	for ( int i = 0; i <= m_nStepCount; ++i )
	{
		...
		float flLifePerc = (pPoint->m_flDieTime - CURTIME) / m_flLifeTime;
