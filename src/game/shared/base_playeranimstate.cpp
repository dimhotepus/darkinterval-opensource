...
#include "filesystem.h"
	...
	ConVar cl_showanimstate( "cl_showanimstate", "-1", FCVAR_DEVELOPMENTONLY, "Show the (client) animation state for the specified entity (-1 for none)." );
	ConVar showanimstate_log( "cl_showanimstate_log", "0", FCVAR_DEVELOPMENTONLY, "1 to output cl_showanimstate to Msg(). 2 to store in AnimStateClient.log. 3 for both." );
#else
	#include "player.h"
	ConVar sv_showanimstate( "sv_showanimstate", "-1", FCVAR_DEVELOPMENTONLY, "Show the (server) animation state for the specified entity (-1 for none)." );
	ConVar showanimstate_log( "sv_showanimstate_log", "0", FCVAR_DEVELOPMENTONLY, "1 to output sv_showanimstate to Msg(). 2 to store in AnimStateServer.log. 3 for both." );
#endif
...
void CBasePlayerAnimState::ClearAnimationState()
{
	...
	m_bCurrentFeetYawInitialized = false;
	m_flLastAnimationStateClearTime = CURTIME;
}


float CBasePlayerAnimState::TimeSinceLastAnimationStateClear() const
{
	return CURTIME - m_flLastAnimationStateClearTime;
}
...
#ifdef DARKINTERVAL // todo: explain why this was changed?
	return (act == ACT_RUN || act == ACT_WALK || act == ACT_RUN_CROUCH);
#else
...
void CBasePlayerAnimState::ComputePoseParam_MoveYaw( CStudioHdr *pStudioHdr )
{
	...
	{
#ifndef CLIENT_DLL
		// Make the model's angle match the legs so the hitboxes match on both sides.
...
void CBasePlayerAnimState::ComputePoseParam_BodyYaw()
{
...
#ifdef DARKINTERVAL
	else if ( (CURTIME - m_flLastTurnTime) > 3.0 )
#else
	else if ((CURTIME - m_flLastTurnTime) > mp_facefronttime.GetFloat())
#endif
	{
	...
	{
#ifdef DARKINTERVAL
		ConvergeAngles( m_flGoalFeetYaw, 720.0, m_AnimConfig.m_flMaxBodyYawDegrees,
...
void CBasePlayerAnimState::DebugShowAnimState( int iStartLine )
{
	...
	AnimStatePrintf( iLine++, "vel: %.2f, time: %.2f, max: %.2f, animspeed: %.2f", 
		vOuterVel.Length2D(), CURTIME, GetInterpolatedGroundSpeed(), m_pOuter->GetSequenceGroundSpeed(m_pOuter->GetSequence()) );
