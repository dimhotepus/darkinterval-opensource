...
#ifdef DARKINTERVAL // DI allows for custom stepping sounds on ladders
void CFuncLadder::Precache()
{
	PrecacheScriptSound((char *)STRING(m_useladdersound_string));
}
#endif
...
#ifdef DARKINTERVAL // DI uses shortened "ladder_dismount" entity name, for ease of finding in Hammer. Allow both it and info_ladder_dismount
	while ( (pEntity = gEntList.FindEntityByClassnameWithin( pEntity, "*ladder_dismount", origin, radius)) != NULL )
#else
...
#ifdef DARKINTERVAL // DI allows for custom stepping sounds on ladders
	if (m_useladdersound_string != NULL_STRING)
	{
		pPlayer->EmitSound(STRING(m_useladdersound_string));
	}
#else
...
#endif
END_NETWORK_TABLE()
#ifdef DARKINTERVAL // DI uses shortened "ladder_useable" entity name, for ease of finding in Hammer. Allow both it and the old name
#if !defined(CLIENT_DLL)
LINK_ENTITY_TO_CLASS( func_useableladder, CFuncLadder );
LINK_ENTITY_TO_CLASS(ladder_useable, CFuncLadder);
#endif
#else
LINK_ENTITY_TO_CLASS(func_useableladder, CFuncLadder);
#endif // DARKINTERVAL
...
#ifdef DARKINTERVAL
	DEFINE_KEYFIELD(m_useladdersound_string, FIELD_SOUNDNAME, "useladdersound"),
#endif
#if !defined( CLIENT_DLL )
...
LINK_ENTITY_TO_CLASS(ladder_dismount, CInfoLadderDismount);
#endif
#else
LINK_ENTITY_TO_CLASS(info_ladder_dismount, CInfoLadderDismount);
#endif // DARKINTERVAL
