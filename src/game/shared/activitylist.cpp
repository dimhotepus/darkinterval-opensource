...
#ifdef DARKINTERVAL // todo: are these used?
	// Crossbow
...
#ifdef DARKINTERVAL
	REGISTER_SHARED_ACTIVITY(ACT_BUSY_THREAT); // used by critters before player enters hostility radius
#endif
	// Dodge animations
...
#ifdef DARKINTERVAL
	REGISTER_SHARED_ACTIVITY(ACT_IDLE_CARRY2); // used by Eli at the moment - regular carry for the flashlight, carry2 for gravity gun
	REGISTER_SHARED_ACTIVITY(ACT_WALK_CARRY2);
	REGISTER_SHARED_ACTIVITY(ACT_VM_FIZZLE);

	// DI NEW: Live Ragdolls Experiment (inspired by Dark Messiah)
	REGISTER_SHARED_ACTIVITY(ACT_PUSH_RAGDOLL_WAIT_IDLE);
#endif
