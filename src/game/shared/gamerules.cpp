...
#else //}{
#ifndef DARKINTERVAL // unused in DI, as singleplayer
...
void CGameRules::RefreshSkillData ( bool forceUpdate )
{
	...
	SetSkillLevel(skill.IsValid() ? skill.GetInt() : 
#ifdef DARKINTERVAL 
		2
#else
		1
#endif
	); // DI sets difficulty to Medium by default, not Easy
//	Msg("Difficulty currently locked to Medium. Reminder: see gamerules.cpp, line 267.\n");
...
//-----------------------------------------------------------------------------
#ifdef DARKINTERVAL // makes it less cheesy
...
#ifdef DARKINTERVAL // DI allows explosions to do this, if they're the player's. This is so they work against underwater enemies, like hydras.
		if (bInWater && pEntity->GetWaterLevel() == 0 && (!(info.GetAttacker()) || (info.GetAttacker() && !info.GetAttacker()->IsPlayer())))
#else
		if (bInWater && pEntity->GetWaterLevel() == 0)
#endif
			continue;
#ifdef DARKINTERVAL
		if (!bInWater && pEntity->GetWaterLevel() == 3 && (!(info.GetAttacker()) || (info.GetAttacker() && !info.GetAttacker()->IsPlayer())))
#else
...
#ifndef DARKINTERVAL
					if( tr.fraction != 1.0 ) // DI CHANGE: don't allow blocking explosions with thin brushes
					{
						continue;
					}
#endif
...
void CGameRules::Think()
{
	...
	{
		if ( m_flNextVerboseLogOutput < CURTIME )
		{
			ProcessVerboseLogOutput();
			m_flNextVerboseLogOutput = CURTIME + log_verbose_interval.GetFloat();
		}
	}
}
...
bool CGameRules::ShouldCollide( int collisionGroup0, int collisionGroup1 )
{
	...
	//  Interactive Debris doesn't collide with the player.
...
#ifdef DARKINTERVAL
			||	collisionGroup0 == COLLISION_GROUP_PLAYERDEBRIS
#endif
			)
		{
			return false;
...
#endif
	// Don't let player debris collide with the player and vehicles and triggers, 
	// but allow everything else
	if (collisionGroup1 == COLLISION_GROUP_PLAYERDEBRIS)
	{
		if (collisionGroup0 == COLLISION_GROUP_VEHICLE ||
			collisionGroup0 == COLLISION_GROUP_PLAYER)
		{
