...
#ifdef DARKINTERVAL // this is used for secondary viewmodel, which so far is only used for rendertarget HUD model (which itself is unused right now)
	virtual void			SetSwayEnabled(bool enabled) { m_sway_enabled_boolean = enabled; };
#endif
...
#ifdef DARKINTERVAL
	CNetworkVar(bool, m_sway_enabled_boolean); // whether or not to use swaying (also respecting v_sway_enabled convar)
#endif
