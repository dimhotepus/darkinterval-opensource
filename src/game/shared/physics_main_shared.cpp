...
void CBaseEntity::UpdateWaterState()
{
	...
	if( cont & CONTENTS_WATER )
		SetWaterType(WL_WaterType_Water);
	else if( cont & CONTENTS_SLIME )
		SetWaterType(WL_WaterType_Slime);
	else if (cont & CONTENTS_AUX)
		SetWaterType(WL_WaterType_Colloid);
	SetWaterLevel( 1 );
#else
	SetWaterType(cont);
#endif
...
void CBaseEntity::PhysicsCheckWaterTransition( void )
{
	...
	if (GetMoveParent())
		return;
#ifdef DARKINTERVAL
	if (GetSolid() == SOLID_NONE || GetSolidFlags() & FSOLID_NOT_SOLID)
		return; // this way entities like spotlight_end won't try it

	if (GetEffects() & EF_NODRAW)
		return;

	if ( cont > WL_WaterType_None )
#else
	if (cont & MASK_WATER)
#endif
	{
#ifdef DARKINTERVAL
		if ((oldcont == WL_WaterType_None ))
#else
...
bool CBaseEntity::PhysicsRunSpecificThink( int nContextIndex, BASEPTR thinkFunc )
{
	...
	SetLastThink( nContextIndex, CURTIME );
