...
// - removed all CS stuff
// - removed TF/MP stuff such as hwmodels
// - removed fcvar cheat flags from a bunch of convars
// $NoKeywords: $
//=============================================================================//
...
void CBasePlayer::CacheVehicleView( void )
{
	...
	{	
#ifdef DARKINTERVAL
		if (pVehicle->GetVehicleEnt()->m_lifeState != LIFE_ALIVE) return;
#endif
		int nRole = pVehicle->GetPassengerRole( this );
...
#ifdef DARKINTERVAL
void CBasePlayer::PlayJumpSound(bool bLongJump)
{
//	if (gpGlobals->maxClients > 1)
//		return;

	if (bLongJump) EmitSound("ItemLongJump.Boost");
	else EmitSound("NoSuit.Jump");
}
#endif
//-----------------------------------------------------------------------------
// Purpose: 
...
//-----------------------------------------------------------------------------
#ifdef DARKINTERVAL
static ConVar di_enable_footprints("di_enable_footprints", "0");
#endif
...
#ifdef DARKINTERVAL
	if( di_enable_footprints.GetInt())
		m_nImpulse = 800;
#endif
}
...
#ifdef DARKINTERVAL // DI uses this to fire things like crossbow bolts more accurately to the model
Vector CBasePlayer::Weapon_TrueShootPosition_Front( )
{
	Vector	vecEye = EyePosition();
	Vector	vForward, vRight, vUp;

	EyeVectors(&vForward, &vRight, &vUp);
	Vector vecSrc = vecEye + vForward * 16 + vRight * 4 + vUp * -2;
	return vecSrc;
}

Vector CBasePlayer::Weapon_TrueShootPosition_Back()
{
	Vector	vecEye = EyePosition();
	Vector	vForward, vRight, vUp;

	EyeVectors(&vForward, &vRight, &vUp);
	Vector vecSrc = vecEye + vForward * 10 + vRight * 3 + vUp * -1;
	return vecSrc;
}
#endif
...
ConVar	sv_suppress_viewpunch( "sv_suppress_viewpunch", "0", FCVAR_REPLICATED | FCVAR_DEVELOPMENTONLY );
...
void CBasePlayer::CalcViewRoll( QAngle& eyeAngles )
{
	if ( GetMoveType() == MOVETYPE_NOCLIP )
		return;
#ifdef DARKINTERVAL
	float side = CalcRoll( GetAbsAngles(), GetAbsVelocity(), sv_rollangle.GetFloat(), 200.0f );
	eyeAngles[ROLL] += m_lifeState == LIFE_ALIVE ? side : 45; // DI places dead player's camera at an angle, like old games
#else
...
#ifdef DARKINTERVAL // fixes underwater ambient sound not playing
#ifndef CLIENT_DLL
		if ( state )
