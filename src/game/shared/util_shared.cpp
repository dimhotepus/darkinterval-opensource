...
#ifdef DARKINTERVAL
// a workaround for non-fading decals after valve broke the function. 
void UTIL_ParticleDecalTrace(trace_t *pTrace, char const *decalName)
{
	if (pTrace->fraction == 1.0)
		return;

	CBaseEntity *pEntity = pTrace->m_pEnt;
	pEntity->ParticleDecalTrace(pTrace, decalName);
}
#endif
...
// work-around since client header doesn't like inlined CURTIME
