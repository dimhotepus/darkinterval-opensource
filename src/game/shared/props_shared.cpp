...
propdata_interaction_s sPropdataInteractionSections[PROPINTER_NUM_INTERACTIONS] =
{
	...
	{ "physgun_interactions", "onbreak", "explode_fire" },		// PROPINTER_PHYSGUN_BREAK_EXPLODE_IGNITE,
	{ "physgun_interactions", "onbreak", "explode_plasma" },	// PROPINTER_PHYSGUN_BREAK_EXPLODE_PLASMA,
	{ "physgun_interactions", "onbreak", "explode_cryo" },		// PROPINTER_PHYSGUN_BREAK_EXPLODE_CRYO,
	// for plastic bottles hit with bullets
	{ "bullet_interactions", "onfirstimpact", "spray_liquid" },			// PROPINTER_BULLET_BREAK_SPRAY_LIQUID,
#else
...
#ifdef DARKINTERVAL // more control over damage sounds, and strain sounds functionality
	const char *pszBreakableDamageSound;
	if (pBreakableInterface->GetBreakableDamageSound() == NULL_STRING)
	{
		pszBreakableDamageSound = pSection->GetString("breakable_damage_sound", NULL);
	}
	else
	{
		pszBreakableDamageSound = pSection->GetString("breakable_damage_sound", STRING(pBreakableInterface->GetBreakableDamageSound()));
	}
	if (pszBreakableDamageSound && pszBreakableDamageSound[0])
	{
		pBreakableInterface->SetBreakableDamageSound(AllocPooledString(pszBreakableDamageSound));
	}
	else
	{
		pBreakableInterface->SetBreakableDamageSound(NULL_STRING);
	}

	const char *pszStrainSound;
	if (pBreakableInterface->GetStrainSound() == NULL_STRING)
	{
		pszStrainSound = pSection->GetString("strain_sound", NULL);
	}
	else
	{
		pszStrainSound = pSection->GetString("strain_sound", STRING(pBreakableInterface->GetStrainSound()));
	}
	if (pszStrainSound && pszStrainSound[0])
	{
		pBreakableInterface->SetStrainSound(AllocPooledString(pszStrainSound));
	}
	else
	{
		pBreakableInterface->SetStrainSound(NULL_STRING);
	}
#endif // DARKINTERVAL
...
public:
...
#ifdef DARKINTERVAL
		else if (!strcmpi(pKey, "not_debris"))
		{
			pModel->collisionGroup = COLLISION_GROUP_NONE;
			m_wroteCollisionGroup = true;
		}
#endif
...
#ifdef DARKINTERVAL // DI allows specifying a particle effect on break in the model's QC
		else if (!strcmpi(pKey, "emitparticle"))
		{
			Q_strncpy(pModel->particleName, pValue, sizeof(pModel->particleName));
			pModel->shouldEmitParticle = true;
		}
		else if (!strcmpi(pKey, "scale")) // DI allows the breakable chunks to spawn at a different scale 
		{
			pModel->modelScale = atof(pValue);
		}
	}
#endif
	virtual void SetDefaults( void *pData ) 
	{
		...
		pModel->particleName[0] = 0;
		pModel->shouldEmitParticle = false;
#endif
...
#ifdef DARKINTERVAL
	if (iPrecomputedBreakableCount > 1 && !PropBreakableCapEdictsOnCreateAll(modelindex, pPhysics, params, pEntity, iPrecomputedBreakableCount ) )
	{
		Warning( "Failed to create PropBreakable: would exceed MAX_EDICTS!!!\n" );
		return;
	}
#else
...
#ifdef DARKINTERVAL
					QAngle vecAngVel(random->RandomFloat(-list[i].burstScale, list[i].burstScale), 0, 0);
					pBreakable->ApplyAbsVelocityImpulse( vecBurstDir * list[i].burstScale * RandomFloat(0.9, 1.15));
					pBreakable->SetLocalAngularVelocity(vecAngVel);
#else
...
#ifdef DARKINTERVAL
				if (list[i].shouldEmitParticle)
				{
					DispatchParticleEffect(list[i].particleName,
						pBreakable->GetAbsOrigin(),
						pBreakable->GetAbsAngles());
				}

				if ( list[i].modelScale != 1.0)
				{
					CBaseAnimating *breakableAnimating =
						dynamic_cast<CBaseAnimating*>(pBreakable);

				//	AssertMsg(!breakableAnimating, 
				//		"Breakable prop (%f %f %f) can't be translated into C_BaseAnimating!", 
				//		pEntity->GetAbsOrigin().x, pEntity->GetAbsOrigin().y, pEntity->GetAbsOrigin().z);

					breakableAnimating->SetModelScale(list[i].modelScale);
					if (list[i].modelScale < 1.0f)
					{
						// do this because these gib props that were
						// shrinked with small modelscale,
						// tend to have glitchy shadows. 
						// Blown up props also have kinda wrong shadows,
						// but that's easier to deal with.
						pBreakable->AddEFlags(EF_NOSHADOW);
					}
				//	Vector breakmins, breakmaxs;
				//	breakmins = breakableAnimating->WorldAlignMins();
				//	breakmaxs = breakableAnimating->WorldAlignMaxs();
				//	breakmins *= list[i].modelScale;
				//	breakmaxs *= list[i].modelScale;
				//	breakableAnimating->CollisionProp()->SetSurroundingBoundsType(USE_SPECIFIED_BOUNDS, &breakmins, &breakmaxs);
				//	breakableAnimating->VPhysicsUpdate(breakableAnimating->VPhysicsGetObject());
				}
#endif
			}	
		}
...
#ifdef DARKINTERVAL
				breakModel.particleName[0] = 0;
				breakModel.shouldEmitParticle = false;
				breakModel.modelScale = 1.0f;
#endif
...
void PrecacheGibsForModel( int iModel )
{
	...
	while ( !pParse->Finished() )
	{
		...
		if ( !strcmpi( pBlock, "break" ) )
		{
			...
			PrecacheParticleSystem(breakModel.particleName);
#endif
		}
		else
		{
