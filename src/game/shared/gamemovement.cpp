...
// tickcount currently isn't set during prediction, although CURTIME and
...
#define CATEGORIZE_GROUND_SURFACE_TICK_INTERVAL   TICK_INTERVAL //( (int)( CATEGORIZE_GROUND_SURFACE_INTERVAL / TICK_INTERVAL ))

#if 0 //def DARKINTERVAL
...
{
#ifdef DARKINTERVAL // used for the manhack control movement
	Vector vOffset;
	if (player->IsFollowingEntity())// && Q_strcmp(gpGlobals->mapname.ToCStr(), "test_playercontrol") == 0)
		vOffset = vec3_origin;
	else
	{
		if (ducked)
			vOffset = VEC_DUCK_VIEW_SCALED(player);
		else
			vOffset = VEC_VIEW_SCALED(player);
	}
	return vOffset;
#else
...
void CGameMovement::CheckParameters( void )
{
	...
	if ( !IsDead() )
	{
		...
		if ( player->GetMoveType() != MOVETYPE_ISOMETRIC  &&
			 player->GetMoveType() != MOVETYPE_NOCLIP )
		{
#ifdef DARKINTERVAL // reducing amount of convars
			mv->m_vecAngles[ROLL]  = CalcRoll( v_angle, mv->m_vecVelocity, sv_rollangle.GetFloat(), 200.0f );
#else
...
void CGameMovement::ProcessMovement( CBasePlayer *pPlayer, CMoveData *pMove )
{
	...
	//!!HACKHACK: Adrian - slow down all player movement by this factor.
	...
	mv = pMove;
#ifdef DARKINTERVAL // 2004-like movement
	mv->m_flMaxSpeed = sv_maxspeed.GetFloat();
	StartTrackPredictionErrors(player);
#else
...
#ifdef DARKINTERVAL // 2004-like movement
	FinishTrackPredictionErrors(player);
#else
...
#if PREDICTION_ERROR_CHECK_LEVEL > 0
#ifdef DARKINTERVAL
	StartCommand( CBaseEntity::IsServer(), player->m_nTickBase );
#else
...
void CGameMovement::CheckWaterJump( void )
{
	...
	if (player->m_flWaterJumpTime)
		return;
#ifdef DARKINTERVAL // this should be preventing players from insta-jumping upon landing but it doesn't, todo
...
void CGameMovement::WaterMove( void )
{
	...
	{
#ifdef DARKINTERVAL // reducing amount of convars
		newspeed = speed - gpGlobals->frametime * speed * 4.0f * player->m_surfaceFriction;
#else
...
#ifdef DARKINTERVAL // reducing amount of convars
			accelspeed = 10.0f * wishspeed * gpGlobals->frametime * player->m_surfaceFriction;	
#else
...
#endif // DARKINTERVAL
#ifdef DARKINTERVAL // reducing amount of convars
		friction = 4.0f * player->m_surfaceFriction; 
#else
		friction = sv_friction.GetFloat() * player->m_surfaceFriction;
#endif

#ifdef DARKINTERVAL // colloid water type experiment, got cancelled
	//	if (player->GetWaterType() == WL_WaterType_Colloid)
	//		friction *= 0.25f;
#endif
...
#ifdef DARKINTERVAL	// unpress jump in case player is holding it down while falling into water
	mv->m_nOldButtons &= ~IN_JUMP; // forcibly unpress jump
#endif
	//
...
#ifdef DARKINTERVAL // reducing amount of convars
	AirAccelerate(wishdir, wishspeed, 50.0f );
#else
...
#ifdef DARKINTERVAL // reducing amount of convars
	Accelerate ( wishdir, wishspeed, 10.0f ); 
#else
...
void CGameMovement::FullWalkMove( )
{
...
#ifdef DARKINTERVAL // colloid water type experiment, got cancelled
	if (player->GetWaterLevel() < WL_Waist || player->GetWaterType() == WL_WaterType_Colloid)
	{
		if (player->GetWaterType() < WL_WaterType_Colloid)
		{
#endif
		...
		if (!CheckWater() 
#ifdef DARKINTERVAL // colloid water type experiment, got cancelled
			|| player->GetWaterType() == WL_WaterType_Colloid
#endif
			)
		{
			FinishGravity();
	...
	{
#ifdef DARKINTERVAL // colloid water type experiment, got cancelled
		player->GetWaterType() != WL_WaterType_Colloid ? PlaySwimSound() : PlayColloidSound();
#else
		PlaySwimSound();
#endif
#if !defined( CLIENT_DLL )
...
#ifndef DARKINTERVAL // the following doesn't matter for DI
...
#ifdef DARKINTERVAL // reducing amount of convars
		float friction = 4.0f * player->m_surfaceFriction;
#else
...
#ifdef DARKINTERVAL // colloid water type experiment, got cancelled
void CGameMovement::PlayColloidSound()
{
	MoveHelper()->StartSound(mv->GetAbsOrigin(), "Player.ColloidMovement");
}
#endif
//-----------------------------------------------------------------------------
// Purpose: 
//-----------------------------------------------------------------------------
#ifdef DARKINTERVAL // item_longjump
#define PLAYER_LONGJUMP_SPEED 350.0f
#endif
bool CGameMovement::CheckJumpButton( void )
{
#ifdef DARKINTERVAL	// No jump spamming.
	if (player->m_Local.m_flJumpTime > 0.0f)
		return false;
#endif
	if (player->pl.deadflag)
	{
...
#ifdef DARKINTERVAL // todo: since it doesn't prevent bouncing off water, maybe disable it since 2004 bhopping has been brought back?
//	if (player->GetAbsVelocity().z < 0 /*&& player->GetWaterLevel() < WL_Waist*/) 
//	{
//		mv->m_nOldButtons |= IN_JUMP;	// don't jump while moving down (e. g. in water)
//		return false; 
//	}
#endif
	...
	{	
...
#ifdef DARKINTERVAL // colloid water type experiment, got cancelled
		if (player->GetWaterType() == WL_WaterType_Colloid)// DI_QUICKSAND
		{
			mv->m_vecVelocity[2] = -100;
		}
#endif

		if (player->GetWaterType() == WL_WaterType_Water)    // We move up a certain amount
#ifdef DARKINTERVAL
		...
		else if (player->GetWaterType() == WL_WaterType_Slime)
#ifdef DARKINTERVAL
			mv->m_vecVelocity[2] = 30;
#else
    ...
    SetGroundEntity( NULL );
#ifdef DARKINTERVAL	
	if (player->IsSuitEquipped() == false)
	{
	...
	if ( (  player->m_Local.m_bDucking ) || (  player->GetFlags() & FL_DUCKING ) )
	{
		...
		// v = sqrt( g * 2.0 * 45 )
#ifdef DARKINTERVAL // item_longjump
		if (player->IsLongJumpEquipped() &&	(mv->m_nButtons & IN_DUCK) && (player->m_Local.m_flDucktime > 0) &&
			mv->m_vecVelocity.Length() > 50 )
		{
			...
			mv->m_vecVelocity.z = sqrt(2 * 600 * 56.0);
		}
		else
#endif
...
#ifdef DARKINTERVAL // 2004-like movement
	// DI change. Bring back the old easy bunnyhopping
	...
	for (bumpcount=0 ; bumpcount < numbumps; bumpcount++)
	{
			...
			player->GetGroundEntity() == NULL )	
		{
			for ( i = 0; i < numplanes; i++ )
			{
				...
				{
#ifdef DARKINTERVAL // reducing amount of convars
					ClipVelocity( original_velocity, planes[i], new_velocity, 1.0 * (1 - player->m_surfaceFriction) );
#else
...
bool CGameMovement::LadderMove( void )
{
	...
	{
#ifdef DARKINTERVAL
		if (player->GetGroundEntity() == NULL ) //GetFlags() != FL_ONGROUND)
		{
			return false;
		}
#endif
	...
	if ( mv->m_nButtons & IN_JUMP )
	{
		...
		player->SetMoveCollide( MOVECOLLIDE_DEFAULT );
#ifdef DARKINTERVAL
		VectorScale( pm.plane.normal, 100, mv->m_vecVelocity );
#else
...
int CGameMovement::ClipVelocity( Vector& in, Vector& normal, Vector& out, float overbounce )
{
	...
	for (i=0 ; i<3 ; i++)
	{
		...
		out[i] = in[i] - change;
#ifdef DARKINTERVAL // 2004-like movement. Problematic, take out this part
		// If out velocity is too small, zero it out.
	//	if (out[i] > -STOP_EPSILON && out[i] < STOP_EPSILON)
	//		out[i] = 0;
#endif
	}
#if 1 //ndef DARKINTERVAL
...
//	bool	colloid;
...
#ifdef DARKINTERVAL // more obvious logic, same result
	player->SetWaterType(WL_WaterType_None);
#else
	...
	{
#ifdef DARKINTERVAL // colloid water type experiment, got cancelled
		if (cont & CONTENTS_AUX)
		{
		//	Msg("Standing in colloid\n");
			player->SetWaterType(WL_WaterType_Colloid);
		}
		else if (cont & CONTENTS_WATER)
		{
		//	Msg("Standing in water\n");
			player->SetWaterType(WL_WaterType_Water);
		}
		else if (cont & CONTENTS_SLIME)
		{
		//	Msg("Standing in slime\n");
			player->SetWaterType(WL_WaterType_Slime);
		}
#else
...
#ifdef DARKINTERVAL
		if ((cont & CONTENTS_WATER) || (cont & CONTENTS_SLIME) || (cont & CONTENTS_AUX))
#else
		if (cont & MASK_WATER)
#endif
		{
			...
			if ((cont & CONTENTS_WATER) || (cont & CONTENTS_SLIME) || (cont & CONTENTS_AUX))
			{
				...
				if (cont & CONTENTS_AUX)
					Msg("Player fully submerged in quicksand - no way out anymore!\n");
			}
#else
			if (cont & MASK_WATER)
				player->SetWaterLevel(WL_Eyes);  // In over our eyes
...
#ifdef DARKINTERVAL
/*
		point[2] = mv->GetAbsOrigin()[2] + 128;
		NDebugOverlay::Cross3D(point, 8, 255, 0, 0, false, 0.1f);
		NDebugOverlay::Cross3D(point + Vector(0, 0, -256), 8, 0, 255, 0, false, 0.1f);

		trace_t tr;
		UTIL_TraceLine(point, point + Vector(0, 0, -256), MASK_WATER, NULL, 0, &tr);

		if (tr.DidHit())
			NDebugOverlay::Cross3D(tr.endpos, 4, 200, 200, 0, false, 0.1f);
		
#ifdef CLIENT_DLL

		Vector			diffuseColor, baseColor;
		IMaterial*		pTraceMaterial = engine->TraceLineMaterialAndLighting(tr.startpos, tr.endpos, diffuseColor, baseColor);
		
		bool			found;
		IMaterialVar	*pColloidVar = pTraceMaterial->FindVar("$iscolloid", &found, false);

		if (pColloidVar->GetIntValue() != 0)
		{
			Msg("Found colloid water\n");
		}
		else
			Msg("Found regular water\n");
#endif
*/
#endif
...
//-----------------------------------------------------------------------------
#ifdef DARKINTERVAL
ConVar test_ice_surfing("test_ice_surfing", "0", FCVAR_REPLICATED);
#endif
	...
	else
	{
...
#endif
		{
...
#endif
			{
				...
				{
#ifdef DARKINTERVAL
					// DI change: FIX the oversight when the player can safely
					// descend from any steep surface
					// or slowly climb very sharp inclines.
					// 0.25 friction?! that's the same as just not even limiting the player!
					player->m_surfaceFriction = 0.05f;
#else
...
void CGameMovement::CheckFalling( void )
{
	...
	if ( !IsDead() && player->m_Local.m_flFallVelocity >= PLAYER_FALL_PUNCH_THRESHOLD )
	{
		...
		{
			// They landed in water. // No more safe landing in 2-unit layer of water
		}	// See also CheckWaterJump for preventing escape from certain death when falling on water.
		else
#endif
		{
...
#ifdef DARKINTERVAL // reducing amount of convars
			FullNoClipMove( sv_noclipspeed.GetFloat(), 5.0f );
#else
...
/*
#ifndef PORTAL_COMPILE	// Portal inherits from this but needs to declare it's own global interface
