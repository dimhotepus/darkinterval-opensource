...
END_PREDICTION_DATA()
#ifdef DARKINTERVAL
ConVar sk_auto_reload_enabled("sk_auto_reload_enabled", "1", FCVAR_ARCHIVE);
#endif
		...
		if ( pPlayer->IsWeaponLowered() )
		{
			if ( SelectWeightedSequence( ACT_VM_IDLE_LOWERED ) != ACTIVITY_NOT_AVAILABLE )
			{
				if ( DefaultDeploy( (char*)GetViewModel(), (char*)GetWorldModel(), ACT_VM_IDLE_LOWERED, (char*)GetAnimPrefix() ) )
				{
					...
					pPlayer->SetNextAttack( CURTIME + 1.0 );
					m_flNextPrimaryAttack = CURTIME + 1.0;
					m_flNextSecondaryAttack	= CURTIME + 1.0;
					return true;
				}
			}
		}
...
#if !defined( CLIENT_DLL )
//-----------------------------------------------------------------------------
// Purpose: Simulate a physical blast of the muzzle event
// Input : fMagnitude - the strength of the phys explosion, fRadius - radius to search ents in
//-----------------------------------------------------------------------------
void CBaseHLCombatWeapon::DoMuzzlePhysExplosion(const float fMagnitude, const float fRadius)
{
	...
	QAngle dummyAngles;
	GetAttachment(LookupAttachment("muzzle"), vecOrigin, dummyAngles);

	falloff = 1.0 / 2.5;

	while ((pEntity = gEntList.FindEntityInSphere(pEntity, vecOrigin, fRadius)) != NULL)
	{
		...
		if (pEntity->m_takedamage != DAMAGE_NO && (pEntity->GetMoveType() == MOVETYPE_VPHYSICS || (pEntity->VPhysicsGetObject() && !pEntity->IsPlayer())))
		{
			...
			if (fRadius == 0 || flDist <= fRadius)
			{
				adjustedDamage = flDist * falloff;
				adjustedDamage = fMagnitude - adjustedDamage;
...
float CBaseHLCombatWeapon::CalcViewmodelBob( void )
{
	...
	bobtime += ( CURTIME - lastbobtime ) * bob_offset;
	lastbobtime = CURTIME;
...
const WeaponProficiencyInfo_t *CBaseHLCombatWeapon::GetDefaultProficiencyValues()
{
	...
	{
#ifdef DARKINTERVAL
		{ 1.60, 1.0 },//	{ 2.50, 1.0	},	// = WEAPON_PROFICIENCY_POOR
		{ 1.30, 1.0 },//	{ 2.00, 1.0	},	// = WEAPON_PROFICIENCY_AVERAGE
		{ 1.00, 1.0 },//	{ 1.50, 1.0	},	// = WEAPON_PROFICIENCY_GOOD
		{ 0.625, 1.0 },//	{ 1.25, 1.0 },	// = WEAPON_PROFICIENCY_VERY_GOOD
		{ 0.35, 1.0 },//	{ 1.00, 1.0	},	// = WEAPON_PROFICIENCY_PERFECT
#else
		{ 2.50, 1.0 },
		{ 2.00, 1.0 },
		{ 1.50, 1.0 },
