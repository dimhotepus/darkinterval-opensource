...
bool CHL2GameMovement::ContinueForcedMove()
{
	...
	float frac = ( CURTIME - lm->m_flStartTime ) / ( lm->m_flArrivalTime - lm->m_flStartTime );
	if ( frac > 1.0f )
	{
...
void CHL2GameMovement::SetGroundEntity( trace_t *pm )
{
	...
	// Special case for combine balls.
