...
public:
	...
	{
		m_next = CURTIME + interval;
	}

	void Set( float minInterval, float maxInterval )
	{ 
		if ( maxInterval > 0.0 )
			m_next = CURTIME + random->RandomFloat( minInterval, maxInterval );
		else
			m_next = CURTIME + minInterval;
	}

	float GetRemaining() const
	{
		float result = m_next - CURTIME;
		if (result < 0 )
			return 0;
		return result;
	}
...
public:
	...
	{ 
		m_interval = interval;
		m_next = (startExpired) ? -1.0 : CURTIME + m_interval;
	}
...
public:
	...
	{ 
		...
		{
			if ( m_maxInterval == 0 )
				m_next = CURTIME + m_minInterval;
			else
				m_next = CURTIME + random->RandomFloat( m_minInterval, m_maxInterval );
		}
	}
...
public:
	...
	{ 
		m_fIsRunning = true;
		m_next = CURTIME + intervalOverride;
	}
...
public:
	...
	{ 
		m_fIsRunning = true;
		if ( maxOverride == 0 )
			m_next = CURTIME + minOverride;
		else
			m_next = CURTIME + random->RandomFloat( minOverride, maxOverride );
	}
