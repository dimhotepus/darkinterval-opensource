...
#ifdef DARKINTERVAL // for when we want stackable small props (debris clip through each other) that don't make players trip over them
#define SF_PHYSPROP_NO_PLAYER_COLLISION			0x800000		// Behaves like debris to player, like non debris to everything else
#endif
...
enum propdata_interactions_t
{
	...
	PROPINTER_PHYSGUN_BREAK_EXPLODE_IGNITE,	// "onbreak"		"explode_fire"
	PROPINTER_PHYSGUN_BREAK_EXPLODE_PLASMA,	// "onbreak"		"explode_plasma"
	PROPINTER_PHYSGUN_BREAK_EXPLODE_CRYO,	// "onbreak"		"explode_cryo"	
	PROPINTER_BULLET_BREAK_SPRAY_LIQUID, // for plastic bottles hit with bullets	// "onbreak"		"spray_liquid"
#else
...
public:
	...
	virtual void		SetBreakableDamageSound(string_t iszModel) = 0;
	virtual string_t	GetBreakableDamageSound(void) = 0;
	virtual void		SetStrainSound(string_t iszModel) = 0;
	virtual string_t	GetStrainSound(void) = 0;
#endif
	// LOS blocking
...
struct breakmodel_t
{
...
#ifdef DARKINTERVAL
	char		particleName[512]; // DI allows specifying a particle effect on break in the model's QC
#endif
	float		fadeTime;
	float		fadeMinDist;
	float		fadeMaxDist;
	...
	float		burstScale;
#ifdef DARKINTERVAL
	float		modelScale; // DI allows the breakable chunks to spawn at a different scale 
#endif
	int			collisionGroup;
	bool		isRagdoll;
...
#ifdef DARKINTERVAL
	bool		shouldEmitParticle; // DI allows specifying a particle effect on break in the model's QC
#endif
