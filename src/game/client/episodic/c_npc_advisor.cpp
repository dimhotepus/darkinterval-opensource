...
#define ADVISOR_ELIGHT_CVARS 0  // enable/disable tuning advisor elight with console variables
...
void C_NPC_Advisor::StartBeamFX( C_BaseEntity *pOnEntity )
{
	...
	if (!pOnEntity)
		return;
#ifdef DARKINTERVAL
	CNewParticleEffect *pEffect = ParticleProp()->Create( "Advisor_Psychic_Beam", PATTACH_POINT_FOLLOW, "beam_attach" );
#else
	...
	if (!pEffect) return;
#ifdef DARKINTERVAL
	ParticleProp()->AddControlPoint( pEffect, 1, pOnEntity, PATTACH_POINT_FOLLOW, "beam_attach");
#else
...
void C_NPC_Advisor::StartElight()
{
	...
	else
	{
		...
		if ( el )
		{
			...
			el->radius	= 52*12;
			el->decay	= 0.0f;
			el->die = CURTIME + 2000.0f; // 1000 just means " a long time "
