...
int C_HopwireExplosion::DrawModel( int flags )
{
	...
	float refract = m_FXCoreAlpha.Interp( CURTIME );
	float scale = m_FXCoreScale.Interp( CURTIME );
	...
	float sin1 = sinf( CURTIME * 10 );
	float sin2 = sinf( CURTIME );
...
#ifdef DARKINTERVAL
// client-side of env_gravity_vortex, to tie old effects into the new entity
class C_EnvGravityVortex : public C_BaseAnimating
{
	DECLARE_CLASS(C_EnvGravityVortex, C_BaseAnimating);
	DECLARE_CLIENTCLASS();
public:
	C_EnvGravityVortex(void);
	virtual int		DrawModel(int flags);
	virtual void	OnDataChanged(DataUpdateType_t updateType);
	virtual void	ReceiveMessage(int classID, bf_read &msg);
	virtual void	ClientThink(void);
private:
	C_HopwireExplosion	m_ExplosionEffect;
};

IMPLEMENT_CLIENTCLASS_DT(C_EnvGravityVortex, DT_EnvGravityVortex, CEnvGravityVortex)
END_RECV_TABLE()

C_EnvGravityVortex::C_EnvGravityVortex(void)
{
	m_ExplosionEffect.SetActive(false);
}

void C_EnvGravityVortex::ReceiveMessage(int classID, bf_read &msg)
{
	if ( classID != GetClientClass()->m_ClassID )
	{
		// Message is for subclass
...
void C_EnvGravityVortex::ClientThink(void)
{
	BaseClass::ClientThink();
}

void C_EnvGravityVortex::OnDataChanged(DataUpdateType_t updateType)
{
	BaseClass::OnDataChanged(updateType);
...
int	C_EnvGravityVortex::DrawModel(int flags)
{
	if ( m_ExplosionEffect.IsActive() )
		return 1;
