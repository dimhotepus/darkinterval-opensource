...
void CMaterialModifyProxy::OnBindFloatLerp( C_MaterialModifyControl *pControl )
{
	...
	if ( pControl->HasNewAnimationCommands() )
	{
		pControl->SetAnimationStartTime( CURTIME );
		pControl->ClearAnimationCommands();
	}
	...
	if( bFound )
	{
		...
		{
			currentValue = m_flStartValue + ( m_flEndValue - m_flStartValue ) * clamp( ( ( CURTIME - m_flStartTime ) / m_flTransitionTime ), 0.0f, 1.0f );
		}
		else
		{
