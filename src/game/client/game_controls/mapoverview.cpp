...
void CMapOverview::OnThink( void )
{
	...
	{
		Update();
		m_fNextUpdateTime = CURTIME + 0.2f; // update 5 times a second
	}
}

bool CMapOverview::NeedsUpdate( void )
{
	return m_fNextUpdateTime < CURTIME;
}
...
void CMapOverview::Update( void )
{
	// update settings
#ifdef DARKINTERVAL 
	m_bShowNames = ( GetMode() != MAP_MODE_RADAR );
	m_bShowHealth = ( GetMode() != MAP_MODE_RADAR );
	m_bFollowAngle = ( GetMode() == MAP_MODE_RADAR && !IsRadarLocked() );
	m_fTrailUpdateInterval = ( GetMode() != MAP_MODE_RADAR );
#else
...
#ifdef DARKINTERVAL 
	int alpha = 255.0f; clamp(alpha, 1, 255);
#else
...
int	CMapOverview::AddObject( const char *icon, int entity, float timeToLive )
{
	...
	if ( timeToLive > 0 )
		obj.endtime = CURTIME + timeToLive;
	else
		obj.endtime = -1;
...
void CMapOverview::UpdateObjects()
{
	for ( int i = 0; i < m_Objects.Count(); i++ )
	{
		...
		if ( obj->endtime > 0 && obj->endtime < CURTIME )
		{
			m_Objects.Remove( i );
			i--;
			continue;
