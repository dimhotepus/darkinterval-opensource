...
#ifdef DARKINTERVAL
//#include "renderparm.h" // for relaying entity data to shaders, such as the wind strength 
...
#endif
#ifdef DARKINTERVAL // replicated skybox drawing to be able to a) fix the high FOV corner culling of the skybox, 
					// b) dynamically change sky texture w/o restarting
...
static ConVar r_farz( "r_farz", "50000", FCVAR_NONE, "Override the far clipping plane. -1 means to use the value in env_fog_controller." );
...
// DI todo: investigate if we can have custom textured cursor this way
...
{
	if ( m_PitchDrift.laststop == CURTIME )
	{
...
void CViewRender::SetUpOverView()
{
	...
	// render->DrawTopView( true );
					...
					// todo: replace with written-from-scratch new code instead of copying engine code
//static ConVar sv_skybox2_farz("r_skybox_farz", "30000", FCVAR_REPLICATED);
ConVar r_drawskybox2("r_drawskybox2", "1", FCVAR_REPLICATED);
void CViewRender::DrawSecondarySkybox(void)
{
	//ConVarRef drawsky("r_drawskybox");
	//if (drawsky.IsValid())
	//{
	//	if (drawsky.GetBool()) drawsky.SetValue(!r_drawskybox2.GetBool());
	//}

	if (!r_drawskybox2.GetBool()) return;
#if 0
//	if (!GetPlayerViewSetup()) return;

	// draw the sky cube, one face at a time

//	ConVarRef skyname("sv_skyname");
//	char *skytex;
//	Q_snprintf(skytex, 64, skyname.GetString());
//	Q_strcat(skytex, "_bk", 3);
	...
	float skydist = 500; // same as default r_farz
		
	Vector point;
	meshBuilder.TexCoord2f(0, 0, 1);
	point = center + Vector(-skydist, -skydist, 0);
	...
	{
		skies[s] = NULL;
		Q_snprintf(name, sizeof(name), "skybox/%s%s", skyname, skyboxsuffix[s]);
	...
	for (int j = 0; j < 6; j++)
	{
		...
		if (skyboxMaterials[j])
		{
			skyboxMaterials[j]->DecrementReferenceCount();
			skyboxMaterials[j] = NULL;
		}

		// Use the new one
		assert(skies[j]);
		skyboxMaterials[j] = skies[j];
		skyboxMaterials[j]->IncrementReferenceCount();

		// Set the new skybox to not use Z filtering
		skyboxMaterials[j]->SetMaterialVarFlag(MATERIAL_VAR_IGNOREZ, true);
	}
	...
	for (i = 0; i < 6; i++)
	{
		...
		if (skyboxMaterials[skytexorder[i]])
		{
			...
			MakeSkyVec(-1.0f, -1.0f, i, 30000 /*sv_skybox2_farz.GetFloat()*/, positionArray[0], texCoordArray[0]);
			MakeSkyVec(-1.0f, 1.0f, i, 30000 /*sv_skybox2_farz.GetFloat()*/, positionArray[1], texCoordArray[1]);
			MakeSkyVec(1.0f, 1.0f, i, 30000 /*sv_skybox2_farz.GetFloat()*/, positionArray[2], texCoordArray[2]);
			MakeSkyVec(1.0f, -1.0f, i, 30000 /*sv_skybox2_farz.GetFloat()*/, positionArray[3], texCoordArray[3]);
...
void CViewRender::MakeSkyVec(float s, float t, int axis, float zFar, Vector& position, Vector2D &texCoord)
{
	if (!GetPlayerViewSetup()) return;
	...
	Vector viewPoint = GetPlayerViewSetup()->origin;
	//	engine->Con_NPrintf(0, "Secondary skybox viewPoint pos is %.1f %.1f %.1f\n", viewPoint[0], viewPoint[1], viewPoint[1]);
