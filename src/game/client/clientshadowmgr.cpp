...
ConVar r_flashlight_optimised( "r_flashlight_optimised", "0" );

ConVar r_flashlight_resolution( "r_flashlight_resolution", "1024" );

ConVar r_flashlightdepthtexture("r_flashlightdepthtexture", "1");

ConVar r_threaded_client_shadow_manager( "r_threaded_client_shadow_manager", "0" );

static ConVar r_shadows_rtt_edge_fixup("r_shadows_rtt_edge_fixup", "3.0", 0, "Shifts the designated shadow areas for entities to avoid blending\nwith neighbouring shadows on the shared shadow map.\nOld values was 1 pixel, but increased shadowmap resolution needs a larger fixup.\nValues of 3 to 4 seem good.");

static ConVar r_shadowmap_total_resolution("r_shadowmap_total_resolution", "1024", FCVAR_ARCHIVE);
...
#define FLASHLIGHT_SHADOWS_ENABLED	1
//-----------------------------------------------------------------------------
...
private:
	...
	enum
	{
		...
		MAX_TEXTURE_POWER    	= 11, //8
		MIN_TEXTURE_POWER	    = 7, //4
...
void CTextureAllocator::Init()
{
	...
	m_TexturePage.InitRenderTarget( TEXTURE_PAGE_SIZE, TEXTURE_PAGE_SIZE, 
		RT_SIZE_NO_CHANGE, 
		IMAGE_FORMAT_ARGB8888, 
		MATERIAL_RT_DEPTH_NONE, false, "_rt_Shadows" );
}
...
{
	m_nDepthTextureResolution = r_flashlight_resolution.GetInt();
	m_bThreaded = false;
}
...
//	bool bTools = CommandLine()->CheckParm( "-tools" ) != NULL;
	m_nMaxDepthTextureShadows = 6;	// DI change: more projected textures 
	...
	{
		Msg("Setting r_flashlightdepthtexture to 0 due to low settings\n");
	...
	if ( !bLowEnd && r_flashlightdepthtexture.GetBool())
	{
		...
		Msg("Initialised flashlight shadows\n");
	}
#endif
...
void CClientShadowMgr::InitDepthTextureShadows()
{
	...
	if( !m_bDepthTextureActive )
	{
		...
		m_DummyColorTexture.InitRenderTarget(m_nDepthTextureResolution, m_nDepthTextureResolution, 
			RT_SIZE_NO_CHANGE, nullFormat, MATERIAL_RT_DEPTH_NONE, false, "_rt_ShadowDummy");
#else
		m_DummyColorTexture.InitRenderTarget(r_flashlight_resolution.GetInt(),
			r_flashlight_resolution.GetInt(),
			RT_SIZE_OFFSCREEN, nullFormat, MATERIAL_RT_DEPTH_NONE,
		...
		for( int i=0; i < m_nMaxDepthTextureShadows; i++ )
		{
...
#ifdef DARKINTERVAL // from Mapbase
			depthTex.InitRenderTarget(m_nDepthTextureResolution, m_nDepthTextureResolution, 
				RT_SIZE_NO_CHANGE, dstFormat, MATERIAL_RT_DEPTH_NONE, false, strRTName);
			...
			if ( i == 0 )
			{
				...
				r_flashlight_resolution.SetValue( m_nDepthTextureResolution );
			}
...
void CClientShadowMgr::LevelInitPostEntity()
{
	Init();
}
...
void CClientShadowMgr::BuildFlashlight( ClientShadowHandle_t handle )
{
	...
	ClientShadow_t &shadow = m_Shadows[handle];
	if ( r_flashlight_optimised.GetInt() )
	{
...
void CClientShadowMgr::PreRender()
{
...

	{
		...
		if ( r_flashlightdepthtexture.GetBool() && !materials->SupportsShadowDepthTextures() )
		{
			Msg("Setting r_flashlightdepthtexture to 0\n");
			r_flashlightdepthtexture.SetValue(0);
			ShutdownDepthTextureShadows();	
		}

		bool bDepthTextureActive     = r_flashlightdepthtexture.GetBool();
		int  nDepthTextureResolution = r_flashlight_resolution.GetInt();
	...
	bool bRenderToTextureActive = r_shadowrendertotexture.GetBool() && !(GetClientWorldEntity()->m_bForceCheapShadows);
...
void CClientShadowMgr::SetRenderToTextureShadowTexCoords( ShadowHandle_t handle, int x, int y, int w, int h )
{
	...
	u  = ((float)x + (r_shadows_rtt_edge_fixup.GetInt() / 2)) / (float)textureW;
	v  = ((float)y + (r_shadows_rtt_edge_fixup.GetInt() / 2)) / (float)textureH;
	du = ((float)w - r_shadows_rtt_edge_fixup.GetInt()) / (float)textureW;
	dv = ((float)h - r_shadows_rtt_edge_fixup.GetInt()) / (float)textureH;
...
bool CClientShadowMgr::DrawRenderToTextureShadow( unsigned short clientShadowHandle, float flArea )
{
	...
	if ( bNeedsRedraw || bDirtyTexture )
	{
		...
		else
		{
			...
			DevMsg( "Didn't draw shadow hierarchy, could mean bad shadow texcoords. Revisit. ClientShadowMgr.\n" );
		}
...
int CClientShadowMgr::BuildActiveShadowDepthList( const CViewSetup &viewSetup, int nMaxDepthShadows, ClientShadowHandle_t *pActiveDepthShadows )
{
	...
	for ( ClientShadowHandle_t i = m_Shadows.Head(); i != m_Shadows.InvalidIndex(); i = m_Shadows.Next(i) )
	{
		...
		Frustum_t viewFrustum;
		GeneratePerspectiveFrustum( viewSetup.origin, viewSetup.angles, viewSetup.zNear, 
			viewSetup.zFar, viewSetup.fov, viewSetup.m_flAspectRatio, viewFrustum );
...
// Re-render shadow depth textures that lie in the leaf list
//-----------------------------------------------------------------------------
ConVar ortho_shadowmaps("ortho_shadowmaps", "0");
void CClientShadowMgr::ComputeShadowDepthTextures( const CViewSetup &viewSetup )
{
	...
	for ( int j = 0; j < nActiveDepthShadowCount; ++j )
	{
...
#ifndef DARKINTERVAL // from Mapbase
		...
		shadowView.m_bOrtho = ortho_shadowmaps.GetBool();
...
void CClientShadowMgr::ComputeShadowTextures( const CViewSetup &view, int leafCount, LeafIndex_t* pLeafList )
{
	...
	bool LevelDictatesCheapShadows = false;
	if (GetClientWorldEntity()->m_bForceCheapShadows)  { LevelDictatesCheapShadows = true; };
	int nMaxShadows = LevelDictatesCheapShadows ? 0 : r_shadowmaxrendered.GetInt();
