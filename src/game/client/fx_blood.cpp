...
ParticleForBlood_t	bloodCallbacks[] =
{
	...
	{ BLOOD_COLOR_HYDRA,			"blood_impact_blue_01" },
#else
...
{
	CSmartPtr<CSimple3DEmitter> pHunterEmitter = CSimple3DEmitter::Create( "HunterDamage" );
	if ( pHunterEmitter == NULL )
		return;

	pHunterEmitter->SetSortOrigin( data.m_vOrigin );
	...
	pHunterEmitter->m_ParticleCollision.Setup( data.m_vOrigin, &data.m_vNormal, flAngularSpray, flMinSpeed, flMaxSpeed, 600.0f, 0.2f );
	...
	for ( int i = 0; i < numFlecks; i++ )
	{
		...
		pFleckParticle = (Particle3D *) pHunterEmitter->AddParticle( sizeof(Particle3D), g_Mat_Fleck_Antlion[random->RandomInt(0,1)], spawnOffset );
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CrabsynthDamageCallback(const CEffectData &data)
{
	CSmartPtr<CSimple3DEmitter> pCrabsynthEmitter = CSimple3DEmitter::Create("CrabsynthDamage");
	if (pCrabsynthEmitter == NULL)
		return;

	pCrabsynthEmitter->SetSortOrigin(data.m_vOrigin);
	...
	// Setup our collision information
	pCrabsynthEmitter->m_ParticleCollision.Setup(data.m_vOrigin, &data.m_vNormal, flAngularSpray, flMinSpeed, flMaxSpeed, 600.0f, 0.2f);
	...
	{
		spawnOffset = data.m_vOrigin + RandomVector(-32.0f, 32.0f);
		pFleckParticle = (Particle3D *)pCrabsynthEmitter->AddParticle(sizeof(Particle3D), g_Mat_Fleck_Antlion[random->RandomInt(0, 1)], spawnOffset);
...
DECLARE_CLIENT_EFFECT("CrabsynthAPDamage", CrabsynthDamageCallback);
#endif // DARKINTERVAL
