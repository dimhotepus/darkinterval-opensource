...
extern ConVar di_use_particle_splashes;
...
void CCollisionEvent::UpdateFrictionSounds( void )
{
	for ( int i = 0; i < ARRAYSIZE(m_current); i++ )
	{
		...
		{
			if ( m_current[i].flLastUpdateTime < (CURTIME-0.1f) )
			{
...
void PhysicsSplash( IPhysicsFluidController *pFluid, IPhysicsObject *pObject, CBaseEntity *pEntity )
{
	...
	if ((pObject->GetGameFlags() & FVPHYSICS_PART_OF_RAGDOLL) == false)
		return;
#ifdef DARKINTERVAL // fancy, but expensive bumpmapped animated splashes
	bool particleSplashBump = di_use_particle_splashes.GetBool();
	if (RandomFloat(0, 1) < 0.25) particleSplashBump = false;
#endif
	Vector velocity;
	...
	*/
	...
	if ( impactSpeed > 150 )
	{
		...
		{
#ifdef DARKINTERVAL
			FX_GunshotSlimeSplash( centerPoint, normal, random->RandomFloat( 8, 10 ), particleSplashBump);
#else	
		...
		{
#ifdef DARKINTERVAL
			FX_GunshotSplash( centerPoint, normal, random->RandomFloat( 8, 10 ), particleSplashBump); 
#else
	...
	{
#ifdef DARKINTERVAL
		FX_WaterRipple( centerPoint, 1.5f, &color, 1.5f, luminosity, particleSplashBump);
#else
	...
	for ( int i = 0; i < splashes; i++ )
	{
		...
		if ( impactSpeed > 150 )
		{
			...
			{
#ifdef DARKINTERVAL
				FX_GunshotSlimeSplash( centerPoint, normal, random->RandomFloat( 4, 6 ), particleSplashBump);
#else
			...
			{
#ifdef DARKINTERVAL
				FX_GunshotSplash( centerPoint, normal, random->RandomFloat( 4, 6 ), particleSplashBump);
#else
		...
		{
#ifdef DARKINTERVAL
			FX_WaterRipple( point, random->RandomFloat( 0.25f, 0.5f ), &color, luminosity, random->RandomFloat( 0.5f, 1.0f ), particleSplashBump);
#else
...
void CCollisionEvent::UpdateFluidEvents( void )
{
	...
	{
		if ( (CURTIME - m_fluidEvents[i].impactTime) > FLUID_TIME_MAX )
		{
...
float CCollisionEvent::DeltaTimeSinceLastFluid( CBaseEntity *pEntity )
{
	for ( int i = m_fluidEvents.Count()-1; i >= 0; --i )
	{
		...
		{
			return CURTIME - m_fluidEvents[i].impactTime;
		}
	}

	int index = m_fluidEvents.AddToTail();
	m_fluidEvents[index].hEntity = pEntity;
	m_fluidEvents[index].impactTime = CURTIME;
...
void PhysFrictionSound( CBaseEntity *pEntity, IPhysicsObject *pObject, const char *pSoundName, HSOUNDSCRIPTHANDLE& handle, float flVolume )
{
	...
	if ( flVolume > (1.0f/128.0f) )
	{
		...
		pFriction->flLastUpdateTime = CURTIME;
		pFriction->flLastEffectTime = CURTIME;
	}
}
...
float PhysGetSyncCreateTime()
{
	...
	if ( nextTime < simTime )
	{
		...
		return CURTIME + nextTime - simTime;
	}
	return CURTIME;
}
