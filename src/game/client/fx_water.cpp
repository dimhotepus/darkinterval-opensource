...
#ifdef DARKINTERVAL // DI has an option for fancy bumpmapped splash decals. They are disabled by default for perf reasons
CLIENTEFFECT_MATERIAL("effects/splash1_bumpmapped")
#endif
...
//-----------------------------------------------------------------------------
#ifdef DARKINTERVAL
ConVar di_use_particle_splashes("di_use_particle_splashes", "0", FCVAR_ARCHIVE);
void FX_WaterRipple( const Vector &origin, float scale, Vector *pColor, float flLifetime, float flAlpha, bool bBumpMapped )
#else
...
#endif
{
	...
	if ( tr.fraction < 1.0f )
	{
		...
		if (bBumpMapped)
		{
			DispatchParticleEffect("water_splash_new", tr.endpos, QAngle(0, 0, 0));
			startsize = 4.0f;
			endsize = 32.0f;
		}
		else
...
//-----------------------------------------------------------------------------
#ifdef DARKINTERVAL
void FX_GunshotSplash( const Vector &origin, const Vector &normal, float scale, bool bBumpMapped, bool bPlaySound)
#else
...
#endif
{
	...
	// Do a ripple
#ifdef DARKINTERVAL
	FX_WaterRipple( origin, flScale, &color, 1.5f, luminosity, bBumpMapped );
#else
	FX_WaterRipple(origin, flScale, &color, 1.5f, luminosity);

#endif
#ifdef DARKINTERVAL // by default it always played a sound
	//Play a sound
	if (bPlaySound)
#endif
	{
...
//-----------------------------------------------------------------------------
#ifdef DARKINTERVAL
void FX_GunshotSlimeSplash( const Vector &origin, const Vector &normal, float scale, bool bBumpMapped, bool bPlaySound )
#else
...
void SplashCallback( const CEffectData &data )
{
	...
	{
#ifdef DARKINTERVAL
		FX_GunshotSlimeSplash( data.m_vOrigin, Vector(0,0,1), data.m_flScale, di_use_particle_splashes.GetBool() && data.m_flMagnitude != 1, !(data.m_fFlags & FX_NO_SPLASH_SOUND));
#else
	...
	{
#ifdef DARKINTERVAL
		FX_GunshotSplash( data.m_vOrigin, Vector(0,0,1), data.m_flScale, di_use_particle_splashes.GetBool() && data.m_flMagnitude != 1, !(data.m_fFlags & FX_NO_SPLASH_SOUND));
#else
...
{
	bool particleSplashes = di_use_particle_splashes.GetBool() && data.m_flMagnitude != 1;

	if ( data.m_fFlags & FX_WATER_IN_SLIME )
	{
#ifdef DARKINTERVAL
		FX_GunshotSlimeSplash( data.m_vOrigin, Vector(0,0,1), data.m_flScale, particleSplashes);
#else
		FX_GunshotSlimeSplash(data.m_vOrigin, Vector(0, 0, 1), data.m_flScale);
#endif
	}
	...
	{
#ifdef DARKINTERVAL
		FX_GunshotSplash( data.m_vOrigin, Vector(0,0,1), data.m_flScale, particleSplashes);	
#else
		FX_GunshotSplash(data.m_vOrigin, Vector(0, 0, 1), data.m_flScale);
#endif
	}
...
#ifdef DARKINTERVAL // for hitting the water but not producing expensive bumpmapped splashes
void GunshotSplashCheapCallback(const CEffectData &data)
{
	if (data.m_fFlags & FX_WATER_IN_SLIME)
	{
		FX_GunshotSlimeSplash(data.m_vOrigin, Vector(0, 0, 1), data.m_flScale, false);
	}
	else
	{
		FX_GunshotSplash(data.m_vOrigin, Vector(0, 0, 1), data.m_flScale, false);
	}
}

DECLARE_CLIENT_EFFECT("gunshotsplash_cheap", GunshotSplashCheapCallback);
#endif
//-----------------------------------------------------------------------------
// Purpose: 
// Input  : &data - 
...
#ifdef DARKINTERVAL
	FX_WaterRipple( data.m_vOrigin, flScale, &color, 1.5f, luminosity, di_use_particle_splashes.GetBool() && data.m_flMagnitude != 1);
#else
