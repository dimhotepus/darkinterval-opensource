...
void C_TEShatterSurface::PostDataUpdate( DataUpdateType_t updateType )
{
	...
	for (int width=0;width<nNumWide;width++)
	{
		...
		{			
#ifdef DARKINTERVAL // for unknown reason, using random particle switch doesn't work
			pParticle = (Particle3D *) pGlassEmitter->AddParticle( sizeof(Particle3D), hMaterial[1], vCurPos );
#else
