...
// - removed some VR stuff (within MP stuff)
...
#include "hud_history_resource.h"
#include "iinput.h"
#include "input.h"
#include "view.h"
...
#ifdef DARKINTERVAL
	RecvPropBool	(RECVINFO(m_bParalyzed)),
#endif
	RecvPropFloat	(RECVINFO(m_flStepSize)),
	RecvPropInt		(RECVINFO(m_bAllowAutoMovement)),
#ifdef DARKINTERVAL	// DI NEW
	RecvPropInt		(RECVINFO(m_bWearingLongJump)),
	RecvPropBool	(RECVINFO(m_bPlayingArcade)),
	RecvPropBool	(RECVINFO(m_bPlayingArcadeAsManhack)),
	RecvPropBool	(RECVINFO(m_bLocatorVisible)),
	RecvPropBool	(RECVINFO(m_bSuitlessHUDVisible)),
	RecvPropFloat	(RECVINFO(m_WeaponProxyValueSlot01_float)),
	RecvPropFloat	(RECVINFO(m_WeaponProxyValueSlot02_float)),
	RecvPropFloat	(RECVINFO(m_WeaponProxyValueSlot03_float)),
	RecvPropFloat	(RECVINFO(m_WeaponProxyValueSlot04_float)),
	RecvPropFloat	(RECVINFO(m_WeaponProxyValueSlot05_float)),
	RecvPropFloat	(RECVINFO(m_WeaponProxyValueSlot06_float)),
	RecvPropFloat	(RECVINFO(m_WeaponProxyValueSlot07_float)),
	RecvPropFloat	(RECVINFO(m_WeaponProxyValueSlot08_float)),
#endif
	// 3d skybox data
...
#ifdef DARKINTERVAL
	RecvPropInt(RECVINFO(m_skybox3d.disabled)),
#endif
	// 3d skybox fog data
...
#ifdef DARKINTERVAL	// DI NEW
	DEFINE_PRED_FIELD(m_bWearingLongJump, FIELD_BOOLEAN, FTYPEDESC_INSENDTABLE),
	DEFINE_PRED_FIELD(m_bParalyzed, FIELD_BOOLEAN, FTYPEDESC_INSENDTABLE),
	DEFINE_PRED_FIELD(m_bPlayingArcade, FIELD_BOOLEAN, FTYPEDESC_INSENDTABLE ),
	DEFINE_PRED_FIELD(m_bPlayingArcadeAsManhack, FIELD_BOOLEAN, FTYPEDESC_INSENDTABLE),
	DEFINE_PRED_FIELD(m_bLocatorVisible, FIELD_BOOLEAN, FTYPEDESC_INSENDTABLE),
	DEFINE_PRED_FIELD(m_bSuitlessHUDVisible, FIELD_BOOLEAN, FTYPEDESC_INSENDTABLE),
	DEFINE_PRED_FIELD(m_WeaponProxyValueSlot01_float, FIELD_FLOAT, FTYPEDESC_INSENDTABLE),
	DEFINE_PRED_FIELD(m_WeaponProxyValueSlot02_float, FIELD_FLOAT, FTYPEDESC_INSENDTABLE),
	DEFINE_PRED_FIELD(m_WeaponProxyValueSlot03_float, FIELD_FLOAT, FTYPEDESC_INSENDTABLE),
	DEFINE_PRED_FIELD(m_WeaponProxyValueSlot04_float, FIELD_FLOAT, FTYPEDESC_INSENDTABLE),
	DEFINE_PRED_FIELD(m_WeaponProxyValueSlot05_float, FIELD_FLOAT, FTYPEDESC_INSENDTABLE),
	DEFINE_PRED_FIELD(m_WeaponProxyValueSlot06_float, FIELD_FLOAT, FTYPEDESC_INSENDTABLE),
	DEFINE_PRED_FIELD(m_WeaponProxyValueSlot07_float, FIELD_FLOAT, FTYPEDESC_INSENDTABLE),
	DEFINE_PRED_FIELD(m_WeaponProxyValueSlot08_float, FIELD_FLOAT, FTYPEDESC_INSENDTABLE),
#endif
...
#ifdef DARKINTERVAL // necessary for NPC-worn flashlights to avoid clipping
...
void C_BasePlayer::PostDataUpdate( DataUpdateType_t updateType )
{
	...
	if ( IsLocalPlayer() )
	{
		...
		if ( !m_bWasFreezeFraming && GetObserverMode() == OBS_MODE_FREEZECAM )
		{
			...
			m_flFreezeFrameStartTime = CURTIME;
#ifdef DARKINTERVAL
			m_flFreezeFrameDistance = 128;
#else
...
void C_BasePlayer::CalcFreezeCamView( Vector& eyeOrigin, QAngle& eyeAngles, float& fov )
{
	...
	float flCurTime = (CURTIME - m_flFreezeFrameStartTime);
#ifdef DARKINTERVAL
	float flBlendPerc = clamp( flCurTime / 0.7, 0.f, 1.f );
#else
	float flBlendPerc = clamp(flCurTime / spec_freeze_traveltime.GetFloat(), 0.1f, 1.f);
#endif
...
#ifdef DARKINTERVAL
	if ( flCurTime >= 0.7f && !m_bSentFreezeFrame )
#else
...
#ifdef DARKINTERVAL
		view->FreezeFrame( 5.0f );
#else
		view->FreezeTrame(spec_freeze_time.GetFloat());
#endif
	}
}
...
#endif
	float interpolation = ( CURTIME - m_flDeathTime ) / GetDeathCamInterpolationTime();
...
{
#ifdef DARKINTERVAL // remove vr stuff
...
void C_BasePlayer::NotePredictionError( const Vector &vDelta )
{
	...
	m_flPredictionErrorTime = CURTIME;
...
void C_BasePlayer::ForceSetupBonesAtTimeFakeInterpolation( matrix3x4_t *pBonesOut, float curtimeOffset )
{
	...
	Interpolate( CURTIME + curtimeOffset );
	...
	SetupBones( pBonesOut, MAXSTUDIOBONES, BONE_USED_BY_ANYTHING, CURTIME + curtimeOffset );
...
void C_BasePlayer::GetRagdollInitBoneArrays( matrix3x4_t *pDeltaBones0, matrix3x4_t *pDeltaBones1, matrix3x4_t *pCurrentBones, float boneDt )
{
	...
	{
		ForceSetupBonesAtTimeFakeInterpolation( pCurrentBones, ragdollCreateTime - CURTIME );
	}
	else
	{
...
void C_BasePlayer::GetPredictionErrorSmoothingVector( Vector &vOffset )
{
	...
	float errorAmount = ( CURTIME - m_flPredictionErrorTime ) / cl_smoothtime.GetFloat();
...
void C_BasePlayer::FogControllerChanged( bool bSnap )
{
	if ( m_Local.m_PlayerFog.m_hCtrl )
	{
		...
		m_Local.m_PlayerFog.m_flTransitionTime = bSnap ? -1 : CURTIME;
...
void C_BasePlayer::UpdateFogBlend( void )
{
	...
	{
		float flTimeDelta = CURTIME - m_Local.m_PlayerFog.m_flTransitionTime;
