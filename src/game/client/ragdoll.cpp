	...
	bool bFixedConstraints )
{
	...
	// It's moving now...
	m_flLastOriginChangeTime = CURTIME;
...
void CRagdoll::CheckSettleStationaryRagdoll()
{
	...
	float dt = CURTIME - m_flLastOriginChangeTime;
...
//-----------------------------------------------------------------------------
// DI NOTE: the class declaration has been moved to ragdoll_serverside.h
...
IMPLEMENT_CLIENTCLASS_DT(C_ServerRagdoll, DT_Ragdoll, CRagdollProp)
	...
	RecvPropFloat( RECVINFO(m_hGoalValueForControlledVariable )),
#endif
...
void C_ServerRagdoll::PostDataUpdate( DataUpdateType_t updateType )
{
	...
	m_iv_ragPos.NoteChanged( CURTIME, true );
	m_iv_ragAngles.NoteChanged( CURTIME, true );
	// this is the local client time at which this update becomes stale
	m_flLastBoneChangeTime = CURTIME + GetInterpolationAmount(m_iv_ragPos.GetType());
}
...
public:
	...
	virtual void BuildTransformations( CStudioHdr *hdr, Vector *pos, Quaternion q[], const matrix3x4_t& cameraTransform, int boneMask, CBoneBitList &boneComputed )
	{
		...
		float frac = RemapVal( CURTIME, m_parentTime, m_parentTime+ATTACH_INTERP_TIME, 0, 1 );
		...
		{
			Assert( parent != this );
			parent->SetupBones( NULL, -1, BONE_USED_BY_ANYTHING, CURTIME );
