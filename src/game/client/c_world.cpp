...
#ifdef DARKINTERVAL
#include "renderparm.h" // for relaying snow dir info
#endif
...
#ifdef DARKINTERVAL
	RecvPropVector(RECVINFO(cStartDarkColor)),
	RecvPropFloat(RECVINFO(m_flStartDarkTime)),
#endif
...
#ifdef DARKINTERVAL
	RecvPropFloat(RECVINFO(m_flDetailDist)),
	RecvPropInt(RECVINFO(m_bRainExpensive)),
	RecvPropInt(RECVINFO(m_bForceCheapShadows)),
	RecvPropInt(RECVINFO(m_bForceCheapWater)),

	RecvPropVector(RECVINFO(m_SnowDir)), // control dynamic snow from world settings // todo: move to some env_weather_controller entity
	RecvPropFloat(RECVINFO(m_SnowDot)),
#endif
END_RECV_TABLE()
...
void C_World::OnDataChanged( DataUpdateType_t updateType )
{
	...
	if ( updateType == DATA_UPDATE_CREATED )
	{
		...
		if ( m_bStartDark )
		{
			...
			sf.a = 255;
#ifdef DARKINTERVAL
			sf.r = cStartDarkColor.x;
			sf.g = cStartDarkColor.y;
			sf.b = cStartDarkColor.z;
			sf.duration = (float)(1 << SCREENFADE_FRACBITS) * m_flStartDarkTime;//5.0f;
#else
			sf.r = 0;
			sf.g = 0;
			sf.b = 0;
		...
		pRenderContext->SetVectorRenderingParameter(VECTOR_RENDERPARM_SNOW_DIRECTION, m_SnowDir);
		pRenderContext->SetFloatRenderingParameter(FLOAT_RENDERPARM_SNOW_DOT, m_SnowDot);
#endif
	}
}
