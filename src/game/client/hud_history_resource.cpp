...
#ifdef DARKINTERVAL // for ammo name, see ::Paint()
#include "ammodef.h"
#endif
...
void CHudHistoryResource::AddIconToHistory( int iType, int iId, C_BaseCombatWeapon *weapon, int iCount, CHudTexture *icon )
{
	...
	{
		freeslot->DisplayTime = CURTIME + (hud_drawhistory_time.GetFloat() / 2.0f);
	}
	else
	{
		freeslot->DisplayTime = CURTIME + hud_drawhistory_time.GetFloat();
	}
...
void CHudHistoryResource::MsgFunc_AmmoDenied( bf_read &msg )
{
	...
	for ( int i = 0; i < m_PickupHistory.Count(); i++ )
	{
		if ( m_PickupHistory[i].type == HISTSLOT_AMMODENIED && m_PickupHistory[i].iId == iAmmo )
		{
			...
			m_PickupHistory[i].DisplayTime = CURTIME + (hud_drawhistory_time.GetFloat() / 2.0f);
			m_bNeedsDraw = true;
			return;
		}
	}
...
void CHudHistoryResource::Paint( void )
{
	...
	for ( int i = 0; i < m_PickupHistory.Count(); i++ )
	{
		if ( m_PickupHistory[i].type )
		{
			m_PickupHistory[i].DisplayTime = MIN( m_PickupHistory[i].DisplayTime, CURTIME + hud_drawhistory_time.GetFloat() );
			if ( m_PickupHistory[i].DisplayTime <= CURTIME )
			{  
			...
			float elapsed = m_PickupHistory[i].DisplayTime - CURTIME;
			float scale = elapsed * 80;
#ifdef DARKINTERVAL // for use with the worker suit, which has gray HUD
			Color clr = player->IsSuitEquipped() ? gHUD.m_clrYellowish : m_HullColor;;
#else
...
#ifdef DARKINTERVAL // there's often cases of picking up ammo before obtaining the weapon. Look the ammo texture in hud_textures.txt by the ammo name
					if (!pWpnInfo)
					{
						itemAmmoIcon = NULL;
