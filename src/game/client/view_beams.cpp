...
ConVar r_DrawBeams( "r_DrawBeams", "1", FCVAR_NOTIFY, "0=Off, 1=Normal, 2=Wireframe" );
...
void CViewRenderBeams::KillDeadBeams( C_BaseEntity *pDeadEntity )
{
	...
	while (pbeam)
	{
		...
		{
			// Die Die Die!
			pbeam->die = CURTIME - 0.1;  

			// Kill off particles
			pHead = pbeam->trail;
			while (pHead)
			{
				pHead->die = CURTIME - 0.1;
				pHead = pHead->next;
			}
...
void CViewRenderBeams::SetupBeam( Beam_t *pBeam, const BeamInfo_t &beamInfo )
{
	...
	pBeam->freq				= CURTIME * beamInfo.m_flSpeed;
	pBeam->die				= CURTIME + beamInfo.m_flLife;
...
void CViewRenderBeams::UpdateBeam( Beam_t *pbeam, float frametime )
{
	...
	if ( frametime == 0.0f )
	{
		beamRandom.SetSeed( (int)CURTIME );
	}
	...
	case TE_BEAMRINGPOINT:
		{
			...
			if ( dr != 0.0f )
			{
				...
				float remaining = pbeam->die - CURTIME;
	...
	// update life cycle
	pbeam->t = pbeam->freq + (pbeam->die - CURTIME);
	if (pbeam->t != 0)
	{
	...
	int frame, int rendermode, float frametime, const float* color, float flHDRColorScale )
{
	...
	if (pnew)
	{
		...
		pnew->die = CURTIME + pbeam->amplitude;
		VectorCopy( vec3_origin, pnew->vel );

		pbeam->die = CURTIME + pbeam->amplitude;
		pnew->next = particles;
		pbeam->trail = pnew;
		particles = pnew;
	}
...
void CViewRenderBeams::DrawBeam( Beam_t *pbeam )
{
	...
	int frame = ( ( int )( pbeam->frame + CURTIME * pbeam->frameRate) % pbeam->frameCount );
...
#ifdef PORTAL_COMPILE	// Beams need to recursively draw through portals
