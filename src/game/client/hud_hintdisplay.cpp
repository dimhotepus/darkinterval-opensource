...
#if 0 //def DARKINTERVAL // doesn't work for hint displays, unfortunately
...
#else
			const char *key;
			if (!Q_strcmp(binding, "blank"))
				key = "";
			else
				key = engine->Key_LookupBinding(*binding == '+' ? binding + 1 : binding);
#endif
			if (!key)
			{
				key = "< not bound >";
