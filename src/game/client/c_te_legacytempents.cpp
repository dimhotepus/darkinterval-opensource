...
#ifdef DARKINTERVAL
	CLIENTEFFECT_MATERIAL("models/_Weapons/fx/muzzleflash_pistol1")
	CLIENTEFFECT_MATERIAL("models/_Weapons/fx/muzzleflash_npc_pistol1")
	CLIENTEFFECT_MATERIAL("models/_Weapons/fx/muzzleflash_revolver1")
	CLIENTEFFECT_MATERIAL("models/_Weapons/fx/muzzleflash_npc_revolver1")
	CLIENTEFFECT_MATERIAL("models/_Weapons/fx/muzzleflash_smg1")
	CLIENTEFFECT_MATERIAL("models/_Weapons/fx/muzzleflash_npc_smg1")
	CLIENTEFFECT_MATERIAL("models/_Weapons/fx/muzzleflash_shotgun1")
	CLIENTEFFECT_MATERIAL("models/_Weapons/fx/muzzleflash_npc_shotgun1")
	CLIENTEFFECT_MATERIAL("models/_Weapons/fx/muzzleflash_rifle1")
	CLIENTEFFECT_MATERIAL("models/_Weapons/fx/muzzleflash_npc_rifle1")
	CLIENTEFFECT_MATERIAL("models/_Weapons/fx/muzzleflash_combine1")
	CLIENTEFFECT_MATERIAL("models/_Weapons/fx/muzzleflash_npc_combine1")
#endif
...
bool C_LocalTempEntity::Frame( float frametime, int framenumber )
{
	...
	else if ( flags & FTENT_SINEWAVE )
	{
		...
		SetLocalOrigin( Vector(
			x + sin( m_vecTempEntVelocity[2] + CURTIME /* * anim.prevframe */ ) * (10*m_flSpriteScale),
	...
	else if ( flags & FTENT_SPIRAL )
	{
		...
		SetLocalOrigin( GetLocalOrigin() + Vector(
			m_vecTempEntVelocity[0] * frametime + 8 * sin( CURTIME * 20 ),
			m_vecTempEntVelocity[1] * frametime + 4 * sin( CURTIME * 30 ),
	...
	{
		Vector vecWind;
		GetWindspeedAtTime( CURTIME, vecWind );
...
void CTempEnts::FizzEffect( C_BaseEntity *pent, int modelIndex, int density, int current )
{
	...
	for (i=0 ; i<count ; i++)
	{
		...
		pTemp->die = CURTIME + (maxHeight / zspeed) - 0.1;
...
void CTempEnts::Bubbles( const Vector &mins, const Vector &maxs, float height, int modelIndex, int count, float speed )
{
	...
	for (i=0 ; i<count ; i++)
	{
		...
		pTemp->die = CURTIME + ((height - (origin[2] - mins[2])) / zspeed) - 0.1;
		pTemp->m_flFrame = random->RandomInt( 0, frameCount-1 );
...
void CTempEnts::BubbleTrail( const Vector &start, const Vector &end, float flWaterZ, int modelIndex, int count, float speed )
{
	...
	for (i=0 ; i<count ; i++)
	{
		...
		pTemp->die = CURTIME + ((flWaterZ - origin[2]) / zspeed) - 0.1;
		pTemp->m_flFrame = random->RandomInt(0,frameCount-1);
		// Set sprite scale
						   ...
						   float randRange, float life, int count, int modelIndex, char flags)
{
	...
	for ( i = 0; i < count; i++ ) 
	{
		...
		pTemp->die = CURTIME + life + random->RandomFloat(0,1);	// Add an extra 0-1 secs of life
...
C_LocalTempEntity *CTempEnts::TempSprite( const Vector &pos, const Vector &dir, float scale, int modelIndex, int rendermode, int renderfx, float a, float life, int flags, const Vector &normal )
{
	...
	if ( life )
		pTemp->die = CURTIME + life;
	else
		pTemp->die = CURTIME + (frameCount * 0.1) + 1;
...
void CTempEnts::Sprite_Trail( const Vector &vecStart, const Vector &vecEnd, int modelIndex, int nCount, float flLife, float flSize, float flAmplitude, int nRenderamt, float flSpeed )
{
	...
	for ( int i = 0 ; i < nCount; i++ )
	{
		...
		pTemp->die			= CURTIME + flLife + random->RandomFloat( 0, 4 );
	}
}
...
void CTempEnts::KillAttachedTents( int client )
{
	...
	FOR_EACH_LL( m_TempEnts, i )
	{
		...
		if ( pTemp->flags & FTENT_PLYRATTACHMENT )
		{
			...
			{
				pTemp->die = CURTIME;// good enough, it will die on next tent update. 
			}
		}
	}
}
...
void CTempEnts::BloodSprite( const Vector &org, int r, int g, int b, int a, int modelIndex, int modelIndex2, float size )
{
	...
	if ( modelIndex && (model = modelinfo->GetModel( modelIndex ) ) != NULL )
	{
		...
		if ( ( pTemp = TempEntAllocHigh( org, model ) ) != NULL )
		{
			...
			pTemp->die				= CURTIME + (frameCount / pTemp->m_flFrameRate); // Play the whole thing Once
...
C_LocalTempEntity *CTempEnts::DefaultSprite( const Vector &pos, int spriteIndex, float framerate )
{
	...
	pTemp->die = CURTIME + (float)frameCount / framerate;
	pTemp->m_flFrame = 0;
	pTemp->SetLocalOrigin( pos );
...
#ifdef DARKINTERVAL
	pTemp->die = CURTIME + 10.0f + random->RandomFloat( 0.0f, 1.0f );	// Add an extra 0-1 secs of life	
#else
	pTemp->die = CURTIME + 1.0f + random->RandomFloat(0.0f, 1.0f);	// Add an extra 0-1 secs of life	
#endif
}
...
void CTempEnts::MuzzleFlash( int type, ClientEntityHandle_t hEntity, int attachmentIndex, bool firstPerson )
{
	...
	{
#ifdef DARKINTERVAL
	case MUZZLEFLASH_RIFLE:
		if (firstPerson)
		{
			MuzzleFlash_Rifle_Player(hEntity, attachmentIndex);
		}
		else
		{
			MuzzleFlash_Rifle_NPC(hEntity, attachmentIndex);
		}
		break;
#endif
	...
	case MUZZLEFLASH_357:
		if ( firstPerson )
		{
			MuzzleFlash_Revolver_Player( hEntity, attachmentIndex );
		}
		else
		{
			MuzzleFlash_Revolver_NPC(hEntity, attachmentIndex);
		}
		break;
...
#ifdef DARKINTERVAL
	m_pShells[0] = (model_t *) engine->LoadModel( "models/_weapons/shell_pistol.mdl" );
	m_pShells[1] = (model_t *) engine->LoadModel("models/_weapons/shell_revolver.mdl");
	m_pShells[2] = (model_t *) engine->LoadModel("models/_weapons/shell_rifle.mdl");
	m_pShells[3] = (model_t *)engine->LoadModel("models/_weapons/shell_shotgun.mdl");
	m_pShells[4] = (model_t *)engine->LoadModel("models/_weapons/shell_smg1.mdl");
	m_pShells[5] = (model_t *)engine->LoadModel("models/_weapons/shell_flare.mdl");
#else
...
inline void CTempEnts::CacheMuzzleFlashes( void )
{
	...
	for ( int i = 0; i < 4; i++ )
	{
		if ( m_Material_MuzzleFlash_Pistol[i] == NULL )
		{
			m_Material_MuzzleFlash_Pistol[i] = ParticleMgr()->GetPMaterial( VarArgs( "models/_Weapons/fx/muzzleflash_pistol%d", i+1 ) );
		}
	}

	for ( int j = 0; j < 4; j++ )
	{
		if (m_Material_MuzzleFlash_NPC_Pistol[j] == NULL )
		{
			m_Material_MuzzleFlash_NPC_Pistol[j] = ParticleMgr()->GetPMaterial( VarArgs( "models/_Weapons/fx/muzzleflash_npc_pistol%d", j+1 ) );
		}
	}

	// Revolver + revolver NPC
	for ( int k = 0; k < 4; k++)
	{
		if (m_Material_MuzzleFlash_Revolver[k] == NULL)
		{
			m_Material_MuzzleFlash_Revolver[k] = ParticleMgr()->GetPMaterial(VarArgs("models/_Weapons/fx/muzzleflash_revolver%d", k + 1));
		}
	}

	for ( int l = 0; l < 4; l++)
	{
		if (m_Material_MuzzleFlash_NPC_Revolver[l] == NULL)
		{
			m_Material_MuzzleFlash_NPC_Revolver[l] = ParticleMgr()->GetPMaterial(VarArgs("models/_Weapons/fx/muzzleflash_npc_revolver%d", l + 1));
		}
	}
	...
	for ( int m = 0; m < 4; m++)
	{
		if (m_Material_MuzzleFlash_SMG[m] == NULL)
		{
			m_Material_MuzzleFlash_SMG[m] = ParticleMgr()->GetPMaterial(VarArgs("models/_Weapons/fx/muzzleflash_smg%d", m + 1));
		}
	}

	for ( int n = 0; n < 4; n++)
	{
		if (m_Material_MuzzleFlash_NPC_SMG[n] == NULL)
		{
			m_Material_MuzzleFlash_NPC_SMG[n] = ParticleMgr()->GetPMaterial(VarArgs("models/_Weapons/fx/muzzleflash_npc_smg%d", n + 1));
		}
	}
	...
	for ( int o = 0; o < 4; o++)
	{
		if (m_Material_MuzzleFlash_Rifle[o] == NULL)
		{
			m_Material_MuzzleFlash_Rifle[o] = ParticleMgr()->GetPMaterial(VarArgs("models/_Weapons/fx/muzzleflash_rifle%d", o + 1));
		}
	}

	for ( int p = 0; p < 4; p++)
	{
		if (m_Material_MuzzleFlash_NPC_Rifle[p] == NULL)
		{
			m_Material_MuzzleFlash_NPC_Rifle[p] = ParticleMgr()->GetPMaterial(VarArgs("models/_Weapons/fx/muzzleflash_npc_rifle%d", p + 1));
		}
	}
	...
	for ( int q = 0; q < 4; q++)
	{
		if (m_Material_MuzzleFlash_Shotgun[q] == NULL)
		{
			m_Material_MuzzleFlash_Shotgun[q] = ParticleMgr()->GetPMaterial(VarArgs("models/_Weapons/fx/muzzleflash_shotgun%d", q + 1));
		}
	}

	for ( int r = 0; r < 4; r++)
	{
		if (m_Material_MuzzleFlash_NPC_Shotgun[r] == NULL)
		{
			m_Material_MuzzleFlash_NPC_Shotgun[r] = ParticleMgr()->GetPMaterial(VarArgs("models/_Weapons/fx/muzzleflash_npc_shotgun%d", r + 1));
		}
	}
	// Combine + combine NPC
	for ( int s = 0; s < 2; s++ )
	{
		if (m_Material_MuzzleFlash_Combine[s] == NULL )
		{
			m_Material_MuzzleFlash_Combine[s] = ParticleMgr()->GetPMaterial( VarArgs( "models/_Weapons/fx/muzzleflash_combine%d", s+1 ) );
		}
	}

	for ( int t = 0; t < 2; t++ )
	{
		if (m_Material_MuzzleFlash_NPC_Combine[t] == NULL )
		{
			m_Material_MuzzleFlash_NPC_Combine[t] = ParticleMgr()->GetPMaterial( VarArgs( "models/_Weapons/fx/muzzleflash_npc_combine%d", t+1 ) );
		}
	}
#else
	int i;
...
void CTempEnts::MuzzleFlash_Pistol_Player(ClientEntityHandle_t hEntity, int attachmentIndex)
{
	...
	for (int i = 1; i < 6; i++)
	{
		...
		pParticle = (SimpleParticle *)pSimple->AddParticle(sizeof(SimpleParticle), m_Material_MuzzleFlash_Pistol[random->RandomInt(0, 3)], offset);

		if (pParticle == NULL)
			return;

		pParticle->m_flLifetime = 0.0f;
		pParticle->m_flDieTime = random->RandomFloat(0.025f, 0.05f);
...
void CTempEnts::MuzzleFlash_Revolver_Player(ClientEntityHandle_t hEntity, int attachmentIndex)
{
//	VPROF_BUDGET("MuzzleFlash_357_Player", VPROF_BUDGETGROUP_PARTICLE_RENDERING);
	CSmartPtr<CSimpleEmitter> pSimple = CSimpleEmitter::Create("MuzzleFlash_Revolver_Player");
	...
	for (int i = 1; i < 6; i++)
	{
		...
		pParticle = (SimpleParticle *)pSimple->AddParticle(sizeof(SimpleParticle), m_Material_MuzzleFlash_Revolver[random->RandomInt(0, 3)], offset);

		if (pParticle == NULL)
			return;

		pParticle->m_flLifetime = 0.0f;
		pParticle->m_flDieTime = RandomFloat(0.025, 0.05f);
	...
	// Trails
	CSmartPtr<CTrailParticles> pTrails = CTrailParticles::Create("MuzzleFlash_Revolver_Player");
	...
	int i;
	int numTrails = random->RandomInt(3, 5);
	for (i = 0; i < numTrails; i++)
	{
		...
		pTrailParticle->m_flLifetime = 0.1f;
		pTrailParticle->m_flDieTime = random->RandomFloat(0.15f, 0.20f);
		...
		pTrailParticle->m_flLength = 1.0f;
		pTrailParticle->m_flWidth = random->RandomFloat(1.0f, 1.4f);
	}
}
...
//	VPROF_BUDGET("MuzzleFlash_SMG1_Player", VPROF_BUDGETGROUP_PARTICLE_RENDERING);
	...
	SimpleParticle *pParticle;
	Vector			forward(4, 0, 0), offset; //NOTENOTE: All coords are in local space
	...
	for (int i = 1; i < 6; i++)
	{
		offset = Vector (16, 0, 0) + (forward * (i*flScale));

		pParticle = (SimpleParticle *)pSimple->AddParticle(sizeof(SimpleParticle), m_Material_MuzzleFlash_SMG[random->RandomInt(0, 3)], offset);
...
//	VPROF_BUDGET("MuzzleFlash_Shotgun_Player", VPROF_BUDGETGROUP_PARTICLE_RENDERING);
	...
	for (int i = 1; i < 6; i++)
	{
		...
		pParticle = (SimpleParticle *)pSimple->AddParticle(sizeof(SimpleParticle), m_Material_MuzzleFlash_Shotgun[random->RandomInt(0, 3)], offset);
	...
	for (int i = 0; i < numEmbers; i++)
	{
		...
		pTrailParticle->m_flLength = 1.6f;
		pTrailParticle->m_flWidth = random->RandomFloat(1.5f, 1.9f);
	}
}

void CTempEnts::MuzzleFlash_Rifle_Player(ClientEntityHandle_t hEntity, int attachmentIndex)
{
//	VPROF_BUDGET("MuzzleFlash_Rifle_Player", VPROF_BUDGETGROUP_PARTICLE_RENDERING);
	CSmartPtr<CLocalSpaceEmitter> pSimple = CLocalSpaceEmitter::Create("MuzzleFlash_Rifle_Player", hEntity, attachmentIndex, FLE_VIEWMODEL);

	CacheMuzzleFlashes();

	SimpleParticle *pParticle;
	Vector			forward(12, 0, 0), offset; //NOTENOTE: All coords are in local space

	float flScale = random->RandomFloat(1.5f, 1.7f);
	...
	for (int i = 1; i < 6; i++)
	{
		...
		pParticle = (SimpleParticle *)pSimple->AddParticle(sizeof(SimpleParticle), m_Material_MuzzleFlash_Rifle[random->RandomInt(0, 3)], offset);
...
void CTempEnts::MuzzleFlash_Combine_Player(ClientEntityHandle_t hEntity, int attachmentIndex)
{
	...
	CSmartPtr<CLocalSpaceEmitter> pSimple = CLocalSpaceEmitter::Create("MuzzleFlash_Combine", hEntity, attachmentIndex);
...
#define	FRONT_LENGTH 3
	...
	for (int i = 1; i < FRONT_LENGTH; i++)
	{
		...
		pParticle = (SimpleParticle *)pSimple->AddParticle(sizeof(SimpleParticle), m_Material_MuzzleFlash_Combine[random->RandomInt(0, 1)], offset);
		if (pParticle == NULL)
			return;
...
#define	VERTICAL_LENGTH	8
#define	DIAGONAL_LENGTH	6
	Vector dir;
	...
	for (int i = 1; i < VERTICAL_LENGTH; i++)
	{
		...
		pParticle->m_uchStartSize = ((random->RandomFloat(2.0f, 4.0f) * (VERTICAL_LENGTH - (i)) / (VERTICAL_LENGTH*0.5f)) * flScale);
		pParticle->m_uchEndSize = pParticle->m_uchStartSize;
		pParticle->m_flRoll = random->RandomInt(0, 360);
		pParticle->m_flRollDelta = 0.0f;
	}

	// Diagonal part, aimed down and left
	dir = Vector(0, -1, -1);
	burstSpeed = random->RandomFloat(50.0f, 150.0f);
	for (int i = 1; i < DIAGONAL_LENGTH; i++)
	{
		...
		pParticle->m_uchStartSize = ((random->RandomFloat(2.0f, 4.0f) * (DIAGONAL_LENGTH - (i)) / (DIAGONAL_LENGTH*0.5f)) * flScale);
		pParticle->m_uchEndSize = pParticle->m_uchStartSize;
		pParticle->m_flRoll = random->RandomInt(0, 360);
		pParticle->m_flRollDelta = 0.0f;
	}

	// Diagonal part, aimed down and right.
	dir = Vector(0, -1, 1);
	burstSpeed = random->RandomFloat(50.0f, 150.0f);
	for (int i = 1; i < DIAGONAL_LENGTH; i++)
	{
	...
	pParticle = (SimpleParticle *)pSimple->AddParticle(sizeof(SimpleParticle), m_Material_MuzzleFlash_Combine[random->RandomInt(0, 1)], vec3_origin);
	if (pParticle == NULL)
		return;
	...
	// HL2's lame combine muzzleflash
	/*
...
void CTempEnts::MuzzleFlash_Revolver_NPC(ClientEntityHandle_t hEntity, int attachmentIndex)
{
	FX_MuzzleEffectAttached(0.7f, hEntity, attachmentIndex, NULL, true);
}
...
void CTempEnts::MuzzleFlash_Rifle_NPC(ClientEntityHandle_t hEntity, int attachmentIndex)
{
	FX_MuzzleEffectAttached(1.5f, hEntity, attachmentIndex, NULL, true);
}
...
void CTempEnts::MuzzleFlash_Combine_NPC(ClientEntityHandle_t hEntity, int attachmentIndex)
{
	...
	if (m_Material_MuzzleFlash_NPC_Combine[0] == NULL || m_Material_MuzzleFlash_NPC_Combine[1] == NULL)
	{
		return;
	}
...
#define	FRONT_LENGTH_NPC 5 // I don't get it but the front muzzleflash gets huge if it emits 5 particles (i < this define) instead of 4. 

	// Front flash
	for (int i = 1; i < FRONT_LENGTH_NPC; i++)
	{
		offset = (forward * (i*2.0f*flScale));

		pParticle = (SimpleParticle *)pSimple->AddParticle(sizeof(SimpleParticle), m_Material_MuzzleFlash_NPC_Combine[random->RandomInt(0, 1)], offset);
		if (pParticle == NULL)
			return;
...
#define	SIDE_LENGTH_NPC	6

	burstSpeed = random->RandomFloat(50.0f, 150.0f);

	// Diagonal flash
	for (int i = 1; i < SIDE_LENGTH_NPC; i++)
	{
		offset = (dir * (i*flScale));
		...
		pParticle->m_uchStartSize = ((random->RandomFloat(2.0f, 4.0f) * (SIDE_LENGTH_NPC - (i)) / (SIDE_LENGTH_NPC*0.5f)) * flScale);
		pParticle->m_uchEndSize = pParticle->m_uchStartSize;
		pParticle->m_flRoll = random->RandomInt(0, 360);
		pParticle->m_flRollDelta = 0.0f;
	}
	...
	//	if (muzzleflash_light.GetBool())
	{
		...
		if (pEnt)
		{
			dlight_t *el = effects->CL_AllocDlight(LIGHT_INDEX_MUZZLEFLASH + pEnt->entindex());
	...
	for (i = 1; i < 9; i++)
	{
		...
		pParticle->m_flLifetime = 0.0f;
		pParticle->m_flDieTime = RandomFloat(0.015f, 0.05f);
...
void CTempEnts::HL1EjectBrass( const Vector &vecPosition, const QAngle &angAngles, const Vector &vecVelocity, int nType )
{
	...
	pTemp->die = CURTIME + 2.5;
}

#define SHELLTYPE_PISTOL	0
#ifdef DARKINTERVAL
#define SHELLTYPE_REVOLVER	1
#define SHELLTYPE_RIFLE		2
#define SHELLTYPE_SHOTGUN	3
#define SHELLTYPE_SMG		4
#define SHELLTYPE_FLARE		5
#else
