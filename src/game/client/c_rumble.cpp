...
typedef struct
{
	float			starttime;			// When did this effect start playing? (CURTIME)
	...
	for( i = 0 ; i < MAX_RUMBLE_CHANNELS ; i++ )
	{
		...
		else if( pChannel->priority == lowestPriority )
		{
			...
			if( pBestChannel )
			{
				...
				float age = CURTIME - pChannel->starttime;
			...
			else
			{
				// Take it.
				pBestChannel = pChannel;
				oldestChannel = CURTIME - pChannel->starttime;
			}
		}
	}
...
void UpdateRumbleEffects()
{
	...
	g_RumbleEffects.UpdateEffects( CURTIME );
}
