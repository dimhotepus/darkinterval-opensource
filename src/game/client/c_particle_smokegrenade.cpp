...
#ifdef DARKINTERVAL
	float				m_ExpandTime; // instead of hardcoded SMOKESPHERE_EXPAND_TIME
	float				m_VolumeEmissionRate; // instead of hardcoded 40
	Vector				m_VolumeColor;
	Vector				m_VolumeDimensions; // instead of hardcoded SMOKEGRENADE_PARTICLERADIUS
#endif
...
#ifdef DARKINTERVAL
	RecvPropFloat(RECVINFO(m_ExpandTime)),
	RecvPropFloat(RECVINFO(m_VolumeEmissionRate)),
	RecvPropVector(RECVINFO(m_VolumeColor)),
	RecvPropVector(RECVINFO(m_VolumeDimensions)),
#endif
...
#ifdef DARKINTERVAL
#ifdef old_smokevolumes
		m_VolumeColor.Init(0.50, 0.50, 0.50);
#else
		m_VolumeColor.Init(1.00, 1.00, 1.00);
#endif
	m_MinColor.Init(m_VolumeColor.x, m_VolumeColor.y, m_VolumeColor.z);//(0.50, 0.50, 0.50);
	m_MaxColor.Init(m_VolumeColor.x + 0.1, m_VolumeColor.y + 0.1, m_VolumeColor.z + 0.1);//(0.60, 0.60, 0.60);

#ifdef old_smokevolumes
	m_VolumeDimensions.Init(SMOKEGRENADE_PARTICLERADIUS, SMOKEGRENADE_PARTICLERADIUS, SMOKEGRENADE_PARTICLERADIUS);
#else
	m_VolumeDimensions.Init(100, 60, 60);
#endif
#else
...
#ifdef DARKINTERVAL
	m_SmokeTrail.SetSpawnRate(m_VolumeEmissionRate);
#else
...
#ifdef DARKINTERVAL
					- 0.62 // todo: magic numbers aren't a very good thing
#endif
					;
			}
			else
...
#ifdef DARKINTERVAL
#ifdef old_smokevolumes
	m_VolumeDimensions.x = m_VolumeDimensions.y = m_VolumeDimensions.z = SMOKEGRENADE_PARTICLERADIUS;
#endif
#endif
}
...
{
	float flLifetime = CURTIME - m_flSpawnTime;
	...
	// Update our bbox.
#ifdef DARKINTERVAL
	Vector vMins = m_SmokeBasePos - Vector( m_SpacingRadius + m_VolumeDimensions.x, m_SpacingRadius + m_VolumeDimensions.y, m_SpacingRadius + m_VolumeDimensions.z);
	Vector vMaxs = m_SmokeBasePos + Vector( m_SpacingRadius + m_VolumeDimensions.x, m_SpacingRadius + m_VolumeDimensions.y, m_SpacingRadius + m_VolumeDimensions.z);
#else
...
#ifdef DARKINTERVAL
		if (m_ExpandTimeCounter > m_ExpandTime) //SMOKESPHERE_EXPAND_TIME)
			m_ExpandTimeCounter = m_ExpandTime; // SMOKESPHERE_EXPAND_TIME;

		m_ExpandRadius = (m_SpacingRadius * 2) * (float)sin(m_ExpandTimeCounter * M_PI * 0.5 / m_ExpandTime);
#else
...
#ifdef DARKINTERVAL
			float alpha = 1.0 - len / m_ExpandRadius;
#else
...
#ifdef DARKINTERVAL
			Vector color = m_VolumeColor * (pParticle->m_ColorInterp / 255.1f);
#else
...
#ifdef DARKINTERVAL
	m_SpacingRadius = (m_VolumeDimensions.x - overlap) * NUM_PARTICLES_PER_DIMENSION * 0.5f;
#else	
