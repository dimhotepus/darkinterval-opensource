...
#ifdef DARKINTERVAL
	RecvPropBool(RECVINFO(m_bEnableRopeNodeSprites)),
#endif
...
#endif
#ifdef DARKINTERVAL // experimental rope flashlight support, but hoping to do it in the shader instead
static ConVar rope_flashlight_value("rope_flashlight_value", "10", FCVAR_ARCHIVE);
static ConVar rope_flashlight_dot("rope_flashlight_dot", "0.95", FCVAR_ARCHIVE);
static ConVar rope_flashlight_dist("rope_flashlight_dist", "700");
static ConVar rope_light_styles("rope_light_styles", "0");
#endif
static ConVar mat_fullbright( "mat_fullbright", "0" ); // get it from the engine
static ConVar r_drawropes( "r_drawropes", "1", FCVAR_NOTIFY);
...
inline bool ShouldUseFakeAA( IMaterial *pBackMaterial )
{
	return pBackMaterial 
		...
		&& rope_smooth.GetInt()
#endif
		&& engine->GetDXSupportLevel() > 70 && !g_pMaterialSystemHardwareConfig->IsAAEnabled();
}
...
void CRopeManager::RenderSolidRopes( IMatRenderContext *pRenderContext, IMaterial *pMaterial, int nVertCount, int nIndexCount, bool bRenderNonSolid )
{
	...
	if ( bRenderNonSolid )
	{
		...
		for ( int iSegmentCache = 0; iSegmentCache < m_nSegmentCacheCount; ++iSegmentCache )
		{
			...
			if ( 
#ifndef DARKINTERVAL
				rope_solid_minalpha.GetFloat() == 0.0 &&
#endif
				pSegData->m_flMaxBackWidth <= 
#ifdef DARKINTERVAL
				0.3f 
#else
...
void C_RopeKeyframe::CPhysicsDelegate::GetNodeForces( CSimplePhysics::CNode *pNodes, int iNode, Vector *pAccel )
{
	...
	{
		Vector vecWindVel;
		GetWindspeedAtTime(CURTIME, vecWindVel);
...
void C_RopeKeyframe::BuildRope( RopeSegData_t *pSegmentData, const Vector &vCurrentViewForward, const Vector &vCurrentViewOrigin, C_RopeKeyframe::BuildRopeQueuedData_t *pQueuedData, bool bQueued )
{
	...
	{
#if 0 //def DARKINTERVAL // illuminate ropes with the flashlight, per-vertex
		if (rope_flashlight_value.GetFloat() > 0)
		{
			...
			if (player && player->IsEffectActive(EF_DIMLIGHT))
			{
				...
				Vector illumPos = (iNode == 0 ? pPredictedPositions[iNode] : pPredictedPositions[iNode - 1]);
				Vector lookTargetDir = illumPos - player->EyePosition();
				
				VectorNormalize(lookTargetDir);
				float maxDist = player->EyePosition().DistToSqr(illumPos);
				...
				float illumCoeff = rope_flashlight_value.GetFloat();
				if (dotPr > rope_flashlight_dot.GetFloat() && maxDist < rope_flashlight_dist.GetFloat() * rope_flashlight_dist.GetFloat())
				{
					NDebugOverlay::Cross3D(illumPos, 4, 0, 255, 255, true, 0.1f);
					if (iNode > 0)
					{
						pLightValues[iNode - 1] *= illumCoeff;
						Clamp((int)pLightValues[iNode - 1].x, (int)pLightValues[iNode - 1].x, 255);
						Clamp((int)pLightValues[iNode - 1].x, (int)pLightValues[iNode - 1].y, 255);
						Clamp((int)pLightValues[iNode - 1].x, (int)pLightValues[iNode - 1].z, 255);
					}
					else
					{
						pLightValues[iNode] *= illumCoeff;
						Clamp((int)pLightValues[iNode].x, (int)pLightValues[iNode - 1].x, 255);
						Clamp((int)pLightValues[iNode].x, (int)pLightValues[iNode - 1].y, 255);
						Clamp((int)pLightValues[iNode].x, (int)pLightValues[iNode - 1].z, 255);
					}
				}
			}
		}
...
#if DARKINTERVAL
		// DI NEW: if we're a hydra rope, draw glowing spots along our segments to simulate internal organs
	//	char spriteName[512];
	//	Q_snprintf(spriteName, sizeof(spriteName), "%s_sprites", GetSolidMaterial()->GetName());
	//	IMaterial *spriteMaterial = NULL;
	//	spriteMaterial = materials->FindMaterial(spriteName, TEXTURE_GROUP_OTHER, false);
	//	if (IsErrorMaterial(spriteMaterial))
	//		spriteMaterial = NULL;

		CEffectData data;
		if (m_bEnableRopeNodeSprites)
		{
			data.m_nMaterial = reinterpret_cast< int >(this);
			data.m_nHitBox = (iNode << 8);
			data.m_flScale = m_Width * 0.2f;
			data.m_vOrigin = pSegmentData->m_Segments[nSegmentCount].m_vPos;
			DispatchEffect("FX_RopeNodeSprite", data);
		}
#endif		
		++nSegmentCount;
...
#ifdef DARKINTERVAL
		float flExtraScreenSpaceWidth = 1.4f;

		float flMinAlpha = 0.3f;
		float flMaxAlpha = 0.5f;

		float flMinScreenSpaceWidth = 0.3f;
		float flMaxAlphaScreenSpaceWidth = 1.75f;
#else
