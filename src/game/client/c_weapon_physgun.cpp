...
public:
	...
	virtual void	GetRenderBounds(Vector& mins, Vector& maxs)
	{
		...
		mins.Init(-4096, -4096, -4096);
		maxs.Init(4096, 4096, 4096);
	}
...
int	C_BeamQuadratic::DrawModel(int)
{
	...
	points[1].z += 4 * sin(CURTIME * 11) + 5 * cos(CURTIME * 13);
	points[2] = m_worldPosition;
	//	Msg("pos 0 %.1f %.1f %.1f, pos 2 %.1f %.1f %.1f\n", points[0].x, points[0].y, points[0].z, points[2].x, points[2].y, points[2].z);
	...
	float scrollOffset = CURTIME - (int)CURTIME;
