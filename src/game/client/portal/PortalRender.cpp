...
static ConVar r_forcecheapwater( "r_forcecheapwater", "0", FCVAR_CLIENTDLL | FCVAR_NOTIFY, "Force all water to be cheap water, will show old renders if enabled after water has been seen" );
...
int CPortalRender::ShouldForceCheaperWaterLevel() const
{
	...
	{
		if( g_pDIRenderTargets->GetWaterReflectionTextureForStencilDepth( m_iViewRecursionLevel ) == NULL )
			return 0;
...
void CPortalRender::UpdateDepthDoublerTexture( const CViewSetup &viewSetup )
{
	...
	if( bShouldUpdate )
	{
		...
		ITexture *pTexture = g_pDIRenderTargets->GetDepthDoublerTexture();
