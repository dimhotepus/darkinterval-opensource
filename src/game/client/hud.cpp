...
#ifdef DARKINTERVAL // implements the display option to scale transparency of (almost all) HUD elements
ConVar hud_transparency("hud_transparency", "225", FCVAR_ARCHIVE | FCVAR_REPLICATED);
#endif
//-----------------------------------------------------------------------------
...
#ifdef DARKINTERVAL
ConVar hidehud( "hidehud", "0", FCVAR_NOTIFY );
#endif
...
void CHudTexture::DrawSelfCropped( int x, int y, int cropx, int cropy, int cropw, int croph, int finalWidth, int finalHeight, Color clr ) const
{
	...
	else
	{
		if ( textureId == -1 )
			return;
#ifndef DARKINTERVAL // hud scaling
		...
		float fw = (float)EffectiveWidth(screenScale);
		float fh = (float)EffectiveHeight(screenScale);
...
bool CHud::IsHidden( int iHudFlags )
{
	...
	// DI - arcade hud
	if ((iHudFlags & HIDEHUD_ARCADE_HUD) && ( !pPlayer->IsPlayingArcade() ))
		return true;
