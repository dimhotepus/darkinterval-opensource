...
#ifndef DARKINTERVAL // dead weight
#include "hud_pdump.h"
#endif
...
void CPrediction::RunCommand( C_BasePlayer *player, CUserCmd *ucmd, IMoveHelper *moveHelper )
{
	...
	CURTIME		= player->m_nTickBase * TICK_INTERVAL;
	...
	else
	{
		...
		else
		{
			if ( m_bPreviousAckHadErrors )
			{
				...
				float flPrev = CURTIME;
				CURTIME = pLocalPlayer->GetTimeBase() - TICK_INTERVAL;
