...
#ifdef DARKINTERVAL
	DEFINE_FIELD( m_allowplayerweapons_bool, FIELD_BOOLEAN),
#endif
END_DATADESC()
...
C_PropVehicleChoreoGeneric::C_PropVehicleChoreoGeneric( void )
{
	...
	m_flFOV = 0;
#ifdef DARKINTERVAL
	m_allowplayerweapons_bool = false;
#endif
}
...
#ifdef DARKINTERVAL
bool C_PropVehicleChoreoGeneric::GetKeyValue(const char *szKeyName, char *szValue, int iMaxLen)
{
	if (FStrEq(szKeyName, "allowplayerweapons"))
	{
		m_allowplayerweapons_bool = true;
		Msg("choreo vehicle %s allows player weapons\n", GetDebugName());
		return true;
	}
...
bool C_PropVehicleChoreoGeneric::IsPassengerUsingStandardWeapons(int nRole)
{
