...
void C_SmokeTrail::Update( float fTimeDelta )
{
	...
	if ( ( m_StopEmitTime != 0 ) && ( m_StopEmitTime <= CURTIME ) )
		return;
...
void C_SmokeTrail::CleanupToolRecordingState( KeyValues *msg )
{
	...
	bool bEmitterActive = m_bEmit && ( ( m_StopEmitTime == 0 ) || ( m_StopEmitTime > CURTIME ) );
...
void C_FireTrail::Update( float fTimeDelta )
{
	...
	if ( ( m_flLifetime != 0 ) && ( m_flLifetime <= CURTIME ) )
		return;
...
void C_DustTrail::Update( float fTimeDelta )
{
	...
	while( m_ParticleSpawn.NextEvent( tempDelta ) )
	{
		...
		{
			alpha *= sqrt( (m_StopEmitTime - CURTIME) /(m_StopEmitTime - m_StartEmitTime) );
		}
