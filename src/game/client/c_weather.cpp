...
//ConVar r_RainSplashPercentage( "r_RainSplashPercentage", "100", FCVAR_NONE ); // N% chance of a rain particle making a splash.
...
static ConVar r_DrawRain("r_DrawRain", "1", 0, "Enable/disable rain rendering.");
//ConVar r_rain_method("r_rain_method", "0");
//static ConVar r_RainRadius("r_RainRadius", "1400", 0);
ConVar r_rainexpensive("r_rainexpensive", "1", 0, "Use more expensive rain effects - impact particles, higher density");

//static ConVar r_RainSimulate("r_RainSimulate", "1", 0, "Enable/disable rain simulation.");
//static ConVar r_RainHack( "r_RainHack", "0", FCVAR_NONE );
...
CLIENTEFFECT_MATERIAL("particle/rain_cheap")
...
//	void RainImpactFX(Vector vSrc, Vector vEnd);
...
IMPLEMENT_CLIENTCLASS_DT(CClient_Precipitation, DT_Precipitation, CPrecipitation)
	...
	RecvPropInt(RECVINFO(m_WindDir)),
	RecvPropInt(RECVINFO(m_WindSpeed)),
	RecvPropInt(RECVINFO(m_emissionSize)),
END_RECV_TABLE()

static ConVar r_SnowEnable( "r_SnowEnable", "1", 0, "Snow Enable" );
static ConVar r_SnowParticles( "r_SnowParticles", "500", 0, "Snow." );
static ConVar r_SnowInsideRadius( "r_SnowInsideRadius", "256", 0, "Snow." );
static ConVar r_SnowOutsideRadius( "r_SnowOutsideRadius", "1024", 0, "Snow." );
...
// Globals
//-----------------------------------------------------------------------------
ConVar CClient_Precipitation::s_raindensity( "r_raindensity","0.3");
ConVar CClient_Precipitation::s_rainwidth( "r_rainwidth", "0.55");
ConVar CClient_Precipitation::s_rainlength( "r_rainlength", "0.05f");
ConVar CClient_Precipitation::s_rainspeed( "r_rainspeed", "900.0f");
...
//-----------------------------------------------------------------------------
// Saving impact particles
//-----------------------------------------------------------------------------
void CClient_Precipitation::OnSave()
{
}
//-----------------------------------------------------------------------------
// Restoring impact particles
//-----------------------------------------------------------------------------
void CClient_Precipitation::OnRestore()
{
}
//-----------------------------------------------------------------------------
...
{
	if (GetRemainingLifetime( pParticle ) < 0.0f || !r_DrawRain.GetInt())
		return false;
	...
	/*
	if (r_rain_method.GetInt() == 0)
	{
		// Check our current pos to make sure we're not seeping into (or out of) a solid.
		int contents = enginetrace->GetPointContents(vOldPos);
		if (contents & CONTENTS_SOLID) return false;
		// Trace upwards to make sure there's open sky above us. Can't have rain indoors!
		trace_t trace;
		UTIL_TraceLine(vOldPos, vOldPos + Vector(0, 0, 4096), MASK_RAIN, NULL, COLLISION_GROUP_NONE, &trace);
		if (trace.fraction < 1.0 && !trace.DidHitSkybox())
		{
			// hit the ceiling, and that ceiling ain't no sky brush. Remove this particle. 
			return false;
		}
	}
	*/
	
//	NDebugOverlay::Cross3D(vOldPos, 1.0, 255, 0, 0, true, 0.2f);
	...
	if (r_rainexpensive.GetInt())
	{	
		// Make a splash if we hit a valid surface and it's in front of the view.		
	//	if (DotProduct(CurrentViewOrigin(), pParticle->m_Pos) > 0.5)
		{
			if (RandomInt(0, 10) < 9) // reduce ever so slightly.
			{
				trace_t trace;
				UTIL_TraceLine(vOldPos, pParticle->m_Pos, MASK_RAIN, NULL, COLLISION_GROUP_NONE, &trace);

				if (trace.fraction < 1.0)
				{
					const surfacedata_t *psurf = physprops->GetSurfaceData(trace.surface.surfaceProps);

					//	particle = "null";
					if (trace.DidHitNodrawSurface())
					{
						return false;
					}
					else if (trace.DidHitSkybox())
					{
						return false;
					}
					// Only produce particles on these hard surfaces, but not on sand, grass, mud, etc
					else if (psurf->game.material != CHAR_TEX_CONCRETE 
						&& psurf->game.material != CHAR_TEX_GLASS
						&& psurf->game.material != CHAR_TEX_METAL
						&& psurf->game.material != CHAR_TEX_PLASTIC
						&& psurf->game.material != CHAR_TEX_TILE
						&& psurf->game.material != CHAR_TEX_WOOD)
					{
						return false;
					}
					else
					{
						if (RandomInt(0, 10) < 8) // reduce ever so slightly.
						{
							//	NDebugOverlay::Cross3D(trace.endpos, 4.00, 255, 255, 25, true, 0.5f);
							CLocalPlayerFilter filter;
							EmitSound_t params;
							params.m_flVolume = 0.5f;
							params.m_pOrigin = &(trace.endpos);
							params.m_pSoundName = "Rain.HitGround";
							int randomThresh = 15;
							if (trace.contents & CONTENTS_WATER)
							{
								randomThresh = 12;
								params.m_pSoundName = "Rain.HitWater";
								DispatchParticleEffect("rain_impact_single_water", trace.endpos, trace.endpos, QAngle(0, 0, 0));
							}
							else
							{
								DispatchParticleEffect("rain_impact_single", trace.endpos, trace.endpos, QAngle(0, 0, 0));
							}
						//	if (RandomInt(0, randomThresh) == 0) EmitSound(filter, trace.m_pEnt->entindex(), params);
						}
					}
					return false;
				}
...
/*
void CClient_Precipitation::RainImpactFX(Vector vSrc, Vector vEnd)
{
	// Possibly make a splash if we hit a water surface and it's in front of the view.
	if (m_Splashes.Count() < 512) // was 20
	{
		trace_t trace;
		UTIL_TraceLine(vSrc, vEnd, MASK_RAIN, this, COLLISION_GROUP_NONE, &trace);
		if (trace.fraction < 1.0 )
		{
			particle = "null";
						
			if (trace.surface.surfaceProps & (SURF_SKY || SURF_SKY2D || SURF_NODRAW ))
			{
			//	if (RandomInt(0, 10) <= 8) // reduce ever so slightly.
					particle = "rain_impact_single";
			}

			DispatchParticleEffect(particle, trace.endpos, trace.endpos, QAngle(0, 0, 0));
		}
	}
}
*/
...
//	Precache();
	...
	if ( dt )
	{
		...
		QAngle newwindangle = QAngle(0, m_WindDir, 0);

		newwindangle[1] += random->RandomFloat(-5, 5);
		float newwindspeed = m_WindSpeed * (1.0 + random->RandomFloat(-0.1, 0.1));

		AngleVectors(newwindangle, &s_WindVector);
		VectorScale(s_WindVector, newwindspeed, s_WindVector);
...
inline void CClient_Precipitation::RenderParticle( CPrecipitationParticle* pParticle, CMeshBuilder &mb )
{
	...
	if ( m_nPrecipType == PRECIPITATION_TYPE_SNOW )
	{
		...
		{
			SinCos( CURTIME * M_PI * (1+pParticle->m_Mass * 0.1f) + 
...
void CClient_Precipitation::Precache( )
{
	if ( !m_MatHandle )
	{
		...
		case PRECIPITATION_TYPE_RAIN:
			...
			PrecacheScriptSound("Rain.HitWater");
			PrecacheScriptSound("Rain.HitGround");
			PrecacheParticleSystem("rain_impact_single");
			PrecacheParticleSystem("rain_impact_single_water");
			PrecacheParticleSystem("water_splash_new");
...
{
	float timeSinceSpawn = CURTIME - pParticle->m_SpawnTime;
...
bool CClient_Precipitation::ComputeEmissionArea( Vector& origin, Vector2D& size )
{
	...
	float emissionSize = (float)m_emissionSize;/*r_RainRadius.GetFloat();*/	// size of box to emit particles in

	Vector vMins = /*Vector(-1000, -1000, 0);//*/WorldAlignMins();
	Vector vMaxs = /*Vector(1000, 1000, 2000);//*/WorldAlignMaxs();
	if ( GetClientWorldEntity()->m_bRainExpensive )
	{
	...
	float emissionHeight = MIN( vMaxs[2], pPlayer->GetAbsOrigin()[2] + 500 );
...
void CClient_Precipitation::CreateRainOrSnowParticle( Vector vSpawnPosition, Vector vVelocity )
{
	...
	p->m_Velocity[ 0 ] += random->RandomFloat(-100.0f, 100.0f);
	p->m_Velocity[ 1 ] += random->RandomFloat(-100.0f, 100.0f);
...
void CClient_Precipitation::EmitParticles( float fTimeDelta )
{
	...
	if (density <= 0.002f) return; // allows to have "frozen" rain brushes with alpha = 1; alpha = 0 would make them stop emitting forever

		density *= 16.6 / gpGlobals->frametime;

		if (density > 100.00f) 
			density = 100.00f; // was 0,01

		// Compute number of particles to emit based on precip density and emission area and dt
		float fParticles = (size[0] * size[1] * density * fTimeDelta + m_Remainder) / 10000; 
		...
		for ( int i = 0 ; i < cParticles ; i++ )
		{
			...
			// Check our current pos to make sure we're not seeping into (or out of) a solid.
			int contents = enginetrace->GetPointContents(vParticlePos);
			if (contents & CONTENTS_SOLID) continue;
/*
			if (r_rain_method.GetInt() == 1)
			{
				// Trace upwards to make sure there's open sky above us. Can't have rain indoors!
				trace_t trace1;
				UTIL_TraceLine(vParticlePos, vParticlePos + Vector(0, 0, 4096), MASK_RAIN, NULL, COLLISION_GROUP_NONE, &trace1);
				if (trace1.fraction < 1.0 && !trace1.DidHitSkybox())
				{
					// hit the ceiling, and that ceiling ain't no sky brush. Remove this particle. 
					continue;
				}
...
*/
			...
			trace_t trace2;
			UTIL_TraceLine( vPlayerHeight, vParticlePos, MASK_SOLID_BRUSHONLY, NULL, COLLISION_GROUP_NONE, &trace2);
			if (trace2.fraction < 1 )
			{
				// If we hit a brush, then don't spawn the particle.
				if (trace2.surface.flags & SURF_SKY )
				{
					vParticlePos = trace2.endpos;
				}
				else
				{
					continue;
...
void C_EnvWind::ClientThink( )
{
	...
	float flNextThink = m_EnvWindShared.WindThink( CURTIME );
...
int	C_QuadraticBeam::DrawModel( int )
{
	...
	DrawBeamQuadratic( GetRenderOrigin(), m_controlPosition, m_targetPosition, m_flWidth, color, CURTIME*m_scrollRate );
	return 1;
}
...
public:
	...
	void UpdateVelocity( SimpleParticle *pParticle, float timeDelta )
	{
		...
		GetWindspeedAtTime( CURTIME, vecWindVelocity );
		pParticle->m_vecVelocity += ( vecWindVelocity * 0.0035f );
	}
...
void CSnowFallManager::FindSnowVolumes( Vector &vecCenter, float flRadius, Vector &vecEyePos, Vector &vecForward )
{
	...
	for ( iSnow = 0; iSnow < nSnowCount; ++iSnow )
	{
		...
		else		
		{
			...
			vecNewForward = vecForward * 8192.0f;
...
void CSnowFallManager::CreateSnowFall( void )
{
	...
	m_vecSnowFallEmitOrigin += ( vecForward * ( 64.0f + ( flSpeed * 0.4f * 1.0f ) ) );
	m_vecSnowFallEmitOrigin += ( vecVelocity * ( flSpeed * 1.25f * 1.0f ) );
...
void CSnowFallManager::CreateInsideVolumeSnowParticles( float flCurrentTime, float flRadius, const Vector &vecEyePos, const Vector &vecForward, float flZoomScale )
{
	...
	if ( bZoomed )
	{
		vecZoomEmitOrigin = m_vecSnowFallEmitOrigin + ( vecForward * ( 384.0f * flZoomScale ) );
...
void CSnowFallManager::CreateSnowParticlesRay( float flRadius, const Vector &vecEyePos, const Vector &vecForward )
{
	...
	float flRayRadius = 256.0f;

	Vector vecNewForward;
	vecNewForward = vecForward * RandomFloat( flRadius, 8192.0f );
...
{	
	...
	pParticle->m_flLifetime	= 0.0f;
	pParticle->m_vecVelocity = Vector( RandomFloat( -5.0f, 5.0f ), RandomFloat( -5.0f, 5.0f ), ( RandomFloat( -25, -35 ) * 1.5f ) );
	...
	pParticle->m_uchColor[0]	= 150;
	pParticle->m_uchColor[1]	= 175;
