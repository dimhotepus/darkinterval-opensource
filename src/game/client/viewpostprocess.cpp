...
#ifdef DARKINTERVAL // object motion blur ported from 2015
...
float g_flDOFNearBlurDepth =		0.0f;
float g_flDOFNearFocusDepth =		0.0f;
float g_flDOFFarFocusDepth =		0.0f;
float g_flDOFFarBlurDepth =			0.0f;
float g_flDOFNearBlurRadius =		0.0f;
float g_flDOFFarBlurRadius =		0.0f;
#endif
bool g_bFlashlightIsOn = false;

// hdr parameters
ConVar mat_bloomscale( "mat_bloomscale", "0.2" ); // DI changes the default from 1.0 to 0.2
...
static ConVar mat_autoexposure_max( "mat_autoexposure_max", "1.5" ); // DI changes the default from 2 to 1.5
static ConVar mat_autoexposure_min( "mat_autoexposure_min", "0.75" ); // and this from 0.5 to 0.75
static ConVar mat_show_histogram( "mat_show_histogram", "0" );

// DI new HDR override. The point of it is such:
// When the override is inactive, 
// HDR & Bloom settings are enabled by default and controlled by the tonemap controller 
// placed by level designer.
// If there's no controller on level (which should be considered bad practice among mappers)
// then it falls back to the default parameters (in DI they are adjusted to better-looking
// values - Valve's default values are pretty bad.)
//
// However when the override IS active, 
// then it's either when the user opted for Bloom Only or Disable All
// setting under HDR tab, in video settings.
//
// Since we decided to only compile our maps with HDR lightmaps, and 
// cut out LDR lightmaps for sake of speed and filesize (god help the poor soul who decided
// to run Dark Interval on a system without HDR support, but what are you doing, really),
// that means we can't use mat_hdr_level 0, which is regular HL2's setting for
// disabling HDR. 
//
// So I use this override instead to declare that HDR *appearance* should be disabled - 
// essentially we just set max and min exposure to 1.0 - 
// so it *looks* like LDR mode but uses HDR lightmaps.
// This switch is also very fast and doesn't require reloading the map and all the stuff.
#ifdef DARKINTERVAL 
ConVar mat_hdr_override("mat_hdr_override", "0", FCVAR_ARCHIVE);
#endif
...
ConVar mat_disable_bloom("mat_disable_bloom","0", FCVAR_ARCHIVE);
...
//ConVar mat_exposure_center_region_x( "mat_exposure_center_region_x","0.75", FCVAR_NONE );
//ConVar mat_exposure_center_region_y( "mat_exposure_center_region_y","0.80", FCVAR_NONE );
//ConVar mat_exposure_center_region_x_flashlight( "mat_exposure_center_region_x_flashlight","0.33", FCVAR_NONE );
//ConVar mat_exposure_center_region_y_flashlight( "mat_exposure_center_region_y_flashlight","0.33", FCVAR_NONE );
...
{
#ifdef DARKINTERVAL 
	// DI new HDR override. See at top of file for detailed info.
	if (mat_hdr_override.GetBool())
	{
		*flAutoExposureMin = 1.15f; // It's not 1.0 because that's just a bit too dark. 
		*flAutoExposureMax = 1.15f;
	}
	else
#endif
	{
...
#ifndef DARKINTERVAL // normally, TF2 code is to be deleted. But this one's kept because maybe it'll be useful in the future somehow.
...
void DoEnginePostProcessing( int x, int y, int w, int h, bool bFlashlightIsOn, bool bPostVGui )
{
	...
	{   
		...
		case HDR_TYPE_INTEGER:
		{
			...
			bool  bPerformSoftwareAA	= false; // (engine->GetDXSupportLevel() >= 90) && (flAAStrength != 0.0f); // the original code only works for X360
...
void DoImageSpaceMotionBlur( const CViewSetup &view, int x, int y, int w, int h )
{
	...
	if ( true )
	{
		...
		else
		{
			...
			//engine->Con_NPrintf( 4, "gpGlobals->realtime %.2f  CURTIME %.2f", gpGlobals->realtime, CURTIME );
...
// Depth of field =====================================================================================================
//=====================================================================================================================
ConVar mat_dof_enabled("mat_dof_enabled", "1", FCVAR_ARCHIVE | FCVAR_REPLICATED);
//ConVar mat_dof_override("mat_dof_override", "1");
...
ConVar mat_dof_far_focus_depth("mat_dof_far_focus_depth", "7200.0");
ConVar mat_dof_far_blur_depth("mat_dof_far_blur_depth", "42000.0");
ConVar mat_dof_near_blur_radius("mat_dof_near_blur_radius", "2.0");
ConVar mat_dof_far_blur_radius("mat_dof_far_blur_radius", "10.0");
ConVar mat_dof_quality("mat_dof_quality", "2");
ConVar mat_dof_preview("mat_dof_preview", "0");

/*
static float GetNearBlurDepth()
{
	return DOFType() ? mat_dof_near_blur_depth.GetFloat() : g_flDOFNearBlurDepth;
}

static float GetNearFocusDepth()
{
	return DOFType() ? mat_dof_near_focus_depth.GetFloat() : g_flDOFNearFocusDepth;
}

static float GetFarFocusDepth()
{
	return DOFType() ? mat_dof_far_focus_depth.GetFloat() : g_flDOFFarFocusDepth;
}

static float GetFarBlurDepth()
{
	return DOFType() ? mat_dof_far_blur_depth.GetFloat() : g_flDOFFarBlurDepth;
}

static float GetNearBlurRadius()
{
	return DOFType() ? mat_dof_near_blur_radius.GetFloat() : g_flDOFNearBlurRadius;
}

static float GetFarBlurRadius()
{
	return DOFType() ? mat_dof_far_blur_radius.GetFloat() : g_flDOFFarBlurRadius;
}
*/
...
//		return false;
	...
	if (engine->IsPaused()) return true; // so that it always blurs the game when paused
...
void DoDepthOfField(const CViewSetup &view, int getDOFtype)
{
	...
	IMaterial *pMatDOF = materials->FindMaterial("custom_uni/depth_of_field", TEXTURE_GROUP_OTHER, true);
	...
	float GetNearBlurDepth, GetNearFocusDepth, GetFarFocusDepth, GetFarBlurDepth, GetNearBlurRadius, GetFarBlurRadius;

	switch (getDOFtype)
	{
	case DOF_DEFAULT: // get values from convars
		GetNearBlurDepth = mat_dof_near_blur_depth.GetFloat();
		GetNearFocusDepth = mat_dof_near_focus_depth.GetFloat();
		GetFarFocusDepth = mat_dof_far_focus_depth.GetFloat();
		GetFarBlurDepth = mat_dof_far_blur_depth.GetFloat();
		GetNearBlurRadius = mat_dof_near_blur_radius.GetFloat();
		GetFarBlurRadius = mat_dof_far_blur_radius.GetFloat();
		break;

	case DOF_PAUSED_SCREEN: // get values from convars
		GetNearBlurDepth = 20.0f;
		GetNearFocusDepth = 100.0f;
		GetFarFocusDepth = 250.0f;
		GetFarBlurDepth = 1000.0f;
		GetNearBlurRadius = 10.0f;
		GetFarBlurRadius = 5.0f;
		break;
	case DOF_UNDERWATER:
		GetNearBlurDepth = 20;
		GetNearFocusDepth = 100;
		GetFarFocusDepth = 400;
		GetFarBlurDepth = 3500;
		GetNearBlurRadius = 10;
		GetFarBlurRadius = 6;
		break;
	case DOF_EXTRA:
		GetNearBlurDepth = g_flDOFFarBlurDepth;
		GetNearFocusDepth = g_flDOFNearFocusDepth;
		GetFarFocusDepth = g_flDOFFarFocusDepth;
		GetFarBlurDepth = g_flDOFFarBlurDepth;
		GetNearBlurRadius = g_flDOFNearBlurRadius;
		GetFarBlurRadius = g_flDOFFarBlurRadius;
		break;
	default:
		GetNearBlurDepth = 0;
		GetNearFocusDepth = 0;
...
//=====================================================================================================================
ConVar mat_chromaberration_enabled("mat_chromaberration_enabled", "0", FCVAR_ARCHIVE | FCVAR_REPLICATED);
...
bool IsChromAbEnabled()
{
	...
	return mat_chromaberration_enabled.GetBool();
}
...
void DoChromaticAberration(const CViewSetup &view)
{
	...
	IMaterial *pMatChAb = materials->FindMaterial("custom_uni/post_chromatic_aberration", TEXTURE_GROUP_OTHER, true);
...
// Cubic Distortion ===================================================================================================
//=====================================================================================================================
ConVar mat_cubicdistortion_enabled("mat_cubicdistortion_enabled", "1", FCVAR_ARCHIVE | FCVAR_REPLICATED);
...
bool IsCubicDistEnabled()
{
	...
	return mat_cubicdistortion_enabled.GetBool();
}
...
void DoCubicDistortion(const CViewSetup &view)
{
	...
	IMaterial *pMatCuDi = materials->FindMaterial("custom_uni/post_cubic_distortion", TEXTURE_GROUP_OTHER, true);
...
//=====================================================================================================================
// Psychedelic Colour Phasing =========================================================================================
//=====================================================================================================================
ConVar mat_colour_influence_enabled("mat_colour_influence_enabled", "0", FCVAR_ARCHIVE | FCVAR_REPLICATED);
...
bool IsInfluenceEnabled()
{
	...
	return mat_colour_influence_enabled.GetBool();
}
...
void DoInfluence(const CViewSetup &view)
{
	...
	IMaterial *pMatInfl = materials->FindMaterial("custom_uni/post_influence", TEXTURE_GROUP_OTHER, true);
...
// Screen vignette ====================================================================================================
//=====================================================================================================================
ConVar mat_vignette_enabled("mat_vignette_enabled", "0", FCVAR_ARCHIVE|FCVAR_REPLICATED);
...
bool IsVignetteEnabled()
{
	...
	return mat_vignette_enabled.GetBool();
}
...
void DoVignette(const CViewSetup &view)
{
	...
	IMaterial *pMatVig = materials->FindMaterial("custom_uni/post_vignette", TEXTURE_GROUP_OTHER, true);
...
//=====================================================================================================================
// Red filter for manhack =============================================================================================
//=====================================================================================================================
ConVar mat_nightvision_enabled("mat_nightvision_enabled", "0", FCVAR_REPLICATED);
...
bool IsScreenFilterEnabled()
{
	...
	return mat_nightvision_enabled.GetBool();
}
...
void DoScreenFilter(const CViewSetup &view)
{
	...
	IMaterial *pMatVig = materials->FindMaterial("custom_uni/post_nightvision_filter", TEXTURE_GROUP_OTHER, true);
...
// object motion blur ported from 2015
ConVar mat_object_motion_blur_enabled("mat_object_motion_blur_enabled", "0", FCVAR_ARCHIVE);
ConVar mat_object_motion_blur_debug("mat_object_motion_blur_debug", "0");
bool IsObjectMotionBlurEnabled()
{
	...
	return mat_object_motion_blur_enabled.GetBool() && mat_motion_blur_enabled.GetBool();
}

void DoObjectMotionBlur(const CViewSetup &view)
{
	//	Msg("drawing object motion blur, object count: %i\n", g_ObjectMotionBlurManager.GetDrawableObjectCount());
	// can put debug printout of object speeds here if debug convar is on
	CMatRenderContextPtr pRenderContext(materials);
	...
	//
	IMaterial *pGlowColorMaterial = materials->FindMaterial("custom_uni/object_motion_blur_buffer", TEXTURE_GROUP_OTHER, true);
	if (!mat_object_motion_blur_debug.GetBool())
	{
		pRenderContext->PushRenderTargetAndViewport();
	...
	pMotionBlurMaterial = materials->FindMaterial("custom_uni/object_motion_blur", TEXTURE_GROUP_OTHER, true);
	pFBTextureVariable = pMotionBlurMaterial->FindVar("$fb_texture", &bFound1);
	pVelocityTextureVariable = pMotionBlurMaterial->FindVar("$velocity_texture", &bFound2);
	if (bFound1 && bFound2)
	{
