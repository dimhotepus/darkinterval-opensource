...
//-----------------------------------------------------------------------------
static ConVar cl_new_impact_effects( "cl_new_impact_effects", "0" ); // DI TODO: finish the implementation of it? Modders may want it
...
static ImpactEffect_t s_pImpactEffect[26] = 
{
	...
	{ "impact_wood",		"impact_wood_noflecks" },		// CHAR_TEX_EGGSHELL		
	{ NULL,					NULL },							// CHAR_TEX_FLESH			
	{ NULL,					NULL },							// CHAR_TEX_GRATE			
	{ NULL,					NULL },							// CHAR_TEX_ALIENFLESH		
	{ NULL,					NULL },							// CHAR_TEX_CLIP			
	{ NULL,					NULL },							// CHAR_TEX_UNUSED // leak particle #1		
	{ NULL,					NULL },							// CHAR_TEX_UNUSED	// leak particle #2
...
void PerformCustomEffects( const Vector &vecOrigin, trace_t &tr, const Vector &shotDir, int iMaterial, int iScale, int nFlags )
{
...
#ifdef DARKINTERVAL // DI wants world surfaces and props to emit blood if hit
	else if (iMaterial == CHAR_TEX_ALIENFLESH)
	{
		QAngle vecAngles;
		VectorAngles(-shotDir, vecAngles);
		DispatchParticleEffect("blood_impact_yellow_01", vecOrigin, vecAngles);
	}
	else if (iMaterial == CHAR_TEX_FLESH)
	{
		QAngle vecAngles;
		VectorAngles(-shotDir, vecAngles);
		DispatchParticleEffect("blood_impact_red_01", vecOrigin, vecAngles);
	}
	else if (iMaterial == CHAR_TEX_ICE)
	{
		FX_GlassImpact(vecOrigin, tr.plane.normal);
	}
	else if (iMaterial == CHAR_TEX_GLASS)
	{
		FX_GlassImpact(vecOrigin, tr.plane.normal);
	}
#endif
...
#ifdef DARKINTERVAL
		|| ( iMaterial == CHAR_TEX_SNOW) ) // DI also wants snow to kick up white dust, which it does not in HL2 or Episodes
#endif
	{
	...
	//	This is the effect used on metal brushes and metal static props, but not metal physics props.
...
#ifdef DARKINTERVAL // for static Hydra biomass
	else if (iMaterial == CHAR_TEX_HYDRA)
	{
		QAngle vecAngles;
		VectorAngles(-shotDir, vecAngles);
		DispatchParticleEffect("blood_impact_blue_01", vecOrigin, vecAngles);
	}
#endif
...
// VMT-defined leaking effect from hitting pipes and such. 
// Inspired by the 2003 leak mechanic, but uses modern particle systems.
// Input   :
// Output  :
//------------------------------------------------------------------------------
void LeakEffect(trace_t &tr)
{
	...
	IMaterialVar	*pLeakPS = pTraceMaterial->FindVar("$leakparticlesystem", &found, false);
	//IMaterialVar	*pLeakColorVar = pTraceMaterial->FindVar("$leakcolor", &found);
	//IMaterialVar	*pLeakColorVar2 = pTraceMaterial->FindVar("$leakcolor2", &found);
	if (!found)
		return;

	if (pLeakVar->GetIntValue() == 0)
		return;
	else
	{
		if (RandomInt(0, 3) == 0)
		{
			...
			DispatchParticleEffect(pLeakPS->GetStringValue(), tr.endpos, angles);

		//	StopParticleEffects(dynamic_cast<C_BaseEntity*>(tr.m_pEnt));
		}
	}

	/*
	else if (pLeakVar->GetIntValue() == 1)
	{
		QAngle angles;
		VectorAngles(tr.plane.normal, angles);
		//Vector color;
		//Vector color2;
		DispatchParticleEffect("steam_jet_50", tr.endpos, angles);// , pLeakColorVar->GetVecValue(color.Base(), 3), pLeakColorVar2->GetVecValue(color2.Base(), 3), true, NULL, 2);
	}
	else if (pLeakVar->GetIntValue() == 2)
	{
		QAngle angles;
		VectorAngles(tr.plane.normal, angles);
		Vector color;
		Vector color2;
		DispatchParticleEffect("steam_jet_80", tr.endpos, angles);// , pLeakColorVar->GetVecValue(color.Base(), 3), pLeakColorVar2->GetVecValue(color2.Base(), 3), true, NULL, 2);
	}
	*/
...
/*
	...
	pLeak->m_flStopEmitTime = CURTIME + 5.0;
	pLeak->SetNextClientThink(CURTIME + 20.0);
*/
}
#endif // DARKINTERVAL