...
#define DETAIL_LEGACY_FIXUP
#endif
#ifdef DETAIL_LEGACY_FIXUP
//#define DETAIL_SPRITE_MATERIAL_ALT1		"detail/detailsprites_xen"
#define DETAIL_SPRITE_MATERIAL_HL2		"detail/detailsprites_hl2" // used for legacy fixup on HL2 maps, so they don't get broken Ep2 grass
#endif

#define DETAIL_SPRITES_FLASHLIGHT // make flashlight work on detail sprites
//-----------------------------------------------------------------------------
// forward declarations
//-----------------------------------------------------------------------------
struct model_t;
#ifdef DETAIL_SPRITES_FLASHLIGHT // make flashlight work on detail sprites
static ConVar cl_detail_flashlight_value("cl_detail_flashlight_value", "0.15");
static ConVar cl_detail_flashlight_dot("cl_detail_flashlight_dot", "0.9");
static ConVar cl_detail_flashlight_dist("cl_detail_flashlight_dist", "700");
static ConVar cl_detail_light_styles("cl_detail_light_styles", "0");

#define FLASHLIGHT_ILLUM_MAGIC_NUMBER	cl_detail_flashlight_value.GetFloat()
#define FLASHLIGHT_ILLUM_DOT			cl_detail_flashlight_dot.GetFloat()
#define FLASHLIGHT_ILLUM_DIST			cl_detail_flashlight_dist.GetFloat()

ConVar cl_detaildist( "cl_detaildist", "3600", 0, "Distance at which detail props are no longer visible" );
ConVar cl_detaildist_new("cl_detaildist_new", "3600", 0, "Distance at which detail props are no longer visible");
ConVar cl_detail_models_gap("cl_detail_models_gap", "300", 0, "Detail props which are models will fade earlier than sprites, at distance of {max detail dist} - {this convar's value}");
ConVar cl_detaildist_override("cl_detaildist_override", 0);
static ConVar cl_detail_max_sway( "cl_detail_max_sway", "10", FCVAR_ARCHIVE, "Amplitude of the detail prop sway" );
static ConVar cl_detail_use_cross("cl_detail_use_cross", "1", FCVAR_NONE);
static ConVar cl_detail_use_tri("cl_detail_use_tri", "1", FCVAR_NONE);
ConVar cl_detail_use_legacy_fixup("cl_detail_use_legacy_fixup", "0", 0, "0 = use material detailsprites.vmt, 1 = use detailsprites_hl2.vmt. Setting this to 1 makes grass appear as normal in HL2 campaign levels (done automatically during level load for older levels)");
#else
...
static ConVar mat_fullbright( "mat_fullbright", "0" ); // hook into engine's cvars..
#ifdef DARKINTERVAL
...
public:
	...
	{
#ifdef DARKINTERVAL
		m_flDefaultFadeStart = 400;
		m_flDefaultFadeEnd = cl_detaildist_new.GetFloat();
#else
...
#ifdef DARKINTERVAL
	CMaterialReference m_DetailSpriteMaterial_hl2;
#endif
	...
	int orientation)
{
	m_Color = lighting;
#ifdef DARKINTERVAL
if (cl_detail_light_styles.GetBool())
#endif
{
...
#ifdef DARKINTERVAL // brithen detail models
	float addedLight = 0.0f;

	if (m_Type == DETAIL_PROP_TYPE_MODEL) addedLight = 0.5f;
#endif
	Vector tmp;
	Vector normal( 1, 0, 0);
//	engine->ComputeLighting(m_Origin, &normal, false, tmp);
	engine->ComputeDynamicLighting(m_Origin, &normal, tmp );

	float val = engine->LightStyleValue( 0 );
#ifdef DARKINTERVAL
	color[0] = MIN(1.0f, addedLight + tmp[0] + val * TexLightToLinear( m_Color.r, m_Color.exponent ));
	color[1] = MIN(1.0f, addedLight + tmp[1] + val * TexLightToLinear( m_Color.g, m_Color.exponent ));
	color[2] = MIN(1.0f, addedLight + tmp[2] + val * TexLightToLinear( m_Color.b, m_Color.exponent ));
#else
...
#endif
		{
			...
			if (iInfo != gm_LightStylesMap.InvalidIndex())
			{
				...
				for (int i = 0; i < nLightStyles; ++i)
				{
					...
					{
#ifdef DARKINTERVAL
						color[0] += MIN(1.0f, addedLight + val * TexLightToLinear(lighting.m_Lighting.r, lighting.m_Lighting.exponent));
						color[1] += MIN(1.0f, addedLight + val * TexLightToLinear(lighting.m_Lighting.g, lighting.m_Lighting.exponent));
						color[2] += MIN(1.0f, addedLight + val * TexLightToLinear(lighting.m_Lighting.b, lighting.m_Lighting.exponent));
#else
...
void CDetailModel::DrawSprite( CMeshBuilder &meshBuilder )
{
	...
	case DETAIL_PROP_TYPE_SHAPE_CROSS:
	{
		if( cl_detail_use_cross.GetBool())
...
void CDetailModel::DrawTypeSprite( CMeshBuilder &meshBuilder )
{
...
#ifndef DETAIL_SPRITES_FLASHLIGHT
	Vector vecColor;
	...
	if ( m_pAdvInfo )
	{
		...
		if ( flSwayAmplitude > 0 )
		{
			...
			vecSway += dx * sin(CURTIME+m_Origin.x) * flSwayAmplitude;
		}
	}
#endif
...
#ifdef DETAIL_SPRITES_FLASHLIGHT
	Vector vecColor;
	GetColorModulation(vecColor.Base());

	// After we get colour modulation from the lightmap, see if player
	// has the flashlight on. 
	C_BasePlayer *player = C_BasePlayer::GetLocalPlayer();
	// if yes, brighten the mesh while attenuating based on distance.
	if (player && player->IsEffectActive(EF_DIMLIGHT))
	{
		Vector vHeadDir;
		player->EyeVectors(&vHeadDir);
		Vector lookTargetDir = vecOrigin - player->EyePosition();
		VectorNormalize(lookTargetDir);
		float maxDist = player->EyePosition().DistToSqr(vecOrigin);

		// Illuminate the mesh if player is looking the right way and isn't too far away
		float dotPr = DotProduct(lookTargetDir, vHeadDir);
		//	if (dotPr < FLASHLIGHT_ILLUM_DOT) dotPr *= dotPr;
		if (dotPr < 0.0f) dotPr = 0.0f;
		float illumCoeff = FLASHLIGHT_ILLUM_MAGIC_NUMBER;
		if (dotPr > FLASHLIGHT_ILLUM_DOT && maxDist < FLASHLIGHT_ILLUM_DIST * FLASHLIGHT_ILLUM_DIST)
		{
			vecColor += illumCoeff; // (FLASHLIGHT_ILLUM_MAGIC_NUMBER * dotPr * dotPr) * (FLASHLIGHT_ILLUM_DIST / sqrt(maxDist));
		}
	}

	// Don't let it get too bright - messes up the colours.
	if (vecColor[0] > 1.0) vecColor[0] = 1.0;
	if (vecColor[1] > 1.0) vecColor[1] = 1.0;
	if (vecColor[2] > 1.0) vecColor[2] = 1.0;
	...
	color[2] = (unsigned char)(vecColor[2] * 255.0f);
	color[3] = m_Alpha;
#endif // DETAIL_SPRITES_FLASHLIGHT
...
void CDetailModel::DrawTypeShapeCross( CMeshBuilder &meshBuilder )
{
	...
	if ( flSwayAmplitude > 0 )
	{
		vecSway += UTIL_YawToVector( m_pAdvInfo->m_flSwayYaw ) * sin(CURTIME+m_Origin.x) * flSwayAmplitude;
	}
...
void CDetailModel::DrawTypeShapeTri( CMeshBuilder &meshBuilder )
{
	...
	while( iDrawn < 3 )
	{
		...
		Vector vecSway = ( m_pAdvInfo->m_vecCurrentAvoid * flWidth ) + 
			vecSwayYaw * sin(CURTIME+m_Origin.x+iBranch) * flSwayAmplitude;
...
void CDetailObjectSystem::LevelInitPreEntity()
{
	...
	m_DetailSpriteMaterial.Init( DETAIL_SPRITE_MATERIAL, TEXTURE_GROUP_OTHER );
#ifdef DETAIL_LEGACY_FIXUP
	m_DetailSpriteMaterial_hl2.Init(DETAIL_SPRITE_MATERIAL_HL2, TEXTURE_GROUP_OTHER);
#endif
...
#ifdef DETAIL_LEGACY_FIXUP
		PrecacheMaterial(DETAIL_SPRITE_MATERIAL_HL2);
	//	PrecacheMaterial(DETAIL_SPRITE_MATERIAL_ALT1);
#endif
...
{
#ifdef DARKINTERVAL
	// Check map name. If it's a HL2 map, fixup detail texture name. 
	char map_full_name[256];
	Q_strncpy(map_full_name, engine->GetLevelName(), sizeof(map_full_name));
	Q_FixSlashes(map_full_name);
	Q_strlower(map_full_name);

	char map_short_name[64];
	Q_StripExtension(map_full_name, map_short_name, sizeof(map_short_name));
	const char *material_name_fixup = DETAIL_SPRITE_MATERIAL;
//	else // otherwise check if there's custom override in world settings. (HL2 never does that)
	{
		C_World *pWorld = GetClientWorldEntity();
		if (pWorld && pWorld->GetDetailSpriteMaterial() && *(pWorld->GetDetailSpriteMaterial()))
		{
			material_name_fixup = pWorld->GetDetailSpriteMaterial();
		}
	}
	m_DetailSpriteMaterial.Init( material_name_fixup, TEXTURE_GROUP_OTHER );
	m_DetailSpriteMaterial_hl2.Init(DETAIL_SPRITE_MATERIAL_HL2, TEXTURE_GROUP_OTHER);
	...
	Q_strlower(mapname);
	ConColorMsg(Color(175, 255, 25, 255), "Detail system initiated for %s, material %s\n", mapname, cl_detail_use_legacy_fixup.GetBool() ? m_DetailSpriteMaterial_hl2->GetName() : m_DetailSpriteMaterial->GetName());
#else
...
#ifdef DETAIL_LEGACY_FIXUP
	m_DetailSpriteMaterial_hl2.Shutdown();
#endif
	if ( m_pFastSpriteData )
	{
		MemAlloc_FreeAligned( m_pFastSpriteData );
			...
			switch( lump.m_Type )
			{
				...
				case DETAIL_PROP_TYPE_SHAPE_TRI:
				{
					for( int i=0;i<SPRITE_MULTIPLIER;i++)
					{
						...
						++detailObjectCount;
#ifdef DARKINTERVAL // todo: what does this do?
						Vector debugpos = pos;
						debugpos.z += 8.0;
#endif
					}
...
#else
	float flDetailDist = cl_detaildist_new.GetFloat();
	if (!cl_detaildist_override.GetBool())
	{
		C_World *pWorld = GetClientWorldEntity();
		if (pWorld)
		{
			if (pWorld->GetDetailDist() != NULL)
				flDetailDist = pWorld->GetDetailDist();
		}
	}
#endif
	flMaxSqDist = flDetailDist * flDetailDist;
#ifdef DARKINTERVAL
	flFadeSqDist = flDetailDist - 800;
#else
...
#ifdef DETAIL_LEGACY_FIXUP
	m_DetailSpriteMaterial = DETAIL_SPRITE_MATERIAL;
	IMaterial *pMaterial = m_DetailSpriteMaterial;

	if (cl_detail_use_legacy_fixup.GetBool())
	{
		m_DetailSpriteMaterial_hl2 = DETAIL_SPRITE_MATERIAL_HL2;
		pMaterial = m_DetailSpriteMaterial_hl2;
	}
#else
	IMaterial *pMaterial = m_DetailSpriteMaterial;
#endif
...
void CDetailObjectSystem::BuildDetailObjectRenderLists( const Vector &vViewOrigin )
{
	...
	m_flCurMaxSqDist = flDetailDist * flDetailDist;
	m_flCurFadeSqDist = flDetailDist - 800;
#else
