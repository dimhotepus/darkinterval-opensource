...
void CViewEffects::CalcShake( void )
{
	...
	for ( int nShake = nShakeCount - 1; nShake >= 0; nShake-- )
	{
		...
		if ( CURTIME > pShake->nextShake )
		{
			// Higher frequency means we recalc the extents more often and perturb the display again
			pShake->nextShake = CURTIME + (1.0f / pShake->frequency);
		...
		fraction = ( pShake->endtime - CURTIME ) / pShake->duration;
...
void CViewEffects::Shake( ScreenShake_t &data )
{
	if ( ( data.command == SHAKE_START || data.command == SHAKE_START_RUMBLEONLY ) && ( m_ShakeList.Count() < MAX_SHAKES ) )
	{
		...
		pNewShake->endtime = CURTIME + data.duration;
...
void CViewEffects::FadeCalculate( void )
{
	...
	for (i = iSize-1; i >= 0; i-- )
	{
		...
		{
			pFade->Reset = CURTIME + 0.1;
		}

		// All done?
		if ( ( CURTIME > pFade->Reset ) && ( CURTIME > pFade->End ) )
		{
