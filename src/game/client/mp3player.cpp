...
static void mp3_f()
{
	...
	{
		Warning("No mp3 player was found!\n");
	}
#endif
}
...
void CMP3Player::PlaySong( int songIndex, float skipTime /*= 0.0f */ ) 
{
	...
	enginesound->EmitAmbientSound(
		...
		PITCH_NORM,
		0,
		skipTime == 0.0f ? 0.0f : ( CURTIME + skipTime  ) ); 
...
public:
	...
	{
		if( g_pPlayer) g_pPlayer->EnableAutoAdvance( true );
	}
	...
	virtual void LevelShutdownPostEntity()
	{
		...
		if ( engine->IsConnected() )
		{
			if (g_pPlayer) g_pPlayer->EnableAutoAdvance( false );
		}
	}
};
