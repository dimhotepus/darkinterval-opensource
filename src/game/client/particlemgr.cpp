...
ConVar r_DrawParticles("r_drawparticles", "1", FCVAR_NOTIFY, "Enable/disable particle rendering");
...
CParticleEffectBinding::CParticleEffectBinding()
{
	...
	m_LastMin = m_Min;
	m_LastMax = m_Max;
#ifdef DARKINTERVAL // fix for unitialised value
	...
	bool bWireframe )
{
	...
	if( bWireframe )
	{
#ifdef DARKINTERVAL // I believe the particlewireframe material is missing by default, resulting in cherkerboads when enabling mat_wireframe
		IMaterial *pMaterial = m_pParticleMgr->m_pMaterialSystem->FindMaterial( "debug/debugwireframe", TEXTURE_GROUP_OTHER );
#else
...
{
	g_pParticleSystemMgr->SetLastSimulationTime( CURTIME );
}
