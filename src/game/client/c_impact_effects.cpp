...
#ifdef DARKINTERVAL
	g_Mat_BloodPuff[2] = ParticleMgr()->GetPMaterial( "effects/bloodcore");
#endif
...
static void CreateFleckParticles( const Vector& origin, const Vector &color, trace_t *trace, char materialType, int iScale )
{
	...
	case CHAR_TEX_WOOD:
		...
		break;
#ifdef DARKINTERVAL // normally, these flecks only get produced on breakable entities, but DI wants them on world surfaces too, for extra visual reactivity.
	case CHAR_TEX_GLASS:
...
#ifndef DARKINTERVAL // it also gives tiles its own effects, more pronounced and with ceramic shards.
	case CHAR_TEX_TILE:
#endif
	default:
...
#ifdef DARKINTERVAL // for some reason using random particle breaks it, even though this is how it's designed to work
		pParticle = (Particle3D *) pGlassEmitter->AddParticle( sizeof(Particle3D), g_Mat_Fleck_Glass[1], pos );
#else
...
void FX_GaussExplosion( const Vector &pos, const Vector &dir, int type )
{
	...
	dl->color.r = 255;
	dl->color.g = 190;
	dl->color.b = RandomInt(75, 95);
	dl->color.exponent = 2.0f;

	dl->radius = random->RandomInt(140, 170);
