...
void CAchievementNotificationPanel::FireGameEvent( IGameEvent * event )
{
	...
	if ( 0 == Q_strcmp( name, "achievement_event" ) )
	{
...
#ifndef DARKINTERVAL // DI's achievements are local, so they are permitted without steam
		if ( IsPC() )
#else
		if( 0 )
#endif
		...
		{
			// show our own achievement progress UI
...
{
	if ( ( m_flHideTime > 0 ) && ( m_flHideTime < CURTIME ) )
	{
		m_flHideTime = 0;
...
{
	return ( ( m_flHideTime > 0 ) && ( m_flHideTime > CURTIME ) && CHudElement::ShouldDraw() );
}
...
void CAchievementNotificationPanel::ShowNextNotification()
{
	...
	m_flHideTime = CURTIME + ACHIEVEMENT_NOTIFICATION_DURATION;
...
CON_COMMAND_F( achievement_notification_test, "Test the hud notification UI", 0  )
{
	...
	if ( pPanel )
	{		
		pPanel->AddNotification( "ACHIEVEMENT_DI_TEST", L"Achievement Progress", ( 0 == ( iCount % 2 ) ? L"Test Notification Message A (1/10)" :
			L"Test Message B" ) );
	}
