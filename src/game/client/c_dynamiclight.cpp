...
void C_DynamicLight::OnDataChanged(DataUpdateType_t updateType)
{
	...
	{
		SetNextClientThink(CURTIME + 0.05);
	}
...
void C_DynamicLight::Release()
{
	if (m_pDynamicLight)
	{
		m_pDynamicLight->die = CURTIME;
		m_pDynamicLight = 0;
	}
	
	if (m_pSpotlightEnd)
	{
		m_pSpotlightEnd->die = CURTIME;
		m_pSpotlightEnd = 0;
	}
...
void C_DynamicLight::ClientThink(void)
{
	...
	if ( (m_Flags & DLIGHT_NO_MODEL_ILLUMINATION) == 0 )
	{
		...
		m_pDynamicLight->die = CURTIME + 1e6;
...
#endif
	{
		...
		else
		{
			...
			m_pSpotlightEnd->die		= CURTIME + 1e6;
	...
	SetNextClientThink(CURTIME + 0.001);
}
