...
#include "portal/C_BaseHLPlayer.h"
...
#ifdef PORTAL2
	C_BaseHLPlayer *pPlayer = GetPortalPlayer();
#endif
...
void SixenseInput::SwitchViewModes( CUserCmd *pCmd )
{
	...
	C_BaseHLPlayer *pPortalPlayer = C_BaseHLPlayer::GetLocalBaseHLPlayer();
...
void SixenseInput::SixenseUpdateKeys( float flFrametime, CUserCmd *pCmd )
{
	...
	if ( sixense_features_enabled.GetInt() )
	{
		...
		if ( !sixense_dist_one_to_one_enabled.GetInt() && sixense_auto_one_to_one_enabled.GetInt() &&
			IsHoldingObject() &&
			!m_bIs1to1ModeScaling &&
			( C_BaseHLPlayer::GetLocalBaseHLPlayer() &&
			!C_BaseHLPlayer::GetLocalBaseHLPlayer()->IsScalingUseItem() &&
			!C_BaseHLPlayer::GetLocalBaseHLPlayer()->IsScalingUseItemTurret() ) )
		{
			// one to one
			...
			!C_BaseHLPlayer::GetLocalBaseHLPlayer()->IsScalingUseItemTurret() ) )
		{
			...
			if ( !IsInOneToOneMode() &&
				( C_BaseHLPlayer::GetLocalBaseHLPlayer() && C_BaseHLPlayer::GetLocalBaseHLPlayer()->GetActiveWeapon() ) &&
