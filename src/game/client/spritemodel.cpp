...
#ifdef DARKINTERVAL // without this, game crashes as soon as a sprite with missing material comes into view. 
					// Bizarre that it never checks for the material and assumes it exists.
	Assert(pMaterial);
	if (!pMaterial)
	{
		Warning("Failed CEngineSprite::GetMaterial()!!! Returning NULL to safely exit the function.\n");
		return NULL;
	}
#endif
