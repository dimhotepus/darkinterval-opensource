...
ConVar hud_draw_fixed_reticle("hud_draw_fixed_reticle", "0"
#ifndef DARKINTERVAL
	, FCVAR_ARCHIVE 
#endif
);
...
#ifdef DARKINTERVAL // todo: implement it
	AUTOAIM_METHOD_CLASSIC // Half-Life 1 style: move the *crosshair*, do not affect camera *angles*
#endif
};
...
#endif
	return ( (hud_autoaim_method.GetInt() ==
#ifdef DARKINTERVAL
		AUTOAIM_METHOD_CLASSIC || 
#endif
		hud_draw_fixed_reticle.GetBool() 
		|| hud_draw_active_reticle.GetBool()) 
	...
	case AUTOAIM_METHOD_CLASSIC:
	{
		...
		if (crosshair)
		{
			if (pLocalPlayer->m_HL2Local.m_hAutoAimTarget.Get())
			{
				...
				crosshair->SetCrosshairAngle(QAngle(5, 0, 0));
			}
			else
			{
				crosshair->SetCrosshairAngle(vec3_angle);
			}
		}
	}
	break;
