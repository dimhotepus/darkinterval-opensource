...
void C_MortarShell::AddRisingParticles( float flPerc )
{
	...
	float val = RemapValClamped( CURTIME, m_flStarttime, m_flStarttime + m_flLifespan, 0.0f, 1.0f );
...
{
	float val = RemapValClamped( CURTIME, m_flStarttime + m_flLifespan, m_flStarttime + m_flLifespan + 1.0f, 1.0f, 0.0f );
...
int C_MortarShell::DrawModel( int flags )
{
	...
	if ( CURTIME < ( m_flStarttime + m_flLifespan ) )
	{
		flPerc = GetStartPerc();
		ending = false;
	}
