...
int	C_StriderFX::DrawModel( int )
{
	...
	{
		float dt = m_limitHitTime - CURTIME;
		if ( dt < 0 )
		{
			dt = 0;
		}
...
{
#if 0
	Vector RopeVecMins, RopeVecMaxs;
	...
	RopeVecMins.Init(FLT_MAX, FLT_MAX, FLT_MAX);
	RopeVecMaxs.Init(-FLT_MAX, -FLT_MAX, -FLT_MAX);
	...
	for (int i = 0; i < set->numhitboxes; i++)
	{
		...
		VectorMin(RopeVecMins, vecBoxAbsMins, RopeVecMins);
		VectorMax(RopeVecMaxs, vecBoxAbsMaxs, RopeVecMaxs);
	}
	// UNDONE: Disabled this until we can get closer to a final map and tune
	// Cut ropes.
	if ( CURTIME >= m_flNextRopeCutTime )
	{
		// Blow the bbox out a little.
		Vector vExtendedMins = m_vecRenderMins - Vector( 50, 50, 50 );
		Vector vExtendedMaxs = m_vecRenderMaxs + Vector( 50, 50, 50 );

		C_RopeKeyframe *ropes[512];
		int nRopes = C_RopeKeyframe::GetRopesIntersectingAABB( ropes, ARRAYSIZE( ropes ), GetLocalOrigin() + vExtendedMins, GetLocalOrigin() + vExtendedMaxs );
		for ( int i=0; i < nRopes; i++ )
		{
			Msg("Strider intersected rope %i\n", i);
		...
		m_flNextRopeCutTime = CURTIME + 0.5;
	}
#endif
