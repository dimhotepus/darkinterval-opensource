...
bool CHudRadar::Init( KeyValues* pKeyValues, VGuiScreenInitData_t* pInitData )
{
	...
	m_ghostAlpha = 0;
	m_flTimeStartGhosting = CURTIME + 1.0f;
...
void CHudRadar::AddRadarContact( const Vector &vecOrigin, int iType, float flTimeToLive )
{
	...
	if( iExistingContact > -1 )
	{
		...
		m_radarContacts[iExistingContact].m_flTimeToRemove = CURTIME + flTimeToLive;
		return;
	}

	m_radarContacts[m_iNumRadarContacts].m_vecOrigin = vecOrigin;
	m_radarContacts[m_iNumRadarContacts].m_iType = iType;
	m_radarContacts[m_iNumRadarContacts].m_flTimeToRemove = CURTIME + flTimeToLive;
	m_iNumRadarContacts++;
}
...
void CHudRadar::MaintainRadarContacts()
{
	...
	while( bKeepWorking )
	{
		...
		for( int i = 0 ; i < m_iNumRadarContacts ; i++ )
		{
			...
			if( CURTIME >= pContact->m_flTimeToRemove )
			{
...
void CHudRadar::Paint()
{
	...
	if( CURTIME > m_flTimeStartGhosting )
	{
		...
		else
		{
			...
			m_flTimeStopGhosting = CURTIME + RandomFloat( 1.0f, 2.0f );// How long to ghost for
		}
	}
	else if( CURTIME > m_flTimeStopGhosting )
	{
		...
		else
		{
			...
			m_flTimeStartGhosting = CURTIME + RandomFloat( 2.0f, 3.0f );// how long between ghosts
	...
	for( int i = 0 ; i < m_iNumRadarContacts ; i++ )
	{
		int alpha = 90;
		CRadarContact *pContact = &m_radarContacts[ i ];
		float deltaT = pContact->m_flTimeToRemove - CURTIME;
