...
#ifndef DARKINTERVAL // sprint doesn't cost energy anymore and doesn't show up on HUD.
	bool sprintActive = pPlayer->IsSprinting();
#endif
	bool breatherActive = pPlayer->IsBreatherActive();
	int activeDevices = (int)flashlightActive +
#ifndef DARKINTERVAL
		(int)sprintActive + 
#endif
		(int)breatherActive;
...
{
#ifdef DARKINTERVAL // needed for some reason?
	if (!ShouldDraw())
		return;
#endif
	C_BaseHLPlayer *pPlayer = (C_BaseHLPlayer *)C_BasePlayer::GetLocalPlayer();
