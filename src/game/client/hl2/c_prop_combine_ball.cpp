...
int C_PropCombineBall::DrawModel( int flags )
{
	...
	else
	{
		...
		float sinOffs = 1.0f * sin( CURTIME * 25 );
...
/*	
	...
	dl->color.g = 250;
	dl->color.b = 235;
	dl->color.exponent = 0.2f;

	dl->radius = random->RandomInt(400, 500);
...
#ifdef DARKINTERVAL // used for creating combine ball magnets
class C_CombineBallAttractor : public C_BaseAnimating
{
	DECLARE_CLASS(C_CombineBallAttractor, C_BaseAnimating);
public:
	C_CombineBallAttractor() {}
	...
	int InternalDrawModel(int flags)
	{
		if (IsFollowingEntity() && m_boneIndexAttached >= 0)
		{
			...
			if (follow)
			{
				...
				Vector absOrigin;
				MatrixGetColumn(temp, 3, absOrigin);
...
IMPLEMENT_CLIENTCLASS_DT(C_CombineBallAttractor, DT_CombineBallAttractor, CCombineBallAttractor)
