...
void C_ScriptIntro::ClientThink( void )
{
	...
	{
		flPerc = clamp( (CURTIME - m_flBlendStartTime) / (m_flNextBlendTime - m_flBlendStartTime), 0, 1 );
	}
	...
	if ( m_flNextBlendTime >= CURTIME )
	{
		...
		Msg("				 curtime %.2f    StartedAt %.2f    FinishAt: %.2f\n", CURTIME, m_flBlendStartTime, m_flNextBlendTime );
		Msg("				 Perc:   %.2f\n", flPerc );
	}
	*/
...
void C_ScriptIntro::CalculateFOV( void )
{
	...
	if ( m_flNextFOVBlendTime >= CURTIME )
	{
...
void C_ScriptIntro::CalculateAlpha( void )
{
	...
	float flNewAlpha = RemapValClamped( CURTIME, m_flFadeTimeStartedAt, m_flFadeTimeStartedAt + m_flFadeDuration, m_flFadeAlphaStartedAt, m_flFadeAlpha );
	/*
	if ( m_IntroData.m_flCurrentFadeColor[3] != flNewAlpha )
	{
		Msg("INTRO FADING: curtime %.2f    StartedAt %.2f    Duration: %.2f\n", CURTIME, m_flFadeTimeStartedAt, m_flFadeDuration );
		Msg("           TimePassed %.2f    Alpha:    %.2f\n", RemapValClamped( CURTIME, m_flFadeTimeStartedAt, m_flFadeTimeStartedAt + m_flFadeDuration, 0.0, 1.0 ), m_IntroData.m_flCurrentFadeColor[3] );
	}
	*/
