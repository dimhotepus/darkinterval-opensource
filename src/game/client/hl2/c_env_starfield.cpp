...
{		
	if ( updateType == DATA_UPDATE_CREATED )
	{
		...
		Vector vecCenter = MainViewOrigin() + (MainViewForward() * 
#ifndef DARKINTERVAL
			cl_starfield_distance.GetFloat()
#else
			256.0f 
#endif
			);
...
void C_EnvStarfield::ClientThink( void )
{
...
#ifdef DARKINTERVAL // don't let stars accumulate in pause
		|| engine->IsPaused() 
#endif
		)
		return;
	...
	for ( int i = 0; i < iNumParticles; i++ )
	{
		float flDiameter = 
#ifndef DARKINTERVAL
			cl_starfield_diameter.GetFloat()
#else
			128.0f
#endif
			;

		Vector vecStart = vecViewOrigin + (MainViewForward() * 
#ifndef DARKINTERVAL
			cl_starfield_distance.GetFloat()
#else
			256.0f 
