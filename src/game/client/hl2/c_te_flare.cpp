...
public:
	...
	float	m_flScale;
	bool	m_bLight;
#ifndef DARKINTERVAL // DI uses server-side smoke for the flares
	bool	m_bSmoke;
#endif
	bool	m_bPropFlare;
...
void C_Flare::Update( float timeDelta )
{
	...
	if ( ( m_flTimeBurnOut != -1.0f ) && ( ( m_flTimeBurnOut - CURTIME ) <= 10.0f ) )
	{
		baseScale *= ( ( m_flTimeBurnOut - CURTIME ) / 10.0f );
	}
	...
	if ( !bVisible )
	{
		if ( m_pParticle[0] != NULL )
		{	
			m_pParticle[0]->m_flDieTime		= CURTIME;
		...
		if ( m_pParticle[1] != NULL )
		{	
			m_pParticle[1]->m_flDieTime		= CURTIME;
	...
	if ( m_bLight )
	{
		...
		if ( m_bPropFlare == false )
		{
			...
			dl->die		= CURTIME + 0.1f;
#ifdef DARKINTERVAL
			dl->radius = baseScale * random->RandomFloat(256.0f, 320.0f);
#else
...
#ifdef DARKINTERVAL // made smaller in DI, otherwise it casts bad-looking lighting on static props in several maps
				dl->radius = baseScale * random->RandomFloat(110.0f, 128.0f);
#else
...
#ifdef DARKINTERVAL // made smaller in DI, otherwise it casts bad-looking lighting on static props in several maps
				el->radius	= baseScale * random->RandomFloat( 110.0f, 128.0f );
#else
	...
	if ( m_bSmoke )
	{
		while ( m_teSmokeSpawn.NextEvent( dt ) )
		{
			...
			sParticle->m_uchStartAlpha	= random->RandomInt( 160, 220 );
			sParticle->m_uchEndAlpha	= 0;
			sParticle->m_uchStartSize	= random->RandomInt( 6, 12 );
