		...
		if ( i == m_CreditsList.Count()-1 )
		{
			if ( m_bLastOneInPlace == false )
			{
				...
				if ( (int)pCredit->flYPos + ( iFontTall / 2 ) <= iTall / 2 )
				{
					...
					m_flFadeTime = CURTIME + ( IsConsole() ? 2.0f : 10.0f );
				}
			}
			else
			{
				if ( m_flFadeTime <= CURTIME )
				{
					if ( m_Alpha > 0 )
					{
...
void CHudCredits::DrawLogo( void )
{
	...
	switch( m_iLogoState )
	{
		case LOGO_FADEIN:
		{
			float flDeltaTime = ( m_flFadeTime - CURTIME );
			...
			if ( flDeltaTime <= 0.0f )
			{
				...
				m_flFadeTime = CURTIME + m_flLogoDesiredLength;
			}
...
void CHudCredits::DrawIntroCreditsName( void )
{
	...
	for ( int i = 0; i < m_CreditsList.Count(); i++ )
	{
		...
		float localTime = CURTIME - pCredit->flTimeStart;
		...
		if ( m_flLogoTime > CURTIME )
			 continue;
		
		if ( pCredit->flTime - m_flNextStartTime <= CURTIME )
		{
			if ( m_CreditsList.IsValidIndex( i + 3 ) )
			{
				...
				if ( pNextCredits && pNextCredits->flTime == 0.0f )
				{
					...
					if ( i < 3 )
					{
						...
						pNextCredits->flTime = CURTIME + m_flFadeInTime + m_flFadeOutTime + m_flFadeHoldTime + pNextCredits->flTimeAdd;
					}
					else
					{
					...
					pNextCredits->flTimeStart = CURTIME;
...
void CHudCredits::PrepareIntroCredits( void )
{
	...
	for ( int i = 0; i < m_CreditsList.Count(); i++ )
	{
		...
		if ( i < 3 )
		{
			pCredit->bActive = true;
			pCredit->iSlot = iSlot;
			pCredit->flTime = CURTIME + m_flFadeInTime + m_flFadeOutTime + m_flFadeHoldTime;
			pCredit->flTimeStart = CURTIME;
