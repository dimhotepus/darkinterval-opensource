...
C_PropAirboat::C_PropAirboat()
{
	...
	m_pHeadlight = NULL;
#ifdef DARKINTERVAL
	m_ViewSmoothingData.flPitchCurveZero = 25.0f;
	m_ViewSmoothingData.flPitchCurveLinear = 60.0f;
	m_ViewSmoothingData.flRollCurveZero = 90.0f;
	m_ViewSmoothingData.flRollCurveLinear = 120.0f;
#else
...
int C_PropAirboat::GetJoystickResponseCurve() const
{
	return 
		...
		1
#else 
		airboat_joy_response_move.GetInt()
#endif
		;
}
...
void C_PropAirboat::UpdateViewAngles( C_BasePlayer *pLocalPlayer, CUserCmd *pCmd )
{
...
#endif
	{
...
#ifdef DARKINTERVAL
		if ( m_flViewAngleDeltaTime > 1.5f )
#else
...
#endif
		{
			...
			QAngle outAngles;
#ifdef DARKINTERVAL
			InterpolateAngles(pCmd->viewangles, vehicleEyeAngles, outAngles, 0.03f);
#else
...
void C_PropAirboat::UpdateWake( void )
{
	...
	if ( ( pLast == NULL ) || ( pLast->m_vecScreenPos.DistToSqr( screenPos ) > 4.0f ) )
	{
		...
		pNewPoint->m_flDieTime	= CURTIME + WAKE_LIFETIME;
	...
	m_flUpdateTime = CURTIME + ( 0.5f / (float) MAX_WAKE_POINTS );
}
...
#define	WAKE_STEPS	6
	...
	for ( int i = 0; i < WAKE_STEPS; i++ )
	{
		...
		scale = scaleRange + ( 8.0f * sin( CURTIME * 5 * i ) );
...
int C_PropAirboat::DrawWake( void )
{
	...
	currentPoint.m_flDieTime = CURTIME + 0.5f;
	...
	for ( int i = 0; i <= m_nStepCount; ++i )
	{
		...
		float flLifePerc = RemapValClamped( ( pPoint->m_flDieTime - CURTIME ), 0, WAKE_LIFETIME, 0.0f, 1.0f );
