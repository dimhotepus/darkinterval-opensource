	...
	if ( m_bActive && pPlayer )
	{
...
#ifndef DARKINTERVAL // while we have both LDR and HDR in maps, there's no commentary that cares about this... right now, anyway.
...
#ifndef DARKINTERVAL // DI NOTE - we don't want or need this at all as our commentaries play SCRIPT SOUNDS, not plain wavs.
		...
		if ( pHudCloseCaption )
		{
			...
			if ( closecaption.GetBool() || ( !closecaption.GetBool() && !english.GetBool() ) )
			{
				...
				pHudCloseCaption->ProcessCaption(m_iszCommentaryFile, flDuration);
...
void CHudCommentary::Paint()
{
	...
	float flPercentage = clamp( ( CURTIME - m_flStartTime ) / flDuration, 0.f, 1.f );
...
void CHudCommentary::StartCommentary( C_PointCommentaryNode *pNode, char *pszSpeakers, int iNode, int iNodeMax, float flStartTime, float flEndTime )
{
	...
	if ( fabs(flStartTime - CURTIME) < 1.0 )
	{
