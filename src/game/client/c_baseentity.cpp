...
#ifdef DARKINTERVAL
static ConVar  cl_distort_fade_dist("cl_distort_fade_dist", "1000", 0);
#endif
...
static ConVar  r_drawrenderboxes( "r_drawrenderboxes", "0", FCVAR_NOTIFY);
...
void C_BaseEntity::UnlinkFromHierarchy()
{
	...
	// This was causing problems with the local network backdoor with entities coming in and out of the PVS at certain times.
...
int CBaseEntity::BaseInterpolatePart1( float &currentTime, Vector &oldOrigin, QAngle &oldAngles, Vector &oldVel, int &bNoMoreChanges )
{
	...
	if ( GetPredictable() || IsClientCreated() )
	{
		...
		if ( localplayer && currentTime == CURTIME )
		{
...
void C_BaseEntity::BaseInterpolatePart2( Vector &oldOrigin, QAngle &oldAngles, Vector &oldVel, int nChangeFlags )
{
	...
	if ( index == 1 )
	{
		SpewInterpolatedVar( &m_iv_vecOrigin, CURTIME, GetInterpolationAmount( LATCH_SIMULATION_VAR ), true );
	}
#endif
}
...
void C_BaseEntity::CreateLightEffects( void )
{
	...
	{			
		...
		dl->origin = GetAbsOrigin();
#ifdef DARKINTERVAL
		dl->color.r = 250;  dl->color.g = 150; dl->color.b = 100;
#else
		...
		el->color.r = 250;  dl->color.g = 150; dl->color.b = 100;
		el->radius = random->RandomFloat(200, 231);
		el->die = CURTIME + 0.001;
#endif
	}
#ifdef DARKINTERVAL
	if (IsEffectActive(EF_VERYDIMLIGHT))
	{
		dl = effects->CL_AllocDlight(index);
		dl->origin = WorldSpaceCenter();
		dl->color.r = 10;  dl->color.g = 10; dl->color.b = 13;
		dl->color.exponent = 0.1f;
		dl->radius = random->RandomFloat(300, 320);
		dl->die = CURTIME + 0.001;

		el = effects->CL_AllocElight(index);
		el->origin = WorldSpaceCenter();
		el->color.r = 10;  dl->color.g = 10; dl->color.b = 13;
		el->color.exponent = 0.1f;
		el->radius = random->RandomFloat(300, 320);
		el->die = CURTIME + 0.001;
	}
	if (IsEffectActive(EF_CREMATORLIGHT))
	{
		...
		dl->color.b = 70;
		dl->color.exponent = 5;
		dl->radius = random->RandomFloat(300, 340);
		dl->die = CURTIME + 0.001;
		...
		el->radius = random->RandomFloat(300, 340);
		el->die = CURTIME + 0.001;
	}
/*	
	if (IsEffectActive(EF_EP3LIGHT))
	{
		dl = effects->CL_AllocDlight(index);
		dl->origin = GetAbsOrigin();
		dl->color.r = 50;
		dl->color.g = 250;
		dl->color.b = 140;
		dl->color.exponent = 0.5;
		dl->radius = random->RandomFloat(50, 64);
		dl->die = CURTIME + 0.001;
		...
		el->color.g = 250;
		el->color.b = 70;
		el->color.exponent = 0.25;
		el->radius = random->RandomFloat(50, 64);
		el->die = CURTIME + 0.001;
	}
*/
	if (IsEffectActive(EF_PSTORMLIGHT))
	{
		dl = effects->CL_AllocDlight(index);
		dl->origin = GetLocalOrigin();
		dl->color.r = 50;
		dl->color.g = 250;
		dl->color.b = 70;
		dl->color.exponent = 8.0;
		dl->radius = random->RandomFloat(300, 400);
		dl->die = CURTIME + 0.01;

		el = effects->CL_AllocElight(index);
		el->origin = GetLocalOrigin();
		el->color.r = 50;
		el->color.g = 250;
		el->color.b = 70;
		el->color.exponent = 4.0;
		el->radius = random->RandomFloat(300, 400);
		el->die = CURTIME + 0.01;
	}
#endif
}
...
void C_BaseEntity::ProcessInterpolatedList()
{
	...
	for ( int iCur=g_InterpolationList.Head(); iCur != g_InterpolationList.InvalidIndex(); iCur=iNext )
	{
		...
		pCur->m_bReadyToDraw = pCur->Interpolate( CURTIME );
	}
}
...
void C_BaseEntity::ComputeFxBlend( void )
{
	...
	case kRenderFxPulseSlowWide:
		blend = m_clrRender->a + 0x40 * sin( CURTIME * 2 + offset );	
		break;
		
	case kRenderFxPulseFastWide:
		blend = m_clrRender->a + 0x40 * sin( CURTIME * 8 + offset );
		break;
	...
	case kRenderFxPulseFast:
		blend = m_clrRender->a + 0x10 * sin( CURTIME * 8 + offset );
		break;
	...
	case kRenderFxStrobeFast:
		blend = 20 * sin( CURTIME * 16 + offset );
		if ( blend < 0 )
		{
			blend = 0;
		}
	...
	case kRenderFxStrobeFaster:
		blend = 20 * sin( CURTIME * 36 + offset );
		if ( blend < 0 )
		{
			blend = 0;
		}
	...
	case kRenderFxFlickerFast:
		blend = 20 * (sin( CURTIME * 16 ) + sin( CURTIME * 23 + offset ));
		if ( blend < 0 )
		{
			blend = 0;
		}
	...
	case kRenderFxDistort:
		{
			...
			else 
			{
				SetRenderColorA( 180 );
#ifdef DARKINTERVAL
				if ( dist <= cl_distort_fade_dist.GetInt() )
#else
				if (dist <= 100)
#endif
					blend = m_clrRender->a;
...
#ifdef DARKINTERVAL
					blend = (int)((1.0 - (dist - cl_distort_fade_dist.GetInt()) * (1.0 / 400.0)) * m_clrRender->a);
#else
...
#ifdef DARKINTERVAL
	bool showthis = false; //cl_showerror.GetInt() >= 2;
#else
...
void C_BaseEntity::EstimateAbsVelocity( Vector& vel )
{
	...
	CInterpolationContext context;
	context.EnableExtrapolation( true );
	m_iv_vecOrigin.GetDerivative_SmoothVelocity( &vel, CURTIME );
}
