...
#ifdef DARKINTERVAL	
	length /= 2;
	if (length > 100) length = RandomFloat( 800.0f, 1000.0f);
#else
...
#ifdef DARKINTERVAL
	float life = (length) / 100000;	// NOTENOTE: We want the tail to finish its run as well
	FX_AddDiscreetLine(start, shotDir, 100000, length, length, 0.1f, life, "effects/tracer_base");
#else
	//Create the line
	CFXStaticLine	*t;
...
void FX_TracerSound( const Vector &start, const Vector &end, int iTracerType )
{
	...
	// Is it time yet?
	float dt = g_BulletWhiz.m_nextWhizTime - CURTIME;
	if ( dt > 0 )
		return;
	...
	g_BulletWhiz.m_nextWhizTime = CURTIME + random->RandomFloat( flMinWhizTime, flMaxWhizTime );
}
...
void FX_Tracer( Vector& start, Vector& end, int velocity, bool makeWhiz )
{
	...
	{
#ifdef DARKINTERVAL
		float length = random->RandomFloat( 220.0f, 300.0f );
#else
		...
		//Add it
#ifdef DARKINTERVAL
		FX_AddDiscreetLine( start, dir, velocity, length, dist, random->RandomFloat( 0.1f, 0.25f ), life / 2, "effects/tracer_base" );
#else
