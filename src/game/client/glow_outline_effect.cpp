...
#ifdef DARKINTERVAL
			ApplyEntityGlowEffects( pSetup, nSplitScreenSlot, pRenderContext, 32.0f, nX, nY, nWidth, nHeight );
#else
...
#ifdef DARKINTERVAL // more consistent with other paths
	pMatGlowColor = materials->FindMaterial( "custom_uni/glow_color", TEXTURE_GROUP_OTHER, true );
#else
...
void CGlowObjectManager::ApplyEntityGlowEffects( const CViewSetup *pSetup, int nSplitScreenSlot, CMatRenderContextPtr &pRenderContext, float flBloomScale, int x, int y, int w, int h )
{
...
#ifdef DARKINTERVAL
	IMaterial *pMatGlowColor = materials->FindMaterial( "custom_uni/glow_color", TEXTURE_GROUP_OTHER, true );
#else
	...
	
	{
		...
		//=======================================================================================================//
#ifdef DARKINTERVAL
		IMaterial *pMatHaloAddToScreen = materials->FindMaterial( "custom_uni/halo_add_to_screen", TEXTURE_GROUP_OTHER, true );
#else
