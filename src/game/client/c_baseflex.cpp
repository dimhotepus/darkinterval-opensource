...
//
// Purpose: 
// DI note: here a slightly older version of c_baseflex was dropped in 
// to solve the broken blinking flex (years and years ago). There's
// probably a more graceful and economic way of doing it, but that
// costs time which is a premium. So we're staying with this verison
// for now.
// $NoKeywords: $
//===========================================================================//
#include "cbase.h"
...
bool GetHWMExpressionFileName(const char *pFilename, char *pHWMFilename)
{
	...
	if (!UseHWMorphVCDs())
		return false;
#ifndef DARKINTERVAL // we're not using hardware morphs, the rest of the code isn't necessary to compile in
...
bool C_BaseFlex::GetSoundSpatialization(SpatializationInfo_t& info)
{
	...
	if (bret)
	{
#ifdef DARKINTERVAL // todo - consider that it's better to have on both chan voices?
...
void C_BaseFlex::ProcessVisemes(Emphasized_Phoneme *classes)
{
	...
	for (int source = 0; source < MouthInfo().GetNumVoiceSources(); source++)
	{
		...
		float dt = 0.08f; // g_CV_PhonemeFilter.GetFloat();
...
void C_BaseFlex::GetToolRecordingState(KeyValues *msg)
{
...
#ifdef DARKINTERVAL
		m_blinktime = CURTIME + 0.2f;
#else
		m_blinktime = CURTIME + g_CV_BlinkDuration.GetFloat();
#endif
	}
	...
	// blink the eyes
#ifdef DARKINTERVAL
	float t = (m_blinktime - CURTIME) * M_PI * 0.5 * (1.0 / 0.2f);
#else
	float t = ( m_blinktime - CURTIME ) * M_PI * 0.5 * ( 1.0 / g_CV_BlinkDuration.GetFloat() );
#endif
	if (t > 0)
	{
...
void C_BaseFlex::SetupWeights(const matrix3x4_t *pBoneToWorld, int nFlexWeightCount, float *pFlexWeights, float *pFlexDelayedWeights)
{
	...
	{
		m_prevblinktoggle = m_blinktoggle;
		m_blinktime = CURTIME + 0.2f; // g_CV_BlinkDuration.GetFloat();
	}
	...
	// FIXME: this needs a better algorithm
	// blink the eyes
	float flBlinkDuration = 0.2f; // g_CV_BlinkDuration.GetFloat();
	float flOOBlinkDuration = (flBlinkDuration > 0) ? 1.0f / flBlinkDuration : 0.0f;
	float t = (m_blinktime - CURTIME) * M_PI * 0.5 * flOOBlinkDuration;
	if (t > 0)
	{
		// do eyeblink falloff curve
		t = cos(t);
...
{
	// process the delayed version of the flexweights
	if ( flFlexDelayTime > 0.0f && flFlexDelayTime < CURTIME )
	{
		float d = clamp(CURTIME - flFlexDelayTime, 0.0, gpGlobals->frametime);
