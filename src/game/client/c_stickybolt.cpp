...
//			after being hit by a crossbow bolt.
//
// $Workfile:     $
// $Date:         $
//
...
public:
	...
	IterationRetval_t EnumElement( IHandleEntity *pHandleEntity )
	{
		...
		if ( pModel == NULL )
			return ITERATION_CONTINUE;
#ifdef DARKINTERVAL // Ragdolls that are pinned by the bolt don't accept any further bolts. This fixes the funny HL2 oversight that lets you make ragdolls go crazy fighting constraints.
		if (pEnt->GetEFlags() & EF_NOCROSSBOWHIT)
			return ITERATION_CONTINUE;
#endif
		trace_t tr;
...
#ifdef DARKINTERVAL // Ragdolls that are pinned by the bolt don't accept any further bolts. This fixes the funny HL2 oversight that lets you make ragdolls go crazy fighting constraints.
			pEnt->AddEFlags(EF_NOCROSSBOWHIT);
#endif
