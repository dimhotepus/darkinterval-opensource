...
void C_Fish::ClientThink()
{
	...
	switch( m_localLifeState )
	{
		...
		case LIFE_DEAD:
		{
			...
			m_angles.z += rollAmp * sin( rollFreq * (CURTIME + 10.0f * entindex()) ) * deltaT;

			const float rollAmp2 = 7.0f;
			const float rollFreq2 = 4.0f;
			m_angles.x += rollAmp2 * sin( rollFreq2 * (CURTIME + 10.0f * entindex()) ) * deltaT;

			const float bobAmp = 0.75f;
			const float bobFreq = 4.0f;
			m_vel.z += bobAmp * sin( bobFreq * (CURTIME + 10.0f * entindex()) ) * deltaT;

			const float bobAmp2 = 0.75f;
			const float bobFreq2 = 3.333f;
			m_vel.z += bobAmp2 * sin( bobFreq2 * (CURTIME + 10.0f * entindex()) ) * deltaT;
