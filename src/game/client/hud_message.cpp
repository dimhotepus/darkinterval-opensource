...
void CHudMessage::Paint()
{
	...
	{
		float localTime = CURTIME - m_gameTitleTime;
		float brightness;

		// Maybe timer isn't set yet
		if ( m_gameTitleTime > CURTIME )
		{
			m_gameTitleTime = CURTIME;
		}
		...
		{
#ifdef DARKINTERVAL // I believe this has to do with an obscure and silly crash that happens if the old, HL1
					// title texture is missing, as original code makes no check and just assumes it exists
			if (m_iconTitleHalf != NULL && m_iconTitleLife != NULL)
			{
#endif
	...
	for ( i = 0; i < maxHUDMessages; i++ )
	{
		...
		if ( m_pMessages[i] )
		{
			...
			if ( m_startTime[i] > CURTIME )
				m_startTime[i] = CURTIME + m_parms.time - m_startTime[i] + 0.2;	// Server takes 0.2 seconds to spawn, adjust for this
		}
	}
	...
	for ( i = 0; i < maxHUDMessages; i++ )
	{
		if ( m_pMessages[i] )
		{
			...
			if ( CURTIME <= endTime )
			{
				float messageTime = CURTIME - m_startTime[i];
