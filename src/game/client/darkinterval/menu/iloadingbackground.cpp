...
void CMapLoadBG::ApplySchemeSettings(IScheme *pScheme)
{
	...
	if (m_pLoadingTip)
	{
		...
		m_pLoadingTip->SetPos(iWide / 2 - m_pLoadingTip->GetWide() / 2, iTall * 0.8);
	}
#endif
...
void CMapLoadBG::OnMessage(const KeyValues *params, VPANEL fromPanel)
{
	...
	if (m_pLoadingTip)
	{
		...
		m_pLoadingTip->SizeToContents();
		m_pLoadingTip->SetPos(ScreenWidth() / 2 - m_pLoadingTip->GetWide() / 2, ScreenHeight() * 0.8); // FIXME: without it, the labels is misaligned even though it should be a_center?
	}
#endif
