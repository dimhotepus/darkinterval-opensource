...
IMPLEMENT_CLIENTCLASS_DT(C_NPC_Hydra, DT_NPC_Hydra_Ken, CNPC_Hydra_Ken)
...
void C_NPC_Hydra::PostDataUpdate(DataUpdateType_t updateType)
{
	if (m_bSetup)
...
void C_NPC_Hydra::OnLatchInterpolatedVariables(int flags)
{
	if (flags & LATCH_ANIMATION_VAR)
	{
		if (m_bSetup)
		{
			...
			{
				m_iv_vecPos[i].NoteChanged(CURTIME, true);
			}
		}
	}
...
void  C_NPC_Hydra::StandardBlendingRules(CStudioHdr* hdr, Vector pos[], Quaternion q[], float currentTime, int boneMask)
{
	...
	if (m_numHydraBones != hdr->numbones())
	{
		...
		for (i = 0; i < m_numHydraBones; i++)
		{
			...
			m_iv_vecPos[i].SetInterpolationAmount(0.2f);
		}
		...
		{
			m_boneLength[i] = 10.0f; // (pos[i + 1] - pos[i]).Length();
