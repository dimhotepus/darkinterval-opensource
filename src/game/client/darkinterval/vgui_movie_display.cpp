...
#include "darkinterval/c_movie_display.h"
...
CMovieDisplayScreen::CMovieDisplayScreen(vgui::Panel *parent, const char *panelName)
	: BaseClass(parent, "CMovieDisplayScreen", vgui::scheme()->LoadSchemeFromFileEx(enginevgui->GetPanel(PANEL_CLIENTDLL), "resource/ui/MovieDisplayScreen.res", "Movie_Display_Screen"))
{
...
bool CMovieDisplayScreen::BeginPlayback(const char *pFilename)
{
	...
	{
		Q_snprintf(szMaterialName, sizeof(szMaterialName), "%s_%s", pFilename, m_hScreenEntity->GetEntityName()); // look for networking m_iName in c_baseentity and baseentity to get this to work.
	}
