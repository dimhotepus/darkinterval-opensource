//==========================================================================//
//
// Purpose: VGUI Keypad made by Maestra Fenix 2015
//
// $NoKeywords: $
//===========================================================================//
#include "cbase.h"
#if 0 // DI REVISIT if we ever get back to these!
...
//	m_pBackgroundPanel->SetImage(scheme()->GetImage("keypad/keypad_base", false));
	m_pBackgroundPanel->SetSize(300, 450);
	...
	m_pLabel->DrawBox(0, 0, GetWide(), GetTall(), Color(0, 150, 255), 150);
...
{
//	vgui::surface()->DrawSetColor(255, 255, 0, 75);	
}
...
void CVgui_keypad::Paint()
{
	...
	vgui::surface()->DrawOutlinedRect(4, 4, GetWide()-4, GetTall()-4);
	vgui::surface()->DrawSetColor(50, 50, 50, 200);
	vgui::surface()->DrawFilledRect(5, 5, GetWide() - 5, GetTall() - 5);
...
void CVgui_keypad::CheckPass(int number)
{
	...
	if (m_pHidePass)
	{
		//SysOp: so fluffing lazy, blame me
		if (m_pPressedNumbers == 1)
			m_pLabel->SetText("*");
