...
ConVar sv_noclipduringpause( "sv_noclipduringpause", "0", FCVAR_REPLICATED | FCVAR_NOTIFY, "If cheats are enabled, then you can noclip with the game paused (for doing screenshots, etc.)." );
...
#ifdef DARKINTERVAL // early on there were plans for a hotkey-based locator, but currently it is always on once it's been added to the HEV
static	kbutton_t	in_locator;
#endif
...
#ifdef DARKINTERVAL // early on there were plans for a hotkey-based locator, but currently it is always on once it's been added to the HEV
void IN_LocatorUp(const CCommand &args) {KeyUp(&in_locator, args[1]);}
void IN_LocatorDown(const CCommand &args) { KeyDown(&in_locator, args[1]); }
#endif
...
{
#ifndef DARKINTERVAL // no thanks, DI intentionally wants klook to be accessible whenever the player wants to
...
#ifndef PORTAL_COMPILE	// Don't constrain Roll in Portal because the player can be upside down! -Jeep // DI todo - consider this useful for DI in general
...
#ifdef DARKINTERVAL // early on there were plans for a hotkey-based locator, but currently it is always on once it's been added to the HEV
	CalcButtonBits( bits, IN_LOCATOR, s_ClearInputState, &in_locator, bResetState);
#endif
...
static ConCommand impulse("impulse", IN_Impulse, "Available impulse commands are:\n     50 - enter squad command mode;\n     51 - give an item_dynamic_resupply;\n     52 - rangefinder (print position and distance to crosshair);\n     81 - give weapon_cubemap;\n     82 - spawn HL2 buggy with the tau cannon;\n     83 - spawn the airboat;\n     100 - toggle flashlight;\n     101 - give 25 health, suit, and all weapons;\n     102 - spawn a skull gib prop;\n     103 - report AI state for NPC under crosshair;\n     106 - report name and model of entity under crosshair;\n     107 - report texture under crosshair;\n     108 - enable debug mode for NPC under crosshair;\n     200 - hide active weapon (until reset by zoom, pickup, vehicles, etc;\n     201 - spray the player logo under crosshair;\n     202 - spawn blood under crosshair;\n     203 - remove entity under crosshair (careful, may remove invisible entities in front of visible ones too);\n 800 - spawn footprint decal under player.\n");
...
#ifdef DARKINTERVAL // early on there were plans for a hotkey-based locator, but currently it is always on once it's been added to the HEV
static ConCommand showlocator("+locator", IN_LocatorUp);
static ConCommand hidelocator("-locator", IN_LocatorDown);
#endif
