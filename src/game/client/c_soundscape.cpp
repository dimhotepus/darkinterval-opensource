...
#if SOUNDSCAPE_SUBTITLES
		else if (!Q_strcasecmp(pKey->GetName(), "subtitles"))
		{
			KeyValues *pWavesSub = pKey->GetFirstSubKey();
			sound.pWavesSub = pWavesSub;
		}
#endif
...
//		this prevents pops
#ifdef DARKINTERVAL
ConVar soundscape_reset_positioned_looping_sounds("soundscape_reset_positioned_looping_sounds", "1");
#endif
...
#ifdef DARKINTERVAL
					if(soundscape_reset_positioned_looping_sounds.GetBool())
#endif
						StopLoopingSound(sound);
					pSoundSlot = &sound;
...
int C_SoundscapeSystem::AddRandomSound( const randomsound_t &sound )
{
	...
	m_randomSounds[index].nextPlayTime = CURTIME + 0.5 * RandomInterval( sound.time );
...
void C_SoundscapeSystem::PlayRandomSound( randomsound_t &sound )
{
...
#if SOUNDSCAPE_SUBTITLES
	bool bHaveSubtitle = true;
	KeyValues *pWavesSub = sound.pWavesSub;
	if (!pWavesSub)
	{
	//	Warning("no sub for rndwave\n");
		bHaveSubtitle = false;
	}
#endif
	while ( waveId > 0 && pWaves )
	{
		pWaves = pWaves->GetNextKey();
#if SOUNDSCAPE_SUBTITLES
		if(bHaveSubtitle && pWavesSub) pWavesSub = pWavesSub->GetNextKey();
#endif
		waveId--;
	}
	if ( !pWaves )
...
#if SOUNDSCAPE_SUBTITLES
	const char *pWaveSubName = ""; 
	if(bHaveSubtitle) pWaveSubName = pWavesSub->GetString();
	if (!pWaveSubName) bHaveSubtitle = false;
#endif
	...
	if (bHaveSubtitle)
	{
		// ignore "none" key, lets us have selected sounds inside rndwave without a subtitle
		if (!Q_strcmp(pWaveSubName, "none"))
		{
			return;
		}
		...
		const char	*subName;
		char		token[128];
		Q_strncpy(token, pWaveSubName, sizeof(token));

		// Extract the subtitle name
		subName = strtok(token, " ");
		
		// Extract the subtitle duration
		float subLength = atof(nexttoken(token, pWaveSubName, ' '));
		if (subLength == NULL) subLength = 1.0f;

		//Msg("Name: %s Length: %.1f\n", subName, subLength);

		// Emit the subtitle.
		clientdll->EmitCloseCaption(subName, subLength);
	}
#endif
}
