...
#ifdef DARKINTERVAL
ConVar r_jigglebones("r_jigglebones", "1");
ConVar draw_nameplates("di_draw_nameplates", "1", FCVAR_REPLICATED);

extern ConVar item_blink;
...
#ifdef DARKINTERVAL
	RecvPropString(RECVINFO(m_nameplate_string)) // DI NEW: meshBuilder based text strings on models
#endif
END_RECV_TABLE()
...
#ifdef DARKINTERVAL
	DEFINE_FIELD(m_nameplate_save, FIELD_STRING),
#endif
...
{
#ifdef DARKINTERVAL
	m_nameplate_save = m_nameplate_string; // write given nameplate string into save data
#endif
}
...
void C_ClientRagdoll::OnRestore( void )
{
	...
	SetupBones( NULL, -1, BONE_USED_BY_ANYTHING, CURTIME );
...
#ifdef DARKINTERVAL
	// restore nameplate information
	if( draw_nameplates.GetBool())
		SetNameplateText(m_nameplate_save);
#endif
	BaseClass::OnRestore();
	...
	switch ( m_iFrictionAnimState )
	{
		case RAGDOLL_FRICTION_NONE:
		{
			...
			if ( m_iMinFriction != 0 || m_iMaxFriction != 0 )
			{
				...
				m_flFrictionTime = CURTIME + m_flFrictionModTime;
		...
		{
			float flDeltaTime = (m_flFrictionTime - CURTIME);
...
#ifdef DARKINTERVAL
	// DI NEW: Live Ragdolls Experiment (inspired by Dark Messiah)
	if( m_live_ragdoll_ent)	m_live_ragdoll_ent = NULL;
#endif
}
...
#ifdef DARKINTERVAL
//-----------------------------------------------------------------------------
// Purpose: DI NEW: meshBuilder based text strings on models
//-----------------------------------------------------------------------------
void C_BaseAnimating::SetNameplateText(string_t newString)
{
	string_t string = newString;
	if (string == NULL) newString = "";
	Q_strncpy(m_nameplate_string, newString, sizeof(m_nameplate_string));
}
#endif
// FIXME: redundant?
...
#ifdef DARKINTERVAL
	// DI NEW: Live Ragdolls Experiment (inspired by Dark Messiah)
	bool hasLiveRagdoll = m_live_ragdoll_ent.Get() && m_live_ragdoll_ent.Get()->GetBaseAnimating();
#endif
...
#ifdef DARKINTERVAL		// DI NEW: Live Ragdolls Experiment (inspired by Dark Messiah)
		if (hasLiveRagdoll && boneComputed.IsBoneMarked(i))
		{			
			Vector bonePos;
			QAngle boneAng;
			m_live_ragdoll_ent.Get()->GetBaseAnimating()->GetBonePosition(i, bonePos, boneAng);
			AngleMatrix(boneAng, bonePos, GetBoneForWrite(i));
			continue;
		}
#endif
	}
	...
	case kRenderFxExplode:
		{
			...
			scale = 1.0 + (CURTIME - m_flAnimTime) * 10.0;
...
void C_BaseAnimating::CreateUnragdollInfo( C_BaseAnimating *pRagdoll )
{
	...
	pRagdoll->SetupBones( NULL, -1, BONE_USED_BY_ANYTHING, CURTIME );
...
#ifdef DARKINTERVAL
//	pRagdoll->SaveRagdollInfo( numbones, parentTransform, m_BoneAccessor );
#endif
	if ( !m_pRagdollInfo )
	{
	...
	m_pRagdollInfo->m_flSaveTime = CURTIME;
...
void C_BaseAnimating::MaintainSequenceTransitions( IBoneSetup &boneSetup, float flCycle, Vector pos[], Quaternion q[] )
{
...
#ifdef DARKINTERVAL
	if (IsAboutToRagdoll()) // ragdoll fix / deathposes fix
	{
		m_nPrevNewSequenceParity = m_nNewSequenceParity;
		return;
	}
	...
	for (int i = m_SequenceTransitioner.m_animationQueue.Count() - 2; i >= 0; i--)
	{
		...
		float dt = (CURTIME - blend->m_flLayerAnimtime);
		...
		{
			DevMsgRT( "%8.4f : %30s : %5.3f : %4.2f  +\n", CURTIME, boneSetup.GetStudioHdr()->pSeqdesc( blend->m_nSequence ).pszLabel(), flCycle, (float)blend->m_flWeight );
		}
#endif

		boneSetup.AccumulatePose( pos, q, blend->m_nSequence, flCycle, blend->m_flWeight, CURTIME, m_pIk );
	}
}
...
void C_BaseAnimating::StandardBlendingRules( CStudioHdr *hdr, Vector pos[], Quaternion q[], float currentTime, int boneMask )
{
	...
	// DI NEW: Live Ragdolls Experiment (inspired by Dark Messiah)
	int i;
	for (i = 0; i < hdr->numbones(); i++) // output our current bones info for another entity to use
	{
		m_liveragdoll_out_bone_pos[i] = pos[i];
		m_liveragdoll_out_bone_quat[i] = q[i];
	}
#endif
#endif // DARKINTERVAL
...
bool C_BaseAnimating::CalcAttachments()
{
	...
	return SetupBones( NULL, -1, BONE_USED_BY_ATTACHMENT, CURTIME );
}
...
#ifdef DARKINTERVAL
	// DI NEW: Live Ragdolls Experiment (inspired by Dark Messiah)
//	if (m_live_ragdoll_ent.Get())
//	{
//		*info.pOrigin = m_live_ragdoll_ent.Get()->WorldSpaceCenter();
//	} // commented out - crashes the game at certain times right now
#endif
	return true;
}
...
CBoneCache *C_BaseAnimating::GetBoneCache( CStudioHdr *pStudioHdr )
{
	...
	SetupBones( NULL, -1, boneMask, CURTIME );
	C_BaseAnimating::PopBoneAccess( "GetBoneCache" );

	if ( pcache )
	{
		// still in memory but out of date, refresh the bones.
		pcache->UpdateBones( m_CachedBoneData.Base(), pStudioHdr->numbones(), CURTIME );
	}
	else
	{
...
ConVar r_forcedrawothermodels("r_forcedrawothermodels", "0", FCVAR_NONE, "Turn on if you want dynamic models to appear while building cubemaps.");

static ConVar nameplate_step_var("nameplate_step", "0.03125");
static ConVar nameplate_ratio("nameplate_ratio", "3");

int c_to_n(char c)
{
	int n = -1;
	char * const alphabet = " ABCDEFGHIJKLMNOPQRSTUVWXYZ-.|"; // DI NOTE: this is specific to the texture we're using
...
int C_BaseAnimating::DrawModel( int flags )
{
	...
	if (!m_bReadyToDraw)
	{
	//	Msg("not ready to draw entity %s\n", GetClassname());
		return 0;
	}
	int drawn = 0;
...
#ifdef DARKINTERVAL
		|| r_forcedrawothermodels.GetBool() 
#endif
		)
	{
		MDLCACHE_CRITICAL_SECTION();
...
#ifdef DARKINTERVAL		////////
		if (m_nameplate_string != NULL_STRING && draw_nameplates.GetBool())
		{
			int NUM_LETTERS = 11;

			Vector	topRight_1, topLeft_1, bottomRight_1, bottomLeft_1;
			QAngle angles;

			GetAttachment(LookupAttachment("nameplate_1_top_left"), topLeft_1, angles);
			GetAttachment(LookupAttachment("nameplate_1_top_right"), topRight_1, angles);
			GetAttachment(LookupAttachment("nameplate_1_bottom_left"), bottomLeft_1, angles);
			GetAttachment(LookupAttachment("nameplate_1_bottom_right"), bottomRight_1, angles);

			Vector start_top = topLeft_1;
			Vector end_top = topRight_1;
			Vector dir_top = end_top - start_top;

			Vector start_bot = bottomLeft_1;
			Vector end_bot = bottomRight_1;
			Vector dir_bot = end_bot - start_bot;

			float nameplate_step = nameplate_step_var.GetFloat();

			IMaterial *pMaterial = materials->FindMaterial("sprites/texfont", NULL, false);
			...
			meshBuilder.Begin(pMesh, MATERIAL_QUADS, NUM_LETTERS);

			for (int i = 0; i < NUM_LETTERS; i++)
			{
				int j = c_to_n(m_nameplate_string[i]);

				////
				meshBuilder.Position3fv((start_bot + dir_bot * nameplate_step * nameplate_ratio.GetFloat() * i).Base());
				meshBuilder.Color4fv(color);
				meshBuilder.TexCoord2f(0, nameplate_step * j, 1);
				meshBuilder.AdvanceVertex();

				meshBuilder.Position3fv((start_top + dir_top * nameplate_step * nameplate_ratio.GetFloat() * i).Base());
				meshBuilder.Color4fv(color);
				meshBuilder.TexCoord2f(0, nameplate_step * j, 0);
				meshBuilder.AdvanceVertex();

				meshBuilder.Position3fv((start_top + dir_top * nameplate_step * nameplate_ratio.GetFloat() * (i + 1)).Base());
				meshBuilder.Color4fv(color);
				meshBuilder.TexCoord2f(0, nameplate_step * (j + 1), 0);
				meshBuilder.AdvanceVertex();

				meshBuilder.Position3fv((start_bot + dir_bot * nameplate_step * nameplate_ratio.GetFloat() * (i + 1)).Base());
				meshBuilder.Color4fv(color);
				meshBuilder.TexCoord2f(0, nameplate_step * (j + 1), 1);
				meshBuilder.AdvanceVertex();
			}
...
void C_BaseAnimating::ProcessMuzzleFlashEvent()
{
	...
	if (m_Attachments.Count() > 0)
	{
		...
		if (true) //(muzzleflash_a_b_testing.GetBool())
		{
		...
		else
		{
			...
			dl->radius = RandomInt(50, 75);
			dl->decay = dl->radius * 2.0f;
			dl->die = CURTIME + 0.05f;
			dl->color.r = 255;
			dl->color.g = 160;
			dl->color.b = 64;
			dl->color.exponent = 5;
...
#ifdef DARKINTERVAL
		else if (Q_stricmp(token, "REVOLVER") == 0)
		{
			weaponType = MUZZLEFLASH_357;
		}
#endif
...
{
	*r = m_fGlowColorRed; // di_glow_color_r.GetFloat();//0.76f;
	*g = m_fGlowColorGreen; // di_glow_color_g.GetFloat();//0.76f;
	*b = m_fGlowColorBlue; // di_glow_color_b.GetFloat();//0.76f;
}
...
#ifdef DARKINTERVAL
	case AE_CL_CREATE_DLIGHT:
	{
		int iAttachment = -1;
		char token[256];
		int r, g, b;
		float e, radius, life, decay;
		...
		if (token)
		{
			...
			if (token[0] != '0' && iAttachment == 0)
			{
				...
				if (iAttachment <= 0)
				{
					Warning("Failed to find attachment point specified for dlight anim event. Trying to spawn dlight on entity '%s' on attachment named '%s'\n", GetDebugName(), token);
					return;
				}
			}
		}
		...
		Vector pos;
		GetAttachment(iAttachment, pos);
		te->DynamicLight(filter, 0, &pos, r, g, b, e, radius, life, decay);
	}
	break;
#endif // DARKINTERVAL
	default:
...
const Vector& C_BaseAnimating::GetRenderOrigin( void )
{
	...
	if (m_live_ragdoll_ent.Get())
	{
		return m_live_ragdoll_ent.Get()->GetRenderOrigin();
	}
	if ( IsRagdoll() )
	{
...
const QAngle& C_BaseAnimating::GetRenderAngles( void )
{
	...
	if (m_live_ragdoll_ent.Get())
	{
		return m_live_ragdoll_ent.Get()->GetRenderAngles();
	}
#endif
	if ( IsRagdoll() )
	{
...
void C_BaseAnimating::GetRagdollInitBoneArrays( matrix3x4_t *pDeltaBones0, matrix3x4_t *pDeltaBones1, matrix3x4_t *pCurrentBones, float boneDt )
{
	...
	ForceSetupBonesAtTime( pDeltaBones1, CURTIME );
...
{
	// We now create a separate entity that becomes this entity's ragdoll.
...
#ifdef DARKINTERVAL
	pRagdoll->SetNameplateText(m_nameplate_string);
#endif
...
#define MAX_ANIMTIME_INTERVAL 0.2f
	...
	float flInterval = MIN( CURTIME - m_flAnimTime, MAX_ANIMTIME_INTERVAL );
	return flInterval;
}
...
void C_BaseAnimating::StudioFrameAdvance()
{
	...
	//{
		//anim.prevanimtime = m_flAnimTime = CURTIME;
	//}
	...
	if ( watch )
	{
		Msg("%s %6.3f : %6.3f (%.3f)\n", GetClassname(), CURTIME, m_flAnimTime, flInterval );
	}
...
void C_BaseAnimating::GetBlendedLinearVelocity( Vector *pVec )
{
	...
	for (int i = m_SequenceTransitioner.m_animationQueue.Count() - 2; i >= 0; i--)
	{
		...
		float flWeight = blend->GetFadeout( CURTIME );
...
float C_BaseAnimating::FrameAdvance( float flInterval )
{
	...
	if ( bWatch )
	{
		Msg("%i CLIENT Time: %6.3f : (Interval %f) : cycle %f rate %f add %f\n", 
			gpGlobals->tickcount, CURTIME, flInterval, flNewCycle, cyclerate, addcycle );
	}
...
void C_BaseAnimating::SetModelScale( float scale, float change_duration /*= 0.0f*/  )
{
	if ( change_duration > 0.0f )
	{
		...
		mvs->m_flModelScaleStartTime = CURTIME;
...
void C_BaseAnimating::UpdateModelScale()
{
	...
	float frac = ( CURTIME - mvs->m_flModelScaleStartTime ) / dt;
	frac = clamp( frac, 0.0f, 1.0f );

	if ( CURTIME >= mvs->m_flModelScaleFinishTime )
	{
...
void C_BaseAnimating::GetToolRecordingState( KeyValues *msg )
{
	...
	if ( hdr )
	{
		SetupBones( pBones, hdr->numbones(), BONE_USED_BY_ANYTHING, CURTIME );
	}
	else
	{
		SetupBones( NULL, -1, BONE_USED_BY_ANYTHING, CURTIME );
