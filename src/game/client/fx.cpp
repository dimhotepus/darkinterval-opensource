...
#ifdef DARKINTERVAL // for rope sprites
#include "c_rope.h"
#include "tempent.h"
#include "c_te_legacytempents.h"
#endif
...
#ifndef DARKINTERVAL // irrelevant for DI
	if (!ToolsEnabled())
		return;
...
public:
	...
	void CreateSpurtParticles( void )
	{
		...
		m_flLastParticleSpawnTime = CURTIME + m_flSpawnRate;
	}
	...
	virtual void SimulateParticles( CParticleSimulateIterator *pIterator )
	{
		...
		while ( pParticle )
		{
			...
			if ( m_flDeathTime > CURTIME )
			{
				if ( m_flLastParticleSpawnTime <= CURTIME )
				{
...
#ifdef DARKINTERVAL
	FX_GunshipImpact( vecPosition, Vector( 0, 0, 1 ), 15, 75, 100 );
#else
...
int g_nRopeNodeSpriteColor = 0;

// 4 light colors
Color g_rgbaHydraRed(255, 0, 0, 255);
Color g_rgbaHydraYellow(2, 110, 197, 255);
Color g_rgbaHydraGreen(117, 193, 8, 255);
Color g_rgbaHydraBlue(255, 151, 29, 255);

Color *(rgbaRopeNodeSpriteColors[]) = { &g_rgbaHydraRed,
&g_rgbaHydraYellow,
&g_rgbaHydraGreen,
&g_rgbaHydraBlue };

struct RopeNodeSpriteData_t
{
	Vector vOrigin;
	int nID;
	int nSubID;
...
void CreateRopeNodeSprite(const RopeNodeSpriteData_t &RopeNodeSprite);

class CRopeNodeSpriteManager : public CAutoGameSystemPerFrame
{
public:
	CRopeNodeSpriteManager(char const *name);
	...
	void AddRopeNodeSprite(const CEffectData &data);

private:

	CUtlVector< RopeNodeSpriteData_t > m_PendingLightData;
};

CRopeNodeSpriteManager g_CRopeNodeSpriteManager("CRopeNodeSpriteManager");


CRopeNodeSpriteManager::CRopeNodeSpriteManager(char const *name) : CAutoGameSystemPerFrame(name)
{
}

// Methods of IGameSystem
void CRopeNodeSpriteManager::Update(float frametime)
{
	for (int i = 0; i < m_PendingLightData.Count(); ++i)
	{
		CreateRopeNodeSprite(m_PendingLightData[i]);
	}

	m_PendingLightData.RemoveAll();
}

void CRopeNodeSpriteManager::LevelInitPostEntity(void)
{
	m_PendingLightData.RemoveAll();
}

void CRopeNodeSpriteManager::LevelShutdownPreEntity()
{
	m_PendingLightData.RemoveAll();
}

void CRopeNodeSpriteManager::AddRopeNodeSprite(const CEffectData &data)
{
	...
	// Too far away?
	if (pPlayer->GetAbsOrigin().DistTo(data.m_vOrigin) > 1000.0f)
		return;
	
	RopeNodeSpriteData_t newData;
...
void FX_RopeNodeSpriteCallback(const CEffectData &data)
{
	g_CRopeNodeSpriteManager.AddRopeNodeSprite(data);
}

void CreateRopeNodeSprite(const RopeNodeSpriteData_t &RopeNodeSprite)
{
	const model_t *pModel = engine->LoadModel("effects/hydra_cord_glow.vmt");
	if (!pModel)
		return;

	Assert(pModel);

	C_LocalTempEntity *pTemp = tempents->FindTempEntByID(RopeNodeSprite.nID, RopeNodeSprite.nSubID);

	if (!pTemp)			
	{
		// Didn't find one with that ID, so make a new one!
		// Randomize the angle
		QAngle angOrientation = vec3_angle;
		pTemp = tempents->SpawnTempModel(pModel, RopeNodeSprite.vOrigin, angOrientation, vec3_origin, 2.0f, FTENT_NEVERDIE);
		if (!pTemp)
		{
			return;
		}	
		...
		// HACK: Use these ints to ID the light later
		pTemp->m_nSkin = RopeNodeSprite.nID;
		pTemp->hitSound = RopeNodeSprite.nSubID;

		// Skybox lights pass in a smaller scale
		pTemp->m_flSpriteScale = RopeNodeSprite.fScale;

		// Smuggle the color index here
		pTemp->m_nHitboxSet = g_nRopeNodeSpriteColor;

		// Set the color
	//	pTemp->SetRenderColor(rgbaRopeNodeSpriteColors[g_nRopeNodeSpriteColor]->r(),
	//		rgbaRopeNodeSpriteColors[g_nRopeNodeSpriteColor]->g(),
	//		rgbaRopeNodeSpriteColors[g_nRopeNodeSpriteColor]->b(),
	//		rgbaRopeNodeSpriteColors[g_nRopeNodeSpriteColor]->a());
		pTemp->SetRenderColor(180, 110, 170, 100);

		// Next color in the pattern
		g_nRopeNodeSpriteColor = (g_nRopeNodeSpriteColor + 1) % ARRAYSIZE(rgbaRopeNodeSpriteColors);
	...
	{
		// Update the position
		pTemp->SetAbsOrigin(RopeNodeSprite.vOrigin);
	...
	//		int nCycle = (pTemp->hitSound + static_cast< int >(CURTIME * 2.0f)) % (pTemp->m_nHitboxSet + ARRAYSIZE(rgbaRopeNodeSpriteColors) + 1);
	//		pTemp->SetRenderColorA(nCycle < ARRAYSIZE(rgbaRopeNodeSpriteColors) ? 255 : 64);
	//	}
		...
		// Extend it's life
		pTemp->die = CURTIME + 2.0f;
	}
}

DECLARE_CLIENT_EFFECT("FX_RopeNodeSprite", FX_RopeNodeSpriteCallback);
//// !HYDRA CORD GLOW POINTS
#endif // DARKINTERVAL
//-----------------------------------------------------------------------------
// Purpose: 
//-----------------------------------------------------------------------------
...
//			makeWhiz - 
//-----------------------------------------------------------------------------
void FX_GenericBulletTracer(Vector& start, Vector& end, int velocity, bool makeWhiz)
{
	...
	if( totalDist > 256) totalDist /= 2;
	if (totalDist < 256) totalDist = 256;

	float life = (totalDist) / 100000;	// NOTENOTE: We want the tail to finish its run as well

	FX_AddDiscreetLine(start, shotDir, 100000, totalDist, totalDist, 0.1f, life, "effects/tracer_base");
...
//			makeWhiz - 
//-----------------------------------------------------------------------------
void FX_TurretTracer(Vector& start, Vector& end, int velocity, bool makeWhiz)
{
	VPROF_BUDGET("FX_TurretTracer", VPROF_BUDGETGROUP_PARTICLE_RENDERING);
	Vector	vNear, dStart, dEnd, shotDir;
	float	totalDist;
	...
	float life = (totalDist + length) / velocity;	//NOTENOTE: We want the tail to finish its run as well
													...
													//Add it
	FX_AddDiscreetLine(start, shotDir, velocity, length, totalDist, 3.0f, life, "effects/turret_tracer");
...
void FX_GunshipTracer( Vector& start, Vector& end, int velocity, bool makeWhiz )
{
	...
	float length = random->RandomFloat( 128.0f, 256.0f );
	float life = ( totalDist + length ) / velocity;	//NOTENOTE: We want the tail to finish its run as well
#ifdef DARKINTERVAL // for 2003 style purple spark
	//Add it
...
#ifdef DARKINTERVAL // for 2003 style purple spark
	//Add it
	FX_AddDiscreetLine( start, shotDir, velocity, length, totalDist, 2.5f, life, "effects/bluespark" );
#else
