...
#ifdef DARKINTERVAL // DI uses a separate font that can be larger, without changing the debug font
	m_hFont = pScheme->GetFont( "FPSPanelFont" );
#else
...
#ifdef DARKINTERVAL
	int nFPSThreshold1 = 60;
	int nFPSThreshold2 = 30;
#else
	...
	if ( cl_showfps.GetInt() && realFrameTime > 0.0 )
	{
		if ( m_lastRealTime != -1.0f )
		{
...
#ifdef DARKINTERVAL // brevity
				g_pMatSystemSurface->DrawColoredText( m_hFont, x, 2, ucColor[0], ucColor[1], ucColor[2], 255, "%3i fps %.1f ms", nFps, /*m_low, m_high,*/ frameMS/*, engine->GetLevelName()*/ );
#else
			...
			else
			{
				...
				GetFPSColor( nFps, ucColor );
#ifdef DARKINTERVAL // brevity
				g_pMatSystemSurface->DrawColoredText( m_hFont, x, 2, ucColor[0], ucColor[1], ucColor[2], 255, "%3i fps", nFps/*, engine->GetLevelName()*/ );
#else
