...
void C_PhysPropClientside::ClientThink( void )
{
	...
	float alpha = (m_fDeathTime - CURTIME)/FADEOUT_TIME;
...
{
	m_fDeathTime = CURTIME + fDelay + FADEOUT_TIME;

	SetNextClientThink( CURTIME + fDelay );
}
...
bool C_FuncPhysicsRespawnZone::Initialize( void )
{
	...
	SetNextClientThink( CURTIME + (cl_phys_props_respawnrate.GetFloat() * RandomFloat(1.0,1.1)) );
