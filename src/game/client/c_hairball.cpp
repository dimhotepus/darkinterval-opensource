...
#ifdef DARKINTERVAL
ConVar hairball_sphere_radius("hairball_sphere_radius", "20", 0, "Radius of the sphere to which hairs are attached");
ConVar hairball_hair_amount("hairball_hair_amount", "100", 0, "Number of hairs to spawn on an individual hair sphere");
ConVar hairball_hair_length("hairball_hair_length", "20", 0, "Length of each hair on the sphere");
ConVar hairball_hair_subdiv("hairball_hair_subdiv", "6", 0, "Each hair is a segmented beam. This number defines the number of segments");
ConVar hairball_fly_speed("hairball_fly_speed", "150", 0, "The momentum of the hairball during movement");
#endif
...
{
#ifdef DARKINTERVAL
	m_nHairs = hairball_hair_amount.GetInt(); //100
	m_nNodesPerHair = hairball_hair_subdiv.GetInt(); //3	
	float flHairLength = hairball_hair_length.GetFloat(); //20
#else
	m_nHairs = 100;
	m_nNodesPerHair = 3;
...
#ifdef DARKINTERVAL
	m_flSphereRadius = hairball_sphere_radius.GetFloat();
#else
	m_flSphereRadius = 20;
#endif
	m_vMoveDir.Init();
...
#ifdef DARKINTERVAL
	m_pMaterial = materials->FindMaterial( "cable/hydra_gut", TEXTURE_GROUP_OTHER );
#else
...
#ifdef DARKINTERVAL
			m_vMoveDir *= hairball_fly_speed.GetFloat(); // set speed.
#else
			m_vMoveDir *= 650;
#endif
