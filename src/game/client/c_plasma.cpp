...
void RecvProxy_PlasmaScaleTime( const CRecvProxyData *pData, void *pStruct, void *pOut )
{
	...
	if ( pPlasmaSmoke->m_flScaleTime != time )
	{
		if ( time == -1.0f )
		{
			pPlasmaSmoke->m_flScaleTimeStart	= CURTIME-1.0f;
			pPlasmaSmoke->m_flScaleTimeEnd	= pPlasmaSmoke->m_flScaleTimeStart;
		}
		else
		{
			pPlasmaSmoke->m_flScaleTimeStart	= CURTIME;
			pPlasmaSmoke->m_flScaleTimeEnd	= CURTIME + time;
		}
...
float C_Plasma::GetFlickerScale( void )
{
	...
	result = sin( CURTIME * 10000.0f );
	result += 0.5f * sin( CURTIME * 2000.0f );
	result -= 0.5f * cos( CURTIME * 8000.0f );
