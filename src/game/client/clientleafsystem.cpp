...
static ConVar r_portalsopenall( "r_portalsopenall", "0", FCVAR_NOTIFY, "Open all portals" );
...
#ifdef DARKINTERVAL // intended to allow occluders to cull these, but work incomplete
		RENDER_FLAGS_SPRITE_MODEL = 0x40,
		RENDER_FLAGS_DETAIL_MODEL = 0x80,
#endif
	};
...
#ifdef DARKINTERVAL
	m_ShadowsInLeaf.EnsureCapacity( 1024 );
#else
...
#ifdef DARKINTERVAL // intended to allow occluders to cull these, but work incomplete
	else if (modelType == mod_sprite)
	{
		flags |= RENDER_FLAGS_SPRITE_MODEL;
	}
#endif
...
#ifndef DARKINTERVAL
	//  What is the point? Why 2000? It's not in any other version of the code - 
	//	not 2004, 2006, 2007, or 2015, it's *only* in 2013.
...
void CClientLeafSystem::ProjectFlashlight( ClientLeafShadowHandle_t handle, int nLeafCount, const int *pLeafList )
{
	...
	// Remove the shadow from any leaves it currently exists in
	RemoveShadowFromLeaves( handle );
	RemoveShadowFromRenderables( handle );
...
#ifdef DARKINTERVAL
ConVar r_occlusion_allow_for_brushmodels("r_occlusion_allow_for_brushmodels", "1", FCVAR_REPLICATED);
#endif
...
#ifdef DARKINTERVAL
	//	if (renderable.m_Flags & RENDER_FLAGS_SPRITE_MODEL)
		if( renderable.m_RenderGroup != RENDER_GROUP_TRANSLUCENT_ENTITY )
		{
			// test to see if this renderable is occluded by the engine's occlusion system
			if (engine->IsOccluded(absMins, absMaxs))
		...
		if (r_occlusion_allow_for_brushmodels.GetBool() && (renderable.m_Flags & RENDER_FLAGS_BRUSH_MODEL))
		{
			// test to see if this renderable is occluded by the engine's occlusion system
			if (engine->IsOccluded(absMins, absMaxs))
				continue;
