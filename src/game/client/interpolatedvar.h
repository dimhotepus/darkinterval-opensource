...
#define EXTRA_INTERPOLATION_HISTORY_STORED 0.2f //0.05f	// It stores this much extra interpolation history,
...
inline bool CInterpolatedVarArrayBase<Type, IS_ARRAY>::NoteChanged( float changetime, float interpolation_amount, bool bUpdateLastNetworkedValue )
{
	...
	if ( m_bDebug )
	{
		...
		Msg( "%s LatchChanged at %f changetime %f:  %s\n", GetDebugName(), CURTIME, changetime, pDiffString );
	}
	...
	RemoveOldEntries( CURTIME - interpolation_amount - 2.0f );
#else
	...
	// any data we're going to need.  Unless CURTIME is different when samples are added vs. when
	// they are interpolated I can't see this having any ill effects.  
	RemoveEntriesPreviousTo( CURTIME - interpolation_amount - EXTRA_INTERPOLATION_HISTORY_STORED );
#endif
...
inline void CInterpolatedVarArrayBase<Type, IS_ARRAY>::Reset()
{
	...
	if ( m_pValue )
	{
		AddToHead( CURTIME, m_pValue, false );
		AddToHead( CURTIME, m_pValue, false );
		AddToHead( CURTIME, m_pValue, false );
