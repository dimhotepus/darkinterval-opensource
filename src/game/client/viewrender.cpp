...
#ifdef DARKINTERVAL 
#include "c_func_water_analog.h" // unfinished prototype for having water with dynamic reflection plane (can change height, etc)
#endif
#include "keyvalues.h"
...
#ifdef DARKINTERVAL
#include "c_world.h" // for cheap reflections dictated by level maker
#endif
#ifdef PORTAL_COMPILE
//#include "C_BaseHLPlayer.h"
//#include "portal_render_targets.h"// necessary rendertargets are included in rendertargets_are_stupid.h
...
#include "viewdebug.h"
#ifdef DARKINTERVAL 
#include "darkinterval/rendertargets_are_stupid.h"
#endif
#ifdef USE_MONITORS
...
#ifdef DARKINTERVAL // experiment
static ConVar hud_draw_into_rendertarget("hud_draw_into_rendertarget", "0", 0, "If 1, player's hud will be drawn into _rt_testhud and only be visible on models or quads that are using that material.");
#endif
//-----------------------------------------------------------------------------
...
ConVar r_drawviewmodel( "r_drawviewmodel","1", FCVAR_NOTIFY);
...
static ConVar r_threaded_renderables( "r_threaded_renderables", "1", FCVAR_NONE);

ConVar mat_planar_reflections_level("mat_planar_reflections_level", "2", FCVAR_ARCHIVE,
	"Controls the Screen Space Reflections setting. 0 = None, 1 = Simple (no drawing entities), 2 = Normal (controlled by level designer), 3 = Full (very expensive; reflect everything in view at all time)");

static ConVar r_DrawDetailProps( "r_DrawDetailProps", "1", 0 );
#ifdef DARKINTERVAL // A/B testing convar for global prop fading
static ConVar r_disablepropfade("r_disablepropfade", "0", 0);
#endif
...
static ConVar fog_start( "fog_start", "0", FCVAR_NONE);
static ConVar fog_end( "fog_end", "2048", FCVAR_NONE);
static ConVar fog_density("fog_density", "0.0f", FCVAR_NONE);
static ConVar fog_color("fog_color", "17 17 17", FCVAR_NONE);

static ConVar fog_skybox_enable("fog_skybox_enable", "1", FCVAR_NONE);
static ConVar fog_skybox_start( "fog_skybox_start", "0", FCVAR_NONE);
static ConVar fog_skybox_end( "fog_skybox_end", "2048", FCVAR_NONE);
static ConVar fog_skybox_density( "fog_skybox_density", "0.0f", FCVAR_NONE);
static ConVar fog_skybox_color( "fog_skybox_color", "17 17 17", FCVAR_NONE);
static ConVar fog_startskybox("fog_startskybox", "0", FCVAR_NONE);
static ConVar fog_endskybox("fog_endskybox", "2048", FCVAR_NONE);
...
#ifdef DARKINTERVAL
//static ConVar r_waterforcesimple("r_waterforcesimple", "0", FCVAR_ARCHIVE);
static ConVar r_waterforcecheap("r_waterforcecheap", "0", FCVAR_ARCHIVE);
#endif
static ConVar r_waterforceexpensive( "r_waterforceexpensive", "0", FCVAR_ARCHIVE );
static ConVar r_waterforcereflectentities( "r_waterforcereflectentities", "0", FCVAR_ARCHIVE);
...
static ConVar mat_drawwater( "mat_drawwater", "1", FCVAR_NOTIFY);
...
#ifdef DARKINTERVAL // naming consistency
static ConVar r_drawmonitors( "r_drawmonitors", "1" );
#else
...
#ifdef DARKINTERVAL
	CLIENTEFFECT_MATERIAL("custom_uni/blurgaussian_3x3")
	CLIENTEFFECT_MATERIAL("custom_uni/depth_of_field")
	CLIENTEFFECT_MATERIAL("custom_uni/di_depth_of_field")
	CLIENTEFFECT_MATERIAL("custom_uni/glow_color")
	CLIENTEFFECT_MATERIAL("custom_uni/glow_color2")
	CLIENTEFFECT_MATERIAL("custom_uni/halo_add_to_screen")
	CLIENTEFFECT_MATERIAL("custom_uni/post_cubic_distortion")
	CLIENTEFFECT_MATERIAL("custom_uni/post_influence")
//	CLIENTEFFECT_MATERIAL("custom_uni/post_chromatic_aberration")
	CLIENTEFFECT_MATERIAL("custom_uni/post_vignette")
	CLIENTEFFECT_MATERIAL("custom_uni/post_nightvision_filter")
	CLIENTEFFECT_MATERIAL("custom_uni/object_motion_blur")
	CLIENTEFFECT_MATERIAL("custom_uni/object_motion_blur_buffer")
#endif
...
bool CViewRender::ShouldDrawViewModel( bool bDrawViewmodel )
{
	...
	if (player && player->IsZoomed() && player->GetFOV() <= 60) // fov check so viewmodels don't hide immediately; beware, this 60 is a magic number used for zooming without the weapon (largest FOV among zooms)
		return false;
#endif
	return true;
}
...
#ifdef PORTAL_COMPILE //in portal, we'd like a copy of the front buffer without the gun in it for use with the depth doubler
...
#ifdef PORTAL_COMPILE //the depth range hack doesn't work well enough for the portal mod (and messing with the depth hack values makes some models draw incorrectly)
				//step up to a full depth clear if we're extremely close to a portal (in a portal environment)
	extern bool LocalPlayerIsCloseToPortal( void ); //defined in C_BaseHLPlayer.cpp, abstracting to a single bool function to remove explicit dependence on C_BaseHLPlayer.h/cpp, you can define the function as a "return true" in other build configurations at the cost of some perf
	...
	if (!building_cubemaps.GetBool())
	{
		if (IsDepthOfFieldEnabled())
		{
			pRenderContext.GetFrom(materials);
			{
				...
				ConVarRef rdv("r_drawvgui");
				if (engine->IsPaused() && rdv.GetBool())
					DoDepthOfField(view, DOF_PAUSED_SCREEN);
			//	else											// Don't draw it when un-paused - doubles DOF effect. We only really care about the old case of weapons being un-DOFed in pause screen.
			//		DoDepthOfField(view, DOF_EXTRA);
			}
								...
								int nClearFlags, view_id_t viewID, bool bDrawViewModel, int baseDrawFlags, ViewCustomVisibility_t *pCustomVisibility )
{
...
#if 1 //FLASHLIGHT_SHADOWS_ENABLED
	...
	// Draw rain.. // DI add - draw only when open sky is visible from our position
#ifdef DARKINTERVAL
	if (nSkyboxVisible != SKYBOX_NOT_VISIBLE)
		DrawPrecipitation();
#endif
...
static void GetFogColorTransition( fogparams_t *pFogParams, float *pColorPrimary, float *pColorSecondary )
{
	...
	if ( pFogParams->lerptime >= CURTIME )
	{
		float flPercent = 1.0f - (( pFogParams->lerptime - CURTIME ) / pFogParams->duration );
...
static float GetFogMaxDensity( fogparams_t *pFogParams )
{
	...
	if ( fog_override.GetInt() )
	{
		if ( fog_density.GetFloat() == -1.0f )
			return pFogParams->maxdensity;
		else
			return fog_density.GetFloat();
	}
	else
		return pFogParams->maxdensity;
}
...
{			   
	...
	const char *fogColorString = fog_skybox_color.GetString();
	if( fog_override.GetInt() && fogColorString )
	{
		sscanf( fogColorString, "%f%f%f", pColor, pColor+1, pColor+2 );
	}
...
static float GetSkyboxFogStart( void )
{
	...
	if( fog_override.GetInt() )
	{
		if( fog_skybox_start.GetFloat() == -1.0f )
		{
			return local->m_skybox3d.fog.start;
		}
		else
		{
			return fog_skybox_start.GetFloat();
		}
	}
	else
	{
...
static float GetSkyboxFogEnd( void )
{
	...
	if( fog_override.GetInt() )
	{
		if( fog_skybox_end.GetFloat() == -1.0f )
		{
			return local->m_skybox3d.fog.end;
		}
		else
		{
			return fog_skybox_end.GetFloat();
		}
	}
	else
	{
...
static float GetSkyboxFogMaxDensity()
{
	...
	if ( fog_override.GetInt() )
	{
		if ( fog_skybox_density.GetFloat() == -1.0f )
			return local->m_skybox3d.fog.maxdensity;
		else
			return fog_skybox_density.GetFloat();
	}
	else
		return local->m_skybox3d.fog.maxdensity;
}
...
void CViewRender::FreezeFrame( float flFreezeTime )
{
	...
	{
		if ( m_flFreezeFrameUntil > CURTIME )
		{
			m_flFreezeFrameUntil += flFreezeTime;
		}
		else
		{
			m_flFreezeFrameUntil = CURTIME + flFreezeTime;
...
void CViewRender::RenderView( const CViewSetup &view, int nClearFlags, int whatToDraw )
{
	...
	// DI recommends running on DX9 or higher
	if ( g_pMaterialSystemHardwareConfig->GetDXSupportLevel() < 90 )
	{
	//	if ( ( Q_stricmp( COM_GetModDirectory(), "darkinterval" ) == 0 ) 
	//		|| ( Q_stricmp( COM_GetModDirectory(), "darkinterval-release" ) == 0 )
	//		|| (Q_stricmp(COM_GetModDirectory(), "darkinterval-beta") == 0))
		{
			static bool bFirstTime = true;
			if ( bFirstTime )
			{
				bFirstTime = false;
				Msg( "This game has a RECOMMENDED requirement of DirectX 9.0 or above to run properly.\nYou may run it in DirectX 8 mode, but many effects\nmay render incorrectly, or not render at all.\n" );
			}
	//		return;
		}
	}
	...
	if ( !m_rbTakeFreezeFrame[ view.m_eStereoEye ] && m_flFreezeFrameUntil > CURTIME )
	{
	...
	else
	{
...
#ifdef DARKINTERVAL // replicated skybox drawing to be able to a) fix the high FOV corner culling of the skybox, b) dynamically change sky texture w/o restarting
					// todo: replace with written-from-scratch new code instead of copying engine code
		////////////////////////
		DrawSecondarySkybox(); // needs to be placed before AddViewToScene, otherwise large 3d skybox elements clip against the secondary skybox
		////////////////////////
#endif 
		...
		if ( !building_cubemaps.GetBool())
		{
				...
				{						
					...
					else
					{
						...
						if( player->GetWaterLevel() > WL_Waist )
							DoDepthOfField(view, DOF_UNDERWATER);
						else
							DoDepthOfField(view, DOF_DEFAULT); // disabled for non paused non overlayed screens. It was too messy and unreliable for very little aesthetic benefit.
					}
				}
				pRenderContext.SafeRelease();
			}
			...
			if (IsChromAbEnabled())
			{
				pRenderContext.GetFrom(materials);
				{
					PIXEVENT(pRenderContext, "DoChromaticAberraion()");
					DoChromaticAberration(view);
				}
				pRenderContext.SafeRelease();
			}
			*/
			if (IsCubicDistEnabled())
			{
				pRenderContext.GetFrom(materials);
				{
					PIXEVENT(pRenderContext, "DoCubicDistortion()");
					DoCubicDistortion(view);
				}
				pRenderContext.SafeRelease();
			}
			if (IsInfluenceEnabled())
			{
				pRenderContext.GetFrom(materials);
				{
					PIXEVENT(pRenderContext, "DoInfluence()");
					DoInfluence(view);
				}
				pRenderContext.SafeRelease();
			}
			if (IsScreenFilterEnabled())
			{
				pRenderContext.GetFrom(materials);
				{
					PIXEVENT(pRenderContext, "DoScreenFilter()");
					DoScreenFilter(view);
				}
				pRenderContext.SafeRelease();
			}
			if (IsVignetteEnabled())
			{
				pRenderContext.GetFrom(materials);
				{
					PIXEVENT(pRenderContext, "DoVignette()");
					DoVignette(view);
				}
				pRenderContext.SafeRelease();
			}
			if (IsObjectMotionBlurEnabled())
			{
				pRenderContext.GetFrom(materials);
				{
					PIXEVENT(pRenderContext, "DoObjectMotionBlur()");
...
#ifndef DARKINTERVAL // unreliable feature that always gives trouble
	...
	if ( whatToDraw & RENDERVIEW_DRAWHUD )
	{
...
#endif
#ifdef DARKINTERVAL
		ITexture *pLeftHUDTexture = hud_draw_into_rendertarget.GetBool() ? GetHUDLeftTexture() : NULL;
		bool bClear = (pLeftHUDTexture != NULL);
#endif
		...
		// Draw the LEFT hud
#ifdef DARKINTERVAL
		pRenderContext->PushRenderTargetAndViewport(pLeftHUDTexture, NULL, viewX, viewY, viewActualWidth, viewActualHeight );
#else
		...
		if( bClear )
		{
#ifdef DARKINTERVAL // purpose?
			pRenderContext->ClearColor4ub(0x00, 0x00, 0x00, 0x00);
		...
		VGui_PreRender();
#ifdef DARKINTERVAL // what's the difference?
		vgui::VPANEL root;
		root = enginevgui->GetPanel( PANEL_CLIENTDLL );
#else
	...
	if (whatToDraw & RENDERVIEW_DRAWHUD)
	{
		...
		ITexture *pRightHUDTexture = hud_draw_into_rendertarget.GetBool() ? GetHUDRightTexture() : NULL;
		bool bClear = ( pRightHUDTexture != NULL);
		...
		// constrain where VGUI can render to the view
		// Draw the RIGHT hud
		pRenderContext->PushRenderTargetAndViewport(pRightHUDTexture, NULL, viewX, viewY, viewActualWidth, viewActualHeight);
...
void CViewRender::DetermineWaterRenderInfo( const VisibleFogVolumeInfo_t &fogVolumeInfo, WaterRenderInfo_t &info )
{
	...
	if (engine->GetDXSupportLevel() < 80)
		return;
#ifdef DARKINTERVAL
	// if convar is 1, it'll always be cheap. Otherwise, look into vmt
	...
	{	
#ifdef DARKINTERVAL
		// not by convar... but by material, cheap can be forced
		if( !bForceCheap )
#endif
		bForceCheap = (pForceCheapVar->GetIntValueFast() != 0);
		// either convar or material forced
		if (bForceCheap)
		{
			bForceExpensive = false;
		}
	...
	// forceexpensive can't be set if it's already forcing cheap.
...
#ifdef PORTAL_COMPILE //in portal, we can't use the fog volume for the camera since it's almost never in the same fog volume as what's in front of the portal
	if( g_pPortalRender->GetViewRecursionLevel() == 0 )
	{
...
#ifdef DARKINTERVAL // I believe this was done to be able to have expensive water on moving surfaces, but left unfinished
			|| IsWaterAnalogInView( viewIn, glassReflectionPlane) 
#endif
			)
		{								    
...
#ifndef DARKINTERVAL // not sure why it's ifdef'd out, investigate
...
#ifdef PORTAL_COMPILE //when rendering portal views point/plane intersections just don't cut it.
	if( g_pPortalRender->GetViewRecursionLevel() != 0 )
...
#ifdef DARKINTERVAL // probably don't need to do this for Portal?
	pReplacementWaterReflectionTexture.Init( g_pDIRenderTargets->GetWaterReflectionTextureForStencilDepth( iRecursionLevel ) );
#else
...
#ifdef DARKINTERVAL // probably don't need to do this for Portal?
	pReplacementWaterRefractionTexture.Init(g_pDIRenderTargets->GetWaterRefractionTextureForStencilDepth( iRecursionLevel ) );
#else
...
//-------------------------------------
#ifdef DARKINTERVAL // A/B testing
...
{
	pBaseAnimating->SetupBones( NULL, -1, -1, CURTIME );
}
...
static ConVar r_opaques_draw_old("r_opaques_draw_old", "0");
...
#ifdef DARKINTERVAL // I believe this doesn't work, so don't use this
...
#ifdef PORTAL_COMPILE //if we're in the portal mod, we need to make a detour so we can render portal views using stencil areas
...
#ifdef DARKINTERVAL // do NOT complain ("false" at the end). It creates so much console spam for no benefit
				IMaterial *pMaterial = materials->FindMaterial( "debug/showz", TEXTURE_GROUP_OTHER, false );
#else
...
sky3dparams_t *CSkyboxView::PreRender3dSkyboxWorld( SkyboxVisibility_t nSkyboxVisible )
{
	...
	if ( local->m_skybox3d.area == 255 )
		return NULL;
#ifdef DARKINTERVAL // for being able to disable 3d skybox on the cameras for perf boost, although it seems to somehow cause issues
	if (local->m_skybox3d.disabled)
	{
		return NULL;
	}
#endif
...
void CFreezeFrameView::Draw( void )
{
	...
	if ( g_flFreezeFlash >= CURTIME && engine->IsTakingScreenshot() == false )
	{
		// Overlay screen fade on entire screen
		IMaterial* pMaterial = m_TranslucentSingleColor;

		int iFadeAlpha = FREEZECAM_SNAPSHOT_FADE_SPEED * ( g_flFreezeFlash - CURTIME );
...
{
#ifdef DARKINTERVAL // changed bDrawSkybox to true... don't remember why
	BaseClass::Setup( view, nClearFlags, true, fogInfo, waterInfo, NULL );
#else
...
void CReflectiveGlassView::Draw()
{
	...
	bool bVisOcclusion = r_visocclusion.GetInt();
	r_visocclusion.SetValue( 0 );
#ifdef DARKINTERVAL // refactored settings
	switch (mat_planar_reflections_level.GetInt())
	{
	case 0:
		{
			return;
	...
	case 1:
		...
		m_DrawFlags &= ~DF_DRAW_ENTITITES;
		m_DrawFlags |= DF_DRAWSKYBOX | DF_CLIP_SKYBOX;;
		m_DrawFlags |= DF_RENDER_REFLECTION;
		}
		break;
	case 2:
		{
			if ( GetClientWorldEntity()->m_bForceCheapWater)
				m_DrawFlags &= ~DF_DRAW_ENTITITES;
			else
				m_DrawFlags |= DF_DRAW_ENTITITES;
