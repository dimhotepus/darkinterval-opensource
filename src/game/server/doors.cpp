...
// - removed all CS/TF/MP/HL1 stuff
// $NoKeywords: $
//=============================================================================//
...
void PlayLockSounds(CBaseEntity *pEdict, locksound_t *pls, int flocked, int fbutton)
{
	...
	if ( flocked )
	{
		int		fplaysound = (pls->sLockedSound != NULL_STRING && CURTIME > pls->flwaitSound);
		int		fplaysentence = (pls->sLockedSentence != NULL_STRING && !pls->bEOFLocked && CURTIME > pls->flwaitSentence);
		...
		if (fplaysound)
		{
			...
			pls->flwaitSound = CURTIME + flsoundwait;
		}
		...
		if (fplaysentence)
		{
			...
			pls->flwaitSentence = CURTIME + DOOR_SENTENCEWAIT;
		}
	}
	else
	{
		...
		int fplaysound = (pls->sUnlockedSound != NULL_STRING && CURTIME > pls->flwaitSound);
		int fplaysentence = (pls->sUnlockedSentence != NULL_STRING && !pls->bEOFUnlocked && CURTIME > pls->flwaitSentence);
		float fvol;
...
void CBaseDoor::MovingSoundThink( void )
{
	...
	if( ShouldLoopMoveSound() )
	{
		...
		SetContextThink( &CBaseDoor::MovingSoundThink, CURTIME + duration, "MovingSound" );
	}
}
...
{
	SetContextThink( NULL, CURTIME, "MovingSound" );
	char *pSoundName;
...
{
	UpdateAreaPortals( false );
	SetContextThink( NULL, CURTIME, CLOSE_AREAPORTAL_THINK_CONTEXT );
}
...
void CBaseDoor::DoorHitBottom( void )
{
	...
	SetContextThink( &CBaseDoor::CloseAreaPortalsThink, CURTIME + 0.5f, CLOSE_AREAPORTAL_THINK_CONTEXT );
}
