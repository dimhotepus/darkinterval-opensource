...
#include "npc_makers.h"
#include "weapon_rpg.h"
#ifdef DARKINTERVAL
#include "_sharedents_encompass.h" // for the flechette. It was moved from Hunter's code to there
#endif
...
#ifndef DARKINTERVAL // the flechette code has been moved to _sharedents_encompass.cpp
//-----------------------------------------------------------------------------
...
void CHunterFlechette::StickTo(CBaseEntity *pOther, trace_t &tr)
{
	...
	if ( !bAttachedToBuster )
	{
		...
		SetNextThink(CURTIME + ( hunter_flechette_explode_delay.GetFloat() - HUNTER_FLECHETTE_WARN_TIME ));
	} else
	{
		DangerSoundThink();
	}
...
void CHunterFlechette::DangerSoundThink()
{
	...
	SetNextThink(CURTIME + HUNTER_FLECHETTE_WARN_TIME);
}
...
int	CNPC_Hunter::DrawDebugTextOverlays()
{
	...
	if (m_debugOverlays & OVERLAY_TEXT_BIT)
	{
		...
		{
			EntityText( text_offset, CFmtStr("Next Siege Attempt:%f", m_flTimeNextSiegeTargetAttack - CURTIME ), 0 );
			text_offset++;
		}
	}
...
void CNPC_Hunter::PrescheduleThink()
{
	...
	{
		if ( m_flPupilDilateTime < CURTIME )
		{
...
void CNPC_Hunter::GatherChargeConditions()
{
	...
	if ( !hunter_charge_test.GetBool() && CURTIME < m_flNextChargeTime )
		return;
			...
			if ( pVehicle->GetSmoothedVelocity().LengthSqr() > Square( 200 ) )
			{
					 ...
					 t > 0.0 && t < 1.0 )
				{
					...
					if ( hunter_dodge_debug. GetBool() )
					{
						...
						if ( timeDrawnArrow != CURTIME )
						{
							timeDrawnArrow = CURTIME;
...
{
	if( CURTIME < m_flTimeNextSiegeTargetAttack )
		return;
	...
	m_flTimeNextSiegeTargetAttack = CURTIME + (hunter_siege_frequency.GetFloat() * RandomFloat( 0.8f, 1.2f) );
	CBasePlayer *pPlayer = AI_GetSinglePlayer();
	...
	if( bCreateSiegeTarget )
	{
		...
		if( pSiegeTargetLocation != NULL )
		{
			...
			if( pMemory )
			{
				...
				pMemory->timeFirstSeen = CURTIME - 5.0f;
				pMemory->timeLastSeen = CURTIME - 1.0f;
			}
		}
	}
}
...
void CNPC_Hunter::BuildScheduleTestBits()
{
	...
	if ( IsCurSchedule( SCHED_HUNTER_RANGE_ATTACK2, false ) && ( CURTIME < m_flShootAllowInterruptTime ) )
	{
	...
	else if ( !IsInLargeOutdoorMap() && IsCurSchedule( SCHED_HUNTER_FLANK_ENEMY, false ) && GetEnemy() != NULL )
	{
		...
		{
			if( (CURTIME - m_flTimeSawEnemyAgain) >= 2.0f )
			{
...
int CNPC_Hunter::SelectCombatSchedule()
{
	...
	if ( bStriderBuster )
	{
		if ( CURTIME - CAI_HunterEscortBehavior::gm_flLastDefendSound > 10.0 )
		{
			EmitSound( "NPC_Hunter.DefendStrider" );
			CAI_HunterEscortBehavior::gm_flLastDefendSound = CURTIME;
		}
...
/*
	if ( CURTIME > m_flNextSideStepTime )
	{
		if ( HasCondition( COND_ENEMY_FACING_ME ) && ( UTIL_DistApprox( GetEnemy()->GetAbsOrigin(), GetAbsOrigin() ) < HUNTER_FACE_ENEMY_DIST ) )
		{
			m_flNextSideStepTime = CURTIME + random->RandomFloat( 1.0f, 3.0f );
			return SCHED_HUNTER_SIDESTEP;
		}
	}
*/
	if ( HasCondition( COND_HEAVY_DAMAGE ) && ( CURTIME > m_flNextSideStepTime ) )
	{
		m_flNextSideStepTime = CURTIME + random->RandomFloat( 1.0f, 3.0f );
		return SCHED_HUNTER_SIDESTEP;
	}
...
void CNPC_Hunter::TaskFail( AI_TaskFailureCode_t code )
{
	...
	{
		// cornered!
		if ( m_fCorneredTimer < CURTIME )
		{
			m_fCorneredTimer = CURTIME + 6.0f;
		}
	}
	...
	switch ( pTask->iTask )
	{
		case TASK_HUNTER_FINISH_RANGE_ATTACK:
		{
			if( GetEnemy() != NULL && GetEnemy()->Classify() == CLASS_PLAYER_ALLY_VITAL )
			{
				...
				GetEnemies()->SetTimeValidEnemy( GetEnemy(), CURTIME + 10.0f );
			}
		...
		case TASK_HUNTER_SHOOT_COMMIT:
		{
			...
			m_flShootAllowInterruptTime = CURTIME + 100.0f;
			TaskComplete();
			break;
		}
		...
		case TASK_RANGE_ATTACK2:
		{
			if ( GetEnemy() )
			{
				...
				BeginVolley( nShots, CURTIME + initialDelay );
		...
		{
			m_fCorneredTimer = CURTIME + pTask->flTaskData;
		...
		{
			m_flNextChargeTime = CURTIME + pTask->flTaskData;
			TaskComplete();
			break;
		}
...
void CNPC_Hunter::RunTask( const Task_t *pTask )
{
	switch ( pTask->iTask )
	{
		...
		case TASK_RANGE_ATTACK2:
		{
			...
			if ( CURTIME >= m_flNextFlechetteTime )
			{
				...
				if ( GetEnemy() != NULL && m_nFlechettesQueued > 0 )
				{
					...
					else
					{
						...
						m_flNextFlechetteTime = CURTIME + hunter_flechette_delay.GetFloat();
					}
				}
				else
				{
		...
		case TASK_HUNTER_CHARGE:
		{
			...
			if ( eActivity == ACT_HUNTER_CHARGE_STOP || eActivity == ACT_HUNTER_CHARGE_CRASH ) 
			{
				if ( IsActivityFinished() )
				{
					m_flNextChargeTime = CURTIME + hunter_charge_min_delay.GetFloat() + random->RandomFloat( 0, 2.5 ) + random->RandomFloat( 0, 2.5 );
					float delayMultiplier = ( g_pGameRules->IsSkillLevel( SKILL_EASY ) ) ? 1.5 : 1.0;
					float groupDelay = CURTIME +  ( 2.0  + random->RandomFloat( 0, 2 ) ) * delayMultiplier;
					for ( int i = 0; i < g_Hunters.Count(); i++ )
					{
...
bool CNPC_Hunter::HandleChargeImpact( Vector vecImpact, CBaseEntity *pEntity )
{
	...
	if ( 
#ifndef DARKINTERVAL
		IRelationType( pEntity ) == D_HT &&
#endif
		(pEntity->ClassMatches(GetClassname()) == false) && ( GetNextAttack() < CURTIME ) )
	{
...
void CNPC_Hunter::InputUseSiegeTargets( inputdata_t &inputdata )
{
	...
	m_flTimeNextSiegeTargetAttack = CURTIME + random->RandomFloat( 1, hunter_siege_frequency.GetFloat() );
...
bool CNPC_Hunter::IsValidEnemy( CBaseEntity *pTarget )
{
	if ( IsStriderBuster( pTarget) )
	{
		...
		if ( pTarget->VPhysicsGetObject() )
		{
			...
			{
				if ( CURTIME - StriderBuster_GetPickupTime( pTarget ) > hunter_hate_held_striderbusters_delay.GetFloat())
				{
...
int CNPC_Hunter::MeleeAttack1Conditions ( float flDot, float flDist )
{
	...
	if ( ( CURTIME < m_flNextMeleeTime ) && // allow berzerk bashing if cornered
		!( m_hAttachedBusters.Count() > 0 && CURTIME < m_fCorneredTimer ) )
	{
		return COND_NONE;
	}
...
int CNPC_Hunter::RangeAttack2Conditions( float flDot, float flDist )
{
	...
	if ( !bIsPerfectBullseye && !bIsBuster && !hunter_flechette_test.GetBool() && ( CURTIME < m_flNextRangeAttack2Time ) )
	{
		return COND_NONE;
	}
...
CBaseEntity *CNPC_Hunter::MeleeAttack( float flDist, int iDamage, QAngle &qaViewPunch, Vector &vecVelocityPunch, int BloodOrigin  )
{
	...
	m_flNextMeleeTime = CURTIME + hunter_melee_delay.GetFloat();
...
void CNPC_Hunter::PainSound( const CTakeDamageInfo &info )
{
	if ( CURTIME > m_flNextDamageTime )
	{
		...
		m_flNextDamageTime = CURTIME + random->RandomFloat( 0.5, 1.2 ); 
	}
}
...
void CNPC_Hunter::TeslaThink()
{
	...
	if ( CURTIME < m_flTeslaStopTime )
	{
		SetContextThink( &CNPC_Hunter::TeslaThink, CURTIME + random->RandomFloat( 0.1f, 0.3f ), HUNTER_ZAP_THINK ); 
	}
}
...
void CNPC_Hunter::BleedThink()
{
	...
	SetNextThink( CURTIME + random->RandomFloat( 0.6, 1.5 ), HUNTER_BLEED_THINK );
}
...
int CNPC_Hunter::OnTakeDamage_Alive( const CTakeDamageInfo &info )
{
	...
	if ( pInflictor )
	{
		if ( IsStriderBuster( pInflictor ) )
		{
			...
			SetContextThink( &CNPC_Hunter::TeslaThink, CURTIME, HUNTER_ZAP_THINK ); 
			m_flTeslaStopTime = CURTIME + 2.0f;
		...
		else
		{
				   ...
				   ( ( bHitByUnoccupiedCar = ( dynamic_cast<CPropVehicleDriveable *>(pInflictor) && static_cast<CPropVehicleDriveable *>(pInflictor)->GetDriver() == NULL ) )  == false ) ) )
			{
				...
				if ( vecVelDir.Length() >= hunter_jostle_car_min_speed.GetFloat() )
				{
					...
					SetContextThink( &CNPC_Hunter::JostleVehicleThink, CURTIME, HUNTER_JOSTLE_VEHICLE_THINK );
				}
			}
		...
		if( pAttacker && pAttacker->IsPlayer() )
		{
			...
			if( !GetEnemy() || !GetEnemy()->IsPlayer() )
			{
				...
				if( GetEnemy() )
				{
					...
					GetEnemies()->SetTimeValidEnemy( GetEnemy(), CURTIME + HUNTER_IGNORE_ENEMY_TIME );
				}
			}
		}
	...
	if ( IsUsingSiegeTargets() && info.GetAttacker() != NULL && info.GetAttacker()->IsPlayer() )
	{
		...
		m_flTimeNextSiegeTargetAttack = CURTIME;
	}
...
void CNPC_Hunter::StartBleeding()
{
	...
	SetContextThink( &CNPC_Hunter::BleedThink, CURTIME + 0.1, HUNTER_BLEED_THINK );
}
...
void CNPC_Hunter::DelayRangedAttackers( float minDelay, float maxDelay, bool bForced )
{
	...
	{
		m_flNextRangeAttack2Time = CURTIME + random->RandomFloat( minDelay, maxDelay ) * delayMultiplier;
		return;
	}
	...
	for ( int i = 0; i < g_Hunters.Count(); i++ )
	{
		...
		if ( pOtherHunter->GetEnemy() == pEnemy )
		{
			float nextTime = CURTIME + random->RandomFloat( minDelay, maxDelay ) * delayMultiplier;
...
void CNPC_Hunter::FootFX( const Vector &origin )
{
	...
	for ( int i = 0; i < 3; i++ )
	{
		...
		dir.z = 0.25;
		VectorNormalize( dir );
		g_pEffects->Dust( tr.endpos, dir, 8, 50 );
	}
#else
	return;
#endif
...
void CNPC_Hunter::PlayerHasIlluminatedNPC( CBasePlayer *pPlayer, float flDot )
{
	...
	if ( PlayerFlashlightOnMyEyes( pPlayer ) )
	{
		...
		m_flPupilDilateTime = CURTIME + 0.2f;
	}
}
...
void CAI_HunterEscortBehavior::OnDamage( const CTakeDamageInfo &info )
{
	if ( info.GetDamage() > 0 && info.GetAttacker()->IsPlayer() &&
		...
		( GetOuter()->GetSquad()->GetSquadSoundWaitTime() <= CURTIME ) ) // && !FarFromFollowTarget()
	{
		// Start the clock ticking. We'll return the the strider when the timer elapses.
		m_flTimeEscortReturn = CURTIME + random->RandomFloat( 15.0f, 25.0f );
...
void CAI_HunterEscortBehavior::BuildScheduleTestBits()
{
	...
	if ( ( m_flTimeEscortReturn != 0 ) && ( CURTIME > m_flTimeEscortReturn ) )
	{
...
{
	if ( m_flTimeEscortReturn != 0 && ( FarFromFollowTarget() || CURTIME >= m_flTimeEscortReturn ) )
	{
		...
		if ( GetOuter()->GetSquad() )
		{
			GetOuter()->GetSquad()->SetSquadSoundWaitTime( CURTIME + random->RandomFloat( 5.0f, 12.0f ) );
		}
	}
}
...
void CAI_HunterEscortBehavior::GatherConditions( void )
{
	...
	{
		if ( GetOuter()->GetSquad()->GetSquadSoundWaitTime() <= CURTIME && ((CBasePlayer *)GetEnemy())->IsInAVehicle() )
		{
			m_flTimeEscortReturn = CURTIME + random->RandomFloat( 15.0f, 25.0f );
			GetOuter()->GetSquad()->SetSquadSoundWaitTime( m_flTimeEscortReturn + 1.0 ); // prevent others from breaking escort
		}
...
bool CAI_HunterEscortBehavior::ShouldFollow()
{
	...
	if ( m_flTimeEscortReturn <= CURTIME )
	{
	...
	case TASK_MOVE_TO_FOLLOW_POSITION:
		{
			if ( GetEnemy() )
			{
				if ( GetOuter()->OccupyStrategySlot( SQUAD_SLOT_RUN_SHOOT ) )
				{
					...
					{
						GetOuter()->BeginVolley( NUM_FLECHETTE_VOLLEY_ON_FOLLOW, CURTIME + 1.0 + random->RandomFloat( 0, .25 ) + random->RandomFloat( 0, .25 ) );
					}
					else
					{
			...
			else
			{
				if ( GetEnemy() )
				{
					...
					if ( HasCondition( COND_SEE_ENEMY ) )
					{
						...
						{
							if ( CURTIME >= pHunter->m_flNextFlechetteTime )
							{
								if ( !bHasSlot )
								{
									if ( GetOuter()->OccupyStrategySlot( SQUAD_SLOT_RUN_SHOOT ) )
								...
								if ( bHasSlot )
								{
									...
									{
										pHunter->m_flNextFlechetteTime = CURTIME + hunter_flechette_delay.GetFloat();
									}
									else
									{
										// Stop the firing sound.
										...
										bVacate = true;
										pHunter->BeginVolley( NUM_FLECHETTE_VOLLEY_ON_FOLLOW, CURTIME + 1.0 + random->RandomFloat( 0, .25 ) + random->RandomFloat( 0, .25 ) );
									}
								}
							}
						}
...
{
	if ( g_TimeLastDistributeFreeHunters != -1 && CURTIME - g_TimeLastDistributeFreeHunters < FREE_HUNTER_DISTRIBUTE_INTERVAL )
	{
		return;
	}

	g_TimeLastDistributeFreeHunters = CURTIME;
...
void CAI_HunterEscortBehavior::DrawDebugGeometryOverlays()
{
	...
	if ( FarFromFollowTarget() )
	{
		if ( CURTIME >= m_flTimeEscortReturn )
		{
