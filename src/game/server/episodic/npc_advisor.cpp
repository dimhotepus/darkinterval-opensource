...
// Skill settings.
//
ConVar sk_advisor_health( "sk_advisor_health", "100" );
#ifndef DARKINTERVAL
...
ConVar advisor_throw_velocity( "npc_advisor_throw_velocity", "1100" );
ConVar advisor_throw_rate( "npc_advisor_throw_rate", "4" );					// Throw an object every 4 seconds.
ConVar advisor_throw_warn_time( "npc_advisor_throw_warn_time", "1.0" );		// Warn players one second before throwing an object.
ConVar advisor_throw_lead_prefetch_time ( "npc_advisor_throw_lead_prefetch_time", "0.66", FCVAR_NONE, "Save off the player's velocity this many seconds before throwing.");
ConVar advisor_throw_stage_distance("npc_advisor_throw_stage_distance","180.0",FCVAR_NONE,"Advisor will try to hold an object this far in front of him just before throwing it at you. Small values will clobber the shield and be very bad.");
#ifdef DARKINTERVAL
ConVar advisor_staging_num("npc_advisor_staging_num","1",FCVAR_NONE,"Advisor will queue up this many objects to throw at Gordon.");
#endif
ConVar advisor_throw_clearout_vel("npc_advisor_throw_clearout_vel","200",FCVAR_NONE,"TEMP: velocity with which advisor clears things out of a throwable's way");
#ifdef DARKINTERVAL
ConVar advisor_staging_duration("npc_advisor_staging_duration", "1.0f", FCVAR_NONE, "how long it will take an object to get hauled to the staging point");
#endif
#define STAGING_OBJECT_FALLOFF_TIME 0.15f
#endif // NPC_ADVISOR_HAS_BEHAVIOR
//
// Spawnflags.
//
...
public:
	...
	//
#ifdef DARKINTERVAL
	virtual float MaxYawSpeed() { return 30.0f; }
#else
	...
	/// true if I recently threw the given object (not so fast)
...
#endif // NPC_ADVISOR_HAS_BEHAVIOR
#ifdef DARKINTERVAL
	float m_advisor_min_object_mass_float;
	float m_advisor_max_object_mass_float;
#endif
...
#endif
#ifdef DARKINTERVAL
	SetHullType( HULL_ADVISOR );
#else
	SetHulType(HULL_LARGE_CENTERED);
#endif
	SetHullSizeNormal();
...
CBaseEntity *CNPC_Advisor::ThrowObjectPrepare()
{
	...
	if (pThrowable)
	{
		...
		m_flThrowPhysicsTime = CURTIME + advisor_throw_warn_time.GetFloat();
...
void CNPC_Advisor::StartTask( const Task_t *pTask )
{
	switch ( pTask->iTask )
	{
...
#ifdef DARKINTERVAL
					if ( CanLevitateEntity( pTouch, m_advisor_min_object_mass_float, m_advisor_max_object_mass_float) )
#else
		...
		case TASK_ADVISOR_LEVITATE_OBJECTS:
		{
			...
			m_flThrowPhysicsTime = CURTIME + advisor_throw_rate.GetFloat();
		...
		case TASK_ADVISOR_STAGE_OBJECTS:
		{
			...
			*/
			...
			m_flStagingEnd = CURTIME + pTask->flTaskData;
		...
		case TASK_ADVISOR_PIN_PLAYER:
		{
			...
			m_playerPinFailsafeTime = CURTIME + 10.0f;
...
void CNPC_Advisor::RunTask( const Task_t *pTask )
{
	...
	switch ( pTask->iTask )
	{
		...
		case TASK_ADVISOR_LEVITATE_OBJECTS:
		{
			float flTimeToThrow = m_flThrowPhysicsTime - CURTIME;
			if ( flTimeToThrow < 0 )
			{
				TaskComplete();
				return;
			...
			for ( int i = 0; i < m_physicsObjects.Count(); i++ )
			{
				...
				IPhysicsObject *pPhys = pEnt->VPhysicsGetObject();
				if ( pPhys && pPhys->IsMoveable() && developer.GetInt() > 0)
				{
					NDebugOverlay::Box( pEnt->GetAbsOrigin(), pEnt->CollisionProp()->OBBMins(), pEnt->CollisionProp()->OBBMaxs(), 0, 255, 0, 1, 0.1 );
				}
			}
...
// advisor_staging_num
			...
			{	
				// pull another object
				bool bDesperate = m_flStagingEnd - CURTIME < 0.50f; // less than one half second left
		...
		case TASK_ADVISOR_BARRAGE_OBJECTS:
		{
			...
			if ( (CURTIME > m_flThrowPhysicsTime - advisor_throw_lead_prefetch_time.GetFloat()) && 
			...
			if (CURTIME > m_flThrowPhysicsTime)
			{
				IPhysicsObject *pPhys = pThrowable->VPhysicsGetObject();
				Assert(pPhys);
				...
				m_flLastThrowTime = CURTIME;
				m_flThrowPhysicsTime = CURTIME + 0.75f;
		...
		case TASK_ADVISOR_PIN_PLAYER:
		{
			...
			if ( CURTIME > m_playerPinFailsafeTime )
			{
				GetEnemy()->SetGravity(1.0f);
				GetEnemy()->SetMoveType( MOVETYPE_WALK );
...
{
	float threeSecondsAgo = CURTIME - 3.0f; // two seconds ago
...
bool CNPC_Advisor::DidThrow(const CBaseEntity *pEnt)
{
	...
	float threeSecondsAgo = CURTIME - 3.0f; 
...
void CNPC_Advisor::AddToThrownObjects(CBaseEntity *pEnt)
{
	...
	m_flaRecentlyThrownObjectTimes[oldestThrownObject] = CURTIME;
...
bool CNPC_Advisor::QueryHearSound( CSound *pSound )
{
	...
	if ( pOwner && pSound->IsSoundType( SOUND_COMBAT ) && pSound->SoundChannel() != SOUNDENT_CHANNEL_NPC_FOOTSTEP && pSound->m_hOwner.IsValid() && pOwner->IsPlayer() )
	{
		...
		m_flLastPlayerAttackTime = CURTIME;
	}
...
{
	return gAdvisorImpactDamageTable; // : BaseClass::GetPhysicsImpactDamageTable();
}
...
#ifdef DARKINTERVAL
	DEFINE_KEYFIELD(m_advisor_min_object_mass_float, FIELD_FLOAT, "levitationminmass"),
	DEFINE_KEYFIELD(m_advisor_max_object_mass_float, FIELD_FLOAT, "levitationmaxmass"),
#endif
