...
//
// DI todo - condense two jeeps into the episodic one with HL2 features
// intact (gun, no hazard lights, no locator)
//=============================================================================
...
public:
	...
	void CargoTouch( CBaseEntity *pOther )
	{
		...
		if ( ( m_hIgnoreEntity == pOther ) || ( m_flIgnoreDuration >= CURTIME ) )
			return;
	...
	void IgnoreTouches( CBaseEntity *pIgnoreEntity )
	{
		...
		m_flIgnoreDuration = CURTIME + 0.5f;
	}
...
//	CPropJeepEpisodic
//
#ifdef DARKINTERVAL // to keep HL2 jeep intact
	...
	for ( int i = 0; i < NUM_WHEEL_EFFECTS; i++ )
	{
		...
		if ( bCreateDust )
		{
			...
			if ( m_WaterData.m_bWheelInWater[i] )
			{
				...
				if ( m_flNextWaterSound < CURTIME )
				{
					m_flNextWaterSound = CURTIME + random->RandomFloat( 0.25f, 1.0f );
...
void CPropJeepEpisodic::UpdateRadar( bool forceUpdate )
{
	...
	if( !forceUpdate && CURTIME < m_flNextRadarUpdateTime )
		return;
	...
	if( bDetectedDog )
	{
		...
		m_flNextRadarUpdateTime = CURTIME + RADAR_UPDATE_FREQUENCY_FAST;
	}
...
#ifdef DARKINTERVAL
	if ( m_flCargoStartTime + 1.0f < CURTIME )
#else
	if ( m_flCargoStartTime + jalopy_cargo_anim_time.GetFloat() < CURTIME )
#endif
	{
		// Close the hold immediately if we're finished
		if ( m_bAddingCargo )
		{
			m_flAmmoCrateCloseTime = CURTIME;
			m_bAddingCargo = false;
		}
...
#ifdef DARKINTERVAL
	float flPerc = ( CURTIME - m_flCargoStartTime );
#else
	float flPerc = (jalopy_cargo_anim_time.GetFloat()) ? (( CURTIME - m_flCargoStartTime ) / jalopy_cargo_anim_time.GetFloat()) : 1.0f;
#endif
...
{
	if ( m_flNextAvoidBroadcastTime > CURTIME )
		return;
	...
	m_flNextAvoidBroadcastTime = CURTIME + VEHICLE_AVOID_BROADCAST_RATE;
}
...
void CPropJeepEpisodic::Think( void )
{
	...
	{
		m_throttleDisableTime = CURTIME + 0.25f;		
	}
			...
			if ( index < 0 )
			{
				...
				if ( pNPC->VPhysicsGetObject() && pNPC->VPhysicsGetObject()->GetShadowController() && pNPC->GetMoveType() == MOVETYPE_STEP )
				{
					if ( pNPC->HasDataObjectType(VPHYSICSUPDATEAI) )
					{
						...
						if ( (CURTIME - pUpdate->startUpdateTime) > 0.5f && DotProduct(velocity,normal) > 0)
						{
...
void CPropJeepEpisodic::CreateHazardLights( void )
{
	...
	// Setup our blink
	SetContextThink( &CPropJeepEpisodic::HazardBlinkThink, CURTIME + 0.1f, "HazardBlink" );
}
...
void CPropJeepEpisodic::DestroyHazardLights( void )
{
	...
	SetContextThink( NULL, CURTIME, "HazardBlink" );
}
...
void CPropJeepEpisodic::SpawnRadarPanel()
{
	...
	m_iNumRadarContacts = 0;
	m_flNextRadarUpdateTime = CURTIME - 1.0f;
}
...
void CPropJeepEpisodic::HazardBlinkThink( void )
{
	if ( m_bBlink )
	{
		...
		SetContextThink( &CPropJeepEpisodic::HazardBlinkThink, CURTIME + 0.25f, "HazardBlink" );
	}
	else
	{
		for ( int i = 0; i < NUM_HAZARD_LIGHTS; i++ )
		...
		SetContextThink( &CPropJeepEpisodic::HazardBlinkThink, CURTIME + 0.5f, "HazardBlink" );
	}
...
#ifdef DARKINTERVAL 
	return GetSmoothedVelocity().IsLengthLessThan( 50.0f );
#else
