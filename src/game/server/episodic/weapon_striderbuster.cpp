...
{	
	...
	AddFlag( FL_AIMTARGET|FL_VISIBLE_TO_NPCS );
	...
	if ( m_flCollisionSpeedSqr > 50 && !m_bNoseDiving )
	{
		...
		if ( ShouldStickToEntity( pOther ) )
		{
			...
			if ( CreateConstraintToObject( pOther ) )
			{
				...
				SetContextThink( &CWeaponStriderBuster::BusterPingThink, CURTIME + BUSTER_PING_SOUND_FREQ, s_pBusterPingThinkContext );
		...
		if ( ( GetHealth() > 0 ) && ( pInflictor != NULL ) && FClassnameIs( pInflictor, "hunter_flechette" ) )
		{
			...
			if ( IsAttachedToStrider() || !( info.GetDamageType() & DMG_NEVERGIB ) )
			{
				if( striderbuster_die_detach.GetBool() && IsAttachedToStrider() )
				{
					...
					SetContextThink( NULL, CURTIME, s_pBusterPingThinkContext );
			...
			if ( info.GetDamage() < 5 )
			{
				...
				m_CarryAngles.y = 15 * ( sin( CURTIME ) + cos( CURTIME * 0.5 ) ) * .5  + random->RandomFloat( -15, 15 );
				m_CarryAngles.z = 7.5 * ( sin( CURTIME ) + sin( CURTIME * 2.0 ) ) * .5 + random->RandomFloat( -7.5, 7.5 );
			}
