...
#ifdef DARKINTERVAL
	DEFINE_INPUTFUNC( FIELD_VOID, "UpdatePositions", InputUpdatePositions ),
#endif
...
#ifdef DARKINTERVAL
// ----------------------------------------------------------------------------- //
// Purpose: by default it only happens on level restart, make it more dynamic
// ----------------------------------------------------------------------------- //
void CEnvSoundscape::InputUpdatePositions(inputdata_t &inputdata)
{
	...
	if ( soundscape_debug.GetBool() )
	{
 		...
 		if ( update.pPlayer )
		{
			...
			else
			{
				...
				float periodic = 2.0f * sin((fmod(CURTIME,2.0f) - 1.0f) * M_PI); // = -4f .. 4f
		...
		engine->Con_NXPrintf(&info, "Active soundscape:\n%s", STRING(m_soundscapeName));
#endif
	}
}
