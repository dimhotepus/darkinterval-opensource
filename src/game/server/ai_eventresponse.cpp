...
void CNPCEventResponseSystem::TriggerEvent( const char *pResponse, bool bForce, bool bCancelScript )
{
	...
	else 
	{
		...
		m_ActiveEvents[iIndex].flEventTime = CURTIME;
...
void CNPCEventResponseSystem::FrameUpdatePreEntityThink()
{
	...
	if ( m_flNextEventPoll > CURTIME )
		return;
	m_flNextEventPoll = CURTIME + 0.2;
	...
	{	
		...
		if ( !m_ActiveEvents[i].bPreventExpiration && (flTime + NPCEVENTRESPONSE_GIVEUP_TIME) < CURTIME )
		{
			if ( ai_debug_eventresponses.GetBool() ) 
			{
		...
		else if ( m_ActiveEvents[i].flNextResponseTime < CURTIME )
		{
			...
			else
			{
				...
				if ( pNearestNPC )
				{
					if ( pNearestNPC->RespondedTo( pResponse, m_ActiveEvents[i].bForce, m_ActiveEvents[i].bCancelScript ) )
					{
						...
						m_ActiveEvents[i].flNextResponseTime = CURTIME + NPCEVENTRESPONSE_REFIRE_TIME;

						if ( ai_debug_eventresponses.GetBool() ) 
						{
							Msg( "NPCEVENTRESPONSE: (%.2f) Event '%s' responded to by NPC '%s'. Refire available at: %.2f\n", CURTIME, pResponse, pNearestNPC->GetDebugName(), m_ActiveEvents[i].flNextResponseTime );
						}
