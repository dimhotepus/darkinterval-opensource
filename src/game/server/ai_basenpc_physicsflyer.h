...
protected:
	...
	void AddNoiseToVelocity( float noiseScale = 1.0 )
	{
		if( m_vNoiseMod.x )
		{
			m_vCurrentVelocity.x += noiseScale*sin(m_vNoiseMod.x * CURTIME + m_vNoiseMod.x);
		}

		if( m_vNoiseMod.y )
		{
			m_vCurrentVelocity.y += noiseScale*cos(m_vNoiseMod.y * CURTIME + m_vNoiseMod.y);
		}

		if( m_vNoiseMod.z )
		{
			m_vCurrentVelocity.z -= noiseScale*cos(m_vNoiseMod.z * CURTIME + m_vNoiseMod.z);
		}
	}
...
#ifdef DARKINTERVAL // make it equal to default gravity
		linear.z += 600.0f; 
#else
		linear.z += 800.0f;
#endif
