...
extern CBaseEntity *CreateRagGib( const char *szModel, const Vector &vecOrigin, 
									const QAngle &vecAngles, const Vector &vecForce, 
									float flFadeTime = 0.0, bool bShouldIgnite = false, 
#ifdef DARKINTERVAL // skin support for gibs - for example, for antlions and synth scanners
									int iSkin = 0 
#endif
);
...
public:
	...
	static	void SpawnSpecificGibs( CBaseEntity *pVictim, int nNumGibs, 
		float fMaxVelocity, float fMinVelocity, 
