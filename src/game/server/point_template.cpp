...
#define SF_POINTTEMPLATE_DONTREMOVETEMPLATEENTITIES		1<<0
...
#define	SF_POINTTEMPLATE_PRESERVE_NAMES					1<<1

#ifdef DARKINTERVAL // DI NEW: add options to make this point template spawn things on selected difficulties
#define SF_POINTTEMPLATE_DIFFICULTY_NOT_EASY				1<<16
#define SF_POINTTEMPLATE_DIFFICULTY_NOT_MEDIUM				1<<17
#define SF_POINTTEMPLATE_DIFFICULTY_NOT_HARD				1<<18
#endif
...
bool CPointTemplate::CreateInstance( const Vector &vecOrigin, const QAngle &vecAngles, CUtlVector<CBaseEntity*> *pEntities )
{
	...
	if ((skill.GetInt() == 1 && HasSpawnFlags(SF_POINTTEMPLATE_DIFFICULTY_NOT_EASY))
		|| (skill.GetInt() == 2 && HasSpawnFlags(SF_POINTTEMPLATE_DIFFICULTY_NOT_MEDIUM))
			|| (skill.GetInt() == 3 && HasSpawnFlags(SF_POINTTEMPLATE_DIFFICULTY_NOT_HARD)))
			return false;
#endif
