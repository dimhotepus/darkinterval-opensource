...
public:
...
#ifdef DARKINTERVAL
	// DI NE, a whole bunch of convenience inputs
	...
	void InputMakeSolid(inputdata_t &inputdata);
	void InputMakeNonSolid(inputdata_t &inputdata);
	void InputSetTargetName(inputdata_t &inputdata);
	void InputClearTargetName(inputdata_t &inputdata);
	void InputAddNodraw(inputdata_t &inputdata);
	void InputRemoveNodraw(inputdata_t &inputdata);
...
#ifdef DARKINTERVAL // spelling convenience
	void		SetClassName(const char *className);
...
#ifdef DARKINTERVAL
	virtual void			ParticleDecalTrace(trace_t *pTrace, char const *decalName); // used to be used for particle-based decal replacements to fix non-fading decals (Valve's fault)
#endif
...
#ifdef DARKINTERVAL
	void					SUB_FadeIn(void); // opposite of fading out, not implemented
#endif
...
#ifdef DARKINTERVAL	
	COutputEvent m_OnSpawned; // DI NEW - useful for launching events right after an actor or other entity spawned
#endif
...
{
	float flNavIgnoreUntilTime = ( duration == FLT_MAX ) ? FLT_MAX : CURTIME + duration;
...
{
	return ( CURTIME <= m_flNavIgnoreUntilTime );
}
