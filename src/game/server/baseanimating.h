...
public:
	...
	void SetNameplateText(string_t newString);
#endif
// Controllers.
	...
	// Fire
#ifdef DARKINTERVAL // todo - finish this, the coloured flames
	virtual void Ignite( float flFlameLifetime, bool bNPCOnly = true, float flSize = 0.0f, bool bCalledByLevelDesigner = false, int flame_color = FLAME_COLOR_DEFAULT );
	virtual void IgniteLifetime( float flFlameLifetime, int flame_color = FLAME_COLOR_DEFAULT);
	virtual void IgniteNumHitboxFires( int iNumHitBoxFires, int flame_color = FLAME_COLOR_DEFAULT);
	virtual void IgniteHitboxFireScale( float flHitboxFireScale, int flame_color = FLAME_COLOR_DEFAULT);
#else
...
#ifdef DARKINTERVAL	// Ice, ported freezing from ASW
	...
	void			InputUnfreeze(inputdata_t &inputdata);
#endif
...
#ifdef DARKINTERVAL	// DI NEW: meshBuilder based text strings on models
	void InputSetNameplateText(inputdata_t &inputdata);
	void InputSetModelName(inputdata_t &inputdata);
#endif
#ifdef DARKINTERVAL	// DI NEW: Live Ragdolls Experiment (inspired by Dark Messiah)
	void InputSetLiveRagdollEnt(inputdata_t &inputdata);
	void InputClearLiveRagdollEnt(inputdata_t &inputdata);
#endif
...
//	void InputUpdateGlowEffect(inputdata_t &inputdata);

	void AddGlowEffect(float r, float g, float b);
	void RemoveGlowEffect(void);
	bool IsGlowEffectActive(void);

private:
	int m_remember_transmit_state_int;

protected:
	CNetworkVar(bool, m_bGlowEnabled);
	CNetworkVar(float, m_fGlowColorRed);
	CNetworkVar(float, m_fGlowColorGreen);
	CNetworkVar(float, m_fGlowColorBlue);
...
#ifdef DARKINTERVAL	// DI NEW: Live Ragdolls Experiment (inspired by Dark Messiah)
	CNetworkHandle(CBaseEntity, m_live_ragdoll_ent);
#endif
private:
...
#ifdef DARKINTERVAL // DI NEW: meshBuilder based text strings on models
public:
	CNetworkVar(string_t, m_nameplate_string);
#endif
};
...
#ifndef DARKINTERVAL // not that useful, spams
