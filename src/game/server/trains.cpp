...
public:
	...
	COutputEvent	m_OnReachedEitherEnd;
#endif
	DECLARE_DATADESC();
...
BEGIN_DATADESC( CFuncPlat )
	...
	DEFINE_OUTPUT(m_OnReachedEitherEnd, "OnFinishedMovement"),
#endif
END_DATADESC()
...
void CFuncPlat::HitBottom( void )
{
	...
	m_OnReachedEitherEnd.FireOutput(this, this);
#endif
}
...
{
	if ( CURTIME < m_flNextBlockTime )
		return;

	m_flNextBlockTime = CURTIME + 0.5;
...
{
#ifdef DARKINTERVAL // DI wants to be unobstructed by clamps
	SetSpeed( inputdata.value.Float() );
#else
...
void CFuncTrackTrain::SoundUpdate( void )
{
	...
	{
		if ( m_flNextMPSoundTime > CURTIME )
			return;

		m_flNextMPSoundTime = CURTIME + 1.0;
	}
	...
	if (!m_bSoundPlaying)
	{
		...
		m_flNextMoveSoundTime = CURTIME + RemapVal( flSpeedRatio, 0, 1, m_flMoveSoundMaxTime, m_flMoveSoundMinTime );
	...
	else
	{
		...
		if ( ( m_iszSoundMovePing != NULL_STRING ) && ( CURTIME > m_flNextMoveSoundTime ) )
		{
...
#ifndef PORTAL_COMPILE	//env_portal_path_track is a child of path_track and would like to get found
		 && !FClassnameIs( m_ppath, "env_portal_path_track" )
#endif //#ifndef PORTAL_COMPILE
		)
	{
