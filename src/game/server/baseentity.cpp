...
void CBaseEntity::AddTimedOverlay( const char *msg, int endTime )
{
	...
	pNewTO->msgEndTime = CURTIME + endTime;
	pNewTO->msgStartTime = CURTIME;
...
void CBaseEntity::DrawTimedOverlays(void)
{
	...
	while (pTO)
	{
		...
		if ((!CBaseEntity::Debug_IsPaused() && CURTIME > pTO->msgEndTime) ||
			(nCount > 10))
		{
			if (pLastTO)
			{
		...
		else
		{
			...
			{
				nAlpha = 255*((CURTIME - pTO->msgStartTime)/(pTO->msgEndTime - pTO->msgStartTime));
			}
			int r = 185;
			int g = 145;
			int b = 145;
...
void CBaseEntity::TakeDamage( const CTakeDamageInfo &inputInfo )
{
	...
	else
	{
		...
		//Msg("%s took %.2f Damage, at %.2f\n", GetClassname(), info.GetDamage(), CURTIME );
...
//	DEFINE_FIELD( m_ToolHandle,		FIELD_INTEGER ),
...
#ifdef DARKINTERVAL	// DI NEW - a whole bunch of convenience inputs
	...
	DEFINE_INPUTFUNC(FIELD_VOID, "MakeSolid", InputMakeSolid),
	DEFINE_INPUTFUNC(FIELD_VOID, "MakeNonSolid", InputMakeNonSolid),
	DEFINE_INPUTFUNC(FIELD_STRING, "SetTargetName", InputSetTargetName),
	DEFINE_INPUTFUNC(FIELD_STRING, "ClearTargetName", InputClearTargetName),
	...
	DEFINE_INPUTFUNC(FIELD_VOID, "RemoveNodraw", InputRemoveNodraw),
	DEFINE_INPUTFUNC(FIELD_VOID, "AddNodraw", InputAddNodraw),
...
{
#ifndef DARKINTERVAL // Unnecessary check anymore because DI permits picking up fleshy things
	IPhysicsObject *pList[VPHYSICS_MAX_OBJECT_LIST_COUNT];
	int count = VPhysicsGetObjectList( pList, ARRAYSIZE(pList) );
	for ( int i = 0; i < count; i++ )
	{
...
void CBaseEntity::ShadowCastDistThink( )
{
	...
	SetContextThink( NULL, CURTIME, "ShadowCastDistThink" );
}
...
void CBaseEntity::SetShadowCastDistance( float flDesiredDistance, float flDelay )
{
	...
	{
		SetContextThink( &CBaseEntity::ShadowCastDistThink, CURTIME + flDelay, "ShadowCastDistThink" );
	}
}
...
void CBaseEntity::DrawInputOverlay(const char *szInputName, CBaseEntity *pCaller, variant_t Value)
{
	...
	{
		Q_snprintf( bigstring,sizeof(bigstring), "%3.1f  (%s,%d) <-- (%s)\n", CURTIME, szInputName, Value.Int(), pCaller ? pCaller->GetDebugName() : NULL);
	}
	else if ( Value.FieldType() == FIELD_STRING )
	{
		Q_snprintf( bigstring,sizeof(bigstring), "%3.1f  (%s,%s) <-- (%s)\n", CURTIME, szInputName, Value.String(), pCaller ? pCaller->GetDebugName() : NULL);
	}
	else
	{
		Q_snprintf( bigstring,sizeof(bigstring), "%3.1f  (%s) <-- (%s)\n", CURTIME, szInputName, pCaller ? pCaller->GetDebugName() : NULL);
	}
...
void CBaseEntity::DrawOutputOverlay(CEventAction *ev)
{
	...
	if ( ev->m_flDelay )
	{
		Q_snprintf( bigstring,sizeof(bigstring), "%3.1f  (%s) --> (%s),%.1f) \n", CURTIME, STRING(ev->m_iTargetInput), STRING(ev->m_iTarget), ev->m_flDelay);
	}
	else
	{
		Q_snprintf( bigstring,sizeof(bigstring), "%3.1f  (%s) --> (%s)\n", CURTIME,  STRING(ev->m_iTargetInput), STRING(ev->m_iTarget));
	}
	AddTimedOverlay(bigstring, 10.0);
...
void CBaseEntity::OnEntityEvent( EntityEvent_t event, void *pEventData )
{
	...
	{
#ifdef DARKINTERVAL
		nNewContents = WL_WaterType_Water;
#else
	...
	{
#ifdef DARKINTERVAL
 		nNewContents = WL_WaterType_Slime;
#else
		nNewContents |= CONTENTS_SLIME;
#endif
	}
#ifdef DARKINTERVAL
	if (m_nColloidTouch > 0)
	{
		nNewContents = WL_WaterType_Colloid;
	}
#endif
	if (( nNewContents & MASK_WATER ) == 0)
	{
		SetWaterLevel( 0 );
#ifdef DARKINTERVAL
		SetWaterType(WL_WaterType_None);
#else
		...
		for ( int i = 0; i < dmap->dataNumFields; i++ )
		{
			if ( dmap->dataDesc[i].flags & FTYPEDESC_INPUT )
			{
				if ( !Q_stricmp(dmap->dataDesc[i].externalName, szInputName) )
				{
					...
					{
#ifndef DARKINTERVAL
						Q_snprintf( szBuffer, sizeof(szBuffer), "(%0.2f) input %s: %s.%s(%s)\n", CURTIME, STRING(pCaller->m_iName), GetDebugName(), szInputName, Value.String() );
#else
						Q_snprintf( szBuffer, sizeof(szBuffer), "(%0.2f) input %s: %s.%s(%s)\n", CURTIME, STRING(pCaller->m_iName.Get()), GetDebugName(), szInputName, Value.String() );
#endif
					}
					else
					{
						Q_snprintf( szBuffer, sizeof(szBuffer), "(%0.2f) input <NULL>: %s.%s(%s)\n", CURTIME, GetDebugName(), szInputName, Value.String() );
					}
					DevMsg( 2, "%s", szBuffer );
...
{
#ifdef DARKINTERVAL // safety checks
	if (this->IsPlayer()) return; // NEVER try to kill/delete the player in an input.

	if (this->ClassMatches("worldspawn"))
	{
		Msg("entity %s cannot be killed\n", GetDebugName());
		return; // NEVER try to kill/delete the world in an input.
	}
#endif
...
{
#ifdef DARKINTERVAL // convenience
	// if (errnousely) using this input to give parent to an entity that doesn't have one,
	// just interpret it as regular setparent and the name - as the entity name. 
	if (!GetMoveParent())
	{
		SetParent(inputdata.value.StringID(), inputdata.pActivator);
	}
...
#ifdef DARKINTERVAL
				pPlayer->SetLocalAngles(*newAngles);
#endif
			}
		}
...
{
#if 1 //ndef DARKINTERVAL // don't remember why this was removed
	if ( !pName || !pName[ 0 ] )
	{
		return;
	}
...
int CBaseEntity::PrecacheModel( const char *name, bool bPreload )
{
	...
	if ( !CBaseEntity::IsPrecacheAllowed() )
	{
		...
		{
#ifndef DARKINTERVAL // do not care
...
{
#ifdef DARKINTERVAL // more convenient than requiring developer 
...
#ifdef DARKINTERVAL
// Purpose : find_ent that can look for model names (including inline models i. e. for brushes)
void CC_Find_Ent_Model(const CCommand& args)
{
	if (args.ArgC() < 2)
	{
		Msg("Format: find_ent_model <model>\n");
		return;
	}

	int iCount = 0;
	const char *pszSubString = args[1];
	Msg("Searching for entities with model name or index containing substring: '%s'\n", pszSubString);

	CBaseEntity *ent = NULL;
	while ((ent = gEntList.NextEnt(ent)) != NULL)
	{
		const char *pszModelname = STRING(ent->GetModelName());

		bool bMatches = false;
		if (pszModelname && pszModelname[0])
		{
			if (Q_stristr(pszModelname, pszSubString))
			{
				bMatches = true;
			}
		}
		...
		{
			iCount++;
			Msg("   '%s' : '%s' : '%s' : (entindex %d) \nPosition: %.1f %.1f %.1f\n", 
				ent->GetClassname(), 
				ent->GetEntityName().ToCStr(), 
				ent->GetModelName().ToCStr(), 
				ent->entindex(),
				ent->GetAbsOrigin().x, ent->GetAbsOrigin().y, ent->GetAbsOrigin().z);

			if(developer.GetInt() > 0)
				SetDebugBits(UTIL_GetCommandClient(), ent->GetEntityName().ToCStr(), OVERLAY_TEXT_BIT);
		}
	}

	Msg("Found %d matches.\n", iCount);
}
static ConCommand find_ent_model("find_ent_model", CC_Find_Ent_Model, "Display data for entity matching specified model.\nFormat: find_ent_model <model>\n Can be model name or inline model number.\n", FCVAR_CHEAT);
#endif
// Purpose : 
//------------------------------------------------------------------------------
...
#ifndef DARKINTERVAL
		Assert( false ); // fluff this assert. Even the base games are choke full of this error
#endif
		return;
	}
...
bool CBaseEntity::ContextExpired( int index ) const
{
	...
	return ( m_ResponseContexts[ index ].m_fExpirationTime <= CURTIME );
}
...
#ifdef DARKINTERVAL
//-----------------------------------------------------------------------------
// Purpose: MakeSolid/NonSolid input handler
//-----------------------------------------------------------------------------
void CBaseEntity::InputMakeNonSolid(inputdata_t &inputdata)
{
	AddSolidFlags(FSOLID_NOT_SOLID);
}

void CBaseEntity::InputMakeSolid(inputdata_t &inputdata)
{
	RemoveSolidFlags(FSOLID_NOT_SOLID);
}

//-----------------------------------------------------------------------------
// Purpose: Replace this entity's targetname
//-----------------------------------------------------------------------------
void CBaseEntity::InputSetTargetName(inputdata_t &inputdata)
{
	this->SetName(AllocPooledString(inputdata.value.String()));
}

//-----------------------------------------------------------------------------
// Purpose: Replace this entity's targetname
//-----------------------------------------------------------------------------
void CBaseEntity::InputClearTargetName(inputdata_t &inputdata)
{
	this->SetName(AllocPooledString(NULL_STRING.ToCStr()));
}

//-----------------------------------------------------------------------------
// Purpose: Adds EF_NODRAW. Can be achieved with AddEffects, this input works
// as a convenience shortcut.
//-----------------------------------------------------------------------------
void CBaseEntity::InputAddNodraw(inputdata_t &inputdata)
{
	this->AddEffects(EF_NODRAW);
}

//-----------------------------------------------------------------------------
// Purpose: Removes EF_NODRAW. Can be achieved with RemoveEffects, this input 
// works as a convenience shortcut.
//-----------------------------------------------------------------------------
void CBaseEntity::InputRemoveNodraw(inputdata_t &inputdata)
{
	this->RemoveEffects(EF_NODRAW);
}

//-----------------------------------------------------------------------------
// Purpose: Passes the integer value into AddEffects function on this entity.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Passes the integer value into RemoveEffects function on this entity.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Passes the integer value into AddSpawnFlags function on this entity.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Passes the integer value into RemoveSpawnFlags function on this entity.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Sets the render mode of this entity via passed integer value.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Sets the render FX of this entity via passed integer value.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Passes the integer value into AddEFlags function on this entity.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Passes the integer value into RemoveEFlags function on this entity.
//-----------------------------------------------------------------------------
	...
	while (dmap)
	{
		...
		for (int i = 0; i < fields; i++)
		{
			...
			if ((dataDesc->fieldType == FIELD_CUSTOM) && (dataDesc->flags & FTYPEDESC_OUTPUT))
			{
				...
				if (NamesMatch(szOutput, MAKE_STRING(dataDesc->externalName)))
				{
					CBaseEntityOutput *pOutput = (CBaseEntityOutput *)((int)this + (int)dataDesc->fieldOffset[0]);
...
//-----------------------------------------------------------------------------
// Purpose: In addition to the input itself, parameter may include !activator, 
// !caller, and what to pass.
//-----------------------------------------------------------------------------
