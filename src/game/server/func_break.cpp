...
#ifdef DARKINTERVAL // for certain new types of break, such as web
...
#ifdef DARKINTERVAL	// DI NEW, customisable hit sound and pressure strain sound
...
#ifdef DARKINTERVAL // for certain new types of break, such as web
	PrecacheScriptSound( "Web.ImpactBreak");
	PrecacheParticleSystem("web_impact_01");
	PrecacheParticleSystem("web_impact_break");
#endif
}
...
{
#ifdef DARKINTERVAL // for directed particles
	QAngle ptrAngle;
	VectorAngles(ptr->endpos, ptrAngle);
#endif
	...
	if (random->RandomInt(0,1) )
	{
		switch( m_Material )
		{
			...
			case matWeb:
			{
				DispatchParticleEffect("web_impact_01", ptr->endpos, ptrAngle );
			}
			break;
#endif
			case matUnbreakableGlass:
...
#endif
#ifdef DARKINTERVAL
		if ( m_Material == matWeb )
		{
			iCount = 0;
			QAngle angle;
			VectorAngles(vecSpot, angle);
			DispatchParticleEffect("web_impact_break", vecSpot, angle);
		}
#endif
