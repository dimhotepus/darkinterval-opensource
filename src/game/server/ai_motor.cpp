...
void CAI_Motor::MoveClimbStart(  const Vector &climbDest, const Vector &climbDir, float climbDist, float yaw  )
{
	...
	if ( fabsf( climbDir.z ) < .1f )
	{
#ifdef DARKINTERVAL
		SetActivity( ACT_STEP_BACK ); // todo, make it a setting in AI?
		// Help the NPC get on ladder - push them backwards a bit
		//	Vector forward;
		//	GetOuter()->GetVectors(&forward, NULL, NULL);
		//	GetOuter()->ApplyAbsVelocityImpulse(forward * -500);
#else
...
{
//	NDebugOverlay::Cross3D(climbDest, 16.0f, 255, 0, 0, true, 0.1f); 
//	CFmtStr str("Dist is %.1f, fabs is %.1f\n", climbDist, fabs(m_vecDismount.z));
//	NDebugOverlay::EntityTextAtPosition(GetOuter()->EyePosition(), 0, str.Access(), 0.1f);
...
AIMoveResult_t CAI_Motor::MoveJumpStop()
{
	...
	if (GetOuter()->GetActivity() == ACT_GLIDE)
	{
		...
		// GetOuter()->SetCycle( flTime - CURTIME );
	}
...
void CAI_Motor::UpdateYaw( int yawSpeed )
{
	...
	float dt = MIN( 0.2, CURTIME - GetLastThink() );
