...
#define	FRONT_AXLE			0
#ifdef DARKINTERVAL
#define MAX_GUAGE_SPEED		50.0	// DI REVISIT: made 50 for APC in Part 2. Revise the whole thing later.
#else
...
#ifdef DARKINTERVAL
	char tempstr[512];
	Q_snprintf(tempstr, sizeof(tempstr), "Health: %d", m_pOuter->GetHealth());
...
bool CFourWheelVehiclePhysics::Think()
{
	...
	{
		m_bLastBoost = true;
		m_turboTimer = CURTIME + 2.75f;		// min duration for turbo sound
	}
	else if( !IsBoosting() && m_bLastBoost )
	{
		if ( CURTIME >= m_turboTimer )
		{
			m_bLastBoost = false;
		}
	}
...
#ifdef DARKINTERVAL
	float flSteerAdj = RemapValClamped( fabs( steering ), 0.0f, 1.0f, 0.0f, 1.0f );
#else
...
void CFourWheelVehiclePhysics::UpdateDriverControls( CUserCmd *cmd, float flFrameTime )
{
	...
	if( cmd->forwardmove > 0.0f )
	{
		...
		{
#ifdef DARKINTERVAL
			m_controls.brake = Approach( BRAKE_MAX_VALUE, m_controls.brake, flFrameTime * 1.5f * BRAKE_BACK_FORWARD_SCALAR );
#else	
	...
	else if( cmd->forwardmove < 0.0f )
	{
		...
		{
#ifdef DARKINTERVAL
			m_controls.brake = Approach( BRAKE_MAX_VALUE, m_controls.brake, flFrameTime * 1.5f );
#else
	...
	{
		m_throttleStartTime = CURTIME;		// need to track how long throttle is down
		m_bLastThrottle = true;
	}
	// throttle up now but not before??
	else if ( !bThrottle && m_bLastThrottle && IsEngineDisabled() == false )
	{
		m_throttleActiveTime = CURTIME - m_throttleStartTime;
		m_bLastThrottle = false;
	}
