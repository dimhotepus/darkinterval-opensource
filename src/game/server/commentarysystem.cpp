...
public:
...
#ifdef DARKINTERVAL
		pkvFile->deleteThis(); // memory leak fix from VDC
#endif
	...
	{
		if ( m_flNextTeleportTime > CURTIME || m_iTeleportStage != TELEPORT_NONE )
			return;
...
void CPointCommentaryNode::StartCommentary( void )
{
	...
	if ( m_hViewTarget || m_hViewPosition.Get() )
	{
		m_vecOriginalAngles = pPlayer->EyeAngles();
 		SetContextThink( &CPointCommentaryNode::UpdateViewThink, CURTIME, s_pCommentaryUpdateViewThink );
	}

	//SetContextThink( &CPointCommentaryNode::FinishCommentary, CURTIME + flDuration, s_pFinishCommentaryThink );
}
...
void CPointCommentaryNode::UpdateViewThink( void )
{
	...
	if ( m_hViewTarget )
	{
		...
		SetNextThink( CURTIME, s_pCommentaryUpdateViewThink );
	}
 	...
 	if ( m_hViewPosition.Get() )
	{
		...
		// Blend to the target position over time. 
 		float flCurTime = (CURTIME - m_flStartTime);
...
void CPointCommentaryNode::UpdateViewPostThink( void )
{
 	...
 	if ( m_hViewPosition.Get() && m_hViewPositionMover )
	{
 		// Blend back to the player's position over time.
   		float flCurTime = (CURTIME - m_flFinishedTime);
...
void CPointCommentaryNode::FinishCommentary( bool bBlendOut )
{
	...
	if ( bBlendOut && m_hViewPositionMover )
	{
		...
		SetContextThink( &CPointCommentaryNode::UpdateViewPostThink, CURTIME, s_pCommentaryUpdateViewThink );
		return;
	}
