...
#ifdef DARKINTERVAL // tweak humanoid damage convars.
...
#ifdef DARKINTERVAL // tweak humanoid damage convars.
float CAI_BaseHumanoid::GetHitgroupDamageMultiplier(int iHitGroup, const CTakeDamageInfo &info)
{
	switch (iHitGroup)
	{
	...
	case HITGROUP_HEAD:
	{
		if( (info.GetDamageType() == DMG_CLUB) 
			...
			return sk_npc_head.GetFloat() * RandomFloat( 2.0f, 3.0f ); // DI NEW this is made mainly with metrocops in mind. 2.0 makes them die from two crowbar hits (or one alt-attack hit). 3.0 makes them die from a single hit in the head.
		else
			return sk_npc_head.GetFloat() * 1.5f;
	}
...
//=========================================================
// BaseHumanoid clone minus blended animation requirements. 
// Stupid hack to get a BaseActor clone that can do choreo
// and behaviors without needing blended movement. 
// For example, houndeyes and hoppers don't need blended.
//=========================================================
#ifdef DARKINTERVAL
bool CAI_BaseHumanoidNoBlend::HandleInteraction(int interactionType, void *data, CBaseCombatCharacter* sourceEnt)
{
	// Annoying to ifdef this out. Copy it into all the HL2 specific humanoid NPC's instead?
	if (interactionType == g_interactionBarnacleVictimDangle)
	{
...
void CAI_BaseHumanoidNoBlend::CheckAmmo(void)
{
	BaseClass::CheckAmmo();
...
void CAI_BaseHumanoidNoBlend::BuildScheduleTestBits()
{
	BaseClass::BuildScheduleTestBits();
...
bool CAI_BaseHumanoidNoBlend::OnMoveBlocked(AIMoveResult_t *pResult)
{
	if (*pResult != AIMR_BLOCKED_NPC && GetNavigator()->GetBlockingEntity() && !GetNavigator()->GetBlockingEntity()->IsNPC())
	{
		CBaseEntity *pBlocker = GetNavigator()->GetBlockingEntity();
...
void CAI_BaseHumanoidNoBlend::TraceAttack(const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr)      //)
{
	bool bSneakAttacked = false;
...
// DI NEW: tweak humanoid damage convars.
float CAI_BaseHumanoidNoBlend::GetHitgroupDamageMultiplier(int iHitGroup, const CTakeDamageInfo &info)
{
	switch (iHitGroup)
	{
	case HITGROUP_GENERIC:
...
void CAI_BaseHumanoidNoBlend::StartTaskRangeAttack1(const Task_t *pTask)
{
	if ((CapabilitiesGet() & bits_CAP_USE_SHOT_REGULATOR) == 0)
	{
		BaseClass::StartTask(pTask);
...
void CAI_BaseHumanoidNoBlend::StartTask(const Task_t *pTask)
{
	switch (pTask->iTask)
	{
	case TASK_RANGE_ATTACK1:
...
void CAI_BaseHumanoidNoBlend::RunTaskRangeAttack1(const Task_t *pTask)
{
	if ((CapabilitiesGet() & bits_CAP_USE_SHOT_REGULATOR) == 0)
	{
		BaseClass::RunTask(pTask);
...
void CAI_BaseHumanoidNoBlend::RunTask(const Task_t *pTask)
{
	switch (pTask->iTask)
	{
	case TASK_RANGE_ATTACK1:
