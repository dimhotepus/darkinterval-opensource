...
#ifdef DARKINTERVAL
	ADD_CONDITION_TO_SR( COND_HEAR_HIDE_POS ); // e. g. a point inside smoke grenade, good idea to hide in
#endif
...
#ifdef DARKINTERVAL
	ADD_CONDITION_TO_SR( COND_ENEMY_VULNERABLE);
	// DI NEW: Live Ragdolls Experiment (inspired by Dark Messiah)
	ADD_CONDITION_TO_SR(COND_DI_FALL_IN_RAGDOLL);
	ADD_CONDITION_TO_SR(COND_DI_IS_IN_LIVE_RAGDOLL);
#endif
}
