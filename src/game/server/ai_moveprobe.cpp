...
//	if ( r_visualizetraces.GetBool() )
	if (CommandLine()->FindParm("-visualisetraces")
		|| CommandLine()->FindParm("-visualizetraces")
		|| CommandLine()->FindParm("-vt"))
	{
...
#ifdef DARKINTERVAL
	if ( abs(gravity.z) < 0.01 ) // DI CHANGE: allow negative gravities for NPCs. Meant for ceiling crawlers but unfinished
#else
	if ( gravity.z < 0.01 )
#endif
	{
...
#ifdef DARKINTERVAL
//-----------------------------------------------------------------------------
// Purpose: DI NEW - water nodes
//-----------------------------------------------------------------------------
void CAI_MoveProbe::SwimMoveLimit(const Vector &vecStart, const Vector &vecEnd,
	unsigned int collisionMask, const CBaseEntity *pTarget, AIMoveTrace_t *pMoveTrace) const
{
	// NOTE: Never call this directly!!! Always use MoveLimit!!
	// This assertion should ensure this happens
	...
	if (tr.fraction < 1)
	{
		...
		pMoveTrace->flDistObstructed = ComputePathDistance(NAV_SWIM, tr.endpos, vecEnd);
		pMoveTrace->vEndPosition = tr.endpos;
		return;
	}
	...
	float pctToCheckStandPositions, unsigned flags, AIMoveTrace_t* pTrace)
{
	...
	case NAV_SWIM: // DI NEW - water nodes
		SwimMoveLimit(vecStart, vecEnd, collisionMask, pTarget, pTrace);
		break;
#endif
	default:
