	...
	if ( HasGoal() )
	{
		...
		if ( m_weaponname != NULL_STRING )
		{
			...
			if ( pFollower && !pFollower->Weapon_OwnsThisType( STRING(m_weaponname) ) )
			{
				...
				if ( !m_flWeaponSafetyTimeOut || (m_flWeaponSafetyTimeOut > CURTIME) )
...
void CAI_LeadBehavior::StartTask( const Task_t *pTask )
{
	switch ( pTask->iTask )
	{
		...
		case TASK_WAIT_FOR_SPEAK_FINISH:
		{
			...
			{
				if ( CURTIME - GetOuter()->GetTimeTaskStarted() > 0.3 )
				{
					TaskComplete();
				}
			}
	...
	switch ( pTask->iTask )
	{
		...
		case TASK_LEAD_WALK_PATH:
		{
			...
			if ( TaskIsRunning() && IsCurSchedule( SCHED_LEAD_PLAYER, false ) )
			{
				...
				if ( m_flNextLeadIdle < CURTIME && !IsSpeaking() )
				{
					m_flNextLeadIdle = CURTIME + RandomFloat( 10,15 );
...
bool CAI_LeadBehavior::Speak( AIConcept_t concept )
{
	...
	if ( bNag )
	{
		if ( m_flSpeakNextNagTime > CURTIME )
		{
	...
	{
		m_flSpeakNextNagTime = CURTIME + LEAD_NAG_TIME;
		return true;
	}
