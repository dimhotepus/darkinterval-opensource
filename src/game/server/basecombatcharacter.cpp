...
#include "nav_mesh.h"
#ifdef DARKINTERVAL // death poses fix
...
#else
#ifndef _RETAIL																																										//ConVar ai_use_visibility_cache( "ai_use_visibility_cache", "1" );
...
void *SendProxy_SendBaseCombatCharacterLocalDataTable( const SendProp *pProp, const void *pStruct, const void *pVarData, CSendProxyRecipients *pRecipients, int objectID )
{
	...
	if ( pBCC != NULL)
	{
		...
		{
			// If it's a vehicle, send to "driver"
...
#ifdef DARKINTERVAL // todo - this isn't really glow. It's the filled colour overlay for the OICW zoom. This needs to be reworked at some point.
	SendPropFloat(SENDINFO(m_fGlowColorRed)),
	SendPropFloat(SENDINFO(m_fGlowColorGreen)),
	SendPropFloat(SENDINFO(m_fGlowColorBlue)),
#endif
#endif // GLOWS_ENABLE
...
bool CBaseCombatCharacter::FVisible( CBaseEntity *pEntity, int traceMask, CBaseEntity **ppBlocker )
{
	...
	{
		if ( CURTIME - g_VisibilityCache[iCache].time < VIS_CACHE_ENTRY_LIFE )
		{
	...
	g_VisibilityCache[iCache].time = CURTIME;
...
#ifdef DARKINTERVAL
// Combat confidence, moved to base class from DI squid npc
int CBaseCombatCharacter::CombatConfidence(void)
{
	m_iCombatConfidence = m_iHealth + RandomFloat(-5.0f, 10.0f); // generally dependent on health, with some random inclinations
																 //	if (GetSquad()->NumMembers() < 2) m_flCombatConfidence /= 2; // we're the last one standing - cut confidence in half
	if (m_iCombatConfidence < 10) m_iCombatConfidence = 10; // min clamp at 10
	if (m_iCombatConfidence > 100) m_iCombatConfidence = 100; // max clamp at 100

	if (m_iMaxCombatConfidence > 0) m_iCombatConfidence = m_iMaxCombatConfidence;
	return m_iCombatConfidence;
}

void CBaseCombatCharacter::InputSetCombatConfidence(inputdata_t &inputdata)
{
	m_iMaxCombatConfidence = inputdata.value.Int();
}

void CBaseCombatCharacter::InputSetFlexWeight(inputdata_t &inputdata)
{
	// Parse the keyvalue data
	char parseString[32];
	Q_strncpy(parseString, inputdata.value.String(), sizeof(parseString));

	// Look for the flex int
	char *flexString = strtok(parseString, " ");
	while (flexString)
	{
		// Get the float value
		char *flexFloatString = strtok(NULL, " ");
		if (flexFloatString)
		{
			float value = atof(flexFloatString);
			SetFlexWeight(flexString, value);
			return;
		}
	}
}
...
{
#ifndef DARKINTERVAL // because DI allows saving NPCs
#ifdef HL2_DLL
...
CBaseCombatCharacter::CBaseCombatCharacter( void )
{
...
#ifdef DARKINTERVAL
	m_bForceServerRagdoll = HasSpawnFlags(SF_ALWAYS_SERVER_RAGDOLL);

	m_iCombatConfidence = 100; // spawn with full confidence
	m_iMaxCombatConfidence = 0;
#else
	...
	m_fGlowColorRed.Set(1.0);
	m_fGlowColorGreen.Set(1.0);
	m_fGlowColorBlue.Set(1.0);
#endif // GLOWS_ENABLE
}
...
#ifdef DARKINTERVAL
	DevMsg("Actor %s selected death activity number %d\n", GetDebugName(), (int)deathActivity);
#endif
...
CBaseEntity *CBaseCombatCharacter::CheckTraceHullAttack( const Vector &vStart, const Vector &vEnd, const Vector &mins, const Vector &maxs, int iDamage, int iDmgType, float flForceScale, bool bDamageAnyNPC )
{
...
#ifndef DARKINTERVAL // DI CHANGE. Ep2 uses check for COLLISION_GROUP_PROJECTILE. 
	// However, for DI purposes, collision between that group and player, are disabled.
	// This in turn breaks melee attacks for several classes - zombie, antlion, etc.
	// so for the time being, let's revert to original check w/ COLLISION_GROUP_NONE.
	...
	if ( IRelationType( pEntity ) == D_HT )
	{
		if ( iDamage > 0 )
		{
			...
			CalculateMeleeDamageForce( &info, (vEnd - vStart), vStart, flForceScale );
			pEntity->TakeDamage( info );
		}
	}
	return pEntity;
...
bool CBaseCombatCharacter::BecomeRagdollBoogie( CBaseEntity *pKiller, const Vector &forceVector, float duration, int flags )
{
	...
	CRagdollBoogie::Create( pRagdoll, 200, CURTIME, duration, flags );
...
//-----------------------------------------------------------------------------
#ifdef DARKINTERVAL
ConVar g_test_server_deathposes("g_test_server_deathposes", "0");
#endif
bool CBaseCombatCharacter::BecomeRagdoll( const CTakeDamageInfo &info, const Vector &forceVector )
{
	if ( (info.GetDamageType() & DMG_VEHICLE) )
	{
		...
		CBaseEntity *pRagdoll = CreateServerRagdoll( this, m_nForceBone, info2, COLLISION_GROUP_INTERACTIVE_DEBRIS, true, ACT_DIERAGDOLL, CanGibRagdoll(), GetMaxHealth() * 4);
	...
	{
		CBaseEntity *pRagdoll = CreateServerRagdoll( this, m_nForceBone, newinfo, COLLISION_GROUP_DEBRIS, false, ACT_DIERAGDOLL, CanGibRagdoll(), GetMaxHealth() * 4);
		FixupBurningServerRagdoll( pRagdoll );
		RemoveDeferred();
		return true;
	}
...
#ifdef DARKINTERVAL
	if ( HasSpawnFlags(SF_ALWAYS_SERVER_RAGDOLL) || m_bForceServerRagdoll == true 
		|| ( ( bMegaPhyscannonActive == true ) && !IsPlayer() 
		&& Classify() != CLASS_PLAYER_ALLY_VITAL && Classify() != CLASS_PLAYER_ALLY ) )
#else
	if ( m_bForceServerRagdoll == true || ( ( bMegaPhyscannonActive == true ) 
		&& !IsPlayer() && Classify() != CLASS_PLAYER_ALLY_VITAL 
...
#ifdef DARKINTERVAL
		Activity death = (g_test_server_deathposes.GetBool()) ? GetDeathActivity() : ACT_DIERAGDOLL;

		if(g_test_server_deathposes.GetBool())
			SetSequence(SelectWeightedSequence(ACT_RUN));
		CBaseEntity *pRagdoll = CreateServerRagdoll( this, m_nForceBone, newinfo, COLLISION_GROUP_INTERACTIVE_DEBRIS, true/*, death*/, ACT_DIERAGDOLL, CanGibRagdoll(), GetMaxHealth());
#else
...
void CBaseCombatCharacter::Event_Killed( const CTakeDamageInfo &info )
{
	...
	if ( ShouldGib( info ) == false )
	{
...
#ifdef DARKINTERVAL
		if ((info.GetDamageType() & DMG_DISSOLVE_BURN) && CanBecomeRagdoll())
		{
			int nDissolveType = ENTITY_DISSOLVE_BURN;
			bRagdollCreated = Dissolve(NULL, CURTIME, false, nDissolveType);

			// Also dissolve any weapons we dropped
			if (pDroppedWeapon)
			{
				pDroppedWeapon->Dissolve(NULL, CURTIME, false, nDissolveType);
			}
		}

		else if ( (info.GetDamageType() & DMG_DISSOLVE) && CanBecomeRagdoll() )
#else
		...
		else if ( PlayerHasMegaPhysCannon() )
		{
			if ( pDroppedWeapon )
			{
				pDroppedWeapon->Dissolve( NULL, CURTIME, false, ENTITY_DISSOLVE_NORMAL );
			}
		}
...
void CBaseCombatCharacter::Weapon_Drop( CBaseCombatWeapon *pWeapon, const Vector *pvecTarget /* = NULL */, const Vector *pVelocity /* = NULL */ )
{
	...
	else
	{
		...
		{
			// Nowhere in particular; just drop it.
			float throwForce = ( IsPlayer() ) ? 100.0f : random->RandomInt( 64, 128 );
...
//-----------------------------------------------------------------------------
#ifdef DARKINTERVAL
void CBaseCombatCharacter::Weapon_Equip( CBaseCombatWeapon *pWeapon, bool bSwitchToNewWeapon)
#else
...
#endif
{
	...
	{
#ifndef DARKINTERVAL // DI doesn't care, although you can play HL2 maps inside of DI
...
#ifdef DARKINTERVAL
	if ( IsPlayer() == false && bSwitchToNewWeapon )
#else
	if ( IsPlayer() == false )
#endif
	{
...
#endif
	{
		...
		{
#ifdef DARKINTERVAL  // Do not truncate int to float.
			m_hActiveWeapon->m_fMaxRange1 = FLT_MAX;
			m_hActiveWeapon->m_fMaxRange2 = FLT_MAX;
#else
...
int CBaseCombatCharacter::OnTakeDamage( const CTakeDamageInfo &info )
{
	...
	if ( info.GetDamageType() & DMG_SHOCK )
	{
		...
		// DI todo - consider some electrical effects?
	}
...
#ifdef DARKINTERVAL // in DI, ranged weapons like the pistol could be acquired before infinite weapons like the crowbar, 
	// and be the only weapon. In that case, autoswitch should be prohibited so the weapon can be 
	// deployed even if empty (reported user case: Radiostation or Subt, spend all ammo, 
	// then pick up a prop - weapon now can't be properly redeployed after letting go of the prop)
	// This code is a bit of a hack to redeploy the current hidden weapon upon picking up ammo for it.

	// Note, this is disabled until edge cases can be sufficiently accounted for.
	// For example, it counters stuff like impulse 200.
//	if (GetActiveWeapon() && (GetActiveWeapon()->GetPrimaryAmmoType() == iAmmoIndex || (GetActiveWeapon()->GetSecondaryAmmoType() == iAmmoIndex)))
...
#ifdef DARKINTERVAL
			 FClassnameIs( pEnts[i], "prop_ragdoll") ||
#endif
...
{
#ifdef DARKINTERVAL // DI todo - this whole system doesn't work, apparently
	fogparams_t fog;
//	GetFogParams( &fog );
	...
	float ratio = (range - 100) / (1000 - 100);
...
{
#ifdef DARKINTERVAL // DI is singleplayer, so it ditched teams
	return area != NULL;
#else
...
#ifdef DARKINTERVAL
// DI NEW - combat confidence/morale
DEFINE_FIELD(m_iCombatConfidence, FIELD_INTEGER),
DEFINE_FIELD(m_iMaxCombatConfidence, FIELD_INTEGER),
DEFINE_INPUTFUNC(FIELD_INTEGER, "SetCombatConfidence", InputSetCombatConfidence),
// DI NEW - more flex functionality
DEFINE_INPUTFUNC(FIELD_STRING, "SetFlexWeight", InputSetFlexWeight),
#endif
...
#ifdef DARKINTERVAL
DEFINE_FIELD(m_remember_transmit_state_int, FIELD_INTEGER),
#endif
END_DATADESC()
