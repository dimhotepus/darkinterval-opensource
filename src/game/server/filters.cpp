...
END_DATADESC()
#ifdef DARKINTERVAL // to filter out things like APC selectively
// ###################################################################
//	> CFilterVehicleClass
// ###################################################################
class CFilterVehicleClass : public CBaseFilter
{
	DECLARE_CLASS(CFilterVehicleClass, CBaseFilter);
	DECLARE_DATADESC();
...
public:
	...
	bool PassesFilterImpl(CBaseEntity *pCaller, CBaseEntity *pEntity)
	{
		CBasePlayer *pPlayerEnt = ToBasePlayer(pEntity);
		if (!pPlayerEnt) return false;
		if (!pPlayerEnt->IsInAVehicle()) return false;

		return pPlayerEnt->GetVehicleEntity()->ClassMatches(STRING(m_iFilterClass));
	}
};

LINK_ENTITY_TO_CLASS(filter_driven_vehicle_class, CFilterVehicleClass);

BEGIN_DATADESC(CFilterVehicleClass)
