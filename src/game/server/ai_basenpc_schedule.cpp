...
void CAI_BaseNPC::SetSchedule( CAI_Schedule *pNewSchedule )
{
	...
	m_ScheduleState.timeCurTaskStarted = m_ScheduleState.timeStarted = CURTIME;
...
// you of every schedule selection the NPC makes.
...
#endif
	{
		...
		choice.m_flTimeSelected = CURTIME;
...
void CAI_BaseNPC::MaintainSchedule ( void )
{
	...
	for ( i = 0; i < MAX_TASKS_RUN && !bStopProcessing; i++ )
	{
		...
		{
#ifdef DARKINTERVAL // more specific error messages
			if (!GetCurSchedule())
			{
				DevMsg("ERROR: Missing or invalid schedule - couldn't find schedule!\n");
			}
			if (GetCurSchedule()->NumTasks() == 0)
			{
				DevMsg("ERROR: Missing or invalid schedule - zero tasks!\n");
			}
		...
		{	
			...
			m_ScheduleState.timeCurTaskStarted = CURTIME;
			...
			// DevMsg( "%.2f StartTask( %s )\n", CURTIME, m_pTaskSR->GetStringText( pTask->iTask ) );
		}
...
#ifdef DARKINTERVAL // prototype physics cover code
bool CAI_BaseNPC::FindPhysicsCoverInRadius(CBaseEntity *pEnemy, const Vector &goalPos, Vector *pResult/*, CBaseEntity *pTarget*/)
{
	AI_PROFILE_SCOPE(CAI_BaseNPC_FindPhysicsCoverInRadius);
	...
	Vector					coverPos = vec3_invalid;
//	CAI_TacticalServices *	pTacticalServices = GetTacticalServices();
	const Vector &			enemyPos = pEnemy->GetLocalOrigin();
	Vector					enemyEyePos = pEnemy->EyePosition();
	/////////
	...
	vecDirToEnemy = enemyPos - GetLocalOrigin();
	float dist = VectorNormalize(vecDirToEnemy);
	vecDirToEnemy.z = 32;
	
	float flNearestDist = MIN(dist, 512.0*512.0 * 1.0);
	Vector vecDelta(flNearestDist, flNearestDist, GetHullHeight() * 1.0);
	
	class CPhysicsCoverEnum : public CFlaggedEntitiesEnum
	{
	public:
		CPhysicsCoverEnum(CBaseEntity **pList, int listMax )
		...
		virtual IterationRetval_t EnumElement(IHandleEntity *pHandleEntity)
		{
			...
			IPhysicsObject *e_PhysObj;
			Vector objMins, objMaxs;

			if (pEntity && (FClassnameIs(pEntity, "prop_physics")) || (FClassnameIs(pEntity, "prop_dynamic")))
			{
				e_PhysObj = pEntity->VPhysicsGetObject();
				objMins = pEntity->WorldAlignMins(); 
				objMaxs = pEntity->WorldAlignMaxs();
			//	vcollide_t *pCollide = modelinfo->GetVCollide(pEntity->GetModelIndex());
			//	physcollision->CollideGetAABB(&objMins, &objMaxs, pCollide->solids[0], pEntity->GetLocalOrigin(), pEntity->GetLocalAngles());

				if (e_PhysObj 
					&& e_PhysObj->IsAsleep() 
					&& pEntity->GetCollisionGroup() != COLLISION_GROUP_DEBRIS)
				{
					NDebugOverlay::Box(pEntity->GetLocalOrigin(), 
						objMins, objMaxs, 0, 25, 255, 75, 5.0f);
	...
	CPhysicsCoverEnum CoverEnum(pList, 64);

	int count = UTIL_EntitiesInBox(goalPos - vecDelta, goalPos + vecDelta, &CoverEnum);

//	NDebugOverlay::Box(goalPos, goalPos - vecDelta, goalPos + vecDelta, 0, 255, 225, 150, 5.0f);
	...
	for (i = 0; i < count; i++)
	{
		...
		if ((pPhysObj && pPhysObj->IsAsleep()))
		{
			...
			flDist = UTIL_DistApprox2D(GetLocalOrigin(), center);

			if (flDist >= flNearestDist)
			{
				Msg("Farther than nearest dist\n");
				NDebugOverlay::Box(pList[i]->GetLocalOrigin(), pList[i]->WorldAlignMins(), pList[i]->WorldAlignMaxs(), 255, 5, 5, 100, 5.0f);
				continue;
			}

			if (center.z > EyePosition().z)
			{
				NDebugOverlay::Box(pList[i]->GetLocalOrigin(), pList[i]->WorldAlignMins(), pList[i]->WorldAlignMaxs(), 255, 255, 5, 100, 5.0f);
				Msg("Above me\n");
				continue;
			}
		}
	...
	Vector goalBottom = pNearest->GetLocalOrigin() + Vector(32, 0, 0);;
	//	goalBottom.z = 0;
	coverPos = goalBottom;
	NDebugOverlay::Line(coverPos, goalPos, 255, 255, 0, false, 1.0f);
	NDebugOverlay::Box(pNearest->GetLocalOrigin(), pNearest->WorldAlignMins(), pNearest->WorldAlignMaxs(), 255, 255, 255, 100, 5.0f);	
...
//	*pTarget = pNearest;
	...
	//Vector vecWaistHigh;
	//CollisionProp()->NormalizedToWorldSpace(Vector(0.5f, 0.5f, 0.25f), &vecWaistHigh);

	for (i = 0; i < count; i++)
	{
		pPhysObj = pList[i]->VPhysicsGetObject();

		//	Assert(!(!pPhysObj || !pPhysObj->IsAsleep()));
		...
		if ((pPhysObj && pPhysObj->IsAsleep()))
		{
			...
			/*
					...
					if (flDist <= UTIL_DistApprox2D(center, GetEnemy()->GetLocalOrigin()))
						continue;
			*/
			/*
					...
					if ((center.z + pList[i]->BoundingRadius()) < vecWaistHigh.z)
						continue;
			*/
			...
			//		Vector nearestBottom = pNearest->WorldSpaceCenter();
			//		nearestBottom.z = 0;
			//		NDebugOverlay::Box(nearestBottom, objMins, objMaxs, 0, 255, 25, 175, 5.0f);
		}
		...
		if (pNearest != NULL)
		{
			coverPos = pNearest->WorldSpaceCenter() + Vector(16, 0, 0);
			NDebugOverlay::Cross3D(coverPos, 16.0f, 255, 0, 0, false, 5.0f);
		}
		/////////
	}
#endif
			...
			else 
			{
				...
				if (!GetNavigator()->SetGoal( goal, AIN_DISCARD_IF_FAIL ))
				{
					if ( GetNavigator()->GetNavFailCounter() == 0 )
					{
						...
						Warning("%s %s failed Urgent Movement, retrying\n", GetDebugName(), TaskName( task ) );
						return;
					}

					// FIXME: scripted sequences don't actually know how to handle failure, but we're failing.  This is serious
					Warning("%s %s failed Urgent Movement, teleporting NPC to goal point\n", GetDebugName(), TaskName( task ) );
#ifdef DARKINTERVAL // in DI, scripted sequences can force teleport stuck actor instead of failing
					SetLocalOrigin(m_hTargetEnt->GetAbsOrigin());
#else
...
void CAI_BaseNPC::StartTask( const Task_t *pTask )
{
	...
	case TASK_DEFER_DODGE:
		m_flNextDodgeTime = CURTIME + pTask->flTaskData;
		TaskComplete();
		break;
	...
	case TASK_LOCK_HINTNODE:
	{
		...
		{
#ifdef DARKINTERVAL
			DevMsg("TASK_LOCK_HINTNODE failed - no hintnode!\n");
#endif
			TaskFail(FAIL_NO_HINT_NODE);
		}
	...
	case TASK_DI_FALL_IN_RAGDOLL_PUSH_BEGIN: // create ragdoll copy, become invisible and intangible
	{
		if ((CapabilitiesGet() & bits_CAP_LIVE_RAGDOLL) == false)
		{
			TaskFail("this npc cannot be ragdolled");
			return;
		}
		...
		m_flUnragdollTime = CURTIME + 30; // extreme case

		SetCondition(COND_DI_IS_IN_LIVE_RAGDOLL);
		TaskComplete();
		return;
	}
	break;

	case TASK_DI_FALL_IN_RAGDOLL_PUSH: // create ragdoll copy, become invisible and intangible
	{
	//	SetRenderMode(kRenderNone); // NPC stays visible now, the ragdoll goes invisible instead
		m_rememberMoveType = (int)GetMoveType();
		SetMoveType(MOVETYPE_NONE);
		m_rememberSolidFlags = GetSolidFlags();
		...
		CBaseEntity *pCreateRagdoll = CreateServerRagdoll(this, -1, info, COLLISION_GROUP_NONE, false, ACT_DIERAGDOLL, false, 100, true);
		if (!pCreateRagdoll)
		{
			DevWarning(2, "Failed to create ragdoll double (%s, %s)!!\n", GetClassname(), GetDebugName());
			RestoreNPCAfterRagdoll();
			TaskFail("failed to create ragdoll double");
			return;
		}
		m_live_ragdoll_handle = static_cast<CRagdollProp*>(pCreateRagdoll);
		...
		else // (m_live_ragdoll_handle)
		{
			m_live_ragdoll_ent = m_live_ragdoll_handle; // set bone "glueing" ragdoll handle
			AddEffects(EF_NOSHADOW); // apply AFTER ragdoll is created so it doesn't inherit the flag
			SetNextThink(CURTIME);
			m_live_ragdoll_handle->SetRenderMode(kRenderNone);
			m_live_ragdoll_handle->AddSolidFlags( FSOLID_CUSTOMBOXTEST);
			m_live_ragdoll_handle->SetNextThink(CURTIME);
			m_live_ragdoll_handle->SetCollisionGroup(COLLISION_GROUP_DEBRIS);
			m_live_ragdoll_handle->SetUnragdoll(this);
			//
			m_live_ragdoll_handle->SetDamageEntity(this);
			m_live_ragdoll_handle->m_takedamage = DAMAGE_YES;
			m_live_ragdoll_handle->SetHealth(1000);
			//
			m_OnFallInRagdoll.Set(m_live_ragdoll_handle, this, this);
			//
			m_flStartRagdollTime = CURTIME;
	...
	case TASK_DI_WAIT_UNTIL_RAGDOLL_ENDS_FALL: // continuously check ragdoll position, follow it (teleport), measure speed and ground entity
	{
		if (!m_live_ragdoll_handle)
		{
			DevWarning(2, "Lost our ragdoll entity; emergency restoring NPC to non-ragdoll state (%s, %s)!!\n", GetClassname(), GetDebugName());
			RestoreNPCAfterRagdoll();
			TaskFail("no ragdoll");
			return;
		}
	...
	case TASK_DI_WAIT_UNTIL_RAGDOLL_ENDS_FALL_END: // once it slowed down and is resting, tell it to unragdoll back into us, and un-hide ourselves
	{
		if (!m_live_ragdoll_handle)
		{
			DevWarning(2, "Lost our ragdoll entity; emergency restoring NPC to non-ragdoll state (%s, %s)!!\n", GetClassname(), GetDebugName());
		...
		{
			trace_t ttele;
			Vector vtele = m_live_ragdoll_handle->WorldSpaceCenter() + Vector(0, 0, 8);
			UTIL_TraceLine(vtele, vtele - Vector(0, 0, 64), MASK_NPCSOLID, m_live_ragdoll_handle, COLLISION_GROUP_NONE, &ttele);
			if (ttele.fraction != 1.0)
			{
				vtele = ttele.endpos;
			}
			QAngle qtele = m_live_ragdoll_handle->GetAbsAngles();
			Vector rCenter = m_live_ragdoll_handle->WorldSpaceCenter();
			Vector rHead = m_live_ragdoll_handle->EyePosition();
			Vector rDir = rCenter - rHead;
			VectorAngles(rDir, qtele);
			qtele.x = 0;
			Teleport(&vtele, &qtele, NULL);

			m_flUnragdollTime = CURTIME + 0.1f;

			m_live_ragdoll_handle->ThinkSet(&CBaseEntity::SUB_Remove);
			m_live_ragdoll_handle->SetNextThink(CURTIME + 0.1f);
		//	Msg("Removed ragdoll at mark %.3f\n", CURTIME);
			
			SetMoveType(MoveType_t(m_rememberMoveType));
			SetSolidFlags(m_rememberSolidFlags);
			SetIdealActivity(ACT_STAND);
		}
	}
	break;
#endif // DARKINTERVAL
#ifdef DARKINTERVAL // this is a fix attempt for NPCs that reset their idle anims before they fully play out
//==================================================
// TASK_PLAY_ACT_IDLE
//==================================================
	case TASK_PLAY_ACT_IDLE:
	{
		if (IsSequenceFinished()) // whatever previous activity (including previous idle) has finished. We're allowed to set ACT_IDLE again
		{ 
		//	Msg("TASK_PLAY_ACT_IDLE: previous activity is done playing. Resetting to ACT_IDLE\n"); 
			ResetIdealActivity(ACT_IDLE);
		}
		else
		{
			if (GetCurrentActivity() == (Activity)(TranslateActivity(ACT_IDLE))) // we're not done playing previous anim. Don't interrupt it.
			{
			//	Msg("TASK_PLAY_ACT_IDLE: not done playing previous anim. Cancel interruption.\n");
				return;
			}
			else
			{
			//	Msg("TASK_PLAY_ACT_IDLE: interrupting previous anim, resetting to ACT_IDLE\n");
				ResetIdealActivity(ACT_IDLE); // if it were some other anim like cowering or angry idle, we're allowed to reset it.
			}
		}
	}
	break;
...
//==================================================
	...
	case TASK_SMALL_FLINCH:
		{
			...
			m_flNextFlinchTime = CURTIME + random->RandomFloat( 3, 5 );
			break;
		}
	...
	case TASK_IGNORE_OLD_ENEMIES:
		m_flAcceptableTimeSeenEnemy = CURTIME;
...
void CAI_BaseNPC::RunTask( const Task_t *pTask )
{
	...
	case TASK_TURN_LEFT:
		...
		//	GetMotor()->SetIdealYawAndUpdate( UTIL_AngleMod(flCurrentYaw + pTask->flTaskData), AI_CALC_YAW_SPEED);
		//	SetTurnActivity();
	...
	case TASK_FACE_ENEMY:
		{
			...
			{
				GetMotor()->SetIdealYawToTarget( vecEnemyLKP );
				GetMotor()->SetIdealYawAndUpdate( CalcReasonableFacing( true ) ); // CalcReasonableFacing() is based on previously set ideal yaw
			}
			else
			{
				float flReasonableFacing = CalcReasonableFacing( true );
	...
	case TASK_WAIT_PVS:
		{
			if ( ShouldAlwaysThink() || 
				 ...
				 ( GetState() == NPC_STATE_COMBAT && GetEnemy() && CURTIME - GetEnemies()->LastTimeSeen( GetEnemy() ) < 15 ) )
			{
				TaskComplete();
			}
			break;
			...
			case 2:
				{
					...
					if ( GetTacticalServices()->FindCoverPos( GetLocalOrigin(), EyePosition(), 0, CoverRadius(), &coverPos ) && IsValidMoveAwayDest( GetNavigator()->GetGoalPos() ) ) 
					{
						...
						m_flMoveWaitFinished = CURTIME + 2;
					}
					else
					{
						// no coverwhatsoever.
	...
	case TASK_DI_WAIT_UNTIL_RAGDOLL_ENDS_FALL: // continuously check ragdoll position, follow it (teleport), measure speed and ground entity
	{
		if (!m_live_ragdoll_handle)
		{
			DevWarning(2, "TASK_DI_WAIT_UNTIL_RAGDOLL_ENDS_FALL :: Lost our ragdoll entity; emergency restoring NPC to non-ragdoll state (%s, %s)!!\n", GetClassname(), GetDebugName());
			RestoreNPCAfterRagdoll();
			TaskFail("no ragdoll");
			return;
		}
		...
		{
			if (CURTIME > m_flStartRagdollTime + 0.5) // HACKHACK: needed for some reason, otherwise NPC pops up first time
			{
				trace_t ttele;
				Vector vtele = m_live_ragdoll_handle->WorldSpaceCenter() + Vector(0, 0, 8);
				UTIL_TraceLine(vtele, vtele - Vector(0, 0, 64), MASK_NPCSOLID, m_live_ragdoll_handle, COLLISION_GROUP_NONE, &ttele);
			...
			float MIN_PUSH_RAGDOLL_TIME = m_flExtraRagdollTime;
			if (MIN_PUSH_RAGDOLL_TIME == -1) // caller specified in the input that this ragdoll state should be indefinite
			{
				return; // just chill in there.
			}
			else
			{
				if (CURTIME > m_flStartRagdollTime + 30) // extreme case! force to leave ragdoll if been in it for this long.
				{										// (but not if it's intentional, via input)
					TaskComplete();
					return;
				}

				trace_t tr;
				AI_TraceHull(m_live_ragdoll_handle->GetAbsOrigin(), m_live_ragdoll_handle->GetAbsOrigin() - Vector(0, 0, 16), GetHullMins(), GetHullMaxs(), MASK_NPCSOLID, m_live_ragdoll_handle, COLLISION_GROUP_NONE, &tr);
				if (tr.fraction == 1.0)
					return; // this means we're still falling or being carried or some such scenario.

				if ( CURTIME > m_flStartRagdollTime + MIN_PUSH_RAGDOLL_TIME
					|| MIN_PUSH_RAGDOLL_TIME == 0)
				{
					TaskComplete();
					return;
				}
	...
	case TASK_DI_WAIT_UNTIL_RAGDOLL_ENDS_FALL_END: // once it slowed down and is resting, tell it to unragdoll back into us, and un-hide ourselves
	{
		if (!m_live_ragdoll_handle)
		{
			DevWarning(2, "TASK_DI_WAIT_UNTIL_RAGDOLL_ENDS_FALL_END :: Lost our ragdoll entity; emergency restoring NPC to non-ragdoll state (%s, %s)!!\n", GetClassname(), GetDebugName());
			RestoreNPCAfterRagdoll();
			TaskFail("no ragdoll");
			return;
		}
		...
		{
			if (CURTIME >= m_flUnragdollTime )
			{
			//	m_live_ragdoll_handle->ThinkSet(&CRagdollProp::SUB_Remove);
			//	m_live_ragdoll_handle->SetNextThink(CURTIME + 0.1f);
			//	SetNextThink(CURTIME + 0.11f);
				RemoveSpawnFlags(SF_NPC_GAG);
			//	SetRenderMode(kRenderNormal);
				RemoveEffects(EF_NOSHADOW);
				ClearCondition(COND_DI_IS_IN_LIVE_RAGDOLL);
				if( m_live_ragdoll_ent)	m_live_ragdoll_ent = NULL; // clear bone "glueing" ragdoll handle
			//	Msg("Restored NPC at mark %.3f\n", CURTIME);
				m_OnExitRagdoll.FireOutput(this, this);
				TaskComplete();
				return;
			}
		}
	...
	case TASK_PLAY_ACT_IDLE:
	{
		if (IsSequenceFinished()) { /*Msg("TASK_PLAY_ACT_IDLE: started playing ACT_IDLE, task complete.\n");*/ TaskComplete(); return; }
	}
	break;
#endif
	default:
...
//=========================================================
#ifdef DARKINTERVAL
ConVar ai_report_turn_activities("ai_report_turn_activities", "0");
#endif
void CAI_BaseNPC::SetTurnActivity ( void )
{
...
#ifdef DARKINTERVAL
	if (ai_report_turn_activities.GetBool()) ("Selecting turn activity (yaw %.0f):", flYD);
	if (fabs(flYD) >= 160 && SelectWeightedSequence(ACT_180_RIGHT) != ACTIVITY_NOT_AVAILABLE)
	{
		Remember(bits_MEMORY_TURNING);
		SetActivity(ACT_180_RIGHT);
		if (ai_report_turn_activities.GetBool()) Msg("180 right\n");
		return;
	}

	// for some models that may have left but not right
	if (fabs(flYD) >= 160 && SelectWeightedSequence(ACT_180_LEFT) != ACTIVITY_NOT_AVAILABLE)
	{
		Remember(bits_MEMORY_TURNING);
		SetActivity(ACT_180_LEFT);
		if (ai_report_turn_activities.GetBool()) Msg("180 left\n");
		return;
	}
#endif
	if( flYD <= -80 && flYD >= -100 && SelectWeightedSequence( ACT_90_RIGHT ) != ACTIVITY_NOT_AVAILABLE )
	{
		...
		Remember( bits_MEMORY_TURNING );
#ifdef DARKINTERVAL
		SetActivity( ACT_90_RIGHT );
		if ( ai_report_turn_activities.GetBool() ) Msg("90 right\n");
#else
	...
	if( flYD >= 80 && flYD <= 100 && SelectWeightedSequence( ACT_90_LEFT ) != ACTIVITY_NOT_AVAILABLE )
	{
		...
		Remember( bits_MEMORY_TURNING ); 
#ifdef DARKINTERVAL
		SetActivity( ACT_90_LEFT );
		if (ai_report_turn_activities.GetBool()) Msg("90 left\n");
#else
	...
	{// big right turn
		...
		if (ai_report_turn_activities.GetBool()) Msg("Turn right\n");
#else
	...
	{// big left turn
		...
		if (ai_report_turn_activities.GetBool()) Msg("Turn left\n");
#else
	...
	SetIdealActivity( ACT_IDLE ); // failure case
#else
	if (ai_report_turn_activities.GetBool()) Msg("Failure case\n");
#endif
}
...
int CAI_BaseNPC::SelectAlertSchedule()
{
	...
	if ( CURTIME - GetEnemies()->LastTimeSeen( AI_UNKNOWN_ENEMY ) < TIME_CARE_ABOUT_DAMAGE )
...
int CAI_BaseNPC::SelectCombatSchedule()
{
	...
	if ( HasCondition(COND_NEW_ENEMY) && CURTIME - GetEnemies()->FirstTimeSeen(GetEnemy()) < 2.0 )
	{
...
void CAI_BaseNPC::PlayFlinchGesture()
{
	...
	if ( iFlinchActivity != ACT_INVALID )
	{
		...
		m_flNextFlinchTime = CURTIME + flNextFlinch;
	}
}
...
#ifdef DARKINTERVAL // DI NEW: Live Ragdolls Experiment (inspired by Dark Messiah)
	if (HasCondition(COND_DI_FALL_IN_RAGDOLL))
	{
