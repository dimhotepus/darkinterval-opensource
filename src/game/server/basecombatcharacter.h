...
enum Capability_t 
{
	...
	bits_CAP_MOVE_SWIM				= 0x00000010, // navigate in water			// DI NEW - water nodes
	...
	bits_CAP_LIVE_RAGDOLL			= 0x00000200, // can perform the schedule SCHED_DI_FALL_IN_RAGDOLL
#endif
};
...
public:
	...
	virtual void		Weapon_Equip(CBaseCombatWeapon *pWeapon, bool switchToNewWeapon = true);			// Adds weapon to player
...
#ifdef DARKINTERVAL
	virtual bool			CanGibRagdoll(void) { return false; } // off by default, leaf classes can override
#endif
...
#ifdef DARKINTERVAL	// DI NEW - combat confidence/morale system
	int			CombatConfidence(void);
	void		InputSetCombatConfidence(inputdata_t &inputdata);

	// DI NEW  - more flex functionality
	void		InputSetFlexWeight(inputdata_t &inputdata);
#endif
	// Weapons..
...
#ifndef DARKINTERVAL // moved to CBaseAnimating
#ifdef GLOWS_ENABLE
	// Glows
...
#ifdef DARKINTERVAL
	float		m_iCombatConfidence;
	float		m_iMaxCombatConfidence;
#endif
public:
