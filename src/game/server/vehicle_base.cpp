...
#ifdef DARKINTERVAL
	DEFINE_INPUTFUNC( FIELD_STRING, "ChangeVehicleScript", InputChangeVehicleScript),
#endif
END_DATADESC()
...
#ifdef DARKINTERVAL
void CPropVehicle::InputChangeVehicleScript(inputdata_t &inputdata)
{
	return; // it's not functional at the moment, sadly
	/*
	AssertMsg(inputdata.value.String(), "Improper ChangeVehicleScript input called on %s!\nMake sure you're passing a string value which is the name of your script with path and file extension!\nE. g. ChangeVehicleScript scripts/vehicles/myvehicle.txt!", GetEntityName());
	m_vehicleScript = MAKE_STRING(inputdata.value.String());
	
	IServerVehicle *pServerVehicle = GetServerVehicle();
	AssertMsg(pServerVehicle, "Couldn't GetServerVehicle() for %s during ChangeVehicleScript input call!", GetEntityName());
	if (pServerVehicle)
	{
		const char* oldscript = m_vehicleScript.ToCStr();
		pServerVehicle->ReloadScript();
		ConColorMsg(COLOR_YELLOW, "Reloaded vehicle script (%s) for entity (%s), previous script was (%s)\n", inputdata.value.String(), GetEntityName(), oldscript);
	}
	*/
}
#endif
...
void CPropVehicleDriveable::OnRestore( void )
{
	...
	m_flNoImpactDamageTime = CURTIME + 5.0f;
...
void CPropVehicleDriveable::Think()
{
	...
	if ( m_flTurnOffKeepUpright )
	{
		...
		// Time up?
		if ( m_hKeepUpright != NULL && m_flTurnOffKeepUpright < CURTIME )
		{
...
#ifdef DARKINTERVAL
//-----------------------------------------------------------------------------
// Purpose: DI NEW. Like TurnOff, but without suppressing guns and engine sound.
//-----------------------------------------------------------------------------
void CPropVehicleDriveable::InputDisableMovement(inputdata_t &inputdata)
{
	m_bEngineLocked = true;
}

//-----------------------------------------------------------------------------
// Purpose: DI NEW. For using with conjunction with DisableMovement.
//-----------------------------------------------------------------------------
void CPropVehicleDriveable::InputEnableMovement(inputdata_t &inputdata)
{
	m_bEngineLocked = false;
}
#endif
...
//=============================================================================
...
#ifdef DARKINTERVAL
	// Ignore breakable prop_physics
	CBreakableProp *pBreakableProp = dynamic_cast<CBreakableProp *>(pHitEntity);
	if (pBreakableProp)
	{
		//if ( pBreakableProp->HasSpawnFlags( SF_PHYSICS_BREAK_IMMEDIATELY ) )
		return;
	}
#endif
	...
	if ( flDamage > 0 && m_flNoImpactDamageTime < CURTIME )
	{
		Vector damagePos;
...
void CPropVehicleDriveable::TraceAttack( const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr )
{
	...
	if ( info.GetDamageType() & DMG_PHYSGUN )
	{
		...
		if ( flUprightStrength )
		{
			...
			m_flTurnOffKeepUpright = CURTIME + GetUprightTime();
			SetNextThink( CURTIME );
		}
...
void CFourWheelServerVehicle::NPC_SetDriver( CNPC_VehicleDriver *pDriver )
{
	if ( pDriver )
	{
		...
		// Start Thinking
		GetFourWheelVehicle()->SetNextThink( CURTIME );
	}
	else
	{
