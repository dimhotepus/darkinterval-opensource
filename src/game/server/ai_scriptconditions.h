...
private:
	...
	void SetThinkTime()			{ SetNextThink( CURTIME + 0.250 ); }
...
#ifdef DARKINTERVAL
	bool			EvalPlayerFlashlightActive(const EvalArgs_t &args);
	ThreeState_t	m_check_player_flashlight_state;
	// todo: can add a lot more inputs to change other checks on the fly
	void			InputSetFlashlightCheckState(inputdata_t &inputdata);
#endif
	//---------------------------------
