...
#ifdef DARKINTERVAL // it's not guaranteed that pCurrentSound exists
			if (pCurrentSound)
...
{
	if ( m_TimeLastLook != CURTIME || m_LastLookDist != iDistance )
	{
		//-----------------------------
...
{
	int nSeen = 0;
	if ( CURTIME - m_TimeLastLookHighPriority > AI_HIGH_PRIORITY_SEARCH_TIME )
	{
		AI_PROFILE_SENSES(CAI_Senses_LookForHighPriorityEntities);
		m_TimeLastLookHighPriority = CURTIME;
...
int CAI_Senses::LookForNPCs( int iDistance )
{
	...
	if ( CURTIME - m_TimeLastLookNPCs > timeNPCs )
	{
		AI_PROFILE_SENSES(CAI_Senses_LookForNPCs);

		m_TimeLastLookNPCs = CURTIME;
...
{	
	const int BOX_QUERY_MASK = FL_VISIBLE_TO_NPCS;
	int	nSeen = 0;

	if ( CURTIME - m_TimeLastLookMisc > AI_MISC_SEARCH_TIME )
	{
		AI_PROFILE_SENSES(CAI_Senses_LookForObjects);
		m_TimeLastLookMisc = CURTIME;
...
{
	if ( ( pEntity->GetFlags() & FL_VISIBLE_TO_NPCS) && !pEntity->IsPlayer() && !pEntity->IsNPC() )
	{
...
void CAI_SensedObjectsManager::AddEntity( CBaseEntity *pEntity )
{
	...
	pEntity->AddFlag(FL_VISIBLE_TO_NPCS);
	m_SensedObjects.AddToTail( pEntity );
}
