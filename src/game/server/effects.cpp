...
void CBubbling::InputSetFrequency( inputdata_t &inputdata )
{
	...
	if ( m_state )
	{
		...
		{
			SetNextThink( CURTIME + 2.5 - (0.1 * m_frequency) );
		}
	}
}
...
void CEnvTracer::Activate( void )
{
	...
	if (pEnd != NULL)
	{
		...
		SetNextThink( CURTIME + m_flDelay );
	}
	else
	{
...
{
#ifndef DARKINTERVAL // DI doesn't care about this setting
...
void CGibShooter::InitPointGib( CGib *pGib, const Vector &vecShootDir, float flSpeed )
{
	if ( pGib )
	{
		...
		float thinkTime = ( pGib->GetNextThink() - CURTIME );
...
#endif
		{
			pGib->SetNextThink( CURTIME + pGib->m_lifeTime );
...
CGib *CEnvShooter::CreateGib ( void )
{
	...
	pGib->m_lifeTime = CURTIME + m_flGibLife;
...
CBaseEntity *CRotorWashShooter::DoWashPush( float flWashStartTime, const Vector &vecForce )
{
	...
	{
		if ( m_flNextGibTime > CURTIME )
			return NULL;
	}

	m_flLastWashStartTime = flWashStartTime;
	m_flNextGibTime	= CURTIME + m_flTimeUnderRotor + random->RandomFloat( -1, 1) * m_flTimeUnderRotorVariance;
	if ( m_flNextGibTime <= CURTIME )
	{
		m_flNextGibTime = CURTIME + 0.01f;
	}
...
void CTestEffect::Think( void )
{
	...
	if (t < 3.0)
	{
		for (i = 0; i < m_iBeam; i++)
		{
			t = (CURTIME - m_flBeamTime[i]) / ( 3 + m_flStartTime - m_flBeamTime[i]);
...
public:
	...
	void	OnRestore();
	void	InputSetEmissionRadius(inputdata_t &inputdata);
	void	InputSetPrecipitationType(inputdata_t &inputdata);
#endif
	CNetworkVar( PrecipitationType_t, m_nPrecipType );
#ifdef DARKINTERVAL
	CNetworkVar(int, m_WindDir);
	CNetworkVar(int, m_WindSpeed);
	CNetworkVar(int, m_emissionSize);
// rain sounds overhaul
private:
	CSoundPatch *m_pOutdoorRainSound;
	CSoundPatch *m_pIndoorRainSound;
	string_t m_OutdoorRainSoundName;
	string_t m_IndoorRainSoundName;
	float m_outdoorrainsound_volume;
	float m_indoorrainsound_volume;
	float m_outdoorrainsound_transitionlength;
	float m_indoorrainsound_transitionlength;
	bool m_bPlayerIsIndoors;
	bool m_bPlayRainSound;
public:
	void InputSetOutdoorRainSoundVolume(inputdata_t &inputdata);
	void InputSetIndoorRainSoundVolume(inputdata_t &inputdata);
	void InputPlayerIsOutdoors(inputdata_t &inputdata);
	void InputPlayerIsIndoors(inputdata_t &inputdata);
	void InputPlayRainSound(inputdata_t &inputdata);
	void RainThink(void); 
	void UpdateOnRemove(void);
#endif // DARKINTERVAL
};
#ifdef DARKINTERVAL
#define SF_PRECIPITATION_PLAY_RAIN_SOUNDS 1<<16
#endif
...
DEFINE_KEYFIELD(m_WindDir, FIELD_INTEGER, "winddir"),
DEFINE_KEYFIELD(m_emissionSize, FIELD_INTEGER, "emissionsize"),
DEFINE_INPUTFUNC(FIELD_INTEGER, "SetRainRadius", InputSetEmissionRadius),
DEFINE_INPUTFUNC(FIELD_STRING, "SetPrecipitationType", InputSetPrecipitationType),
// rain sounds overhaul
DEFINE_FIELD(m_bPlayerIsIndoors, FIELD_BOOLEAN), // if player is indoors, fade out outdoor loop sound and fade in indoor loop sound
DEFINE_KEYFIELD(m_bPlayRainSound, FIELD_BOOLEAN, "playrainsound"), // if player is completely out of rain hearing range, stop playing sounds
DEFINE_SOUNDPATCH(m_pOutdoorRainSound),
DEFINE_SOUNDPATCH(m_pIndoorRainSound),
DEFINE_KEYFIELD(m_OutdoorRainSoundName, FIELD_SOUNDNAME, "OutdoorSound"),
DEFINE_KEYFIELD(m_IndoorRainSoundName, FIELD_SOUNDNAME, "IndoorSound"),
DEFINE_KEYFIELD(m_outdoorrainsound_volume, FIELD_FLOAT, "outdoorvolume"), // max volume when looping outdoor sound
DEFINE_KEYFIELD(m_indoorrainsound_volume, FIELD_FLOAT, "indoorvolume"), // max volum when looping indoor sound
DEFINE_KEYFIELD(m_outdoorrainsound_transitionlength, FIELD_FLOAT, "outdoortransition"), // length of transition from indoor to outdoor
DEFINE_KEYFIELD(m_indoorrainsound_transitionlength, FIELD_FLOAT, "indoortransition"), // length of transition from outdoor to indoor
DEFINE_INPUTFUNC(FIELD_INTEGER, "SetRainRadius", InputSetEmissionRadius),
DEFINE_INPUTFUNC(FIELD_FLOAT, "SetOutdoorRainSoundVolume", InputSetOutdoorRainSoundVolume),
DEFINE_INPUTFUNC(FIELD_FLOAT, "SetIndoorRainSoundVolume", InputSetIndoorRainSoundVolume),
DEFINE_INPUTFUNC(FIELD_VOID, "PlayerIsOutdoors", InputPlayerIsOutdoors),
DEFINE_INPUTFUNC(FIELD_VOID, "PlayerIsIndoors", InputPlayerIsIndoors),
DEFINE_INPUTFUNC(FIELD_BOOLEAN, "PlayRainSound", InputPlayRainSound),
//
// Function Pointers
DEFINE_FUNCTION(RainThink),
#endif // DARKINTERVAL
END_DATADESC()
...
#ifdef DARKINTERVAL
	SendPropInt(SENDINFO(m_WindSpeed), -1, SPROP_NOSCALE),
	SendPropInt(SENDINFO(m_WindDir), -1, SPROP_NOSCALE),
	SendPropInt(SENDINFO(m_emissionSize), -1, SPROP_NOSCALE),
#endif
END_SEND_TABLE()
...
CPrecipitation::CPrecipitation()
{
	...
	m_emissionSize = 1000;
	// rain sounds overhaul
	m_OutdoorRainSoundName = MAKE_STRING("Rain.LoopOutdoors");
	m_IndoorRainSoundName = MAKE_STRING("Rain.LoopIndoors");
	m_bPlayRainSound = false;
	m_bPlayerIsIndoors = false;
	m_outdoorrainsound_transitionlength = 0.5;
	m_indoorrainsound_transitionlength = 0.5;
	m_outdoorrainsound_volume = 1;
	m_indoorrainsound_volume = 1;
	//
#endif
}
#ifdef DARKINTERVAL
...
void CPrecipitation::OnSave(IEntitySaveUtils *pUtils)
{
	BaseClass::OnSave(pUtils);
	DispatchParticleEffect("null", PATTACH_WORLDORIGIN, this, -1, true);
}

void CPrecipitation::OnRestore()
{
	BaseClass::OnRestore();
	DispatchParticleEffect("null", PATTACH_WORLDORIGIN, this, -1, true);
}

void CPrecipitation::Precache(void)
{
	...
	PrecacheParticleSystem("rain_impact_single");
	PrecacheParticleSystem("rain_impact_single_water");
//	PrecacheParticleSystem("slime_splash_01");
	PrecacheParticleSystem("null");

	PrecacheScriptSound(STRING(m_OutdoorRainSoundName));
	PrecacheScriptSound(STRING(m_IndoorRainSoundName));
...
#ifdef DARKINTERVAL
	SetThink(&CPrecipitation::RainThink);
	SetNextThink(CURTIME);
#endif
}
#ifdef DARKINTERVAL
void CPrecipitation::InputSetEmissionRadius(inputdata_t &inputdata)
{
	m_emissionSize = inputdata.value.Int();
}

void CPrecipitation::InputSetPrecipitationType(inputdata_t &inputdata)
{
	if (MAKE_STRING(inputdata.value.String()) == NULL_STRING)
	{
		Warning("When calling SetPrecipitationType on entity %s, need to specify the type!!!\nPermitted options are 'rain', 'snow', 'ash', 'snowfall'!\n", GetDebugName());
		return;
	}
	if (!Q_strcmp(inputdata.value.String(), "rain"))
		m_nPrecipType = PRECIPITATION_TYPE_RAIN;
	else if (!Q_strcmp(inputdata.value.String(), "snow"))
		m_nPrecipType = PRECIPITATION_TYPE_SNOW;
	else if (!Q_strcmp(inputdata.value.String(), "ash"))
		m_nPrecipType = PRECIPITATION_TYPE_ASH;
	else if (!Q_strcmp(inputdata.value.String(), "snowfall"))
		m_nPrecipType = PRECIPITATION_TYPE_SNOWFALL;
	else
	{
		Warning("Unrecognised precipitation type %s called on entity %s!!!\nPermitted options are 'rain', 'snow', 'ash', 'snowfall'!\n", inputdata.value.String(), GetDebugName());
		return;
	}
}

// Rain sounds overhaul
void CPrecipitation::UpdateOnRemove(void)
{
	if (m_pOutdoorRainSound)
	{
		CSoundEnvelopeController::GetController().SoundDestroy(m_pOutdoorRainSound);
		m_pOutdoorRainSound = NULL;
	}

	if (m_pIndoorRainSound)
	{
		CSoundEnvelopeController::GetController().SoundDestroy(m_pIndoorRainSound);
		m_pIndoorRainSound = NULL;
	}
...
void CPrecipitation::InputSetOutdoorRainSoundVolume(inputdata_t &inputdata)
{
	m_outdoorrainsound_volume = inputdata.value.Float();
}
void CPrecipitation::InputSetIndoorRainSoundVolume(inputdata_t &inputdata)
{
	m_indoorrainsound_volume = inputdata.value.Float();
}
void CPrecipitation::InputPlayerIsOutdoors(inputdata_t &inputdata)
{
	if (m_bPlayerIsIndoors) m_bPlayerIsIndoors = false;
}
void CPrecipitation::InputPlayerIsIndoors(inputdata_t &inputdata)
{
	if (!m_bPlayerIsIndoors) m_bPlayerIsIndoors = true;
}
void CPrecipitation::InputPlayRainSound(inputdata_t &inputdata)
{
	m_bPlayRainSound = inputdata.value.Bool();
}

void CPrecipitation::RainThink(void)
{
	if (HasSpawnFlags(SF_PRECIPITATION_PLAY_RAIN_SOUNDS) && m_bPlayRainSound)
	{
		if (!m_pOutdoorRainSound)
		{
			...
			m_pOutdoorRainSound = controller.SoundCreate(filter, player->entindex(), STRING(m_OutdoorRainSoundName));
			controller.Play(m_pOutdoorRainSound, 0.001f, 100);
		}
		if (!m_pIndoorRainSound)
		{
			CBasePlayer *player = UTIL_GetLocalPlayer();
			...
			CSoundEnvelopeController &controller = CSoundEnvelopeController::GetController();
			CPASAttenuationFilter filter(this);
			m_pIndoorRainSound = controller.SoundCreate(filter, player->entindex(), STRING(m_IndoorRainSoundName));
			controller.Play(m_pIndoorRainSound, 0.001f, 100);
		}
	}
	if (m_pOutdoorRainSound)
	{
		...
		if (!m_bPlayRainSound)
		{
			controller.SoundChangeVolume(m_pOutdoorRainSound, 0.001f, m_outdoorrainsound_transitionlength);
		}
		else
		{
			if (!m_bPlayerIsIndoors)
				controller.SoundChangeVolume(m_pOutdoorRainSound, m_outdoorrainsound_volume / 10, m_outdoorrainsound_transitionlength);
			else
				controller.SoundChangeVolume(m_pOutdoorRainSound, 0.001f, m_outdoorrainsound_transitionlength);
		}
	}
	...
	if (m_pIndoorRainSound)
	{
		...
		if (!m_bPlayRainSound)
		{
			controller.SoundChangeVolume(m_pIndoorRainSound, 0.001f, m_indoorrainsound_transitionlength);
		}
		else
		{
			if (m_bPlayerIsIndoors)
				controller.SoundChangeVolume(m_pIndoorRainSound, m_indoorrainsound_volume / 10, m_indoorrainsound_transitionlength);
			else
				controller.SoundChangeVolume(m_pIndoorRainSound, 0.001f, m_indoorrainsound_transitionlength);
		}
	}
...
{
	SetNextThink( m_EnvWindShared.WindThink( CURTIME ) );
}
...
void CEnvGunfire::ShootThink()
{
	...
	SetNextThink( CURTIME + m_flRateOfFire );
	...
	{
		StartShooting();
		SetNextThink( CURTIME + random->RandomFloat( m_flMinBurstDelay, m_flMaxBurstDelay ) );
	}
}
