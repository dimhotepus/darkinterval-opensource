...
#ifdef DARKINTERVAL // the driver model entity that sits on the vehicle attachemnt and plays an idle animation
LINK_ENTITY_TO_CLASS(vehicledriver_dummy, CVehicleDriverDummy);

void CVehicleDriverDummy::Spawn()
{
	...
	{
		Warning("vehicle driver model was created without owner!\n");
	}
}
#endif
//------------------------------------
...
#ifdef DARKINTERVAL
	DEFINE_INPUTFUNC(FIELD_STRING, "GotoPathTrackViaNearestPath", InputGotoPathTrackViaNearestPath),
	DEFINE_FIELD(m_driverDummyHandle, FIELD_CLASSPTR),

	DEFINE_KEYFIELD(m_placementAttachment, FIELD_STRING, "placementAttachment"),
	DEFINE_KEYFIELD(m_placementSequence, FIELD_STRING, "placementSequence"),
	DEFINE_FIELD(m_customModel_bool, FIELD_BOOLEAN),
#endif
END_DATADESC()
...
{
#ifdef DARKINTERVAL
	if (m_driverDummyHandle != NULL) UTIL_Remove(m_driverDummyHandle);
#endif
	ClearWaypoints();
}
...
void CNPC_VehicleDriver::Precache(void)
{
	...
	if (!GetModelName())
	{
		SetModelName(MAKE_STRING("models/roller_vehicledriver.mdl"));
	}

	m_customModel_bool = ((!Q_stricmp(pModelName, "models/roller_vehicledriver.mdl") == false));
...
void CNPC_VehicleDriver::Activate( void )
{
...
#ifdef DARKINTERVAL
	bool success = false;
	if( m_customModel_bool && m_driverDummyHandle == NULL)
	{
		if (m_placementAttachment != NULL_STRING && m_hVehicleEntity->GetBaseAnimating())
		{ // todo: consider default hardcoded offset based on apc model? 

			CBaseEntity *pMark = CreateEntityByName("vehicledriver_dummy");
			pMark->SetOwnerEntity(this);
			pMark->SetModelName(GetModelName());
			pMark->Spawn();
			m_driverDummyHandle = assert_cast<CVehicleDriverDummy*>(pMark);

			m_driverDummyHandle->SetParent(m_hVehicleEntity);
			m_driverDummyHandle->SetParentAttachment("SetParentAttachment", m_placementAttachment.ToCStr(), false);

			if (m_placementSequence != NULL_STRING)
			{
				int seq = LookupSequence(m_placementSequence.ToCStr());
				if (seq != ACT_INVALID) // looking up sequence in our own model, because dummy copies it
				{
					m_driverDummyHandle->SetSequence(seq); //ResetIdealActivity((Activity)LookupActivity(GetSequenceActivityName(LookupSequence(m_placementSequence.ToCStr()))));
					success = true;
				}
			}
		}
	...
	if (!success)
	{
		Warning("vehicle driver %s failed to correctly set its dummy driver model\n", GetDebugName());
	}
#endif // DARKINTERVAL
}
...
#ifdef DARKINTERVAL
	if (m_customModel_bool && m_placementAttachment != NULL_STRING && m_hVehicleEntity->GetBaseAnimating())
	{ // todo: consider default hardcoded offset based on apc model? 
		Vector placementOrigin;
		QAngle placementAng;
		int placementAttach = m_hVehicleEntity->GetServerVehicle()->GetVehicleEnt()->GetBaseAnimating()->LookupAttachment("damage0");
		GetAttachment(LookupAttachment(m_placementAttachment.ToCStr()), placementOrigin, placementAng);
		SetAbsOrigin(placementOrigin);
		SetAbsAngles(placementAng);
	}
	else
#endif
	{
...
int	CNPC_VehicleDriver::RangeAttack1Conditions( float flDot, float flDist )
{
	...
	if ( m_pVehicleInterface->Weapon_PrimaryCanFireAt() > CURTIME )
		return 0;
...
{
	// Vehicle not ready to fire again yet?
	if ( m_pVehicleInterface->Weapon_SecondaryCanFireAt() > CURTIME )
		return 0;
...
bool CNPC_VehicleDriver::OverridePathMove( float flInterval )
{
	...
	float flIncTime = CURTIME - GetLastThink();
...
#ifndef DARKINTERVAL // todo - why was this disabled?
...
#ifdef DARKINTERVAL
//-----------------------------------------------------------------------------
// Purpose: DI NEW - lets vehicledrivers calculate the route to the end
// path corner beginning with a path that is closest to their current
// position. 
//-----------------------------------------------------------------------------
void CNPC_VehicleDriver::InputGotoPathTrackViaNearestPath(inputdata_t &inputdata)
{
	if (!m_hVehicleEntity)
	{
		Warning("npc_vehicledriver %s doesn't have a vehicle to rule!\n", STRING(GetEntityName()));
		return; // no point!
	}
	string_t iszPathName = inputdata.value.StringID();
	if (iszPathName == NULL_STRING)
	{
		Warning("The input GotoPathTrackViaNearestPath for %s must specify goal path name!\n", STRING(GetEntityName()));
		return;
	}
	CBaseEntity *pEntity = gEntList.FindEntityByName(NULL, iszPathName);
	if (!pEntity)
	{
		Warning("npc_vehicledriver %s couldn't find goal path_track named %s\n", STRING(GetEntityName()), STRING(iszPathName));
		return;
	}

	Vector origin = m_hVehicleEntity->GetAbsOrigin();

	float searchDist = 4096.0;

	CBaseEntity *nearbyEnts[2048];
	CPathTrack *nearestTracks[64];
	float distToTrack = 0;
	int nearbyCount = UTIL_EntitiesInSphere(nearbyEnts, ARRAYSIZE(nearbyEnts), origin, searchDist, 0);
	int tracksCount = 0;
	int i;

	for (i = 0; i < nearbyCount; i++)
	{
		CBaseEntity *ent = nearbyEnts[i];

		CPathTrack *track = dynamic_cast<CPathTrack *>(ent);
		if (track)
		{
			if (track->GetLastPath() && track->GetLastPath()->NameMatches(STRING(iszPathName)))
			{
				if (tracksCount < ARRAYSIZE(nearestTracks))
				{
					nearestTracks[tracksCount] = track;
					tracksCount++;
					continue;
				}
			}
...
*/                                                                                        
		else // if it's not a path_track, ignore.
		{
		//	NDebugOverlay::Sphere(ent->GetAbsOrigin(), 4, 255, 150, 50, true, 5);
			nearbyEnts[i] = NULL;
		}
	}
	...
	if (tracksCount > 0)
	{
		...
		{
			distToTrack = nearestTracks[i]->GetAbsOrigin().DistTo(origin);

			float distToTrack = (nearestTracks[i]->GetAbsOrigin() - origin).Length();

			if (searchDist > distToTrack)
			{
				pEntity = nearestTracks[i];
				searchDist = distToTrack;
			}
		}
	}
	else
	{
		Warning("npc_vehicledriver %s couldn't find any goal path_tracks leading to %s\n", STRING(GetEntityName()), STRING(iszPathName));
		return;
	}

	if (developer.GetInt() > 0)
	{
		CPathTrack *entTrack = dynamic_cast<CPathTrack *>(pEntity);
		if (entTrack && entTrack->GetLastPath())
		{
			NDebugOverlay::Sphere(entTrack->GetAbsOrigin(), 32, 50, 255, 50, true, 10);
			NDebugOverlay::Line(entTrack->GetAbsOrigin(), entTrack->GetLastPath()->GetAbsOrigin(), 50, 255, 50, true, 10);
			NDebugOverlay::Sphere(entTrack->GetLastPath()->GetAbsOrigin(), 32, 50, 255, 50, true, 10);
		}
	}
	ClearWaypoints();
