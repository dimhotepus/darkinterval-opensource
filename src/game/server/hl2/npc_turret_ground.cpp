...
void CNPC_GroundTurret::Spawn( void )
{
	...
	m_flTimeNextShoot = CURTIME;
	m_flTimeNextPing = CURTIME;
...
void CNPC_GroundTurret::DeathEffects()
{
	...
	else
	{
		...
		SetNextThink( CURTIME + random->RandomFloat( 0.5, 2.5 ) );
	}
}
...
void CNPC_GroundTurret::GatherConditions()
{
	...
	{
		if( pEMemory->timeLastSeen < CURTIME - GROUNDTURRET_RETIRE_TIME )
		{
	...
	{
		m_flTimeLastSawEnemy = CURTIME;
	}
	else
	{
		if( CURTIME - m_flTimeLastSawEnemy >= GROUNDTURRET_RETIRE_TIME )
		{
...
void CNPC_GroundTurret::Shoot()
{
	...
	m_flTimeNextShoot = CURTIME + 0.09;
}
...
void CNPC_GroundTurret::Scan()
{
	...
	scanAngle = GetAbsAngles();
	scanAngle.y += (GROUNDTURRET_VIEWCONE / 2.0f) * sin( CURTIME * 3.0f );
