...
void CNPC_Strider::Spawn()
{
	...
	m_flTimeNextHuntSound = CURTIME - 1.0f;
}
...
void CNPC_Strider::GatherConditions()
{
	...
	if( IsUsingAggressiveBehavior() )
	{
		if( GetEnemy() )
		{
			...
			{
				if( CURTIME >= m_flTimeNextHuntSound && !m_pMinigun->IsShooting() )
				{
					HuntSound();
				}
			}
		...
		if( m_hPlayersMissile )
		{
			if( !m_pMinigun->IsShooting() && GetEnemy() && GetEnemy()->IsPlayer() )
			{
				...
				float flTimeSuppressed = CURTIME - m_flTimePlayerMissileDetected;
		...
		else if( GetEnemy() )
		{
			if ( strider_distributed_fire.GetBool() && !IsUsingAggressiveBehavior() )
			{
				...
				for( AI_EnemyInfo_t *pEMemory = GetEnemies()->GetFirst(&iter); pEMemory != NULL; pEMemory = GetEnemies()->GetNext(&iter) )
				{
					...
					{
						if( pEMemory->timeLastSeen == CURTIME )
						{
							m_iVisibleEnemies++;
						}
					}
				...
				if( GetEnemy() && m_pMinigun->IsShooting() && GetTimeEnemyAcquired() != CURTIME )
				{
					if( m_pMinigun->IsOnTarget( 3 ) && !FClassnameIs( GetEnemy(), "npc_bullseye" ) )
					{
						if( m_iVisibleEnemies > 1 )
						{
							...
							GetEnemies()->SetTimeValidEnemy( GetEnemy(), CURTIME + ( STRIDER_IGNORE_TARGET_DURATION * m_iVisibleEnemies ) );
							SetEnemy( NULL, false );
							ChooseEnemy();
						}
...
*/
	...
	if( m_NPCState == NPC_STATE_COMBAT )
	{
		...
		{
			if ( m_hRagdoll.Get() && (CURTIME > m_ragdollTime  || HasCondition( COND_STRIDER_DO_FLICK ) ) )
			{
...
int CNPC_Strider::TranslateSchedule( int scheduleType )
{
	...
	case SCHED_ESTABLISH_LINE_OF_FIRE_FALLBACK:
		...
		{
			if ( CURTIME - GetEnemyLastTimeSeen() < TIME_CARE_ENEMY )
				return SCHED_STRIDER_COMBAT_FACE;
			else if ( GetGoalEnt() )
				return SCHED_STRIDER_HUNT;
			else
...
bool CNPC_Strider::HandleInteraction( int interactionType, void *data, CBaseCombatCharacter* sourceEnt )
{
	...
	{
		m_flTimePlayerMissileDetected = CURTIME;
...
{
	if( ( CURTIME - m_flTimeLastAlertSound ) > 2.0f )
	{
		EmitSound( "NPC_Strider.Alert" );
		m_flTimeLastAlertSound = CURTIME;
	}
}
...
#ifdef DARKINTERVAL // DI striders can be killed with bullets (fast-firing weapons), don't make them whimper too often
	m_flNextPainSoundTime = CURTIME + RandomFloat(2, 4);
#endif
}
...
void CNPC_Strider::HuntSound()
{
	if( m_PlayerFreePass.HasPass() )
	{
		...
		m_flTimeNextHuntSound = CURTIME + random->RandomFloat( 8.0f, 12.0f );
	}
}
...
int CNPC_Strider::OnTakeDamage_Alive( const CTakeDamageInfo &info )
{
	...
	if ( info.GetInflictor() && info.GetInflictor()->GetOwnerEntity() == this )
		return 0;
#ifdef DARKINTERVAL	// no friendly damage from fellow striders
...
#ifdef DARKINTERVAL
	if ((info.GetDamageType() & DMG_SHOCK)) // DI: vorts vs striders
	{
	...
	if ((info.GetDamageType() & DMG_BULLET)) // DI: handle attacking striders with bullets.
	{
		int damage = info.GetDamage();
		...
		if (info.GetAmmoType() == GetAmmoDef()->Index("Buckshot"))
		{
			damage *= 0.2;
		}

		if (info.GetAmmoType() == GetAmmoDef()->Index("APCTurret"))
		{
			if (m_flNextPainSoundTime < CURTIME)
				PainSound(info);
			damage *= 2;
...
#ifdef DARKINTERVAL
			int maxRagdolls = ( dxlevel >= 90 ) ? 32 : ( dxlevel >= 80 ) ? 4 : 0; //2 to 1. Pathetic
#else	
...
bool CNPC_Strider::OverrideMove( float flInterval )
{
	...
	else if ( GetEnemy() )
	{
		...
		float timeSinceSeenEnemy = CURTIME - GetEnemyLastTimeSeen();
...
void CNPC_Strider::MaintainTurnActivity( void )
{
	...
	if (!IsMoving() && m_doTurn > 180.0f && m_flNextTurnAct < CURTIME )
	{
		...
		if (iSeq != ACT_INVALID)
		{
			...
			if (iLayer != -1)
			{
				...
				m_flNextTurnAct = CURTIME + GetLayerDuration( iLayer );
			}
			else
			{
				// too busy, try again in half a second
				m_flNextTurnAct = CURTIME + 0.5;
			}
		}
...
void CNPC_Strider::FireCannon() 
{
	...
	if ( GetNextThink( "CANNON_HIT" ) > CURTIME )
	{
		DevMsg( "Strider refiring cannon?\n" );
		return;
	}

	m_nextShootTime = CURTIME + 5;
	trace_t tr;
	Vector vecShootPos;
	GetAttachment( gm_CannonAttachment, vecShootPos );
	...
	SetContextThink( &CNPC_Strider::CannonHitThink, CURTIME + 0.2f, "CANNON_HIT" );
}
...
#ifdef DARKINTERVAL // congealed three foot hit functions into one
//---------------------------------------------------------
//---------------------------------------------------------
Vector CNPC_Strider::FootHit( float eventtime, int leg )
{
	Vector footPosition;
	QAngle angles;
...
void CStriderMinigun::StartShooting( IStriderMinigunHost *pHost, CBaseEntity *pTarget, float duration )
{
	...
	m_nextBulletTime = CURTIME;
	m_burstTime = CURTIME + duration;
...
{
	m_burstTime = CURTIME + timeExtend;
...
void CStriderMinigun::StopShootingForSeconds( IStriderMinigunHost *pHost, CBaseEntity *pTarget, float duration )
{
	...
	ClearOnTarget();
	m_nextTwitchTime = CURTIME + random->RandomFloat( 2.0, 4.0 );
		...
		if( !pHost->GetEntity()->HasCondition( COND_SEE_ENEMY )  )
		{
			...
			if( !pStrider->WeaponLOSCondition( pStrider->GetAdjustedOrigin(), pTargetEnt->WorldSpaceCenter(), false ) )
			{
				if( pStrider->IsUsingAggressiveBehavior() && pTargetEnt->IsPlayer() && !pStrider->HasPass() )
				{
					...
					float flTimeSinceLastSeen = CURTIME - pStrider->GetEnemies()->LastTimeSeen(pTargetEnt);
...
void CStriderMinigun::Think( IStriderMinigunHost *pHost, float dt )
{
	...
	if ( !GetTarget() && m_nextTwitchTime <= CURTIME )
	{
		...
		m_nextTwitchTime = CURTIME + random->RandomFloat( 0.3, 2 );
	}
	...
	if( IsShooting() )
	{
		if( CURTIME > m_burstTime || !pTargetEnt )
		{
			// Time to stop firing.
			if( m_bOverrideEnemy )
			{
	...
	if ( m_nextBulletTime <= CURTIME && !IsPegged() )
	{
		...
		m_nextBulletTime = CURTIME + (1.0f / pHost->GetMinigunRateOfFire() );
	}
}
...
// Concussive blast previously shared from weapon_cguard
#ifdef DARKINTERVAL 
