...
#ifdef DARKINTERVAL
LINK_ENTITY_TO_CLASS(npc_icky, CNPC_Ichthyosaur);
#endif
...
void CNPC_Ichthyosaur::Spawn( void )
{
	...
	m_flNextBiteTime = CURTIME;
	m_flHoldTime = CURTIME;
	m_flNextPingTime = CURTIME;
	m_flNextGrowlTime = CURTIME;
...
int CNPC_Ichthyosaur::SelectSchedule( void )
{
	...
	{
		if ( m_flHoldTime > CURTIME )
			return SCHED_ICH_DROWN_VICTIM;

		if ( m_flNextBiteTime > CURTIME )
...
void CNPC_Ichthyosaur::AddSwimNoise( Vector *velocity )
{
	...
	lNoise = LATERAL_NOISE_MAX * sin( CURTIME * LATERAL_NOISE_FREQ );
	vNoise = VERTICAL_NOISE_MAX * sin( CURTIME * VERTICAL_NOISE_FREQ );
...
void CNPC_Ichthyosaur::Bite( void )
{
	...
	if ( pHurt != NULL )
	{
		...
		if ( pHurt->IsPlayer() )
		{
			...
			if ( pPlayer )
			{
				if ( ( ( m_flHoldTime < CURTIME ) && ( pPlayer->m_iHealth < (pPlayer->m_iMaxHealth*0.5f)) ) || ( pPlayer->GetWaterLevel() < 1 ) )
				{
					//EnsnareVictim( pHurt );
				}
				else
		...
		m_flNextBiteTime = CURTIME + random->RandomFloat( 2.0f, 4.0f );
...
void CNPC_Ichthyosaur::PrescheduleThink( void )
{
	...
	*/
	...
	//Pings
	if ( m_flNextPingTime < CURTIME )
	{
		m_flNextPingTime = CURTIME + random->RandomFloat( 3.0f, 8.0f );
	}

	//Growls
	if ( ( m_NPCState == NPC_STATE_COMBAT || m_NPCState == NPC_STATE_ALERT ) && ( m_flNextGrowlTime < CURTIME ) )
	{
		m_flNextGrowlTime = CURTIME + random->RandomFloat( 2.0f, 6.0f );
	}
...
void CNPC_Ichthyosaur::EnsnareVictim( CBaseEntity *pVictim )
{
	...
	if ( pBCC && pBCC->DispatchInteraction( g_interactionBarnacleVictimGrab, NULL, this ) )
	{
		if ( pVictim->IsPlayer() )
		{
			...
			if ( pPlayer )
			{
				m_flHoldTime = MAX( CURTIME+3.0f, pPlayer->PlayerDrownTime() - 2.0f );
			}
		}
		else
		{
			m_flHoldTime = CURTIME + 4.0f;
		}
...
void CNPC_Ichthyosaur::ReleaseVictim( void )
{
	...
	m_pVictim = NULL;
	m_flNextBiteTime = CURTIME + 8.0f;
	m_flHoldTime = CURTIME - 0.1f;
}
