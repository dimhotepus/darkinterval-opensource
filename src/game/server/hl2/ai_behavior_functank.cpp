...
{
	SetBusy( CURTIME + AI_FUNCTANK_BEHAVIOR_BUSYTIME );
	...
	case TASK_FIRE_FUNCTANK:
		{
			...
			GetOuter()->m_flWaitFinished = CURTIME + FUNCTANK_FIRE_TIME;
			break;
		}
	...
	case TASK_FIRE_FUNCTANK:
		{
			...
			if( GetOuter()->m_flWaitFinished < CURTIME )
			{
				TaskComplete();
			}
			...
			{
				GetOuter()->SetLastAttackTime( CURTIME );
...
void CAI_FuncTankBehavior::GatherConditions()
{
	...
	{
		if ( CURTIME - GetOuter()->GetEnemyLastTimeSeen() >= 3.0f )
		{
