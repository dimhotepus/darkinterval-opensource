...
#ifdef DARKINTERVAL
#define PROP_COMBINE_BALL_MODEL_SMALL	"models/_Weapons/pr_fireball.mdl"
#endif
...
#ifdef DARKINTERVAL
ConVar  sk_combineball_small_bounce("sk_combineball_small_bounce", "1", FCVAR_REPLICATED); // if false, blow up on impact
ConVar  sk_combineball_attractor_force("sk_combineball_attractor_force", "900", FCVAR_REPLICATED);
#endif
...
#ifdef DARKINTERVAL // get pulled toward an attractor, if present
static const char *s_pGravitateContext = "GravitateContext";
#endif
//-----------------------------------------------------------------------------
// Purpose: 
// Input  : radius - 
// Output : CBaseEntity
//-----------------------------------------------------------------------------
#ifdef DARKINTERVAL // now with the option to make smaller balls
CBaseEntity *CreateCombineBall( const Vector &origin, const Vector &velocity, float radius, float mass, float lifetime, CBaseEntity *pOwner, bool isSmall )
#else
...
#endif
{
	...
	{
		pBall->m_is_small_bool = isSmall;
	//	pBall->SetModel(PROP_COMBINE_BALL_MODEL_SMALL);
		pBall->SetMaxBounces(sk_combineball_small_bounce.GetBool() ? 1 : -1);
		pBall->StartGravitateThink();
	}
#endif
	return pBall;
}
...
#ifdef DARKINTERVAL
	DEFINE_FIELD( m_is_small_bool, FIELD_BOOLEAN),
#endif
...
#ifdef DARKINTERVAL
	DEFINE_THINKFUNC( GravitateThink ), // get pulled toward an attractor, if present
#endif
...
#ifdef DARKINTERVAL
	PrecacheModel( PROP_COMBINE_BALL_MODEL_SMALL );
#endif
...
#ifndef DARKINTERVAL // want old sounds
...
#ifdef DARKINTERVAL
		PrecacheScriptSound("NPC_CombineBall.ExplosionSmall");
#endif
...
bool CPropCombineBall::CreateVPhysics()
{
	...
	pPhysicsObject->Wake();
#ifdef DARKINTERVAL
	pPhysicsObject->SetMass( m_is_small_bool ? 50 : 750.0f );
#else
	...
	{
#ifdef DARKINTERVAL
		if( !m_is_small_bool ) // small balls act as traditional projectiles, don't vaporise targets immediately
#endif
...
void CPropCombineBall::Spawn( void )
{
	...
	m_flNextDamageTime = CURTIME; 
#ifdef DARKINTERVAL // default to large balls
	m_is_small_bool = false;
#endif
}
...
void CPropCombineBall::StartAnimating( void )
{
	...
	SetContextThink( &CPropCombineBall::AnimThink, CURTIME + random->RandomFloat( 0.0f, 0.1f), s_pAnimThinkContext );
...
{
	SetContextThink( NULL, CURTIME, s_pAnimThinkContext );
}
...
void CPropCombineBall::CaptureBySpawner( )
{
	...
	if ( flSpeed > 25.0f )
	{
		...
		SetContextThink( &CPropCombineBall::CaptureBySpawner, CURTIME + 0.01f,	s_pCaptureContext );
		return;
	}

	// Ok, we're captured
	SetContextThink( NULL, CURTIME,	s_pCaptureContext );
...
void CPropCombineBall::ReplaceInSpawner( float flSpeed )
{
	...
	// Stop whiz noises
	SetContextThink( NULL, CURTIME, s_pWhizThinkContext );
...
{
	SetContextThink( &CPropCombineBall::ExplodeThink, CURTIME + flDuration, s_pExplodeTimerContext );
}
...
{
	// Prevent it from exploding
	SetContextThink( NULL, CURTIME, s_pExplodeTimerContext );
}
...
void CPropCombineBall::FadeOut( float flDuration )
{
	...
	SetNextThink( CURTIME + flDuration );
}
...
{
	SetContextThink( &CPropCombineBall::WhizSoundThink, CURTIME + 2.0f * TICK_INTERVAL, s_pWhizThinkContext );
}
		...
		if ( pPlayer )
		{
			...
			if ( DotProduct( vecDelta, vecVelocity ) > 0.5f )
			{
				...
				if ( flDist < 200.0f )
				{
					...
					SetContextThink( &CPropCombineBall::WhizSoundThink, CURTIME + 0.5f, s_pWhizThinkContext );
					return;
				}
			}
		}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CPropCombineBall::StartGravitateThink(void)
{
	SetContextThink(&CPropCombineBall::GravitateThink, CURTIME + 0.2f, s_pGravitateContext);
}

//-----------------------------------------------------------------------------
// Gravitate the ball towards the attractor blob
//-----------------------------------------------------------------------------
void CPropCombineBall::GravitateThink()
{
	Vector vecPosition, vecVelocity;
	IPhysicsObject *pPhysicsObject = VPhysicsGetObject();
	...
	if (pPhysicsObject == NULL)
	{
		//NOTENOTE: We should always have been created at this point
		Assert(0);
		SetContextThink(&CPropCombineBall::GravitateThink, CURTIME + 0.1f, s_pGravitateContext);
		return;
	}

	CBaseEntity *pAttractor = NULL;

	while ((pAttractor = gEntList.FindEntityByClassnameWithin(pAttractor, "prop_combine_ball_attractor", GetAbsOrigin(), 1000)) != NULL)
	{
		// Pull it towards the magnet
		Vector attractorOrigin = pAttractor->GetAbsOrigin();
		if (pAttractor->IsFollowingEntity() && pAttractor->GetOwnerEntity()) attractorOrigin = pAttractor->GetOwnerEntity()->WorldSpaceCenter();
		Vector vecNewVelocity = (attractorOrigin - GetAbsOrigin());
		VectorNormalize(vecNewVelocity);
		vecNewVelocity *= sk_combineball_attractor_force.GetFloat() * pPhysicsObject->GetMass();
		pPhysicsObject->AddVelocity(&vecNewVelocity, NULL);
	}
...
void CPropCombineBall::OnPhysGunPickup( CBasePlayer *pPhysGunUser, PhysGunPickup_t reason )
{
	...
	if ( reason != PUNTED_BY_CANNON )
	{
		...
		SetContextThink( &CPropCombineBall::DissolveRampSoundThink, CURTIME + GetBallHoldSoundRampTime(), s_pHoldDissolveContext );
...
#ifdef DARKINTERVAL // only large balls are deadly
	if (!m_is_small_bool)
#endif
	{
...
void CPropCombineBall::OnPhysGunDrop( CBasePlayer *pPhysGunUser, PhysGunDrop_t Reason )
{
	...
	SetContextThink( NULL, CURTIME, s_pHoldDissolveContext );
...
void CPropCombineBall::DissolveRampSoundThink( )
{
	...
	SetContextThink( &CPropCombineBall::DissolveThink, CURTIME + dt, s_pHoldDissolveContext );
}
...
void CPropCombineBall::DoExplosion()
{
	...
	if (OutOfBounces() == false)
	{
...
#ifdef DARKINTERVAL
			if( m_is_small_bool )
				EmitSound("NPC_CombineBall.ExplosionSmall");
			else
#endif
		...
		te->BeamRingPoint(filter2, 0, GetAbsOrigin(),	//origin
			m_is_small_bool ? m_flRadius / 8 : m_flRadius,	//start radius
			m_is_small_bool ? 64 : 1024,		//end radius
			s_nExplosionTexture, //texture
			0,			//halo index
			0,			//start frame
			2,			//framerate
			0.2f,		//life
			m_is_small_bool ? 16 : 64,			//width
			0,			//spread
			0,			//amplitude
			255,	//r
			255,	//g
...
#ifdef DARKINTERVAL
		te->BeamRingPoint(filter2, 0, GetAbsOrigin(),	//origin
			m_is_small_bool ? 8 : 128,	//start radius
			m_is_small_bool ? 32 : 384,		//end radius
			s_nExplosionTexture, //texture
			0,			//halo index
			0,			//start frame
			2,			//framerate
			0.25f,		//life
			m_is_small_bool ? 12 : 48,			//width
			0,			//spread
			0,			//amplitude
			255,	//r
			255,	//g
...
#ifdef DARKINTERVAL // this is primarily for small balls and used with attractors, but let's have it for all
	RadiusDamage(CTakeDamageInfo(this, GetOwnerEntity(), 10, DMG_SHOCK), GetAbsOrigin(), 32, CLASS_NONE, this);
#endif
	...
	SetContextThink(&CPropCombineBall::SUB_Remove, CURTIME + 0.5f, s_pRemoveContext);
	StopLoopingSounds();
}
...
bool CPropCombineBall::DissolveEntity( CBaseEntity *pEntity )
{
	...
	pEntity->GetBaseAnimating()->Dissolve( "", CURTIME, false, ENTITY_DISSOLVE_NORMAL );
...
#ifdef DARKINTERVAL
	// Just detonate if it's the small version
	if (m_is_small_bool)
	{
		DoExplosion();
		return;
			...
			if( WasFiredByNPC() || m_nMaxBounces == -1 )
			{
				...
				if( CURTIME >= m_flNextDamageTime )
				{
					...
					else
					{
						...
						m_flNextDamageTime = CURTIME + 0.1f;
					}
...
void CPropCombineBall::VPhysicsCollision( int index, gamevcollisionevent_t *pEvent )
{
...
#ifdef DARKINTERVAL // detonate immediately on impact
			if( m_is_small_bool )	DoExplosion();
#endif
	...
	if ( CURTIME - m_flLastBounceTime < 0.25f )
		return;

	// Save off our last bounce time
	m_flLastBounceTime = CURTIME;

	// Reset the sound timer
	SetContextThink( &CPropCombineBall::WhizSoundThink, CURTIME + 0.01, s_pWhizThinkContext );
...
{
	StudioFrameAdvance();
	SetContextThink( &CPropCombineBall::AnimThink, CURTIME + 0.1f, s_pAnimThinkContext );
}
#ifdef DARKINTERVAL
// Combine ball attractor fired from AR2 alt fire. Sticks to targets/surfaces and attracts small combine balls.

#define STICKY_BOMB_MODEL	"models/_Weapons/pr_fireball.mdl"

// global list for fast searching
CUtlVector<CCombineBallAttractor *> CCombineBallAttractor::m_stickyList;

CCombineBallAttractor::CCombineBallAttractor()
{
...
void CCombineBallAttractor::Precache()
{
...
void CCombineBallAttractor::Spawn()
{
	...
	SetRenderColor(100, 90, 110);

	// destroy any already existing stickies
	for (int i = m_stickyList.Count() - 1; i >= 0; --i)
	{
		if (m_stickyList[i] != this )
		{
			m_stickyList[i]->ThinkSet(&CBaseEntity::SUB_Remove);
			m_stickyList[i]->SetNextThink(CURTIME);
		}
	}
}
void CCombineBallAttractor::SetVelocity(const Vector &velocity, const AngularImpulse &angVelocity)
{
	SetAbsVelocity(velocity);
}

void CCombineBallAttractor::SetBombOrigin()
{
...
void CCombineBallAttractor::NotifySystemEvent(CBaseEntity *pNotify, notify_system_event_t eventType, const notify_system_event_params_t &params)
{
...
void CCombineBallAttractor::Touch(CBaseEntity *pOther)
{
	...
	if (GetMoveType() == MOVETYPE_FLYGRAVITY)
	{
		...
		if (FClassnameIs(pOther, "prop_combine_ball_attractor") || (tr.surface.flags & SURF_SKY))
		{
			// bounce
...
void LaunchCombineBallAttractor(CBaseCombatCharacter *pOwner, const Vector &origin, const Vector &direction)
{
	CCombineBallAttractor *pGrenade = (CCombineBallAttractor *)CBaseEntity::Create("prop_combine_ball_attractor", origin, vec3_angle, pOwner);
...
// Save/Restore
//---------------------------------------------------------
BEGIN_DATADESC(CCombineBallAttractor)
...
LINK_ENTITY_TO_CLASS(prop_combine_ball_attractor, CCombineBallAttractor);

IMPLEMENT_SERVERCLASS_ST(CCombineBallAttractor, DT_CombineBallAttractor)
...
void CFuncCombineBallSpawner::BallGrabbed( CBaseEntity *pCombineBall )
{
	...
	{
		pCombineBall->SetContextThink( NULL, CURTIME, s_pCaptureContext );
	}
}
...
void CFuncCombineBallSpawner::GrabBallTouch( CBaseEntity *pOther )
{
	...
	if ( CURTIME - pBall->LastCaptureTime() < 0.5f )
		return;
...
void CFuncCombineBallSpawner::RespawnBall( float flRespawnTime )
{
	...
	m_BallRespawnTime.AddToTail( CURTIME + flRespawnTime - m_flDisableTime );
}
...
void CFuncCombineBallSpawner::RespawnBallPostExplosion( void )
{
	...
	{
		m_BallRespawnTime.AddToTail( CURTIME + 4.0f - m_flDisableTime );
	}
	else
	{
		m_BallRespawnTime.AddToTail( CURTIME + m_flBallRespawnTime - m_flDisableTime );
	}
}
...
void CFuncCombineBallSpawner::BallThink()
{
	for ( int i = m_BallRespawnTime.Count(); --i >= 0; )
	{
		if ( m_BallRespawnTime[i] < CURTIME )
		{
			SpawnBall();
