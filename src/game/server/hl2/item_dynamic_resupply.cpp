...
{ 
	...
	if ( !HasSpawnFlags( SF_DYNAMICRESUPPLY_IS_MASTER | SF_DYNAMICRESUPPLY_ALTERNATE_MASTER ) )
	{
		...
		SetNextThink( CURTIME + RandomFloat(0.2f, 0.4f) );
...
{ 
	...
	if ( !HasSpawnFlags( SF_DYNAMICRESUPPLY_ALTERNATE_MASTER ) && HasSpawnFlags( SF_DYNAMICRESUPPLY_USE_MASTER ) && CURTIME < 1.0 )
	{
		if ( !g_MasterResupply )
		{
...
void CItem_DynamicResupply::CheckPVSThink( void )
{
	...
	SetNextThink( CURTIME + DYNAMIC_ITEM_THINK );
}
