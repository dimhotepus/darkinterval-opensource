...
//	DEFINE_FIELD( m_iSoundState,		FIELD_INTEGER ),		// Don't save, precached
	...
	DEFINE_KEYFIELD( m_flRopeShakeForce, FIELD_FLOAT, "RopeShakeForce"),
	DEFINE_INPUTFUNC(FIELD_FLOAT, "SetRopeShakeForce", InputSetRopeShakeForce),
#endif
...
void CBaseHelicopter::HelicopterThink( void )
{
	...
	SetNextThink( CURTIME + HELICOPTER_THINK_INTERVAL );
...
void CBaseHelicopter::RotorWashThink( void )
{
	if ( m_lifeState == LIFE_ALIVE || m_lifeState == LIFE_DYING )
	{
		...
		SetContextThink( &CBaseHelicopter::RotorWashThink, CURTIME + HELICOPTER_ROTORWASH_THINK_INTERVAL, s_pRotorWashThinkContext );
	}
	else
	{
		SetContextThink( NULL, CURTIME, s_pRotorWashThinkContext );
	}
}
...
void CBaseHelicopter::DrawRotorWash( float flAltitude, const Vector &vecRotorOrigin )
{
	...
	{
#ifdef DARKINTERVAL
		CRopeKeyframe::ShakeRopes( GetAbsOrigin(), flAltitude, force);
#else
	...
	if ( m_flRotorWashEntitySearchTime > CURTIME )
		return;

	// Only push every half second
	m_flRotorWashEntitySearchTime = CURTIME + 0.5f;
}
...
bool CBaseHelicopter::DoWashPush( washentity_t *pWash, const Vector &vecWashOrigin )
{
	...
	float flPushTime = (CURTIME - pWash->flWashStartTime);
...
void CBaseHelicopter::UpdateEnemy()
{
	...
	if( m_lifeState == LIFE_ALIVE )
	{
		...
		if( HasEnemy() )
		{
			...
			if ( FVisible( pEnemy ) )
			{
				if (m_flLastSeen < CURTIME - 2)
				{
					m_flPrevSeen = CURTIME;
				}

				m_flLastSeen = CURTIME;
...
void CBaseHelicopter::UpdateFacingDirection()
{
	if ( 1 )
	{
		...
		if ( !IsCrashing() && m_flLastSeen + 5 > CURTIME ) //&& DotProduct( targetDir, desiredDir) > 0.25)
		{
...
{
	// ALERT( at_console, "%.0f %.0f %.0f\n", CURTIME, m_flLastSeen, m_flPrevSeen );
	if (m_fHelicopterFlags & BITS_HELICOPTER_GUN_ON)
	{
		//if ( (m_flLastSeen + 1 > CURTIME) && (m_flPrevSeen + 2 < CURTIME) )
		{
			if (FireGun( ))
			{
...
void CBaseHelicopter::ComputeActualTargetPosition( float flSpeed, float flTime, float flPerpDist, Vector *pDest, bool bApplyNoise )
{
	...
	if ( bApplyNoise && m_flRandomOffsetTime <= CURTIME )
	{
		m_vecRandomOffset.Random( -25.0f, 25.0f );
		m_flRandomOffsetTime = CURTIME + 1.0f;
	}
...
void CBaseHelicopter::InputKill( inputdata_t &inputdata )
{
	...
	m_bSuppressSound = true;
	SetContextThink( &CBaseHelicopter::DelayedKillThink, CURTIME + 3.0f, s_pDelayedKillThinkContext );
}
...
void CBaseHelicopter::Startup( void )
{
	...
	m_flRotorWashEntitySearchTime = CURTIME;
	SetContextThink( &CBaseHelicopter::RotorWashThink, CURTIME, s_pRotorWashThinkContext );
}
...
/*
	...
	{
		m_flNextRocket = CURTIME + 4.0;
	}
	else
	{
		m_flNextRocket = CURTIME + 15.0;
	}
*/	
	StopRotorWash();
...
#ifdef DARKINTERVAL // make it an option because we don't always want the helicopters making ropes go crazy
//-----------------------------------------------------------------------------
// Override the force with which the ropes must be shaken, if necessary
//-----------------------------------------------------------------------------
void CBaseHelicopter::InputSetRopeShakeForce(inputdata_t &inputdata)
{
	m_flRopeShakeForce = inputdata.value.Float();
}
#endif
//------------------------------------------------------------------------------
// Input values
...
bool CBaseHelicopter::ChooseEnemy( void )
{
	...
	if ( pNewEnemy != GetEnemy() )
	{
		if ( pNewEnemy != NULL )
		{
			...
			SetEnemy( pNewEnemy );
			m_flLastSeen = m_flPrevSeen = CURTIME;
		}
		else
		{
			SetEnemy( NULL );
