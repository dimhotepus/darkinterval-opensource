...
#endif
ConVar sk_turret_floor_health("sk_turret_floor_health", "125");
...
#ifdef DARKINTERVAL
ConVar sk_sentry_refire_delay("sk_sentry_refire_delay", "0.4");
#endif
#define	LASER_BEAM_SPRITE			"effects/laser1.vmt"
#ifdef DARKINTERVAL
#define	FLOOR_TURRET_MODEL			"models/_Tech/Floor_sentry.mdl"
#define	FLOOR_TURRET_MODEL_COMBINE	"models/combine_turrets/floor_turret.mdl"
...
#ifdef DARKINTERVAL // bone followers used to be used for retro tripod turrets, but not used right now
/*
...
#ifdef DARKINTERVAL
//-----------------------------------------------------------------------------
// Purpose: Differentiate between the "earth" brown turret and Combine dark turret
// DI todo - this won't play well with the NPC-script based system if that is
// one day added to these
//-----------------------------------------------------------------------------
bool CNPC_FloorTurret::IsCombineTurret(void)
{
	if (Q_strcmp((GetModelName().ToCStr()), FLOOR_TURRET_MODEL_COMBINE) == 0)
	{
		return true;
	}
	if (Q_strcmp((GetModelName().ToCStr()), FLOOR_TURRET_MODEL_CITIZEN) == 0)
	{
		return true;
	}
	return false;
...
{
#ifdef DARKINTERVAL // just precache all of them
	PrecacheModel(FLOOR_TURRET_MODEL);
	PrecacheModel(FLOOR_TURRET_MODEL_COMBINE);
	PrecacheModel(FLOOR_TURRET_MODEL_CITIZEN);
	PrecacheModel(LASER_BEAM_SPRITE);
	const char *pModelName = STRING(GetModelName());
	pModelName = ( pModelName && pModelName[ 0 ] != '\0' ) ? pModelName : FLOOR_TURRET_MODEL_COMBINE;
#else
	const char *pModelName = STRING(GetModelName());
...
#ifndef DARKINTERVAL // this is precached always
	if ( IsCitizenTurret() )
	{
		PrecacheModel(LASER_BEAM_SPRITE);
...
#endif
	// Activities // DI todo - why is this here instead of regular adding of activities before custom AI?
...
{ 
	...
	const char *pModelName = STRING( GetModelName() );
	SetModel( ( pModelName && pModelName[ 0 ] != '\0' ) ? pModelName : FLOOR_TURRET_MODEL_COMBINE );
#ifndef DARKINTERVAL // weirdness
	...
	{
		m_iMuzzleAttachment = LookupAttachment("Muzzleflash_Fwd");
		m_iEyeAttachment = LookupAttachment("Eyes");
		GetAttachment(m_iMuzzleAttachment, m_HackedGunPos);
		m_takedamage = DAMAGE_YES;
	}

	m_flFieldOfView = -0.4f; // 90 degrees

	m_iHealth	= sk_turret_floor_health.GetInt();
	m_iMaxHealth = sk_turret_floor_health.GetInt();
	...
	m_iAmmoType = IsCombineTurret() ? GetAmmoDef()->Index( "Pistol" ) : GetAmmoDef()->Index("357");
#else
	m_HackedGunPos = Vector(0, 0, 12.75);
	SetViewOffset(EyeOffset(ACT_IDLE));
	...
	SetNextThink(CURTIME + random->RandomFloat(0.1f, 0.3f));
...
void CNPC_FloorTurret::Deploy( void )
{
	...
	if ( IsActivityFinished() )
	{
		...
		m_flShotTime = CURTIME; // +0.1f;

		m_flPlaybackRate = 0;
		SetThink( &CNPC_FloorTurret::SearchThink );
#ifndef DARKINTERVAL
		EmitSound( "NPC_FloorTurret.Move" ); // TODO: this is a looping sound that shouldn't be that. Fix it later
#endif
	}

	m_flLastSight = CURTIME + FLOOR_TURRET_MAX_WAIT;	
}
...
void CNPC_FloorTurret::OnPhysGunDrop( CBasePlayer *pPhysGunUser, PhysGunDrop_t Reason )
{
	...
	{
		m_flPlayerDropTime = CURTIME + 2.0;
	}
...
{
#ifndef DARKINTERVAL // DI allows it
...
bool CNPC_FloorTurret::OnAttemptPhysGunPickup( CBasePlayer *pPhysGunUser, PhysGunPickup_t reason )
{
	...
	if (!IsCombineTurret())
	{
		return false; // forbid picking up oldies
	}
#endif
	// Prevent players pulling turrets from afar if they're in front of the turret
	...
	if (reason == PICKED_UP_BY_PLAYER) return false; // cannot move turret by hand
...
#endif
	{
		if ( interactionType == g_interactionCombineBash )
		{
			...
			if ( !m_hLastNPCToKickMe )
			{
				...
				m_flKnockOverFailedTime = CURTIME + 3.0;
			}
...
void CNPC_FloorTurret::DryFire( void )
{
	...
	{
		m_flShotTime = CURTIME + random->RandomFloat( 1, 2.5 );
	}
	else
	{
		m_flShotTime = CURTIME;
	}
}
...
void CNPC_FloorTurret::SuppressThink( void )
{
	...
	if ( CURTIME > m_flLastSight )
	{
		...
		if ( m_spawnflags & SF_FLOOR_TURRET_FASTRETIRE )
		{
			...
			m_flLastSight = CURTIME + FLOOR_TURRET_SHORT_WAIT;
		}
		else
		{
			m_flLastSight = CURTIME + FLOOR_TURRET_MAX_WAIT;
...
#endif
	if ( m_flShotTime < CURTIME && m_vecEnemyLKP != vec3_invalid )
	{
		Vector vecMuzzle, vecMuzzleDir;
		UpdateMuzzleMatrix();
		MatrixGetColumn( m_muzzleToWorld, 0, vecMuzzleDir );
		...
		if ( bCanShoot ) // 10 degree slop XY
		{
			...
			else
			{
				...
				else
				{
					...
					//Fire the weapon
					Shoot(vecMuzzle, vecMuzzleDir, (dot3d < DOT_10DEGREE));
					m_flShotTime = CURTIME + sk_sentry_refire_delay.GetFloat();
				}
#endif
			}
		} 
...
void CNPC_FloorTurret::SearchThink( void )
{
	...
	if ( GetEnemy() != NULL )
	{
		...
		if ( CURTIME > m_flNextActivateSoundTime )
		{
			EmitSound( "NPC_FloorTurret.Activate" );
			m_flNextActivateSoundTime = CURTIME + 3.0;
		}
		return;
	}
	...
	if (IsCombineTurret())
	{
		m_vecGoalAngles.x = GetAbsAngles().x + (sin(CURTIME * 1.0f) * 15.0f);
		m_vecGoalAngles.y = GetAbsAngles().y + (sin(CURTIME * 2.0f) * 60.0f);
	}
	else
#endif
	{
		//	m_vecGoalAngles.x = GetAbsAngles().x + ( sin( CURTIME * 1.0f ) * 10.0f );
		m_vecGoalAngles.y = GetAbsAngles().y + (sin(CURTIME * 2.0f) * 60.0f);
	}
...
void CNPC_FloorTurret::AutoSearchThink( void )
{
	...
	SetNextThink( CURTIME + random->RandomFloat( 0.05f, 0.15f ) );
...
{
#ifdef DARKINTERVAL // for retro turrets
...
#ifdef DARKINTERVAL
				if (!IsCombineTurret()) // oldies self-destruct when killed
#endif
				{
...
bool CNPC_FloorTurret::PreThink( turretState_e state )
{
	...
	if ( !IsBeingCarriedByPlayer() && IsCombineTurret() )
	{
		if ( OnSide() == false )
		{
			// If I still haven't fallen over after an NPC has tried to knock me down, let them know
			if ( m_hLastNPCToKickMe && m_flKnockOverFailedTime < CURTIME )
			{
		...
		else
		{
			if ( HasSpawnFlags( SF_FLOOR_TURRET_OUT_OF_AMMO ) == false )
			{
				...
				m_flThrashTime = CURTIME + random->RandomFloat( 2.0f, 2.5f );
				SetNextThink( CURTIME + 0.05f );
...
#ifdef DARKINTERVAL // DI wants lasers on all turrets
...
void CNPC_FloorTurret::Ping( void )
{
	...
	m_flPingTime = CURTIME + FLOOR_TURRET_PING_TIME;
}
...
#ifdef DARKINTERVAL // retro turrets
	else
	{
		if (!m_bActive)
			newInfo.ScaleDamage(0.1f);

		if (m_bSelfDestructing) return 0;
...
#ifdef DARKINTERVAL // DI's turrets are more accurate, both varieties
	return VECTOR_CONE_5DEGREES * ((CBaseHLCombatWeapon::GetDefaultProficiencyValues())[ weaponProficiency ].spreadscale);
#else	
...
CBasePlayer *CNPC_FloorTurret::HasPhysicsAttacker( float dt )
{
	...
	if ( IsHeldByPhyscannon( ) || (CURTIME - dt <= m_flLastPhysicsInfluenceTime) )
	{
...
//	m_BoneFollowerManager.UpdateBoneFollowers(this);
	...
	for (int i = 0; i < 3; i++)
	{
		vecStart.x = GetAbsOrigin().x + random->RandomFloat(-8.0f, 8.0f);
		vecStart.y = GetAbsOrigin().y + random->RandomFloat(-8.0f, 8.0f);
...
void CNPC_FloorTurret::SelfDestructThink( void )
{
	...
	if ( ( CURTIME - m_flDestructStartTime ) >= SELF_DESTRUCT_DURATION )
	{
	...
	float flDestructPerc = clamp( ( CURTIME - m_flDestructStartTime ) / SELF_DESTRUCT_DURATION, 0.0f, 1.0f );
	...
	if ( CURTIME > ( m_flPingTime + flBeepTime ) )
	{
...
#ifndef DARKINTERVAL // unused in DI
