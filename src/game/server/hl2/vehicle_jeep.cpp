...
int CPropJeep::OnTakeDamage( const CTakeDamageInfo &inputInfo )
{
	...
	if ( info.GetDamageType() == DMG_SHOCK )
	{
		if ( info.GetDamage() <= 10 )
		{
			...
			m_throttleDisableTime = CURTIME + 2;
		}
	}
...
void CPropJeep::HandleWater( void )
{
	...
	if ( CheckWater() )
	{
		for ( int iWheel = 0; iWheel < JEEP_WHEEL_COUNT; ++iWheel )
		{
			...
			if ( m_WaterData.m_bWheelInWater[iWheel] && m_WaterData.m_bWheelWasInWater[iWheel] )
			{
				if ( m_WaterData.m_flNextRippleTime[iWheel] < CURTIME )
				{
					// Stagger ripple times
					m_WaterData.m_flNextRippleTime[iWheel] = CURTIME + RandomFloat( 0.1, 0.3 );
					CreateRipple( m_WaterData.m_vecWheelContactPoints[iWheel] );
				}
			}
		}
...
#ifdef DARKINTERVAL
		vecAttachPoint.z += 10.0f * vecUp.z;
#else
...
void CPropJeep::Think( void )
{
	...
	if ( ( m_flAmmoCrateCloseTime < CURTIME ) && ( GetSequence() == LookupSequence( "ammo_open" ) ) )
	{
		m_flAnimTime = CURTIME;
...
void CPropJeep::FireCannon( void )
{
	...
	if ( m_flCannonTime > CURTIME )
		return;

	if ( m_bUnableToFire )
		return;

	m_flCannonTime = CURTIME + 0.2f;
	m_bCannonCharging = false;
	...
	if ( m_hPlayer )
	{
		m_hPlayer->SetMuzzleFlashTime( CURTIME + 0.5 );
...
void CPropJeep::FireChargedCannon( void )
{
	...
	float flChargeAmount = ( CURTIME - m_flCannonChargeStartTime ) / MAX_GAUSS_CHARGE_TIME;
...
#ifndef DARKINTERVAL // decaltrace is broken in 2013 (never fades), use a temp effect instead
	...
	if ( m_hPlayer )
	{
		m_hPlayer->SetMuzzleFlashTime( CURTIME + 0.5f );
	}
...
void CPropJeep::ChargeCannon( void )
{
	...
	{
		m_flCannonChargeStartTime = CURTIME;
...
void CPropJeep::Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	if ( tr.m_pEnt == this && tr.hitgroup == JEEP_AMMOCRATE_HITGROUP )
	{
		...
		m_flAmmoCrateCloseTime = CURTIME + JEEP_AMMO_CRATE_CLOSE_DELAY;
		return;
	}
...
#ifdef DARKINTERVAL
	ComputePDControllerCoefficients( flCoefficients, 7.0f, 1.0f, flFrameTime );
#else
...
void CPropJeep::DampenUpMotion( Vector &vecVehicleEyePos, QAngle &vecVehicleEyeAngles, float flFrameTime )
{
	...
	vecUp.z = clamp( vecUp.z, 0.0f, vecUp.z );
#ifdef DARKINTERVAL
	vecVehicleEyePos.z += 10.0f * vecUp.z;
#else
...
void CPropJeep::SetupMove( CBasePlayer *player, CUserCmd *ucmd, IMoveHelper *pHelper, CMoveData *move )
{
	...
	CUserCmd tmp;
	if ( ( m_throttleDisableTime > CURTIME ) || ( IsOverturned() ) )
	{
		m_bUnableToFire = true;
...
{
#ifndef DARKINTERVAL // DI wants headlights to stay on for example when driving in dark locations
	HeadlightTurnOff();
#endif
	...
	m_flPlayerExitedTime = CURTIME;
	m_flLastSawPlayerAt = CURTIME;
	...
	if ( GlobalEntity_GetState( "no_seagulls_on_jeep" ) == GLOBAL_OFF )
	{
		...
		if ( pHint )
		{
			...
			SetContextThink( &CPropJeep::JeepSeagullThink, CURTIME + JEEP_SEAGULL_THINK_INTERVAL, g_pJeepThinkContext );
		}
	}
}
...
void CPropJeep::JeepSeagullThink( void )
{
	...
	if ( m_hSeagull )
	{
		...
		if ( pSeagull )
		{
			...
			if ( pSeagull->m_bOnJeep == true )
			{
				...
				SetContextThink( &CPropJeep::JeepSeagullThink, CURTIME + JEEP_SEAGULL_POOP_INTERVAL, g_pJeepThinkContext );
			}
			else
			{
	...
	SetContextThink( &CPropJeep::JeepSeagullThink, CURTIME + 0.2, g_pJeepThinkContext );

	// Not taken enough time yet?
	float flHiddenTime = (CURTIME - m_flLastSawPlayerAt);
