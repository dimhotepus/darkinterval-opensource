...
int CNPC_PoisonZombie::RangeAttack1Conditions( float flDot, float flDist )
{
	...
	if ( m_flNextCrabThrowTime > CURTIME )
	{
...
void CNPC_PoisonZombie::HandleAnimEvent( animevent_t *pEvent )
{
	...
	if ( pEvent->event == AE_ZOMBIE_POISON_THROW_CRAB )
	{
...
#ifdef DARKINTERVAL
		// If we're set to have server ragdoll, apply it to our children as well
		if (HasSpawnFlags(SF_ALWAYS_SERVER_RAGDOLL))
		{
			pCrab->AddSpawnFlags(SF_ALWAYS_SERVER_RAGDOLL);
		}
#endif
		...
		m_flNextCrabThrowTime = CURTIME + random->RandomInt( ZOMBIE_THROW_MIN_DELAY, ZOMBIE_THROW_MAX_DELAY );
		return;
	}
...
void CNPC_PoisonZombie::PrescheduleThink( void )
{
	...
	{
		m_flNextCrabThrowTime = CURTIME + random->RandomInt( ZOMBIE_THROW_FIRST_MIN_DELAY, ZOMBIE_THROW_FIRST_MAX_DELAY );
	}
...
void CNPC_PoisonZombie::PainSound( const CTakeDamageInfo &info )
{
	...
	{	
		...
		m_flNextPainSoundTime = CURTIME + random->RandomFloat( 4.0, 7.0 );
	}
}
