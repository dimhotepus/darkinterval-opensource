...
#ifdef DARKINTERVAL // for event listening
...
#define SF_CITIZEN_IGNORE_SEMAPHORE ( 1 << 21 ) //2097152
...
#ifdef DARKINTERVAL
//-------------------------------------
class CCitizenOrnament : public CBaseAnimating
{
	DECLARE_CLASS(CCitizenOrnament, CBaseAnimating);
	void Spawn();
};
//-------------------------------------
#endif
...
public:
	CNPC_Citizen()
	 :	m_iHead( -1 )
#ifdef DARKINTERVAL
		, m_ornamentOffset(Vector(0,0,0)),
		m_ornamentHandle(NULL),
		m_ornamentScale(1),
		m_bGrabbedByBarnacle(false),
		m_bReleasedByBarnacle(false),
		m_flReleasedByBarnacleTime(FLT_MAX)
#endif
	{
#ifdef DARKINTERVAL
		ListenForGameEvent("di_event_test");
#endif
	}
#ifdef DARKINTERVAL
	~CNPC_Citizen()
	{
		if (m_ornamentHandle != NULL) UTIL_Remove(m_ornamentHandle);
	}
#endif
	//---------------------------------
...
//	void			TraceAttack(const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr);
	...
	bool			m_bGrabbedByBarnacle; // this is used to scream for help on our way up
	bool			m_bReleasedByBarnacle; // this is used to scream for help on our way up
	float			m_flReleasedByBarnacleTime; // marks when we were released
	CSimpleSimTimer	m_ScreamForHelpTimer;
#endif
	//---------------------------------
	// Hints
	//---------------------------------
	...
	void			InputSpeakIdleResponseQuestion(inputdata_t &inputdata);
	void			InputSpeakIdleResponseAnswer(inputdata_t &inputdata);
#endif
	//---------------------------------
	//	Sounds & speech
	//---------------------------------
...
#ifdef DARKINTERVAL
	string_t		m_ornamentModel;
	Vector			m_ornamentOffset;
	string_t		m_ornamentAttachment;
	float			m_ornamentScale;
	float			m_ornamentSkin;
	CCitizenOrnament*	m_ornamentHandle;
	void			AddOrnament();
	void			RemoveOrnament();
#endif
private:
	...
	int				m_iHead;
#ifdef DARKINTERVAL // can set health in Hammer
	int				m_healthoverride_int;
#endif
