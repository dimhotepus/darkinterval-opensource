...
{ 
	float flTimeDelay = CURTIME - m_lastSightTime;
...
void CFuncTank::Spawn( void )
{
	...
	m_flStartLeadFactorTime = CURTIME;
	m_flNextLeadFactorTime = CURTIME + 1.0f;
	...
	if ( IsActive() || (IsControllable() && !HasController()) )
	{
		...
		m_flNextControllerSearch = CURTIME + 1.0f;
	}
...
void CFuncTank::StopControl()
{
	...
	if ( IsActive() || (IsControllable() && !HasController()) )
	{
		...
		m_flNextControllerSearch = CURTIME + 1.0f;
#else
		m_flNextControllerSearch = CURTIME + 5.0f;
...
void CFuncTank::ControllerPostFrame( void )
{
	...
	m_fireLast = CURTIME - (1/m_fireRate) - 0.01;  // to make sure the gun doesn't fire too many bullets
	
	int bulletCount = (CURTIME - m_fireLast) * m_fireRate;
	...
	SetNextAttack( CURTIME + (1/m_fireRate) );
}
...
{
	SetNextAttack( CURTIME + m_flPlayerLockTimeBeforeFire );
}
...
//		m_fireLast = CURTIME - (1/m_fireRate) - 0.01;  // to make sure the gun doesn't fire too many bullets		
	...
	{	
		...
		SetNextAttack( CURTIME + flFireTime );
	}
	else
	{
		SetNextAttack( CURTIME + m_fireTime );
	}
}
...
void CFuncTank::Think( void )
{
	...
	if ( IsControllable() && !HasController() && (m_flNextControllerSearch <= CURTIME) )
	{
		if ( m_bShouldFindNPCs && CURTIME > 5.0f )
		{
		...
		if( bThinkFast )
		{
			m_flNextControllerSearch = CURTIME + 0.1f;
		}
		else
		{
			m_flNextControllerSearch = CURTIME + 2.0f;
		}
#else
		// Keep thinking, in case they turn NPC finding back on
		if ( !HasController() )
...
void CFuncTank::CalcPlayerCrosshairTarget( Vector *pVecTarget )
{
	...
	if (true) // DI todo - investigate for autoaim
	{
...
void CFuncTank::ComputeLeadingPosition( const Vector &vecShootPosition, CBaseEntity *pTarget, Vector *pLeadPosition )
{
	...
	// Guesstimate...
	if ( m_flNextLeadFactorTime < CURTIME )
	{
		m_flStartLeadFactor = m_flNextLeadFactor;
		m_flStartLeadFactorTime = CURTIME;
		m_flNextLeadFactor = random->RandomFloat( 0.8f, 1.3f );
		m_flNextLeadFactorTime = CURTIME + random->RandomFloat( 2.0f, 4.0f );
	}

	float flFactor = (CURTIME - m_flStartLeadFactorTime) / (m_flNextLeadFactorTime - m_flStartLeadFactorTime);
...
void CFuncTank::AimFuncTankAtTarget( void )
{
	...
	if ( bUpdateTime )
	{
		if( (CURTIME - m_lastSightTime >= 1.0) && (CURTIME > m_flNextAttack) )
		{
			// Enemy was hidden for a while, and I COULD fire right now. Instead, tack a delay on.
			m_flNextAttack = CURTIME + 0.5;
		}

		m_lastSightTime = CURTIME;
		m_persist2burst = 0;
	}
...
void CFuncTank::TrackTarget( void )
{
	...
	if( !m_bReadyToFire && m_flNextAttack <= CURTIME )
	{
		m_OnReadyToFire.FireOutput( this, this );
		m_bReadyToFire = true;
	}
...
#ifdef DARKINTERVAL
		else if (m_iEffectHandling == EH_APC && GetMoveParent())
			{
			//	QAngle qFwd;
			//	Vector vecDir;
			//	pAnim->GetVectors(&vecDir, NULL, NULL);
			//	VectorAngles(vecDir, qFwd);
				DispatchParticleEffect("apc_muzzleflash", PATTACH_POINT_FOLLOW, GetMoveParent(), m_nBarrelAttachment);
			}
#endif
		else
		{
...
#ifdef DARKINTERVAL
	else if (m_iEffectHandling == EH_APC)
	{
		DoMuzzleFlash();
...
bool CFuncTank::IsEntityInViewCone( CBaseEntity *pEntity )
{
	...
	if ( m_flPlayerGracePeriod && pEntity->IsPlayer() && (CURTIME - m_flLastSawNonPlayer) < m_flPlayerGracePeriod )
	{
...
void CFuncTankGun::Fire( int bulletCount, const Vector &barrelEnd, const Vector &forward, CBaseEntity *pAttacker, bool bIgnoreSpread )
{
	...
	else // for older maps (HL2 when played in DI), because those specify "bullet" keyvalue and not do it like Episodes do.
	{
		for (i = 0; i < bulletCount; i++)
		{
			switch (m_bulletType)
...
{
	if ( m_pLaser && (CURTIME > m_laserTime) )
		m_pLaser->TurnOff();
...
void CFuncTankLaser::Fire( int bulletCount, const Vector &barrelEnd, const Vector &forward, CBaseEntity *pAttacker, bool bIgnoreSpread )
{
	...
	if ( GetLaser() )
	{
		for ( i = 0; i < bulletCount; i++ )
		{
			...
			m_laserTime = CURTIME;
			m_pLaser->TurnOn();
			m_pLaser->SetFireTime( CURTIME - 1.0 );
...
void CFuncTankRocket::Fire( int bulletCount, const Vector &barrelEnd, const Vector &forward, CBaseEntity *pAttacker, bool bIgnoreSpread )
{
	...
	pRocket->DumbFire();
	pRocket->SetNextThink( CURTIME + 0.1f );
...
{
	if ( CURTIME >= m_flNextHeavyShotTime )
...
void CFuncTankAirboatGun::DoImpactEffect( trace_t &tr, int nDamageType )
{
	...
	if ( m_flLastImpactEffectTime == CURTIME )
		return;

	m_flLastImpactEffectTime = CURTIME;
...
void CFuncTankAirboatGun::Fire( int bulletCount, const Vector &barrelEnd, const Vector &forward, CBaseEntity *pAttacker, bool bIgnoreSpread )
{
	...
	if ( CURTIME >= m_flNextHeavyShotTime )
	{
		m_flNextHeavyShotTime = CURTIME + AIRBOAT_GUN_HEAVY_SHOT_INTERVAL; 
	}
}
...
void CFuncTankAPCRocket::InputDeathVolley( inputdata_t &inputdata )
{
	...
	{
		m_fireRate = random->RandomFloat( DEATH_VOLLEY_MIN_FIRE_RATE, DEATH_VOLLEY_MAX_FIRE_RATE );
		SetNextAttack( CURTIME + (1.0f / m_fireRate ) );
...
CMortarShell *CMortarShell::Create( const Vector &vecStart, const Vector &vecTarget, const Vector &vecShotDir, float flImpactDelay, float flWarnDelay, string_t warnSound )
{
	...
	pShell->m_flImpactTime = CURTIME + flImpactDelay;
...
//			bias - 
//-----------------------------------------------------------------------------
#ifndef DARKINTERVAL // reducing amount of convars // DI todo - investigate this for previewing grenade throws
...
#ifdef DARKINTERVAL
	//	UTIL_VisualizeCurve( CURVE_GAIN, 64, 0.5f );
#else
//	UTIL_VisualizeCurve( CURVE_GAIN, 64, curve_bias.GetFloat() );
#endif
	float lifePerc = 1.0f - ( ( m_flImpactTime - CURTIME ) / ( m_flImpactTime - m_flSpawnedTime ) );
	...
	if( !m_bHasWarned && CURTIME > m_flWarnTime )
	{
		Warn();
	}

	if( CURTIME > m_flImpactTime )
	{
		Impact();
	}
...
void CMortarShell::FadeThink( void )
{
	...
	float lifePerc = 1.0f - ( ( CURTIME - m_flFadeTime  ) / MORTAR_FADE_LENGTH );
	...
	if ( CURTIME > ( m_flFadeTime + MORTAR_FADE_LENGTH ) )
	{
...
{
	if ( CURTIME > GetNextAttack() )
	{
		ShootGun();
		m_fireLast = CURTIME;
		SetNextAttack( CURTIME + (1.0 / m_fireRate ) );
	}
	else
	{
		m_fireLast = CURTIME;
...
{
	if (!m_hBeam && CURTIME >= m_flTimeBeamOn )
	{
		m_hBeam = CBeam::BeamCreate( COMBINE_CANNON_BEAM, 1.0f );
		m_hBeam->SetColor( 255, 255, 255 );
		SetContextThink( &CFuncTankCombineCannon::UpdateBeamThink, CURTIME, s_pUpdateBeamThinkContext );
	}
	else
	{
...
{
	SetContextThink( &CFuncTankCombineCannon::UpdateBeamThink, CURTIME + 0.025, s_pUpdateBeamThinkContext );
...
void CFuncTankCombineCannon::FuncTankPostThink()
{
	...
	if( m_hTarget.Get() == NULL )
	{
		if( CURTIME > m_flTimeNextSweep )
		{
			...
			if( bHarass )
			{
				m_flTimeNextSweep = CURTIME + random->RandomFloat( 0.25f, 0.75f );
			}
			else
			{
				m_flTimeNextSweep = CURTIME + random->RandomFloat( 1, 3 );
			}
		}
	}
	else
...
void CFuncTankCombineCannon::Fire( int bulletCount, const Vector &barrelEnd, const Vector &forward, CBaseEntity *pAttacker, bool bIgnoreSpread )
{
	...
	DestroyBeam();
	m_flTimeBeamOn = CURTIME + 0.2f;

	m_flTimeNextSweep = CURTIME + random->RandomInt( 1.0f, 2.0f );
}
