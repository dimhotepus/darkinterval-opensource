...
void CNPC_EnemyFinder::PrescheduleThink()
{
	...
	if ( GetEnemies()->NumEnemies() > 0 )
	{
		...
		if ( CURTIME - GetEnemyLastTimeSeen() >= 0.5f )
		{
			bHasEnemies = false;
		}
	}
...
public:
	...
	float		m_flTimeWideFOV; // If this is > CURTIME, we have 180 degree viewcone.
...
{
	if( m_flTimeWideFOV > CURTIME && !pTarget->IsPlayer() )
	{
	...
	if( bResult == true && pTarget->IsNPC() && pPlayer != NULL && FInViewCone( pPlayer ) )
	{
		...
		if( m_flTimeStopHateNPC != EF_COMBINE_CANNON_HATE_TIME_INVALID )
		{
			...
			if( CURTIME > m_flTimeStopHateNPC )
			{
				// Don't interfere with the result
				m_flTimeStopHateNPC = EF_COMBINE_CANNON_HATE_TIME_INVALID;
				m_flTimeNextHateNPC = CURTIME + ai_ef_hate_npc_frequency.GetFloat();
				return bResult;
			}
		}
		else
		{
			...
			if( CURTIME > m_flTimeNextHateNPC )
			{
				m_flTimeStopHateNPC = CURTIME + ai_ef_hate_npc_duration.GetFloat();
			}
			else
			{
...
{
	m_flTimeWideFOV	= CURTIME + inputdata.value.Float();
}
