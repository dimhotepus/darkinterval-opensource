...
#ifdef DARKINTERVAL // for armour mitigation figures
#include "ammodef.h"
#endif
#include "soundent.h"
#ifdef DARKINTERVAL // for ricochet
#include "ieffects.h"
#endif
#ifdef DARKINTERVAL // DI NEW - soldiers picking up medkits
#include "items.h"
#endif
#include "game.h"
#include "npcevent.h"
#include "npc_combine.h"
#ifdef DARKINTERVAL // DI NEW - manhack tossing for soldiers
...
#ifdef DARKINTERVAL // DI NEW - RPG-wielding soldiers
#include "weapon_rpg.h"
#endif
#include "soundemittersystem/isoundemittersystembase.h"
#include "npc_headcrab.h"
#ifdef DARKINTERVAL // DI NEW - slam planting soldiers
...
#endif
#ifdef DARKINTERVAL
ConVar g_debug_combine_soldier("g_debug_combine_soldier", "0");
#endif
...
#ifndef DARKINTERVAL				// DI does not distinguish shotgunners, like 2004 HL2 did not
...
#define COMBINE_LIMP_HEALTH				20 // unused
...
#ifdef DARKINTERVAL
#define COMBINE_AE_KICK_THAT_IS_ACTUALLY_A_KICK ( 333 )
// DI NEW - manhack tossing for soldiers
#define COMBINE_AE_START_DEPLOY_MANHACK (334)
#define COMBINE_AE_DEPLOY_MANHACK	(335)
#define COMBINE_BODYGROUP_INSIGNIA	(1)
#define COMBINE_BODYGROUP_MANHACK	(2)
#endif
...
Activity ACT_COBMINE_DEPLOY_MANHACK; // DI NEW - manhack tossing for soldiers
#endif
// -----------------------------------------------
//	> Squad slots
// -----------------------------------------------
...
#ifdef DARKINTERVAL
CBaseEntity *CNPC_Combine::CheckTraceHullAttack(float flDist, const Vector &mins, const Vector &maxs, int iDamage, int iDmgType, float forceScale, bool bDamageAnyNPC)
{
...
class CTraceFilterCombineMelee : public CTraceFilterEntitiesOnly
{
public:
	// It does have a base, but we'll never network anything below here..
	DECLARE_CLASS_NOBASE(CTraceFilterCombineMelee);

	CTraceFilterCombineMelee(const IHandleEntity *passentity, int collisionGroup, CTakeDamageInfo *dmgInfo, float flForceScale, bool bDamageAnyNPC)
...
CBaseEntity *CNPC_Combine::CheckTraceHullAttack(const Vector &vStart, const Vector &vEnd, const Vector &mins, const Vector &maxs, int iDamage, int iDmgType, float flForceScale, bool bDamageAnyNPC)
{

	CTakeDamageInfo	dmgInfo(this, this, iDamage, DMG_SLASH);

	CTraceFilterCombineMelee traceFilter(this, COLLISION_GROUP_NONE, &dmgInfo, flForceScale, bDamageAnyNPC);
...
#ifdef DARKINTERVAL // DI NEW - weapon pickup for soldiers
...
#ifdef DARKINTERVAL
void CNPC_Combine::InputEnablePatrol(inputdata_t &inputdata)
{
	m_shouldpatrol_boolean = true;
}

void CNPC_Combine::InputDisablePatrol(inputdata_t &inputdata)
{
	m_shouldpatrol_boolean = false;
}
#endif
...
#ifdef DARKINTERVAL 
//-----------------------------------------------------------------------------
// DI NEW - slam planting soldiers
// Purpose: Force the combine soldier to run toward this target and plant
// a slamp at the ground.
// Input  : &inputdata - 
//-----------------------------------------------------------------------------
void CNPC_Combine::InputPlantSlamGrenade(inputdata_t &inputdata)
{
	// Ignore if we're inside a scripted sequence
	if (m_NPCState == NPC_STATE_SCRIPT && m_hCine)
		return;
	
	ClearSchedule("Told to plant slam via input");
	SetSchedule(SCHED_COMBINE_PLANT_SLAM);
}

//-----------------------------------------------------------------------------
// DI NEW - manhack tossing for soldiers
// Purpose: Forces a manhack deploy chain of events
//-----------------------------------------------------------------------------
void CNPC_Combine::InputForceManhackToss(inputdata_t &inputdata)
{
	if (IsElite()) return;
	m_iGearAmount++;
...
void CNPC_Combine::Spawn( void )
{
	...
	m_flNextGrenadeCheck	= CURTIME + 1;
	m_flNextPainSoundTime	= CURTIME;
	m_flNextAlertSoundTime	= CURTIME;
	m_shouldpatrol_boolean	= false;
#ifdef DARKINTERVAL
	m_armourvalue_float = 100;
	m_armourrate_float = 0.4;
	...
	// DI NEW - shoulder insignias for soldiers
	SetBodygroup(COMBINE_BODYGROUP_INSIGNIA, m_insignia_bodygroup_int);
#endif
	...
	CapabilitiesAdd( 
#ifndef DARKINTERVAL // to stop complaining in the console
		bits_CAP_TURN_HEAD |
#endif
...
#ifdef DARKINTERVAL	// ladder climb test
	CapabilitiesAdd(bits_CAP_MOVE_CLIMB);
#endif
	CapabilitiesAdd( bits_CAP_NO_HIT_SQUADMATES );

#ifdef DARKINTERVAL	// Make Combine soldiers obey weapon rules (burst, rest period)
	...
	//	CBaseCombatWeapon *pNewWeapon = Weapon_Create("weapon_rpg");
	//	if (pNewWeapon)
	//	{
	//		Weapon_Equip(pNewWeapon);
	//	}
...
{
#ifndef DARKINTERVAL // DI does not care
	if ( IsElite() )
	{
...
#ifdef DARKINTERVAL
void CNPC_Combine::NPCThink()
{
	BaseClass::NPCThink();
	if (g_debug_combine_soldier.GetBool())
	{
		// Draw health bar and armour bar
		Vector vecSpot;
		Vector vecOffset(0, 0, 12);
		const float BARLENGTH = 2.0f;
		const float GRADLENGTH = 4.0f;

		Vector right;
		UTIL_PlayerByIndex(1)->GetVectors(NULL, &right, NULL);

		vecSpot = GetAbsOrigin() - right * 16 + vecOffset;
		NDebugOverlay::Line(vecSpot - right * GRADLENGTH, vecSpot + right * GRADLENGTH, 255, 255, 255, false, 0.1);
		NDebugOverlay::Line(vecSpot, vecSpot + Vector(0, 0, BARLENGTH / 4 * m_armourvalue_float), 255, 255, 0, false, 0.1);
		vecSpot = GetAbsOrigin() - right * 16 + vecOffset + Vector(-8, 0, 0);
		NDebugOverlay::Line(vecSpot - right * GRADLENGTH, vecSpot + right * GRADLENGTH, 255, 255, 255, false, 0.1);
		NDebugOverlay::Line(vecSpot, vecSpot + Vector(0, 0, BARLENGTH * m_iHealth), 255, 0, 0, false, 0.1);
	}
}
#endif
//-----------------------------------------------------------------------------
...
void CNPC_Combine::PrescheduleThink()
{
	...
	if( CURTIME >= m_flStopMoveShootTime )
	{
...
{
	float flNextAltFire = CURTIME + flDelay;
...
{
	// Set this timer so that CURTIME can't catch up to it. 
	...
	// "It's OK to move and shoot until CURTIME == m_flStopMoveShootTime"
	m_flStopMoveShootTime = FLT_MAX;

	if( IsCurSchedule( SCHED_COMBINE_HIDE_AND_RELOAD, false ) )
		m_flStopMoveShootTime = CURTIME + random->RandomFloat( 0.4f, 0.6f );
...
void CNPC_Combine::StartTaskChaseEnemyContinuously( const Task_t *pTask )
{
	...
	float flTaskData = pTask->flTaskData;
#ifdef DARKINTERVAL
	// melee soldiers ignore the task data override and go in for melee distance
	if (GetActiveWeapon() == NULL || GetActiveWeapon()->ClassMatches("weapon_stunstick"))
		flTaskData = 64;
#endif
	// We're done once we get close enough
	if ( WorldSpaceCenter().DistToSqr( pEnemy->WorldSpaceCenter() ) <= flTaskData * flTaskData)
	{
		TaskComplete();
		return;
	}
...
void CNPC_Combine::RunTaskChaseEnemyContinuously( const Task_t *pTask )
{
	...
	if ( m_vSavePosition.DistToSqr( pEnemy->WorldSpaceCenter() ) < (flTaskData * flTaskData) )
		return;
	...
	case TASK_ANNOUNCE_ATTACK:
		{
			...
			if ((int)pTask->flTaskData == 1)
			{
				...
				if	(pBCC && pBCC->IsPlayer() && (!pBCC->FInViewCone ( this )) &&
					(CURTIME - m_last_attack_time > 3.0) )
				{
	...
	case TASK_COMBINE_DEFER_SQUAD_GRENADES:
		{
			if ( m_pSquad )
			{
				...
				while ( pSquadmate )
				{
					...
					if( pCombine )
					{
						pCombine->m_flNextGrenadeCheck = CURTIME + 5;
					}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
float CNPC_Combine::GetHitgroupDamageMultiplier(int iHitGroup, const CTakeDamageInfo &info)
{
	switch (iHitGroup)
	{
	case HITGROUP_HEAD:
...
int CNPC_Combine::OnTakeDamage_Alive(const CTakeDamageInfo &inputInfo)
{
	...
	float damageToArmour = info.GetDamage();

	if (info.GetDamageType() & DMG_SHOCK)
	{
		if (info.GetAttacker()->ClassMatches("npc_vortigaunt"))
		{
			info.SetDamage(MIN(m_iMaxHealth * 0.4f, info.GetDamage())); // never take more than 40% of our max HP from vortigaunt zaps
		}
	}

	if (info.GetDamageType() & DMG_BULLET)
	{
		if (g_debug_combine_soldier.GetBool()) Msg("damage to armour (%.1f) scaled by armour mitigation (%.1f), becomes (%.1f)\n", damageToArmour, GetAmmoDef()->ArmorMitigation(info.GetAmmoType()), damageToArmour * GetAmmoDef()->ArmorMitigation(info.GetAmmoType()));
		damageToArmour *= GetAmmoDef()->ArmorMitigation(info.GetAmmoType());
	}
	else if (info.GetDamageType() & DMG_BLAST || info.GetDamageType() & DMG_BURN)
	{
		damageToArmour *= 2.0f;
	}

	m_armourvalue_float -= damageToArmour;
	if (m_armourvalue_float < 0.0f) m_armourvalue_float = 0.0f;

	if(g_debug_combine_soldier.GetBool()) Msg("remaining armour value is %.1f\n", m_armourvalue_float);

	if (m_armourvalue_float > 0.0f && (
		info.GetDamageType() & DMG_BULLET || 
		info.GetDamageType() & DMG_BURN || 
		info.GetDamageType() & DMG_BLAST || 
		info.GetDamageType() & DMG_CLUB )) // armour only protects from these damage types
	{
		if (g_debug_combine_soldier.GetBool()) Msg("damage (%.1f) scaled by armour rate (%.1f), becomes (%.1f)\n", info.GetDamage(), m_armourrate_float, info.GetDamage() * m_armourrate_float);
		info.ScaleDamage(m_armourrate_float);
	}
...
void CNPC_Combine::TraceAttack(const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr)
{
	...
	{
		SetBloodColor(BLOOD_COLOR_RED);
		if (info.GetAttacker()->ClassMatches("npc_citizen") && info.GetAmmoType() == GetAmmoDef()->Index("Pistol"))
		{
			SetBloodColor(BLOOD_COLOR_MECH); // hack for stopping ch06 apcride soldier from showing blood
			I.SetDamageType(DMG_GENERIC);
			I.SetDamage(0);
			// Weak citizens with pistols can't do anything to a proud Combine soldier
			/*if (RandomFloat(0, 1) > 0.4)*/ g_pEffects->Ricochet(ptr->endpos, (vecDir*-0.5f));
		//	BaseClass::TraceAttack(I, vecDir, ptr);
			return;
		}

		float AM = GetAmmoDef()->ArmorMitigation(info.GetAmmoType());
		...
		{
			if (RandomFloat(0, 1) > 0.2) UTIL_BloodImpact(ptr->endpos, (vecDir*-0.5f), m_bloodColor, 16 );
		}
	}
	BaseClass::TraceAttack(I, vecDir, ptr);
}
		...
		if( m_iLastAnimEventHandled != COMBINE_AE_GREN_TOSS )
		{
			...
			if ( pItem )
			{
				...
				if ( PlayerHasMegaPhysCannon() )
				{
					...
					pWeapon->Dissolve( NULL, CURTIME, false, ENTITY_DISSOLVE_NORMAL );
				}
			}
		}
	}
...
#ifdef DARKINTERVAL
//-----------------------------------------------------------------------------
// Purpose: Makes the soldier fire on the dead enemy for a little while more.
// Input  : 
// Output :
//-----------------------------------------------------------------------------
void CNPC_Combine::Event_KilledOther(CBaseEntity *pVictim, const CTakeDamageInfo &info)
{
	...
	if (GetActiveWeapon() && GetActiveWeapon()->CapabilitiesGet() & bits_CAP_WEAPON_RANGE_ATTACK1)
	{
		...
		pVictim->GetVectors(&victimForward, NULL, NULL);
		// This vector multiplicaton is an attempt to predict where the proxy should be for running enemies.
		// Without it, Combine soldiers shooting at running zombies or other fast enemies,
		// will be seen firing at the empty ground behind the dead zombie whose body flew forward as it died
		// (because it was running at us).
		CAI_BaseNPC *pTarget = CreateCustomTarget(pVictim->GetAbsOrigin() + victimForward * pVictim->GetAbsVelocity().Length(), 1.5f);
...
#ifdef DARKINTERVAL
	// DI NEW - weapon pickup for soldiers
	if (GetCurSchedule()->HasInterrupt(COND_IDLE_INTERRUPT))
	{
		SetCustomInterruptCondition(COND_BETTER_WEAPON_AVAILABLE);
	}
	// DI NEW - RPG-wielding soldiers
	if (GetActiveWeapon())
	{
		if (GetActiveWeapon()->ClassMatches("weapon_rpg") && IsCurSchedule(SCHED_COMBINE_ESTABLISH_LINE_OF_FIRE))
		{
...
Activity CNPC_Combine::NPC_TranslateActivity( Activity eNewActivity )
{
...
#ifdef DARKINTERVAL
	if (GetActiveWeapon() == NULL && !IsInAScript())
	{
	//	if (eNewActivity == ACT_IDLE)
	//		return ACT_IDLE_UNARMED;
	...
	if (eNewActivity == ACT_RUN && GetActiveWeapon() == NULL && !IsInAScript())
	{
	}
#endif
	...
	else if (eNewActivity == ACT_MELEE_ATTACK1)
	{
		if (RandomInt(0,2) < 1)
		{
			eNewActivity = ACT_MELEE_ATTACK2; // switch from punching to kicking on a random chance of ~ 1 in 3
		}
	}
#endif
...
int CNPC_Combine::SelectCombatSchedule()
{
	...
	if ( HasCondition( COND_NEW_ENEMY ) )
	{
		...
		float flTimeSinceFirstSeen = CURTIME - GetEnemies()->FirstTimeSeen( pEnemy );
	...
	if ( IRelationType( GetEnemy() ) == D_FR )
	{
		...
		if ( (pMemory && pMemory->bUnforgettable) || (GetEnemyLastTimeSeen() > (CURTIME - 5.0)) )
		{
...
#ifdef DARKINTERVAL
						const char *pSentenceName = "TLK_DANGER";
#else
...
#ifdef DARKINTERVAL
						DispatchResponse(pSentenceName);
#else
	...
	case NPC_STATE_ALERT:
		{
			...
			{
				if(m_shouldpatrol_boolean || HasCondition( COND_COMBINE_SHOULD_PATROL ) )
					return SCHED_COMBINE_PATROL;
			}
		}
		break;
...
int CNPC_Combine::SelectScheduleAttack()
{
	...
	if ( GetEnemy() && GetEnemy()->Classify() == CLASS_COMBINE && FClassnameIs(GetEnemy(), "npc_turret_floor") )
	{
		...
		{
			float flTimeEnemySeen = CURTIME - flTimeAtFirstHand;
	...
	if ( GetEnemy() && !HasCondition(COND_SEE_ENEMY) )
	{
		...
		flTime = CURTIME - GetEnemies()->LastTimeSeen( GetEnemy() );
...
int CNPC_Combine::SelectScheduleRetrieveItem()
{
	...
	// DI NEW - soldiers picking up medkits
...
//-----------------------------------------------------------------------------
void CNPC_Combine::InputEnableWeaponPickup(inputdata_t &inputdata)
{
...
//-----------------------------------------------------------------------------
void CNPC_Combine::InputDisableWeaponPickup(inputdata_t &inputdata)
{
...
int CNPC_Combine::TranslateSchedule( int scheduleType ) 
{
	...
	{
		if (!GetEnemy() && GetActiveWeapon() && GetActiveWeapon()->m_iClip1 < (GetActiveWeapon()->GetMaxClip1()))
		{
	...
	case SCHED_ESTABLISH_LINE_OF_FIRE:
		{
			...
			if (CanFireRPGAtEnemy(true) && OccupyStrategySlot(SQUAD_SLOT_SPECIAL_ATTACK))
			{
				// If an elite in the squad could fire a combine ball at the player's last known position,
				// do so!
				return SCHED_COMBINE_RANGE_ATTACK1_RPG;
			}
	...
	case SCHED_RANGE_ATTACK1:
	{
		...
		{
			if (false) // (GetSquad()->NumMembers() && GetEnemy())
			{
			//	Vector enemyPos = GetEnemy()->GetAbsOrigin();
			//	Vector squadPos = (GetSquad()->GetSquadMemberNearestTo(enemyPos))->GetAbsOrigin();
			//	if (enemyPos.DistTo(squadPos) < RPG_SAFE_DISTANCE)
				{
					// Don't fire our RPG at an enemy too close to a squadmate
			...
			if ((CapabilitiesGet() & bits_CAP_USE_SHOT_REGULATOR) && GetShotRegulator()->IsInRestInterval())
			return SCHED_STANDOFF;
#endif // DARKINTERVAL
...
#ifdef DARKINTERVAL
/*
	// big flinch - happens on COND_HEAVY_DAMAGE - is broken on soldiers, 
	// flinch gestures play as full anims and make them twist and turn. 
	// This is a temporary fix unfortunately...
...
#ifdef DARKINTERVAL
void CNPC_Combine::CheckFlinches(void)
{
	...
	if (HasCondition(COND_HEAVY_DAMAGE))
	{
		...
		{
			// Base version of this function makes the AI play the flinch gesture only if
			// the schedule isn't set to be interrupted by it. But because Combine soldier
			// attacks usually do have that interrupt, it breaks on them
			// and causes them to play the *animation* not the *gesture*.
			// changing this fixes it and they will flinch from heavy damage with same
			// gestures as with light damage. 
			PlayFlinchGesture();
		}
	...
	if (HasMemory(bits_MEMORY_FLINCHED) && CURTIME > m_flNextFlinchTime)
	{
...
void CNPC_Combine::ReleaseManhack(void)
{
	...
	m_hManhack->ClearSchedule("Manhack released by Combine soldier");
	...
	else
	{
		...
		case COMBINE_AE_GREN_TOSS:
			{
				...
				m_flNextGrenadeCheck = CURTIME + 6;
			}
			handledEvent = true;
			break;
		...
		case COMBINE_AE_GREN_LAUNCH:
			{
				...
				if ( g_pGameRules->IsSkillLevel(SKILL_HARD) )
					m_flNextGrenadeCheck = CURTIME + random->RandomFloat( 2, 5 );// wait a random amount of time before shooting again
				else
					m_flNextGrenadeCheck = CURTIME + 6;// wait six seconds before even looking again to see if a grenade can be thrown.
			}
			handledEvent = true;
			break;
		...
		case COMBINE_AE_GREN_DROP:
			{
				...
				Vector forward, up, slamToss;
				AngleVectors(GetLocalAngles(), &forward, NULL, &up);
				slamToss = forward * 20 - up * 2;
#ifdef DARKINTERVAL // DI NEW - slam planting soldiers
				CSlamGrenade *pGrenade = (CSlamGrenade*)Create("npc_grenade_slam", vecStart, vec3_angle, this);
				pGrenade->SetThrower(this);
				pGrenade->ApplyAbsVelocityImpulse(slamToss);
				pGrenade->SetLocalAngularVelocity(QAngle(0, 400, 0));
				pGrenade->m_bIsLive = true;
				pGrenade->SetDamage(sk_npc_dmg_slam.GetFloat());
				pGrenade->m_bPlayerOwned = false;
				pGrenade->m_NPCOwnerClass = this->Classify();
#else
		...
		case COMBINE_AE_KICK:
			...
			break;
				...
				CBaseEntity *pHurt = CheckTraceHullAttack( 200, GetHullMins(), GetHullMaxs(), 0, DMG_CLUB, 10.0f );
				Msg("Punch\n");
...
#ifdef DARKINTERVAL // DI NEW - manhacks for soldiers
		case COMBINE_AE_START_DEPLOY_MANHACK:
		{
		...
		case COMBINE_AE_DEPLOY_MANHACK:
		{
			// Let it go
			ReleaseManhack();
			...
			m_Sentences.Speak("COMBINE_DEPLOY_MANHACK");
			handledEvent = true;
			break;
		}
#endif // DARKINTERVAL
...
#ifdef DARKINTERVAL
void CNPC_Combine::MeleeAttack(float distance, float damage, QAngle &viewPunch, Vector &shove)
{
	Vector vecForceDir;
	...
	CBaseEntity *pHurt = CheckTraceHullAttack(distance, -Vector(16, 16, 32), Vector(16, 16, 32), damage, DMG_CLUB, 5.0f, false);

	if (pHurt)
	{
		vecForceDir = (pHurt->WorldSpaceCenter() - WorldSpaceCenter());

		//TODO: Until the interaction is setup, kill headcrabs in one hit
		if (FClassnameIs(pHurt, "npc_headcrab"))
		{
			CTakeDamageInfo	dmgInfo(this, this, pHurt->m_iHealth + 25, DMG_CLUB);
...
#ifdef DARKINTERVAL
// Allow Combine soldiers to aim for the head in case of zombies.
// otherwise they'll aim for center of mass... zombies are special case
// because shooting them in the body often leaves the headcrab alive.
Vector CNPC_Combine::GetActualShootTrajectory(const Vector &shootOrigin)
{
	if (GetEnemy() && GetEnemy()->Classify() == CLASS_ZOMBIE)
	{
		Vector vecShootDir = GetEnemy()->HeadTarget(shootOrigin) - shootOrigin;
...
// those ones always.
bool CNPC_Combine::Weapon_CanUse(CBaseCombatWeapon *pWeapon)
{
	if (pWeapon->ClassMatches("weapon_stunstick") ||
		...
		pWeapon->ClassMatches("weapon_sniper") ||
		pWeapon->ClassMatches("weapon_ar1") ||
		pWeapon->ClassMatches("weapon_ar2") ||
		pWeapon->ClassMatches("weapon_rpg") ||
		pWeapon->ClassMatches("weapon_oicw")) return true;
	return false;
}
// DI NEW - weapon pickup for soldiers
// Make the Combine soldiers obey the burst and rest parameters for their weapons
...
void CNPC_Combine::AlertSound( void)
{
	if ( CURTIME > m_flNextAlertSoundTime )
	{
		...
		m_flNextAlertSoundTime = CURTIME + 10.0f;
	}
}
...
bool CNPC_Combine::CanThrowGrenade( const Vector &vecTarget )
{
	...
	if (CURTIME < m_flNextGrenadeCheck )
	{
...
bool CNPC_Combine::CanAltFireEnemy( bool bUseFreeKnowledge )
{
	...
	if( CURTIME < m_flNextAltFireTime )
		return false;
	...
	m_flNextGrenadeCheck = CURTIME + 1.0f;
	return false;
}
#ifdef DARKINTERVAL // DI NEW - RPG-wielding soldiers
bool CNPC_Combine::CanFireRPGAtEnemy(bool bUseFreeKnowledge)
{
//	if (CURTIME < m_flNextAltFireTime)
//		return false;
...
{
#ifdef DARKINTERVAL // just do one proficiency, sub classes can delve into it deeper if needed
...
#ifdef DARKINTERVAL // make the Combine units respect the global state
bool CNPC_Combine::PlayerIsCriminal(void)
{
	if (GlobalEntity_GetState("gordon_precriminal") == GLOBAL_ON)
		return false;
...
BEGIN_DATADESC(CNPC_Combine)
	...
	DEFINE_FIELD(m_shouldpatrol_boolean, FIELD_BOOLEAN),
	...
	DEFINE_KEYFIELD(m_iGearAmount, FIELD_INTEGER, "gear_amount"),
	DEFINE_KEYFIELD(m_iGearType, FIELD_INTEGER, "gear_type"),

	DEFINE_FIELD(m_armourvalue_float, FIELD_FLOAT /*, "ArmourValue"*/),
	DEFINE_FIELD(m_armourrate_float, FIELD_FLOAT /*, "ArmourValue"*/),
#endif
...
#ifdef DARKINTERVAL
	DEFINE_INPUTFUNC(FIELD_VOID, "ForceManhackToss", InputForceManhackToss), // DI NEW - manhack tossing for soldiers
																		 //
	DEFINE_INPUTFUNC(FIELD_VOID, "PlantSlamGrenade", InputPlantSlamGrenade), // DI NEW - slam planting soldiers

	DEFINE_INPUTFUNC(FIELD_VOID, "EnableWeaponPickup", InputEnableWeaponPickup), // DI NEW - weapon pickup for soldiers
	...
	DEFINE_KEYFIELD(m_insignia_bodygroup_int, FIELD_INTEGER, "bodygroup"), // DI NEW - shoulder insignias for soldiers
	DEFINE_KEYFIELD(m_bDontPickupWeapons, FIELD_BOOLEAN, "DontPickupWeapons"), // DI NEW - weapon pickup for soldiers
#endif
END_DATADESC()
...
#ifdef DARKINTERVAL // DI NEW - manhack tossing for soldiers
DECLARE_ACTIVITY( ACT_COBMINE_DEPLOY_MANHACK )
#endif
...
#ifdef DARKINTERVAL  // DI NEW - slam planting soldiers
//=========================================================
// SLAM planting schedule. Called via input. 
// Plays the animation, animevent in it creates the slam.
//=========================================================
DEFINE_SCHEDULE
(
...
//	"		TASK_SPEAK_SENTENCE					5"	// METROPOLICE_SENTENCE_DEPLOY_MANHACK
	"		TASK_PLAY_SEQUENCE					ACTIVITY:ACT_COBMINE_DEPLOY_MANHACK"
	"	"
	"	Interrupts"
	"	"
);
