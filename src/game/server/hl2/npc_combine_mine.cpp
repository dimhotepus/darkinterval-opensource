...
#ifdef DARKINTERVAL
ConVar sk_combine_mine_health("sk_combine_mine_health", "100");
#endif
...
void CBounceBomb::Spawn()
{
	...
	m_flHookPositions = 0;
#ifdef DARKINTERVAL
	SetHealth(sk_combine_mine_health.GetInt());
#else
	SetHealth( 100 );
#endif
//	m_bBounce = true; // DI CHANGE - setting this to true here makes it not work!
	...
	case MINE_STATE_TRIGGERED:
		{
			...
			else
			{
				...
				m_flIgnoreWorldTime = CURTIME + 1.0;
...
void CBounceBomb::CaptiveThink()
{
	...
	float phase = fabs( sin( CURTIME * 4.0f ) );
...
#ifdef DARKINTERVAL
	if (info.GetDamageType() & DMG_BLAST || info.GetDamageType() & DMG_BLAST_SURFACE )
	{
		m_iHealth -= info.GetDamage() * 3; // enough to make it blow up from a single aimed OICW alt-fire
	}
	if ((info.GetDamageType() & DMG_BULLET || info.GetDamageType() & DMG_AIRBOAT) && GetAmmoDef()->ArmorMitigation(info.GetAmmoType() > 1.0f))
	{
		m_iHealth -= info.GetDamage() * GetAmmoDef()->ArmorMitigation(info.GetAmmoType()); // make mines susceptible to bullets, but only if they can pierce armour
		if (m_iHealth < 1) ExplodeThink();
	}
#else
	if (info.GetDamageType() & DMG_BLAST || info.GetDamageType() & DMG_BLAST_SURFACE)
	{
...
void CBounceBomb::SearchThink()
{
	...
	if( m_pConstraint && CURTIME - m_flTimeGrabbed >= 1.0f )
	{
	...
	{
	//	if( m_bBounce ) // DI CHANGE - we want the 'no-bouncing' mines to open their claws, then explode, so it's now incorporated into SetMineState
	//	{
	...
	//		SetThink( &CBounceBomb::ExplodeThink );
	//		SetNextThink( CURTIME + m_flExplosionDelay );
	//	}
	}
}
...
void CBounceBomb::ExplodeTouch( CBaseEntity *pOther )
{
	...
	if( m_flIgnoreWorldTime > CURTIME && !pOther->MyCombatCharacterPointer() )
	{
		return;
	}
...
void CBounceBomb::OnPhysGunPickup( CBasePlayer *pPhysGunUser, PhysGunPickup_t reason )
{
	...
	if( reason == PUNTED_BY_CANNON )
	{
		...
		m_flIgnoreWorldTime = CURTIME + 0.1;
	}
}
