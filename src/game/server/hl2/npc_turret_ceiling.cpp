...
#ifdef DARKINTERVAL
ConVar sk_turret_ceiling_health("sk_turret_ceiling_health", "50");
ConVar sk_turret_ceiling_refire_delay("sk_turret_ceiling_refire_delay", "0.25");

#define	CEILING_TURRET_MODEL		"models/_Tech/Ceiling_Sentry.mdl"
#define	CEILING_TURRET_MODEL_COMBINE	"models/Combine_Turrets/Ceiling_turret.mdl"
#else
...
//Heights
#ifdef DARKINTERVAL
#define	CEILING_TURRET_RETRACT_HEIGHT	10
#define	CEILING_TURRET_DEPLOY_HEIGHT	14
#else
...
protected:	
	...
	float	m_flPingTime;
#ifdef DARKINTERVAL
	float	m_flPlayerGracePeriod; // DI NEW
	float	GetPlayerGracePeriod(void) { return m_flPlayerGracePeriod; }
	void	SetPlayerGracePeriod(float period) { m_flPlayerGracePeriod = period; }
	void	InputSetPlayerGracePeriod(inputdata_t &inputdata) { m_flPlayerGracePeriod = inputdata.value.Float(); }
#endif
...
#ifdef DARKINTERVAL
	DEFINE_FIELD( m_flPlayerGracePeriod, FIELD_FLOAT), // DI NEW
#endif
...
#ifdef DARKINTERVAL
	DEFINE_INPUTFUNC( FIELD_FLOAT, "SetPlayerGracePeriod", InputSetPlayerGracePeriod),
#endif
...
#ifdef DARKINTERVAL
	PrecacheModel( CEILING_TURRET_MODEL_COMBINE );
	const char *pModelName = STRING(GetModelName());
	pModelName = (pModelName && pModelName[0] != '\0') ? pModelName : CEILING_TURRET_MODEL;
#endif
...
#ifdef DARKINTERVAL
	const char *pModelName = STRING(GetModelName());
	SetModel(( pModelName && pModelName[ 0 ] != '\0' ) ? pModelName : CEILING_TURRET_MODEL);
#else
...
#ifdef DARKINTERVAL
	m_iHealth		= sk_turret_ceiling_health.GetFloat();
#else
	m_iHealth = 1000;
#endif
...
#ifdef DARKINTERVAL
	m_flPlayerGracePeriod = 0.5f; // DI NEW
#endif
...
void CNPC_CeilingTurret::SetLastSightTime()
{
	...
	{
		m_flLastSight = CURTIME + CEILING_TURRET_MAX_WAIT;	
	}
}
...
void CNPC_CeilingTurret::ActiveThink( void )
{
	...
	if ( ( bEnemyVisible == false ) || ( flDistToEnemy > CEILING_TURRET_RANGE ))
	{
		if ( m_flLastSight )
		{
			m_flLastSight = CURTIME + 0.5f;
		}
		else if ( CURTIME > m_flLastSight )
		{
...
#ifdef DARKINTERVAL
			m_flShotTime = CURTIME + sk_turret_ceiling_refire_delay.GetFloat();
#endif
		} 
	}
	else
	{
#ifdef DARKINTERVAL
		if( !(GetCurrentActivity() == ACT_CEILING_TURRET_OPEN_IDLE))
#endif
			SetActivity( (Activity) ACT_CEILING_TURRET_OPEN_IDLE );
	}
...
void CNPC_CeilingTurret::SearchThink( void )
{
	...
	if ( GetEnemy() != NULL )
	{
		...
		{
#ifdef DARKINTERVAL
			m_flShotTime  = CURTIME + GetPlayerGracePeriod(); // DI NEW
#else
			m_flShotTime = CURTIME + 0.5f;
#endif
		}
		else
		{
	...
	m_vecGoalAngles.y = GetAbsAngles().y + ( sin( CURTIME * 2.0f ) * 45.0f );
...
void CNPC_CeilingTurret::AutoSearchThink( void )
{
	...
	SetNextThink( CURTIME + random->RandomFloat( 0.2f, 0.4f ) );
...
void CNPC_CeilingTurret::Shoot( const Vector &vecSrc, const Vector &vecDirToEnemy )
{
	if ( m_spawnflags & SF_CEILING_TURRET_OUT_OF_AMMO )
	{
		...
		{
			m_flShotTime = CURTIME + random->RandomFloat( 0.5, 1.5 );
		}
		else
		{
...
void CNPC_CeilingTurret::Ping( void )
{
	...
	m_flPingTime = CURTIME + CEILING_TURRET_PING_TIME;
}
