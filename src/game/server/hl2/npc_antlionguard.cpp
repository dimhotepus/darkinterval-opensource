...
// DI todo - put it on the NPC script system (solve using the guardian model)
//=============================================================================//
...
#ifdef DARKINTERVAL
#define	ANTLIONGUARD_MODEL		"models/_Monsters/Antlions/Guard.mdl" // for part 2.1, no need to use the special model
...
#ifdef DARKINTERVAL	// beta code
	SCHED_ANTLIONGUARD_COVER,
	SCHED_ANTLIONGUARD_COVER_LOOP,
	SCHED_ANTLIONGUARD_COVER_ADVANCE,
	SCHED_ANTLIONGUARD_PAIN, // this calls the cover schedule
//	SCHED_ANTLIONGUARD_EMOTE_SEEN_ENEMY,	
	// end beta code
	SCHED_ANTLIONGUARD_TOSS_PHEROPODS // DI new, our own
#endif
};
...
#ifdef DARKINTERVAL // beta code
	TASK_ANTLIONGUARD_CHECK_UNCOVER,
...
#ifdef DARKINTERVAL // added based on existing anims in the model/sdk sources
...
int AE_ANTLIONGUARD_THROW_PHEROPOD_LEFT;
int AE_ANTLIONGUARD_THROW_PHEROPOD_RIGHT;
#endif
...
#ifdef DARKINTERVAL
	void	InputSetCoverFromAttack(inputdata_t &inputdata); // beta code
	void	InputForceThrowPheropodsAt(inputdata_t &inputdata); // new attack AI
#endif
...
#ifdef DARKINTERVAL
	float			m_flCoverTime; // beta code

	float			m_nextpheropodthrow_time; // new attack stuff
	Vector			m_pheropodthrowtarget_vec3;
	EHANDLE			m_pheropodthrowtarget_ent;
#endif
...
#ifdef DARKINTERVAL
	m_pheropodthrowtarget_vec3 = vec3_invalid;
#endif
}
...
#ifdef DARKINTERVAL
	m_nextpheropodthrow_time = CURTIME; // new attack
	m_pheropodthrowtarget_ent = NULL;

	m_bCoverFromAttack = true; // beta code
	m_flCoverTime = 0;
#endif
...
bool CNPC_AntlionGuard::CanSummon( bool bIgnoreTime )
{
	...
	if ( !bIgnoreTime && m_flNextSummonTime > CURTIME )
		return false;
...
int CNPC_AntlionGuard::SelectUnreachableSchedule( void )
{
	...
	if ( HasCondition( COND_SEE_ENEMY ) )
	{
		...
		if ( m_flNextRoarTime < CURTIME )
		{
			m_flNextRoarTime = CURTIME + RandomFloat( 20,40 );
...
bool CNPC_AntlionGuard::ShouldCharge( const Vector &startPos, const Vector &endPos, bool useTime, bool bCheckForCancel )
{
	...
	if ( !bCheckForCancel )
	{
		...
		if ( useTime && ( m_flChargeTime > CURTIME ) )
			return false;
...
int CNPC_AntlionGuard::SelectSchedule( void )
{
	...
	if ( !m_bInCavern && HasCondition( COND_HEAVY_DAMAGE ) && m_flNextHeavyFlinchTime < CURTIME )
	{
		m_flNextHeavyFlinchTime = CURTIME + 8.0f;
#ifdef DARKINTERVAL
		if (m_bCoverFromAttack) m_flCoverTime = CURTIME + 10;
		return m_bCoverFromAttack ? SCHED_ANTLIONGUARD_PAIN : SCHED_ANTLIONGUARD_PHYSICS_DAMAGE_HEAVY;
#else
...
//-----------------------------------------------------------------------------
#ifdef DARKINTERVAL
ConVar projectile_type("g_antlionguard_projectile_type", "103");
#endif
void CNPC_AntlionGuard::HandleAnimEvent( animevent_t *pEvent )
{
	...
	if ( pEvent->event == AE_ANTLIONGUARD_VOICE_GROWL )
	{
		...
		m_flAngerNoiseTime = CURTIME + duration + random->RandomFloat( 2.0f, 4.0f );
		...
		m_flBreathTime = CURTIME + duration - 0.2f;
	...
	{	
		...
		m_flBreathTime = CURTIME + ( duration * 0.5f );
	...
	{	
		...
		m_flBreathTime = CURTIME + duration;
...
#ifdef DARKINTERVAL	// DI NEW
	if ( pEvent->event == AE_ANTLIONGUARD_THROW_PHEROPOD_LEFT || pEvent->event == AE_ANTLIONGUARD_THROW_PHEROPOD_RIGHT )
	{
		//	if (m_pheropodthrowtarget_vec3 == vec3_invalid) { Msg("1\n"); return; }
		//	if (m_pheropodthrowtarget_ent == NULL) { Msg("2\n"); return; }

		Vector vThrowPos;

		if ( pEvent->event == AE_ANTLIONGUARD_THROW_PHEROPOD_LEFT )
			GetAttachment("0", vThrowPos);
		else
			GetAttachment("1", vThrowPos);

		Vector	vTarget = m_pheropodthrowtarget_vec3;
		Vector vecToss;
		CBaseEntity* pBlocker;
		float flGravity = 600;
		ThrowLimit(vThrowPos, vTarget, flGravity, 3, Vector(0, 0, 0), Vector(0, 0, 0), m_pheropodthrowtarget_ent, &vecToss, &pBlocker);

		CGrenadeBugBait *pGrenade = BugBaitGrenade_Create(vThrowPos, vec3_angle, vecToss, QAngle(600, random->RandomInt(-1200, 1200), 0), this);
		...
		pGrenade->SetProjectileStats(Projectile_LARGE, projectile_type.GetInt(), Projectile_CONTACT, 9.0f, 32.0f, flGravity);
		pGrenade->SetAbsVelocity((vecToss + RandomVector(-0.135f, 0.135f)));
...
void CNPC_AntlionGuard::StartTask( const Task_t *pTask )
{
	...
	case TASK_ANTLIONGUARD_GET_PATH_TO_PHYSOBJECT:
		{
			...
			m_flWaitFinished = CURTIME + 30.0f;
		}
		break;
	...
	case TASK_ANTLIONGUARD_CHECK_UNCOVER:
	{
		...
		// done to stop blended movement
		GetMotor()->MoveStop();

		if ((m_flCoverTime < CURTIME) || (m_bCoverFromAttack == false))
		{
			TaskComplete();
			return;
		}
...
void CNPC_AntlionGuard::SummonAntlions( void )
{
	...
	{
		m_flNextSummonTime = CURTIME + RandomFloat( 15,20 );
	}
	else
	{
		m_flNextSummonTime = CURTIME + RandomFloat( 10,15 );
	}
}
...
{
	m_flAngerNoiseTime = CURTIME + 2.0f;
...
{
	m_flSearchNoiseTime = CURTIME + 2.0f;
...
#ifdef DARKINTERVAL
	if ((baseAct == ACT_RUN) && !IsCurSchedule(SCHED_ANTLIONGUARD_CHARGE) && IsInAScript())
		return (Activity)ACT_ANTLIONGUARD_RUN_FULL;
#endif
...
//	}
	...
	// beta code
	if (m_bCoverFromAttack && IsCurSchedule(SCHED_ANTLIONGUARD_COVER_LOOP) && baseAct == ACT_SMALL_FLINCH)
	{
	//	Msg("Cover flinch\n");
		return (Activity)ACT_ANTLIONGUARD_COVER_FLINCH;
	}
#endif
	return baseAct;
}
...
void CNPC_AntlionGuard::UpdatePhysicsTarget( bool bPreferObjectsAlongTargetVector, float flRadius )
{
	...
	if ( m_flPhysicsCheckTime > CURTIME )
		return;
	...
	m_flPhysicsCheckTime = CURTIME + 1.0f;
}
...
void CNPC_AntlionGuard::PrescheduleThink( void )
{
	...
	if ( m_bIsBurrowed )
		return;
#ifdef DARKINTERVAL // new attack
	// Guard without company will charge, guard with company will throw noxious pods to distract you
	if (m_iNumLiveAntlions >= 2)
	{
	...
	if ( ( m_flGroundSpeed <= 0.0f ) )
	{
		if ( m_bStopped == false )
		{
			...
			m_flBreathTime = CURTIME + duration - (duration*0.75f);
		}
		
		m_bStopped = true;

		if ( m_flBreathTime < CURTIME )
		{
			StartSounds();
			...
			m_flBreathTime = CURTIME + random->RandomFloat( 1.0f, 8.0f );
		}
	}
	else
	{
...
#ifdef DARKINTERVAL // new attack
//-----------------------------------------------------------------------------
// Purpose: start tossing pheropods at specified target's coords - or if emtpy,
// just throw them in front of ourselves
//-----------------------------------------------------------------------------
void CNPC_AntlionGuard::InputForceThrowPheropodsAt(inputdata_t &inputdata)
{
	// Ignore if we're inside a scripted sequence
	if (m_NPCState == NPC_STATE_SCRIPT && m_hCine)
		return;

	if (m_nextpheropodthrow_time > CURTIME)
		return;
	...
	if (pEntity == NULL)
	{
		Warning("**Guard %s cannot find pheropod target %s\n", GetClassname(), inputdata.value.String());
		m_pheropodthrowtarget_ent = NULL;
		return;
	}
	else
	{
		m_pheropodthrowtarget_vec3 = pEntity->GetAbsOrigin();
		m_pheropodthrowtarget_ent = pEntity;
		NDebugOverlay::Cross3D(m_pheropodthrowtarget_vec3, 8, 155, 50, 175, true, 5);
		SetTarget(m_pheropodthrowtarget_ent);
	}
		
	ClearSchedule("Told to throw noxious pods via input");
	SetSchedule(SCHED_ANTLIONGUARD_TOSS_PHEROPODS);
	m_nextpheropodthrow_time = CURTIME + 4.0f;
}
#endif // DARKINTERVAL
//-----------------------------------------------------------------------------
// Purpose: 
...
//		return m_bCoverFromAttack ? SCHED_ANTLIONGUARD_PAIN : BaseClass::TranslateSchedule(scheduleType);
//		break;
#endif // END beta code
	}
...
#ifdef DARKINTERVAL	// beta code
	DEFINE_FIELD(m_bCoverFromAttack, FIELD_BOOLEAN),
	DEFINE_FIELD(m_flCoverTime, FIELD_TIME),
	// END beta code

	// DI NEW
	DEFINE_FIELD(m_nextpheropodthrow_time, FIELD_TIME),
	DEFINE_FIELD(m_pheropodthrowtarget_vec3, FIELD_VECTOR),
	DEFINE_FIELD(m_pheropodthrowtarget_ent, FIELD_EHANDLE),
#endif
...
#ifdef DARKINTERVAL
	DEFINE_INPUTFUNC(FIELD_INTEGER, "SetCoverFromAttack", InputSetCoverFromAttack), // beta code
	DEFINE_INPUTFUNC(FIELD_STRING, "ThrowPheropodsAt", InputForceThrowPheropodsAt), // new attack 
#endif
END_DATADESC()
...
#ifdef DARKINTERVAL	// DI NEW - new attack
	DECLARE_ANIMEVENT( AE_ANTLIONGUARD_THROW_PHEROPOD_LEFT )
	DECLARE_ANIMEVENT( AE_ANTLIONGUARD_THROW_PHEROPOD_RIGHT )
#endif
...
DEFINE_SCHEDULE
(
	SCHED_ANTLIONGUARD_TOSS_PHEROPODS,

	"	Tasks"
	"		TASK_SET_FAIL_SCHEDULE SCHEDULE:SCHED_ANTLIONGUARD_ROAR"
	"		TASK_STOP_MOVING 0"
	"		TASK_FACE_TARGET 0"
	"		TASK_WAIT_FOR_MOVEMENT			0"
	"		TASK_PLAY_SEQUENCE_FACE_TARGET ACTIVITY:ACT_RANGE_ATTACK1"
	"	"
	"	Interrupts"
	""
)
