...
void CAntlionTemplateMaker::Activate( void )
{
	...
	if ( m_iMaxPool )
	{
		...
		if ( m_bDisabled == false && gpGlobals->eLoadType != MapLoad_LoadGame )
		{
			...
			SetContextThink( &CAntlionTemplateMaker::PoolRegenThink, CURTIME + m_flPoolRegenTime, s_pPoolThinkContext );

			// Start our blocked effects cycle
			if ( hl2_episodic.GetBool() == true && HasSpawnFlags( SF_ANTLIONMAKER_DO_BLOCKEDEFFECTS ) )
			{
				SetContextThink( &CAntlionTemplateMaker::FindNodesCloseToPlayer, CURTIME + 1.0f, s_pBlockedEffectsThinkContext );
			}
		}
	}
...
void CAntlionTemplateMaker::Disable( void )
{
	...
	SetContextThink( NULL, CURTIME, s_pPoolThinkContext );
	SetContextThink( NULL, CURTIME, s_pBlockedEffectsThinkContext );
...
bool CAntlionTemplateMaker::FindHintSpawnPosition( const Vector &origin, float radius, string_t hintGroupName, CAI_Hint **pHint, bool bRandom )
{
	...
	if ( pChosenHint != NULL )
	{
		...
		if ( AllHintsFromClusterBlocked( pChosenHint, bChosenHintBlocked ) )
		{
			...
			{
				SetContextThink( &CAntlionTemplateMaker::BlockedCheckFunc, CURTIME + 2.0f, s_pBlockedCheckContext );
			}
...
void CAntlionTemplateMaker::DoBlockedEffects( CBaseEntity *pBlocker, Vector vOrigin )
{
	...
	if( pBlocker )
	{
		...
		if( pPhysObj && pPhysObj->IsAsleep() )
		{
			...
			m_flBlockedBumpTime = CURTIME + random->RandomFloat( 1.75, 2.75 );
		}
	}
}
...
{
	SetContextThink( &CAntlionTemplateMaker::FindNodesCloseToPlayer, CURTIME + random->RandomFloat( 0.75, 1.75 ), s_pBlockedEffectsThinkContext );
...
int CAntlionTemplateMaker::DrawDebugTextOverlays( void )
{
	...
	if ( m_debugOverlays & OVERLAY_TEXT_BIT )
	{
		...
		if ( m_iMaxPool )
		{
			...
			float flTimeRemaining = GetNextThink( s_pPoolThinkContext ) - CURTIME;
