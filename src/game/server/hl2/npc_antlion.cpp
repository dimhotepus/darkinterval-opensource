...
// DI todo - dynamic interaction for impaling soldiers
// DI changes that are not ifdef'd: 
// - *almost* all HL2_EPISODIC was removed, we are running with Episodic antlions
// - hl2_episodic.GetBool() checks were removed (assumed true), same reason.
//=============================================================================//
...
ConVar	sk_antlion_worker_health("sk_antlion_worker_health", "60", FCVAR_NONE, "Hitpoints of an antlion worker. If 0, will use base antlion hitpoints.");
ConVar  sk_antlion_worker_spit_speed("sk_antlion_worker_spit_speed", "600", FCVAR_NONE, "Speed at which an antlion spit grenade travels.");
...
static ConVar  g_test_new_antlion_jump("g_test_new_antlion_jump", "1", FCVAR_ARCHIVE);
...
#ifdef DARKINTERVAL
#define	ANTLION_MELEE1_RANGE		80.0f
#else
...
#ifdef DARKINTERVAL
#define	ANTLION_DRONE_MODEL		"models/_Monsters/Antlions/Antlion_Drone.mdl"
#define ANTLION_WORKER_MODEL	"models/antlion_worker.mdl"
#define	ANTLION_SOLDIER_MODEL	"models/_Monsters/Antlions/Antlion_Soldier.mdl"
#else
...
#ifdef DARKINTERVAL
#define	ANTLION_OBEY_FOLLOW_TIME	FLT_MAX
#else
...
int ACT_ANTLION_CHARGE_OUT;

//int SCHED_ANTLION_CHARGE_ENEMY = 115;
#endif
#ifdef DARKINTERVAL // for antlion charging melee
////////////////////////////////////////////////////////////
class AntlionChargeTraceFilter : public CTraceFilter
{
public:
	// It does have a base, but we'll never network anything below here..
	DECLARE_CLASS_NOBASE(AntlionChargeTraceFilter);

	AntlionChargeTraceFilter(const IHandleEntity *passentity, int collisionGroup, float minMass)
...
inline void AntlionChargeTrace(const Vector &vecAbsStart, const Vector &vecAbsEnd, const Vector &hullMin,
	const Vector &hullMax, unsigned int mask, const CBaseEntity *ignore,
	int collisionGroup, trace_t *ptr, float minMass)
{
	Ray_t ray;
	ray.Init(vecAbsStart, vecAbsEnd, hullMin, hullMax);
	AntlionChargeTraceFilter traceFilter(ignore, collisionGroup, minMass);
...
void CNPC_Antlion::Precache(void)
{
	...
	{
		PrecacheModel(ANTLION_DRONE_MODEL);
		PropBreakablePrecacheAll(MAKE_STRING(ANTLION_DRONE_MODEL));
	...
	PrecacheScriptSound("NPC_Antlion.Distracted");
#ifdef DARKINTERVAL // for antlion charging melee
	PrecacheScriptSound("NPC_Antlion.ChargeIn");
	PrecacheScriptSound("NPC_Antlion.ChargeHit");
#endif
	...
	PrecacheModel("models/gibs/antlion_gib_medium_3b.mdl");
...
void CNPC_Antlion::RunAI(void)
{
	BaseClass::RunAI();
	
	if (m_flAlertRadius > 0)
	{
		CBaseEntity *pEntity = NULL;
		if (IsCurSchedule(SCHED_ANTLION_WAIT_FOR_UNBORROW_TRIGGER))
		{
			// iterate on all entities in the vicinity.
			while ((pEntity = gEntList.FindEntityInSphere(pEntity, GetAbsOrigin(), m_flAlertRadius)) != NULL)
			{
				if (pEntity->IsPlayer())
				{
...
void CNPC_Antlion::Event_Killed(const CTakeDamageInfo &info)
{
	...
	// Stop our zap effect!
	SetContextThink(NULL, CURTIME, "ZapThink");
}
...
#ifndef DARKINTERVAL // DI wants either for soldiers to last longer or to have dynamic interaction implemented (later)
...
void CNPC_Antlion::HandleAnimEvent(animevent_t *pEvent)
{
	...
	if (pEvent->event == AE_ANTLION_WORKER_SPIT)
	{
		if (GetEnemy())
		{
			...
			else // DI todo - maybe tie it to hard difficulty (make it always lead on Hard)
			{
			...
			SetNextAttack(CURTIME + flTime + random->RandomFloat(0.5f, 2.0f));
	...
	if (pBlocker && pBlocker->Classify() == CLASS_ANTLION)
	{
		...
		if (pAntlion)
		{
			if (pAntlion->AllowedToBePushed() == true && GetEnemy() == NULL)
			{
				...
				pAntlion->m_flNextJumpPushTime = CURTIME + 2.0f;
			}
		}
	}
...
void CNPC_Antlion::StartTask(const Task_t *pTask)
{
	...
	case TASK_ANTLION_DROWN:
	{
		...
		m_flTimeDrownSplash = CURTIME + random->RandomFloat(0, 0.5);
		m_flTimeDrown = CURTIME + 4;
		break;
	}
	...
	case TASK_ANTLION_GET_PATH_TO_BUGBAIT:
	{
		...
		if (FindChasePosition(m_vecHeardSound, goalPos))
		{
			...
			if (GetNavigator()->SetGoal(goal, AIN_DISCARD_IF_FAIL) == false)
			{
				...
				{
					TaskFail("Antlion failed to find path to bugbait position (a)\n");
					return;
				}
			}
			else
		...
		TaskFail("Antlion failed to find path to bugbait position (b)\n");
		break;
	}

	case TASK_ANTLION_WAIT_FOR_TRIGGER:
		m_flIdleDelay = CURTIME + 1.0f;
	...
	case TASK_ANTLION_BURROW_WAIT:
		...
		if (pTask->flTaskData == 1.0f)
		{
			...
			m_flBurrowTime = CURTIME + random->RandomFloat(1, 6);
		}
	...
	break;
#ifdef DARKINTERVAL // for antlion charging melee
	case TASK_ANTLION_CHARGE_ENEMY :
	{
		EmitSound("NPC_Antlion.ChargeIn");
		GetMotor()->MoveStop();
		SetActivity(Activity(ACT_ANTLION_CHARGE_IN));
	}
	break;
#endif
	default:
...
void CNPC_Antlion::RunTask(const Task_t *pTask)
{
	...
	case TASK_ANTLION_DROWN:
	{
		if (CURTIME > m_flTimeDrownSplash)
		{
			...
			m_flTimeDrownSplash = CURTIME + random->RandomFloat(0.5, 2.5);
		}

		if (CURTIME > m_flTimeDrown)
		{
			m_bDontExplode = true;
	...
	case TASK_ANTLION_WAIT_FOR_TRIGGER:
		...
		if ((m_flIdleDelay > CURTIME) || GetEntityName() != NULL_STRING)
			return;
	...
	case TASK_ANTLION_CHECK_FOR_UNBORROW:
		...
		m_flBurrowTime = CURTIME + random->RandomFloat(0.5f, 1.0f);
		m_iUnBurrowAttempts++;
	...
	case TASK_ANTLION_CHARGE_ENEMY:
	{
		...
		if (eActivity == Activity(ACT_ANTLION_CHARGE_OUT) )
		{
		...
		if ((eActivity == Activity(ACT_ANTLION_CHARGE_IN)) && (IsActivityFinished()))
		{
			SetActivity(Activity(ACT_ANTLION_CHARGE_LOOP));
		}

		// See if we're still running
		if (eActivity == Activity(ACT_ANTLION_CHARGE_LOOP) || Activity(ACT_ANTLION_CHARGE_IN))
		{
			if ( HasCondition(COND_LOST_ENEMY) || HasCondition(COND_ENEMY_DEAD))
			{
				SetActivity(Activity(ACT_ANTLION_CHARGE_OUT));
				return;
			}
			else
			{
	...
	//	ChargeLookAhead();
		...
		if (AutoMovement(GetEnemy(), &moveTrace) == false)
		{
			...
			if (HandleChargeImpact(moveTrace.vEndPosition, moveTrace.pObstruction))
			{
				...
				if (eActivity == Activity(ACT_ANTLION_CHARGE_IN))
				{
...
#ifdef DARKINTERVAL // for antlion charging melee
bool CNPC_Antlion::HandleChargeImpact(Vector vecImpact, CBaseEntity *pEntity)
{
...
//			m_iChargeMisses++;
	...
	{
		EmitSound("NPC_Antlion.ChargeHit");
		...
		CTakeDamageInfo	info(this, this, vecForce, offset, flDamage, DMG_SLASH);
		pEntity->TakeDamage(info);

//////////////////////////////////////////////////////

		pEntity->ApplyAbsVelocityImpulse((BodyDirection2D() * 40) + Vector(0, 0, 20));
...
float CNPC_Antlion::ChargeSteer(void)
{
	trace_t	tr;
	...
	//Probe
	AntlionChargeTrace(GetAbsOrigin(), testPos, testHullMins, GetHullMaxs(), MASK_SOLID_BRUSHONLY, this, COLLISION_GROUP_NONE, &tr, VPhysicsGetObject()->GetMass() * 0.5f);
...
bool CNPC_Antlion::AllowedToBePushed(void)
{
	...
	if (IsMoving() == false && IsCurSchedule(SCHED_ANTLION_FLIP) == false
		&& GetNavType() != NAV_JUMP && m_flNextJumpPushTime <= CURTIME)
	{
		return true;
	}
...
#ifdef DARKINTERVAL
	if( IsSoldier())
		return BaseClass::IsJumpLegal(startPos, apex, endPos, 128, 128, 256);
	else
#endif
...
bool CNPC_Antlion::ShouldJump(void)
{
	...
	{
		m_flJumpTime = CURTIME + random->RandomFloat(1.0f, 2.0f);
		return false;
	}
...
void CNPC_Antlion::ZapThink(void)
{
	...
	if (m_flZapDuration > CURTIME)
	{
		SetContextThink(&CNPC_Antlion::ZapThink, CURTIME + random->RandomFloat(0.05f, 0.25f), "ZapThink");
	}
	else
	{
		SetContextThink(NULL, CURTIME, "ZapThink");
...
int CNPC_Antlion::SelectSchedule(void)
{
	...
	if (HasCondition(COND_ANTLION_FLIPPED))
	{
		...
		if (m_flZapDuration > CURTIME)
		{
			SetContextThink(&CNPC_Antlion::ZapThink, CURTIME, "ZapThink");
	...
	if (HasCondition(COND_HEAR_BUGBAIT) && (m_bIgnoreBugbait == false))
	{
		...
		if (m_bHasHeardSound)
		{
			...
			while ((pTarget = gEntList.FindEntityInSphere(pTarget, soundOrg, 512 /*bugbait_radius.GetInt()*/)) != NULL)
			{
	...
	case NPC_STATE_COMBAT:
	{
		...
		if ( IsWorker())
		{
			...
			{
				SetNextAttack(CURTIME + random->RandomFloat(2.0f, 4.0f));
			...
			if (HasCondition(COND_CAN_RANGE_ATTACK1))
			{
				...
				{
					EmitSound("NPC_Antlion.PoisonBurstScream");
					SetNextAttack(CURTIME + random->RandomFloat(0.5f, 2.5f));
					if (GetEnemy())
					{
		...
		else
		{
			...
			if (HasCondition(COND_CAN_MELEE_ATTACK2))
			{
				...
				if (m_bLeapAttack == true)
#ifdef DARKINTERVAL // for antlion charging melee
					return SCHED_ANTLION_CHARGE_ENEMY;
#else
					return SCHED_ANTLION_POUNCE_MOVING;
#endif
				else
...
{
	// this is base HL2 version. Episodic version ensures they burn down, but we don't want that to always happen.
...
int CNPC_Antlion::OnTakeDamage_Alive(const CTakeDamageInfo &info)
{
	...
	if (CURTIME - m_flLastDamageTime < 0.5f)
	{
		// Accumulate it
...
void CNPC_Antlion::TraceAttack(const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr)
{
	...
	vecShoveDir.z = 0.0f;
#ifdef DARKINTERVAL // antlion soldiers in DI (not drones!) are much harder to flip over
	float shoveMult = random->RandomFloat(500.0f, 1000.0f); 
	float flipChance = 1.0;
	if (FClassnameIs(this, "npc_antlion_soldier"))
	{
		shoveMult = random->RandomFloat(250.0f, 400.0f);
		flipChance = 0.7;
	}
#endif
	...
	if (IsFlipped())
	{
		...
		{
			PainSound(newInfo);
			Vector vecForce = (vecShoveDir * shoveMult) + Vector(0, 0, 64.0f);
		...
		(newInfo.GetDamageType() & (DMG_BLAST | DMG_CRUSH) && newInfo.GetDamage() >= 25.0f))
	{
		...
		if (!IsRunningDynamicInteraction())
		{
			//Grenades, physcannons, and physics impacts make us flip
			if (hl2_episodic.GetBool())
			{
				...
				{
#ifdef DARKINTERVAL
					if (flipChance >= 1 || RandomFloat(0, 1) < flipChance)
#endif
					{
			...
			else
			{
				...
				if (GetFlags() & FL_ONGROUND)
				{
					...
					//Get tossed!
					ApplyAbsVelocityImpulse((vecShoveDir * shoveMult) + Vector(0, 0, 64.0f));
					SetGroundEntity(NULL);
				}
			}
		}
...
bool CNPC_Antlion::HandleInteraction(int interactionType, void *data, CBaseCombatCharacter *sender)
{
	...
	if (interactionType == g_interactionAntlionFiredAtTarget)
	{
		...
		if (IsWorker())
		{
			...
			SetNextAttack(CURTIME + flDuration);
		}
	}
#ifdef DARKINTERVAL
	if (interactionType == g_interactionAntlionGrubAlert)
	{
		...
		if( sender ) UpdateEnemyMemory(sender->GetEnemy(), sender->GetAbsOrigin(), sender);
		return true;
	}
#endif
...
void CNPC_Antlion::StartJump(void)
{
	...
	m_flJumpTime = CURTIME + random->RandomInt(2, 6);
}
...
bool CNPC_Antlion::IsValidEnemy(CBaseEntity *pEnemy)
{
	...
	if (IsAllied() && pEnemy->IsPlayer())
		return false;
#ifdef DARKINTERVAL // also makes sense to not kill citizens, vorts, companion characters if allied
	if ( IsAllied() && ( pEnemy->Classify() == CLASS_PLAYER_ALLY
		|| pEnemy->Classify() == CLASS_PLAYER_ALLY_VITAL
		|| pEnemy->Classify() == CLASS_VORTIGAUNT
		|| pEnemy->Classify() == CLASS_CITIZEN_PASSIVE
		|| pEnemy->Classify() == CLASS_CITIZEN_REBEL ) )
		return false;
#endif
	if (pEnemy->IsWorld())
		return false;
	...
	if (IsCurSchedule(GetGlobalScheduleId(SCHED_ANTLION_CHASE_BUGBAIT)) && (GetNavigator() != NULL))
	{
		...
		if ((GetNavigator()->IsGoalActive()) && (GetNavigator()->GetGoalPos() - pEnemy->GetAbsOrigin()).Length() > 512 ) //bugbait_radius.GetFloat())
			return false;
	}
...
bool CNPC_Antlion::ShouldGib(const CTakeDamageInfo &info)
{
	...
	if (info.GetDamageType() & (DMG_ALWAYSGIB | DMG_BLAST))
		return true;
#ifdef DARKINTERVAL // make gibbing happen more often
	if (m_iHealth < -(m_iMaxHealth * 0.66))
#else
	if ( m_iHealth < -20 )
#endif
		return true;
...
#ifdef DARKINTERVAL // spawn legs that player can pick up and toss
//	CGib::SpawnSpecificGibs(this, 2, 120, 250, "models/gibs/antlion_gib_medium_3b.mdl", 1200);
	/*
	CPhysicsProp *pLeg1 = dynamic_cast< CPhysicsProp * >(CreateEntityByName("prop_physics"));
	if (pLeg1)
	{
		Vector vLeg1pos;
		int aLeg1pos = LookupAttachment("rightfront");
		GetAttachment(aLeg1pos, vLeg1pos);
		vLeg1pos[2] += 12.0f;
		QAngle aLeg1ang = RandomAngle(0, 180);

		char buf[512];
		// Pass in standard key values
		Q_snprintf(buf, sizeof(buf), "%.10f %.10f %.10f", vLeg1pos.x, vLeg1pos.y, vLeg1pos.z);
		pLeg1->KeyValue("origin", buf);
		Q_snprintf(buf, sizeof(buf), "%.10f %.10f %.10f", aLeg1ang.x, aLeg1ang.y, aLeg1ang.z);
		pLeg1->KeyValue("angles", buf);
		pLeg1->KeyValue("model", "models/gibs/antlion_gib_medium_3b.mdl");
		pLeg1->KeyValue("fademindist", "-1");
		pLeg1->KeyValue("fademaxdist", "0");
		pLeg1->KeyValue("fadescale", "1");
		pLeg1->KeyValue("inertiaScale", "1.0");
		pLeg1->KeyValue("physdamagescale", "0.1");
		pLeg1->Precache();
		DispatchSpawn(pLeg1);
		pLeg1->Activate();
	//	pLeg1->SetCollisionGroup(COLLISION_GROUP_DEBRIS);
		pLeg1->Teleport(&(pLeg1->GetLocalOrigin() + Vector(0, 0, 12)), &aLeg1ang, &(RandomVector(-150, 150)));
		pLeg1->ApplyLocalAngularVelocityImpulse(angVelocity);
	}
	*/
	/*
	CPhysicsProp *pLeg2 = dynamic_cast< CPhysicsProp * >(CreateEntityByName("prop_physics"));
	if (pLeg2)
	{
		Vector vLeg2pos;
		int aLeg2pos = LookupAttachment("leftfront");
		GetAttachment(aLeg2pos, vLeg2pos);
		vLeg2pos[2] += 12.0f;
		QAngle aLeg2ang = RandomAngle(0, 180);

		char buf[512];
		// Pass in standard key values
		Q_snprintf(buf, sizeof(buf), "%.10f %.10f %.10f", vLeg2pos.x, vLeg2pos.y, vLeg2pos.z);
		pLeg2->KeyValue("origin", buf);
		Q_snprintf(buf, sizeof(buf), "%.10f %.10f %.10f", aLeg2ang.x, aLeg2ang.y, aLeg2ang.z);
		pLeg2->KeyValue("angles", buf);
		pLeg2->KeyValue("model", "models/gibs/antlion_gib_medium_3b.mdl");
		pLeg2->KeyValue("fademindist", "-1");
		pLeg2->KeyValue("fademaxdist", "0");
		pLeg2->KeyValue("fadescale", "1");
		pLeg2->KeyValue("inertiaScale", "1.0");
		pLeg2->KeyValue("physdamagescale", "0.1");
		pLeg2->Precache();
		DispatchSpawn(pLeg2);
		pLeg2->Activate();
	//	pLeg2->SetCollisionGroup(COLLISION_GROUP_DEBRIS);
		pLeg2->Teleport(&(pLeg2->GetLocalOrigin() + Vector(0, 0, 12)), &aLeg2ang, &(RandomVector(-150, 150)));
		pLeg2->ApplyLocalAngularVelocityImpulse(angVelocity);
	}
	*/
#endif // DARKINTERVAL
	return true;
...
bool CNPC_Antlion::OverrideMoveFacing(const AILocalMoveGoal_t &move, float flInterval)
{
	...
	// Make antlions face the thumper while they flee away.
	if (IsCurSchedule(SCHED_ANTLION_FLEE_THUMPER))
	{
		CSound *pSound = GetLoudestSoundOfType(SOUND_THUMPER);
...
void CNPC_Antlion::SetFollowTarget(CBaseEntity *pTarget)
{
	...
	m_flObeyFollowTime = CURTIME + ANTLION_OBEY_FOLLOW_TIME;
...
void CNPC_Antlion::SetMoveState(AntlionMoveState_e state)
{
	...
	case ANTLION_MOVE_FIGHT_TO_GOAL:
		...
		m_flSuppressFollowTime = CURTIME + random->RandomInt(10, 15);
		break;
...
void CNPC_Antlion::Flip(bool bZapped /*= false*/)
{
	...
	if (bZapped)
	{
		m_flZapDuration = CURTIME + SequenceDuration(SelectWeightedSequence((Activity)ACT_ANTLION_ZAP_FLIP)) + 0.1f;

		EmitSound("NPC_Antlion.ZappedFlip");
	}
}
#ifdef DARKINTERVAL
void CNPC_Antlion::InputStartFollowingTarget(inputdata_t &inputdata)
{
...
void CNPC_Antlion::InputStopFollowing(inputdata_t &inputdata)
{
	SetFollowTarget(NULL);
}

void CNPC_Antlion::InputFlip(inputdata_t &inputdata)
{
	Flip(false); // no zap
}

void CNPC_Antlion::InputFlipZapped(inputdata_t &inputdata)
{
	Flip(true); // do zap
}
#endif // DARKINTERVAL
...
#ifndef DARKINTERVAL // don't remember why this is disabled...
...
BEGIN_DATADESC(CNPC_Antlion)
	...
	#ifdef DARKINTERVAL // for easier scripting such as the CH09 BEACH vortigaunt shepherd
	...
	DEFINE_INPUTFUNC(FIELD_VOID, "Flip", InputFlip), // DI NEW
	DEFINE_INPUTFUNC(FIELD_VOID, "FlipZapped", InputFlipZapped), // DI NEW
	#endif
...
#ifdef DARKINTERVAL
LINK_ENTITY_TO_CLASS(npc_antlion_drone, CNPC_Antlion);
#endif
...
#ifdef DARKINTERVAL // for antlion charging melee
DECLARE_TASK(TASK_ANTLION_CHARGE_ENEMY)
#endif
//Activities
...
DECLARE_ACTIVITY(ACT_ANTLION_CHARGE_OUT)
#endif
//Events
...
//// DI NEW: Soldier Antlion
#ifdef DARKINTERVAL
ConVar	sk_antlion_soldier_health("sk_antlion_soldier_health", "60");
ConVar	sk_antlion_soldier_swipe_damage("sk_antlion_soldier_swipe_damage", "12");

class CNPC_AntlionSoldier : public CNPC_Antlion
{
	DECLARE_CLASS(CNPC_AntlionSoldier, CNPC_Antlion);

	bool IsWorker(void) const { return false; }
	bool IsSoldier(void) const { return true; }
	...
	{
		BaseClass::Precache();
		PrecacheModel(ANTLION_SOLDIER_MODEL);
		PropBreakablePrecacheAll(MAKE_STRING(ANTLION_SOLDIER_MODEL));
	}
	void Spawn(void)
	{
		BaseClass::Spawn();
		SetModel(ANTLION_SOLDIER_MODEL);
		SetBloodColor(BLOOD_COLOR_ANTLION);
		m_iHealth = sk_antlion_soldier_health.GetInt();
		SetHullType(HULL_WIDE_HUMAN);
		SetHullSizeSmall();
	}
	...
	{
		if (scheduleType == SCHED_ANTLION_CHARGE_ENEMY)
			return SCHED_ANTLION_POUNCE;
	...
	// the soldier caste only has its strong melee attack, no leaping.
	...
	void MeleeAttack(float distance, float damage, QAngle &viewPunch, Vector &shove)
	{
		...
		{
			vecForceDir = (GetEnemy()->GetAbsOrigin() - GetAbsOrigin());
			CTakeDamageInfo info(this, this, sk_antlion_soldier_swipe_damage.GetFloat(), DMG_SLASH);
			CalculateMeleeDamageForce(&info, vecForceDir, GetEnemy()->GetAbsOrigin());
			GetEnemy()->TakeDamage(info);
			return;
		}

		CBaseEntity *pHurt = CheckTraceHullAttack(distance, -Vector(16, 16, 32), Vector(16, 16, 32), sk_antlion_soldier_swipe_damage.GetFloat(), DMG_SLASH, 5.0f);
		...
		if (pHurt)
		{
			...
			if (pPlayer != NULL)
			{
				...
				if (!(pPlayer->GetFlags() & FL_GODMODE) && pPlayer->GetMoveType() != MOVETYPE_NOCLIP)
				{
					pPlayer->ViewPunch(QAngle(RandomFloat(18,24.0f), 0.0f, 0.0f));
					...
					//Push the target back
					pHurt->ApplyAbsVelocityImpulse(-right * 1.0 - forward * -(RandomFloat(350,370)));
				}
			}
...
LINK_ENTITY_TO_CLASS(npc_antlion_soldier, CNPC_AntlionSoldier);
#endif // DARKINTERVAL