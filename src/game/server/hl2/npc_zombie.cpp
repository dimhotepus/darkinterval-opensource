...
// DI todo - put it on the NPC-script based system, except the model path
//=============================================================================//
...
#ifdef DARKINTERVAL // customisable Hammer keyvalues, as we have new varieties of zombies
	string_t			m_TorsoModel;
	string_t			m_LegsModel;
#endif
};
...
void CZombie::Precache( void )
{
	...
	if (m_TorsoModel != NULL_STRING)
		PrecacheModel(m_TorsoModel.ToCStr());
	if (m_LegsModel != NULL_STRING)
		PrecacheModel(m_LegsModel.ToCStr());

//	PrecacheModel( "models/_Monsters/Zombies/classic.mdl" );
	PrecacheModel( "models/_Monsters/Zombies/classic_torso.mdl" );
	PrecacheModel( "models/_Monsters/Zombies/classic_legs.mdl" );
#else
...
{
#ifdef DARKINTERVAL	// DI NEW: Zombies are allowed to use multiple models
	char *szModel = (char *)STRING(GetModelName());
	if (!szModel || !*szModel)
	{
		szModel = "models/_Monsters/Zombies/classic.mdl";
  	...
  	if( CURTIME > m_flNextMoanSound )
  	{
  		if( CanPlayMoanSound() )
  		{
			...
			IdleSound();
  			...
  			m_flNextMoanSound = CURTIME + random->RandomFloat( 2.0, 5.0 );
  		}
  		else
 		{
  			m_flNextMoanSound = CURTIME + random->RandomFloat( 1.0, 2.0 );
  		}
  	}
...
{
#ifdef DARKINTERVAL
	if( HasCondition( COND_PHYSICS_DAMAGE ) && !m_ActBusyBehavior.IsActive() && !m_hCine 
		/*&& !(GetCurrentActivity() == ACT_ZOMBIE_SLUMP_A) 
		&& !(GetCurrentActivity() == ACT_ZOMBIE_SLUMP_A_RISE)
		&& !(GetCurrentActivity() == ACT_ZOMBIE_SLUMP_B)
		&& !(GetCurrentActivity() == ACT_ZOMBIE_SLUMP_B_RISE)*/) // oof. So much stuff to filter out
#else
...
#ifdef DARKINTERVAL
	if (IsOnFire())
		return; // don't emit alert sound while burning! it makes them stop their pain yell, and not resume it.
#endif
...
{
#ifdef DARKINTERVAL
	return (m_LegsModel != NULL_STRING) ? (m_LegsModel.ToCStr()) : ("models/_Monsters/Zombies/classic_legs.mdl");
#else
...
{
#ifdef DARKINTERVAL
	return (m_TorsoModel != NULL_STRING) ? (m_TorsoModel.ToCStr()) : ("models/_Monsters/Zombies/classic_torso.mdl");
#else
...
void CZombie::SetZombieModel(void)
{
	...
	if ( m_fIsTorso )
	{
		if ( FClassnameIs(this, "npc_zombie") ) // zombies who got split in half and survived will have separate model specified for this.
			SetModel(GetTorsoModel());
		else
			SetModel(szModel); // torso zombies who were placed by the mapper will have this specified as their main model.
...
#ifdef DARKINTERVAL // make zombies hit with physics slump into a ragdoll, then get up
	if (scheduleType == SCHED_FLINCH_PHYSICS)
	{
		m_flExtraRagdollTime = RandomFloat(0.5, 2.5);
...
Activity CZombie::NPC_TranslateActivity( Activity newActivity )
{
	...
	if ( m_fIsTorso && ( newActivity == ACT_ZOMBIE_TANTRUM ) )
		return ACT_IDLE;
#ifdef DARKINTERVAL // used for getting up from live ragdoll
	if (newActivity == ACT_STAND)
	{
		if (entindex() % 2)
		{
...
#ifdef DARKINTERVAL // DI todo - this is meant to prevent flinches during getting up, but doesn't seem to work?
			&& !(GetActivity() == ACT_ZOMBIE_SLUMP_A)
			&& !(GetActivity() == ACT_ZOMBIE_SLUMP_A_RISE)
			&& !(GetActivity() == ACT_ZOMBIE_SLUMP_B)
...
bool CZombie::IsHeavyDamage( const CTakeDamageInfo &info )
{
	...
	{
#ifdef DARKINTERVAL // DI todo - this is meant to prevent flinches during getting up, but doesn't seem to work?
		if (m_hCine || !(GetCurrentActivity() == ACT_ZOMBIE_SLUMP_A)
			|| !(GetCurrentActivity() == ACT_ZOMBIE_SLUMP_A_RISE)
			|| !(GetCurrentActivity() == ACT_ZOMBIE_SLUMP_B)
			|| !(GetCurrentActivity() == ACT_ZOMBIE_SLUMP_B_RISE)) // oof. So much stuff to filter out
		{
...
#ifdef DARKINTERVAL // customisable Hammer keyvalues, as we have new varieties of zombies
	DEFINE_KEYFIELD(m_TorsoModel, FIELD_STRING, "TorsoModel"),
	DEFINE_KEYFIELD(m_LegsModel, FIELD_STRING, "LegsModel"),
#endif
END_DATADESC()
