...
#include "soundent.h"
#ifdef DARKINTERVAL
#include "player_pickup.h" // the plan is to make crabs pick-upable with the gravity gun. DI todo - this needs a whole lot of work still.

abstract_class CBaseHeadcrab : public CAI_BaseNPC, public CDefaultPlayerPickupVPhysics
#else
...
public:
	...
	virtual bool CanGibRagdoll(void) { return true; }
#endif
...
#ifdef DARKINTERVAL // currently there's no art for it, so don't
	virtual bool ShouldGib(const CTakeDamageInfo &info) { return false; }
	virtual bool CanGibRagdoll(void) { return false; }
#endif
	//
