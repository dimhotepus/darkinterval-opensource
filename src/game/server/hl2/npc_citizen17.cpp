...
#endif
#endif
#define MEDIC_THROW_SPEED 650//npc_citizen_medic_throw_speed.GetFloat()
#ifndef DARKINTERVAL // reducing amount of convars
...
int AE_CITIZEN_HEAL;
#ifdef DARKINTERVAL
int AE_CITIZEN_SPAWNITEM; // DI NEW
#endif
//-------------------------------------
//-------------------------------------
#ifndef DARKINTERVAL // reducing amount of convars
...
extern ConVar	sk_npc_chest;
extern ConVar	sk_npc_stomach;
extern ConVar	sk_npc_arm;
extern ConVar	sk_npc_leg;

#ifdef DARKINTERVAL
#define DebuggingCommanderMode() ((m_debugOverlays & OVERLAY_NPC_SELECTED_BIT))
#else
...
//-----------------------------------------------------------------------------
#ifdef DARKINTERVAL
class CMattsPipe : public CPlayerCrowbar//CWeaponCrowbar
{
	DECLARE_CLASS( CMattsPipe, CPlayerCrowbar/*CWeaponCrowbar*/ );
#else
...
static const char *g_ppszRandomHeads[] = 
{
	...
	"female_04.mdl",
	"male_05.mdl",
	"female_05.mdl", // DI NEW - Linda citizen
	"male_06.mdl",
	"female_06.mdl",
	"male_07.mdl",
	"female_07.mdl",
...
//						m_FuncTankBehavior
...
#ifdef DARKINTERVAL
	DEFINE_KEYFIELD(	m_healthoverride_int,		FIELD_INTEGER, "healthoverride" ),
#endif
...
#ifdef DARKINTERVAL
	DEFINE_KEYFIELD(	m_ornamentModel,			FIELD_STRING, "ornamentModel" ),
	DEFINE_KEYFIELD(	m_ornamentSkin,				FIELD_INTEGER, "ornamentSkin"),
	DEFINE_KEYFIELD(	m_ornamentAttachment,		FIELD_STRING, "ornamentAttachment" ),
	DEFINE_KEYFIELD(	m_ornamentOffset,			FIELD_VECTOR, "ornamentOffset" ),
	DEFINE_KEYFIELD(	m_ornamentScale,			FIELD_FLOAT, "ornamentScale"),
	DEFINE_FIELD(		m_ornamentHandle,			FIELD_CLASSPTR ),
#endif
...
#ifdef DARKINTERVAL
	DEFINE_INPUTFUNC(FIELD_VOID, "SpeakIdleResponseQuestion", InputSpeakIdleResponseQuestion),
	DEFINE_INPUTFUNC(FIELD_VOID, "SpeakIdleResponseAnswer", InputSpeakIdleResponseAnswer),
	...
	// this is used to scream for help on our way up
	DEFINE_FIELD( m_bGrabbedByBarnacle, FIELD_BOOLEAN),
	DEFINE_FIELD( m_bReleasedByBarnacle, FIELD_BOOLEAN),
	DEFINE_FIELD(m_flReleasedByBarnacleTime, FIELD_FLOAT),
	DEFINE_EMBEDDED(m_ScreamForHelpTimer),
#endif
#if HL2_EPISODIC
...
#ifdef DARKINTERVAL
	PrecacheScriptSound("Arcade.CitizenDeath");
#endif
...
#ifdef DARKINTERVAL
	UTIL_PrecacheOther("citizen_ornament");
	if (m_ornamentModel != NULL_STRING)
		PrecacheModel(m_ornamentModel.ToCStr());
#endif
	BaseClass::Precache();
}
...
void CNPC_Citizen::Spawn()
{
...
#ifdef DARKINTERVAL
	m_iHealth = (m_healthoverride_int > 0 ? m_healthoverride_int : sk_citizen_health.GetFloat());
#else
	...
	m_flNextHealthSearchTime = CURTIME;
...
#ifdef DARKINTERVAL // Hook up citizens to game event systems to broadcast different things to them, such as reacting to player's evil deeds
void CNPC_Citizen::FireGameEvent(IGameEvent * event)
{
	const char *type = event->GetName();

	if ( Q_strcmp(type, "di_event_test") == 0 )
	{
	//	Ignite(30);
	}
}
...
#ifdef DARKINTERVAL
	if (pWeapon && pWeapon->ClassMatches("weapon_frag"))
	{
		pWeapon->AddEffects(EF_NODRAW); // fix for Eric on Outerwall
	}
#endif
}
...
void CNPC_Citizen::GatherConditions()
{
	...
	if( ShouldLookForHealthItem() )
	{
		...
		m_flNextHealthSearchTime = CURTIME + 4.0;
	}
	...
	if( CanHeal() )
	{
		...
		if( pPlayer->IsAlive() && flPlayerDamage > 0 && (flDistSqr <= flStareDist * flStareDist) && pPlayer->FInViewCone( this ) && pPlayer->FVisible( this ) )
		{
			if( m_flTimePlayerStare == FLT_MAX )
			{
				...
				m_flTimePlayerStare = CURTIME;
			}

			// Heal if it's been long enough since last time I healed a staring player.
#ifdef DARKINTERVAL
			if( CURTIME - m_flTimePlayerStare >= 1.0f && CURTIME > m_flTimeNextHealStare && !IsCurSchedule( SCHED_CITIZEN_HEAL ) )
#else
			if ( CURTIME - m_flTimePlayerStare >= sk_citizen_player_stare_time.GetFloat() && CURTIME > m_flTimeNextHealStare && !IsCurSchedule(SCHED_CITIZEN_HEAL) )
#endif
			{
				...
				{
#ifdef DARKINTERVAL
					m_flTimeNextHealStare = CURTIME + 1.0f * .5f;
#else
					m_flTimeNextHealStare = CURTIME + sk_citizen_stare_heal_time.GetFloat() * .5f;
#endif
...
void CNPC_Citizen::PrescheduleThink()
{
	...
	if ( !npc_citizen_insignia.GetBool() && npc_citizen_squad_marker.GetBool() && IsInPlayerSquad() )
	{
		...
		float timeInSquad = CURTIME - m_flTimeJoinedPlayerSquad;
...
#ifdef DARKINTERVAL
	if (m_bGrabbedByBarnacle)
	{
		if (m_ScreamForHelpTimer.Expired())
		{
			Speak(TLK_IDLE/*, "pulledbybarnacle:1"*/);
			m_ScreamForHelpTimer.Set(RandomFloat(1.5, 3));
		}
	}
	else if (m_bReleasedByBarnacle)
	{
		if (CURTIME > m_flReleasedByBarnacleTime + 4) // need this time to fall and raise after being released
		{
			SpeakIfAllowed(TLK_USE/*, "pulledbybarnacle:1"*/);
			int releasedContext = FindContextByName("releasedbybarnacle");
			if (releasedContext != -1) m_ResponseContexts.Remove(releasedContext);
			m_bReleasedByBarnacle = false;
		}
	}
#endif
}
...
int CNPC_Citizen::SelectFailSchedule( int failedSchedule, int failedTask, AI_TaskFailureCode_t taskFailCode )
{
	...
	case SCHED_MOVE_TO_WEAPON_RANGE:
		...
		break;
#ifdef DARKINTERVAL // DI todo - citizens crouching when failing to run from danger, comes from here
...
//	{
//		return SCHED_FLINCH_PHYSICS; // this is supposed to go thru TranslateSchedule and result in live ragdolling from physics impacts. 
//	}								 // however, there's too many edge cases on maps where we can't allow it - displacements, slopes, etc.
									 // At least not until heuristics is built to detect safe zones for live ragdolling...
#endif
...
#ifndef DARKINTERVAL //HL2_EPISODIC
	if ( CanHeal() )
	{
...
int CNPC_Citizen::SelectScheduleRetrieveItem()
{
	...
	if( HasCondition(COND_HEALTH_ITEM_AVAILABLE) )
	{
		...
		if( !IsInPlayerSquad() )
		{
			...
			ClearCondition( COND_HEALTH_ITEM_AVAILABLE ); // DI CHANGE: be merciful, let them have it
		}
		else
#endif
		{
...
#ifndef DARKINTERVAL
	if (m_NPCState == NPC_STATE_COMBAT && GetEnemy() && GetActiveWeapon() != NULL)
	{
		if (HasCondition(COND_CAN_RANGE_ATTACK1))
		{
			return SCHED_RANGE_ATTACK1;
		...
		{
		//	if (!GetActiveWeapon()->UsesClipsForAmmo1()) // most likely an RPG
		//	{
		...
		//	}
			...
			{
				if (FindEntityRelationship(GetEnemy())->disposition != D_FR)
...
int CNPC_Citizen::TranslateSchedule( int scheduleType ) 
{
	...
	case SCHED_MOVE_TO_WEAPON_RANGE:
#ifdef DARKINTERVAL // hacky fix for Canyon RPG guy
		if ( GetActiveWeapon() && FClassnameIs(GetActiveWeapon(), "weapon_rpg") && GetEnemy() && !HasCondition(COND_TOO_CLOSE_TO_ATTACK) )
		{
			return SCHED_RANGE_ATTACK1;
		}
#endif
	...
	case SCHED_RUN_FROM_ENEMY_FALLBACK:
	{
		if (GetActiveWeapon() && FClassnameIs(GetActiveWeapon(), "weapon_rpg") && !HasCondition(COND_TOO_CLOSE_TO_ATTACK))
		{
			return SCHED_RANGE_ATTACK1;
		}
		break;
...
//	case SCHED_RELOAD:
	case SCHED_HIDE_AND_RELOAD:
	{
		if (GetActiveWeapon() && FClassnameIs(GetActiveWeapon(), "weapon_rpg") && !HasCondition(COND_TOO_CLOSE_TO_ATTACK))
		{
			...
			return BaseClass::TranslateSchedule(SCHED_HIDE_AND_RELOAD);
		break;
	}
#endif
		...
		if ( !IsMortar( GetEnemy() ) && GetActiveWeapon() && FClassnameIs( GetActiveWeapon(), "weapon_rpg" ) )
		{
			...
			else
			{
				...
				if ( pPlayer && GetEnemy() && ( ( GetEnemy()->GetAbsOrigin() - 
					pPlayer->GetAbsOrigin() ).LengthSqr() < RPG_SAFE_DISTANCE * RPG_SAFE_DISTANCE ) && (GetEnemy() != pPlayer) )
#else
				if ( pPlayer && GetEnemy() && ( ( GetEnemy()->GetAbsOrigin() -
...
#ifdef DARKINTERVAL
/*
	case SCHED_FLINCH_PHYSICS: // recursion problem. SCHED_DI_FALL_IN_RAGDOLL_PUSH has SCHED_FLINCH_PHYSICS as its own fail schedule...
		{			
		...
		case TASK_CIT_RPG_AUGER:
			{
				...
				float imprecision = 18*sin(CURTIME);
...
void CNPC_Citizen::TaskFail( AI_TaskFailureCode_t code )
{
	...
	{
#ifdef DARKINTERVAL
		m_flPlayerHealTime 	= CURTIME + 20.0f;
#else
		m_flPlayerHealTime = CURTIME + sk_citizen_heal_ally_delay.GetFloat();
#endif
	}
...
//=========================================================
void CNPC_Citizen::SetTurnActivity(void)
{
	...
	if (flYD <= -80 && flYD >= -100 && SelectWeightedSequence(ACT_PICKUP_RACK) != ACTIVITY_NOT_AVAILABLE)
	{
		// 90 degree right.
	...
	if (flYD >= 80 && flYD <= 100 && SelectWeightedSequence(ACT_PICKUP_GROUND) != ACTIVITY_NOT_AVAILABLE)
	{
		// 90 degree left.
		Remember(bits_MEMORY_TURNING);
	...
	if (fabs(flYD) >= 160 && SelectWeightedSequence((Activity)ACT_CIT_BLINDED) != ACTIVITY_NOT_AVAILABLE)
	{
		Remember(bits_MEMORY_TURNING);
		SetIdealActivity((Activity)ACT_CIT_BLINDED);
		return;
	}

	if (flYD <= -45 && SelectWeightedSequence(ACT_SIGNAL1) != ACTIVITY_NOT_AVAILABLE)
	{// big right turn
		SetIdealActivity(ACT_SIGNAL1);
		return;
	}
	if (flYD >= 45 && SelectWeightedSequence(ACT_SIGNAL2) != ACTIVITY_NOT_AVAILABLE)
	{// big left turn
...
void CNPC_Citizen::HandleAnimEvent( animevent_t *pEvent )
{
	...
	else if ( pEvent->event == AE_CITIZEN_HEAL )
	{
		...
		if ( USE_EXPERIMENTAL_MEDIC_CODE() && IsMedic() )
		{
			...
			if ( pTarget )
			{
				m_flPlayerHealTime = CURTIME + sk_citizen_heal_toss_player_delay.GetFloat();;
...
#ifdef DARKINTERVAL
	else if (pEvent->event == AE_CITIZEN_SPAWNITEM) // DI NEW
	{
		CBaseEntity *item = CreateEntityByName(pEvent->options);
		if (item)
		{
			Vector attach;
			QAngle attachang;
			GetAttachment(LookupAttachment("anim_attachment_LH"), attach, attachang);
			item->SetAbsOrigin(attach);
			item->SetAbsAngles(attachang);
			DispatchSpawn(item);
			item->ApplyAbsVelocityImpulse(BodyDirection3D() * 100);
		}
		return;
	}
#endif
...
{
	CBaseCombatWeapon *pWeapon = GetActiveWeapon();
#ifdef DARKINTERVAL // DI todo - is this certainly the right way?
	return ( pWeapon && pWeapon->ClassMatches( "weapon_crowbar" ) );
#else	
		...
		if( pWeapon )
		{
			...
			else if( FClassnameIs( pWeapon, "weapon_shotgun" ) )
			{
				...
				else
				{
					...
					{
						m_flNextWeaponSearchTime = CURTIME + SHOTGUN_DEFER_SEARCH_TIME;
					}
					else
					{
						m_flNextWeaponSearchTime = CURTIME + SHOTGUN_DEFER_SEARCH_TIME + 10.0f;
					}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
#ifdef DARKINTERVAL // DI todo - replace with a Hammer keyvalue?
bool CNPC_Citizen::CanBecomeRagdoll()
{
	return (!Q_strcmp(STRING(GetModelName()), "models/humans/arcade/sim.mdl") == false);
}
#endif
...
int CNPC_Citizen::OnTakeDamage_Alive( const CTakeDamageInfo &info )
{
	...
	if( (infoCopy.GetDamageType() & DMG_BURN) && (infoCopy.GetDamageType() & DMG_DIRECT) )
	{
		Scorch( CITIZEN_SCORCH_RATE, CITIZEN_SCORCH_FLOOR );
	}

	if (HasCondition(COND_DI_IS_IN_LIVE_RAGDOLL)) { infoCopy.ScaleDamage(0.33f); }
...
float CNPC_Citizen::GetHitgroupDamageMultiplier(int iHitGroup, const CTakeDamageInfo &info)
{
	...
	case HITGROUP_HEAD:
	{
		return info.GetAttacker()->IsPlayer() ? sk_npc_head.GetFloat() * 5.0f : sk_npc_head.GetFloat() * 1.5f;
	}
...
/*
void CNPC_Citizen::TraceAttack(const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr)      //)
{
	bool bCriticalDamage = false;

	if (ptr->hitgroup == HITGROUP_HEAD)
	{
		if (info.GetAttacker() && info.GetAttacker()->IsPlayer() && !IsInAScript())
		{
			// Shot in the head by the player. Always lethal because citizens have virtually no armour.
...
bool CNPC_Citizen::ShouldAutoSummon()
{
	...
	else if ( m_AutoSummonTimer.Expired() )
	{
...
#ifdef DARKINTERVAL
		if ( CURTIME - m_AutoSummonTimer.GetNext() > 5.0f * 2 )
#else
		if ( CURTIME - m_AutoSummonTimer.GetNext() > player_squad_autosummon_time.GetFloat() * 2 )
#endif
		{
			bSetFollow = true;
#ifndef DARKINTERVAL
			if ( player_squad_autosummon_debug.GetBool() )
				DevMsg( "Auto summoning squad: long time (%f)\n", ( CURTIME - m_AutoSummonTimer.GetNext() ) + player_squad_autosummon_time.GetFloat() );
#endif
		}
		...
		if ( distMovedSq > Square(12) )
		{
			...
			{
#ifdef DARKINTERVAL
				float closenessTolerance = 20.0f * 12;
#else
		...
		if ( bSetFollow && bTestEnemies )
		{
			for ( int i = 0; i < g_AI_Manager.NumAIs(); i++ )
			{
				CAI_BaseNPC *pNpc = g_AI_Manager.AccessAIs()[i];
#ifdef DARKINTERVAL
				float timeSinceCombatTolerance = 8.0f;
#else
				float timeSinceCombatTolerance = player_squad_autosummon_time_after_combat.GetFloat();
#endif
				if ( pNpc->IsInPlayerSquad() )
				{
					if ( CURTIME - pNpc->GetLastAttackTime() > timeSinceCombatTolerance || 
						 CURTIME - pNpc->GetLastDamageTime() > timeSinceCombatTolerance )
						continue;
				}
				else if ( pNpc->GetEnemy() )
				{
					CBaseEntity *pNpcEnemy = pNpc->GetEnemy();
					if ( !IsSniper( pNpc ) && ( CURTIME - pNpc->GetEnemyLastTimeSeen() ) > timeSinceCombatTolerance )
						continue;
		...
		else if ( GetCurSchedule() && ConditionInterruptsCurSchedule( COND_IDLE_INTERRUPT ) )
		{
			if ( SpeakIfAllowed( TLK_QUESTION, NULL, true ) )
			{
				if ( random->RandomInt( 1, 4 ) < 4 )
				{
					...
					if ( pRespondant )
					{
						g_EventQueue.AddEvent( pRespondant, "SpeakIdleResponse", ( GetTimeSpeechComplete() - CURTIME ) + .2, this, this );
					}
				}
			}
		}
...
void CNPC_Citizen::UpdatePlayerSquad()
{
	...
	if ( ( pPlayer->GetAbsOrigin().AsVector2D() - GetAbsOrigin().AsVector2D() ).LengthSqr() < Square(20*12) )
		m_flTimeLastCloseToPlayer = CURTIME;
	...
	if ( pPlayerSquad )
	{
		...
		for ( CAI_BaseNPC *pPlayerSquadMember = pPlayerSquad->GetFirstMember(&iter); pPlayerSquadMember; pPlayerSquadMember = pPlayerSquad->GetNextMember(&iter) )
		{
			...
			if ( !pCitizen->m_bNeverLeavePlayerSquad &&
				 ...
				 CURTIME - pCitizen->m_FollowBehavior.GetTimeFailFollowStarted() > 20 &&
				   ...
				   ( pCitizen->m_FollowBehavior.GetFollowTarget()->GetAbsOrigin().AsVector2D() - pCitizen->GetAbsOrigin().AsVector2D() ).LengthSqr() > Square(50*12) ) )
			{
				if ( DebuggingCommanderMode() )
				{
					DevMsg( "Player follower is lost (%d, %f, %d)\n", 
						 ...
						 CURTIME - pCitizen->m_FollowBehavior.GetTimeFailFollowStarted(), 
		...
		for ( i = 0; i < g_AI_Manager.NumAIs(); i++ )
		{
			...
			else
			{
				...
				if ( distSq > JOIN_PLAYER_XY_TOLERANCE_SQ && 
					( pCitizen->m_flTimeJoinedPlayerSquad == 0 || CURTIME - pCitizen->m_flTimeJoinedPlayerSquad > 60.0 ) && 
					( pCitizen->m_flTimeLastCloseToPlayer == 0 || CURTIME - pCitizen->m_flTimeLastCloseToPlayer > 15.0 ) )
					continue;
...
void CNPC_Citizen::FixupPlayerSquad()
{
	...
	m_flTimeJoinedPlayerSquad = CURTIME;
...
CAI_BaseNPC *CNPC_Citizen::GetSquadCommandRepresentative()
{
	...
	if ( IsInPlayerSquad() )
	{
		...
		if ( CURTIME - lastTime > 2.0 || !hCurrent || !hCurrent->IsInPlayerSquad() ) // hCurrent will be NULL after level change
		{
			lastTime = CURTIME;
			hCurrent = NULL;
...
//-----------------------------------------------------------------------------
#ifdef DARKINTERVAL
ConVar di_barnacle_rescue_live_ragdoll("di_barnacle_rescue_live_ragdoll", "1");
#endif
bool CNPC_Citizen::HandleInteraction(int interactionType, void *data, CBaseCombatCharacter* sourceEnt)
{
	...
	else if (interactionType ==	g_interactionScannerInspectBegin)
	{
		...
		SetNextScannerInspectTime( CURTIME + CIT_INSPECTED_DELAY_TIME );
	...
	{
		m_bGrabbedByBarnacle = true;
		m_bReleasedByBarnacle = false;
		AddContext("pulledbybarnacle:1");
		int releasedContext = FindContextByName("releasedbybarnacle");
		if (releasedContext != -1) m_ResponseContexts.Remove(releasedContext);
		SpeakIfAllowed(TLK_IDLE/*, "pulledbybarnacle:1"*/); // scream for help the first time. Then scream on the way up (in PrescheduleThink)
	...
	//	AddFlag(FL_ONGROUND);
		SetMoveType(MOVETYPE_STEP);
		m_bGrabbedByBarnacle = false;
		m_bReleasedByBarnacle = true;
		m_flReleasedByBarnacleTime = CURTIME;

		ResetIdealActivity(ACT_IDLE);
		
		int pulledContext = FindContextByName("pulledbybarnacle");
		if (pulledContext != -1) m_ResponseContexts.Remove(pulledContext);
		AddContext("releasedbybarnacle:1");
...
bool CNPC_Citizen::ShouldHealTarget( CBaseEntity *pTarget, bool bActiveUse )
{
		...
		Vector toPlayer = ( pTarget->GetAbsOrigin() - GetAbsOrigin() );
			...
			)
	 	{
			if ( pTarget->m_iHealth > 0 )
			{
...
#ifdef DARKINTERVAL
					float timeRecharge = 25.0f;
					float maximumHealAmount = 25.0f;
#else	
					float timeRecharge = sk_citizen_heal_player_delay.GetFloat();
					float maximumHealAmount = sk_citizen_heal_player.GetFloat();
#endif
					float healAmt = ( maximumHealAmount * ( 1.0 - ( timeFullHeal - CURTIME ) / timeRecharge ) );
				...
				bool bReadyToHeal = ( ( bTargetIsPlayer && m_flPlayerHealTime <= CURTIME ) || 
									  ( !bTargetIsPlayer && m_flAllyHealTime <= CURTIME ) );
...
#ifdef DARKINTERVAL
					if ( bTargetIsPlayer )
						requiredHealth = pTarget->GetMaxHealth() - 25.0f;
					else
						requiredHealth = pTarget->GetMaxHealth() * 0.60f;
#else
					if ( bTargetIsPlayer )
	...
	{
		if ( m_flPlayerGiveAmmoTime <= CURTIME )
		{
...
#ifdef DARKINTERVAL
	m_flTimeNextHealStare = CURTIME + 5.0f;
#else
	m_flTimeNextHealStare = CURTIME + sk_citizen_stare_heal_time.GetFloat();
#endif
	if ( IsMedic() )
	{
		float timeFullHeal;
...
#ifdef DARKINTERVAL
			m_flPlayerGiveAmmoTime = CURTIME + 10.0f;
#else
			m_flPlayerGiveAmmoTime = CURTIME + sk_citizen_giveammo_player_delay.GetFloat();
#endif
		}
	}
}
...
bool CNPC_Citizen::ShouldLookForHealthItem()
{
	...
	if( CURTIME < m_flNextHealthSearchTime )
		return false;
...
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
void CNPC_Citizen::InputSetNotCommandable(inputdata_t &inputdata)
{
	AddSpawnFlags(SF_CITIZEN_NOT_COMMANDABLE);
	if (IsInPlayerSquad()) RemoveFromPlayerSquad();
}
#endif
//-----------------------------------------------------------------------------
// Purpose: 
...
#ifdef DARKINTERVAL
void CNPC_Citizen::InputSpeakIdleResponseAnswer(inputdata_t &inputdata)
{
	SpeakIfAllowed(TLK_ANSWER, NULL, false);
}

void CNPC_Citizen::InputSpeakIdleResponseQuestion(inputdata_t &inputdata)
{
	SpeakIfAllowed(TLK_QUESTION, NULL, false);
}

void CNPC_Citizen::InputUnholsterWeapon(inputdata_t &inputdata) // temp hack until better holstering and unholstering is added
{
...
void CNPC_Citizen::DeathSound( const CTakeDamageInfo &info )
{
	...
	SentenceStop();
#ifdef DARKINTERVAL
	if (!Q_strcmp(STRING(GetModelName()), "models/humans/arcade/sim.mdl"))
		EmitSound("Arcade.CitizenDeath");
	else
#endif
...
#ifdef DARKINTERVAL
	DECLARE_ANIMEVENT( AE_CITIZEN_SPAWNITEM ) // DI NEW
#endif
	//=========================================================
	// > SCHED_SCI_HEAL
	//=========================================================
...
{
	m_flResponseAddedTime[ iTrigger ] = CURTIME;
	...
	if ( AI_IsSinglePlayer() )
	{
		for ( int i = 0; i < MAX_CITIZEN_RESPONSES; i++ )
		{
			if ( m_flResponseAddedTime[i] )
			{
				...
				if ( (m_flResponseAddedTime[i] + CITIZEN_RESPONSE_GIVEUP_TIME) < CURTIME )
				{
					m_flResponseAddedTime[i] = 0;
				}
				else if ( m_flNextResponseTime < CURTIME )
				{
					...
					{
						m_flResponseAddedTime[i] = 0;
						m_flNextResponseTime = CURTIME + CITIZEN_RESPONSE_REFIRE_TIME;
...
#ifdef DARKINTERVAL
void CNPC_Citizen::AddOrnament()
{
	CBaseEntity *pMark = CreateEntityByName( "citizen_ornament" );
	pMark->SetOwnerEntity( this );
	pMark->Spawn();
	m_ornamentHandle = assert_cast<CCitizenOrnament*>(pMark);
}

void CNPC_Citizen::RemoveOrnament()
{
	if (m_ornamentHandle != NULL)
	{
		CPhysicsProp *pDropped = assert_cast<CPhysicsProp*>(CreateEntityByName("prop_physics"));
		pDropped->SetModel(m_ornamentModel.ToCStr());
		pDropped->SetAbsOrigin(m_ornamentHandle->GetAbsOrigin());
		pDropped->SetAbsAngles(m_ornamentHandle->GetAbsAngles());
		pDropped->m_nSkin = m_ornamentHandle->m_nSkin;
		pDropped->SetModelScale(m_ornamentHandle->GetModelScale());
		pDropped->SetMoveType(MOVETYPE_VPHYSICS);
		pDropped->SetCollisionGroup(COLLISION_GROUP_INTERACTIVE);
		
		UTIL_Remove(m_ornamentHandle);
		m_ornamentHandle = NULL;

		pDropped->Spawn();
		pDropped->ApplyAbsVelocityImpulse(GetAbsVelocity() + Vector(0, 0, RandomInt(40,100)));
	}
}

//-----------------------------------------------------------------------------
LINK_ENTITY_TO_CLASS( citizen_ornament, CCitizenOrnament);

void CCitizenOrnament::Spawn()
{
	CAI_BaseNPC *pOwner = ( GetOwnerEntity() ) ? GetOwnerEntity()->MyNPCPointer() : NULL;

	if (pOwner)
	{
		CNPC_Citizen *pCitOwner = assert_cast<CNPC_Citizen*>(pOwner);

		if (pCitOwner)
		{
			int attachment = pCitOwner->m_ornamentAttachment == NULL_STRING ? pCitOwner->LookupAttachment("eyes") : pCitOwner->LookupAttachment(pCitOwner->m_ornamentAttachment.ToCStr());
			if (attachment)
			{
				SetAbsAngles(pCitOwner->GetAbsAngles());
				SetParent(pCitOwner, attachment);
				SetLocalOrigin(pCitOwner->m_ornamentOffset);
			}

			SetModel(pCitOwner->m_ornamentModel.ToCStr());
			m_nSkin = pCitOwner->m_ornamentSkin;
			SetSolid(SOLID_NONE);
			SetModelScale(pCitOwner->m_ornamentScale);
		}
	}
}
#else
