...
public:
	...
	{	
#ifndef DARKINTERVAL // DI doesn't use this difficulty tweaking
...
public:
	...
	virtual void		SuspendUse( float flDuration ) { m_flTimeUseSuspended = CURTIME + flDuration; }
...
#ifdef DARKINTERVAL
	void SetSuitCommentsEnabled(bool bState); // comments are disabled at the end of ch03_lab/start of ch04_warehouses
#endif
	// Apply a battery
...
#ifdef DARKINTERVAL // defined by each weapon
	enum PlayerZoomType_t
	{
		ZOOM_DISABLED = -1,
		ZOOM_NAKEDEYE,
		ZOOM_SUIT,
...
#ifdef DARKINTERVAL
	void SetSprintPowerDrainScale(float flScale) { m_flSprintPowerDrainScale = flScale; }
#endif
	bool CanZoom( CBaseEntity *pRequester );
#ifdef DARKINTERVAL
	int  GetZoomTypeFromWeapon(void);
#endif
	void ToggleZoom(void);
#ifdef DARKINTERVAL
	void StartZooming(int zoomtype = ZOOM_SUIT );
#else
	...
	void				InputDisableFlashlightFlicker(inputdata_t &inputdata);
	void				SetForceFlashlightFlicker(bool force);
	void				InputEnableLocator(inputdata_t &inputdata);
	void				InputDisableLocator(inputdata_t &inputdata);
	void				BreakCurrentWeapon(CBaseCombatWeapon *pWeapon);
#endif
...
#ifdef DARKINTERVAL
	// Sprint toggle
	int					ToggleSprintIsOn(void);
	void				ToggleSprintTurnOn(void);
	void				ToggleSprintTurnOff(void);
#endif
	...
	void StartArmorReduction( void ) { m_flArmorReductionTime = CURTIME + ARMOR_DECAY_TIME; 
...
#ifdef DARKINTERVAL
	float				m_flSprintPowerDrainScale; // no suit/worker suit is regular limited sprint, HEV sets the drain to 0
#endif
	CNetworkVar( bool, m_fIsSprinting );
	CNetworkVarForDerived( bool, m_fIsWalking );
#ifdef DARKINTERVAL
	void				UpdateLocator(bool forceUpdate = false); 
...
inline CHL2_Player *GetHL2Player(int iPlayerIndex)
{
	return static_cast<CHL2_Player*>(UTIL_PlayerByIndex(iPlayerIndex));
}
//#endif
