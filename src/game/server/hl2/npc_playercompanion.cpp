...
#ifdef DARKINTERVAL // DI enables friendly fire both way, from and against the player
...
void CNPC_PlayerCompanion::GatherConditions()
{
	...
	if ( AI_IsSinglePlayer() )
	{
		...
		if ( m_AnnounceAttackTimer.Expired() &&
			 ( GetLastEnemyTime() == 0.0 || CURTIME - GetLastEnemyTime() > 20 ) )
		{
...
#ifdef DARKINTERVAL
	return HasSpawnFlags(SF_NPC_NO_PLAYER_PUSHAWAY);
#else
	return false;
#endif
}
...
#ifdef DARKINTERVAL
				if (pSound->m_hOwner->IsNPC() && FClassnameIs(pSound->m_hOwner, "npc_manhack"))
				{
					// Blast away at the manhack if we can.
					if (GetActiveWeapon() != NULL && GetEnemy() && HasCondition(COND_CAN_RANGE_ATTACK1))
...
Activity CNPC_PlayerCompanion::NPC_TranslateActivity( Activity activity )
{
	...
	{
		if ( (m_NPCState == NPC_STATE_COMBAT || m_NPCState == NPC_STATE_ALERT) && CURTIME - m_last_attack_time < 3)
		{
...
bool CNPC_PlayerCompanion::IsValidEnemy( CBaseEntity *pEnemy )
{
	if ( GetFollowBehavior().GetFollowTarget() && GetFollowBehavior().GetFollowTarget()->IsPlayer() && IsSniper( pEnemy ) )
	{
		...
		if ( pInfo )
		{
			if ( CURTIME - pInfo->timeLastSeen > 10 )
			{
			...
			if ( !pEntity || GetEnemies()->NumEnemies() > 1 )
			{
				...
				for ( AI_EnemyInfo_t *pEnemyInfo = GetEnemies()->GetFirst(&iter); pEnemyInfo != NULL; pEnemyInfo = GetEnemies()->GetNext(&iter) )
				{
					...
					if ( pEnemy )
					{
						if ( pEnemy != GetEnemy() )
						{
							if ( pEnemyInfo->timeAtFirstHand == AI_INVALID_TIME || CURTIME - pEnemyInfo->timeLastSeen > 10.0 )
								continue;
							gm_bFindingCoverFromAllEnemies = true;
						}
	...
	if ( pAttacker )
	{
		...
		else
		{
			if ( FVisible( pSquadmate ) )
			{
				...
				if ( !pInfo || ( CURTIME - pInfo->timeLastSeen ) > 15.0 )
...
void CNPC_PlayerCompanion::LockReadiness( float duration )
{
	...
	{
		m_flReadinessLockedUntil = CURTIME + duration;
	}
}
...
{
	// Set to the past
	m_flReadinessLockedUntil = CURTIME - 0.1f;
}
//------------------------------------------------------------------------------
#ifdef HL2_EPISODIC
//-----------------------------------------------------------------------------
...
#ifdef DARKINTERVAL
void CNPC_PlayerCompanion::InputStartIgnoringPlayerPush(inputdata_t &inputdata)
{
	AddSpawnFlags(SF_NPC_NO_PLAYER_PUSHAWAY);
	RemoveSpawnFlags(SF_NPC_ALTCOLLISION);
}

void CNPC_PlayerCompanion::InputStopIgnoringPlayerPush(inputdata_t &inputdata)
{
	AddSpawnFlags(SF_NPC_ALTCOLLISION);
...
void CNPC_PlayerCompanion::OnPlayerKilledOther( CBaseEntity *pVictim, const CTakeDamageInfo &info )
{
	...
	if ( dynamic_cast<CBreakableProp *>( pInflictor ) && ( info.GetDamageType() & DMG_BLAST ) )
	{
		...
		if ( CURTIME - m_fLastBarrelExploded >= MAX_TIME_BETWEEN_BARRELS_EXPLODING )
		{
			m_iNumConsecutiveBarrelsExploded = 0;
		}
		m_iNumConsecutiveBarrelsExploded++;
		m_fLastBarrelExploded = CURTIME;
	...
	else
	{
		...
		if ( CURTIME - m_fLastPlayerKill >= MAX_TIME_BETWEEN_CONSECUTIVE_PLAYER_KILLS )
		{
			m_iNumConsecutivePlayerKills = 0;
		}
		m_iNumConsecutivePlayerKills++;
		m_fLastPlayerKill = CURTIME;
...
#ifdef DARKINTERVAL
	DEFINE_INPUTFUNC(FIELD_VOID, "StartIgnoringPlayerPush", InputStartIgnoringPlayerPush),
	DEFINE_INPUTFUNC(FIELD_VOID, "StopIgnoringPlayerPush", InputStopIgnoringPlayerPush),
#endif
#if HL2_EPISODIC
