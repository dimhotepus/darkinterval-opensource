...
static ConVar physcannon_maxmass( "physcannon_maxmass", "250" );
static ConVar player_throwforce("player_throwforce", "3000");
#ifndef DARKINTERVAL // reducing amount of convars
static ConVar physcannon_minforce( "physcannon_minforce", "700" );
static ConVar physcannon_maxforce( "physcannon_maxforce", "1500" );
...
#else
ConVar di_pickup_prop_transparency("di_pickup_prop_transparency", "1", FCVAR_ARCHIVE | FCVAR_REPLICATED); 
static ConVar weapon_physcannon_suppress_sprint_at_mass("weapon_physcannon_suppress_sprint_at_mass", "100", 0, "If the carried object is this heavy or heavier, the player will not be able to sprint while carrying it");
// for picking up live NPCs like headcrabs or birds. Needs more work
ConVar physcannon_enable_spin_mechanic("physcannon_enable_spin_mechanic", "0", 0, "If 1, pressing reload button will toggle spinning of held objects");
#endif
extern ConVar hl2_normspeed;
extern ConVar hl2_walkspeed;
#ifdef DARKINTERVAL // changed the sprite paths so it's easier to tell which are used and how
#define PHYSCANNON_BEAM_SPRITE		"sprites/physcannon/beam_sprite.vmt"
#define PHYSCANNON_GLOW_SPRITE		"sprites/physcannon/glow_sprite.vmt"
#define PHYSCANNON_ENDCAP_SPRITE	"sprites/physcannon/endcap_sprite.vmt"
#define PHYSCANNON_CENTER_GLOW		"sprites/physcannon/center_glow.vmt"
#define PHYSCANNON_BLAST_SPRITE		"sprites/physcannon/blast_sprite.vmt"

#define MEGACANNON_BEAM_SPRITE		"sprites/physcannon/mega_beam_sprite.vmt"
#define MEGACANNON_GLOW_SPRITE		"sprites/physcannon/mega_glow_sprite.vmt"
#define MEGACANNON_ENDCAP_SPRITE	"sprites/physcannon/mega_endcap_sprite.vmt"
#define MEGACANNON_CENTER_GLOW		"sprites/physcannon/mega_center_glow.vmt"
#define MEGACANNON_BLAST_SPRITE		"sprites/physcannon/mega_blast_sprite.vmt"
#else
...
public:
	...
	bool UpdateObject(CBasePlayer *pPlayer, float flError, bool doSpin = false );
...
float CGrabController::ComputeError()
{
	...
	CHL2_Player *pPortalPlayer = (CHL2_Player *)GetPlayerHoldingEntity(pAttached);
...
{
#ifdef PORTAL_COMPILE
	CHL2_Player *pPortalPlayer = ToHL2Player(pPlayer);
#endif
...
#ifdef DARKINTERVAL // for pick-up transparency
	byte				heldObjectAlpha = 255;			// default alpha for held objects
	RenderMode_t		heldObjectRM = kRenderNormal; // default render mode for held objects
#endif
};
...
BEGIN_DATADESC(CPlayerPickupController)
	...
	#ifdef DARKINTERVAL // for pick-up transparency
	DEFINE_FIELD(heldObjectAlpha, FIELD_CHARACTER),
	DEFINE_FIELD(heldObjectRM, FIELD_INTEGER),
	#endif
END_DATADESC()
...
void CPlayerPickupController::Init( CBasePlayer *pPlayer, CBaseEntity *pObject )
{
	...
	{
#ifdef DARKINTERVAL
		bool objectSmall = false;

		if (pObject->VPhysicsGetObject()->GetMass() <= weapon_physcannon_suppress_sprint_at_mass.GetFloat())
		{
			objectSmall = true;
		}

		pOwner->EnableSprint( objectSmall ); // DI CHANGE:
		// can run while holding smaller object. Imprecise mechanic because doesn't
		// take into account shape and volume, but what can you do...
		// we actually can measure the size, with WorldAlignMaxs and Mins; But it's kinda 
		// difficult to estimate for all cases. So let's just worry about mass. 
#else
...
#ifdef DARKINTERVAL // for pick-up transparency
	//DI test
	if (di_pickup_prop_transparency.GetBool())
	{
		//	DevMsg("Transparent physprops FTW!\n");
		if (Q_strncmp((pObject->GetModelName().ToCStr()), "models/props_junk/plastic", 25) && !(pObject->ClassMatches("func_physbox")))
		{
			// Retrieve these values and save them.
			heldObjectRM = pObject->GetRenderMode();
			heldObjectAlpha = pObject->GetRenderColor().a;

			DevMsg("Picked object non transparent; class %s; mass %f; rendermode %i; renderalpha %i, switching render params...\n",
				pObject->GetClassname(),
				pObject->VPhysicsGetObject()->GetMass(),
				pObject->GetRenderMode(),
				pObject->GetRenderColor().a);

			// Make the prop semi-transparent. 
			// 151 is a very specific number that wouldn't happen normally - helps identify this prop in double-check. 
			pObject->SetRenderMode(kRenderTransAlpha);
			pObject->SetRenderColorA(151);
		}
	}
#endif // DARKINTERVAL
...
#endif
#ifdef DARKINTERVAL // for pick-up transparency
	// DI NEW - without this check, +using props reenables weapons disabled via proxy script (such as in Eli's cave)
	if (!(m_pPlayer->m_Local.m_suppressing_weapons_bool))
#endif
	{
...
void CPlayerPickupController::Shutdown( bool bThrown )
{
	...
	if (pObject == NULL)
	{
		DevMsg("Attached prop disappeared; removing PickupController\n"); // DI change: added so breakable props don't crash the game
		
		// Re-enable weapon selection and all the rest.
		if (m_pPlayer)
		{
...
#ifdef DARKINTERVAL // for pick-up transparency
	// Do NOT test for convar again. 
	// props will only have 151 alpha in this very specific
	// scenario, AND if player decides to change the setting while
	// holding a prop, this check will fail and prop won't revert
	// to non-transparent.
//	if (di_pickup_prop_transparency.GetBool())
	{
		//	DevMsg("Transparent physprops FTW!\n");
			// double check here that this object was affected by our held. 
			// IsTransparent() == 0 means this wasn't some transaprent plastic crate, etc. to begin with.
			// kRenderTransAlpha means object had its RM switched.
			// 151 is this tightly specific value that would ordinary never happen, so we pretty much can be certain this is our affected object.
		if (pObject->GetRenderMode() == kRenderTransAlpha
			&& pObject->GetRenderColor().a == 151)
		{
			DevMsg("Dropping non-transparent object (class %s), render params rolled back to: mode %i, alpha %i\n",
				pObject->GetClassname(),
				pObject->GetRenderMode(),
				pObject->GetRenderColor().a);
			// ideally this was set on pickup (see Init(...)), and we're now restoring it. 
			// by default alpha would be 255.
			pObject->SetRenderColorA(heldObjectAlpha);
			// If it somehow wasn't set, it'll be kRenderNormal.
			pObject->SetRenderMode(heldObjectRM);
		}
	}
#endif // DARKINTERVAL
	if ( m_pPlayer )
	{
		...
		m_pPlayer->SetUseEntity( NULL );
#ifdef DARKINTERVAL // DI NEW - without this check, +using props reenables weapons disabled via proxy script (such as in Eli's cave)
		if (!(m_pPlayer->m_Local.m_suppressing_weapons_bool))
#endif
		{
			if (m_pPlayer->GetActiveWeapon())
...
void CPlayerPickupController::Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	if ( ToBasePlayer(pActivator) == m_pPlayer )
	{
		...
		//Our object became motion disabled, let go!
...
#ifdef DARKINTERVAL
	void	InputBeginUpgrade(inputdata_t &inputdata) 
	{
		BeginUpgrade();
	}
#endif
...
protected:
	...
	bool	AttachObject( CBaseEntity *pObject, const Vector &vPosition, bool emitParticles = 0 );
#ifdef DARKINTERVAL
	void	AttachObjectNPC(CBaseEntity *pObject); // for picking up live NPCs like headcrabs or birds. Needs more work
#endif
	void	UpdateObject( void );
	void	DetachObject( bool playSound = true, bool wasLaunched = false );
#ifdef DARKINTERVAL
	void	DetachObjectNPC(CBaseEntity *pObject); // for picking up live NPCs like headcrabs or birds. Needs more work
#endif
...
#ifdef DARKINTERVAL // experiment for spinning sawblades and such
	bool	m_spin_held_objects_boolean;
#endif
...
#ifdef DARKINTERVAL // whirlwind particles - unused now
//	PrecacheParticleSystem("Weapon_Physcannon_Debris_B");
#endif
	BaseClass::Precache();
}
...
bool CWeaponPhysCannon::Deploy( void )
{
	...
	m_flTimeNextObjectPurge = CURTIME;
...
void CWeaponPhysCannon::PuntNonVPhysics( CBaseEntity *pEntity, const Vector &forward, trace_t &tr )
{
	...
	m_flElementDebounce = CURTIME + 0.5f;
	m_flCheckSuppressTime = CURTIME + 0.25f;
}
...
void CWeaponPhysCannon::PuntVPhysics( CBaseEntity *pEntity, const Vector &vecForward, trace_t &tr )
{
	...
	if ( Pickup_OnAttemptPhysGunPickup( pEntity, pOwner, PUNTED_BY_CANNON ) )
	{
		...
		if ( !IsMegaPhysCannon() && !Pickup_ShouldPuntUseLaunchForces( pEntity, PHYSGUN_FORCE_PUNTED ) )
		{
			...
			if ( pVehicle )
			{
#ifdef DARKINTERVAL // adjsutments for punting vehicles liek the apc
				forward *= 0.2;
				forward.z *= -0.5;
#endif
...
#ifdef DARKINTERVAL	//Newton's 3rd law - Knock the player backwards depending on the mass
	float	knockPerc = (pEntity->VPhysicsGetObject()->GetMass() / physcannon_maxmass.GetFloat());
	...
	knockDir *= -knockForce;
	knockDir[2] /= 4; // don't push upwards too much,
	knockDir[2] += 8; // but ensure some amount of it regardless
...
//	pOwner->RemoveFlag(FL_ONGROUND);
	...
	m_flNextSecondaryAttack = m_flNextPrimaryAttack = CURTIME + 0.5f;
}
...
void CWeaponPhysCannon::PrimaryAttack( void )
{
...
#ifdef DARKINTERVAL
				CBaseEntity *pObject = FindObjectInCone( start, forward, 0.997f );
#else
	...
	if ( pEntity->GetMoveType() != MOVETYPE_VPHYSICS )
	{
		...
		{
#ifdef DARKINTERVAL
			// Let the player zap any NPC's except ally main characters.
			if( pEntity->IsNPC() && pEntity->Classify() == CLASS_PLAYER_ALLY_VITAL )
#else	
		...
		{
#ifdef DARKINTERVAL
			if ( pEntity->IsNPC() && pEntity->MyNPCPointer()->CanBecomeRagdoll() )
#else
	...
	else
	{
		...
		if ( !IsMegaPhysCannon() )
		{
#ifndef DARKINTERVAL // DI allows punting all flesh enemies and ragdolls
...
{
	if ( m_flNextSecondaryAttack > CURTIME )
		return;
	...
	if ( ( m_bActive ) && ( pOwner->m_afButtonPressed & IN_ATTACK2 ) )
	{
		...
		m_flNextPrimaryAttack = CURTIME + 0.5;
		m_flNextSecondaryAttack = CURTIME + 0.5;
	...
	else
	{
		...
		case OBJECT_NOT_FOUND:
			m_flNextSecondaryAttack = CURTIME + 0.1f;
			CloseElements();
			break;

		case OBJECT_BEING_DETACHED:
			m_flNextSecondaryAttack = CURTIME + 0.01f;
			break;
		}
...
// Input  : *pObject - 
//-----------------------------------------------------------------------------
bool CWeaponPhysCannon::AttachObject( CBaseEntity *pObject, const Vector &vPosition, bool emitParticles )
{
	if ( m_bActive )
		return false;
	...
	if ( CanPickupObject( pObject ) == false )
		return false;
#ifdef DARKINTERVAL // experiment for spinning sawblades and such
	m_spin_held_objects_boolean = false; // start without spinning
#endif
...
#ifdef DARKINTERVAL // for picking up live NPCs like headcrabs or birds. Needs more work
		else
		{
			AttachObjectNPC( pObject );
		}
#endif
	}
	...
	if( pOwner )
	{
		...
		pOwner->EnableSprint(objectSmall);
#else
		pOwner->EnableSprint(false);
#endif
	...
	m_flNextSecondaryAttack = CURTIME + 0.4f;
...
#ifndef DARKINTERVAL // disabled because physcannon_right_turrets is 0 by default and was used here
	if( physcannon_right_turrets.GetBool() && pObject->ClassMatches("npc_turret_floor") )	
	{
		...
		if( flDot < 0.5f )
		{
			...
			m_flTimeForceView = CURTIME + 1.0f;
		}
	}
#endif
#ifdef DARKINTERVAL // whirlwind particles - unused now
/*	
	if( emitParticles )
		DispatchParticleEffect("Weapon_Physcannon_Debris_B", 
...
#ifdef DARKINTERVAL  // for picking up live NPCs like headcrabs or birds. Needs more work
void CWeaponPhysCannon::AttachObjectNPC(CBaseEntity *pObject)
{
#ifdef PHYSCANNON_NPC_PICKUP
	// DI todo: figure out everything needed to make this a reality. 

	pObject->SetSolid(SOLID_BBOX);
	pObject->AddSolidFlags(FSOLID_NOT_STANDABLE);
	pObject->RemoveSolidFlags(FSOLID_NOT_SOLID);
	pObject->SetMoveType(MOVETYPE_VPHYSICS);
	pObject->SetCollisionGroup(COLLISION_GROUP_NONE);
	pObject->SetParent(GetOwner()); // todo: consider doing it without parenting. 
									// create a stand-in physics object (cardboard box) that is invisible, 
									// write the NPC side routine to follow it while disabling rest of AI (no attacking or moving).
									// Make it safe and failproof. Make it handled by approved NPC classes only.

	/*
	if (pObject->GetMoveType() == MOVETYPE_STEP)
	{
		pObject->SetMoveType(MOVETYPE_VPHYSICS);
		pObject->RemoveSolidFlags(FSOLID_NOT_SOLID);
	}
...
void CWeaponPhysCannon::DetachObjectNPC(CBaseEntity *pObject)
{
#ifdef PHYSCANNON_NPC_PICKUP
	// DI todo: figure out everything needed to make this a reality.
	...
	if (pObject->IsNPC())
	{
		...
		pObject->AddSolidFlags(FSOLID_NOT_STANDABLE);
		pObject->RemoveSolidFlags(FSOLID_NOT_SOLID);
		pObject->SetMoveType(MOVETYPE_STEP);
		pObject->SetCollisionGroup(COLLISION_GROUP_NPC);
		pObject->SetLocalAngles(QAngle(0, pObject->GetLocalAngles().y, 0));
	}

	//	if (pObject->IsNPC()) pObject->SetMoveType(MOVETYPE_STEP);
#endif
}
#endif // DARKINTERVAL
...
CWeaponPhysCannon::FindObjectResult_t CWeaponPhysCannon::FindObject( void )
{
	...
	if ( !IsMegaPhysCannon() )
	{
		if (!bAttach && !bPull)
		{
			pConeEntity = FindObjectInCone( start, forward, 0.97f );
		}
	}
	else
	{
		pConeEntity = MegaPhysCannonFindObjectInCone( start, forward, 
			0.97f, 0.997f, bAttach || bPull );
	}
	...
	if ( bAttach )
	{
		return AttachObject( pEntity, tr.endpos, true ) ? OBJECT_FOUND : OBJECT_NOT_FOUND;
	}
	...
	pullDir *= IsMegaPhysCannon() ? 8000.0f : 4000.0f;
...
#ifdef DARKINTERVAL
	float flNearestDist = 250.0f + 1.0; //Use regular distance.
#else	
...
//-----------------------------------------------------------------------------
#ifdef DARKINTERVAL // experiment for spinning sawblades and such
bool CGrabController::UpdateObject( CBasePlayer *pPlayer, float flError, bool doSpin )
#else
...
#ifdef DARKINTERVAL // slightly changed positioning
	Vector start;
//	if( !g_debug_physcannon.GetInt() )
		start = pPlayer->Weapon_ShootPosition() /*+ right * 3.0f*/ + forward * 8.0f + up * -2.0f;
//	else
//		start = pPlayer->Weapon_ShootPosition();
	...
	if (doSpin)
	{
		//	AngularImpulse angVel(0, 0, 500.0);
		//	pEntity->VPhysicsGetObject()->AddVelocity(NULL, &angVel);

		//	// no angular drag on this object anymore
		//	float angDrag = 0.0f;
		//	pEntity->VPhysicsGetObject()->SetDragCoefficient(NULL, &angDrag);

		angles = pEntity->GetAbsAngles() + QAngle(0, RandomFloat(50, 55), 0);
	}
#endif
...
#ifdef DARKINTERVAL // experiment for spinning sawblades and such
	if ( !m_grabController.UpdateObject( pPlayer, flError, m_spin_held_objects_boolean ) )
#else
...
void CWeaponPhysCannon::DetachObject( bool playSound, bool wasLaunched )
{
	if ( m_bActive == false )
		return;
#ifdef DARKINTERVAL // experiment for spinning sawblades and such
	m_spin_held_objects_boolean = false;
#endif
	CHL2_Player *pOwner = (CHL2_Player *)ToBasePlayer( GetOwner() );
	if( pOwner != NULL )
	{
...
void CWeaponPhysCannon::ItemPreFrame()
{
	...
	{
#ifdef DARKINTERVAL
		if (pOwner->m_afButtonPressed & IN_RELOAD && physcannon_enable_spin_mechanic.GetBool())
		{
			m_spin_held_objects_boolean = !m_spin_held_objects_boolean;
		}
#endif
		UpdateObject();
	}

	if( CURTIME >= m_flTimeNextObjectPurge )
	{
		PurgeThrownObjects();
		m_flTimeNextObjectPurge = CURTIME + 0.5f;
	}
}
...
void CWeaponPhysCannon::CheckForTarget( void )
{
	...
	if ( m_flCheckSuppressTime > CURTIME )
		return;
	...
	if ( ( m_flElementDebounce < CURTIME ) && ( m_nChangeState == ELEMENT_STATE_NONE ) )
	{
		m_nChangeState = ELEMENT_STATE_CLOSED;
		m_flElementDebounce = CURTIME + 0.5f;
	}
...
void CWeaponPhysCannon::BeginUpgrade()
{
	...
	SetContextThink( &CWeaponPhysCannon::WaitForUpgradeThink, CURTIME + 6.0f, s_pWaitForUpgradeContext );
...
void CWeaponPhysCannon::WaitForUpgradeThink()
{
	...
	{
		SetContextThink( &CWeaponPhysCannon::WaitForUpgradeThink, CURTIME + 0.1f, s_pWaitForUpgradeContext );
		return;
	}
	...
	SetContextThink( NULL, CURTIME, s_pWaitForUpgradeContext );
}
	...
	if ( m_bPhyscannonState != IsMegaPhysCannon() )
	{
		...
		if ( m_bPhyscannonState == false )
		{
			...
			if ( pBeamEnt )
			{
				...
				pCore->SetNextThink( CURTIME + 10.0f );
	...
	if ( IsMegaPhysCannon() )
	{
		...
		if ( random->RandomInt( 0, 100 ) == 0 && !engine->IsPaused() )
		{
			...
			if ( m_hEndSprites[sprite] != NULL )
			{
				...
				m_flEndSpritesOverride[sprite] = CURTIME + lifetime;
...
void CWeaponPhysCannon::LaunchObject( const Vector &vecDir, float flForce )
{
	...
	if( m_grabController.GetAttached() )
	{
		...
		// Don't allow the gun to regrab a thrown object!!
		m_flNextSecondaryAttack = m_flNextPrimaryAttack = CURTIME + 0.5;
	...
	m_nChangeState = ELEMENT_STATE_CLOSED;
	m_flElementDebounce = CURTIME + 0.1f;
	m_flCheckSuppressTime = CURTIME + 0.25f;
}
...
//-----------------------------------------------------------------------------
#ifdef PHYSCANNON_NPC_PICKUP
ConVar physcannon_enable_npc_pickup("physcannon_enable_npc_pickup", "0", FCVAR_NONE, "If 1, enables prototype mechanic for picking up small NPCs (headcrabs, hoppers, birds...");
ConVar physcannon_npc_pickup_maxmass("physcannon_npc_pickup_maxmass", "50", FCVAR_NONE, "NPCs up to this mass will be considered for pickup (if it's enabled)");
#endif
bool CWeaponPhysCannon::CanPickupObject( CBaseEntity *pTarget )
{
	...
	{
#ifdef PHYSCANNON_NPC_PICKUP
		if (physcannon_enable_npc_pickup.GetBool() == 1 && 
			pTarget->IsNPC() 
			&& pTarget->VPhysicsGetObject() 
			&& pTarget->VPhysicsGetObject()->GetMass() < physcannon_npc_pickup_maxmass.GetFloat())
		{
			return true;
		}
#endif
...
#ifdef DARKINTERVAL // whirlwind particles - unused now
	// turn off whirlwind dust particle
...
void CWeaponPhysCannon::DoMegaEffectReady( void )
{
	...
	for ( int i = 0; i < 2; i++ )
	{
		if ( m_hEndSprites[i] != NULL )
		{
			if ( m_flEndSpritesOverride[i] < CURTIME )
			{
				m_hEndSprites[i]->TurnOff();
			}
		}
...
void CWeaponPhysCannon::PurgeThrownObjects()
{
	...
	do
	{
		...
		for( int i = 0 ; i < count ; i++ )
		{
			...
			else if( CURTIME > (m_ThrownEntities[i].fTimeThrown + PHYSCANNON_THROWN_LIST_TIMEOUT) )
			{
				bRemove = true;
			}
			else
...
#ifdef DARKINTERVAL // experiment for spinning sawblades and such
	DEFINE_FIELD(m_spin_held_objects_boolean, FIELD_BOOLEAN),
#endif
...
#ifdef DARKINTERVAL
	DEFINE_INPUTFUNC(FIELD_VOID, "BeginUpgrade", InputBeginUpgrade),
#endif
END_DATADESC()
