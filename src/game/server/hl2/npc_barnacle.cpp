...
#ifndef DARKINTERVAL // reducing amount of convar
ConVar	sk_barnacle_health("sk_barnacle_health", "0"); // and this was replaced with NPC script
...
#ifdef DARKINTERVAL // DI barnacles spawn extra gibs - skeletal remains
const char *CNPC_Barnacle::m_szGibNamesLargeTorso[ NUM_BARNACLE_GIBS - 1 ] =
{
...
const char *CNPC_Barnacle::m_szGibNamesLargeLimbs[ NUM_BARNACLE_GIBS ] =
{
	"models/skeleton/skeleton_humerus_l.mdl",
	"models/skeleton/skeleton_humerus_r.mdl",
	"models/skeleton/skeleton_femur_l.mdl",
	"models/skeleton/skeleton_femur_r.mdl",
};

const char *CNPC_Barnacle::m_szGibNamesRare[ NUM_BARNACLE_GIBS ] =
{
...
{
#ifdef DARKINTERVAL // NPC script
	m_hNPCFileInfo = LookupNPCInfoScript("npc_barnacle");
#endif
...
#ifdef DARKINTERVAL // DI barnacles spawn extra gibs - skeletal remains
	for ( int i = 0; i < ARRAYSIZE(m_szGibNamesLargeTorso); i++ )
	{
		PrecacheModel(m_szGibNamesLargeTorso[ i ]);
	}
	for ( int i = 0; i < ARRAYSIZE(m_szGibNamesLargeLimbs); i++ )
	{
		PrecacheModel(m_szGibNamesLargeLimbs[ i ]);
	}
	for ( int i = 0; i < ARRAYSIZE(m_szGibNamesRare); i++ )
	{
		PrecacheModel(m_szGibNamesRare[ i ]);
	}
#endif
...
#ifdef DARKINTERVAL
	PrecacheScriptSound("NPC_Barnacle.LetGo"); // let go of lifted prey if wounded enough
#endif
...
void CNPC_Barnacle::Spawn()
{
	...
	// On Barnacle, random scale tends to break the tongue visuals.
	...
	// Don't do this for Barnacle. They already init their needed parameters.
	//	NPCInit();
}
...
void CNPC_Barnacle::HandleAnimEvent(animevent_t *pEvent)
{
	if ( pEvent->event == AE_BARNACLE_PUKEGIB )
	{
		...
		bool spawnGibs = ( !HasSpawnFlags(SF_BARNACLE_CHEAP_DEATH) );

		if ( spawnGibs && random->RandomInt(0, 2) > 0 )
		{
			SpawnDeathGibs(); // moved here from EventKilled. More natural to have gibs 
							  // drop together with the particle blood, not instantly. 
		}
#else
...
void CNPC_Barnacle::BarnacleThink(void)
{
	...
	if ( CURTIME < m_pain_tongue_raise_float )
	{
		SetAltitude(16); // keep our tongue raised if we were recently wounded and forced to drop prey
	}
...
#ifdef DARKINTERVAL // 0-25 was way too rare
	...
	else if ( GetEnemy() )
	{
		...
		{
#ifdef DARKINTERVAL
			if ( GetEnemy()->GetHealth() <= 0 && ( GetEnemy()->IsNPC() || GetEnemy()->IsPlayer() ) )
				LostPrey(true);
			else
#endif
				LiftPrey();
	...
	else
	{
		...
		// NOTE: Use the surrounding bounds so that we'll think often even if the tongue
		// tip is in the PVS but the body isn't
		Vector vecSurroundMins, vecSurroundMaxs;
		CollisionProp()->WorldSpaceSurroundingBounds(&vecSurroundMins, &vecSurroundMaxs);
		if ( !UTIL_FindClientInPVS(vecSurroundMins, vecSurroundMaxs) )
		{
			SetNextThink(CURTIME + random->RandomFloat(1, 1.5));	// Stagger a bit to keep barnacles from thinking on the same frame
		}
...
#ifdef DARKINTERVAL // increase chances
		if ( m_cGibs && random->RandomInt(0, 99) < 7 )
#else
...
bool CNPC_Barnacle::CanPickup(CBaseCombatCharacter *pBCC)
{
	...
	if ( FClassnameIs(pBCC, "npc_turret_floor") )
		return false;
#ifdef DARKINTERVAL
	// Don't pickup cremators
	if ( FClassnameIs(pBCC, "npc_cremator") )
		return false;

	// Don't pickup crabsynths
	if ( FClassnameIs(pBCC, "npc_crabsynth") )
		return false;

	// Don't pickup pstorms
	if ( FClassnameIs(pBCC, "npc_pstorm") )
		return false;

	// Don't pickup houndeyes - swallowing them looks stupid
	if ( FClassnameIs(pBCC, "npc_houndeye") )
		return false;
#endif
...
bool CNPC_Barnacle::WaitForRagdollToSettle(float flBiteZOffset)
{
	...
	if ( !m_hRagdoll )
	{
		AssertMsg(0, "Barnacle lost the m_hRagdoll!");
		return false;
	}
	...
	if ( !pRagdollPhys )
	{
		AssertMsg(0, "Barnacle lost the pRagdollPhys of the m_hRagdoll!");
		return false;
	}
#endif
...
void CNPC_Barnacle::LiftPhysicsObject(float flBiteZOffset)
{
	...
	if ( GetAbsOrigin().z - m_vecTip.Get().z < flBiteZOffset ) // then yes, let's chomp
	{
		...
		// DI todo - add healthkit chomping in here to have healthkits heal the barnacle
...
#ifdef DARKINTERVAL
	if ( pTouchEnt->HasSpawnFlags(SF_ALWAYS_SERVER_RAGDOLL) )
	{
		pTouchEnt->RemoveSpawnFlags(SF_ALWAYS_SERVER_RAGDOLL);
	}
#endif
	// Increase the tongue's spring constant while lifting 
	m_hTongueTip->m_pSpring->SetSpringConstant(BARNACLE_TONGUE_SPRING_CONSTANT_LIFTING);
...
CRagdollProp *CNPC_Barnacle::AttachRagdollToTongue(CBaseAnimating *pAnimating)
{
	...
	{
		// Metrocops, Combine soldiers
		m_iGrabbedBoneIndex = pAnimating->LookupBone("ValveBiped.Bip01_Neck1");
	}
	...
	if ( m_iGrabbedBoneIndex == -1 )
	{
		// Bullsquids, althought they currently don't get grabbed
		m_iGrabbedBoneIndex = pAnimating->LookupBone("Bullsquid.Head_Bone");
	}
#endif // DARKINTERVAL
	if ( m_iGrabbedBoneIndex == -1 )
	{
...
void CNPC_Barnacle::BitePrey(void)
{
	...
	if ( enemyClass == CLASS_ZOMBIE )
	{
		if ( m_hRagdoll )
		{
			m_hRagdoll->SetBodygroup(/*ZOMBIE_BODYGROUP_HEADCRAB*/ 1, false);
	...
	// Disable collisions with the world so we can pull it through the roof
...
{
	if ( CURTIME < m_flNextBloodTime )
		return;

	m_flNextBloodTime = CURTIME + 0.2f;
...
#ifdef DARKINTERVAL // fix old bug
		float digestDelay = m_bSwallowingPoison ? 0.48f : 10.0f;
		m_flDigestFinish = CURTIME + digestDelay;
#else
		m_flDigestFinish = ( CURTIME + m_bSwallowingPoison ) ? 0.48f : 10.0f;
#endif
		m_flDigestFinish = CURTIME + 10.0;
#endif
	}
...
#ifdef DARKINTERVAL // DI new - barnacle rescue
		{
			if ( ( GetEnemy()->MyNPCPointer()->CanBeRescuedFromBarnacle() ) == false )
			{ // Doing this because those that can be rescued will have this in their HandleInteraction.
				GetEnemy()->RemoveEffects(EF_NODRAW);
				GetEnemy()->RemoveSolidFlags(FSOLID_NOT_SOLID);
			}
			// prototyping barnacle rescue
			/**/if ( GetEnemy()->MyNPCPointer() )
				/**/	GetEnemy()->MyNPCPointer()->ClearSchedule("");
			/**/if ( GetEnemy()->MyNPCPointer() && !( GetEnemy()->MyNPCPointer()->DispatchInteraction(g_interactionBarnacleVictimReleased, NULL, this) ) )
				/**/ {
				UTIL_Remove(GetEnemy());
				/**/
			} else
			{
			}
...
void CNPC_Barnacle::LostPrey(bool bRemoveRagdoll)
{
	...
	if ( pEnemy )
	{
		...
		{
			// prototyping barnacle rescue
			/**///	pVictim->DispatchInteraction(g_interactionBarnacleVictimReleased, NULL, this); // moved to RemoveRagdoll
			pVictim->RemoveEFlags(EFL_IS_BEING_LIFTED_BY_BARNACLE);
			...
			{
				pVictim->TakeDamage(CTakeDamageInfo(this, this, GetEnemy()->GetHealth(), DMG_CRUSH | DMG_REMOVENORAGDOLL));
			}
		}
#else
		{
	...
	if ( m_hTongueTip )
	{
		...
		if ( pPhysicsObject && pPhysicsObject->GetShadowController() )
		{
			...
			PhysEnableEntityCollisions(pPhysicsObject, g_PhysWorldObject);
...
#ifndef DARKINTERVAL // DI doesn't want barnacles to instantly die from the crowbar
	if ( info.GetDamageType() & DMG_CLUB )
	{
		info.SetDamage(m_iHealth);
	}
#else
	bool forceRetire = false;
	if ( ( info.GetDamage() >= GetMaxHealth() / 2 ) || ( GetHealth() <= GetMaxHealth() / 2 ) )
	{
		if ( GetEnemy() && GetEnemy()->IsPlayer() )
		{
			LostPrey(false);
			EmitSound("NPC_Barnacle.LetGo");
			m_pain_tongue_raise_float = CURTIME + RandomFloat(8.5, 10.5); // keep our tongue raised for ~10 seconds.
		}
	}
#endif
...
void CNPC_Barnacle::SpawnDeathGibs(void)
{
	...
	if ( RandomInt(0, 2) > 1 )
	{
		...
		for ( int i = 0; i < ARRAYSIZE(m_szGibNamesLargeLimbs); i++ )
		{
			CGib::SpawnSpecificGibs(this, 1, 32, 1, m_szGibNamesLargeLimbs[ i ], 25.0f, 2);
		}

		if ( random->RandomInt(0, 2) < 1 ) // spawn the torso gib occasionally
		{
			CBaseEntity *pGib;

			pGib = CreateRagGib(m_szGibNamesLargeTorso[ random->RandomInt(0, 2) ], GetLocalOrigin(), RandomAngle(-180, 180),
				vrf, 0.0f, false, 2);
			//CGib::SpawnSpecificGibs(this, 1, 32, 1, m_szGibNamesLargeTorso[random->RandomInt(0,2)], 25.0f, 2);
		}

		if ( random->RandomInt(0, 4) < 1 ) // outside the cycle: spawn just one of these rarer gibs. And make probability 20% for now.
		{
			CGib::SpawnSpecificGibs(this, 1, 32, 1, m_szGibNamesRare[ random->RandomInt(0, 2) ]);
		}
...
void CNPC_Barnacle::Event_Killed(const CTakeDamageInfo &info)
{
	...
	// Are we lifting prey?
#ifdef DARKINTERVAL // fixed version of the routine to prevent dropped ragdolls from falling through the floor
	if ( GetEnemy() )
	{
		if ( m_bSwallowingPrey && m_hRagdoll )
		{
			...
			//	PhysDisableEntityCollisions(m_hRagdoll->VPhysicsGetObject(), g_PhysWorldObject);
			PhysEnableEntityCollisions(m_hRagdoll->VPhysicsGetObject(), g_PhysWorldObject);
			DetachAttachedRagdoll(m_hRagdoll);
		}

		bool dropLiveEnemy = false;
		if ( GetEnemy() != NULL && GetEnemy()->IsNPC() )
		{
			if ( GetEnemy()->MyNPCPointer()->CanBeRescuedFromBarnacle() ) // prototyping barnacle rescue
			{
				dropLiveEnemy = ( GetEnemy()->GetHealth() > 0 );
			}
		}

		// Cleanup
		// This will apply for non-rescuable prey
		// that should be released before it was bitten.
		// If it was bitten, m_bSwallowingPrey will be true and the ragdoll
		// will be released by the code above.

		// If the prey is rescuable, it will go through its own g_interactionBarnacleVictimReleased
		// and drop either as live ragdoll or just as NPC.
		LostPrey(dropLiveEnemy);
	}
#else
	if ( GetEnemy() )
	{
...
#ifndef DARKINTERVAL // MOVED TO HANDLEANIMEVENT - PUKEGIB
	// Puke gibs unless we're told to be cheap
...
#ifdef DARKINTERVAL
	DEFINE_FIELD(m_pain_tongue_raise_float, FIELD_FLOAT), // keep our tongue raised if we were recently wounded and forced to drop prey
#endif
