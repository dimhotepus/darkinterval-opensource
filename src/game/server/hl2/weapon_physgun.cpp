...
ConVar phys_gunvel("phys_gunvel", "1000");
ConVar phys_gunforce("phys_gunforce", "6e6");
ConVar phys_guntorque("phys_guntorque", "100");
ConVar phys_gunglueradius("phys_gunglueradius", "64");
#else
...
public:
	...
	{
#ifdef DARKINTERVAL
		SetModelName(MAKE_STRING("models/_weapons/pr_glueblob.mdl"));
#else
...
IMotionEvent::simresult_e CGravControllerPoint::Simulate(IPhysicsMotionController *pController, IPhysicsObject *pObject, float deltaTime, Vector &linear, AngularImpulse &angular)
{
	...
	Vector world;
#ifdef DARKINTERVAL
	pObject->GetPosition(&world, NULL);
#else
...
#ifdef DARKINTERVAL
	CNetworkVar(Vector, m_targetPosition);
	CNetworkVar(Vector, m_worldPosition);
...
void CWeaponGravityGun::EffectUpdate(void)
{
	...
	if (pObject)
	{
		...
		// If I'm looking more than 5 degrees away from the glue point, then give up
...
#ifdef DARKINTERVAL
		if (DotProduct(pelletDir, forward) < 0.996)	// 0.996 ~= cos(5deg)
#else
		if ( DotProduct(pelletDir, forward) < 0.939 )	// 0.939 ~= cos(20deg)
#endif
		{
			// lose attach for 2 seconds if you're too far away
			m_glueTime = CURTIME + 1;
		}

		if (m_pelletHeld >= 0 && CURTIME > m_glueTime)
		{
...
#ifdef DARKINTERVAL
	m_targetPosition = m_gravCallback.m_targetPosition;
	m_worldPosition = m_gravCallback.m_worldPosition;
#endif
}
...
{
	m_flNextSecondaryAttack = CURTIME + 0.1;
	if (m_active)
	{
		EffectDestroy();
		SoundDestroy();
