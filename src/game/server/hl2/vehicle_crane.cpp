...
#ifdef DARKINTERVAL
	// Make secondary rope between crane arm and crane top
	int iIndex2a = LookupAttachment("armcable_base");
	m_hRope2 = CRopeKeyframe::Create(this, this, 1, iIndex2a);
	if (m_hRope2)
	{
		m_hRope2->m_Width = 3;
		m_hRope2->m_nSegments = ROPE_MAX_SEGMENTS / 2;
		m_hRope2->EnableWind(false);
		m_hRope2->SetupHangDistance(0);
		m_hRope2->m_RopeLength = (m_hCraneMagnet->GetAbsOrigin() - m_hCraneTip->GetAbsOrigin()).Length() * 1.1;
	}
#endif
...
void CPropCrane::DriveCrane( int iDriverButtons, int iButtonsPressed, float flNPCSteering )
{
	...
	if ( iButtonsPressed & (IN_ATTACK | IN_ATTACK2) )
	{
		...
		else if ( !m_bDropping && m_flNextDropAllowedTime < CURTIME )
		{
			TurnMagnetOn();
...
void CPropCrane::RunCraneMovement( float flTime )
{
	...
	if ( m_bDropping )
	{
		...
		if ( m_hCraneMagnet->HasHitSomething() )
		{
			...
			m_bDropping = false;
			m_flNextDropAllowedTime = CURTIME + 3.0;
			m_flSlowRaiseTime = CURTIME;

			m_ServerVehicle.PlaySound( VS_MISC2 );
		}
	}
	else if ( (m_flSlowRaiseTime + CRANE_SLOWRAISE_TIME) > CURTIME )
	{
		float flDelta = (CURTIME - m_flSlowRaiseTime);
...
#ifdef DARKINTERVAL
	if ( (m_flNextDangerSoundTime < CURTIME) && (m_hCraneMagnet->GetTotalMassAttachedObjects() > 0) )
	{
		...
		m_flNextDangerSoundTime = CURTIME + 0.3;
	}
#endif // DARKINTERVAL	

	// Play creak sounds on the magnet if there's heavy weight on it
	if ( (m_flNextCreakSound < CURTIME) && (m_hCraneMagnet->GetTotalMassAttachedObjects() > 100) )
	{
		...
		if ( RandomFloat(0,1) > (0.95 - (0.1 * flSpeedPercentage)) )
		{
			...
			m_flNextCreakSound = CURTIME + 5.0;
		}
	}
}
