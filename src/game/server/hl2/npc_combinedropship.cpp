...
#ifdef DARKINTERVAL
#include "ai_behavior_rappel.h" // for crate-less rappel deploy
#endif
...
#ifdef DARKINTERVAL
#include "prop_scalable.h" // for the forcefield bubble
#endif
#include "props.h"
#include "scripted.h"
...
enum CRATE_TYPES 
{
	...
	CRATE_NONE,
#ifdef DARKINTERVAL
	CRATE_APC_FAKE, // scenic purpose, just a prop dynamic without needing to have a real APC entity
#endif
};
...
#ifdef DARKINTERVAL
ConVar npc_dropship_use_forcefield("npc_dropship_use_forcefield", "0", FCVAR_NONE, "Dropship will create a forcefield bubble upon landing that holds until all its soldiers are deployed");
#endif
//=====================================
// Animation Events
//=====================================
...
#ifdef DARKINTERVAL
Activity ACT_DROPSHIP_FLY_TO_DESCEND;	// DI add
Activity ACT_DROPSHIP_OPEN_DOOR;		// DI add
#endif
...
#ifdef DARKINTERVAL
	// Protective forcefield bubble for deployed troops
	void	CreateForcefieldBubble(float maxExpandRadius);
	void	InputCreateForcefieldBubble(inputdata_t &data);
	bool	m_createdForcefield_bool;
#endif
	// Input handlers.
...
#ifdef DARKINTERVAL
	void	InputDropCrabSynth(inputdata_t &inputdata);
	void	InputDropCurrentCarriedObject(inputdata_t &inputdata);
#endif
...
void	CNPC_CombineDropship::PopulatePoseParameters( void )
{
	if (!m_sbStaticPoseParamsLoaded)
	{
		...
		else // practically, this means there won't be a weapon to poseparam. But oh well.
		{
...
#ifdef DARKINTERVAL
			SF_ENVEXPLOSION_NODAMAGE | SF_ENVEXPLOSION_NOSPARKS | SF_ENVEXPLOSION_NOSMOKE, 0 );
#else
...
void CCombineDropshipContainer::AddSmokeTrail( const Vector &vecPos )
{
	...
	ExplosionCreate( vecPos, vec3_angle, this, 1000, 500.0f, SF_ENVEXPLOSION_NODAMAGE | 
		SF_ENVEXPLOSION_NOSPARKS | SF_ENVEXPLOSION_NOSMOKE, 0 );
#else
	ExplosionCreate(vecPos, vec3_angle, this, 1000, 500.0f, SF_ENVEXPLOSION_NODAMAGE |
...
#ifdef DARKINTERVAL
LINK_ENTITY_TO_CLASS( npc_dropship, CNPC_CombineDropship );
#endif
...
{
#ifdef DARKINTERVAL
	// Collapse forcefield barrier, if have one
	CBaseEntity *barrier = gEntList.FindEntityByName(NULL, AllocPooledString(UTIL_VarArgs("%s_bubble", STRING(GetEntityName()))));
	if (barrier != NULL)
	{
		barrier->SetThink(&CBaseEntity::SUB_Remove);
		barrier->SetNextThink(CURTIME + 1.0f);
	}
#endif
	if ( m_hContainer )
	{
...
{
	Precache( );
#ifdef DARKINTERVAL // DI has both dropships, "Slave-I" type and retail "crustacean" type
	...
	case CRATE_SOLDIER:
#ifdef DARKINTERVAL
		if(Q_strcmp(GetModelName().ToCStr(), "models/combine_dropship.mdl") == 0)
#endif
		...
		{
			m_iAttachmentTroopDeploy = LookupAttachment("deploy_landpoint");
			m_iAttachmentDeployStart = LookupAttachment("Deploy_Start");
		}
#endif
		break;
	...
	case CRATE_APC:
		{
			...
			{
#ifdef DARKINTERVAL
				Warning("Dropship %s is unable to find APC %s\n", GetDebugName(), STRING( m_iszAPCVehicleName ) ); 		
#else
			...
			{
#ifdef DARKINTERVAL
				pPhysicsObject->SetShadow(1e4, 1e4, true, true); // TODO: this fixes aberrant physics behavior from save/reloading carried APCs... but how does it fix it?
#else
				pPhysicsObject->SetShadow(1e4, 1e4, false, false);
#endif
			}
	...
	case CRATE_JEEP:
		...
		break;
#ifdef DARKINTERVAL
	case CRATE_CRABSYNTH:

		if (m_iszAPCVehicleName == NULL_STRING) // if no special crabsynth specified, create a dummy.
		{
			m_hContainer = (CBaseAnimating*)CreateEntityByName("npc_crabsynth");
		}
		else
		{
			m_hContainer = (CBaseAnimating*)gEntList.FindEntityByName(NULL, m_iszAPCVehicleName);
			if (!m_hContainer)
			{
				Warning("Dropship %s is unable to find Crab Synth %s\n", GetDebugName(), STRING(m_iszAPCVehicleName));
				break;
			}
		}
		m_hContainer->SetAbsOrigin(GetAbsOrigin() + Vector(0, 0, -112));
		m_hContainer->SetAbsAngles(GetAbsAngles());
		m_hContainer->SetParent(this, 0);
		m_hContainer->SetOwnerEntity(this);
		m_hContainer->Spawn();
		m_hContainer->ApplyAbsVelocityImpulse(GetAbsVelocity() * 0.5);
	//	m_hContainer->SetAbsOrigin(GetAbsOrigin() + Vector(0, 0, 100));
		break;
	...
	case CRATE_APC_FAKE:
		...
		if (m_hContainer)
		{
			m_hContainer->SetModel("models/combine_apc_static.mdl");
...
#ifdef DARKINTERVAL
	if (HasSpawnFlags(SF_NOROTORWASH)) m_bSuppressSound = true; // handy for dropships in "ambush" without giving themselves away w/ sound

	m_createdForcefield_bool = false;
#endif
}
...
{
	// Models
#ifdef DARKINTERVAL // because DI allows multiple dropship models
	if (!GetModelName())
	{
		SetModelName(MAKE_STRING("models/_Monsters/Combine/dropship.mdl"));
	}
...
#ifdef DARKINTERVAL // for the forcefield bubble
	UTIL_PrecacheOther("prop_scalable");
	PrecacheModel("models/props_combine/sphere.mdl");
#endif
	BaseClass::Precache();
...
void CNPC_CombineDropship::Flight( void )
{
	...
	// Slowly lerp the orientation and position of the cargo into place...
...
void CNPC_CombineDropship::UpdateFacingDirection( void )
{
	if ( GetEnemy() )
	{
		if ( !IsCrashing() && m_flLastSeen + 5 > CURTIME )
		{
...
#ifdef DARKINTERVAL
	if ( !m_hContainer && !bHover && Q_strcmp( GetModelName().ToCStr(), "models/combine_dropship.mdl") == 0 )
#else
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_CombineDropship::InputDropCurrentCarriedObject(inputdata_t &inputdata)
{	
	if (!m_hContainer )
	{
		Warning("npc_combinedropship %s was told to DropCurrentCarriedObject, but isn't carrying anything!\n", STRING(GetEntityName()));
		return;
	}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_CombineDropship::InputDropCrabSynth(inputdata_t &inputdata)
{
	if (!m_hContainer || !FClassnameIs(m_hContainer, "npc_crabsynth"))
	{
		Warning("npc_combinedropship %s was told to drop its Crab Synth, but isn't carrying one!\n", STRING(GetEntityName()));
		return;
	}
	...
	PhysDisableEntityCollisions(this, m_hContainer);
	m_hContainer->SetParent(NULL, 0);
	m_hContainer->SetOwnerEntity(NULL);
	m_hContainer->SetAbsAngles(angles);
	m_hContainer->SetMoveType(MOVETYPE_STEP);
	m_hContainer->SetGravity(GetGravity());
	m_hContainer->SetAbsVelocity(-Vector(0, 0, GetGravity()));
...
{	
	...
	case LANDING_HOVER_LEVEL_OUT:
		{
#ifdef DARKINTERVAL		// DI add, for the "Slave-I" type
			if (IsActivityFinished() && GetActivity() != ACT_DROPSHIP_FLY_TO_DESCEND)
			{
				SetActivity((Activity)ACT_DROPSHIP_FLY_TO_DESCEND);
			}
#endif
...
#endif	
#ifdef DARKINTERVAL
			if( (IsHovering() || m_use_rappel_deploy_bool) && m_hLandTarget != NULL )
#else
...
#define ALTITUDE_CAP	512.0f
			...
			else
			{
				...
				QAngle angles = GetLocalAngles();
				angles.x = m_existPitch + ( sin( CURTIME * 3.5f ) * DROPSHIP_MAX_LAND_TILT );
				angles.z = m_existRoll + ( sin( CURTIME * 3.75f ) * DROPSHIP_MAX_LAND_TILT );
				SetLocalAngles( angles );
			}
...
#ifdef DARKINTERVAL
			if ( !bStillOrienting && ((flAltitude <= 0.5f) || (m_iCrateType == CRATE_APC) || (m_iCrateType == CRATE_CRABSYNTH)) )
#else
	...
	case LANDING_HOVER_TOUCHDOWN:
		{
			...
			*/
...
#ifdef DARKINTERVAL 
			if ( m_soldiersToDrop ) // doesn't require container, because it can be a rappell deploy
#else
...
#endif
			{
				...
				{
					if( !m_use_rappel_deploy_bool) SetActivity(ACT_DROPSHIP_OPEN_DOOR);
				}
#else
				m_hContainer->SetSequence(m_hContainer->LookupSequence("open_idle"));
#endif
				...
				SpawnTroop();
#ifdef DARKINTERVAL
				// create protective bubble
				if (npc_dropship_use_forcefield.GetBool())
				{
					if (!m_createdForcefield_bool)
					{
						CreateForcefieldBubble(450.0f);
					}
				}
#endif
			}
			else
			{
				...
				m_flTimeTakeOff = CURTIME + flHoverTime;
			}
		}
		break;
			...
			if ( !m_flTimeTakeOff )
			{
				...
				if ( !m_hLastTroopToLeave || !m_hLastTroopToLeave->IsAlive() )
				{
					...
					else
					{
						...
						m_flTimeTakeOff = CURTIME + 0.5;
					}
				}
			}
			else
			{
				if( CURTIME > m_flTimeTakeOff )
				{
#ifdef DARKINTERVAL
					...
					if (barrier != NULL)
					{
						...
						barrier->AcceptInput("SetScaleX", this, this, var, 0);
						barrier->AcceptInput("SetScaleY", this, this, var, 0);
						barrier->AcceptInput("SetScaleZ", this, this, var, 0);
				...
				else if ( (m_flTimeTakeOff - CURTIME) < 0.5f )
				{
	...
	case LANDING_LIFTOFF:
		{
			...
			{
				if( !m_use_rappel_deploy_bool )	SetActivity(ACT_DROPSHIP_LIFTOFF);
			}
#endif
		}
		break;
...
{
#ifdef DARKINTERVAL
	if ( !m_hContainer && Q_strcmp( GetModelName().ToCStr(), "models/combine_dropship.mdl") == 0 )
#else
	if ( !m_hContainer )
#endif
	{
	...
	if ( m_flNextTroopSpawnAttempt > CURTIME )
		return;
	...
	if ( tr.m_pEnt )
	{
		...
		m_flNextTroopSpawnAttempt = CURTIME + 1;
		return;
	}
	...
	if (m_hContainer)
		...
		GetAttachment(m_iAttachmentDeployStart, vecSpawnOrigin, vecSpawnAngles);
#else
	m_hContainer->GetAttachment(m_iAttachmentDeployStart, vecSpawnOrigin, vecSpawnAngles);
#endif
...
#ifdef DARKINTERVAL
	CAI_RappelBehavior *rappelBehavior; // to check if the template NPC is able
	if (m_use_rappel_deploy_bool) // rappeling soldiers are deployed from the holes on the bottom side of the dropship.
	{ // get their individual positions based on the model-specific attachments (only present in DI dropship)
		
		// keep it steady so all troops end up on the ground without oscillation
		if (m_hLandTarget != NULL) vecSpawnOrigin.z = m_hLandTarget.Get()->GetAbsOrigin().z - 64;
		switch (m_iCurrentTroopExiting)
		{
		case 1:
			vecSpawnOrigin.x += 40;
			vecSpawnOrigin.y += 40;
	...
	if (m_use_rappel_deploy_bool)
	{
		if (pNPC->GetBehavior(&rappelBehavior))
		{
			pNPC->KeyValue("waitingtorappel", "1"); // waiting to rappel
			pNPC->KeyValue("OnRappelTouchdown", UTIL_VarArgs("%s,NPCFinishDustoff,%s,0,-1", STRING(GetEntityName()), STRING(pNPC->GetEntityName()))); // each soldier will signal the dropship that they finished rappeling
		}
		else
		{
			pNPC->KeyValue("OnSpawned", UTIL_VarArgs("%s,NPCFinishDustoff,%s,0,-1", STRING(GetEntityName()), STRING(pNPC->GetEntityName()))); // non-rappeling NPCs that drop out of holes, just tell the dropship to spawn next one
		}
	}
#endif
	DispatchSpawn( pNPC );
	...
	pNPC->Activate();
#ifdef DARKINTERVAL
	if (m_use_rappel_deploy_bool && pNPC->GetBehavior(&rappelBehavior)) pNPC->BeginRappel();
...
#ifdef DARKINTERVAL
//------------------------------------------------------------------------------
// Purpose : Spawn protective forcefield barrier during troop deployment
//------------------------------------------------------------------------------
void CNPC_CombineDropship::CreateForcefieldBubble(float maxExpandRadius)
{
	if (!m_createdForcefield_bool)
	{
		Msg("Creating barrier\n");

		// Create the visuals (non-solid half-sphere)
		CPropScalable *bubble = (CPropScalable*)CreateEntityByName("prop_scalable");
		if (bubble)
		{
			bubble->SetName(AllocPooledString(UTIL_VarArgs("%s_bubble", STRING(GetEntityName()))));
			bubble->SetModel("models/props_combine/sphere.mdl");
			bubble->SetAbsOrigin(GetAbsOrigin());
		//	bubble->SetLocalOrigin(Vector(0, -64, 0));
			bubble->SetAbsVelocity(GetAbsVelocity());
			bubble->SetLocalAngularVelocity(GetLocalAngularVelocity());
			bubble->SetParent(this);
			bubble->SetOwnerEntity(this);
			bubble->Spawn();

			variant_t varX;
			varX.SetVector3D(Vector(maxExpandRadius, 0.75, 0));
			variant_t varZ;
			varZ.SetVector3D(Vector(maxExpandRadius * 0.65, 0.75, 0));
			bubble->AcceptInput("SetScaleX", this, this, varX, 0);
			bubble->AcceptInput("SetScaleY", this, this, varX, 0);
			bubble->AcceptInput("SetScaleZ", this, this, varZ, 0);

			// create the collision
			bubble->SetCollisionBounds(-Vector(maxExpandRadius, maxExpandRadius, maxExpandRadius * 0.65), Vector(maxExpandRadius, maxExpandRadius, maxExpandRadius * 0.65));
			objectparams_t params = g_PhysDefaultObjectParams;
			params.pGameData = static_cast<void *>(this);
			IPhysicsObject *collision = physenv->CreateSphereObject(maxExpandRadius, 0, bubble->GetAbsOrigin(), bubble->GetAbsAngles(), &params, true);
			if (collision)
			{
				bubble->VPhysicsSetObject(collision);
				bubble->SetMoveType(MOVETYPE_NONE);
				bubble->SetSolid(SOLID_VPHYSICS);
				bubble->SetCollisionGroup(COLLISION_GROUP_PUSHAWAY);
				collision->Wake();
			}


			m_createdForcefield_bool = true;
		}

	}
}

void CNPC_CombineDropship::InputCreateForcefieldBubble(inputdata_t &data)
{
	CreateForcefieldBubble(data.value.Float());
}
#endif // DARKINTERVAL
//-----------------------------------------------------------------------------
...
void CNPC_CombineDropship::GatherEnemyConditions( CBaseEntity *pEnemy )
{
	...
	{
		if ( CURTIME - GetEnemyLastTimeSeen() >= 3.0f )
		{
			MarkEnemyAsEluded();
		}
	}
...
void CNPC_CombineDropship::DoCombatStuff( void )
{
	...
	if ( m_bDropMines )
	{
		...
		case DROP_IDLE:
			{
				...
				m_flDropDelay = CURTIME + DROPSHIP_TIME_BETWEEN_MINES;
		...
		case DROP_NEXT:
			{
				if ( CURTIME > m_flDropDelay )		// time to drop next mine?
				{
					DropMine();
					m_flDropDelay = CURTIME + DROPSHIP_TIME_BETWEEN_MINES;
...
bool CNPC_CombineDropship::FireCannonRound( void )
{
	...
	{
		m_flTimeNextAttack = CURTIME + 0.1;
		return false;
	}
	...
	if ( m_iBurstRounds <= 0 )
	{
		...
		m_flTimeNextAttack = CURTIME + (m_iBurstRounds * 0.1);
		return false;
	}

	// HACK: Return true so the fire sound isn't stopped
	if ( m_flTimeNextAttack > CURTIME )
		return true;
	...
	m_flTimeNextAttack = CURTIME + 0.05;
...
#ifdef DARKINTERVAL
	DEFINE_KEYFIELD(m_use_rappel_deploy_bool, FIELD_BOOLEAN, "RappelDeploy"),
#endif
...
#ifdef DARKINTERVAL
	DEFINE_FIELD(m_createdForcefield_bool, FIELD_BOOLEAN),
#endif
...
#ifdef DARKINTERVAL
	DEFINE_INPUTFUNC(FIELD_VOID, "DropCurrentCarriedObject", InputDropCurrentCarriedObject),
	DEFINE_INPUTFUNC(FIELD_VOID, "DropCrabSynth", InputDropCrabSynth),
#endif
...
#ifdef DARKINTERVAL
	DEFINE_INPUTFUNC(FIELD_FLOAT, "CreateForcefieldBubble", InputCreateForcefieldBubble),
#endif
...
AI_BEGIN_CUSTOM_NPC( npc_combinedropship, CNPC_CombineDropship )
	...
	DECLARE_ACTIVITY(ACT_DROPSHIP_FLY_TO_DESCEND);
