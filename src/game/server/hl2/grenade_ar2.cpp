...
#ifdef DARKINTERVAL
//	DEFINE_FIELD(m_iSpriteTexture, FIELD_INTEGER), // for the unused shockwave
#endif
	// Function pointers
...
void CGrenadeAR2::GrenadeAR2Think( void )
{
	...
	if (!m_bIsLive)
	{
		...
		if (m_fSpawnTime + MAX_AR2_NO_COLLIDE_TIME < CURTIME)
		{
			m_bIsLive  = true;
		}
	}
...
void CGrenadeAR2::Detonate(void)
{
	...
	te->Explosion(filter, 0.0,
		...
		m_DmgRadius,
		m_flDamage);
#else // todo - I don't remember why it needs 3 explosions. To make them brighter? 
	te->Explosion( filter, 0.0,
		&GetAbsOrigin(), 
		g_sModelIndexFireball,
		3.0, 
	...
	te->Explosion(filter, 0.0,
		...
		3.0,
		15,
		TE_EXPLFLAG_NOFIREBALL|TE_EXPLFLAG_NODLIGHTS|TE_EXPLFLAG_NOSOUND,
		m_DmgRadius * 0.6,
		m_flDamage * 0.3);
