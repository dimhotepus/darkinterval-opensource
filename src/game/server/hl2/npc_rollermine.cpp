...
#ifdef DARKINTERVAL
ConVar	sk_rollermine_explosive("sk_rollermine_explosive", "1"); // the 2003 mine: blows up if thrown against something, blows up the car if too many attached at once
#endif
enum
{
...
#ifdef DARKINTERVAL
#define	ROLLERMINE_OPEN_THRESHOLD	512

#define ROLLERMINE_VEHICLE_OPEN_THRESHOLD	800
#define ROLLERMINE_VEHICLE_HOP_THRESHOLD	600
#else
...
public:
	...
	bool	IsShocking() { return CURTIME < m_flShockTime ? true : false; }
	...
	{ 
		...
		if ( m_bHackedByAlyx && ( m_flPowerDownDetonateTime > 0.0f && m_flPowerDownDetonateTime <= CURTIME ) )
	...
	{ 
		return CURTIME > m_flGoIdleTime ? true : false;
	}
...
protected:
	...
	bool	IsActive() { return m_flActiveTime > CURTIME ? false : true; }
...
//				Assert(0);
			...
			flTorqueFactor = 1 + (CURTIME - m_flChargeTime) * 2;
			...
			if ( !EnemyInVehicle() )
			{
				...
				{
					if( CURTIME - m_flChargeTime > 1.0 && flTorqueFactor > 1 &&  flDot < 0.0 )
					{
						if( m_bIsOpen )
						{
							Close();
	...
	case TASK_ROLLERMINE_POWERDOWN:
		{
			if ( m_flPowerDownTime <= CURTIME )
			{
				m_flNextHop = CURTIME;
				m_flPowerDownTime = CURTIME + RandomFloat( 0.3, 0.9 );
			...
			if ( m_flPowerDownDetonateTime <= CURTIME )
			{
...
{
#ifdef DARKINTERVAL
	if ( sk_rollermine_explosive.GetBool() )
	{
...
void CNPC_RollerMine::ShockTouch( CBaseEntity *pOther )
{
	...
	if ( m_bHeld || m_hVehicleStuckTo || CURTIME < m_flShockTime )
		return;
	...
	// Stun the roller
	m_flActiveTime = CURTIME + GetStunDelay();
...
void CNPC_RollerMine::AnnounceArrivalToOthers( CBaseEntity *pOther )
{
	...
	{
		aRollersOnVehicle[i]->PreventUnstickUntil( CURTIME + 1 );
	}

	// See if we've got enough rollers on the vehicle to start being mean
#ifdef DARKINTERVAL
	if ( iRollers >= ROLLERMINE_REQUIRED_TO_EXPLODE_VEHICLE && sk_rollermine_explosive.GetBool() )
	{
		// Alert the others
...
void CNPC_RollerMine::InputConstraintBroken( inputdata_t &inputdata )
{
	...
	if ( m_flPreventUnstickUntil > CURTIME )
		return;
...
{
	m_bPowerDown = true;
	m_flPowerDownTime = CURTIME + RandomFloat( 0.1, 0.5 );
...
int CNPC_RollerMine::OnTakeDamage( const CTakeDamageInfo &info )
{
	...
	if ( info.GetDamageType() & (DMG_BURN|DMG_BLAST) )
	{
		...
		{
			SetThink( &CNPC_RollerMine::PreDetonate );
			SetNextThink( CURTIME + random->RandomFloat( 0.1f, 0.5f ) );
		}
		else
		{
			// dazed
...
void CNPC_RollerMine::Hop( float height )
{
	...
	if ( GetMoveType() == MOVETYPE_VPHYSICS )
	{
		...
		m_flNextHop = CURTIME + ROLLERMINE_HOP_DELAY;
	}
}
