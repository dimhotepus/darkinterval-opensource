...
//
// Purpose: 
// DI changed the class name from CPrototSniper to CNPC_CombineSniper, 
// and changed the filename of the cpp.
// $NoKeywords: $
//=============================================================================//
#include "cbase.h"
...
//=========================================================
//=========================================================
class CNPC_CombineSniper : public CAI_BaseNPC
{
	DECLARE_CLASS( CNPC_CombineSniper, CAI_BaseNPC );

public:
	CNPC_CombineSniper( void );
	void	Precache( void );
	void	Spawn( void );
...
CNPC_CombineSniper::CNPC_CombineSniper( void ) : m_flKeyfieldPaintTime(SNIPER_DEFAULT_PAINT_ENEMY_TIME), 
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_CombineSniper::QuerySeeEntity( CBaseEntity *pEntity, bool bOnlyHateOrFearIfNPC )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_CombineSniper::FInViewCone ( CBaseEntity *pEntity )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_CombineSniper::LaserOff( void )
{
	if( m_pBeam )
	{
		UTIL_Remove( m_pBeam);
...
void CNPC_CombineSniper::LaserOn( const Vector &vecTarget, const Vector &vecDeviance )
{
	if (!m_pBeam)
	{
	...
	// beam movement.
	SetNextThink( CURTIME + 0.02 );
}

//-----------------------------------------------------------------------------
// Crikey!
//-----------------------------------------------------------------------------
float CNPC_CombineSniper::GetPositionParameter( float flTime, bool fLinear )
{
	float flElapsedTime;
	float flTimeParameter;

	flElapsedTime = flTime - (GetWaitFinishTime() - CURTIME);
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_CombineSniper::GetPaintAim( const Vector &vecStart, const Vector &vecGoal, float flParameter, Vector *pProgress )
{
#if 0
	Vector vecDelta;
...
//-----------------------------------------------------------------------------
void CNPC_CombineSniper::PaintTarget( const Vector &vecTarget, float flPaintTime )
{
	Vector vecCurrentDir;
	Vector vecStart;
...
#define THRESHOLD 0.8f
	...
	// mult by P
	vecCurrentDir.x += flNoiseScale * ( sin( 3 * M_PI * CURTIME ) * 0.0006 );
	vecCurrentDir.y += flNoiseScale * ( sin( 2 * M_PI * CURTIME + 0.5 * M_PI ) * 0.0006 );
	vecCurrentDir.z += flNoiseScale * ( sin( 1.5 * M_PI * CURTIME + M_PI ) * 0.0006 );
#endif
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_CombineSniper::IsPlayerAllySniper()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_CombineSniper::InputSetDecoyRadius( inputdata_t &inputdata )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_CombineSniper::OnScheduleChange( void )
{
	LaserOff();
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_CombineSniper::KeyValue( const char *szKeyName, const char *szValue )
{
...
#ifdef DARKINTERVAL // convenient redundant classnames
LINK_ENTITY_TO_CLASS( npc_combine_sniper, CNPC_CombineSniper);
#endif
LINK_ENTITY_TO_CLASS( npc_sniper, CNPC_CombineSniper );
LINK_ENTITY_TO_CLASS( proto_sniper, CNPC_CombineSniper );
...
//
//-----------------------------------------------------------------------------
void CNPC_CombineSniper::Precache( void )
{
...
//
//-----------------------------------------------------------------------------
void CNPC_CombineSniper::Spawn( void )
{
	Precache();
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_CombineSniper::SetSweepTarget( const char *pszTarget )
{
	CBaseEntity *pTarget;
...
//-----------------------------------------------------------------------------
void CNPC_CombineSniper::InputSweepTarget( inputdata_t &inputdata )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_CombineSniper::InputSweepTargetHighestPriority( inputdata_t &inputdata )
{
	SetSweepTarget( inputdata.value.String() );
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_CombineSniper::ClearTargetGroup( void )
{
	int i;
...
//-----------------------------------------------------------------------------
void CNPC_CombineSniper::InputSweepGroupRandomly( inputdata_t &inputdata )
{
	ClearTargetGroup();
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_CombineSniper::InputStopSweeping( inputdata_t &inputdata )
{
	m_hSweepTarget = NULL;
...
// Input  : &inputdata - 
//-----------------------------------------------------------------------------
void CNPC_CombineSniper::InputProtectTarget( inputdata_t &inputdata )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_CombineSniper::InputSetPaintInterval( inputdata_t &inputdata )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_CombineSniper::InputSetPaintIntervalVariance( inputdata_t &inputdata )
{
...
// Output : int
//-----------------------------------------------------------------------------
int CNPC_CombineSniper::IRelationPriority( CBaseEntity *pTarget )
{
...
// Output : 
//-----------------------------------------------------------------------------
Class_T	CNPC_CombineSniper::Classify( void )
{
	if( m_fEnabled )
	{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
Vector CNPC_CombineSniper::GetBulletOrigin( void )
{
	if( m_spawnflags & SF_SNIPER_HIDDEN )
	{
		return GetAbsOrigin();
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_CombineSniper::ClearOldDecoys( void )
{
#if 0
	int i;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_CombineSniper::HasOldDecoy( CBaseEntity *pDecoy )
{
#if 0
	int i;
...
//-----------------------------------------------------------------------------
void CNPC_CombineSniper::AddOldDecoy( CBaseEntity *pDecoy )
{
#if 0
...
int CNPC_CombineSniper::OnTakeDamage_Alive( const CTakeDamageInfo &info )
{
	if( !m_fEnabled )
	{
...
// Output : 
//-----------------------------------------------------------------------------
void CNPC_CombineSniper::Event_Killed( const CTakeDamageInfo &info )
{
...
//---------------------------------------------------------
//---------------------------------------------------------
void CNPC_CombineSniper::Event_KilledOther( CBaseEntity *pVictim, const CTakeDamageInfo &info )
{
...
//---------------------------------------------------------
//---------------------------------------------------------
void CNPC_CombineSniper::UpdateOnRemove( void )
{
	LaserOff();
...
//---------------------------------------------------------
//---------------------------------------------------------
int CNPC_CombineSniper::SelectSchedule ( void )
{
#ifdef DARKINTERVAL
...
//---------------------------------------------------------
//---------------------------------------------------------
int CNPC_CombineSniper::GetSoundInterests( void )
{
...
//---------------------------------------------------------
//---------------------------------------------------------
void CNPC_CombineSniper::OnListened()
{
...
//---------------------------------------------------------
//---------------------------------------------------------
bool CNPC_CombineSniper::FCanCheckAttacks ( void )
{
	return true;
}
...
//---------------------------------------------------------
//---------------------------------------------------------
bool CNPC_CombineSniper::FindDecoyObject( void )
{
...
bool CNPC_CombineSniper::ShouldSnapShot( void )
{
...
//---------------------------------------------------------
//---------------------------------------------------------
bool CNPC_CombineSniper::VerifyShot( CBaseEntity *pTarget )
{
	trace_t tr;
...
//---------------------------------------------------------
//---------------------------------------------------------
int CNPC_CombineSniper::RangeAttack1Conditions ( float flDot, float flDist )
{
	float fFrustration;
	fFrustration = CURTIME - m_flFrustration;
...
//---------------------------------------------------------
//---------------------------------------------------------
int CNPC_CombineSniper::TranslateSchedule( int scheduleType ) 
{
	switch( scheduleType )
	{
...
//---------------------------------------------------------
//---------------------------------------------------------
void CNPC_CombineSniper::ScopeGlint()
{
	CEffectData data;
...
//---------------------------------------------------------
bool CNPC_CombineSniper::FireBullet( const Vector &vecTarget, bool bDirectShot )
{
...
//---------------------------------------------------------
//---------------------------------------------------------
float CNPC_CombineSniper::GetBulletSpeed()
{
#ifdef DARKINTERVAL
	float speed = 6000.0f;
#else
...
//---------------------------------------------------------
//---------------------------------------------------------
void CNPC_CombineSniper::StartTask( const Task_t *pTask )
{
	switch( pTask->iTask )
	{
	...
	case TASK_SNIPER_PAINT_ENEMY:
		...
		else
		{
			if( GetEnemy()->IsPlayer() )
			{
				...
				if( CURTIME - m_flTimeLastAttackedPlayer <= SNIPER_FASTER_ATTACK_PERIOD )
				{
...
//---------------------------------------------------------
//---------------------------------------------------------
void CNPC_CombineSniper::RunTask( const Task_t *pTask )
{
	switch( pTask->iTask )
	{
	case TASK_SNIPER_PLAYER_DEAD:
	...
	case TASK_RANGE_ATTACK1:
		...
		if( FireBullet( LeadTarget( GetEnemy() ), true ) )
		{
			...
			{
				m_flTimeLastAttackedPlayer = CURTIME;
			}
...
//-----------------------------------------------------------------------------
int CNPC_CombineSniper::Restore( IRestore &restore )
{
	ClearOldDecoys();
...
//
//-----------------------------------------------------------------------------
float CNPC_CombineSniper::MaxYawSpeed( void )
{
	return 60;
}
...
//---------------------------------------------------------
//---------------------------------------------------------
void CNPC_CombineSniper::PrescheduleThink( void )
{
	...
	if( GetEnemy() != NULL )
	{
		if( CURTIME - GetEnemies()->LastTimeSeen( GetEnemy() ) > 30 )
		{
...
//---------------------------------------------------------
//---------------------------------------------------------
Vector CNPC_CombineSniper::EyePosition( void )
{
	if( m_spawnflags & SF_SNIPER_HIDDEN )
	{
...
//---------------------------------------------------------
//---------------------------------------------------------
Vector CNPC_CombineSniper::DesiredBodyTarget( CBaseEntity *pTarget )
{
	// By default, aim for the center
	Vector vecTarget = pTarget->WorldSpaceCenter();

	float flTimeSinceLastMiss = CURTIME - m_flTimeLastShotMissed;
...
//---------------------------------------------------------
//---------------------------------------------------------
Vector CNPC_CombineSniper::LeadTarget( CBaseEntity *pTarget )
{
	float targetTime;
	float targetDist;
...
//---------------------------------------------------------
CBaseEntity *CNPC_CombineSniper::PickDeadPlayerTarget()
{
...
//---------------------------------------------------------
//---------------------------------------------------------
void CNPC_CombineSniper::InputEnableSniper( inputdata_t &inputdata )
{
...
//---------------------------------------------------------
//---------------------------------------------------------
void CNPC_CombineSniper::InputDisableSniper( inputdata_t &inputdata )
{
...
//---------------------------------------------------------
//---------------------------------------------------------
bool CNPC_CombineSniper::FindFrustratedShot( float flNoise )
{
	Vector vecForward;
	Vector vecStart;
	Vector vecAimAt;
...
bool CNPC_CombineSniper::FVisible( CBaseEntity *pEntity, int traceMask, CBaseEntity **ppBlocker )
{
	if( m_spawnflags & SF_SNIPER_VIEWCONE )
	{
	...
	if( (pPlayer->GetFlags() & FL_DUCKING) && pPlayer->MuzzleFlashTime() > CURTIME )
	{
...
//-----------------------------------------------------------------------------
int CNPC_CombineSniper::DrawDebugTextOverlays()
{
	int text_offset = 0;
...
//-----------------------------------------------------------------------------
void CNPC_CombineSniper::NotifyShotMissedTarget()
{
	m_flTimeLastShotMissed = CURTIME;
...
// Save/restore
//-----------------------------------------------------------------------------
BEGIN_DATADESC(CNPC_CombineSniper)
...
AI_BEGIN_CUSTOM_NPC( proto_sniper, CNPC_CombineSniper )
...
void CSniperBullet::BulletThink( void )
{
	...
	flInterval = CURTIME - GetLastThink();
	...
	if( tr.fraction != 1.0 )
	{
		...
		else
		{
			...
			if( m_bDirectShot )
			{
				CNPC_CombineSniper *pSniper = dynamic_cast<CNPC_CombineSniper*>(GetOwnerEntity());
				if( pSniper )
				{
...
bool CSniperBullet::Start( const Vector &vecOrigin, const Vector &vecTarget, CBaseEntity *pOwner, bool bDirectShot )
{
	...
	// Set speed;
	CNPC_CombineSniper *pSniper = dynamic_cast<CNPC_CombineSniper*>(pOwner);
	...
	m_SoundTime = CURTIME + flElapsedTime * 0.5;
...
/*
	...
	for( i = 0 ; i < NUM_PENETRATIONS ; i++ )
	{
		...
		// Never let CURTIME get added to the elapsed time!
		m_ImpactTime[ i ] += CURTIME;
