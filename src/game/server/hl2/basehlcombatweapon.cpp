...
BEGIN_DATADESC( CHLMachineGun )
	...
	DEFINE_FIELD( m_shots_fired_int,	FIELD_INTEGER ),
...
void CHLMachineGun::PrimaryAttack( void )
{
...
#ifdef DARKINTERVAL // DI's godmode also lets you have unlimited ammo
...
#ifdef DARKINTERVAL // custom DI hev lines
	...
	pPlayer->SetMuzzleFlashTime( CURTIME + 0.5 );
}
...
#ifndef DARKINTERVAL // DI got rid of CHLSelectFireMachineGun, because no weapon uses it, and it cluttered up the list
...
void CHLSelectFireMachineGun::PrimaryAttack(void)
{
	...
	switch ( m_iFireMode )
	{
		case FIREMODE_FULLAUTO:
			...
			SetWeaponIdleTime(CURTIME + 3.0f);
			break;
		...
		case FIREMODE_3RNDBURST:
			...
			m_flNextPrimaryAttack = CURTIME + GetBurstCycleRate();
			m_flNextSecondaryAttack = CURTIME + GetBurstCycleRate();

			// Pick up the rest of the burst through the think function.
			SetNextThink(CURTIME + GetFireRate());
			break;
	}
...
void CHLSelectFireMachineGun::SecondaryAttack(void)
{
	...
	m_flNextSecondaryAttack = CURTIME + 0.3;
