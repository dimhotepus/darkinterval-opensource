...
void CPropCannon::Think( void )
{
	...
	if ( m_flFlyTime > 0.0f )
	{
		if ( m_flFlyTime - 1.0f <= CURTIME && m_flFlyTime - 0.8f > CURTIME)
		{
...
void CPropCannon::LaunchProjectile( void )
{
	...
	m_flFlyTime = CURTIME + flTravelTime;
	m_vCrashPoint = vOrigin;

	m_flNextAttackTime = CURTIME + g_cannon_reloadtime.GetFloat();
