...
CSkyCamera *CEnvHeadcrabCanister::PlaceCanisterInWorld()
{
	...
	if ( m_iszLaunchPositionName != NULL_STRING )
	{
		...
		else
		{
			...
			m_Shared.InitInWorld( CURTIME, pLaunchPos->GetAbsOrigin(), GetAbsAngles(), 
	...
	else if ( DetectInSkybox() )
	{
		...
		m_Shared.InitInSkybox( CURTIME, m_vecImpactPosition, GetAbsAngles(), vecForward, 
		...
		SetNextThink( CURTIME + m_Shared.GetEnterWorldTime() + TICK_INTERVAL );
	}
	else
	{
		...
		if ( pCamera )
		{
			m_Shared.InitInSkybox( CURTIME, vecStartPosition, vecStartAngles, vecDirection, 
				m_vecImpactPosition, pCamera->m_skyboxData.origin, pCamera->m_skyboxData.scale );
			...
			if ( m_Shared.IsInSkybox() )
			{
				...
				SetNextThink( CURTIME + m_Shared.GetEnterWorldTime() );
			}
			else
			{
				SetThink( &CEnvHeadcrabCanister::HeadcrabCanisterWorldThink );
		...
		{
			m_Shared.InitInWorld( CURTIME, vecStartPosition, vecStartAngles, 
	...
	QAngle vecEndAngles;
	m_Shared.GetPositionAtTime( CURTIME, vecEndPosition, vecEndAngles );
...
void CEnvHeadcrabCanister::HeadcrabCanisterSpawnHeadcrabThink()
{
	...
	if ( m_nHeadcrabCount != 0 )
	{
		...
		SetContextThink( &CEnvHeadcrabCanister::HeadcrabCanisterSpawnHeadcrabThink, CURTIME + flWaitTime, s_pHeadcrabThinkContext );
	}
	else
	{
		SetContextThink( NULL, CURTIME, s_pHeadcrabThinkContext );
	}
}
...
void CEnvHeadcrabCanister::StartSpawningHeadcrabs( float flDelay )
{
	...
	if ( m_nHeadcrabCount != 0 )
	{
		SetContextThink( &CEnvHeadcrabCanister::HeadcrabCanisterSpawnHeadcrabThink, CURTIME + flDelay, s_pHeadcrabThinkContext );
	}
}
...
void CEnvHeadcrabCanister::CanisterFinishedOpening( void )
{
	...
	m_bOpened = true;
	SetContextThink( NULL, CURTIME, s_pOpenThinkContext );
...
void CEnvHeadcrabCanister::WaitForOpenSequenceThink()
{
	...
	{
		SetContextThink( &CEnvHeadcrabCanister::WaitForOpenSequenceThink, CURTIME + 0.01f, s_pOpenThinkContext );
	}
}
...
void CEnvHeadcrabCanister::Detonate( )
{
	...
	if ( HasSpawnFlags( SF_REMOVE_ON_IMPACT ) )
	{
		...
		SetNextThink( CURTIME + ENV_HEADCRABCANISTER_TRAIL_TIME );
	...
	if ( !HasSpawnFlags( SF_NO_IMPACT_EFFECTS ) )
	{
		...
		ExplosionCreate( m_vecImpactPosition, GetAbsAngles(), this, 50.0f, 500.0f, 
			SF_ENVEXPLOSION_NOSPARKS | SF_ENVEXPLOSION_NODAMAGE | SF_ENVEXPLOSION_NOSOUND, 1300.0f );
#else
		ExplosionCreate(m_vecImpactPosition, GetAbsAngles(), this, 50.0f, 500.0f,
...
void CEnvHeadcrabCanister::HeadcrabCanisterSkyboxOnlyThink( void )
{
	...
	SetNextThink( CURTIME + flRefireTime );
}
