...
#include "decals.h"
#include "env_explosion.h" // temp (sjb)
...
#ifdef DARKINTERVAL
ConVar sk_gunship_refire_delay("sk_gunship_refire_delay", "0.2");
#endif
...
#ifdef DARKINTERVAL
	Class_T Classify( void ) { return IsFlaggedEfficient() ? CLASS_NONE : CLASS_COMBINE_GUNSHIP; } // for now
#else
...
private:
	...
	COutputEvent	m_OnStartGroundAttack;
#ifdef DARKINTERVAL
	COutputEvent	m_OnFinishGroundAttack;
	COutputEvent	m_OnCrashed;
#endif
...
#ifdef DARKINTERVAL
	CSoundPatch		*m_pPatrolSound;
	CSoundPatch		*m_pAngrySound;
#endif
...
LINK_ENTITY_TO_CLASS( npc_gunship, CNPC_CombineGunship );
...
void CNPC_CombineGunship::Precache(void)
{
	...
	PrecacheScriptSound("NPC_CombineGunship.CannonSingleFireSound");
#ifdef DARKINTERVAL
	PrecacheScriptSound("NPC_CombineGunship.AngryLoop");
	PrecacheScriptSound("NPC_CombineGunship.PatrolLoop");
#endif
...
#ifdef DARKINTERVAL
	PrecacheParticleSystem("hunter_projectile_explosion_1"); // used for impacts
#endif
#ifndef DARKINTERVAL
...
void CNPC_CombineGunship::Spawn( void )
{
	...
	m_flTimeNextAttack = CURTIME;
	m_flNextSeeEnemySound = CURTIME;
	...
	AddRelationship( "env_flare D_HT 99",	NULL );
	AddRelationship( "rpg_missile D_NU 99", NULL );

	m_flTimeNextPing = CURTIME + 2;
...
void CNPC_CombineGunship::Ping( void )
{
	...
	if (GetEnemy() != NULL)
	{
		if (!HasCondition(COND_SEE_ENEMY) && CURTIME > m_flTimeNextPing)
		{
			EmitSound("NPC_CombineGunship.SearchPing");
			m_flTimeNextPing = CURTIME + 3;
		}
	}
	else
	{
		if (CURTIME > m_flTimeNextPing)
		{
...
bool CNPC_CombineGunship::CheckGroundAttack( void )
{
	...
	if ( m_flNextGroundAttack > CURTIME )
		return false;
...
void CNPC_CombineGunship::StartGroundAttack(void)
{
	...
	m_flGroundAttackTime = CURTIME + 3.0f;
	...
	m_OnStartGroundAttack.FireOutput(this, this, 0);
}
...
void CNPC_CombineGunship::ManageWarningBeam( void )
{
	...
	while ( tr.endpos != m_vecHitPos )
	{
		...
		if ( tr.fraction != 1.0 )
		{
			...
			{
#ifdef DARKINTERVAL
				CTakeDamageInfo	info( this, this, 1.0f, DMG_DISSOLVE );
#else
			...
			if ( tr.startsolid )
			{
				...
				if ( (m_flGroundAttackTime - CURTIME) <= 2.0f )
				{
					g_pEffects->EnergySplash( tr2.endpos + vDir * 8, tr2.plane.normal, true );
				}

				g_pEffects->Sparks( tr2.endpos, 3.0f - (m_flGroundAttackTime-CURTIME), 3.5f - (m_flGroundAttackTime-CURTIME), &tr2.plane.normal );
...
void CNPC_CombineGunship::DoBellyBlastDamage( trace_t &tr, Vector vMins, Vector vMaxs )
{
	...
	for ( int i = 0; i < count; i++ )
	{
		...
		if ( pEntity->GetHealth() <= 0.0f )
		{
			...
			{
				pEntity->GetBaseAnimating()->Dissolve( NULL, CURTIME );
			}
		}
	}
}
...
void CNPC_CombineGunship::StopGroundAttack(bool bDoAttack)
{
	...
	m_flNextGroundAttack = CURTIME + 4.0f;
	m_flTimeNextAttack = CURTIME + 2.0f;
...
#ifdef DARKINTERVAL
	m_OnFinishGroundAttack.FireOutput(this, this, 0);
#endif
}
...
void CNPC_CombineGunship::DoCombat( void )
{
	...
	if ( HasEnemy() )
	{
		...
		{
			if ( GetEnemy() && GetEnemy()->IsPlayer() && m_flNextSeeEnemySound < CURTIME )
			{
				m_flNextSeeEnemySound = CURTIME + 5.0;
			...
			if ( IsTargettingMissile() )
			{
				...
				if ( ( m_flTimeNextAttack > CURTIME ) && ( ( m_flTimeNextAttack - CURTIME ) > GUNSHIP_MISSILE_MAX_RESPONSE_TIME ) )
				{
					m_flTimeNextAttack = CURTIME + GUNSHIP_MISSILE_MAX_RESPONSE_TIME;
	...
	if ( m_bIsFiring )
	{
		...
		if ( ( m_iBurstSize > 0 ) && ( CURTIME > m_flTimeNextAttack ) )
		{
			UpdateEnemyTarget();
			FireCannonRound();
		}
		else if ( m_iBurstSize < 1 )
		{
			...
			if ( IsTargettingMissile() )
			{
				m_flTimeNextAttack = CURTIME + 0.5f;
			}
		}
	}
	else
	{
		...
		if ( m_bIsGroundAttacking )
		{
			...
			if ( m_flGroundAttackTime < CURTIME )
			{
				// Fire!
...
{
	m_flDeltaT = CURTIME - GetLastThink();

	// Are we crashing?
	if ( m_flEndDestructTime && CURTIME > m_flEndDestructTime )
	{
	...
	else if( m_lifeState == LIFE_DYING )
	{
		...
		float flTimeLeft = m_flEndDestructTime - CURTIME;
		if ( flTimeLeft > 1.5 )
		{
		...
		if ( m_flNextGunshipCrashFind && !m_hRagdoll )
		{
			...
			if ( m_flNextGunshipCrashFind < CURTIME )
			{
...
bool CNPC_CombineGunship::FireGun( void )
{
	...
	if ( GetEnemy() && !m_bIsFiring && CURTIME > m_flTimeNextAttack )
	{
...
#ifdef DARKINTERVAL
	m_flTimeNextAttack = CURTIME + sk_gunship_refire_delay.GetFloat();
#else
	m_flTimeNextAttack = CURTIME + 0.05f;
#endif
...
#ifdef DARKINTERVAL		// new sound for cannon - one shot sound per shot!
		EmitSound("NPC_CombineGunship.CannonSingleFireSound");
#endif
...
bool CNPC_CombineGunship::FindNearestGunshipCrash( void )
{
	...
	if ( !pNearest )
	{
		...
		{
			m_flNextGunshipCrashFind = CURTIME + 0.5;
			m_flEndDestructTime = CURTIME + 3.0;
			return true;
		}
...
#ifdef DARKINTERVAL
	m_pPatrolSound		= controller.SoundCreate(filter, entindex(), "NPC_CombineGunship.PatrolLoop");
	m_pAngrySound		= controller.SoundCreate(filter, entindex(), "NPC_CombineGunship.AngryLoop");
#endif
...
#ifdef DARKINTERVAL
	DispatchParticleEffect("hunter_projectile_explosion_1", tr.endpos, vec3_angle);
#endif
...
void CNPC_CombineGunship::StopCannonBurst( void )
{
	...
	m_flTimeNextAttack = CURTIME + flDelay;
	m_iBurstSize = 0;
...
void CNPC_CombineGunship::StopLoopingSounds( void )
{
	...
	if ( m_pAngrySound )
	{
		controller.SoundDestroy(m_pAngrySound);
		m_pAngrySound = NULL;
	}

	if ( m_pPatrolSound )
	{
		controller.SoundDestroy(m_pPatrolSound);
		m_pPatrolSound = NULL;
	}
#endif
...
#ifdef DARKINTERVAL
	DEFINE_SOUNDPATCH(m_pPatrolSound),
	DEFINE_SOUNDPATCH(m_pAngrySound),
#endif
...
#ifdef DARKINTERVAL
	DEFINE_OUTPUT(m_OnStartGroundAttack, "OnStartGroundAttack"),
	DEFINE_OUTPUT(m_OnFinishGroundAttack, "OnFinishGroundAttack"),
#endif
