...
void CNPC_BaseScanner::BlendPhyscannonLaunchSpeed()
{
	...
	if ( HasPhysicsAttacker( SCANNER_SMASH_TIME ) && !IsHeldByPhyscannon( ) )
	{
		...
		float flLerpFactor = (CURTIME - m_flLastPhysicsInfluenceTime) / SCANNER_SMASH_TIME;
...
void CNPC_BaseScanner::MoveExecute_Alive(float flInterval)
{
	...
	{
		return; // No movement when sleeping.
	}
#endif
	...
	if ( pPlayer )
	{
		...
		if ( DotProduct( vecDelta, vecVelocity ) > 0.5f )
		{
			...
			if ( flDist < 200.0f )
			{
				...
				SetContextThink( &CNPC_BaseScanner::DiveBombSoundThink, CURTIME + 0.5f, s_pDiveBombSoundThinkContext );
				return;
			}
		}
	}

	SetContextThink( &CNPC_BaseScanner::DiveBombSoundThink, CURTIME + 2.0f * TICK_INTERVAL, s_pDiveBombSoundThinkContext );
}
