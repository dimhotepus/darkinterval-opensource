...
#ifdef DARKINTERVAL
		pSporeExplosion->SetLifetime(5.0f);
#else
...
#ifdef DARKINTERVAL
		CSoundEnt::InsertSound( SOUND_BUGBAIT, GetAbsOrigin(), 2500, 5.0f, GetThrower() );
#else
	...
	if ( !bSqueezed )
	{
		for ( int i = 0; i < count; i++ )
		{
...
#ifdef DARKINTERVAL
			if ( UTIL_DistApprox( pList[i]->WorldSpaceCenter(), vecOrigin ) < 150 )
#else
...
#endif
			{
				...
				{
					CAI_BaseNPC *pCitizen = pList[i]->MyNPCPointer();

					if (pCitizen != NULL)
					{
						trace_t tr;
						UTIL_TraceLine(vecOrigin, pCitizen->EyePosition(), MASK_ALL, pOwner, COLLISION_GROUP_NONE, &tr);

						if (tr.fraction == 1.0 || tr.m_pEnt == pCitizen)
						{
							variant_t var;
							var.SetString(MAKE_STRING("TLK_SMELL"));
							g_EventQueue.AddEvent(pCitizen, "SpeakResponseConcept", var, RandomFloat(0, 1.0), pOwner, pOwner);
						}
					}
				}
				*/
