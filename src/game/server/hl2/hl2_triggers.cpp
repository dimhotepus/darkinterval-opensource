...
#include "triggers.h"
#ifdef DARKINTERVAL 
#include "te_effect_dispatch.h" // for icky watery death splashes
#endif
//-----------------------------------------------------------------------------
...
void CTriggerWeaponDissolve::Activate( void )
{
	...
	SetContextThink( &CTriggerWeaponDissolve::DissolveThink, CURTIME + 0.1f, s_pDissolveThinkContext );
}
...
void CTriggerWeaponDissolve::DissolveThink( void )
{
	...
	for ( int i = 0; i < numWeapons; i++ )
	{
		...
		pWeapon->Dissolve( NULL, CURTIME + ( 3.0f - flLifetime ), false );
		...
		m_pWeapons.Remove( i );
		SetContextThink( &CTriggerWeaponDissolve::DissolveThink, CURTIME + random->RandomFloat( 0.5f, 1.5f ), s_pDissolveThinkContext );
		return;
	}
...
void CTriggerPhysicsTrap::Touch( CBaseEntity *pOther )
{
	...
	pAnim->Dissolve( NULL, CURTIME, false, m_nDissolveType );
}
...
void CWateryDeathLeech::Precache( void )
{
	...
	if (!GetModelName())
	{
		SetModelName(MAKE_STRING("models/leech.mdl"));
	}
...
enum
{
	DEATH_TYPE_LEECHES = 0, // leeches and slow death
	DEATH_TYPE_HYDRAS, // mini hydras and slow death
	DEATH_TYPE_ICKY // icky sounds and instant death later
#endif
};
...
#ifdef DARKINTERVAL 
DEFINE_KEYFIELD(m_deathType, FIELD_INTEGER, "DeathType"),
#endif
END_DATADESC()
...
void CTriggerWateryDeath::Spawn(void)
{
	...
	m_flNextPullSound = 0;
#ifdef DARKINTERVAL 
	if (m_deathType == DEATH_TYPE_LEECHES || m_deathType == DEATH_TYPE_HYDRAS)
#endif
	{
		m_flPainValue = 0;
	}
...
void CTriggerWateryDeath::Precache(void)
{
	...
	else if (m_deathType == DEATH_TYPE_ICKY)
	{
		PrecacheScriptSound("WateryDeath.Bite"); // still in the game!
		PrecacheScriptSound("WateryDeath.Pull");
		PrecacheScriptSound("WateryDeath.Warn");
	}
#else
	PrecacheModel("models/leech.mdl");
	PrecacheScriptSound("coast.leech_bites_loop");
...
#ifdef DARKINTERVAL 
			if (m_deathType == DEATH_TYPE_LEECHES)
				pLeech->SetModelName(AllocPooledString("models/leech.mdl"));
			else if( m_deathType == DEATH_TYPE_HYDRAS)
				pLeech->SetModelName(AllocPooledString("models/_Monsters/Xen/hydra_micro.mdl"));
#endif
			pLeech->Spawn();
...
void CTriggerWateryDeath::Touch(CBaseEntity *pOther)
{
	...
	// Time to kill it?
	if (CURTIME > flKillTime)
	{
#ifdef DARKINTERVAL 
		if (m_deathType == DEATH_TYPE_ICKY)
		{
			// Chomp!
...
#endif
		{
			...
			m_flEntityKillTimes[iIndex] = CURTIME + WD_KILLTIME_NEXT_BITE;
		}
#ifdef DARKINTERVAL 
		else if (m_deathType == DEATH_TYPE_ICKY)
		{
	...
	{
		if (m_deathType == DEATH_TYPE_ICKY && pOther->IsPlayer())
		{
			Vector vecSurroundMins, vecSurroundMaxs;
			pOther->CollisionProp()->WorldSpaceSurroundingBounds(&vecSurroundMins, &vecSurroundMaxs);

			// Warn the target?
			if ((CURTIME + WD_KILLTIME_DRAGDOWN_TIME) < flKillTime)
			{
				// We're still giving the player some warning time
			}

			else if (vecSurroundMaxs.z > (WorldAlignMaxs().z - flDistanceBelow))
			{
				if (m_flNextPullSound < CURTIME)
				{
					m_flNextPullSound = CURTIME + 5.0;
					...
					if (vecSurroundMaxs.z > WorldAlignMaxs().z)
					{
						CEffectData	data;
...
void CTriggerWateryDeath::StartTouch(CBaseEntity *pOther)
{
	...
	if (m_hTouchingEntities.Find(hOther) != m_hTouchingEntities.InvalidIndex())
	{
...
#endif
		{
			...
			if (pOther->IsPlayer())
			{
				m_flEntityKillTimes.AddToTail(CURTIME + WD_KILLTIME_NEXT_BITE);
			}
			else
			{
				m_flEntityKillTimes.AddToTail(CURTIME + WD_KILLTIME_NEXT_BITE);
		...
		else if (m_deathType == DEATH_TYPE_ICKY)
		{
			...
			if (pOther->IsPlayer())
			{
				...
				m_flEntityKillTimes.AddToTail(CURTIME + WD_KILLTIME_WARN_TIME);
			}
			else
			{
				m_flEntityKillTimes.AddToTail(CURTIME + WD_KILLTIME_DRAGDOWN_TIME);
			}
		}
#endif
	}
...
void CTriggerWateryDeath::EndTouch(CBaseEntity *pOther)
{
...
#endif
	{
		if (pOther->IsPlayer())
		{
			...
			if (pHL2Player)
			{
				//        EndTouch is called not only when the player leaves
