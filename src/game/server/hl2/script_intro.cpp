...
void CScriptIntro::InputSetBlendMode( inputdata_t &inputdata )
{
	...
	m_flBlendStartTime = m_flNextBlendTime = CURTIME;
	m_iQueuedBlendMode = -1;
	SetContextThink( NULL, CURTIME, "BlendComplete" );

	if ( cl_spewscriptintro.GetInt() )
	{
		DevMsg( 1, "%.2f INPUT: Blend mode set to %d\n", CURTIME, m_iBlendMode.Get() );
	}
}
...
void CScriptIntro::InputSetFOVBlendTime( inputdata_t &inputdata )
{
	...
	m_flNextFOVBlendTime = CURTIME + inputdata.value.Float();
	m_flFOVBlendStartTime = CURTIME;
}
...
{
	m_flNextBlendTime = CURTIME + inputdata.value.Float();
	...
	if ( cl_spewscriptintro.GetInt() )
	{
		DevMsg( 1, "%.2f BLEND STARTED: %d to %d, end at %.2f\n", CURTIME, m_iBlendMode.Get(), m_iNextBlendMode.Get(), m_flNextBlendTime.Get() );
	}
...
void CScriptIntro::InputFadeTo( inputdata_t &inputdata )
{
	...
	//Msg("%.2f INPUT FADE: Fade to %.2f. End at %.2f\n", CURTIME, m_flFadeAlpha.Get(), CURTIME + m_flFadeDuration.Get() );
}
