...
public:
...
#endif
	virtual bool IsWorker(void) const { return HasSpawnFlags(SF_ANTLION_WORKER); }	// NOTE: IsAntlionWorker function must agree!
#ifdef DARKINTERVAL // a new subtype in DI, much more armoured, vulnerable when it opens its wings
	virtual bool IsSoldier(void) const { return false; }
#endif
	...
	void		Activate(void);
#ifdef DARKINTERVAL // for the OnPlayerInAlertRadius output
	void		RunAI(void);
#endif
...
#ifdef DARKINTERVAL // gibbing ragdolls is iffy for the moment, keep it off
	bool		CanGibRagdoll(void) { return false; } // (!ClassMatches("npc_antlion_soldier"));}
#endif
	...
	void		InputFlip(inputdata_t &inputdata); // DI NEW - behave as if pushed by the gg
	void		InputFlipZapped(inputdata_t &inputdata); // DI NEW - behave as if pushed by the vort zap attack
#endif
...
private:
...
#ifdef DARKINTERVAL	// DI NEW - restore charging attack for regular lions
	...
	COutputEvent	m_OnObjectInAlertRadius;	// Some object has entered within m_flAlertRadius sphere
#endif
...
#ifdef DARKINTERVAL
		SCHED_ANTLION_CHARGE_ENEMY // restore charging attack for regular lions
#endif
	};
...
#ifdef DARKINTERVAL
		TASK_ANTLION_CHARGE_ENEMY, // restore charging attack for regular lions
#endif
	};
};
