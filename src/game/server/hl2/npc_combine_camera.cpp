...
ConVar	g_debug_combine_camera("g_debug_combine_camera", "0"); // Debug visualization
...
protected:
	...
	CSprite *m_pEyeFlash;
#ifdef DARKINTERVAL
	COutputEvent m_OnDetectedCameraTarget;
	COutputEvent m_OnLostCameraTarget;
#endif
	DECLARE_DATADESC();
};
...
bool CNPC_CombineCamera::UpdateFacing()
{
	...
	if (bMoved && (m_flMoveSoundTime < CURTIME))
	{
		EmitSound("NPC_CombineCamera.Move");
		m_flMoveSoundTime = CURTIME + CAMERA_MOVE_INTERVAL;
	}
...
void CNPC_CombineCamera::ActiveThink()
{
	...
	if ( !pTarget )
	{
		...
		SetAngry(false);
#ifdef DARKINTERVAL
		m_OnLostCameraTarget.FireOutput(this, this);
#endif
		SetThink(&CNPC_CombineCamera::SearchThink);
		SetNextThink( CURTIME );
		return;
	...
	if ( pTarget != m_hEnemyTarget )
	{
		...
		if ( (flDist < m_nInnerRadius) && FInViewCone(pTarget) )
		{
			...
			if ( pTarget->IsPlayer() )
			{
				...
				{
#ifdef DARKINTERVAL
					m_OnDetectedCameraTarget.FireOutput(this, this);
#endif
					SetAngry(true);
				}
				else
...
void CNPC_CombineCamera::MaintainEye()
{
	...
	{
		if ((m_flClickTime != 0) && (m_flClickTime < CURTIME))
		{
			m_pEyeFlash->SetScale(1.0);
			...
			m_flTurnOffEyeFlashTime = CURTIME + 0.1;
			m_flClickTime = CURTIME + CAMERA_CLICK_INTERVAL;
		}
		else if ((m_flTurnOffEyeFlashTime != 0) && (m_flTurnOffEyeFlashTime < CURTIME))
		{
...
void CNPC_CombineCamera::SetEyeState(eyeState_t state)
{
	...
	if ( state == CAMERA_EYE_IDLE && m_flEyeHappyTime > CURTIME )
	{
...
void CNPC_CombineCamera::Ping()
{
	...
	m_flPingTime = CURTIME + COMBINE_CAMERA_PING_TIME;
}
...
#ifdef DARKINTERVAL
	DEFINE_OUTPUT(m_OnDetectedCameraTarget, "OnDetectedCameraTarget"),
	DEFINE_OUTPUT(m_OnLostCameraTarget, "OnLostCameraTarget"),
#endif
END_DATADESC()