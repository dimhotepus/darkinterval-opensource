...
BEGIN_DATADESC( CShieldBeam )
	...
	// Function Pointers
	DEFINE_FUNCTION( ShieldBeamThink ),
...
{
	float fInterval = CURTIME - m_flAnimTime;
	if (fInterval <= 0.001)
	{
...
void CShieldBeam::ShieldBeamThink( void )
{
	...
	if (!m_bKillBeam	&& 
		m_hHead != NULL &&
		CURTIME > m_fNextNodeTime)
	{
		m_fNextNodeTime = CURTIME + 0.2;
...
void CShieldBeam::ShieldBeamStart(  CBaseEntity* pHeadEntity, CBaseEntity*	pTailEntity,  const Vector &vInitialVelocity, float speed )
{
	...
	// Create beam 
	// -------------
	m_pBeam = CBeam::BeamCreate( "sprites/hydragutbeam.vmt", 1 );//<<TEMP>>temp art
...
CShieldBeam *CShieldBeam::ShieldBeamCreate(CBaseEntity* pHeadEntity, int nAttachment)
{
	...
	pShieldBeam->SetNextThink( CURTIME );
