...
class CSpriteTrail;
#ifdef DARKINTERVAL // magnet-like entity that can make combine balls gravitate toward it
class CCombineBallAttractor : public CBaseAnimating
{
	DECLARE_DATADESC();
	DECLARE_CLASS(CCombineBallAttractor, CBaseAnimating);
public:
	CCombineBallAttractor();
	~CCombineBallAttractor();
	void Precache();
	void Spawn();
...
private:
	static CUtlVector<CCombineBallAttractor *> m_stickyList;
...
#ifdef DARKINTERVAL
	void StartGravitateThink(void); // smaller balls will gravitate towards ball attractor if it's near.
#endif
