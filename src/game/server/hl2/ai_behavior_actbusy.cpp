...
#ifdef DARKINTERVAL // todo: this isn't used anywhere, what was it intended for?
	DEFINE_FIELD( m_flTimeToActDelayed, FIELD_TIME ),
#endif
END_DATADESC();
...
void CAI_ActBusyBehavior::Enable( CAI_ActBusyGoal *pGoal, float flRange, bool bVisibleOnly )
{
	...
	m_bLeaving = false;
	m_flNextBusySearchTime = CURTIME + ai_actbusy_search_time.GetFloat();
	m_flEndBusyAt = 0;
	...
	if ( IsCurScheduleOverridable() )
	{
		...
		m_flNextBusySearchTime = CURTIME;
...
#ifdef DARKINTERVAL
//-----------------------------------------------------------------------------
// Purpose: Fix the lame not allowing forced switches
//-----------------------------------------------------------------------------
void CAI_ActBusyBehavior::ForceExitAnim(void)
{
	if (ai_debug_actbusy.GetInt() == 4)
	{
		Msg("ACTBUSY: ForceExitAnim on NPC %s (%s)\n", GetOuter()->GetClassname(), GetOuter()->GetDebugName());
	}

	PlayAnimForActBusy(BA_EXIT);
	Enable(NULL, m_flBusySearchRange, false);
	m_bForceActBusy = true;
	m_bLeaving = false;
	m_ForcedActivity = ACT_INVALID;
	m_hSeeEntity = NULL;
...
bool CAI_ActBusyBehavior::CanSelectSchedule( void )
{
	...
	return (m_flNextBusySearchTime < CURTIME);
}
...
void CAI_ActBusyBehavior::OnFriendDamaged( CBaseCombatCharacter *pSquadmate, CBaseEntity *pAttacker )
{
	if( IsCombatActBusy() && pSquadmate->IsPlayer() && IsInSafeZone( pAttacker ) )
	{
		...
		m_flDeferUntil = CURTIME + 4.0f;	// Stop actbusying and go deal with that enemy!!
	}
...
void CAI_ActBusyBehavior::GatherConditions( void )
{
	...
	if ( m_hSeeEntity && m_bBusy )
	{
		...
		{
			m_fTimeLastSawSeeEntity = CURTIME;
			ClearCondition( COND_ACTBUSY_LOST_SEE_ENTITY );
		}
		else if( m_hActBusyGoal )
		{
			float fDelta = CURTIME - m_fTimeLastSawSeeEntity;
			...
			case BA_INT_ZOMBIESLUMP:
				{
					...
					if( pPlayer )
					{
						...
						if( flDist <= 60 )
						{
							...
							if (ai_debug_actbusy.GetInt() == 4)
							{
								Msg("ACTBUSY: BA_INT_ZOMBIESLUP interrupt on NPC %s (%s) fired due to the player being closer than 60 units\n", GetOuter()->GetClassname(), GetOuter()->GetDebugName());
							}
#endif
							StopBusying();
						}
...
Activity CAI_ActBusyBehavior::NPC_TranslateActivity( Activity nActivity )
{
	...
	{
#ifdef DARKINTERVAL
		// DI NEW: allow to use readiness overrides with actbusy
		CNPC_PlayerCompanion *pComp = dynamic_cast<CNPC_PlayerCompanion*>(GetOuter());
		if (pComp)
		{
			return pComp->TranslateActivityReadiness(nActivity);
		}
#endif
		...
		if ( GetHintNode()->HintType() == HINT_NPC_EXIT_POINT )
		{
			...
			if ( UTIL_DistApprox( GetHintNode()->GetAbsOrigin(), GetAbsOrigin() ) < 64 )
			{
				if ( !GetOuter()->IsMarkedForDeletion() )
				{
					...
					GetOuter()->SetNextThink( CURTIME + 0.1 );
...
int CAI_ActBusyBehavior::SelectScheduleWhileNotBusy( int iBase )
{
	...
	if ( m_bForceActBusy || m_flNextBusySearchTime < CURTIME )
	{
		// If we're being forced, think again quickly
		if ( m_bForceActBusy || IsCombatActBusy() )
		{
			m_flNextBusySearchTime = CURTIME + 2.0;
		}
		else
		{
			m_flNextBusySearchTime = CURTIME + RandomFloat(ai_actbusy_search_time.GetFloat(), ai_actbusy_search_time.GetFloat()*2);
		}
			...
			if ( HasAnimForActBusy( iBusyAnim, BA_BUSY ) )
			{
				if ( HasCondition(COND_ACTBUSY_LOST_SEE_ENTITY) )
				{
					...
					if ( m_bForceActBusy )
					{
						...
						if (ai_debug_actbusy.GetInt() == 4)
						{
							Msg("ACTBUSY: NPC %s (%s) lost Sight Entity\n", GetOuter()->GetClassname(), GetOuter()->GetDebugName());
						}
#endif
						StopBusying();
					}
		...
		else
		{
			...
			if( IsCombatActBusy() )
			{
				...
				m_flNextBusySearchTime = CURTIME + 10.0;
			}
		}
	}
...
int	CAI_ActBusyBehavior::SelectScheduleWhileBusy( void )
{
	...
	if ( !ActBusyNodeStillActive() || (m_flEndBusyAt && CURTIME >= m_flEndBusyAt) )
	{
		if ( ai_debug_actbusy.GetInt() == 4 )
		{
	...
	case TASK_ACTBUSY_PLAY_EXIT:
		{
			...
			// Play the exit animation. If it fails, we don't have an exit anim, so complete immediately.
	...
	case TASK_ACTBUSY_PLAY_BUSY_ANIM:
		{
			...
			else
			{
				...
				if( HasCondition(COND_ACTBUSY_LOST_SEE_ENTITY) )
				{
					...
					if (ai_debug_actbusy.GetInt() == 4)
					{
						Msg("ACTBUSY: NPC %s (%s) lost Sight Entity during its TASK_ACTBUSY_PLAY_BUSY_ANIM\n", GetOuter()->GetClassname(), GetOuter()->GetDebugName());
					}
#endif
					StopBusying();
					TaskComplete();
...
#ifdef DARKINTERVAL
	DEFINE_INPUTFUNC( FIELD_EHANDLE, "ExitCurrentBusy", InputExitCurrentBusy),
#endif
	// Outputs
...
#ifdef DARKINTERVAL
//-----------------------------------------------------------------------------
// Purpose: Allow forced exit via input
//-----------------------------------------------------------------------------
void CAI_ActBusyGoal::InputExitCurrentBusy(inputdata_t &inputdata)
{
	CAI_ActBusyBehavior *pBehavior = GetBusyBehaviorForNPC(inputdata.value.Entity(), "InputExitCurrentBusy");
	if (!pBehavior)
		return;

	pBehavior->ForceExitAnim();
}
#endif
//-----------------------------------------------------------------------------
// Purpose: 
...
void CAI_ActBusyQueueGoal::InputActivate( inputdata_t &inputdata )
{
	if ( !IsActive() )
	{
		...
		SetContextThink( &CAI_ActBusyQueueGoal::QueueThink, CURTIME + 5, QUEUE_THINK_CONTEXT );
	}
...
void CAI_ActBusyQueueGoal::MoveQueueUp( void )
{
	...
	if ( GetNextThink( QUEUE_MOVEUP_THINK_CONTEXT ) < CURTIME )
	{
		float flTime = CURTIME + RandomFloat( 0.3, 0.5 );
