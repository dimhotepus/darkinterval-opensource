...
// DI todo - implement saving headcrabs from barnacles, fix live ragdolls,
// implement picking up with gravity gun
//=============================================================================//
...
#ifdef DARKINTERVAL
#include "props.h" // for gibbing
#endif
#include "soundent.h"
#ifdef DARKINTERVAL
#include "te_effect_dispatch.h" // for gibbing
#endif
...
#ifndef DARKINTERVAL // these are now baked into the models as breakables
...
#ifdef DARKINTERVAL	// DI CHANGE: attempt to fix ceiling crabs dropping to floor
	if (HasSpawnFlags(SF_HEADCRAB_START_HANGING))
		AddFlag(FL_FLY);
#endif
	CapabilitiesClear();
	CapabilitiesAdd( bits_CAP_MOVE_GROUND | bits_CAP_INNATE_RANGE_ATTACK1 );
	CapabilitiesAdd( bits_CAP_SQUAD );
#ifdef DARKINTERVAL // DI todo - live ragdolls are quite bad on headcrabs, investigate
...
void CBaseHeadcrab::Leap( const Vector &vecVel )
{
	...
	m_flIgnoreWorldCollisionTime = CURTIME + HEADCRAB_IGNORE_WORLD_COLLISION_TIME;
...
void CBaseHeadcrab::RunTask( const Task_t *pTask )
{
	switch ( pTask->iTask )
	{
		...
		case TASK_HEADCRAB_WAIT_FOR_BARNACLE_KILL:
			...
			{
				m_flNextFlinchTime = CURTIME + random->RandomFloat( 1.0f, 2.0f );
...
void CBaseHeadcrab::LeapTouch( CBaseEntity *pOther )
{
	...
	else if( !(GetFlags() & FL_ONGROUND) )
	{
		...
		if( CURTIME < m_flIgnoreWorldCollisionTime )
		{
...
// DI todo - investigate if this can be helpful for held headcrabs and live ragdolls
//-----------------------------------------------------------------------------
void CBaseHeadcrab::EliminateRollAndPitch()
{
	...
	SetContextThink( &CBaseHeadcrab::EliminateRollAndPitch, CURTIME + TICK_INTERVAL, s_pPitchContext );
}
...
void CBaseHeadcrab::DropFromCeiling( void )
{
	...
	if ( HL2GameRules()->IsAlyxInDarknessMode() )
	{
		if ( IsHangingFromCeiling() )
		{
			...
			{
				m_flIlluminatedTime = CURTIME + HEADCRAB_ILLUMINATED_TIME;
				return;
			}

			if ( m_flIlluminatedTime <= CURTIME )
			{
...
{
	if ( flDot < 0.97387f ) // DI todo - check if this lines up with DI's slightly bigger flashlight fov
		return;
...
bool CBaseHeadcrab::CorpseGib( const CTakeDamageInfo &info )
{
	...
	// we could handle the particle through the model itself, but if someone replaces the HC model, they still should get the particle
	DispatchParticleEffect("GrubSquashBlood", GetAbsOrigin(), QAngle(0, 0, 0));

	EmitSound("NPC_HeadCrab.Gib");
	return true; // don't go into baseclass, or it'll spawn a bunch of skull gibs!!!
#else
	EmitSound( "NPC_HeadCrab.Gib" );	
...
{
	// If we're being hoisted, do not gib from the barnacle's own bite, or he'll be left without anything to eat!
	...
	if ( info.GetDamageType() & DMG_DROWN )
		return false; // as comical as it is, headcrabs falling apart in water is wrong

	if ( info.GetDamageType() & ( DMG_NEVERGIB | DMG_DISSOLVE ) )
		return false;

	if ( info.GetDamageType() & ( DMG_ALWAYSGIB | DMG_BLAST | DMG_VEHICLE ) )
		return true;

	if ( info.GetDamageType() & DMG_BUCKSHOT && m_iHealth <= m_iMaxHealth * 0.2 )
		return true;
	...
	switch ( m_NPCState )
	{
		case NPC_STATE_ALERT:
		{
			if (HasCondition( COND_LIGHT_DAMAGE ) || HasCondition( COND_HEAVY_DAMAGE ))
			{
				...
				{
					m_flNextFlinchTime = CURTIME + random->RandomFloat( 1, 3 );
...
#ifdef DARKINTERVAL // DI todo - what's this for? prone is set when we're being stomped or stabbed, so?
		case NPC_STATE_PRONE:
		{
			return SCHED_HEADCRAB_BURROW_IN;
			break;
		}
#endif
	}
...
void CBaseHeadcrab::Ignite( float flFlameLifetime, bool bNPCOnly, float flSize, bool bCalledByLevelDesigner )
{
	...
	if( !bWasOnFire )
	{
#ifndef DARKINTERVAL // DI todo - reconsider adding this, or too perf heavy?
#ifdef HL2_EPISODIC
...
bool CBaseHeadcrab::HandleInteraction(int interactionType, void *data, CBaseCombatCharacter* sourceEnt)
{
	...
	{
		// Die instantly
		ClearSchedule("Grabbed by barnacle");
		return true; // di new
	}
#endif
...
void CBaseHeadcrab::CreateDust( bool placeDecal )
{
	...
	if ( tr.fraction < 1.0f )
	{
		...
		if ( ( (char) pdata->game.material == CHAR_TEX_CONCRETE ) 
			|| ((char) pdata->game.material == CHAR_TEX_DIRT )
#ifdef DARKINTERVAL
			|| ((char) pdata->game.material == CHAR_TEX_SAND)
#endif
			)
		{
...
{
#ifdef DARKINTERVAL
	PrecacheModel( "models/_Monsters/Xen/headcrab.mdl" );
	PropBreakablePrecacheAll(MAKE_STRING("models/_Monsters/Xen/headcrab.mdl"));
#else
...
void CHeadcrab::Spawn( void )
{
	Precache();
#ifdef DARKINTERVAL
	SetModel( "models/_Monsters/Xen/headcrab.mdl" );
#else
...
{
#ifdef DARKINTERVAL
	PrecacheModel("models/_Monsters/Xen/headcrab_fast.mdl");
	PropBreakablePrecacheAll(MAKE_STRING("models/_Monsters/Xen/headcrab_fast.mdl"));
#else
...
void CFastHeadcrab::Spawn( void )
{
	Precache();
#ifdef DARKINTERVAL
	SetModel( "models/_Monsters/Xen/headcrab_fast.mdl" );
#else
	SetModel("models/headcrab.mdl");
#endif
	BaseClass::Spawn();
	...
	if( IsAlive() && GetNavigator()->IsGoalActive() )
	{
		...
		case HEADCRAB_RUNMODE_ACCELERATE:
			...
			if( m_flPlaybackRate >= HEADCRAB_RUN_MAXSPEED )
			{
				...
				m_flSlowRunTime = CURTIME + random->RandomFloat( 0.1, 1.0 );
			}
			break;
		...
		case HEADCRAB_RUNMODE_DECELERATE:
			...
			if( m_flPlaybackRate <= HEADCRAB_RUN_MINSPEED )
			{
				...
				m_flPauseTime = CURTIME + random->RandomFloat( 0.2, 0.5 );
		...
		case HEADCRAB_RUNMODE_PAUSE:
			{
				if( CURTIME > m_flPauseTime )
				{
					m_iRunMode = HEADCRAB_RUNMODE_IDLE;
					SetActivity( ACT_RUN );
					GetNavigator()->SetMovementActivity(ACT_RUN);
					m_flPauseTime = CURTIME - 1;
					m_flRealGroundSpeed = m_flGroundSpeed;
				}
			}
			break;
...
#ifdef DARKINTERVAL
Activity CFastHeadcrab::NPC_TranslateActivity(Activity eNewActivity)
{
	if (eNewActivity == ACT_STAND)
		return ACT_RANGE_ATTACK1;
...
{
#ifdef DARKINTERVAL
	PrecacheModel("models/_Monsters/Xen/headcrabblack.mdl");
#else
...
void CBlackHeadcrab::Spawn(void)
{
	Precache();
#ifdef DARKINTERVAL
	SetModel("models/_Monsters/Xen/headcrabblack.mdl");
#else
...
int CBlackHeadcrab::TranslateSchedule( int scheduleType )
{
	switch ( scheduleType )
	{
		...
		case SCHED_FAIL_TAKE_COVER:
		{
			if ( ( m_bPanicState ) && ( CURTIME > m_flPanicStopTime ) )
			{
...
int CBlackHeadcrab::SelectSchedule( void )
{
	...
	if ( !IsHangingFromCeiling() )
	{
		...
		if ( HasCondition( COND_LIGHT_DAMAGE ) || HasCondition( COND_HEAVY_DAMAGE ) || (HasCondition(COND_HEAR_BULLET_IMPACT)))
		{
			if ( ( CURTIME >= m_flNextHopTime ) && SelectWeightedSequence( ACT_SMALL_FLINCH ) != -1 )
			{
				m_flNextHopTime = CURTIME + random->RandomFloat( 1, 3 );
				return SCHED_SMALL_FLINCH;
			}
		}
...
{
	m_flPanicStopTime = CURTIME + flDuration;
	m_bPanicState = true;
}
