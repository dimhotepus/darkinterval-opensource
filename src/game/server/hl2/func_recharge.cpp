...
void CNewRecharge::InputSetCharge( inputdata_t &inputdata )
{
	ResetSequence( LookupSequence( "idle" ) );
#ifdef DARKINTERVAL
	int iJuice = min(inputdata.value.Int(), m_iMaxJuice);
#else
...
#ifdef DARKINTERVAL
void CNewRecharge::InputDisable(inputdata_t &inputdata)
{
...
void CNewRecharge::InputEnable(inputdata_t &inputdata)
{
...
{ 
	...
	if ( HasSpawnFlags(	SF_CITADEL_RECHARGER ) )
	{
		...
		// Also give health for the citadel version.
		if ( pActivator->GetHealth() < pActivator->GetMaxHealth() && m_flNextCharge < CURTIME )
		{
			pActivator->TakeHealth( 5, DMG_GENERIC );
		}
	}
...
#ifdef DARKINTERVAL
	// Inform the suit
	CHL2_Player *pl = ToHL2Player(UTIL_GetLocalPlayer());
	Assert(pl != NULL);
	// Hacky way to send over suit update
	pl->ApplyBattery(0, true, false);	
#endif
	m_iOn = 0;
	m_flJuice = m_iJuice;
