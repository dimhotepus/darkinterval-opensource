...
#ifdef DARKINTERVAL // really old experiment from DI Part 1. Unused but kept for sentimental reasons.
...
#ifdef DARKINTERVAL // really old experiment from DI Part 1. Unused but kept for sentimental reasons.
	DEFINE_THINKFUNC(BreakFxThink),
	DEFINE_FIELD(m_flBreakFxTime, FIELD_TIME ),
#endif
END_DATADESC()
...
void CWeaponStunStick::Precache()
{
	...
	PrecacheParticleSystem("explosion_turret_break_pre_sparks");
	PrecacheScriptSound("DoSpark");
	PrecacheScriptSound("NPC_Turret.Detonate");
#endif
}
...
// Purpose: 
//-----------------------------------------------------------------------------
ConVar di_stunstick_fx_think("di_stunstick_fx_think", "3.0");

void CWeaponStunStick::BreakWeapon(void)
{
	m_flBreakFxTime = CURTIME + di_stunstick_fx_think.GetFloat();
	SetThink(&CWeaponStunStick::BreakFxThink);
	SetNextThink(CURTIME + 0.1f);
}

void CWeaponStunStick::BreakFxThink(void)
{
	CBasePlayer *owner = ToBasePlayer(GetOwner());

//	if (m_flBreakFxTime < CURTIME + 0.5f)
//	{
	if (owner && owner != NULL)
	{
		DispatchParticleEffect("explosion_turret_break_pre_sparks", PATTACH_POINT_FOLLOW, owner->GetViewModel(), "breakFX", false);
		EmitSound("DoSpark");
//		}
//	}
		if (m_flBreakFxTime < CURTIME)
		{
			color32 white = { 255,225,205,64 };
			UTIL_ScreenFade(owner, white, 0.1f, 0.0f, FFADE_IN);
			EmitSound("NPC_Turret.Detonate");
			const char *name = "weapon_broken";
			SetName(AllocPooledString(name));
			owner->Weapon_Drop(this, NULL, NULL);
			UTIL_Remove(this);
		}
	}
