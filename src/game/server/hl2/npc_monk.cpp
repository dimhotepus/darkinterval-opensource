...
Activity CNPC_Monk::NPC_TranslateActivity( Activity eNewActivity )
{
	...
	if ( (m_NPCState == NPC_STATE_COMBAT || m_NPCState == NPC_STATE_ALERT) )
	{
		...
		bGunUp = (CURTIME - m_last_attack_time < 4);
...
Vector CNPC_Monk::GetActualShootTrajectory( const Vector &shootOrigin )
{
	if( GetEnemy() && GetEnemy()->Classify() == CLASS_ZOMBIE )
	{
		Vector vecShootDir;
#ifdef DARKINTERVAL
		if( m_bPerfectAccuracy || random->RandomInt( 1, 2 ) == 1 )
#else
...
#ifdef DARKINTERVAL // DI todo - turn into a spawnflag or setting? or just rely on map filters?
	if (pEnemy->ClassMatches("npc_antlion")) return false; // hack for Haven's Gregory!
	if (pEnemy->ClassMatches("npc_squid")) return false; // hack for Haven's Gregory!
	if (pEnemy->ClassMatches("npc_watersquid")) return false; // hack for Haven's Gregory!
#endif
	return false;
}
