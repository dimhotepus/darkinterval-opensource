...
#ifdef DARKINTERVAL
static int ACT_CROW_LAND_MOVEMENT;
static int ACT_CROW_HOP_MOVEMENT;
#endif
//
// Animation events.
//
...
#ifdef DARKINTERVAL
static int AE_CROW_HOP_MOVEMENT;
static int AE_CROW_LANDED;
#endif
//
// Skill settings.
//
...
{
	BaseClass::Precache();
#ifdef DARKINTERVAL // has some activity changes, but isn't really necessary
	PrecacheModel("models/_Monsters/Earth/crow.mdl");
#else
...
#ifdef DARKINTERVAL // don't remember - why is it necessary?
...
void CNPC_Crow::Spawn( void )
{
	...
	{
#ifdef DARKINTERVAL // has some activity changes, but isn't really necessary
		szModel = "models/_Monsters/Earth/crow.mdl";
#else
...
#ifdef DARKINTERVAL
	// DI CHANGE: Ignore the player. Otherwise their presence breaks idle animations!
//	AddClassRelationship(CLASS_PLAYER, D_NU, 0);
#endif
	m_bSoar = false;
	m_bOnJeep = false;
	m_flSoarTime = CURTIME;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_Crow::CreateBehaviors()
{
...
#ifdef DARKINTERVAL // DI allows the player to get closer, because crows are cool
	if ( m_flEnemyDist < 192 )
#else
...
void CNPC_Crow::HandleAnimEvent( animevent_t *pEvent )
{
...
#ifdef DARKINTERVAL
	if (pEvent->event == AE_CROW_HOP_MOVEMENT)
	{
		m_nMorale -= random->RandomInt(1, 6);
		if (m_nMorale <= 0)
		{
	...
	if( pEvent->event == AE_CROW_FLY )
	{
		...
		m_bSoar = false;
		m_flSoarTime = CURTIME + random->RandomFloat( 3, 5 );

		return;
	}
#ifdef DARKINTERVAL
	if (pEvent->event == AE_CROW_LANDED)
	{
	...
	if ( IsFlying() )
	{
		if ( GetNavigator()->GetPath()->GetCurWaypoint() )
		{
			if ( m_flLastStuckCheck <= CURTIME )
			{
				...
				m_flLastStuckCheck = CURTIME + 1.0f;
			}

			if (m_bReachedMoveGoal )
			{
				SetIdealActivity( (Activity)ACT_CROW_LAND_MOVEMENT );
...
#ifdef DARKINTERVAL
	if ( flDistance < 128 && !m_bReachedMoveGoal)
#else
	if ( flDistance < CROW_AIRSPEED && !m_bReachedMoveGoal )
#endif
	{
		if ( GetNavigator()->IsGoalActive() )
		{
...
void CNPC_Crow::SetFlyingState( FlyState_t eState )
{
	if ( eState == FlyState_Flying )
	{
		...
		SetMoveType( MOVETYPE_STEP );
		m_vLastStoredOrigin = GetAbsOrigin();
		m_flLastStuckCheck = CURTIME + 3.0f;
		m_flGroundIdleMoveTime = CURTIME + random->RandomFloat( 5.0f, 10.0f );
	}
...
#ifdef DARKINTERVAL
	else if (eState == FlyState_Landing)
	{ 
		// Landing is same as flying, except it adds the landing animation and goes to Falling on animevent
		SetGroundEntity(NULL);
		AddFlag(FL_FLY);
		SetNavType(NAV_GROUND);
		CapabilitiesRemove(bits_CAP_MOVE_GROUND);
	...
	else if (eState == FlyState_Falling)
#else
	else
#endif
	{
...
//		}
		...
		case TASK_FIND_HINTNODE:
		{
			...
			if ( !GetHintNode() )
			{
#ifdef DARKINTERVAL // DI todo - is this even used? don't see hintCriteria coming up
				CHintCriteria hintCriteria;
				hintCriteria.SetHintType(HINT_CROW_FLYTO_POINT);
				hintCriteria.SetFlag(bits_HINT_NODE_NEAREST | bits_HINT_NODE_USE_GROUP | bits_HINT_NODE_CLEAR | bits_HINT_NOT_CLOSE_TO_ENEMY);


				// Add the search position
				hintCriteria.AddIncludePosition(GetAbsOrigin(), 512);

				if( GetEnemy())
				hintCriteria.AddIncludePosition(GetEnemy()->GetAbsOrigin(), 512);
#endif 	
		...
		case TASK_CROW_FLY:
		{
			...
			m_bSoar = false;
			m_flSoarTime = CURTIME + random->RandomFloat( 2, 5 );
...
void CNPC_Crow::RunTask( const Task_t *pTask )
{
	switch ( pTask->iTask )
	{
		...
		case TASK_CROW_WAIT_FOR_BARNACLE_KILL:
		{
			...
			{
				m_flNextFlinchTime = CURTIME + random->RandomFloat( 0.5f, 2.0f );
				// dvs: TODO: squirm
...
int CNPC_Crow::SelectSchedule( void )
{
	...
	//
	if ( !( GetFlags() & FL_ONGROUND ) && ( CURTIME > 2.0 ) && m_bOnJeep == false )
	{
		return SCHED_CROW_FLY_AWAY;
	}
	...
	{
#ifdef DARKINTERVAL
		if (HasCondition(COND_HEAR_BULLET_IMPACT) || HasCondition( COND_HEAR_DANGER ) || HasCondition( COND_HEAR_COMBAT ) )
#else
		if ( HasCondition(COND_HEAR_DANGER) || HasCondition(COND_HEAR_COMBAT) )
#endif
		{
			m_flDangerSoundTime = CURTIME + 10.0f;
			return SCHED_CROW_FLY_AWAY;
		}
	}
...
#else
	if ( HasCondition(COND_CROW_ENEMY_TOO_CLOSE) && m_flDangerSoundTime <= CURTIME )
#endif
	{
...
Activity CNPC_Crow::NPC_TranslateActivity(Activity eNewActivity)
{
	...
	if (eNewActivity == ACT_FLY)
	{
		if (m_flSoarTime < CURTIME)
		{
			if (random->RandomInt(0, 100) <= 50 && m_bSoar == false && GetAbsVelocity().z < 0)
			{
				m_bSoar = true;
				m_flSoarTime = CURTIME + random->RandomFloat(1, 4);
			}
			else
			{
				m_bSoar = false;
...
BEGIN_DATADESC(CNPC_Crow)
	...
	// Inputs
	#ifdef DARKINTERVAL
	DEFINE_INPUTFUNC(FIELD_STRING, "FlyAwayToATarget", InputFlyAway),
	#endif
...
AI_BEGIN_CUSTOM_NPC( npc_crow, CNPC_Crow )
...
#ifdef DARKINTERVAL
	DECLARE_ACTIVITY( ACT_CROW_HOP_MOVEMENT)
	DECLARE_ACTIVITY( ACT_CROW_LAND_MOVEMENT)
#endif
	DECLARE_ANIMEVENT( AE_CROW_HOP )
	DECLARE_ANIMEVENT( AE_CROW_FLY )
	DECLARE_ANIMEVENT( AE_CROW_TAKEOFF )
#ifdef DARKINTERVAL
	DECLARE_ANIMEVENT( AE_CROW_HOP_MOVEMENT)
	DECLARE_ANIMEVENT( AE_CROW_LANDED )
#endif
	...
	(
		...
		"	Tasks"
		"		TASK_SET_FAIL_SCHEDULE			SCHEDULE:SCHED_CROW_IDLE_STAND"
		"		TASK_WAIT_FOR_MOVEMENT			0"
		"		"
		"	Interrupts"
		"		COND_CROW_FORCED_FLY"
