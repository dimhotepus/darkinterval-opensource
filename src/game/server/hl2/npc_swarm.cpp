//========================================================================//
//
// npc_swarm - experimental, cpu-intensive monster made of lots of smaller elements
//
//=============================================================================//
#include "cbase.h"
...
ConVar swarm_mindist( "swarm_mindist", "120.0" );
ConVar swarm_element_speed( "swarm_element_speed", "187" );
ConVar npc_swarm_idle_speed_factor( "npc_swarm_idle_speed_factor", "0.5" );

ConVar swarm_numelements( "swarm_numelements", "20" );
ConVar swarm_batchpercent( "swarm_batchpercent", "100" );

ConVar swarm_radius( "swarm_radius", "160" );

//ConVar swarm_min_element_speed( "swarm_min_element_speed", "50" );
//ConVar swarm_max_element_speed( "swarm_max_element_speed", "250" );

ConVar npc_swarm_use_threading( "npc_swarm_use_threading", "1" );

ConVar npc_swarm_sin_amplitude( "npc_swarm_sin_amplitude", "60.0f" );

ConVar npc_swarm_show_centroid( "npc_swarm_show_centroid", "0" );

ConVar npc_swarm_straggler_dist( "npc_swarm_straggler_dist", "240" );

ConVar npc_swarm_use_orientation( "npc_swarm_use_orientation", "1" );
ConVar npc_swarm_use_model( "npc_swarm_use_model", "5" );

ConVar npc_swarm_think_interval( "npc_swarm_think_interval", "0.025" );

ConVar npc_swarm_do_damage("npc_swarm_do_damage", "1");


#define NPC_SWARM_MODEL "models/headcrab.mdl"
...
//=========================================================
//=========================================================
class CSwarmElement : public CBaseAnimating
{
public:
	void Precache();
	void Spawn();
...
LINK_ENTITY_TO_CLASS( swarm_element, CSwarmElement );

//---------------------------------------------------------
// Save/Restore
//---------------------------------------------------------
BEGIN_DATADESC( CSwarmElement )
...
const char *pszSwarmModels[] =
{
	"models/chumtoad.mdl",
	"models/headcrab.mdl",
	"models/headcrabclassic.mdl",
	"models/chefhat.mdl",
	"models/_Monsters/Xen/xen_hair.mdl",
	"models/_Monsters/Combine/Crabsynth_micro.mdl"
};

const char *GetSwarmModelName()
{
	int index = npc_swarm_use_model.GetInt();

	return pszSwarmModels[ index ];
}

//---------------------------------------------------------
//---------------------------------------------------------
void CSwarmElement::Precache()
{
	PrecacheModel( GetSwarmModelName() );
...
void CSwarmElement::Spawn()
{
	...
	if(npc_swarm_use_model.GetInt() == 0) // HACKHACK, hardcoded animation choice for model 0 (chumtoad)
		SetSequence(SelectWeightedSequence(ACT_STEP_FORE));
	else if (npc_swarm_use_model.GetInt() == 2)
		SetSequence(LookupSequence("drown"));
	else
		SetSequence(SelectWeightedSequence(ACT_IDLE));
	SetCycle(RandomFloat(0.0f, 1.0f));
	SetPlaybackRate(RandomFloat(0.7f, 1.1f));
	ResetSequenceInfo();
...
//---------------------------------------------------------
//---------------------------------------------------------
int CSwarmElement::DrawDebugTextOverlays(void) 
{
...
// runs.
//---------------------------------------------------------
void CSwarmElement::SetElementVelocity( Vector vecVelocity, bool bPlanarOnly )
{
...
//---------------------------------------------------------
void CSwarmElement::AddElementVelocity( Vector vecVelocityAdd, bool bPlanarOnly )
{
...
//---------------------------------------------------------
// This function seeks to keep the swarm element moving along
// multiple different types of surfaces (climbing walls, etc)
//---------------------------------------------------------
#define BLOB_TRACE_HEIGHT 8.0f
void CSwarmElement::ModifyVelocityForSurface( float flInterval, float flSpeed )
{
	trace_t tr;
...
// Most often used to move along with the npc_swarm that 
// is directing me.
//---------------------------------------------------------
void CSwarmElement::MoveTowardsTargetEntity( float speed )
{
...
// This is often used to send all swarm elements to specific
// locations, causing the swarm to appear as though it has
// formed a specific shape.
//---------------------------------------------------------
void CSwarmElement::MoveTowardsTargetLocation( float speed )
{
...
//---------------------------------------------------------
void CSwarmElement::ReconfigureRandomParams()
{
...
//---------------------------------------------------------
void CSwarmElement::EnforceSpeedLimits( float flMinSpeed, float flMaxSpeed )
{
...
//=========================================================
//=========================================================
class CNPC_Swarm : public CAI_BaseNPC
{
	DECLARE_CLASS( CNPC_Swarm, CAI_BaseNPC );
...
public:
	...
	void	DoSwarmBatchedAI( int iStart, int iEnd );
	...
	CSwarmElement *CreateNewElement();
	...
	CUtlVector<CHandle< CSwarmElement > >m_Elements;
...
LINK_ENTITY_TO_CLASS( npc_swarm, CNPC_Swarm );
IMPLEMENT_CUSTOM_AI( npc_swarm,CNPC_Swarm );


//---------------------------------------------------------
// Save/Restore
//---------------------------------------------------------
BEGIN_DATADESC( CNPC_Swarm )
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_Swarm::InitCustomSchedules(void) 
{
	INIT_CUSTOM_AI(CNPC_Swarm);

	ADD_CUSTOM_TASK(CNPC_Swarm,		TASK_MYCUSTOMTASK);

	ADD_CUSTOM_SCHEDULE(CNPC_Swarm,	SCHED_MYCUSTOMSCHEDULE);

	ADD_CUSTOM_CONDITION(CNPC_Swarm,	COND_MYCUSTOMCONDITION);
}
...
//
//-----------------------------------------------------------------------------
void CNPC_Swarm::Precache( void )
{
	PrecacheModel( NPC_SWARM_MODEL );
	UTIL_PrecacheOther( "swarm_element" );
...
//
//-----------------------------------------------------------------------------
void CNPC_Swarm::Spawn( void )
{
	Precache();

	SetModel( NPC_SWARM_MODEL );
	...
	m_iHealth = swarm_numelements.GetInt() * 10;
	m_iMaxHealth = swarm_numelements.GetInt() * 10;
	...
	m_flMinElementDist = swarm_mindist.GetFloat();
}
...
// Output : 
//-----------------------------------------------------------------------------
Class_T	CNPC_Swarm::Classify( void )
{
...
void CNPC_Swarm::RunAI()
{
	...
	int iIdealNumElements = swarm_numelements.GetInt() * m_iHealth / m_iMaxHealth;
	...
	if( npc_swarm_show_centroid.GetBool() )
	{
		NDebugOverlay::Cross3D( m_vecCentroid + Vector( 0, 0, 12 ), 32, 0, 255, 0, false, 0.025f );
	}

	if( npc_swarm_use_threading.GetBool() )
	{
		IterRangeParallel( this, &CNPC_Swarm::DoSwarmBatchedAI, 0, m_Elements.Count() );
	}
	else
	{
		DoSwarmBatchedAI( 0, m_Elements.Count() );
	}
	...
	if( GetEnemy() != NULL )
	{
		...
		if( flEnemyDistSqr <= Square( 64.0f ) )
		{
			if( GetEnemy()->Classify() == CLASS_COMBINE )
			{
				if( !m_bEatCombineHack )
				{
					...
					swarm_radius.SetValue( 48.0f );
					RecomputeIdealElementDist();
				}
			}
			else
			{
				if (npc_swarm_do_damage.GetBool())
				{
	...
	SetNextThink( CURTIME + npc_swarm_think_interval.GetFloat() );
}

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_Swarm::GatherConditions( void )
{
	if( m_bEatCombineHack )
	{
		// We just ate someone.
...
//-----------------------------------------------------------------------------
int CNPC_Swarm::SelectSchedule( void )
{
	if( GetEnemy() == NULL )
...
//-----------------------------------------------------------------------------
void CNPC_Swarm::ComputeCentroid()
{
...
//-----------------------------------------------------------------------------
void CNPC_Swarm::DoSwarmBatchedAI( int iStart, int iEnd )
{
	float flInterval = CURTIME - GetLastThink();

	// Local fields for sin-wave movement variance
	float flMySine;
	float flAmplitude = npc_swarm_sin_amplitude.GetFloat();
	float flMyAmplitude;
	Vector vecRight;
	Vector vecForward;
	...
	float flSwarmSpeed = swarm_element_speed.GetFloat();
	float flSpeed;

	// Local fields for speed limiting
	float flMinSpeed = swarm_element_speed.GetFloat() * 0.5f;
	float flMaxSpeed = swarm_element_speed.GetFloat() * 1.5f;
	bool bEnforceSpeedLimit;
	bool bEnforceRelativePositions;
	bool bDoMovementVariation;
	bool bDoOrientation = npc_swarm_use_orientation.GetBool();
	float flIdleSpeedFactor = npc_swarm_idle_speed_factor.GetFloat();

	// Group cohesion
	float flSwarmRadiusSqr = Square( swarm_radius.GetFloat() + 48.0f ); // Four feet of fudge
	...
	{
		CSwarmElement *pThisElement = m_Elements[ i ];
		...
		case BLOB_MOVE_TO_TARGET_ENTITY:
			{
				...
				{
					if( pThisElement->GetAbsOrigin().DistToSqr( GetAbsOrigin() ) <= flSwarmRadiusSqr )
					{
		...
		default:
			Msg("ERROR: Swarm Element with unspecified Movement Rule\n");
			break;
		}
		...
		if( bEnforceRelativePositions )
		{
			for( int j = 0 ; j < m_Elements.Count() ; j++ )
			{
				...
				CSwarmElement *pThatElement = m_Elements[ j ];
				if( i != j )
				{
		...
		{
			flMySine = sin( CURTIME * pThisElement->GetSineFrequency() );
		...
		//
	//	float stragglerDist = npc_swarm_straggler_dist.GetFloat();
	//	if( pThisElement->m_flDistFromCentroidSqr > stragglerDist * stragglerDist)
	//	{
	//		pThisElement->SUB_Remove();
	//		m_Elements.Remove(pThisElement->m_iElementNumber);
	//	}
	}
}
...
//-----------------------------------------------------------------------------
void CNPC_Swarm::RemoveAllElementsExcept( int iExempt )
{
...
//-----------------------------------------------------------------------------
// Purpose: The swarm has too many elements. Locate good candidates and remove
// this many elements.
//-----------------------------------------------------------------------------
void CNPC_Swarm::RemoveExcessElements( int iNumElements )
{
...
//-----------------------------------------------------------------------------
// Purpose: This swarm has too few elements. Add this many elements by stacking
// them on top of existing elements and allowing them to disperse themselves
// into the swarm.
//-----------------------------------------------------------------------------
void CNPC_Swarm::AddNewElements( int iNumElements )
{
	int i;
	...
	for( i = 0 ; i < iNumElements ; i++ )
	{
		CSwarmElement *pElement = CreateNewElement();
		...
		if( pElement != NULL )
		{
			...
			// other element, and then be pushed out by the swarm's repel rules.
...
void CNPC_Swarm::FormShapeFromPath( string_t iszPathName )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_Swarm::SetRadius( float flRadius )
{
	swarm_radius.SetValue( flRadius );
	RecomputeIdealElementDist();
}

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_Swarm::InputFormPathShape( inputdata_t &inputdata )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_Swarm::InputSetRadius( inputdata_t &inputdata )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_Swarm::InputChaseEntity( inputdata_t &inputdata )
{
	...
	if ( pEntity )
	{
		for( int i = 0 ; i < m_Elements.Count() ; i++ )
		{
			CSwarmElement *pElement = m_Elements[ i ];
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_Swarm::InputIsolateElement( inputdata_t &inputdata )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_Swarm::InputFormHemisphere( inputdata_t &inputdata )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_Swarm::InputFormTwoSpheres( inputdata_t &inputdata )
{
	Vector center = GetAbsOrigin();
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CNPC_Swarm::ComputeBatchSize()
{
	int batchSize = m_Elements.Count() / ( 100 / swarm_batchpercent.GetInt() );
	return batchSize;
}
...
//-----------------------------------------------------------------------------
// Creates a new swarm element from scratch and adds it to the swarm
//-----------------------------------------------------------------------------
CSwarmElement *CNPC_Swarm::CreateNewElement()
{
	CSwarmElement *pElement = static_cast<CSwarmElement*>(CreateEntityByName( "swarm_element" ));
	...
	Warning("Swarm could not spawn new element!\n");
	return NULL;
}

//-----------------------------------------------------------------------------
// Create, initialize, and distribute all swarm elements
//-----------------------------------------------------------------------------
void CNPC_Swarm::InitializeElements()
{
	// Squirt all of the elements out into a circle
	int i;
	QAngle angDistributor( 0, 0, 0 );

	int iNumElements = swarm_numelements.GetInt();
	...
	for( i = 0 ; i < iNumElements ; i++ )
	{
		...
		if( !pElement )
		{
			Msg("Swarm could not create all elements!!\n");
			return;
		}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_Swarm::RecomputeIdealElementDist()
{
	float radius = swarm_radius.GetFloat();
	float area = M_PI * Square(radius);

	//Msg("Area of swarm is: %f\n", area );
