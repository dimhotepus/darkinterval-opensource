...
#ifdef DARKINTERVAL
#include "te_effect_dispatch.h" // for particle-based beams
#endif
...
#ifdef DARKINTERVAL
	PrecacheParticleSystem("pstorm_aura_electrical"); // FIXME: make separate unique art for it
	PrecacheParticleSystem("pstorm_electrical_arc_01_system_damage"); // FIXME: make separate unique art for it
#endif
}
...
void CGrenadeHomer::StartRocketTrail(void)
{
	...
	if(pRocketTrail)
	{
		...
		{
#ifdef DARKINTERVAL
			pRocketTrail->m_StartColor.Init(0.8, 0.0, 0.8);
			pRocketTrail->m_EndColor.Init(0.1, 0.0, 0.4);

			// TEMP: for particle storm guided projectiles
			// FIXME: make unique art for this
			DispatchParticleEffect("pstorm_electrical_arc_01_system_damage", PATTACH_ABSORIGIN_FOLLOW, this);
#else
...
#ifndef DARKINTERVAL
	UTIL_ScreenShake( GetAbsOrigin(), 5.0f, 150.0f, 0.5f, 750, SHAKE_START );
#else
...
{
	if (CURTIME > m_flNextFlySoundTime)
	{
		...
		m_flNextFlySoundTime	= CURTIME + 1.0;
	}
}
...
{ 
	...
	if (m_hTarget != NULL)
	{
		...
		// Delay
		// ---------
		if		(CURTIME < flHomingRampUpStartTime)
		{
		...
		//  Ramp Up
		// ----------
		else if (CURTIME < flHomingSustainStartTime)
		{
			float flAge			= CURTIME - flHomingRampUpStartTime;
		...
		//  Sustain
		// ----------
		else if (CURTIME < flHomingRampDownStartTime)
		{
		...
		//  Ramp Down
		// -----------
		else if (CURTIME < flHomingEndHomingTime)
		{
			float flAge			= CURTIME - flHomingRampDownStartTime;
...
#ifdef DARKINTERVAL
				|| m_nRocketTrailType == HOMER_SMOKE_TRAIL_ALIEN
#endif
				)
			{
	...
	{
		vecImpulse.x += m_flSpinMagnitude*sin(m_flSpinSpeed * CURTIME + m_flSpinOffset);
		vecImpulse.y += m_flSpinMagnitude*cos(m_flSpinSpeed * CURTIME + m_flSpinOffset);
		vecImpulse.z -= m_flSpinMagnitude*cos(m_flSpinSpeed * CURTIME + m_flSpinOffset);
	}
	...
	if( CURTIME > m_flNextWarnTime )
	{
		...
		NDebugOverlay::Line( vecWarnLocation, vecWarnLocation - vecRight * flSpeed * WARN_INTERVAL * 0.5, 255,255,0, true, 10);
#endif
		m_flNextWarnTime = CURTIME + WARN_INTERVAL;
	}
#endif // BUBBLE
