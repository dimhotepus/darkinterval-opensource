...
bool CPropAirboat::ShouldDrawWaterImpacts( void )
{
	...
	// of the impact effects. Also, we need to do fewer of them.
	if ( m_flLastImpactEffectTime >= CURTIME )
		return false;

	m_flLastImpactEffectTime = CURTIME + 0.05f;
...
#ifdef DARKINTERVAL
		if ( m_iNumberOfEntries < 10 && !m_bExitAnimOn )
#else
...
void CPropAirboat::UpdateEngineSound( CSoundEnvelopeController &controller, float speedRatio )
{
	...
	{ 
		if ( CURTIME > m_flEngineIdleTime )
		{
			...
			m_flEngineDuckTime = CURTIME + ENGINE_DUCK_TIME;
		}
	}
	else if ( CURTIME > m_flEngineDuckTime )
	{
	...
	else
	{
		...
		if (speedRatio <= 0.1)
		{
			if (!controller.SoundGetVolume(m_pWaterStoppedSound))
			{
				...
				m_flWaterStoppedPitchTime = CURTIME + random->RandomFloat(1.0, 3.0);
			}
			else if (CURTIME > m_flWaterStoppedPitchTime)
			{
				...
				m_flWaterStoppedPitchTime = CURTIME + random->RandomFloat(2.0, 4.0);
			}
		}
		else
		{
...
void CPropAirboat::FireGun( )
{
	...
	*/
	...
	if ( CURTIME >= m_flNextHeavyShotTime )
	{
		m_flNextHeavyShotTime = CURTIME + CANNON_HEAVY_SHOT_INTERVAL; 
	}

	if ( CURTIME >= m_flNextGunShakeTime )
	{
		UTIL_ScreenShakeObject( this, WorldSpaceCenter(), 0.2, 250.0, CANNON_SHAKE_INTERVAL, 250, SHAKE_START );
		m_flNextGunShakeTime = CURTIME + 0.5 * CANNON_SHAKE_INTERVAL; 
	}
	...
	CAPCMissileNPC *pEnt = FindAPCMissileInCone( vecEyePosition, vecEyeDirection, 2.5f );
...
void CPropAirboat::DampenUpMotion( Vector &vecVehicleEyePos, QAngle &vecVehicleEyeAngles, float flFrameTime )
{
	...
	vecVehicleEyePos.z += 0.0f * vecUp.z;
...
#ifndef DARKINTERVAL
	if ( airboat_fatal_stress.GetFloat() > 0 ) // it's 5000, it's always > 0
#endif
	{
...
#ifdef DARKINTERVAL
	if ( stressOut.hasNonStaticStress && ( stressOut.receivedStress > 5000 ) )
#else
