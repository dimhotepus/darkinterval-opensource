...
// DI todo - add functional primary/secondary weapon system and switching
// and health items pickup (fully AI based)
// Also finalise cops occasionally dropping detached masks on death, revealing
// the face
//=============================================================================//
...
#ifdef DARKINTERVAL
#include "ammodef.h" // for buckshort resistance in OnTakeDamage_Alive(...)
#endif
#include "basegrenade_shared.h"
#include "globalstate.h"
#ifdef DARKINTERVAL
#include "grenade_frag.h" // for smoke grenades
#endif
...
#ifdef DARKINTERVAL
#define	METROPOLICE_BODYGROUP_GEAR	1
#else
...
#ifdef DARKINTERVAL // for picking up health
...
#ifdef DARKINTERVAL
ConVar  metropolice_use_physics_cover_proto_code("metropolice_use_physics_cover_proto_code", "0");
#endif
...
Activity ACT_THROW_GRENADE;
#endif
LINK_ENTITY_TO_CLASS( npc_metropolice, CNPC_MetroPolice );
#ifdef DARKINTERVAL
LINK_ENTITY_TO_CLASS( monster_metropolice, CNPC_MetroPolice );
#endif
...
{
#ifdef DARKINTERVAL
	m_hNPCFileInfo = LookupNPCInfoScript("npc_metropolice");
	m_vecTossVelocity = vec3_origin;
	m_bDontPickupWeapons = true; // default to true if no keyvalue specified by mapper (old map, etc)
#endif
}
...
{
#ifdef DARKINTERVAL // NPC script-based model selection
	const char *pModelName = GetNPCScriptData().NPC_Model_Path_char;

	SetModelName(MAKE_STRING(pModelName));

	if ( IsElite() ) // DI todo - this being hardcoded defeats the purpose of a script somewhat!
	{
		SetModelName(AllocPooledString("models/_Monsters/Combine/metropolice_elite.mdl"));
	}

	PrecacheModel(STRING(GetModelName()));
	PrecacheModel("models/_Monsters/Combine/metropolice_faceplate.mdl"); // for throwing out a mask "gib"

	UTIL_PrecacheOther("npc_manhack");
	UTIL_PrecacheOther("weapon_frag");

	PrecacheScriptSound("NPC_Metropolice.GrenadeLaunch");
...
void CNPC_MetroPolice::Spawn(void)
{
	...
	if ( ( GetNPCScriptData().NPC_Capable_MeleeAttack1_boolean ) == true )
	{
		if ( SelectWeightedSequence(ACT_MELEE_ATTACK1) != ACT_INVALID )
			CapabilitiesAdd(bits_CAP_INNATE_MELEE_ATTACK1);
	}

	if ( ( GetNPCScriptData().NPC_Capable_MeleeAttack2_boolean ) == true )
	{
		if ( SelectWeightedSequence(( Activity )ACT_KICK_PLAYER) != ACT_INVALID )
	...
	// ladder climb test
	if ( ( GetNPCScriptData().NPC_Capable_Climb_boolean ) == true )
	{
		if ( SelectWeightedSequence(ACT_CLIMB_UP) != ACT_INVALID
			&& SelectWeightedSequence(ACT_CLIMB_DOWN) != ACT_INVALID
			&& SelectWeightedSequence(ACT_CLIMB_DISMOUNT) != ACT_INVALID )
	...
	m_hManhack = NULL;
#ifdef DARKINTERVAL // for CH02 ArcadeExterior
	if ( NameMatches("mp_suitcase_thrower") )
	{
		// Give the citizen a package
		CBaseCombatWeapon *pWeapon = Weapon_Create("weapon_citizensuitcase");
		if ( pWeapon )
		{
	...
	if ( GetActiveWeapon() )
	{
...
#ifdef DARKINTERVAL
		// DI edit. Revolver cops are more fair if they have
		// to stop in order to aim and shoot.
		...
		{
			AddSpawnFlags(SF_NPC_NO_WEAPON_DROP); // DI HAAACK.
		}
#endif
	}
...
#ifndef DARKINTERVAL 
			RemoveSpawnFlags(SF_METROPOLICE_ALWAYS_STITCH); // don't do that. Trust the level designer to set these in a right way.
#endif
		}
	}
	...
	if ( IsElite() )
	{
		//	m_iGearType = 1; // replace manhacks with grenades
		if ( m_iGearAmount == 0 ) m_iGearAmount = 1; // start with at least one
	}

	// Turn on gear bodygroup
	if ( m_iGearAmount > 0 && !IsElite() )
	{
		SetBodygroup(METROPOLICE_BODYGROUP_GEAR, true);
	}
#else
	...
	m_flNextGrenadeCheck = CURTIME;

	m_PanicSentencesTimer.Set(1);
#endif
}
#ifdef DARKINTERVAL
// Thinks
void CNPC_MetroPolice::NPCThink()
{
	BaseClass::NPCThink();

	if ( m_NPCState == NPC_STATE_IDLE || m_NPCState == NPC_STATE_ALERT && !IsFlaggedEfficient() )
	{
		CBasePlayer *player = UTIL_GetLocalPlayer();
		if ( player )
		{
			//	if( FVisible(player) && DotProduct(EyeDirection3D(), player->GetAbsOrigin()) <= m_flFieldOfView)
			if ( FInViewCone(player->EyePosition()) )
				m_OnPlayerInViewcone.FireOutput(this, this);
		}
	}
}
#endif
...
void CNPC_MetroPolice::PrescheduleThink( void )
{
	...
	// DI NEW. Metrocops who have stunstick while player has guns, and who fear player,
	// will be uttering panicing sentences on simple timer.
	if ((HasBaton() || !GetActiveWeapon()) && pPlayer != NULL && FindEntityRelationship(pPlayer)->disposition == D_FR && pPlayer == GetEnemy() && ThisNPCInPVS())
	{
		if (m_PanicSentencesTimer.Expired())
		{
			m_Sentences.Speak("METROPOLICE_DANGER_FEARPLAYER", SENTENCE_PRIORITY_HIGH, SENTENCE_CRITERIA_NORMAL);
			m_PanicSentencesTimer.Set(RandomFloat(5, 10));
		}
	}
#endif
	...
	if ( PlayerIsCriminal() == false && !IsInAScript() )
	{		
		if ( pPlayer && ( pPlayer->WorldSpaceCenter() - WorldSpaceCenter() ).LengthSqr() < (128*128) )
		{
			...
			{
				m_flBatonDebounceTime = CURTIME + random->RandomFloat( 2.5f, 4.0f );
				SetTarget( pPlayer );
				SetBatonState( true );
			}
		}
		...
		{
			if ( m_PolicingBehavior.IsEnabled() == false && CURTIME > m_flBatonDebounceTime )
			{
				SetBatonState( false );
			}
	...
	if (IsCoveredInAcid())
	{
		SetCondition(COND_METROPOLICE_COVERED_IN_ACID);
	}
	else
	{
		ClearCondition(COND_METROPOLICE_COVERED_IN_ACID);
	}
#endif
	if( IsOnFire() )
	{
	...
	if (CURTIME > m_flRecentDamageTime + RECENT_DAMAGE_INTERVAL)
	{
		m_nRecentDamage = 0;
...
//-----------------------------------------------------------------------------
void CNPC_MetroPolice::Weapon_Equip( CBaseCombatWeapon *pWeapon, bool bSwitchToNewWeapon)
{
	BaseClass::Weapon_Equip( pWeapon, bSwitchToNewWeapon);
...
void CNPC_MetroPolice::PickupItem(CBaseEntity *pItem)
{
	...
	{
		DevMsg("Combine Soldier doesn't know how to pick up %s!\n", pItem->GetClassname());
	}
}
...
//-----------------------------------------------------------------------------
void CNPC_MetroPolice::InputEnableWeaponPickup(inputdata_t &inputdata)
{
...
//-----------------------------------------------------------------------------
void CNPC_MetroPolice::InputDisableWeaponPickup(inputdata_t &inputdata)
{
	m_bDontPickupWeapons = true;
}

// DI NEW: override this boolean for metrocops. Use general rules except !IsPlayerAlly() && GetActiveWeapon() one.
bool CNPC_MetroPolice::ShouldLookForBetterWeapon()
{
	if (m_bDontPickupWeapons)
		return false;

	if (m_flNextWeaponSearchTime > CURTIME)
		return false;
...
void CNPC_MetroPolice::SpeakStandoffSentence( int nSentenceType )
{
	...
	case STANDOFF_SENTENCE_STAND_CHECK_TARGET:
		if ( gm_flTimeLastSpokePeek != 0 && CURTIME - gm_flTimeLastSpokePeek > 20 )
		{
			m_Sentences.Speak( "METROPOLICE_SO_PEEK" );
			gm_flTimeLastSpokePeek = CURTIME;
		}
		break;
	}
}
...
void CNPC_MetroPolice::SpeakSentence( int nSentenceType )
{
	...
	case METROPOLICE_SENTENCE_DEPLOY_MANHACK:
		...
		// DI todo - add a sentence for smoke grenade throwing?
...
#ifdef DARKINTERVAL // Part 2.1 patch balancing
const float MIN_MIN_PISTOL_REST_INTERVAL = 1.0;
const float MAX_MIN_PISTOL_REST_INTERVAL = 1.5;

// Range for rest period maximums
const float MIN_MAX_PISTOL_REST_INTERVAL = 1.5;
...
const int 	MAX_MAX_PISTOL_BURST = 5;
#else
// Range for rest period minimums
...
void CNPC_MetroPolice::SetBurstMode( bool bEnable )
{
	...
	m_flBurstPredictTime = CURTIME - 1.0f;
	if ( GetActiveWeapon() )
	{
...
#ifdef DARKINTERVAL   // Expected to be float.
	...
	case BURST_STEER_ADJUST_FOR_SPEED_CHANGES:
		{
			...
			if ( m_flBurstPredictTime <= CURTIME )
				return;

			float flPredictTime = m_flBurstPredictTime - CURTIME;
...
#ifdef DARKINTERVAL
void CNPC_MetroPolice::InputForceManhackToss(inputdata_t &inputdata)
{
	if (IsElite()) return;
	m_iGearAmount++;
//	SetBodygroup(METROPOLICE_BODYGROUP_GEAR, true);
//	SetIdealActivity(Activity(ACT_METROPOLICE_DEPLOY_MANHACK));
	ClearSchedule("forced to do scripted manhack toss");
	SetSchedule(SCHED_METROPOLICE_DEPLOY_MANHACK);
}

void CNPC_MetroPolice::InputFakeWeaponSwitch(inputdata_t &inputdata)
{
	//	SetBodygroup(METROPOLICE_BODYGROUP_GEAR, true);
	//	SetIdealActivity(Activity(ACT_METROPOLICE_DEPLOY_MANHACK));
	ClearSchedule("forced to do fake weapon switch");
	SetSchedule(SCHED_METROPOLICE_SWITCH_WEAPONS);
}
#endif
//-----------------------------------------------------------------------------
// Purpose: 
...
void CNPC_MetroPolice::LostEnemySound( void)
{
	...
	if ( CURTIME <= m_flNextLostSoundTime )
		return;

	const char *pSentence;
	if (!(CBaseEntity*)GetEnemy() || CURTIME - GetEnemyLastTimeSeen() > 10)
	{
	...
	{
		m_flNextLostSoundTime = CURTIME + random->RandomFloat(5.0,15.0);
	}
}
...
void CNPC_MetroPolice::PainSound( const CTakeDamageInfo &info )
{
	...
	if ( healthRatio > 0.0f )
	{
		...
		m_flNextPainSoundTime = CURTIME + 1;
	}
}
...
int CNPC_MetroPolice::GetSoundInterests( void )
{
	return SOUND_WORLD | SOUND_COMBAT | SOUND_PLAYER | SOUND_PLAYER_VEHICLE | SOUND_DANGER | 
		SOUND_PHYSICS_DANGER | SOUND_BULLET_IMPACT | SOUND_MOVE_AWAY | SOUND_HIDE_POSITION;
}
...
Disposition_t CNPC_MetroPolice::IRelationType(CBaseEntity *pTarget)
{
	...
	int prio = IRelationPriority(pTarget); // take our individual value, not base
	...
	{
		// if it's that high, we'll hate him no matter what.
		if (prio > 999 && disp == D_HT)
			return D_HT; 

		if ( !PlayerIsCriminal() && (disp == D_HT) && prio < 999 )
		{
			// If we're pissed at the player, we're allowed to hate them.
			if ( m_flChasePlayerTime && m_flChasePlayerTime > CURTIME )
				return D_HT;
			return D_NU;
		}
	}

	if (pTarget->Classify() == CLASS_CITIZEN_PASSIVE)
		return D_NU; // do NOT attack normal citizens
...
void CNPC_MetroPolice::OnAnimEventStartDeployManhack( void )
{
	...
	if ( m_iGearAmount <= 0 )
	{
		SetBodygroup( METROPOLICE_BODYGROUP_GEAR, false );
	}
...
{
	CBaseEntity *pHurt = CheckTraceHullAttack( 50, -Vector(16, 16, 32), Vector(16, 16, 32), 0, DMG_CLUB, 2.0f, false );
	...
	if ( pHurt )
	{
		...
		kickInfo.SetDamage(15);
		kickInfo.SetDamageType(DMG_CLUB);
		kickInfo.ScaleDamageForce(4.0f);
		kickInfo.SetAttacker(this);
		kickInfo.SetInflictor(this);
		CalculateMeleeDamageForce(&kickInfo, vecForceDir, WorldSpaceCenter(), 4.0f);

		if (!pHurt->IsAlive())
		{
			// if we're kicking neither an NPC nor player, and it's a prop instead, 
			// and this prop explodes on impact, then only apply kick force, NO damage.
			CPhysicsProp *pProp = dynamic_cast<CPhysicsProp*>(pHurt);
			if (pProp)
			{
				if (pProp->HasInteraction(PROPINTER_PHYSGUN_BREAK_EXPLODE_IGNITE))
				{
		...
		{
			//pHurt->TakeDamage(kickInfo); // this is where shoving NPCs would go, but we don't want to deal damage here because we have scripted sequences with it.
		}
#endif
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_MetroPolice::OnAnimEventKick(void)
{
	CBaseEntity *pHurt = CheckTraceHullAttack(50, -Vector(16, 16, 32), Vector(16, 16, 32), 0, DMG_CLUB, 4.0f, false);
	...
	if (pHurt)
	{
		...
		else if (pHurt->IsPlayer())
		{
			...
			if (pPlayer != NULL)
			{
				...
				if( PlayerIsCriminal())	pPlayer->TakeDamage(kickInfo);
			}
		}
		else
		{
			pHurt->TakeDamage(kickInfo);
		}
		// Play a random attack hit sound
		EmitSound("NPC_Metropolice.Shove");
	}
...
#ifdef DARKINTERVAL 
	// Kick the bastard!
	if (pEvent->event == AE_METROPOLICE_KICK)
	{
		OnAnimEventKick();
		return;
	}
...
#ifdef DARKINTERVAL 
	if (pEvent->event == AE_METROPOLICE_GRENADETHROW) // DI todo - redo throws as rolls
	{
		Vector vecSpin;
		...
		vecThrow = m_vecTossVelocity; //forward * 150 + up * 50;
		Fraggrenade_Create(vecStart, vec3_angle, vecThrow, vecSpin, this, 2.0f, true, FRAGGRENADE_TYPE_SMOKE);
		EmitSound("NPC_Metropolice.GrenadeLaunch");
		m_Sentences.Speak("METROPOLICE_SO_FORCE_COVER");

		m_iGearAmount--;

		// wait thirty seconds before even looking again to see if a grenade can be thrown.
		m_flNextGrenadeCheck = CURTIME + 30.0f;
...
bool CNPC_MetroPolice::HandleInteraction(int interactionType, void *data, CBaseCombatCharacter* sourceEnt)
{
	if ( interactionType == g_interactionMetrocopStartedStitch )
	{
		...
		m_flValidStitchTime = CURTIME + random->RandomFloat( METROPOLICE_SQUAD_STITCH_MIN_INTERVAL, METROPOLICE_SQUAD_STITCH_MAX_INTERVAL );
		return true;
	}
	...
	if ( interactionType == g_interactionHitByPlayerThrownPhysObj )
	{
		...
		// See if the object is the cupcop can. If so, fire the output
...
Activity CNPC_MetroPolice::NPC_TranslateActivity( Activity newActivity )
{
	...
	{
		return (Activity)ACT_THROW_GRENADE;
	}

	if (newActivity == ACT_ARM)
	{
		return (Activity)ACT_METROPOLICE_DRAW_PISTOL;
	}

	if (newActivity == ACT_MELEE_ATTACK_SWING && RandomFloat(0,1) > 0.6)
	{
		return (Activity)ACT_MELEE_ATTACK_THRUST;
	}
#endif
	...
	if ( newActivity == ACT_PUSH_PLAYER )
	{
		if (m_nNumWarnings >= METROPOLICE_MAX_WARNINGS)
		{
			...
			if (SelectWeightedSequence((Activity)ACT_KICK_PLAYER) == ACT_INVALID)
				...
				return HasBaton() ? ACT_MELEE_ATTACK1 : (Activity)ACT_KICK_PLAYER;
#endif
		}
	}
#ifdef DARKINTERVAL 
	// Try anything.
	if (newActivity == ACT_MELEE_ATTACK1 && !GetActiveWeapon())
	{
		if (RandomFloat(0, 1) > 0.5)
			return (Activity)ACT_KICK_PLAYER;
		else
			return	ACT_MELEE_ATTACK_SWING;
	}

	// Kicking is our melee attack 2. // TODO: Add more? soldier-style elbow kick?
	if (newActivity == ACT_MELEE_ATTACK2)
	{
		return (Activity)ACT_KICK_PLAYER;
	}
...
#ifdef DARKINTERVAL
	m_hManhack->SetCondition(COND_LIGHT_DAMAGE); // force it to unpack
#endif
...
#ifdef DARKINTERVAL // DI todo - finalise
//	SetModel("models/_Monsters/Combine/metropolice_unmasked_eric.mdl");
	
//	SetBodygroup(2, 1);
//	CGib::SpawnSpecificGibs(this, 1, 150, 100, "models/_Monsters/Combine/metropolice_faceplate.mdl", 6000.0f, 0);
#endif
	BaseClass::Event_Killed( info );
}
#ifdef DARKINTERVAL
//-----------------------------------------------------------------------------
// Purpose: Makes the metrocop fire on the dead enemy for a little while more.
// Input  : 
// Output :
//-----------------------------------------------------------------------------
void CNPC_MetroPolice::Event_KilledOther(CBaseEntity *pVictim, const CTakeDamageInfo &info)
{
	...
	{
		//	AnnounceEnemyKill(GetEnemy()); // DI todo - implement
	}
...
int CNPC_MetroPolice::SelectScheduleNewEnemy()
{
	...
	{
		m_flNextLedgeCheckTime = CURTIME;
...
int CNPC_MetroPolice::SelectScheduleRetrieveItem()
{
	if (HasCondition(COND_BETTER_WEAPON_AVAILABLE))
	{
		...
		if (pWeapon)
		{
			m_flNextWeaponSearchTime = CURTIME + 10.0;
...
//-----------------------------------------------------------------------------
// Override base class because we want UnholsterWeapon equate to forcing
// "weapondrawn 1" for the metrocop.
//-----------------------------------------------------------------------------
int CNPC_MetroPolice::UnholsterWeapon(void)
{
	m_fWeaponDrawn = true;
...
bool CNPC_MetroPolice::OnObstructionPreSteer( AILocalMoveGoal_t *pMoveGoal, float distClear, AIMoveResult_t *pResult )
{
	if ( pMoveGoal->directTrace.pObstruction )
	{
		...
		if ( pProp && (pProp->GetHealth() || pProp->GetMass() < 20 ))
		{
			m_hBlockingProp = pProp;
		}
		else
		{
#ifdef DARKINTERVAL // Check if it's a dynamic prop - we often use those for fixed but breakable objects.
			CDynamicProp *pPropD = dynamic_cast<CDynamicProp*>(pMoveGoal->directTrace.pObstruction); 
			if (pPropD && pPropD->GetHealth())
			{
				m_hBlockingProp = pPropD;
			}
			else
#endif
			{
...
// Output :
//-----------------------------------------------------------------------------
int CNPC_MetroPolice::MeleeAttack2Conditions(float flDot, float flDist)
{
	// Only kick the player
	if (GetEnemy() && GetEnemy()->IsPlayer() == false)
		return COND_NONE;

	// For precriminal, kicking is instead handled as a substitute of pushing animation.
	if (!PlayerIsCriminal())
		return COND_NONE;
...
// Replaces the baseclass check, because that one goes
// through the weapon's anim table, and cops and soldiers
// fail check against some of the anims.
// Since we know which weapons we'll be giving them, approve
// those ones always.
bool CNPC_MetroPolice::Weapon_CanUse(CBaseCombatWeapon *pWeapon)
{
	if (pWeapon->ClassMatches("weapon_stunstick") ||
		pWeapon->ClassMatches("weapon_crowbar") ||
		pWeapon->ClassMatches("weapon_pistol") ||
		pWeapon->ClassMatches("weapon_357") ||
		pWeapon->ClassMatches("weapon_flaregun") ||
		pWeapon->ClassMatches("weapon_smg1") ||
		pWeapon->ClassMatches("weapon_smg2") ||
		pWeapon->ClassMatches("weapon_shotgun") ||
		pWeapon->ClassMatches("weapon_annabelle") ||
		pWeapon->ClassMatches("weapon_ar1")) return true;
	return false;
}
#endif // DARKINTERVAL
//-----------------------------------------------------------------------------
...
{
#ifdef DARKINTERVAL // some cops which aren't properly armed against Freeman, fear him in DI
	...
	// If you can't attack because there's a physics object in front of you, swat it
#ifdef DARKINTERVAL // batoning activity will be translated into a kick if no baton owned
	if ( m_hBlockingProp )
#else
...
int CNPC_MetroPolice::SelectCombatSchedule()
{
	...
	if (HasCondition(COND_CAN_MELEE_ATTACK2)) // kick
	{
	...
	if ( HasCondition( COND_CAN_RANGE_ATTACK1 ) )
	{
		...
		if (IsInAScript()) return SCHED_RANGE_ATTACK1;
		else
#endif
		{
	...
	if ( HasCondition( COND_LOW_PRIMARY_AMMO ) || HasCondition( COND_NO_PRIMARY_AMMO ) && !HasCondition(COND_CAN_MELEE_ATTACK2))
	{
		AnnounceOutOfAmmo( );
	...
	if ( HasCondition(COND_WEAPON_SIGHT_OCCLUDED) && !HasBaton() )
	{
		...
		if ( pBlocker && ( pBlocker->GetHealth() > 0 || pBlocker->GetMoveType() != MOVETYPE_NONE) && OccupyStrategySlotRange( SQUAD_SLOT_POLICE_ATTACK_OCCLUDER1, SQUAD_SLOT_POLICE_ATTACK_OCCLUDER2 ) )
		{
	...
	if (HasCondition(COND_ENEMY_OCCLUDED))
	{
		if ( GetEnemy() && !(GetEnemy()->GetFlags() & FL_NOTARGET) )
		{
			...
			if (nResult != SCHED_NONE) // Charge in and break the enemy's cover!
#endif
...
float CNPC_MetroPolice::StitchAtWeight( float flDist, float flSpeed, float flDot, float flReactionTime, const Vector &vecTargetToGun )
{
	...
	if ( m_flValidStitchTime > CURTIME )
		return 0.0f;
...
int CNPC_MetroPolice::SelectMoveToLedgeSchedule()
{
	...
	if ( m_flNextLedgeCheckTime > CURTIME )
		return SCHED_NONE;
	...
	{
		m_flNextLedgeCheckTime = CURTIME + 3.0f;
...
{
	// This condition is needed so that gestures
	// don't interrupts walking (which makes cops
	// slide on ground while flinching), or rappeling
	// which just overrides their rappel anim with 
	// flinch. And it's easier than going thru anims
	// adding weightlists. 
#ifdef DARKINTERVAL
	if( !IsMoving() && GetGroundEntity())
#endif
		BaseClass::PlayFlinchGesture();

#ifndef DARKINTERVAL // DI doesn't want this
	// To ensure old playtested difficulty stays the same, stop cops shooting for a bit after gesture flinches
	GetShotRegulator()->FireNoEarlierThan( CURTIME + 0.5 );
#endif
}
...
void CNPC_MetroPolice::IncrementPlayerCriminalStatus( void )
{
	...
	m_flBatonDebounceTime = CURTIME + random->RandomFloat( 2.0f, 4.0f );
...
int CNPC_MetroPolice::SelectShoveSchedule( void )
{
	...
	// Stop chasing the player now that we've taken a swing at them
	m_flChasePlayerTime = 0;
	return SCHED_METROPOLICE_SHOVE; // it translates into a kick for cops without stunsticks
}
...
{
#ifndef DARKINTERVAL // DI is singleplayer only
	...
	if ( !IsInAScript() && (m_NPCState != NPC_STATE_SCRIPT) && HasSpawnFlags( SF_METROPOLICE_ALLOWED_TO_RESPOND ) )
	{
...
#endif
		{
			...
			m_flChasePlayerTime = CURTIME + RandomFloat(3, 7);
		}
...
int CNPC_MetroPolice::SelectSchedule( void )
{
...
#ifdef DARKINTERVAL
	if (HasCondition(COND_METROPOLICE_COVERED_IN_ACID))
	{
		m_Sentences.Speak("METROPOLICE_ON_FIRE", SENTENCE_PRIORITY_INVALID, SENTENCE_CRITERIA_ALWAYS);
		return SCHED_METROPOLICE_ACID_STAND;
	}

	if (m_hForcedGrenadeTarget)
	{
		if (m_flNextGrenadeCheck < CURTIME)
		{
	...
	{
		if ( m_flLastPhysicsFlinchTime + 4.0f <= CURTIME )
		{
			m_flLastPhysicsFlinchTime = CURTIME;
	...
	if ( m_NPCState == NPC_STATE_COMBAT && m_flChasePlayerTime > CURTIME && HasBaton() )
...
int CNPC_MetroPolice::SelectFailSchedule( int failedSchedule, int failedTask, AI_TaskFailureCode_t taskFailCode )
{
...
#ifdef DARKINTERVAL
	if (failedSchedule == SCHED_NEW_WEAPON)
	{
		...
		m_flNextWeaponSearchTime = CURTIME + 1.0f;
	}
	...
	if (failedSchedule == SCHED_HIDE_AND_RELOAD || failedSchedule == SCHED_TAKE_COVER_FROM_ENEMY)
	{
...
int CNPC_MetroPolice::TranslateSchedule( int scheduleType )
{
	...
	case SCHED_ALERT_STAND:
	{
		if (!GetEnemy() 
			&& GetActiveWeapon() 
			&& GetActiveWeapon()->CapabilitiesGet() & bits_CAP_WEAPON_RANGE_ATTACK1
			&& GetActiveWeapon()->m_iClip1 < (GetActiveWeapon()->GetMaxClip1())
			&& !HasBaton())
		{
			// If we're not doing anything, reload.
			// This helps with situations when the combine just killed
			// an enemy, another appears, they shoot it a couple times, 
			// and need to reload already.
			return SCHED_RELOAD;
		}
	}
	break;
	...
	case SCHED_METROPOLICE_ADVANCE:
		if (m_NextChargeTimer.Expired() && metropolice_charge.GetBool())
		{
			...
			{
				if (GetEnemy() && GetEnemy()->GetAbsOrigin().DistToSqr(GetAbsOrigin()) > 200 * 200)
				{
					if (OccupyStrategySlot(SQUAD_SLOT_POLICE_CHARGE_ENEMY))
					{
						m_NextChargeTimer.Set(6, 12);
						return SCHED_METROPOLICE_CHARGE;
					}
				}
			}
	...
	case SCHED_TAKE_COVER_FROM_ENEMY:
		{
			if (metropolice_use_physics_cover_proto_code.GetBool())
				return SCHED_METROPOLICE_FIND_BEST_PHYSICS_COVER;
			else
				return SCHED_TAKE_COVER_FROM_ENEMY;
		}
		break;

	case SCHED_METROPOLICE_DEPLOY_MANHACK:
		{
			if (IsElite())
			{
				return SCHED_METROPOLICE_FORCED_GRENADE_THROW;
...
#ifdef DARKINTERVAL
void CNPC_MetroPolice::InputThrowGrenadeAtTarget(inputdata_t &inputdata)
{
...
void CNPC_MetroPolice::StartTask( const Task_t *pTask )
{
	...
	case TASK_METROPOLICE_ARREST_ENEMY:
		m_flTaskCompletionTime = CURTIME + pTask->flTaskData;
		break;
	...
	case TASK_METROPOLICE_GET_PATH_TO_BESTSOUND_LOS:
		...
		break;
#ifdef DARKINTERVAL
	case TASK_METROPOLICE_SELECT_BEST_PHYSICS_COVER:
	{
		Vector pResult;
	//	CBaseEntity pTarget = NULL;
		if (BaseClass::FindPhysicsCoverInRadius(GetEnemy(), GetLocalOrigin(), &pResult/*, &pTarget*/))
		{
			AI_NavGoal_t goal(pResult, ACT_RUN, AIN_DEF_TOLERANCE);
	//		goal.pTarget = &pTarget;
		...
		{
			Msg("Found no physics cover!\n");
			TaskFail(FAIL_NO_COVER);
		}
	}
	break;

	case TASK_MP_FACE_TOSS_DIR:	break;

	case TASK_MP_GET_PATH_TO_FORCED_GREN_LOS:
	{
		if (!m_hForcedGrenadeTarget)
		{
	...
	case TASK_MP_THROW_SMOKE:
	{
		if (!IsElite())
		{
			TaskComplete("non-elite cop can't throw smokes");
			return;
		}
		if (!ShouldThrowSmokeGrenade())
		{
			Msg("%s: wanted to throw a smoke, didn't meet conditions!\n", GetEntityName());
			TaskComplete("bad conditions for smoke throw"); // just go right to next step in the schedule
			return;
		}
		...
		{
			m_Sentences.Speak("METROPOLICE_SO_FORCE_COVER");
			Msg("%s: throwing smoke grenade against player!\n", GetEntityName());
			SetActivity(ACT_THROW_GRENADE);
			TaskComplete();
		}
	}
	break;

	case TASK_MP_SWITCH_WEAPONS:
	{		
		TaskComplete();
	}
	break;
...
void CNPC_MetroPolice::RunTask(const Task_t *pTask)
{
	...
	case TASK_METROPOLICE_ARREST_ENEMY:
	{
		...
		if (CURTIME >= m_flTaskCompletionTime)
		{
			TaskComplete();
			break;
		}
...
#ifdef DARKINTERVAL
//-----------------------------------------------------------------------------
// Balance it for a bit more challenging fights
//-----------------------------------------------------------------------------
float CNPC_MetroPolice::GetHitgroupDamageMultiplier(int iHitGroup, const CTakeDamageInfo &info)
{
	...
	else if (iHitGroup == HITGROUP_CHEST || iHitGroup == HITGROUP_STOMACH)
	{
		return 0.85f;
	}
...
//-----------------------------------------------------------------------------
#ifdef DARKINTERVAL
#define METROCOP_SCORCH_RATE	6
#define METROCOP_SCORCH_FLOOR	75
#endif
int CNPC_MetroPolice::OnTakeDamage_Alive( const CTakeDamageInfo &inputInfo )
{
	...
	if (info.GetAttacker() == GetEnemy())
	{
		...
		m_flRecentDamageTime = CURTIME;
	}
#ifdef DARKINTERVAL
	if (!HasSpawnFlags(SF_METROPOLICE_SIMPLE_VERSION))
	{
		if (info.GetAmmoType() == GetAmmoDef()->Index("Buckshot")) info.ScaleDamage(0.8f);
	}

	if ((info.GetDamageType() & DMG_BURN) && (info.GetDamageType() & DMG_DIRECT))
	{
		Scorch(METROCOP_SCORCH_RATE, METROCOP_SCORCH_FLOOR);
	}
#endif
...
{
#ifdef DARKINTERVAL
	if (IsElite() &&  CURTIME < m_flNextGrenadeCheck)
		return false;
#endif
	if ( HasSpawnFlags( SF_METROPOLICE_NO_MANHACK_DEPLOY ) )
		return false;
...
bool CNPC_MetroPolice::CheckCanThrowGrenade(const Vector &vecTarget)
{
	...
	if (vecTarget.DistTo(GetAbsOrigin()) < 200) // if aiming at our feet or otherwise close, then just throw slightly in front of ourselves.
	{
		Vector forward;
		GetVectors(&forward, NULL, NULL);
		vecToss = forward * vecTarget.DistTo(GetAbsOrigin());
	}
	else
	{
		if (FInViewCone(vecTarget) && CBaseEntity::FVisible(vecTarget))
		{
			vecToss = VecCheckThrow(this, EyePosition(), vecTarget, 300, 1.0, &vecMins, &vecMaxs);
		}
		else
		{
	...
	if (vecToss != vec3_invalid)
	{
		m_vecTossVelocity = vecToss;

		// don't check again for a while.
		m_flNextGrenadeCheck = CURTIME + 1; // 1/3 second.
		return true;
	}
	else
	{
		// don't check again for a while.
		m_flNextGrenadeCheck = CURTIME + 1; // one full second.
		return false;
	}
}

bool CNPC_MetroPolice::ShouldThrowSmokeGrenade(void)
{
//	if (m_iGearAmount < 1) return false; // spent all our smokes or simply didn't have any
	if (!IsElite()) return false;
//	if (CURTIME < m_flNextGrenadeCheck) return false; // recently used smoke 
//	if (GetEnemy() && !(GetEnemy()->IsPlayer())) return false;

//	if (!m_pSquad) return false; // lone MPs shouldn't do this
//	if (m_hForcedGrenadeTarget != NULL) return false; // don't attempt normal throws if we're told to perform scripted throw. Don't worry, this gets NULL'd in SelectSchedule.
//	if (GetSquad()->NumMembers(false) < 2) return false; // "2" counts ourselves, so this is I + at least 1 more members
//	if (!GetEnemy()) return false;
//	if (GetEnemy() && !(GetEnemy()->IsPlayer())) return false;

//	CAI_BaseNPC *nearestSquad = GetSquad()->GetSquadMemberNearestTo(GetAbsOrigin());
//	float distToMemberSqr = (nearestSquad->GetAbsOrigin() - GetAbsOrigin()).LengthSqr();

//	if ((distToMemberSqr > (42.0f*42.0f) || !FVisible(nearestSquad))) return false; // our guy is either too far or like on other side of a wall
	
	// todo: 
	//		- check if intended smoke throw spot will be visible to player
...
void CNPC_MetroPolice::BuildScheduleTestBits( void )
{
	...
	if ( !IsInAScript() && 
		 ...
		 !IsCurSchedule( SCHED_MELEE_ATTACK2 ) &&
		 !IsCurSchedule( SCHED_RELOAD ) &&
		 !IsCurSchedule(SCHED_METROPOLICE_ACTIVATE_BATON) &&
#ifdef DARKINTERVAL
		 !IsCurSchedule( SCHED_METROPOLICE_DRAW_PISTOL ) &&
		 !IsCurSchedule( SCHED_METROPOLICE_DEPLOY_MANHACK ) &&
		 !IsCurSchedule( SCHED_DI_FALL_IN_RAGDOLL_PUSH ) //// DI NEW: Live Ragdolls Experiment, inspired by Dark Messiah - don't switch schedules if in special ragdoll sched
#endif
		)
	{
	...
	if( !IsCurSchedule(SCHED_METROPOLICE_ACID_STAND )) 
	{
		SetCustomInterruptCondition(COND_METROPOLICE_COVERED_IN_ACID);
	}
#endif
	...
	{
		if ( CURTIME - m_flLastDamageFlinchTime < 10.0 )
		{
			ClearCustomInterruptCondition( COND_LIGHT_DAMAGE );
			ClearCustomInterruptCondition( COND_HEAVY_DAMAGE );
		}
...
#ifdef DARKINTERVAL
	if (IsCurSchedule(SCHED_METROPOLICE_ADVANCE))
	{
		if (IsElite() && m_iGearAmount > 0)
		{
			if (CURTIME < GetTimeScheduleStarted() + 3.0f)
			{
				// give us some time to throw that smoke grenade
...
#ifdef DARKINTERVAL
void CNPC_MetroPolice::InputGiveWeapon(inputdata_t &inputdata)
{
...
{
#ifdef DARKINTERVAL // DI gives metrocops a greater variety of weapons, plus there's elite versions. Rebalance their proficiencies
	bool bE = IsElite();
	
	if( FClassnameIs( pWeapon, "weapon_pistol" ) )
	{
		return !bE ? WEAPON_PROFICIENCY_GOOD : WEAPON_PROFICIENCY_VERY_GOOD;
	}

	if( FClassnameIs( pWeapon, "weapon_smg1" ) )
	{
		return !bE ? WEAPON_PROFICIENCY_POOR : WEAPON_PROFICIENCY_AVERAGE;
	}

	if (FClassnameIs(pWeapon, "weapon_357"))
	{
		return !bE ? WEAPON_PROFICIENCY_POOR : WEAPON_PROFICIENCY_VERY_GOOD;
	}

	if (FClassnameIs(pWeapon, "weapon_shotgun"))
	{
		return !bE ? WEAPON_PROFICIENCY_AVERAGE : WEAPON_PROFICIENCY_GOOD;
	}
#else
	if ( FClassnameIs(pWeapon, "weapon_pistol") )
	{
...
void CNPC_MetroPolice::GatherConditions( void )
{
	...
	else
	{
		...
		if ( (CURTIME - GetLastAttackTime()) > 3 )
		{
			m_iNumPlayerHits = 0;
		}
...
BEGIN_DATADESC(CNPC_MetroPolice)
	...
	DEFINE_KEYFIELD(m_iGearAmount, FIELD_INTEGER, "manhacks"),
	DEFINE_KEYFIELD(m_iGearType, FIELD_INTEGER, "gear_type"),
...
#ifdef DARKINTERVAL
	DEFINE_INPUTFUNC(FIELD_VOID, "ForceManhackToss", InputForceManhackToss),
	DEFINE_INPUTFUNC(FIELD_VOID, "FakeWeaponSwitch", InputFakeWeaponSwitch),
	DEFINE_INPUTFUNC(FIELD_STRING, "SetPoliceGoal", InputSetPoliceGoal),
	DEFINE_INPUTFUNC(FIELD_STRING, "ThrowSmokeGrenadeAtTarget", InputThrowGrenadeAtTarget),
	DEFINE_INPUTFUNC(FIELD_STRING, "GiveWeapon", InputGiveWeapon),
	//	DEFINE_INPUTFUNC( FIELD_VOID, "SwitchWeapons", InputSwitchWeapons),
	...
	DEFINE_OUTPUT(m_OnPlayerInViewcone, "OnPlayerInViewcone"),
#endif
	DEFINE_EMBEDDED(m_PanicSentencesTimer),
END_DATADESC()
...
#ifdef DARKINTERVAL
	DECLARE_ANIMEVENT( AE_METROPOLICE_KICK);
#endif
...
#ifdef DARKINTERVAL
	DECLARE_ANIMEVENT( AE_METROPOLICE_GRENADETHROW );
#endif
...
#ifdef DARKINTERVAL
	DECLARE_ACTIVITY( ACT_KICK_PLAYER);
#endif
...
#ifdef DARKINTERVAL
	DECLARE_ACTIVITY( ACT_THROW_GRENADE )
#endif
...
#ifdef DARKINTERVAL
//	DECLARE_TASK(TASK_METROPOLICE_COLLECT_DATA_ON_PHYSICS_COVER);
	DECLARE_TASK(TASK_METROPOLICE_SELECT_BEST_PHYSICS_COVER);
	DECLARE_TASK(TASK_MP_FACE_TOSS_DIR)
	DECLARE_TASK(TASK_MP_GET_PATH_TO_FORCED_GREN_LOS)
	DECLARE_TASK(TASK_MP_THROW_SMOKE)
	DECLARE_TASK(TASK_MP_SWITCH_WEAPONS);
#endif
	DECLARE_CONDITION( COND_METROPOLICE_ON_FIRE );
#ifdef DARKINTERVAL
	DECLARE_CONDITION( COND_METROPOLICE_COVERED_IN_ACID );
#endif
...
(
	...
	"	Tasks"
	"		TASK_SPEAK_SENTENCE					5"	// METROPOLICE_SENTENCE_DEPLOY_MANHACK
	"		TASK_PLAY_SEQUENCE					ACTIVITY:ACT_METROPOLICE_DRAW_PISTOL"
	"		TASK_MP_SWITCH_WEAPONS				0"
	"	"
	"	Interrupts"
	"	"
);
...
//	"		TASK_FACE_PLAYER					0" // FIXME: use target/enemy in the future
	"		TASK_FACE_ENEMY						0"
...
(
	...
	"		TASK_STOP_MOVING				0"
	"		TASK_FACE_PLAYER				0.1"	//FIXME: This needs to be the target or enemy
	"		TASK_PLAY_SEQUENCE				ACTIVITY:ACT_KICK_PLAYER"
	""
	"	Interrupts"
);
#endif
...
(
	...
	"	Tasks"
	"		TASK_SET_FAIL_SCHEDULE		SCHEDULE:SCHED_METROPOLICE_BURNING_STAND"
	"		TASK_METROPOLICE_SELECT_BEST_PHYSICS_COVER 0"
	"		TASK_RUN_PATH				0"
	"		TASK_WAIT_FOR_MOVEMENT		0"
...
(
	...
	"		TASK_SET_ACTIVITY				ACTIVITY:ACT_IDLE_ON_FIRE"
	"		TASK_WAIT						10.0"
	"		TASK_METROPOLICE_DIE_INSTANTLY	DMG_ACID"
	"	"
	"	Interrupts"
);
...
(
	...
	"	Tasks "
	"		TASK_SET_TOLERANCE_DISTANCE					48"
	"		TASK_MP_GET_PATH_TO_FORCED_GREN_LOS			0"
