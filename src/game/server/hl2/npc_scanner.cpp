...
//
// Purpose: 
// DI todo - make the scanner use NPC-scritp based system, and use the 
// alternative model selection (todo) as the claw scanner model
//=============================================================================//
...
#ifdef DARKINTERVAL
		PrecacheScriptSound("NPC_CScanner.Attention");
#endif
...
void CNPC_CScanner::Spawn(void)
{
	...
	m_fInspectEndTime		= 0;
	m_fCheckCitizenTime		= CURTIME + SCANNER_CIT_INSPECT_DELAY;
...
#ifdef DARKINTERVAL // ambushing scanners sometimes use this
...
void CNPC_CScanner::Activate()
{
	...
	m_pEyeFlash->SetHDRColorScale(0.5f);
}
...
bool CNPC_CScanner::IsValidInspectTarget(CBaseEntity *pEntity)
{
	...
	{
		if (((CNPC_Citizen*)pEntity)->GetNextScannerInspectTime() > CURTIME)
		{
			return false;
		}
	}
...
void CNPC_CScanner::GatherConditions( void )
{
	...
	{
		m_fInspectEndTime = CURTIME + SCANNER_CIT_INSPECT_LENGTH;
	}
	...
	if ( CURTIME	> m_fCheckCitizenTime && HaveInspectTarget() == false )
	{
	...
	if ( m_nFlyMode == SCANNER_FLY_PHOTO )
	{
		...
		if ( CURTIME > m_fNextPhotographTime && HaveInspectTarget() && GetCurrentVelocity().LengthSqr() < (64*64) )
		{
...
#ifdef DARKINTERVAL // used to disable the light (perf/visuals reason) but not the beam itself
...
#ifdef DARKINTERVAL
			float flFadeTime = 1.0f; // DI CHANGE, scanner flash is shorter now
#else	
...
void CNPC_CScanner::StartTask( const Task_t *pTask )
{
	...
	case TASK_CSCANNER_GET_PATH_TO_INSPECT_TARGET:
		{
			...
			if ( pCitizen )
			{
				pCitizen->SetNextScannerInspectTime( CURTIME + 5.0 );
			}
	...
	case TASK_CSCANNER_SET_FLY_PHOTO:
	{
		...
		// Leave spotlight off for a while
		m_fNextSpotlightTime = CURTIME + 2.0;
...
void CNPC_CScanner::AdjustScannerVelocity( void )
{
	if ( m_bIsClawScanner )
	{
		m_vCurrentVelocity *= ( 1 + sin( ( CURTIME + m_flFlyNoiseBase ) * 2.5f ) * .1 );
	}
}
...
bool CNPC_CScanner::HandleInteraction(int interactionType, void *data, CBaseCombatCharacter* pSourceEnt)
{
	...
	if (interactionType ==	g_interactionScannerSupportEntity)
	{
		...
		if (GetEnemy() == NULL && !HaveInspectTarget())
		{
			...
			{
				float fInspectTime = (((CNPC_CScanner*)pSourceEnt)->m_fInspectEndTime - CURTIME);
