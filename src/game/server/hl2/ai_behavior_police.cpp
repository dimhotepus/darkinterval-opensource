...
{
	if ( ( m_flTargetHostileTime < CURTIME ) && ( !m_bTargetIsHostile ) )
		return false;
...
#ifdef DARKINTERVAL //// DI NEW: Live Ragdolls Experiment, inspired by Dark Messiah - don't switch schedules if in special ragdoll sched
...
void CAI_PolicingBehavior::GatherConditions( void )
{
	...
	if ( m_bTargetIsHostile && m_flAggressiveTime < CURTIME && IsCurSchedule(SCHED_CHASE_ENEMY) )
	{
...
{
	m_flTargetHostileTime = CURTIME + time;
}
	...
	case TASK_POLICE_ANNOUNCE_HARASS:
		{
			...
			m_flNextHarassTime = CURTIME + random->RandomInt( 4, 6 );
...
int CAI_PolicingBehavior::SelectSuppressSchedule( void )
{
	...
	m_flAggressiveTime = CURTIME + 4.0f;
...
int CAI_PolicingBehavior::SelectSchedule( void )
{
	...
	if ( ( m_flAggressiveTime >= CURTIME ) && HasCondition( COND_CAN_MELEE_ATTACK1 ) )
	{
