...
#include "gib.h"
#ifdef DARKINTERVAL
#include "globalstate.h" // for Arcade global state
#endif
#include "ieffects.h"
...
#ifdef DARKINTERVAL // for Arcade versions
...
void CNPC_Manhack::Spawn(void)
{
	...
	m_flNextBurstTime = CURTIME;

	CapabilitiesAdd(bits_CAP_INNATE_MELEE_ATTACK1 | bits_CAP_MOVE_FLY | bits_CAP_SQUAD);

	m_flNextEngineSoundTime = CURTIME;
	m_flWaterSuspendTime = CURTIME;
	m_flEngineStallTime = CURTIME;
	m_fForceMoveTime = CURTIME;
	m_vForceMoveTarget = vec3_origin;
	m_fSwarmMoveTime = CURTIME;
...
int	CNPC_Manhack::OnTakeDamage_Alive(const CTakeDamageInfo &info)
{
	...
	else if (info.GetDamageType() & DMG_CLUB)
	{
		...
		//		-Take 300% damage from club attacks. This makes crowbar duels take one hit. // DI change - original is 150% and two hits
#ifdef DARKINTERVAL
		tdInfo.ScaleDamage(3.00);
#else
...
#define	MANHACK_PHYSICS_SEARCH_RADIUS	128
		...
		m_flEngineStallTime = CURTIME + 0.5f;
		StopBurst(true);
	}
	else
	{
		...
		m_flEngineStallTime = CURTIME + 0.25f;
...
void CNPC_Manhack::Event_Killed(const CTakeDamageInfo &info)
{
...
#ifdef DARKINTERVAL // Arcade version produces blue light
	if (HasSpawnFlags(SF_MANHACK_ARCADE_VERSION) || GlobalEntity_GetState("playing_arcade") == GLOBAL_ON)
		te->DynamicLight(filter, 0.0, &GetAbsOrigin(), 100, 180, 255, 0.1, 256, 0.1, 0);
	else
#endif
...
#ifdef DARKINTERVAL // Arcade version disappears in an energy splash without gibs
	if (HasSpawnFlags(SF_MANHACK_ARCADE_VERSION) || GlobalEntity_GetState("playing_arcade") == GLOBAL_ON)
	{
		DispatchParticleEffect("Weapon_Combine_Ion_Cannon", GetAbsOrigin(), GetAbsAngles());
		KillSprites(0.0f);
		UTIL_Remove(this);
	}
...
#endif
	{
		...
		else
		{
			...
			// DI todo - fix and have long sprites attached to the ragdoll
			KillSprites(0.0f);
		}
	}
...
void CNPC_Manhack::HandleAnimEvent(animevent_t *pEvent)
{
	...
	case MANHACK_AE_START_ENGINE:
#ifdef DARKINTERVAL // different order, because...
		StartEngine(true);
		m_spawnflags &= ~SF_MANHACK_PACKED_UP;
		StartEye(); // because starteye checks if spawnflag is set, and sets brightness to zero if it is
#else
		StartEye();
		StartEngine(true);
		m_spawnflags &= ~SF_MANHACK_PACKED_UP;
		...
		m_flNextBurstTime = CURTIME + FLT_MAX;
		break;

	case MANHACK_AE_DONE_UNPACKING:
		m_flNextBurstTime = CURTIME + 2.0;
		break;
...
#ifdef DARKINTERVAL // used for 'start packed', ambush mode manhacks
	if (HasSpawnFlags(SF_MANHACK_PACKED_UP))
	{
		if (HasCondition(COND_LIGHT_DAMAGE))
		{
			ClearSchedule("cleared via condition change");
			SetSchedule(SCHED_MANHACK_DEPLOY);
			ClearCondition(COND_LIGHT_DAMAGE);
			m_spawnflags &= ~SF_MANHACK_PACKED_UP;
			SetEyeState(MANHACK_EYE_STATE_CHARGE);
			m_OnUnpacked.FireOutput(this, this);
		}
	}
#endif
}
...
{
#ifndef DARKINTERVAL // used for 'start packed', ambush mode manhacks
...
void CNPC_Manhack::PrescheduleThink(void)
{
	...
	if (m_flWaterSuspendTime > CURTIME)
	{
		// Stuck in water!
	...
	if (GetWaterLevel() > 0)
	{
		...
		if (IsAlive())
		{
			...
			if (CURTIME - m_flWaterSuspendTime > 2.0)
			{
				m_flWaterSuspendTime = CURTIME + 1.0;
			}
		}
	}
	else
...
void CNPC_Manhack::StartTask(const Task_t *pTask)
{
	...
	case TASK_MANHACK_MOVEAT_SAVEPOSITION:
	{
		...
		{
			m_fSwarmMoveTime = CURTIME + RandomFloat(pTask->flTaskData * 0.8, pTask->flTaskData * 1.2);
		}
	}
	break;
...
void CNPC_Manhack::RunTask(const Task_t *pTask)
{
	...
	case TASK_MANHACK_MOVEAT_SAVEPOSITION:
	{
		...
		float t = m_fSwarmMoveTime - CURTIME;
...
void CNPC_Manhack::InputPowerdown(inputdata_t &inputdata)
{
	m_iHealth = 0;
}
...
bool CNPC_Manhack::OverrideMove(float flInterval)
{
	...
	// ------------------------------------------------------------------
	else if (m_fSwarmMoveTime > CURTIME)
	{
...
void CNPC_Manhack::MoveToTarget(float flInterval, const Vector &vMoveTarget)
{
	...
	// -----------------------------------------
	if (CURTIME < m_flEngineStallTime || m_iHealth <= 0)
		return;
...
void CNPC_Manhack::MoveExecute_Alive(float flInterval)
{
	...
	if (m_fEnginePowerScale < GetMaxEnginePower() && CURTIME > m_flWaterSuspendTime)
	{
	...
	if ((CBaseEntity*)GetEnemy())
	{
		...
		if (IsInEffectiveTargetZone(GetEnemy()) && flDist < MANHACK_CHARGE_MIN_DIST && CURTIME > m_flNextBurstTime)
		{
			Vector vecCurrentDir = GetCurrentVelocity();
			VectorNormalize(vecCurrentDir);
			...
			if (flDot > 0.75)
			{
				...
				offsetDir.z = 0.0f;
#ifdef DARKINTERVAL // DI makes the thrusts faster and less predictable
				m_vForceVelocity += (offsetDir * (offsetSpeed.Length2D() * 1.25f));
#else
	...
	if (HasPhysicsAttacker(MANHACK_SMASH_TIME) && (!IsHeldByPhyscannon()) && VPhysicsGetObject())
	{
		...
		float flLerpFactor = (CURTIME - m_flLastPhysicsInfluenceTime) / MANHACK_SMASH_TIME;
	...
	//  Stalling
	// ------------------------------------------
	if (CURTIME < m_flEngineStallTime)
	{
		/*
...
#ifdef DARKINTERVAL // re-enable unused HL2 code to check it out
		// Using our steering if we're not otherwise affecting our panels
		if (m_flEngineStallTime < CURTIME && m_flBurstDuration < CURTIME)
		{
...
void CNPC_Manhack::MoveExecute_Dead(float flInterval)
{
	...
	if (CURTIME > m_fSmokeTime && GetWaterLevel() == 0)
	{
	//	UTIL_Smoke(GetAbsOrigin(), random->RandomInt(10, 15), 10);
		m_fSmokeTime = CURTIME + random->RandomFloat(0.1, 0.3);
	}

	// Periodically emit sparks.
	if (CURTIME > m_fSparkTime)
	{
		g_pEffects->Sparks(GetAbsOrigin());
		m_fSparkTime = CURTIME + random->RandomFloat(0.1, 0.3);
	}
...
void CNPC_Manhack::Loiter()
{
	...
	if (!m_bHeld)
	{
		...
		if (distSqr > MAX_LOITER_DIST_SQR)
		{
			...
			if (CURTIME > m_fTimeNextLoiterPulse)
			{
				...
				m_fTimeNextLoiterPulse = CURTIME + 1.0f;
			}
			else
			{
				m_vForceVelocity = vec3_origin;
...
// Functions called anywhere above.
...
void CNPC_Manhack::PlayFlySound(void)
{
	...
	if (m_flWaterSuspendTime > CURTIME)
	{
		...
		if (m_bDirtyPitch)
		{
			...
			m_flEnginePitch1Time = CURTIME + 0.5f;
			m_nEnginePitch2 = MANHACK_WATER_PITCH2;
			m_flEnginePitch2Time = CURTIME + 0.5f;
			m_bDirtyPitch = false;
		}
	}
	...
	else if (GetEnemy() && IsAlive())
	{
		if (flEnemyDist < MANHACK_PITCH_DIST1)
		{
			...
			m_flEnginePitch1Time = CURTIME + 0.1f;
			m_nEnginePitch2 = iPitch2;
			m_flEnginePitch2Time = CURTIME + 0.1f;
		...
		else if (m_bDirtyPitch)
		{
			...
			m_flEnginePitch2Time = CURTIME + 0.2f;
			m_bDirtyPitch = false;
		}
	}
	...
	if (CURTIME > m_flNextEngineSoundTime && flEnemyDist < 48)
	{
		m_flNextEngineSoundTime = CURTIME + random->RandomFloat(3.0, 10.0);
...
void CNPC_Manhack::StopLoopingSounds(void)
{
	...
	m_nEnginePitch1 = -1;
	m_flEnginePitch1Time = CURTIME;
	m_nEnginePitch2 = -1;
	m_flEnginePitch2Time = CURTIME;
}
...
#ifdef DARKINTERVAL // used for 'start packed', ambush mode manhacks
	if (HasSpawnFlags(SF_MANHACK_PACKED_UP) && !IsCurSchedule(SCHED_MANHACK_DEPLOY))
	{
		if (m_pEyeGlow)
			m_pEyeGlow->SetBrightness(0);
		if (m_pLightGlow)
			m_pLightGlow->SetBrightness(0);
	}
#endif
}
...
#ifndef DARKINTERVAL // allow friendly fire // DI todo - revisit it for Part 3, however
...
void CNPC_Manhack::Bump(CBaseEntity *pHitEntity, float flInterval, trace_t &tr)
{
	...
	if (m_flBumpSuppressTime > CURTIME)
		return;
	...
	if (flInterval > 0)
	{
		...
		if (tr.allsolid || tr.startsolid)
		{
			...
			m_flBumpSuppressTime = CURTIME + 0.5f;
		}
		else
		{
...
void CNPC_Manhack::StartBurst(const Vector &vecDirection)
{
	if (m_flBurstDuration > CURTIME)
		return;
#ifdef DARKINTERVAL // DI makes the thrusts faster and less predictable
	m_vForceVelocity = (GetCurrentVelocity() * 2.0f);
#endif
	ShowHostile();

	// Don't burst attack again for a couple seconds
	m_flNextBurstTime = CURTIME + 2.0;
	m_flBurstDuration = CURTIME + 1.0;
...
void CNPC_Manhack::StopBurst(bool bInterruptSchedule /*= false*/)
{
	...
	m_flNextBurstTime = CURTIME + 2.0f; //FIXME: Skill level based
	m_flBurstDuration = CURTIME - 0.1f;
...
void CNPC_Manhack::OnPhysGunPickup( CBasePlayer *pPhysGunUser, PhysGunPickup_t reason )
{
	...
	if ( reason == PUNTED_BY_CANNON )
	{
		...
		m_flEngineStallTime = CURTIME + 2.0f;
		SetEyeState( MANHACK_EYE_STATE_STUNNED );
	}
	else
	{
...
BEGIN_DATADESC(CNPC_Manhack)
	...
	DEFINE_OUTPUT(m_OnUnpacked, "OnUnpacked"),
#endif
