...
// - replaced numerous DevMsgs with normal or coloured messages
//=============================================================================//
...
void CAI_NetworkManager::LoadNetworkGraph( void )
{
	...
	// ---------------------------------------------------
	ConColorMsg(Color(0,255,0,255), "Loading AI graph\n" );
	...
	{
		ConColorMsg(Color(255, 255, 0, 255), "Couldn't read AIN file %s! Will need rebuilding!\n", szNrpFilename );
		return;
	}
	...
	if ( mapversion != gpGlobals->mapversion && !g_ai_norebuildgraph.GetBool() )
	{
...
#ifndef DARKINTERVAL
		if ( !V_stricmp( szLoweredGameDir, "hl2" ) 
			|| !V_stricmp( szLoweredGameDir, "episodic" )
			)
		{
			bOK = true;
		}
#else
		if ( !V_stricmp(szLoweredGameDir, "darkinterval")
			|| !V_stricmp(szLoweredGameDir, "darkinterval-part2")
			) // DI REVISIT FOR SHIPPING PART 2
		{
		//	bOK = true;
		}
#endif
		if ( !bOK )
		{
			ConColorMsg(Color(100, 255, 100, 255), "AI node graph %s is out of date (map version changed)\n", szNrpFilename );
			return;
		}
	}
	...
	CAI_DynamicLink::gm_bInitialized = false;
	ConColorMsg(Color(0, 255, 0, 255), "Done loading AIN file - NetworksLoaded( ) is now TRUE\n");
}
...
{
#ifdef DARKINTERVAL
	ConColorMsg(Color(0, 255, 0, 255), "Deleting networks - NetworksLoaded( ) is now FALSE\n");
#endif
	CAI_DynamicLink::gm_bInitialized = false;
...
void CAI_NetworkManager::InitializeAINetworks()
{
	...
	{
		ConColorMsg(Color(255, 255, 0, 255), "AIN file is not current or not present - will now rebuild the graph\n");
	}
#endif
...
bool CAI_NetworkManager::IsAIFileCurrent(const char *szMapName)
{
	...
	if (!V_stricmp(szLoweredGameDir, "hl2")
		|| !V_stricmp(szLoweredGameDir, "episodic")
		|| !V_stricmp(szLoweredGameDir, "ep2")
		|| !V_stricmp(szLoweredGameDir, "portal")
		|| !V_stricmp(szLoweredGameDir, "lostcoast")
		|| !V_stricmp(szLoweredGameDir, "hl1"))
#else
	if (!V_stricmp(szLoweredGameDir, "darkinterval")
		|| !V_stricmp(szLoweredGameDir, "darkinterval-part2"))
#endif
...
void CAI_NetworkManager::DelayedInit( void )
{
	...
	if ( !g_ai_norebuildgraph.GetInt() )
	{
		...
		else if ( !m_bDontSaveGraph && ( !CAI_NetworkManager::NetworksLoaded() || engine->IsInEditMode() ) )
		{
			...
			ConColorMsg(Color(255, 255, 0, 255), "Node Graph out of Date. Rebuilding... (%d, %d, %d)\n", (int)m_bDontSaveGraph, (int)CAI_NetworkManager::NetworksLoaded(), (int) engine->IsInEditMode() );
#ifndef DARKINTERVAL // disable the famous message : (
			UTIL_CenterPrintAll( "Node Graph out of Date. Rebuilding...\n" );
#endif
			m_bNeedGraphRebuild = true;
			g_pAINetworkManager->SetNextThink( CURTIME + 1 );
			return;
		}	
...
int CAI_NetworkEditTools::GetNodeIdFromWCId( int nWCId )
{
	...
	{
#ifdef DARKINTERVAL
		DevMsg("Attention: Node WC id -1 found!\n");
#endif
		return -1;
	}
...
#ifdef DARKINTERVAL
								// DI NEW - water nodes
								else if ((m_debugNetOverlays & bits_debugOverlaySwimConnections) && (moveTypes & bits_CAP_MOVE_SWIM))
								{
									NDebugOverlay::Line(srcPos, desPos, 50, 50, 255, false, flDrawDuration);
								}
#endif
								else  
...
#ifdef DARKINTERVAL
									bool isSwim = (srcType == NODE_WATER || desType == NODE_WATER); // DI NEW - water nodes
#endif
									bool isJump = true;
...
#ifdef DARKINTERVAL
										( isSwim && (m_debugNetOverlays & bits_debugOverlaySwimConnections)) || // DI NEW - water nodes
#endif
										( !isFly && !isJump ) )
									{
...
#ifdef DARKINTERVAL
						// DI NEW - water nodes
						else if (pAINode[node]->GetType() == NODE_WATER)
						{
							r = 50;
							g = 50;
							b = 255;
...
#ifdef DARKINTERVAL
		// If under water, note that the node is in water // DI NEW - water nodes // todo - make it all work, currently doesn't
	//	if (UTIL_PointContents(pNode->GetOrigin()) & MASK_WATER)
	//	{
	//		pNode->SetType(NODE_WATER); //m_eNodeType |= NODE_WATER;
	//	}
#endif
		return;
	}
...
#ifdef DARKINTERVAL
			|| testNode->GetType() == NODE_WATER // DI NEW - water nodes
#endif
			)
		{
...
int CAI_NetworkBuilder::ComputeConnection( CAI_Node *pSrcNode, CAI_Node *pDestNode, Hull_t hull )
{
	...
	{
		DevWarning( 2, "OFFGROUND node at %.f %.f %.f!\n", pSrcNode->GetOrigin().x, pSrcNode->GetOrigin().y, pSrcNode->GetOrigin().z);
	}
...
#ifdef DARKINTERVAL
		&& pDestNode->GetType() != NODE_WATER && pSrcNode->GetType() != NODE_WATER // DI new - water nodes
#endif
		)
	{
	...
	// DI NEW - water nodes
	// ==============================================================
	// WATER NODES (SWIMMING)
	// ==============================================================
	else if (pSrcNode->m_eNodeType == NODE_WATER || pDestNode->GetType() == NODE_WATER)
	{
		AI_PROFILE_SCOPE(CAI_Node_InitLinks_Water);

		// Water nodes only connect to other water nodes and nothing else
		if (pSrcNode->m_eNodeType == NODE_WATER && pDestNode->GetType() == NODE_WATER)
		{
			AI_TraceHull(pSrcNode->GetOrigin(), pDestNode->GetOrigin(), NAI_Hull::Mins(hull), NAI_Hull::Maxs(hull), MASK_NPCWORLDSTATIC, m_pTestHull, COLLISION_GROUP_NONE, &tr);
			if (!tr.startsolid && tr.fraction == 1.0)
			{
				result |= bits_CAP_MOVE_SWIM;
				DebugConnectMsg(srcId, destId, "      Connect by swimming\n");
			}
		}
	}
#endif
