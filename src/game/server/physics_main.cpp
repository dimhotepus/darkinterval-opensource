...
#if 0 //def DARKINTERVAL // fix angles becoming too big and complaining about it // caused bugs in momentary rot buttons
	for (int i = 0; i < 3; i++)
	{
		if (angles[i] > 360) angles[i] = remainderf(angles[i], 360);
		else if (angles[i] < -360) angles[i] = remainderf(angles[i], -360);
	}
#endif
	...
	for (bumpcount=0 ; bumpcount<numbumps ; bumpcount++)
	{
		...
		if (trace.plane.normal[2] > 0.7)
		{
			...
			if (CanStandOn( trace.m_pEnt ))
			{
				...
				{
					SetGroundChangeTime( CURTIME + (flTime - (1 - trace.fraction) * time_left) );
				}
		...
		if ( GetMoveType() == MOVETYPE_WALK && (!(GetFlags() & FL_ONGROUND) || GetFriction()!=1) )	// relfect player velocity
		{
			for ( i = 0; i < numplanes; i++ )
			{
				...
				{
#ifdef DARKINTERVAL
					PhysicsClipVelocity( original_velocity, planes[i], new_velocity, 1.0 + 0.0f * (1-GetFriction()) );
#else
...
void CBaseEntity::PhysicsStep()
{
	...
	if ( HasDataObjectType(VPHYSICSUPDATEAI) )
	{
		...
		if ( pUpdate->stopUpdateTime > CURTIME )
		{
...
void Physics_SimulateEntity( CBaseEntity *pEntity )
{
	...
	if ( pEntity->edict() )
	{
		...
		if ( pEntity->IsPlayerSimulated() )
		{
			...
			if ( simulatingPlayer &&
				( simulatingPlayer->GetTimeBase() > CURTIME - PLAYER_PACKETS_STOPPED_SO_RETURN_TO_PHYSICS_TIME ) )
			{
