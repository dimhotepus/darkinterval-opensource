...
#ifdef DARKINTERVAL // thread display (houndeye, hopper...)
	ADD_ACTIVITY_TO_SR(ACT_BUSY_THREAT);
#endif
	// Dodge animations
...
#ifdef DARKINTERVAL
	ADD_ACTIVITY_TO_SR(ACT_IDLE_CARRY2); // Eli's gravity gun carrying anims
	ADD_ACTIVITY_TO_SR(ACT_WALK_CARRY2);
	// Portalgun - unused at the time
	ADD_ACTIVITY_TO_SR( ACT_VM_FIZZLE );

	// DI NEW: Live Ragdolls Experiment (inspired by Dark Messiah)
	ADD_ACTIVITY_TO_SR(ACT_PUSH_RAGDOLL_WAIT_IDLE);
#endif
}
