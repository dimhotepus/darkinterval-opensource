...
#ifdef DARKINTERVAL
	DEFINE_KEYFIELD(m_check_player_flashlight_state, FIELD_INTEGER, "CheckPlayerFlashlight"),
	DEFINE_INPUTFUNC(FIELD_INTEGER, "SetFlashlightCheckState", InputSetFlashlightCheckState),
#endif
END_DATADESC()
...
#ifdef DARKINTERVAL
		EVALUATOR( PlayerFlashlightActive ),
#endif
#ifdef HL2_EPISODIC
...
#ifdef DARKINTERVAL
	if (!pNpc) return false; // it can crash without this check
#endif
...
#ifdef DARKINTERVAL
		if (!pNPCActor) return false; // it can crash without this check
#endif
#ifdef HL2_EPISODIC
...
#ifdef DARKINTERVAL
void CAI_ScriptConditions::InputSetFlashlightCheckState(inputdata_t &inputdata)
{
	m_check_player_flashlight_state = (ThreeState_t)inputdata.value.Int();
	if (m_check_player_flashlight_state < 0) m_check_player_flashlight_state = TRS_FALSE;
	if (m_check_player_flashlight_state > 2) m_check_player_flashlight_state = TRS_NONE;
}
bool CAI_ScriptConditions::EvalPlayerFlashlightActive(const EvalArgs_t &args)
{
	// We don't care
	if (m_check_player_flashlight_state == TRS_NONE)
		return true;
	...
	return (!!args.pPlayer->FlashlightIsOn() == m_check_player_flashlight_state);
}
#endif
...
void CAI_ScriptConditions::Enable( void )
{
	...
	{		
		...
		{
#ifdef DARKINTERVAL
			// DI add: try classname instead
			pActor = gEntList.FindEntityByClassname(NULL, m_Actor.ToCStr());
			if (pActor == NULL)
			{
		...
		{
			Warning("Script condition warning: actor (%s) is not an NPC; use Target (Optional) field with this entity instead!\n", pActor->GetDebugName());
			Disable();
			return;
		}
	}
