...
{
	if( m_TimeNextAvailable > CURTIME )
	{
		return false;
	}
...
void CNPCSpawnDestination::OnSpawnedNPC( CAI_BaseNPC *pNPC )
{
	...
	m_TimeNextAvailable = CURTIME + m_ReuseDelay;
}
...
bool CBaseNPCMaker::CanMakeNPC( bool bIgnoreSolidEntities )
{
	...
	if ((skill.GetInt() == 1 && HasSpawnFlags(SF_NPCMAKER_DIFFICULTY_NOT_EASY))
		|| (skill.GetInt() == 2 && HasSpawnFlags(SF_NPCMAKER_DIFFICULTY_NOT_MEDIUM))
		|| (skill.GetInt() == 3 && HasSpawnFlags(SF_NPCMAKER_DIFFICULTY_NOT_HARD)))
		return false;
...
BEGIN_DATADESC( CNPCMaker )
	...
	DEFINE_KEYFIELD( m_iszNPCSkin,			FIELD_INTEGER,	"NPCSkin"),
	...
	DEFINE_KEYFIELD( m_emitAISound,			FIELD_BOOLEAN,	"ScareNPCs"),

	DEFINE_KEYFIELD( m_AddSpawnFlag1,		FIELD_INTEGER,	"AddSpawnFlag1"),
	DEFINE_KEYFIELD( m_AddSpawnFlag2,		FIELD_INTEGER,  "AddSpawnFlag2"),
	DEFINE_KEYFIELD( m_AddSpawnFlag3,		FIELD_INTEGER,  "AddSpawnFlag3"),
	DEFINE_KEYFIELD( m_AddSpawnFlag4,		FIELD_INTEGER,  "AddSpawnFlag4"),
...
void CNPCMaker::MakeNPC( void )
{
	...
	if (HasSpawnFlags(SF_NPCMAKER_SERVERRAGDOLL))
		pent->AddSpawnFlags(SF_ALWAYS_SERVER_RAGDOLL);

	if (m_AddSpawnFlag1 > 0) pent->AddSpawnFlags(m_AddSpawnFlag1);
	if (m_AddSpawnFlag2 > 0) pent->AddSpawnFlags(m_AddSpawnFlag2);
	if (m_AddSpawnFlag3 > 0) pent->AddSpawnFlags(m_AddSpawnFlag3);
	if (m_AddSpawnFlag4 > 0) pent->AddSpawnFlags(m_AddSpawnFlag4);
	...
	if (m_iszNPCSkin > -1)
		pent->m_nSkin = m_iszNPCSkin;

	if ( m_emitAISound )
	{
		CSoundEnt::InsertSound(SOUND_DANGER, GetAbsOrigin(), 128, 0.5f);
	}
...
{
	SetNextThink( CURTIME + m_flSpawnFrequency );
...
BEGIN_DATADESC( CTemplateNPCMaker )
	...
	DEFINE_KEYFIELD( m_iszOptionalModelName, FIELD_STRING, "OptionalModelName"),
...
void CTemplateNPCMaker::MakeNPC( void )
{
	...
	if (m_iszOptionalModelName != NULL_STRING)
		pent->SetModelName(m_iszOptionalModelName);
