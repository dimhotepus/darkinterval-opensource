...
#ifdef DARKINTERVAL // blinking items, previously an Xbox exclusive feature, is a setting in DI
...
#ifdef DARKINTERVAL	// DI new - inputs for sane handling of items as physical objects
...
#ifdef DARKINTERVAL	// DI new - inputs for sane handling of items as physical objects
//-----------------------------------------------------------------------------
void CItem::InputEnableMotion(inputdata_t &data)
{
...
void CItem::InputDisableMotion(inputdata_t &data)
{
...
#ifdef DARKINTERVAL // DI has some special cases such as the APC that does not pick up items for the player (need to get out to collect)
		return pPlayer->GetVehicle()->VehicleCanPickupItems();
#else
		return true;
#endif
...
{
#ifndef DARKINTERVAL // DI has some special cases such as the APC that does not pick up items for the player (need to get out to collect)
	...
	{
#ifdef DARKINTERVAL
	//	pPlayer->ForceDropOfCarriedPhysObjects(this); // di change - added this to force picked-up objects to drop to avoid crashing. todo - investiage if still needed, it's a bit annoying to have
#endif
		m_OnPlayerTouch.FireOutput(pOther, this);
#ifdef DARKINTERVAL
		if (ConsumedInstantlyByTouch()) // some things like the healthkits aren't consumed but have multiple "charges"/reserve
#endif
		{
			SetTouch(NULL);
			SetThink(NULL);
...
CBaseEntity* CItem::Respawn( void )
{
	...
	SetNextThink( CURTIME + g_pGameRules->FlItemRespawnTime( this ) );
	return this;
}
