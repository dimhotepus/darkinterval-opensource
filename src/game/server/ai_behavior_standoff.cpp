...
#ifdef DARKINTERVAL  // Add missed fPlayerIsBattleline initializer.
...
public:
	...
	void Spawn()
	{
		if ( m_fActive )
		{
			...
			SetNextThink( CURTIME + AIBL_THINK_INTERVAL );
	...
	if ( GetEnemy() )
	{
		...
		if ( pEnemyInfo &&
			 ...
			 ( ( pEnemyInfo->timeAtFirstHand != AI_INVALID_TIME && 
			   ...
			   ( pEnemyInfo->timeAtFirstHand == AI_INVALID_TIME && 
			     CURTIME  - pEnemyInfo->timeFirstSeen > m_params.flAbandonTimeLimit * 2 ) ) )
		{
