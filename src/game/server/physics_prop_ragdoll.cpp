...
#ifdef DARKINTERVAL
#define	SF_RAGDOLLPROP_ALLOWPICKUP			0x20000
#endif
#ifdef DARKINTERVAL // DI NEW: Live Ragdolls Experiment (inspired by Dark Messiah) - here to tell if we were created to serve as a live ragdoll
#define SF_RAGDOLL_FOR_LIVE_RAGDOLL			0x40000 
#endif
//-----------------------------------------------------------------------------
// Networking
//-----------------------------------------------------------------------------
...
#ifdef DARKINTERVAL // proxy
	SendPropFloat( SENDINFO( m_hGoalValueForControlledVariable), 0, SPROP_NOSCALE ),
#endif
...
#ifdef DARKINTERVAL
	DEFINE_INPUTFUNC(FIELD_STRING, "SetUnragdollTarget", InputSetUnragdollTarget),
	DEFINE_INPUTFUNC(FIELD_STRING, "BeginUnragdoll", InputBeginUnragdoll),
	DEFINE_INPUTFUNC(FIELD_STRING, "UnragdollToEntity", InputUnragdollToEntity),
#endif
...
#ifdef DARKINTERVAL // proxy
	DEFINE_THINKFUNC( PostmortemProxyThink ),
#endif
...
#ifdef DARKINTERVAL
	if (HasSpawnFlags(SF_RAGDOLLPROP_DEBRIS))
	{
		SetThink(&CRagdollProp::PostmortemProxyThink);
...
#ifdef DARKINTERVAL // proxy
	m_hGoalValueForControlledVariable = 0.0f;
#endif
}
#ifdef DARKINTERVAL // Think func for fade effects (see RagdollPostmortem proxy in ragdoll.cpp on Client ) // DI NEW
void CRagdollProp::PostmortemProxyThink(void)
{
	if (m_hGoalValueForControlledVariable > 0)
	{
		m_hGoalValueForControlledVariable -= 0.02f;
...
#ifdef DARKINTERVAL // proxy
	m_hGoalValueForControlledVariable = 1.0f;
#endif
}
...
#ifdef DARKINTERVAL
	PropBreakablePrecacheAll((GetModelName())); // for gibbing corpses
#endif
	BaseClass::Precache();
}
...
#ifdef DARKINTERVAL // for picking up ragdolls
		| FCAP_IMPULSE_USE
#endif
		;
}
...
{
	if (!HasSpawnFlags(SF_RAGDOLLPROP_DEBRIS)/* && HasSpawnFlags(SF_RAGDOLLPROP_ALLOWPICKUP)*/)
	{
		...
		//	if (m_hDamageEntity.Get()) // todo: bring it back (if it makes sense) once the live ragdolls schedules aren't interrupted by +uses (happens with following on +use)
		//	{
		//		m_hDamageEntity.Get()->Use(pActivator, pCaller, useType, value);
		//	}
...
void CRagdollProp::OnPhysGunPickup( CBasePlayer *pPhysGunUser, PhysGunPickup_t reason )
{
	...
	m_flLastPhysicsInfluenceTime = CURTIME;
	...
	if( HasPhysgunInteraction( "onpickup", "boogie" ) )
	{
		...
		{
			CRagdollBoogie::Create( this, 150, CURTIME, 3.0f, SF_RAGDOLL_BOOGIE_ELECTRICAL );
		}
		else
		{
			CRagdollBoogie::Create( this, 150, CURTIME, 2.0f, 0.0f );
		}
	}
...
void CRagdollProp::OnPhysGunDrop( CBasePlayer *pPhysGunUser, PhysGunDrop_t Reason )
{
	...
	{
		SetContextThink( &CRagdollProp::SetDebrisThink, CURTIME + 5, s_pDebrisContext );
	}
...
//	Msg("server ragdoll selected overlay sequence %i\n", seq);
}
#ifdef DARKINTERVAL
ConVar di_breakable_ragdolls("di_breakable_ragdolls", "1");
void CRagdollProp::InitRagdoll( const Vector &forceVector, int forceBone, const Vector &forcePos, matrix3x4_t *pPrevBones, matrix3x4_t *pBoneToWorld, float dt, int collisionGroup, bool activateRagdoll, bool bWakeRagdoll, bool bBreakable, int iBreakableHealth)
#else
...
#endif
{
	...
	{
#ifdef DARKINTERVAL
		if (!m_ragdoll.list[i].pObject->GetCollide())
		{
			Warning("Could not get collision object for ragdoll part %i on %s (look near %.0f %.0f %.0f)!!!\n", i, GetDebugName(), GetAbsOrigin().x, GetAbsOrigin().y, GetAbsOrigin().z);
			UTIL_Remove(this);
			return;
		}
#endif
...
#ifdef DARKINTERVAL	// proxy and gibbing corpses

	SetThink(&CRagdollProp::PostmortemProxyThink);
	SetNextThink(CURTIME);

	if (di_breakable_ragdolls.GetBool())
	{
		//	IBreakableWithPropData *pBreakableInterface = dynamic_cast<IBreakableWithPropData*>(this);
		//	if (!pBreakableInterface)
		//		return;

		//	if (pBreakableInterface->GetBreakableModel() != NULL_STRING && pBreakableInterface->GetBreakableCount() > 0)
		//	{
		if (bBreakable && iBreakableHealth > 0)
		{
			m_takedamage = DAMAGE_YES;
			m_iHealth = iBreakableHealth * 10;
		}
	}
#endif
}
...
int	CRagdollProp::OnTakeDamage( const CTakeDamageInfo &info )
{
	...
	{
#ifdef DARKINTERVAL
		if (info.GetAttacker() && info.GetInflictor()->IsPlayer()) // only pass along weapon damage. Otherwise ragdoll takes physical damage from falling and kills the NPC!
		{
			if (info.GetDamage() >= m_hDamageEntity.Get()->GetHealth())
			{
				if (GetRenderMode() == kRenderNone)
					SetRenderMode(kRenderNormal); // this will kill the entity. Unhide ourselves (redundancy measure, the NPC's EventKilled also does this)
			}
#endif
...
#ifdef DARKINTERVAL // for gibbing ragdolls
	if (di_breakable_ragdolls.GetBool() && m_takedamage == DAMAGE_YES && !fellApart)
	{
		CTakeDamageInfo subInfo = info;
		if (info.GetAttacker() == GetWorldEntity() || info.GetAttacker() == NULL)
		{
			subInfo.SetDamage(0.0f);
			return 0;
		}
		if (subInfo.GetDamage() >= m_iHealth &&
			(subInfo.GetDamageType() & DMG_BUCKSHOT) ||
			(subInfo.GetDamageType() & DMG_SLASH) ||
			(subInfo.GetDamageType() & DMG_ALWAYSGIB) ||
			(subInfo.GetDamageType() & DMG_GIB_CORPSE)
			|| subInfo.GetWeapon() && subInfo.GetDamageType() & DMG_BULLET) // FIXME: the GetWeapon check makes bullet firearms not gib ragdolls. 
																			// But removing it makes them gib it in one shot. This needs something better,
																			// for now leaving it in to at least not make them instantly gib from pistol/smg
		{

			if (subInfo.GetDamageType() & DMG_CRUSH) return 0;
			Vector angVelocity;
			QAngleToAngularImpulse(GetLocalAngularVelocity(), angVelocity);
			PropBreakableCreateAll(GetModelIndex(), VPhysicsGetObject(), GetAbsOrigin(), GetAbsAngles(), GetAbsVelocity(), angVelocity, 1.0, 100, COLLISION_GROUP_NPC, this);

			if (BloodColor() == BLOOD_COLOR_RED || BloodColor() == BLOOD_COLOR_YELLOW || BloodColor() == BLOOD_COLOR_GREEN)
			{
				EmitSound("Flesh.Gib");
...
#ifdef DARKINTERVAL // DI allows ragdolls to bleed when hit
	int BC = BloodColor();
	if ( BC != BLOOD_COLOR_MECH && BC != DONT_BLEED )
	{
		IPhysicsObject *pObj = VPhysicsGetObject();
		...
		trace_t tr;
		UTIL_TraceLine(vecPos, vecPos + dir * 32.0f, MASK_SHOT, this, COLLISION_GROUP_NONE, &tr);

		UTIL_BloodDecalTrace(&tr, BC );

		UTIL_BloodSpray(vecPos, dir, BC, 2.0f, FX_BLOODSPRAY_ALL);
	}
#endif
...
void CRagdollProp::VPhysicsUpdate( IPhysicsObject *pPhysics )
{
	...
	if ( m_allAsleep && GetCollisionGroup() == COLLISION_GROUP_INTERACTIVE_DEBRIS )
	{
		SetCollisionGroup( COLLISION_GROUP_DEBRIS );
		RecheckCollisionFilter();
		SetContextThink( NULL, CURTIME, s_pDebrisContext );
	}
...
void CRagdollProp::FadeOut( float flDelay, float fadeTime ) 
{
	...
	m_flFadeOutStartTime = CURTIME + flDelay;
	m_flFadeScale = 0;
	SetContextThink( &CRagdollProp::FadeOutThink, CURTIME + flDelay + 0.01f, s_pFadeOutContext );
}

bool CRagdollProp::IsFading()
{
	return ( GetNextThink( s_pFadeOutContext ) >= CURTIME );
}

void CRagdollProp::FadeOutThink(void) 
{
	float dt = CURTIME - m_flFadeOutStartTime;
	if ( dt < 0 )
	{
		SetContextThink( &CRagdollProp::FadeOutThink, CURTIME + 0.1, s_pFadeOutContext );
	}
	else if ( dt < m_flFadeTime )
	{
		...
		NetworkStateChanged();
		SetContextThink( &CRagdollProp::FadeOutThink, CURTIME + TICK_INTERVAL, s_pFadeOutContext );
	}
	else
	{
...
#ifdef DARKINTERVAL // DI NEW: Live Ragdolls Experiment (inspired by Dark Messiah) - overloaded with a bForLiveRagdoll check to tell if we were created to serve as one
CBaseEntity *CreateServerRagdoll( CBaseAnimating *pAnimating, int forceBone, const CTakeDamageInfo &info, int collisionGroup, bool bUseLRURetirement, Activity pDeathActivity, bool bBreakable, int iBreakableHealth, bool bForLiveRagdoll )
#else
...
#endif
{
...
#ifdef DARKINTERVAL
	pRagdoll->SetName(pAnimating->GetEntityName());
	pRagdoll->SetNameplateText(pAnimating->m_nameplate_string);
	if (pDeathActivity != ACT_DIERAGDOLL)
	{
		pRagdoll->SetOverlaySequence(pDeathActivity);
	}
#endif
	...
	if ( (info.GetDamageType() & DMG_VEHICLE) && pAnimating->MyNPCPointer() )
	{
		...
		pRagdoll->InitRagdoll( vec3_origin, -1, vec3_origin, pBoneToWorld, pBoneToWorldNext, dt, collisionGroup, true, true 
#ifdef DARKINTERVAL // for gibbing ragdolls
			,bBreakable, iBreakableHealth 
#endif
		);
		...
		for ( int i = 0; i < count; i++ )
		{
			...
			if( pPhysics) pPhysics->ApplyForceCenter( info.GetDamageForce() * pPhysics->GetMass() * massScale );
		}
	}
	else
	{
		pRagdoll->InitRagdoll( info.GetDamageForce(), forceBone, info.GetDamagePosition(), pBoneToWorld, pBoneToWorldNext, dt, collisionGroup, true, true 
#ifdef DARKINTERVAL // for gibbing ragdolls
			,bBreakable, iBreakableHealth
#endif
		);
...
#ifdef DARKINTERVAL	// DI NEW: Live Ragdolls Experiment (inspired by Dark Messiah) - here to tell if we were created to serve as a live ragdoll
	if (bForLiveRagdoll) pRagdoll->AddSpawnFlags(SF_RAGDOLL_FOR_LIVE_RAGDOLL);
#endif	
	return pRagdoll;
}
...
void CRagdollProp::InputStartRadgollBoogie( inputdata_t &inputdata )
{
	...
	CRagdollBoogie::Create( this, 100, CURTIME, duration, 0 );
}
#ifdef DARKINTERVAL
void CRagdollProp::InputSetUnragdollTarget(inputdata_t &inputdata)
{
	CBaseEntity *pInputEnt = gEntList.FindEntityByName(NULL, inputdata.value.String(), NULL, inputdata.pActivator, inputdata.pCaller);
	CBaseAnimating *pUnragdollEnt = dynamic_cast<CBaseAnimating *>(pInputEnt);
	if (pUnragdollEnt != NULL)
	{
		SetUnragdoll(pUnragdollEnt);
	}
}

void CRagdollProp::InputBeginUnragdoll(inputdata_t &inputdata)
{
	if (m_hUnragdoll.Get() != NULL)
	{
		SetThink(&CRagdollProp::SUB_Remove);
		SetNextThink(CURTIME + 0.1f);
	}
	else
	{
		Warning("Cannot unragdoll without unragdoll entity set first!\n");
	}
}

void CRagdollProp::InputUnragdollToEntity(inputdata_t &inputdata)
{
	CBaseEntity *pInputEnt = gEntList.FindEntityByName(NULL, inputdata.value.String(), NULL, inputdata.pActivator, inputdata.pCaller);
	CBaseAnimating *pUnragdollEnt = dynamic_cast<CBaseAnimating *>(pInputEnt);
	if (pUnragdollEnt != NULL)
	{
	//	Vector urOrigin = GetAbsOrigin() + Vector(0,0,1);
	//	QAngle urAngles = GetAbsAngles();
	//	urAngles.x = urAngles.y = 0;
	//	pUnragdollEnt->SetRenderMode(kRenderNone);
	//	pUnragdollEnt->Teleport(&urOrigin, &urAngles, NULL);
		SetUnragdoll(pUnragdollEnt);
		SetThink(&CRagdollProp::SUB_Remove);
		SetNextThink(CURTIME + 0.1f);
	}
