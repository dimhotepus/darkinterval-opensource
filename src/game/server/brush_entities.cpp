...
//	}
	...
	{		
		...
		SetNextThink( CURTIME + .2 );	// leave a magic delay for client to start up
	}
...
void CFuncRotating::RampPitchVol( void )
{
	...
	if( m_flSpeed != 0.0f) EmitSound( filter, entindex(), ep );
}
...
void CFuncRotating::UpdateSpeed( float flNewSpeed )
{
	...
	else if ( (( flOldSpeed != 0 ) && ( m_flSpeed == 0 )) || m_flSpeed == 0 )
	{
