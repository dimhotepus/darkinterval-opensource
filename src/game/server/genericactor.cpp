...
#ifdef DARKINTERVAL
	DEFINE_KEYFIELD(kFlyingActor, FIELD_BOOLEAN, "flying_actor"),
#endif
END_DATADESC()
...
*/
	...
	if ( kFlyingActor )
	{
		// solution for the crappy flying npc mechanic
...
void CFlextalkActor::ProcessSceneEvents( void )
{
	...
	if (GetNumFlexControllers() > 2)
	{
		...
		{
			m_flNextEyeLookTime = CURTIME + 1000.0;
		}
		else if (flex_talk.GetInt() == -3)
		{
			m_flNextEyeLookTime = CURTIME;
			flex_talk.SetValue( "0" );
		}
