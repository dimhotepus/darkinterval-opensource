...
void CAI_BaseNPC::InitDefaultScheduleSR(void)
{
	...
	ADD_DEF_SCHEDULE("SCHED_DI_FALL_IN_RAGDOLL_PUSH", SCHED_DI_FALL_IN_RAGDOLL_PUSH);
#endif
}
...
#ifdef DARKINTERVAL	// DI NEW: Live Ragdolls Experiment (inspired by Dark Messiah)
	AI_LOAD_DEF_SCHEDULE(CAI_BaseNPC, SCHED_DI_FALL_IN_RAGDOLL_PUSH);
#endif
	return true;
}
...
(
	...
	"	Tasks"
	"		TASK_STOP_MOVING		1"
	"		TASK_PLAY_ACT_IDLE		0" // DI change. Attempt to fix randomly resetting ACT_IDLE (in case of having more than 1) by using a separate task that can check if the activity is finished.
	"		TASK_WAIT				5"
	"		TASK_WAIT_PVS			0"
	""
	"	Interrupts"
...
#else // don't remember why it was disabled...
AI_DEFINE_SCHEDULE
(
	SCHED_DIE,
...
//	"		TASK_SET_TOLERANCE_DISTANCE		48"
	...
	"		TASK_GET_PATH_TO_RANDOM_NODE	1500" // DI's nodes are more sparse owing to its large maps
	"		TASK_RUN_PATH					0"
	"		TASK_WAIT_FOR_MOVEMENT			0"
	""
	"	Interrupts"
...
//	"		TASK_SET_TOLERANCE_DISTANCE		48"
	"		TASK_SET_ROUTE_SEARCH_TIME		5"	// Spend 5 seconds trying to build a path if stuck
	"		TASK_GET_PATH_TO_RANDOM_NODE	500" // DI's nodes are more sparse owing to its large maps
	"		TASK_WALK_PATH					0"
	"		TASK_WAIT_FOR_MOVEMENT			0"
	"		TASK_WAIT_PVS					0"
	""
...
(
	...
	"	Tasks"
	"		TASK_SET_ROUTE_SEARCH_TIME		5"	// Spend 5 seconds trying to build a path if stuck
	"		TASK_GET_PATH_TO_RANDOM_NODE	600" // DI's nodes are more sparse owing to its large maps
	"		TASK_WALK_PATH					0"
	"		TASK_WAIT_FOR_MOVEMENT			0"
	""
	"	Interrupts"
...
(
	...
	"	Tasks"
	"		TASK_SET_FAIL_SCHEDULE SCHEDULE:SCHED_FLINCH_PHYSICS"
	"		TASK_DI_FALL_IN_RAGDOLL_PUSH_BEGIN 0" // hide the npc, made them intangible
	"		TASK_DI_FALL_IN_RAGDOLL_PUSH 0" // create ragdoll copy
	"		TASK_DI_WAIT_UNTIL_RAGDOLL_ENDS_FALL 0" // continuously check ragdoll position, follow it (teleport), measure speed and ground entity
	"		TASK_DI_WAIT_UNTIL_RAGDOLL_ENDS_FALL_END 0" // once it slowed down and is resting, tell it to unragdoll back into us, and un-hide ourselves
	"		TASK_PLAY_SEQUENCE ACTIVITY:ACT_STAND" // the ragdoll is gone, we're visible - play the stand up animation
	""
	"	Interrupts"
	""
	//"COND_DI_STANDUP" // the condition can be forced via input or if it's been too long and we have to stand up now
);
#endif
