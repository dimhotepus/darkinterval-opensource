...
void CAI_Navigator::Restore( IRestore &restore )
{
	...
	if ( minPathArray.Count() )
	{
		...
		m_flTimeClipped = CURTIME + 1000; // time passes between restore and onrestore
	}
...
{
#ifdef DARKINTERVAL // don't remember why this magic number is here. Tweaking?
	return GetOuter()->MaxYawSpeed() * 0.8;
#else
...
bool CAI_Navigator::PrependLocalAvoidance( float distObstacle, const AIMoveTrace_t &directTrace )
{
	...
	if ( m_flTimeLastAvoidanceTriangulate >= CURTIME )
		return false; // Only triangulate once per think at most

	m_flTimeLastAvoidanceTriangulate = CURTIME;
...
{
#ifdef WIN32
	Assert( (int)AIN_NO_NODE == NO_NODE );
#endif
...
bool CAI_Navigator::OnObstructionPreSteer( AILocalMoveGoal_t *pMoveGoal, float distClear, AIMoveResult_t *pResult )
{
	...
	if ( 
#ifndef DARKINTERVAL
		ai_vehicle_avoidance.GetBool() &&
#endif
		pMoveGoal->directTrace.pObstruction != NULL && pMoveGoal->directTrace.pObstruction->GetServerVehicle() != NULL )
	{
...
bool CAI_Navigator::OnFailedSteer( AILocalMoveGoal_t *pMoveGoal, float distClear, AIMoveResult_t *pResult )
{
	...
	if ( m_vPosBeginFailedSteer == vec3_invalid || ( m_vPosBeginFailedSteer - GetAbsOrigin() ).LengthSqr() > Square(MOVE_TOLERANCE) )
	{
		...
		m_timeBeginFailedSteer = CURTIME;
	}
	else if ( GetNavType() == NAV_GROUND && 
			  CURTIME - m_timeBeginFailedSteer > TIME_TOLERANCE && 
...
AIMoveResult_t CAI_Navigator::MoveEnact( const AILocalMoveGoal_t &baseMove )
{
	...
	result = GetLocalNavigator()->MoveCalc( &move, ( m_flLastSuccessfulSimplifyTime == CURTIME ) );
...
#ifdef DARKINTERVAL
	else if (goalType == NAV_SWIM && curType != NAV_SWIM) // DI NEW - water nodes
	{
		AssertMsg(0, ("GetNavType() == NAV_SWIM"));
		return false;
	}
#endif
	// --------------------------------
...
bool CAI_Navigator::Move( float flInterval ) 
{
	...
	if ( !GetOuter()->OverrideMove( flInterval ) )
	{
		...
		if ( bHasGoal )
		{
			if ( bIsTurning )
			{
				if ( CURTIME - GetPath()->GetStartTime() > 5 )
				{
		...
		if ( fShouldMove )
		{
			...
			if ( result == AINR_OK )
			{
				...
				while ( moveResult >= AIMR_OK && !m_fNavComplete )
				{
					...
					case NAV_FLY:
#ifdef DARKINTERVAL
					case NAV_SWIM: // DI NEW - water nodes
#endif
...
bool CAI_Navigator::SimplifyPath( bool bFirstForPath, float scanDist )
{
...
#ifdef DARKINTERVAL
		|| navType == NAV_SWIM // DI NEW - water nodes
#endif
		)
	{
		AI_Waypoint_t *pCurWaypoint = GetPath()->GetCurWaypoint();
		...
		bFullSimplify = ( m_flNextSimplifyTime <= CURTIME );
		...
		if ( bFullSimplify )
		{
			...
			m_flNextSimplifyTime = CURTIME + simplifyDelay;
		...
		{
			if ( !AIStrongOpt() || CURTIME - m_flLastSuccessfulSimplifyTime > QUICK_SIMPLIFY_TIME_DELAY[AIStrongOpt()] )
			{
	...
	if ( bRetVal )
	{
		m_flLastSuccessfulSimplifyTime = CURTIME;
...
bool CAI_Navigator::SimplifyFlyPath(  const AI_ProgressFlyPathParams_t &params )
{
	...
	if ( m_flNextSimplifyTime > CURTIME)
		return false;

	m_flNextSimplifyTime = CURTIME + FLY_ROUTE_SIMPLIFY_TIME_DELAY;
...
bool CAI_Navigator::FindPath( bool fSignalTaskStatus, bool bDontIgnoreBadLinks )
{
	...
	if ( bRetrying )
	{
		...
		if (m_timePathRebuildFail < CURTIME)
		{
		...
		else if ( m_timePathRebuildNext > CURTIME )
		{
			return false;
		}
	}
	...
	else
	{
		if ( !bRetrying )
		{
			...
			m_timePathRebuildFail = CURTIME + m_timePathRebuildMax;
		}
		m_timePathRebuildNext = CURTIME + m_timePathRebuildDelay;
		return false;
	}
	return true;
}
	...
	if ( endID != NO_NODE )
	{
		...
		if ( bBlockAll )
		{
			...
			for ( int i = 0; i < pDestNode->NumLinks(); i++ )
			{
				...
				pLink->m_timeStaleExpires = CURTIME + 4.0;
				didMark = true;
			}
...
bool CAI_Navigator::DoFindPathToPos(void)
{
	...
	if ( CURTIME - m_flTimeClipped > 0.11  || m_bLastNavFailed )
		m_pClippedWaypoints->RemoveAll();
...
#ifdef DARKINTERVAL
	case NAV_SWIM:
		return Vector(50, 50, 255); // DI NEW - water nodes
#endif
	default:
		return Vector(255,0,0);
	}
