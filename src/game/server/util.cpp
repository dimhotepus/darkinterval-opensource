...
void UTIL_Remove( IServerNetworkable *oldObj )
{
	...
	if ( pBaseEnt )
	{
#ifdef PORTAL_COMPILE //make sure entities are in the primary physics environment for the portal mod, this code should be safe even if the entity is in neither extra environment
...
void UTIL_SetModel( CBaseEntity *pEntity, const char *pModelName )
{
	...
	if ( i == -1 )	
	{
		Warning("%i/%s - %s:  UTIL_SetModel:  not precached: %s; forcing Late Precache\n", pEntity->entindex(),
...
static edict_t *UTIL_GetCurrentCheckClient()
{
	...
	float delta = CURTIME - g_CheckClient.m_lastchecktime;
	if ( delta >= 0.1 || delta < 0 )
	{
		g_CheckClient.m_lastcheck = UTIL_GetNewCheckClient( g_CheckClient.m_lastcheck );
		g_CheckClient.m_lastchecktime = CURTIME;
	}
