...
void CAI_ShotRegulator::Reset( bool bStartShooting )
{
	...
	if ( bStartShooting )
	{
		m_flNextShotTime = CURTIME;
		m_bInRestInterval = false;
	}
	else
	{
		m_flNextShotTime = CURTIME + random->RandomFloat( m_flMinRestInterval, m_flMaxRestInterval );
...
{ 
	return ( !m_bDisabled && (m_flNextShotTime <= CURTIME) ); 
}
...
void CAI_FreePass::Update( )
{
	...
	if ( !HasPass() )
	{
		...
		if ( timePlayerLastSeen == AI_INVALID_TIME || CURTIME - timePlayerLastSeen > .15 ) // If didn't see the player last think
		{
			trace_t tr;
			...
			if ( tr.fraction != 1.0 && tr.m_pEnt != pTarget )
			{
				...
				{
					if ( ( timePlayerLastSeen == AI_INVALID_TIME || CURTIME - timePlayerLastSeen > m_Params.timeToTrigger ) &&
						 ( lastTimeDamagedBy == AI_INVALID_TIME || CURTIME - lastTimeDamagedBy > m_Params.timeToTrigger ) )
					{
						m_FreePassTimeRemaining = m_Params.duration;
						m_FreePassMoveMonitor.SetMark( pTarget, m_Params.moveTolerance );
					}
...
bool CAI_FreePass::ShouldAllowFVisible(bool bBaseResult )
{
	...
	if ( m_Params.peekTime > 0.1 )
	{
		...
		if ( ( lastTimeSeen == AI_INVALID_TIME || CURTIME - lastTimeSeen > m_Params.peekTime ) &&
			 ( lastTimeDamagedBy == AI_INVALID_TIME || CURTIME - lastTimeDamagedBy > m_Params.peekTimeAfterDamage ) )
		{
			Vector vToTarget;
...
bool CTraceFilterNav::ShouldHitEntity( IHandleEntity *pHandleEntity, int contentsMask )
{
	...
	//If I'm flagged as using the new collision method, then ignore the player when trying
