...
#ifdef DARKINTERVAL
	DEFINE_INPUTFUNC(FIELD_VOID, "GetLastPathName", InputGetLastPathName),
#endif
	// Outputs
...
#ifdef DARKINTERVAL
	AssertMsg(!m_pnext.IsValid() || m_pnext.Get() != NULL, "Trying to set next path track (%s) but it's not valid or was deleted!", GetDebugName() );
#else
...
#ifdef DARKINTERVAL
	AssertMsg( !m_pprevious.IsValid() || m_pprevious.Get() != NULL, "Trying to get previous path track (%s) but it's not valid or was deleted!", GetDebugName());
#else
...
#ifdef DARKINTERVAL // DI NEW - function to iterate down the path chain and return the last one
CPathTrack *CPathTrack::GetLastPath(void)
{
	// The paths shouldn't normally be getting deleted so assert that if it was set, it's valid.
	AssertMsg(!m_pnext.IsValid() || m_pnext.Get() != NULL, "Trying to set next path track (%s) but it's not valid or was deleted!", GetDebugName());

	CPathTrack* m_pLast = this;

	int myentindex = entindex();
	
	while (m_pLast->GetNext() != NULL)
	{
		m_pLast = m_pLast->GetNext();

		if (m_pLast->GetNext() != NULL)
		{
			if (m_pLast->GetNext()->entindex() == myentindex)
			{
				return m_pLast; // we ended up in a loop. Return the last path before the start.
			}
			continue;
		}
		return m_pLast;
...
void CPathTrack::InputGetLastPathName(inputdata_t &inputdata)
{
	Msg("%s\n", STRING(GetLastPath()->GetEntityName()));
}
#endif // DARKINTERVAL
//-----------------------------------------------------------------------------
