...
#ifdef DARKINTERVAL
		(HasSpawnFlags(SF_TRIGGER_ALLOW_NPCS) && FClassnameIs(pOther, "npc_furniture")) ||
		(HasSpawnFlags(SF_TRIGGER_ALLOW_NPCS) && FClassnameIs(pOther, "cycler")) ||
		(HasSpawnFlags(SF_TRIGGER_ALLOW_NPCS) && FClassnameIs(pOther, "cycler_actor")) ||
		(HasSpawnFlags(SF_TRIGGER_ALLOW_NPCS) && FClassnameIs(pOther, "cycler_flex")) ||
#endif
		(HasSpawnFlags(SF_TRIGGER_ALLOW_PUSHABLES) && FClassnameIs(pOther, "func_pushable")) ||
#ifdef DARKINTERVAL
		(HasSpawnFlags(SF_TRIGGER_ALLOW_PUSHABLES) && FClassnameIs(pOther, "prop_dynamic")) ||
#endif
...
#ifdef DARKINTERVAL // make sure to prevent double OnStartTouch firing in trigger_once
		if (ClassMatches("trigger_once")) Disable();
#endif
	}
}
...
void CTriggerRemove::Spawn( void )
{
	...
	InitTrigger();
#ifdef DARKINTERVAL
	RemoveSpawnFlags(SF_TRIGGER_ALLOW_CLIENTS); // NEVER attempt to remove the client.
#endif
}
...
void CTriggerRemove::Touch( CBaseEntity *pOther )
{
	if (!PassesTriggerFilters(pOther))
		return;
#ifdef DARKINTERVAL
	if (pOther->IsPlayer()) return; // NEVER attempt to remove the player.
#endif
	UTIL_Remove( pOther );
}
...
void CTriggerHurt::Spawn( void )
{
	...
	if (m_bitsDamageInflict & DMG_RADIATION)
	{
		...
		SetNextThink( CURTIME + random->RandomFloat(0.0, 0.5) );
	}
}
...
void CTriggerHurt::RadiationThink( void )
{
	...
	float dt = CURTIME - m_flLastDmgTime;
	if ( dt >= 0.5 )
	{
		HurtAllTouchers( dt );
	}

	SetNextThink( CURTIME + 0.25 );
}
...
int CTriggerHurt::HurtAllTouchers( float dt )
{
	...
	if( m_damageModel == DAMAGEMODEL_DOUBLE_FORGIVENESS )
	{
		...
		else
		{
			...
			m_flDmgResetTime = CURTIME + TRIGGER_HURT_FORGIVE_TIME;
		}
	}
...
#ifdef DARKINTERVAL
	DEFINE_KEYFIELD(m_nTimesToFire, FIELD_INTEGER, "timestofire"),
#endif
	// Outputs
...
#ifdef DARKINTERVAL
	if (m_nTimesToFire == 0) // Assume this is a mapping mistake and remove the trigger. 
	{
		Warning("trigger_multple (%s) was created with TimesToFire set to 0! Removing...\n", GetDebugName());
		SetTouch(NULL);
...
void CTriggerMultiple::ActivateMultiTrigger(CBaseEntity *pActivator)
{
	...
	if (m_nTimesToFire == 0)
	{
		...
		if( developer.GetInt() > 0)
			Msg("trigger_multiple (%s) reached TimesToFire limit and will be removed\n", GetDebugName());
		SetTouch(NULL);
		SetNextThink(CURTIME + 0.1f);
		SetThink(&CTriggerMultiple::SUB_Remove);
		return;
...
void CTriggerLook::StartTouch(CBaseEntity *pOther)
{
	...
	if (pOther->IsPlayer() && m_flTimeoutDuration)
	{
		...
		SetNextThink(CURTIME + m_flTimeoutDuration);
	}
}
...
void CTriggerLook::Touch(CBaseEntity *pOther)
{
	...
	if (pOther->IsPlayer())
	{
		...
		if (fDotPr > m_flFieldOfView)
		{
			...
			{
				m_flLookTimeLast	= CURTIME;
				m_flLookTimeTotal	= 0;
			}
			else
			{
				m_flLookTimeTotal	+= CURTIME - m_flLookTimeLast;
				m_flLookTimeLast	=  CURTIME;
			}
...
private:
	...
	bool m_bTouched; 
#ifdef DARKINTERVAL
	bool m_bShowLoadingImage = true; // on some levels, such as ch02_arcade, it is used to have "seamless" transitions
#endif
	// Outputs
...
#ifdef DARKINTERVAL
	DEFINE_KEYFIELD(m_bShowLoadingImage, FIELD_BOOLEAN, "showloadingimage"),
#endif
...
#ifdef DARKINTERVAL
	engine->ClientCommand(pPlayer->edict(), "cl_showloadingimage %i", m_bShowLoadingImage);
#endif
...
#ifdef DARKINTERVAL // for things like doors, which shouldn't ever be twinned on transitions
	if (caps & FCAP_DONT_TRANSITION_EVER)
	{
		if (true)
		{
			Msg("IGNORED due to being marked \"Don't transition ever - DI ADD\".\n");
		}
		return 0;
	}
#endif
...
#ifdef DARKINTERVAL
	// Outputs
	COutputEvent m_OnTrigger; // DI NEW - just like regular triggers.
#endif
};
...
void CTriggerSave::Touch( CBaseEntity *pOther )
{
	...
	if ( !pOther->IsPlayer() )
		return;
#ifdef DARKINTERVAL
	m_OnStartTouch.FireOutput(this, this); // DI NEW
	m_OnTrigger.FireOutput(this, this); // DI NEW
#endif
	if ( m_fDangerousTimer != 0.0f )
	{
		if ( g_ServerGameDLL.m_fAutoSaveDangerousTime != 0.0f && g_ServerGameDLL.m_fAutoSaveDangerousTime >= CURTIME )
		{
			// A previous dangerous auto save was waiting to become safe
			CBasePlayer *pPlayer = UTIL_PlayerByIndex( 1 );

			if ( pPlayer->GetDeathTime() == 0.0f || pPlayer->GetDeathTime() > CURTIME )
			{
	...
	if ( m_fDangerousTimer != 0.0f )
	{
		...
		if (pPlayer && pPlayer->GetHealth() >= m_minHitPoints)
		{
			...
			g_ServerGameDLL.m_fAutoSaveDangerousTime = CURTIME + m_fDangerousTimer;
		}
	}
	else
	{
...
void CTriggerGravity::GravityTouch( CBaseEntity *pOther )
{
	...
	// Only save on clients
	if ( !pOther->IsPlayer() )
		return; // DI CHANGE: what the hell is this. The trigger literally has
				// all the spawnflags to specifically select different types of objects.
				// but this crap here makes it useless!
#else
	if (!PassesTriggerFilters(pOther))
	{
	...
	pOther->VelocityPunch(Vector(0, 0, 1));
	if (!pOther->IsPlayer())
	{
		QAngle vecAngVel(random->RandomFloat(-10, -50), random->RandomFloat(-10, -50), 0);
		pOther->SetLocalAngularVelocity(vecAngVel);
	}
#endif
	pOther->SetGravity(GetGravity());
}
...
BEGIN_DATADESC( CTriggerCamera )
	...
	DEFINE_INPUTFUNC( FIELD_VOID, "Disable", InputDisable ),
#ifdef DARKINTERVAL
	DEFINE_INPUTFUNC(FIELD_FLOAT, "SetSpeed", InputSetSpeed), // DI NEW
#endif
	// Function Pointers
...
#ifdef DARKINTERVAL
//------------------------------------------------------------------------------
// Purpose: DI NEW - be able to regulate point_viewcountrol's speed
//------------------------------------------------------------------------------
void CTriggerCamera::InputSetSpeed(inputdata_t &inputdata)
{
...
void CTriggerCamera::Enable( void )
{
	...
	m_flReturnTime = CURTIME + m_flWait;
...
void CTriggerCamera::FollowTarget( )
{
	...
	if ( !HasSpawnFlags(SF_CAMERA_PLAYER_INFINITE_WAIT) && (!m_hTarget || m_flReturnTime < CURTIME) )
	{
		Disable();
		return;
	}
...
void CTriggerCamera::Move()
{
...
#endif
	{
		...
		if ( m_moveDistance <= 0 )
		{
			...
			else 
			{
				...
				m_flStopTime = CURTIME + m_pPath->GetDelay();
			}
		}

		if ( m_flStopTime > CURTIME )
	...
	else if (m_bInterpolatePosition)
	{
		...
		float tt = (CURTIME - m_flInterpStartTime) / kflPosInterpTime;
		if (tt >= 1.0f)
		{
...
public:
	simresult_e Simulate( IPhysicsMotionController *pController, IPhysicsObject *pObject, float deltaTime, Vector &linear, AngularImpulse &angular )
	{
		...
		vWindAngle[1] = m_nWindYaw+(30*cos(nNoiseMod * CURTIME + nNoiseMod));
		Vector vWind;
		AngleVectors(vWindAngle,&vWind);

		// Add lift with noise
		vWind.z = 1.1 + (1.0 * sin(nNoiseMod * CURTIME + nNoiseMod));
...
void CTriggerWind::Spawn( void )
{
	...
	SetContextThink( &CTriggerWind::WindThink, CURTIME, WIND_THINK_CONTEXT );
}
...
void CTriggerWind::InputEnable( inputdata_t &inputdata )
{
	...
	SetContextThink( &CTriggerWind::WindThink, CURTIME + 0.1f, WIND_THINK_CONTEXT );
}
...
{
	// By default...
	SetContextThink( &CTriggerWind::WindThink, CURTIME + 0.1, WIND_THINK_CONTEXT );
...
float CTriggerVPhysicsMotion::LinearLimit()
{
	...
	float dt = CURTIME - m_linearLimitStartTime;
...
void CTriggerVPhysicsMotion::InputSetVelocityLimitTime( inputdata_t &inputdata )
{
	...
	m_linearLimitStartTime = CURTIME;
