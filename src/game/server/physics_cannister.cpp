...
#ifdef DARKINTERVAL 
#include "world.h" // to be able to assign unaccounted damage to world
#endif
...
#ifdef DARKINTERVAL
	DEFINE_FIELD( m_activate_cnt, FIELD_INTEGER),
#endif
	// Physics Influence
...
int CPhysicsCannister::OnTakeDamage( const CTakeDamageInfo &info )
{
	...
	if ( !m_takedamage )
		return 0;
#ifdef DARKINTERVAL
	CBaseEntity *pAttacker = GetWorldEntity();
	if (info.GetAttacker() != NULL) pAttacker = info.GetAttacker();
#endif
	if ( !m_active )
	{
		...
		{
#ifdef DARKINTERVAL
			if (HasSpawnFlags(SF_CANNISTER_EXPLODE)) // DI FIX: there used to be no check whatsoever for this
			{
				Explode(pAttacker);
			}
			else
			{
				CannisterActivate(pAttacker, g_vecAttackDir);
			}
			return 1;
#else
		...
		else
		{
			...
			if ( ( info.GetDamageType() & DMG_BLAST ) ||
				( ( info.GetDamageType() & ( DMG_CLUB | DMG_SLASH | DMG_CRUSH ) ) ) )
			{
			...
			if ( info.GetDamageType() & ( DMG_BULLET ) || info.GetDamageType() & ( DMG_BUCKSHOT ) )
			{
				if ( m_iHealth <= m_iMaxHealth * 0.5 )
				{
					...
					if ( HasSpawnFlags(SF_CANNISTER_EXPLODE) ) // DI FIX: there used to be no check whatsoever for this
					{
						if ( m_iHealth <= m_iMaxHealth * 0.25 )
						{
							Explode(pAttacker);
						}
					}
...
void CPhysicsCannister::CannisterActivate( CBaseEntity *pActivator, const Vector &thrustOffset )
{
	...
	m_active = true;
	m_activateTime = CURTIME;
	SetNextThink( CURTIME + m_thrustTime );
...
#ifdef DARKINTERVAL
	if( m_activate_cnt == 0)
		m_onActivate.FireOutput(pAttacker, this, 0);
#endif
	// don't recurse
	m_takedamage = 0;
	Deactivate();
