...
void CTimerEntity::ResetTimer( void )
{
	...
	SetNextThink( CURTIME + m_flRefireTime );
}
...
void CTimerEntity::InputSubtractFromTimer( inputdata_t &inputdata )
{
	...
	float flNextThink = GetNextThink();
	if ( ( flNextThink - CURTIME ) <= inputdata.value.Float() )
	{
...
int CTimerEntity::DrawDebugTextOverlays( void ) 
{
	...
	if (m_debugOverlays & OVERLAY_TEXT_BIT) 
	{
		...
		{
			float flNextThink = GetNextThink();
			Q_snprintf( tempstr, sizeof( tempstr ), "      firing in: %.2f sec", flNextThink - CURTIME );
			EntityText( text_offset, tempstr, 0);
			text_offset++;
		}
	}
...
#ifdef DARKINTERVAL // fixes the output name being confused with another keyfield
...
bool CMathCounter::KeyValue(const char *szKeyName, const char *szValue)
{
	...
	{
#ifdef DARKINTERVAL // fixes being unable to use floats in math counter
...
void CLogicAutosave::InputSaveDangerous( inputdata_t &inputdata )
{
	...
	if ( pPlayer->GetHealth() >= m_minHitPoints )
	{
		...
		g_ServerGameDLL.m_fAutoSaveDangerousTime = CURTIME + inputdata.value.Float();
	...
	void SaveThink()
	{
		...
		if ( pPlayer )
		{
			...
			else
			{
				...
				else if ( m_flTimeToTrigger > 0 && CURTIME - m_flStartTime > m_flTimeToTrigger )
				{
					m_flStartTime = -1;
				}
			}
		...
		SetNextThink( CURTIME + thinkInterval );
	}
