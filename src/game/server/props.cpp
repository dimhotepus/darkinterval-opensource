...
#ifdef DARKINTERVAL
#include "particle_system.h" // for breakable dynamics that emit particles
#include "te_effect_dispatch.h"
#include "weapon_stunstick.h" // because they can spawn in crate loot
#endif
...
#ifdef DARKINTERVAL
#define NUM_SUPPLY_CRATE_HUD_HINTS		2
#else
...
#ifdef DARKINTERVAL
ConVar prop_impale_min_speed("prop_impale_min_speed", "600");
#endif
#ifdef DARKINTERVAL
	#define PROP_FLARE_LIFETIME 60.0f
	#define PROP_FLARE_IGNITE_SUBSTRACT 5.0f
	CBaseEntity *CreateFlare(Vector vOrigin, QAngle Angles, CBaseEntity *pOwner, float flDuration, float goalHeatLevel = 0.0f);
...
#ifndef DARKINTERVAL // DI allows mixing prop usage
...
void CBreakableProp::HandleFirstCollisionInteractions( int index, gamevcollisionevent_t *pEvent )
{
	...
	if (HasInteraction(PROPINTER_PHYSGUN_FIRST_PAINT))
	{
		...
		if ( tr.m_pEnt )
		{
#ifndef DARKINTERVAL // DI allows it, it's funny
#ifdef HL2_DLL
...
void CPhysicsProp::HandleAnyCollisionInteractions( int index, gamevcollisionevent_t *pEvent )
{
	...
	if ( HasInteraction( PROPINTER_PHYSGUN_FIRST_IMPALE ) )
	{
...
#ifndef DARKINTERVAL
		if ( speed < prop_impale_min_speed.GetFloat() )
#else
		if ( speed < 1000.0f )
#endif
		{
		...
		else if ( pHitEntity->MyNPCPointer() )
		{
			...
			IPhysicsObject *pObj = VPhysicsGetObject();		
#ifndef DARKINTERVAL // as DI allows friendly fire, it allows friendly impalement
...
#ifndef DARKINTERVAL // DI wants stick sawblades to remain solid // todo - maybe make this into some map/prop setting, not sure
...
BEGIN_DATADESC( CBreakableProp )
	...
	DEFINE_FIELD(m_iszBreakableDamageSound, FIELD_STRING),
	DEFINE_FIELD(m_iszStrainSound, FIELD_STRING),
	DEFINE_FIELD(m_nextstrainsound_time, FIELD_TIME),
#endif
...
#ifdef DARKINTERVAL
	DEFINE_INPUTFUNC( FIELD_INTEGER, "SetWeightMultiplier", InputSetWeightMultiplier ), // for crate loot
#endif
	// Outputs
...
#ifdef DARKINTERVAL	// for crate loot
	DEFINE_FIELD( m_WeightMultiplier_int, FIELD_INTEGER ),
#endif
END_DATADESC()
...
#ifdef DARKINTERVAL
	m_WeightMultiplier_int = 0;
	m_nextstrainsound_time = 0;
#endif
}
...
void CBreakableProp::Spawn()
{
	...
	if ( ( m_iHealth == 0 ) ||
        ( !m_iNumBreakableChunks && 
		    !HasInteraction(PROPINTER_PHYSGUN_BREAK_EXPLODE_IGNITE) &&
#ifdef DARKINTERVAL // DI adds more explosion types to props (wip)
			!HasInteraction( PROPINTER_PHYSGUN_BREAK_EXPLODE_PLASMA ) &&
			!HasInteraction( PROPINTER_PHYSGUN_BREAK_EXPLODE_CRYO) &&
#endif
	...
	else
	{
		...
		{
			if ( HasInteraction(PROPINTER_PHYSGUN_BREAK_EXPLODE_IGNITE) ||
#ifdef DARKINTERVAL // DI adds more explosion types to props (wip)
				HasInteraction( PROPINTER_PHYSGUN_BREAK_EXPLODE_PLASMA ) ||
				HasInteraction( PROPINTER_PHYSGUN_BREAK_EXPLODE_CRYO) ||
#endif
	...
	if (pModelName)
	{
		...
		if (Q_strcmp(pModelName, "models/props_junk/wood_crate001a.mdl") == 0
			|| Q_strcmp(pModelName, "models/props_junk/wood_crate002a.mdl") == 0)
		{
			if (RandomFloat(0, 1) > 0.5f)
			{
				m_WeightMultiplier_int = RandomInt(1, 5);
			//	Msg("Spawned filled crate with weight mult %i", m_WeightMultiplier_int);
			}
			else
			{
				m_WeightMultiplier_int = 0;
...
void CBreakableProp::CopyFadeFrom( CBreakableProp *pSource )
{
	...
	if ( m_flFadeScale != m_flDefaultFadeScale )
	{
		...
		if ( flNextThink < CURTIME + TICK_INTERVAL )
		{
			flNextThink = CURTIME + TICK_INTERVAL;
		}
...
#ifdef DARKINTERVAL // for crate loot
//-----------------------------------------------------------------------------
void CBreakableProp::InputSetWeightMultiplier(inputdata_t &inputdata)
{
	m_WeightMultiplier_int = inputdata.value.Int();
}
#endif
#ifdef DARKINTERVAL
void CBreakableProp::StrainSound(void)
{
	if (!VPhysicsGetObject()) return;

	if (CURTIME < m_nextstrainsound_time) return;
	
	int pitch;
	float fvol;
	const char *soundname = NULL;
	int material = VPhysicsGetObject()->GetMaterialIndex();
	...
	// DI NEW - bring back strain sounds for props
	if (GetStrainSound() != NULL_STRING)
	{
		EmitSound(GetStrainSound().ToCStr());
		m_nextstrainsound_time = CURTIME + RandomFloat(3, 5);
		return; 
	}
	...
	case matUnbreakableGlass:
		soundname = "Strain.MatGlass";
		break;

	case matWood:
		soundname = "Strain.MatWood";
		break;

	case matMetal:
		soundname = "Strain.MatMetal";
		break;

	case matRocks:
	case matCinderBlock:
		soundname = "Strain.MatConcrete";
		break;

	case matComputer:
		soundname = "Strain.Computer";
		break;

	case matWeb:
		soundname = "Web.Impact";
		break;
...
void CBreakableProp::BreakablePropTouch( CBaseEntity *pOther )
{
	...
	{
		if (HasSpawnFlags(SF_PHYSPROP_PRESSURE))
		{
			...
			m_hBreaker = pOther;
#ifdef DARKINTERVAL
			if (m_pfnThink != (void (CBaseEntity::*)())&CBreakableProp::BreakThink && m_flPressureDelay >= 0) // allow "-1" to have strain sound, but no breaking
#else
...
#endif
			{
				...
				// Add optional delay 
				SetNextThink(CURTIME + m_flPressureDelay);
			}
		}
	}
	...
	if ( m_hFlareEnt )
	{
		...
		if ( pNPC && pNPC->AllowedToIgnite() && pNPC->AllowedToIgniteByFlare() && pNPC->IsOnFire() == false )
		{
			pNPC->Ignite( 25.0f );
...
#endif
#ifdef DARKINTERVAL
	// DI NEW - add a damage sound override to propdata.
	// We can't easily have metal panels emit 
	// bullet/melee impact sounds to indicate they are breakable
	// (like func_breakable does), because those sounds
	// depend on SURFACEPROP, not PROPDATA.
	// But we often use those panels as statics, and it means
	// with the new surfaceprop, they'll sound as if they're breakable...
	// So we have to hack and imitate it in this here code.
	if (GetBreakableDamageSound() != NULL_STRING)
	{
		EmitSound(GetBreakableDamageSound().ToCStr());
	}
#endif
...
#ifdef DARKINTERVAL // for OnTakeDamage interactions that need the direction and position of attack
void CBreakableProp::TraceAttack(const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr)
{
	if ( HasInteraction(PROPINTER_BULLET_BREAK_SPRAY_LIQUID) && ( info.GetDamageType() & DMG_BULLET ) )
	{
		float x = fabs(WorldAlignMins().x - WorldAlignMaxs().x);
		float y = fabs(WorldAlignMins().y - WorldAlignMaxs().y);
		float z = fabs(WorldAlignMins().z - WorldAlignMaxs().z);
		float propwidth = min(min(x, y), z);
		QAngle backAng;
		VectorAngles(vecDir, backAng);
		// create first spray, that's facing toward the attacker
		CParticleSystem *pParticle = ( CParticleSystem * )CreateEntityByName("info_particle_system");
		if ( pParticle != NULL )
		{
			// Setup our basic parameters
			pParticle->KeyValue("start_active", "1");
			pParticle->KeyValue("effect_name", "plasma_trail_1");
			pParticle->SetParent(this);
			pParticle->SetAbsOrigin(ptr->endpos);
			pParticle->SetAbsAngles(backAng);
			DispatchSpawn(pParticle);
			pParticle->Activate();
			pParticle->SetThink(&CBaseEntity::SUB_Remove);
			pParticle->SetNextThink(CURTIME + 5.0f);
		}
		VectorAngles(-vecDir, backAng);
		// create the second spray, from the exit bullet hole
		CParticleSystem *pParticle2 = ( CParticleSystem * )CreateEntityByName("info_particle_system");
		if ( pParticle2 != NULL )
		{
			// Setup our basic parameters
			pParticle2->KeyValue("start_active", "1");
			pParticle2->KeyValue("effect_name", "plasma_trail_1");
			pParticle2->SetParent(this);
			pParticle2->SetAbsOrigin(ptr->endpos + vecDir * propwidth);
			pParticle2->SetAbsAngles(backAng);
			DispatchSpawn(pParticle2);
			pParticle2->Activate();
			pParticle2->SetThink(&CBaseEntity::SUB_Remove);
			pParticle2->SetNextThink(CURTIME + 5.0f);
		}
	//	NDebugOverlay::Cross3D(ptr->endpos, 2, 255, 155, 25, false, 5);
	//	NDebugOverlay::Cross3D(ptr->endpos + vecDir * propwidth, 2, 255, 155, 25, false, 5);
		RemoveInteraction(PROPINTER_BULLET_BREAK_SPRAY_LIQUID);
	}
...
bool CBreakableProp::OnAttemptPhysGunPickup( CBasePlayer *pPhysGunUser, PhysGunPickup_t reason )
{
	...
	if ( m_nPhysgunState == PHYSGUN_ANIMATE_ON_PULL )
	{
		...
		if ( iSequence != ACTIVITY_NOT_AVAILABLE )
		{
			...
			SetContextThink( &CBreakableProp::AnimateThink, CURTIME + 0.1, s_pPropAnimateThink );
...
void CBreakableProp::AnimateThink( void )
{
	if ( m_nPhysgunState == PHYSGUN_ANIMATE_IS_PRE_ANIMATING || m_nPhysgunState == PHYSGUN_ANIMATE_IS_POST_ANIMATING )
	{
		...
		DispatchAnimEvents( this );
		SetNextThink( CURTIME + 0.1, s_pPropAnimateThink );
...
{
	m_flFadeScale = 0.0f;
	SetContextThink( NULL, CURTIME, s_pFadeScaleThink );
}
...
void CBreakableProp::RampToDefaultFadeScale()
{
	...
	{
		SetContextThink( &CBreakableProp::RampToDefaultFadeScale, CURTIME + TICK_INTERVAL, s_pFadeScaleThink );
	}
}
...
void CBreakableProp::OnPhysGunPickup( CBasePlayer *pPhysGunUser, PhysGunPickup_t reason )
{
	...
	{
		SetContextThink( &CBreakableProp::RampToDefaultFadeScale, CURTIME + 2.0f, s_pFadeScaleThink );
	}
	...
	SetPhysicsAttacker( pPhysGunUser, CURTIME );
...
{
	// Create the flare
	CBaseEntity *pFlare = ::CreateFlare( GetAbsOrigin(), GetAbsAngles(), this, flLifetime, 32.0f ); // 32 is the goal heat level
	if ( pFlare )
	{
		...
		SetNextThink( CURTIME + flLifetime + 5.0f );
...
{
	if (CURTIME - dt <= m_flLastPhysicsInfluenceTime)
	{
...
// Purpose: 
//-----------------------------------------------------------------------------
const char *pszCrateLootItems[10] = {
...
void CBreakableProp::Precache()
{
	...
	for (int i = 0; i < 10; ++i)
	{
		PrecacheModel(pszCrateLootItems[i]);
	}

	// DI NEW
	if (GetBreakableDamageSound() != NULL_STRING)
	{
		PrecacheScriptSound(GetBreakableDamageSound().ToCStr());
	}

	PrecacheScriptSound("Cardboard.Strain");
	PrecacheScriptSound("Flesh.Strain");
	PrecacheScriptSound("Glass.Strain");
	PrecacheScriptSound("Metal_Box.Strain");
	PrecacheScriptSound("Plastic_Barrel.Strain");
	PrecacheScriptSound("Plastic_Box.Strain");
	PrecacheScriptSound("Rubber_Tire.Strain");
	PrecacheScriptSound("SolidMetal.Strain");
	PrecacheScriptSound("Wood.Strain");
	PrecacheScriptSound("Wood_Box.Strain");
	PrecacheScriptSound("Wood_Crate.Strain");
	PrecacheScriptSound("Wood_Furniture.Strain");
	PrecacheScriptSound("Wood_Panel.Strain");
	PrecacheScriptSound("Wood_Plank.Strain");
	PrecacheScriptSound("Wood_Solid.Strain");
#endif
	BaseClass::Precache();
}
...
void CBreakableProp::Break( CBaseEntity *pBreaker, const CTakeDamageInfo &info )
{
	...
	if (pModelName)
	{
		if (Q_strcmp(pModelName, "models/props_junk/wood_crate001a.mdl") == 0)
		{
			if ( m_WeightMultiplier_int > 0)
			{
				if (RandomFloat(0, 1) > 0.1f)
				{
					for (int i = 0; i < m_WeightMultiplier_int; i++)
					{
						if (RandomFloat(0, 1) > 0.15f)
						{
							// ~6/7 chance it's NOT a flare
							CGib::SpawnSpecificGibs(this, 1, 50, 150, pszCrateLootItems[RandomInt(0, 6)]);
						}
						else 
						{
							// otherwise it IS a flare 
							// (number 7 in the models array) 
							// and we want to spawn it differently
							CPhysicsProp *flare_prop = assert_cast<CPhysicsProp*>(CreateEntityByName("prop_physics"));
							flare_prop->SetModel("models/props_junk/flare.mdl");
							flare_prop->SetAbsOrigin(GetAbsOrigin());
							flare_prop->SetAbsAngles(GetLocalAngles());
							flare_prop->SetMoveType(MOVETYPE_VPHYSICS);
							flare_prop->SetCollisionGroup(COLLISION_GROUP_INTERACTIVE);
							flare_prop->Spawn();
						}

					//	Msg("Spawned random loot for weight\n");
					}
				}
				else // make saw-drops and stunstick-drops pretty rare
				{
					int e;
					if (RandomInt(0, 1) == 0) // and then flip a coin on which one it is
					{
						e = 8;
					}
					else
					...
					for (int q = 0; q < 6; q++)
					{
						if (e == 9)
						{
							CPhysicsProp *prop = assert_cast<CPhysicsProp*>(CreateEntityByName("prop_physics"));
							prop->SetModel(pszCrateLootItems[e]);
							prop->SetAbsOrigin(GetAbsOrigin());
							prop->SetAbsAngles(GetLocalAngles());
							prop->SetMoveType(MOVETYPE_VPHYSICS);
							prop->Spawn();
						}
						else
						{
							CWeaponStunStick *stunstick = assert_cast<CWeaponStunStick*>(CreateEntityByName("weapon_stunstick"));
							stunstick->SetAbsOrigin(GetAbsOrigin());
							stunstick->SetAbsAngles(GetLocalAngles());
							stunstick->SetMoveType(MOVETYPE_VPHYSICS);
							stunstick->Spawn();
						}
					}
				}
		...
		else if (Q_strcmp(pModelName, "models/props_junk/wood_crate002a.mdl") == 0)
		{
			if (m_WeightMultiplier_int > 0)
			{
			//	if (RandomFloat(0, 1) > 0.6f)
				{
					for (int i = 0; i < m_WeightMultiplier_int; i++)
					{
						if (RandomFloat(0, 1) > 0.15f)
						...
						else
						{
							...
							if (RandomFloat(0, 1) > 0.6f) // 30% chance it'll create an extra flare
							{
								CPhysicsProp *flare_prop2 = assert_cast<CPhysicsProp*>(CreateEntityByName("prop_physics"));
								flare_prop2->SetModel("models/props_junk/flare.mdl");
								flare_prop2->SetAbsOrigin(GetAbsOrigin() + RandomVector(-4, 4));
								flare_prop2->SetAbsAngles(GetLocalAngles());
								flare_prop2->SetMoveType(MOVETYPE_VPHYSICS);
								flare_prop2->SetCollisionGroup(COLLISION_GROUP_INTERACTIVE);
								flare_prop2->Spawn();
							}
						}
					}
...
#ifdef DARKINTERVAL // DI adds more explosion types to props (wip)
		/*
		else if (HasInteraction( PROPINTER_PHYSGUN_BREAK_EXPLODE_PLASMA ))
		{
			////
		}
		else if (HasInteraction( PROPINTER_PHYSGUN_BREAK_EXPLODE_CRYO ))
		{
...
#ifdef DARKINTERVAL // DI adds more explosion types to props (wip)
	/*
	if (HasInteraction(PROPINTER_PHYSGUN_BREAK_EXPLODE_CRYO))
	{
		CPASAttenuationFilter filter2(this, "IceBody.Shatter");
		EmitSound(filter2, entindex(), "IceBody.Shatter");
	...
	//	if (bExploded == false)
	//	{
	//		ExplosionCreate(origin, angles, pAttacker, 100, m_explodeRadius,
	//			SF_ENVEXPLOSION_NOSPARKS | SF_ENVEXPLOSION_NODLIGHTS | SF_ENVEXPLOSION_NOSMOKE | SF_ENVEXPLOSION_NODECAL | SF_ENVEXPLOSION_NOFIREBALL, 0.0f, this);
	//	}

		// Find and freeze all NPC's within the radius
		CBaseEntity *pEntity = NULL;
		for (CEntitySphereQuery sphere(origin, m_explodeRadius * GetModelScale()); (pEntity = sphere.GetCurrentEntity()) != NULL; sphere.NextEntity())
		{
			if (pEntity && pEntity->MyCombatCharacterPointer())
			{
				// Check damage filters so we don't ignite friendlies
				...
				//	pEntity->TakeDamage(info_energy);
					pEntity->MyCombatCharacterPointer()->Freeze(0.75f);
				}
			}
		}
	}
...
#ifdef DARKINTERVAL
	DEFINE_KEYFIELD( m_bIgnorePlayerUse, FIELD_BOOLEAN, "IgnorePlayerUse"),
	DEFINE_KEYFIELD( m_bRelayPhysicsForcesToParent, FIELD_BOOLEAN, "RelayPhysicsForcesToParent"),
#endif
...
#ifdef DARKINTERVAL
	DEFINE_INPUTFUNC( FIELD_VECTOR,		"EnableGlow",		InputAddGlowEffect ),
	DEFINE_INPUTFUNC(FIELD_VOID,		"DisableGlow",		InputRemoveGlowEffect),
#endif
	// Outputs
...
CDynamicProp::CDynamicProp()
{
	...
	m_iGoalSequence = -1;
#ifdef DARKINTERVAL
	m_bRelayPhysicsForcesToParent = true;
#endif
}
#ifdef DARKINTERVAL
int CDynamicProp::UpdateTransmitState(void)
{
	return SetTransmitState(FL_EDICT_ALWAYS);
}

int CDynamicProp::ObjectCaps()
{
	int caps = BaseClass::ObjectCaps();

	if (!m_bIgnorePlayerUse)
	{
...
void CDynamicProp::Use(CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value) // DI add: use prop_dynamics as buttons
{
...
void CDynamicProp::Spawn( )
{
	...
	if ( m_bRandomAnimator || ( m_iszDefaultAnim != NULL_STRING ) )
	{
		...
		if ( m_bRandomAnimator )
		{
			...
			m_flNextRandAnim = CURTIME + random->RandomFloat( m_flMinRandAnimTime, m_flMaxRandAnimTime );
			SetNextThink( CURTIME + m_flNextRandAnim + 0.1 );
		}
		else
		{
...
#ifdef DARKINTERVAL
int CDynamicProp::OnTakeDamage(const CTakeDamageInfo &inputInfo)
{
	if (m_bRelayPhysicsForcesToParent && GetMoveParent() && GetMoveParent()->VPhysicsGetObject())
	{
		GetMoveParent()->VPhysicsGetObject()->ApplyForceOffset(inputInfo.GetDamageForce(), WorldSpaceCenter());
	}
...
void CDynamicProp::AnimThink( void )
{
	...
	if ( m_bRandomAnimator && m_flNextRandAnim < CURTIME )
	{
...
#ifdef DARKINTERVAL
void CDynamicProp::InputAddGlowEffect(inputdata_t &inputdata)
{
	Vector vecColor;
	inputdata.value.Vector3D(vecColor);
	AddGlowEffect(vecColor.x, vecColor.y, vecColor.z);
}

void CDynamicProp::InputRemoveGlowEffect(inputdata_t &inputdata)
{
	RemoveGlowEffect();
}
#endif
...
void CDynamicProp::PropSetSequence( int nSequence )
{
	...
	// Msg("%.2f CDynamicProp::PropSetSequence( %d (%d:%.1f:%.3f)\n", CURTIME, nSequence, GetSequence(), GetPlaybackRate(), GetCycle() );
	...
	SetThink( &CDynamicProp::AnimThink );
	if ( GetNextThink() <= CURTIME )
		SetNextThink( CURTIME + flInterval );
}
...
#ifdef DARKINTERVAL // unused experiment, related to shaders
//	DEFINE_INPUTFUNC( FIELD_INTEGER, "IncreaseStainCount", InputIncreaseStainCount ),
//	DEFINE_INPUTFUNC( FIELD_INTEGER, "DecreaseStainCount", InputDecreaseStainCount),
#endif
	DEFINE_FIELD( m_bAwake, FIELD_BOOLEAN ), 
#ifdef DARKINTERVAL // unused experiment, related to shaders
//	DEFINE_FIELD(m_stainCount_int, FIELD_INTEGER),
#endif
...
#ifdef DARKINTERVAL
	DEFINE_OUTPUT(m_OnPhysGunLaunched, "OnPhysGunLaunched"),
#endif
...
#ifdef DARKINTERVAL // unused experiment, related to shaders
//	SendPropInt( SENDINFO( m_stainCount_int ) ),
#endif
END_SEND_TABLE()
...
#ifdef DARKINTERVAL	// Ice, ported from ASW
	m_flFrozenThawRate = 0.1f; // todo - is the rest of icing even implemented? 
#endif
	// Now condense all classnames to one
...
#ifdef DARKINTERVAL
	if (HasSpawnFlags(SF_PHYSPROP_NO_PLAYER_COLLISION))
	{
		SetCollisionGroup(COLLISION_GROUP_PLAYERDEBRIS);
	}
#endif
...
#ifndef DARKINTERVAL // DI doesn't care
	//Episode 1 change:
...
#ifdef DARKINTERVAL	// DI NEW - so that those crate gibs stop getting in the way
...
#ifdef DARKINTERVAL
		m_OnPhysGunLaunched.FireOutput(pPhysGunUser, this);
#endif
...
void CPhysicsProp::VPhysicsCollision( int index, gamevcollisionevent_t *pEvent )
{
	...
	if ( m_bFirstCollisionAfterLaunch )
	{
		...
		SetContextThink( &CPhysicsProp::ClearFlagsThink, CURTIME, "PROP_CLEARFLAGS" );
	}
}
...
#ifdef DARKINTERVAL
	if (numToCreate <= 0) return false;

	if (engine->GetEntityCount() + numToCreate + BREATHING_ROOM > MAX_EDICTS) return false;
...
void CBasePropDoor::Spawn()
{
	...
	AddSolidFlags( FSOLID_CUSTOMRAYTEST | FSOLID_CUSTOMBOXTEST );
#ifdef DARKINTERVAL
	if (m_nHardwareType == 4 ) // it's knob option, but on door03_left it's a keypad.
		SetBodygroup(DOOR_HARDWARE_GROUP, 3);
	else
#endif
...
void CBasePropDoor::CalcDoorSounds()
{
	...
	{
		DevMsg(2, "%s has Door model (%s) with no door_options! Verify that SKIN is valid, and has a corresponding options block in the model QC file\n", GetDebugName(), modelinfo->GetModelName( GetModel() ) );
...
{
#ifdef DARKINTERVAL
	if (HasSpawnFlags(SF_DOOR_IGNORE_USE))
	{
		return; // HOW THE HELL IS THIS NOT HERE ALREADY??? MAKES SPAWNFLAG USELESS!
	}
#endif
...
//===============================================
//catnipty over-complicated door prop.
//===============================================
...
#ifdef DARKINTERVAL // so that doors don't get twinned on level transitions, allowing to safely place them on both levels
	int		ObjectCaps(void)
	{
...
#ifdef DARKINTERVAL
	AddEffects(EF_NOSHADOW); // DI add: Shadows from doors always look bad.
#endif
}
...
void CPropDoorRotating::GetNPCOpenData(CAI_BaseNPC *pNPC, opendata_t &opendata)
{
	...
	if (pNPC->GetAbsOrigin().Dot(vecForward) > GetAbsOrigin().Dot(vecForward))
	{
		...
		opendata.vecStandPos += vecForward * 32;
...
void CPhysicsPropRespawnable::Event_Killed( const CTakeDamageInfo &info )
{
	...
	SetNextThink( CURTIME + m_flRespawnTime );
}
...
bool UTIL_CreateScaledPhysObject( CBaseAnimating *pInstance, float flScale )
{
	...
	if ( flScale != 1.0f )
	{
		...
		for ( int i = 0; i < nNumConvex; i++ )
		{
			...
			AssertMsg( pConvexes[i] != NULL, "Failed to convert scaled object's physics from verts to convex.\nModel ''%s'', Position %.2f %.2f %.2f", pInstance->GetModelName(), pInstance->GetAbsOrigin().x, pInstance->GetAbsOrigin().y, pInstance->GetAbsOrigin().z);
			if ( pConvexes[i] == NULL )
				return false;
		}
