...
#ifdef DARKINTERVAL
	DEFINE_OUTPUT(m_OnScriptEvent[8], "OnScriptEvent09"),
	DEFINE_OUTPUT(m_OnScriptEvent[9], "OnScriptEvent10"),
	DEFINE_OUTPUT(m_OnScriptEvent[10], "OnScriptEvent11"),
	DEFINE_OUTPUT(m_OnScriptEvent[11], "OnScriptEvent12"),
	DEFINE_OUTPUT(m_OnScriptEvent[12], "OnScriptEvent13"),
	DEFINE_OUTPUT(m_OnScriptEvent[13], "OnScriptEvent14"),
	DEFINE_OUTPUT(m_OnScriptEvent[14], "OnScriptEvent15"),
	DEFINE_OUTPUT(m_OnScriptEvent[15], "OnScriptEvent16"),
	DEFINE_OUTPUT(m_OnStartEntryAnimation, "OnStartEntryAnimation" ),
	DEFINE_OUTPUT(m_OnEndEntryAnimation, "OnEndEntryAnimation"),
	DEFINE_OUTPUT(m_OnEndActionAnimation, "OnEndActionAnimation"),
#endif
END_DATADESC()
...
// Input  : nEvent - One based index of the script event from the , from 1 to 16.
//-----------------------------------------------------------------------------
...
void CAI_ScriptedSequence::StartScript( void )
{
	...
	if ( pTarget )
	{
		...
		// Notify the NPC that we're stomping them into a scene!
		...
		{
			DevWarning( "scripted_sequence %d:%s - restarting dormant entity %d:%s : %.1f:%.1f\n", entindex(), GetDebugName(), pTarget->entindex(), pTarget->GetDebugName(), CURTIME, pTarget->GetNextThink() );
			pTarget->SetNextThink( CURTIME );
		}
	}
}
...
void CAI_ScriptedSequence::SynchronizeSequence( CAI_BaseNPC *pNPC )
{
	...
	float flInterval = CURTIME - m_startTime;

	// Msg("%.2f \"%s\"  %s : %f (%f): interval %f\n", CURTIME, GetEntityName().ToCStr(), pNPC->GetClassname(), pNPC->m_flAnimTime.Get(), m_startTime, flInterval );
...
void CAI_ScriptedSequence::SynchNewSequence( CAI_BaseNPC::SCRIPTSTATE newState, string_t iszSequence, bool bSynchOtherScenes )
{
	...
	{
		//Msg("%s (for %s) forcing synch of %s at %0.2f\n", GetTarget()->GetDebugName(), GetDebugName(), iszSequence, CURTIME);
	...
	if ( !pNPC )
 	...
 	//Msg("%s (for %s) starting %s seq at %0.2f\n", pNPC->GetDebugName(), GetDebugName(), iszSequence, CURTIME);
...
{
	//Clear this or the NPC's FL_FLY flag will never be removed because of ClearSchedule
...
#ifdef DARKINTERVAL
	COutputEvent m_OnScheduleComplete;
	COutputEvent m_OnScheduleFailed;
	COutputEvent m_OnScheduleFailedNoTarget;
	COutputEvent m_OnScheduleFailedMovement;
	COutputEvent m_OnScheduleFailedNoNPC;
#endif
	//---------------------------------
...
#ifdef DARKINTERVAL
	DEFINE_OUTPUT( m_OnScheduleComplete, "OnScheduleComplete"), // DI todo - there's no way to measure it from here (it's left up to the NPC), so it is never fired
	DEFINE_OUTPUT(m_OnScheduleFailed, "OnScheduleFailed"),
	DEFINE_OUTPUT(m_OnScheduleFailedNoTarget, "OnScheduleFailedNoTarget"),
	DEFINE_OUTPUT(m_OnScheduleFailedMovement, "OnScheduleFailedMovement"),
	DEFINE_OUTPUT(m_OnScheduleFailedNoNPC, "OnScheduleFailedNoNPC"),
#endif
END_DATADESC()


LINK_ENTITY_TO_CLASS( aiscripted_schedule, CAI_ScriptedSchedule );
#ifdef DARKINTERVAL // bring consistency to entity naming (scripted_sequence, scripted_target...)
LINK_ENTITY_TO_CLASS(scripted_schedule, CAI_ScriptedSchedule); // DI NEW: YES
#endif
//-----------------------------------------------------------------------------
// Purpose: 
//-----------------------------------------------------------------------------
...
#ifdef DARKINTERVAL
		m_OnScheduleFailed.FireOutput(this, this);
		m_OnScheduleFailedNoNPC.FireOutput(this, this);
#endif
		SetNextThink( CURTIME + 1.0f );
	}
	else
...
#ifdef DARKINTERVAL
			m_OnScheduleFailedNoTarget.FireOutput(this, this);
			m_OnScheduleFailed.FireOutput(this, this);
#endif
			return;
		}
...
#ifdef DARKINTERVAL
				m_OnScheduleFailedMovement.FireOutput(this, this);
				m_OnScheduleFailed.FireOutput(this, this);
#endif
				return;
			}
...
void CAI_ScriptedSentence::FindThink( void )
{
	...
	if ( pNPC )
	{
		...
		SetNextThink( CURTIME + delay + m_flRepeat );
		m_active = false;
	...
	else
	{
		...
		SetNextThink( CURTIME + m_flRepeat + 0.5 );
	}
}
