...
AIMotorMoveResult_t CAI_BlendedMotor::MoveGroundExecute( const AILocalMoveGoal_t &move, AIMoveTrace_t *pTraceResult )
{
	...
	{
		// DevMsg( "%6.2f : Speed %.1f : %.1f (%.1f) :  %d\n", CURTIME, flNewSpeed, move.maxDist, move.transitionDist, GetOuter()->m_pHintNode != NULL );
		// DevMsg( "%6.2f : Speed %.1f : %.1f\n", CURTIME, flNewSpeed, GetIdealSpeed() );
	}
	*/
	...
	{
		DevMsg( "%6.2f : Speed %.1f : %.1f : %.2f\n", CURTIME, flNewSpeed, GetIdealSpeed(), flNewSpeed / GetIdealSpeed() );
	}
	*/
...
void CAI_BlendedMotor::MaintainTurnActivity( void )
{
	...
	if (m_flNextTurnGesture > CURTIME || m_flNextTurnAct > CURTIME || GetOuter()->IsMoving() )
	{
		// clear out turn detection if currently turing or moving
		m_doTurn = m_doRight = m_doLeft = 0;
		if ( GetOuter()->IsMoving())
		{
			m_flNextTurnAct = CURTIME + 0.3;
		}
	}
	else 
	{
	...
	if (m_doTurn > 15.0f)
	{
		...
		if (iSeq != ACT_INVALID)
		{
			...
			if (iLayer != -1)
			{
				...
				m_flNextTurnAct = CURTIME + GetOuter()->GetLayerDuration( iLayer );
			}
			else
			{
...
{
#ifndef DARKINTERVAL // DI wants turn gestures to work // it doesn't fix them, however
	...
	if (m_flNextTurnGesture > CURTIME)
	{
		/*
		if ( GetOuter()->m_debugOverlays & OVERLAY_NPC_SELECTED_BIT )
		{
			Msg( "%.1f : [ %.2f ]\n", flYD, m_flNextTurnAct - CURTIME );
		}
		*/
		return false;
	}
	...
	if (seq != ACTIVITY_NOT_AVAILABLE)
	{
		...
		if (iLayer != -1)
		{
			...
			m_flNextTurnGesture = CURTIME + MAX( turnCompletion * actualDuration, 0.3 );
