...
{
	m_bEnabled = true;
	m_flTimeStartFadeIn = CURTIME;
	m_flStartFadeInWeight = m_flCurWeight;
	SetNextThink ( CURTIME + COLOR_CORRECTION_ENT_THINK_RATE, s_pFadeInContextThink );
}
...
{
	m_bEnabled = false;
	m_flTimeStartFadeOut = CURTIME;
	m_flStartFadeOutWeight = m_flCurWeight;
	SetNextThink ( CURTIME + COLOR_CORRECTION_ENT_THINK_RATE, s_pFadeOutContextThink );
}
...
void CColorCorrection::FadeInThink( void )
{
  ...
  m_flStartFadeInWeight >= m_flMaxWeight )  // already at max weight
	...
	float flFadeRatio = (CURTIME - m_flTimeStartFadeIn) / flTimeToFade;
...
void CColorCorrection::FadeOutThink( void )
{
	...
	Assert ( flTimeToFade > 0.0f );
	float flFadeRatio = (CURTIME - m_flTimeStartFadeOut) / flTimeToFade;
	flFadeRatio = clamp ( flFadeRatio, 0.0f, 1.0f );
