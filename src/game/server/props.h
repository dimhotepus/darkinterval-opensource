...
#ifdef DARKINTERVAL
	void InputSetWeightMultiplier(inputdata_t &inputdata); // for crate loot
#endif
...
#ifdef DARKINTERVAL
	int				m_WeightMultiplier_int; // for crate loot
	float			m_nextstrainsound_time;
#endif
public:
// IBreakableWithPropData
	...
	void			SetBreakableDamageSound(string_t iszSound) { m_iszBreakableDamageSound = iszSound; }
	string_t 		GetBreakableDamageSound(void) { return m_iszBreakableDamageSound; }
	void			SetStrainSound(string_t iszSound) { m_iszStrainSound = iszSound; }
	string_t 		GetStrainSound(void) { return m_iszStrainSound; }
#endif
...
#ifdef DARKINTERVAL	// DI NEW
	string_t		m_iszBreakableDamageSound;
	string_t		m_iszStrainSound;
#endif
...
#ifdef DARKINTERVAL
	virtual void InputAddGlowEffect(inputdata_t &inputdata);
	virtual void InputRemoveGlowEffect(inputdata_t &inputdata);
#endif
	COutputEvent		m_pOutputAnimBegun;
	COutputEvent		m_pOutputAnimOver; 
#ifdef DARKINTERVAL
	COutputEvent		m_OnPlayerUse; // very handy for buttons, levers and things like that without needing a brush entity
#endif
...
#ifdef DARKINTERVAL
	bool				m_bIgnorePlayerUse; // sometimes +use becomes a nuisance - have an option to disable it
	bool				m_bRelayPhysicsForcesToParent; // if being hit by bullets and such, relay that info to parent so they can be knocked around if they're prop physics
	virtual int			OnTakeDamage(const CTakeDamageInfo &info);
#endif
...
#ifdef DARKINTERVAL // unused experiment, related to shaders
/*
	void InputIncreaseStainCount(inputdata_t &inputdata)
	{
		m_stainCount_int += inputdata.value.Int();
		if (m_stainCount_int > 100) m_stainCount_int = 100;
	}

	void InputDecreaseStainCount(inputdata_t &inputdata)
	{
		m_stainCount_int -= inputdata.value.Int();
		if (m_stainCount_int < 0) m_stainCount_int = 0;
	}
*/
#endif
...
#ifdef DARKINTERVAL
	COutputEvent m_OnPhysGunLaunched; // prop physics paradoxically lacks this output - add it for scenes like on CH09 Junkyard
#endif
	float		m_massScale;
...
#ifdef DARKINTERVAL // unused experiment, related to shaders
//	CNetworkVar(int, m_stainCount_int);
#endif
};
