...
void CFire::Update(float simTime)
{
	...
	bool damage = false;
	int outputDamage = 0;
	if (m_flDamageTime <= CURTIME)
	{
		m_flDamageTime = CURTIME + 1.0f;
...
{
	SetNextThink(CURTIME + FIRE_THINK_INTERVAL);
...
void CEnvFireSource::Think()
{
	if (!m_bEnabled)
		return;
	SetNextThink(CURTIME + FIRESOURCE_THINK_TIME);
...
void CEnvFireSensor::Think()
{
	...
	if (HasSpawnFlags(SF_FIRESENSOR_SENSE_FLARES))
	{
		CBaseEntity *pTouch = NULL;
		while ((pTouch = gEntList.FindEntityInSphere(pTouch, GetAbsOrigin(), m_radius)) != NULL)
		{
			CFlare *pFlare = dynamic_cast<CFlare*>(pTouch);
			if (pFlare)
			{
				heat += pFlare->GetHeatLevel();
			}
		}
	}
	//
...
public:
	...
	void DrawDebugGeometryOverlays(void) 
	{
		...
		{	
			if ( m_lastDamage > CURTIME && m_flHeatAbsorb > 0 )
			{
...
void CFire::Update( float simTime )
{
	...
	if ( m_flDamageTime <= CURTIME )
	{
		m_flDamageTime = CURTIME + fire_dmginterval.GetFloat();
