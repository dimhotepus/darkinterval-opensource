	...
	if ( pPlayer != NULL )
	{
		...
		if ( GetDrivableVehicle()->CanEnterVehicle( pPlayer ) )
		{
			...
			if ( pPlayer->CanEnterVehicle( this, VEHICLE_ROLE_DRIVER ) )
			{
				...
				pAnimating->m_flAnimTime = CURTIME;
...
// NOTE: HL2 and Ep1 used a method that relied on the animators to place 
// attachment points in the model which marked where
// the player would exit to.  This was rendered unnecessary in Ep2, 
// but the choreo vehicles of these older products
// did not have proper exit animations and relied on 
// the exact queries that were happening before.  For choreo vehicles,
...
#ifdef DARKINTERVAL // for npc drivers to be able to use APC thrusters to upright themselves
//-----------------------------------------------------------------------------
// Purpose: 
//-----------------------------------------------------------------------------
void CBaseServerVehicle::NPC_Thruster(void)
{
	m_nNPCButtons &= ~IN_FORWARD;
	m_nNPCButtons &= ~IN_BACK;
	m_nNPCButtons |= IN_JUMP;
	m_nNPCButtons |= IN_DUCK;
}
#endif
//-----------------------------------------------------------------------------
// Purpose: 
...
{
	float timeInState = CURTIME - m_soundStateStartTime;
