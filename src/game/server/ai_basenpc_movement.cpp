...
void CAI_BaseNPC::ForceSelectedGo(CBaseEntity *pPlayer, const Vector &targetPos, const Vector &traceDir, bool bRun) 
{
	...
	while (npc)
	{
		if (npc->m_debugOverlays & OVERLAY_NPC_SELECTED_BIT ) 
		{
			...
			npc->m_flMoveWaitFinished = CURTIME;
		}
							   ...
							   float maxUp, float maxDown, float maxDist ) const
{
	...
	{
	//	DevMsg("jump isn't legal because (endPos.z - startPos.z) > maxUp + 0.1! (maxUp is %.1f, dist is %.1f)\n", maxUp, (endPos.z - startPos.z));
		return false;
	}
	if ((startPos.z - endPos.z) > maxDown + 0.1)
	{
	//	DevMsg("jump isn't legal because (startPos.z - endPos.z) > maxDown + 0.1 (maxDown is %.1f, dist is %.1f))\n", maxDown, (startPos.z - endPos.z));
		return false;
	}
	if ((apex.z - startPos.z) > maxUp * 1.25)
	{
	//	DevMsg("jump isn't legal because (apex.z - startPos.z) > maxUp * 1.25 (maxUp is %.1f, dist is %.1f))\n", maxUp, (apex.z - startPos.z));
		return false;
	}
	float dist = (startPos - endPos).Length();
	if (dist > maxDist + 0.1)
	{
	//	DevMsg("jump isn't legal because dist > maxDist + 0.1 (maxDist is %.1f, dist is %.1f))\n", maxDist, dist);
		return false;
	}
	return true;
}
...
{
	return (m_flMoveWaitFinished > CURTIME);
}
...
bool CAI_BaseNPC::AutoMovement( float flInterval, CBaseEntity *pTarget, AIMoveTrace_t *pTraceResult )
{
	...
	{
		// DevMsg( "%.2f : (%.1f) %.1f %.1f %.1f\n", CURTIME, (newPos - GetLocalOrigin()).Length(), newPos.x, newPos.y, newAngles.y );
