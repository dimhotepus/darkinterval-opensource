...
public:
	...
	void Acquire( float time, CBaseEntity *pTalker )		{ m_ReleaseTime = CURTIME + time; m_hCurrentTalker = pTalker; }
	void Release()					{ m_ReleaseTime = 0; m_hCurrentTalker = NULL; }
	
	// Current owner of the semaphore is always allowed to talk
	bool IsAvailable( CBaseEntity *pTalker ) const		{ return ((CURTIME > m_ReleaseTime) || (m_hCurrentTalker == pTalker)); }
