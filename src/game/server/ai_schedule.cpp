...
CAI_Schedule *CAI_SchedulesManager::GetScheduleFromID( int schedID )
{
	...
	Assert(0);
	Msg( "Couldn't find schedule (%s)\nMake sure you added AI_LOAD_SCHEDULE(class, schedule) in your custom AI table!\nCheck the syntax of every custom schedule, make sure\nthey don't have any misnamed or missing activities!\n", CAI_BaseNPC::GetSchedulingSymbols()->ScheduleIdToSymbol(schedID) );
