...
#ifdef DARKINTERVAL // allows updating game text with map-driven values for things like on-screen counters
	void InputSetMessageInt(inputdata_t &inputdata);
	void InputSetMessageFloat(inputdata_t &inputdata);
	void InputSetMessageString(inputdata_t &inputdata);
#endif
	void Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
	{
		Display( pActivator );
...
#ifdef DARKINTERVAL
	string_t m_iszPrefix; // allows having a persistent prefix for things like counters and dev text
#endif
...
#ifdef DARKINTERVAL
	DEFINE_KEYFIELD( m_iszPrefix, FIELD_STRING, "prefix"),
#endif
...
#ifdef DARKINTERVAL
	DEFINE_INPUTFUNC(FIELD_INTEGER, "SetMessageInt", InputSetMessageInt),
	DEFINE_INPUTFUNC(FIELD_FLOAT, "SetMessageFloat", InputSetMessageFloat),
	DEFINE_INPUTFUNC(FIELD_STRING, "SetMessageString", InputSetMessageString),
#endif
END_DATADESC()
...
#ifdef DARKINTERVAL
void CGameText::InputSetMessageInt(inputdata_t &inputdata) 
{
	int inputInt = inputdata.value.Int();
	char msg[255];

	if ( m_iszPrefix != NULL_STRING)
	{
		Q_snprintf(msg, 255, "%s %i", m_iszPrefix, inputInt);
	}
	else
	{
		Q_snprintf(msg, 255, "%i", inputInt);
	}

	MessageSet(msg);
}
void CGameText::InputSetMessageFloat(inputdata_t &inputdata)
{
	float inputFloat = inputdata.value.Float();
	char msg[255];

	if (m_iszPrefix != NULL_STRING)
	{
		Q_snprintf(msg, 255, "%s %.2f", m_iszPrefix, inputFloat);
	}
	else
	{
		Q_snprintf(msg, 255, "%g", inputFloat);
	}

	MessageSet(msg);
}
void CGameText::InputSetMessageString(inputdata_t &inputdata)
{
	const char *inputStr = inputdata.value.String();

	if (MAKE_STRING(inputStr) != NULL_STRING)
	{
		char msg[255];

		if (m_iszPrefix != NULL_STRING)
		{
			Q_snprintf(msg, 255, "%s %s", m_iszPrefix, inputStr);
			MessageSet(msg);
		}
		else
		{
