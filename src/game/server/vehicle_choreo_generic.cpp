...
#ifdef DARKINTERVAL
	bool m_allowplayerweapons_bool;  // DI NEW - to allow specific choreo vehicles with guns and/or not interfere with gun state-altering scripts
#endif
// IDrivableVehicle
public:
...
#ifdef DARKINTERVAL
	virtual bool IsPassengerUsingStandardWeapons(int nRole = VEHICLE_ROLE_DRIVER); // DI NEW - to allow specific choreo vehicles with guns and/or not interfere with gun state-altering scripts
#endif
...
#ifdef DARKINTERVAL
	DEFINE_KEYFIELD( m_allowplayerweapons_bool, FIELD_BOOLEAN, "allowplayerweapons"),
#endif
...
END_SEND_TABLE();
#ifdef DARKINTERVAL
bool CPropVehicleChoreoGeneric::IsPassengerUsingStandardWeapons(int nRole)
{
	return m_allowplayerweapons_bool;
}
#endif
...
#ifdef DARKINTERVAL
	pPlayer->SetLocalAngles(GetLocalAngles());
#endif
}
