...
public:
	...
	virtual bool NodeIsValid( CAI_Node &node )
	{
		...
		if ( m_pNPC->GetNavType() == NAV_FLY && node.GetType() != NODE_AIR )
			return false;
#ifdef DARKINTERVAL
		if (m_pNPC->GetNavType() == NAV_SWIM && node.GetType() != NODE_WATER) // DI NEW - water nodes
			return false;
#endif
...
#ifdef DARKINTERVAL
			||	(node.GetType() == NODE_WATER && !(m_capabilities & bits_CAP_MOVE_SWIM))  // DI NEW - water nodes
#endif
			)
			return false;
...
int	CAI_Network::NearestNodeToPoint( CAI_BaseNPC *pNPC, const Vector &vecOrigin, bool bCheckVisibility, INearestNodeFilter *pFilter )
{
	...
	if ( cachedNode != NO_NODE )
	{
		...
		{
			m_NearestCache[cachePos].expiration	= CURTIME + NEARNODE_CACHE_LIFE;
			return cachedNode;
		}
	}
...
int	CAI_Network::GetCachedNode(const Vector &checkPos, Hull_t nHull, int *pCachePos )
{
	...
	for ( int i = 0; i < NEARNODE_CACHE_SIZE; i++ )
	{
		...
		if ( m_NearestCache[iCurrent].hull == nHull && m_NearestCache[iCurrent].expiration > CURTIME )
		{
...
void CAI_Network::SetCachedNearestNode(const Vector &checkPos, int nodeID, Hull_t nHull)
{
	...
	m_NearestCache[m_iNearestCacheNext].expiration		= CURTIME + NEARNODE_CACHE_LIFE;
...
#ifndef DARKINTERVAL // not that helpful
