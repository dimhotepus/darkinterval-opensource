...
#ifdef DARKINTERVAL
	bits_BUILD_SWIM			=			0x00000200, // DI NEW - water nodes
#endif
};
...
#ifdef DARKINTERVAL
	AI_Waypoint_t	*BuildSwimRoute(const Vector &vStart, const Vector &vEnd, CBaseEntity const *pTarget, int endFlags, int nodeID, int buildFlags, float flYaw, float goalTolerance); // DI NEW - water nodes
#endif
