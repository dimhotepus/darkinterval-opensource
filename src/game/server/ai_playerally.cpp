...
//-----------------------------------------------------------------------------
#ifdef DARKINTERVAL // DI makes them less overpowered to be able to kill allies like Barney
ConVar sk_ally_regen_time( "sk_ally_regen_time", "1.0", FCVAR_NONE, "Time taken for an ally to regenerate a point of health." );
#else
...
#endif
	{ 	TLK_DANGER,			SPEECH_IMPORTANT,	-1,		-1,		-1,		-1,		5,		7,		AICF_DEFAULT,	},
	...
	if ( HasCondition( COND_SEE_PLAYER ) )
	{
		...
		if ( bPlayerIsLooking )
		{
			...
			if ( m_flTimePlayerStartStare == 0 )
				m_flTimePlayerStartStare = CURTIME;
		}
		else
		{
...
void CAI_PlayerAlly::GatherEnemyConditions( CBaseEntity *pEnemy )
{
	...
	if ( GetLastEnemyTime() == 0 || CURTIME - GetLastEnemyTime() > 30 )
	{
#ifdef HL2_DLL
...
void CAI_PlayerAlly::OnStateChange( NPC_STATE OldState, NPC_STATE NewState )
{
	...
	{
		m_flNextIdleSpeechTime = CURTIME + RandomFloat(5,10);
	}
	else
	{
...
void CAI_PlayerAlly::PrescheduleThink( void )
{
	...
	{
		if ( m_flNextIdleSpeechTime && m_flNextIdleSpeechTime < CURTIME )
		{
			...
			if ( SelectNonCombatSpeech( &selection ) )
			{
				...
				m_flNextIdleSpeechTime = CURTIME + RandomFloat( 20,30 );
			}
			else
			{
				m_flNextIdleSpeechTime = CURTIME + RandomFloat( 10,20 );
			}
		}
	}
#endif // HL2_EPISODIC
...
bool CAI_PlayerAlly::SelectQuestionAndAnswerSpeech( AISpeechSelection_t *pSelection )
{
	if ( !IsOkToSpeak( SPEECH_IDLE ) )
		return false;
#ifndef DARKINTERVAL // DI CHANGE: what the catnip? Why not let them do it on the move?
	if ( IsMoving() )
		return false;
#endif
...
void CAI_PlayerAlly::PostSpeakDispatchResponse( AIConcept_t concept, AI_Response *response )
{
	...
	if ( pConceptInfo && (pConceptInfo->flags & AICF_QUESTION) && GetSpeechTarget() )
	{
		...
		float duration = GetExpresser()->GetSemaphoreAvailableTime(this) - CURTIME;
...
// Purpose: 
//-----------------------------------------------------------------------------
#ifdef DARKINTERVAL // DI wants Q&A work between classes, not just player allies
void CAI_PlayerAlly::AnswerQuestion( CBaseEntity *pQuestioner, int iQARandomNum, bool bAnsweringHello )
#else
...
#endif
{
 	...
 	if ( SelectAnswerFriend( pQuestioner, &selection, bAnsweringHello ) )
	{
		...
		SetSpeechTarget( selection.hSpeechTarget );
#ifdef DARKINTERVAL 
	//	if(m_iQARandomNumber == -1) // DI hack. NPC Talker sets -1 when addressing PlayerAlly.
...
int CAI_PlayerAlly::SelectNonCombatSpeechSchedule()
{
	...
	if ( HasPendingSpeech() )
	{
		if ( m_TimePendingSet == CURTIME || IsAllowedToSpeak( m_PendingConcept.c_str() ) )
	...
	if ( pKilled )
	{
		if ( !pKilled->IsNPC() || 
			( pKilled->MyNPCPointer()->GetLastPlayerDamageTime() == 0 ||
			  CURTIME - pKilled->MyNPCPointer()->GetLastPlayerDamageTime() > 5 ) )
		{
...
int CAI_PlayerAlly::OnTakeDamage_Alive( const CTakeDamageInfo &info )
{
...
#ifdef DARKINTERVAL
	int ret = BaseClass::OnTakeDamage_Alive(subInfo);
#endif
	...
	if (subInfo.GetAttacker() && (subInfo.GetAttacker()->IsPlayer()) && subInfo.GetDamage() < GetHealth())
	{
		CBaseEntity *pFriend = FindSpeechTarget(AIST_NPCS);
		if (pFriend )
		{
			...
			if (pTalkNPC)
			{
				if (pTalkNPC->IsOkToCombatSpeak())
				{
					pTalkNPC->SpeakIfAllowed(TLK_NOSHOOT);
				}
			}
		}
	}
...
{
#ifdef DARKINTERVAL // safety check
	if (!pEntity)
		return false;
#endif
	if ( pEntity == this )
...
#ifdef DARKINTERVAL
	if (ClassMatches("npc_houndeye")) return false; // don't remember why this is singled out

	if (flags & AIST_NPCS)
	{
		CAI_PlayerAlly *pAlly = (CAI_PlayerAlly *)(this); // safety check?..
		if (!pAlly) return false; 
	}
#endif
...
#ifndef DARKINTERVAL // not necessary to cast?
	...
	{
		// If not a NPC for some reason, or in a script.
		if ((pEntity->MyNPCPointer()->m_NPCState == NPC_STATE_SCRIPT || pEntity->MyNPCPointer()->m_NPCState == NPC_STATE_PRONE))
			return false;
		...
		// Don't bother people who don't want to be bothered
		if (!pEntity->MyNPCPointer()->CanBeUsedAsAFriend())
			return false;
	}
	else
		return false;
...
bool CAI_PlayerAlly::IsOkToSpeak( ConceptCategory_t category, bool fRespondingToPlayer )
{
...
#ifdef DARKINTERVAL // DI disables this, because people caught by barnacle emit a plea for help
	...
	if ( !UTIL_FindClientInPVS(edict()) )
		return false;
#ifdef DARKINTERVAL
	if ( category != SPEECH_PRIORITY && category != SPEECH_IMPORTANT )
#else
	if ( category != SPEECH_PRIORITY )
#endif
	{
...
{ 
	...
	{
	//	Msg("is allowed to speak, trying to speak...\n");
		return Speak( concept, modifiers, pszOutResponseChosen, bufsize );
	}
	else
	{
	//	Msg("not allowed to speak\n");
	}
	return false;
}
...
void CAI_PlayerAlly::OnSpokeConcept( AIConcept_t concept, AI_Response *response )
{
	...
	if( response != NULL && (response->GetParams()->flags & AI_ResponseParams::RG_WEAPONDELAY) )
	{
		...
		GetShotRegulator()->FireNoEarlierThan( CURTIME + response->GetWeaponDelay() );
	}
#ifdef DARKINTERVAL
	if (response != NULL && (response->GetParams()->flags & AI_ResponseParams::RG_SHOWRADUIOHUD))
	{
		// this response has a scene that's overheard over HEV radio. Signal the client to show the radio HUD
		// Note, this will play to 'all players'. If there's ever MP plans it needs to be changed, 
		// but right now it's not relevant.
		CReliableBroadcastRecipientFilter user;
		UserMessageBegin(user, "SuitRadio");
		WRITE_BYTE(1);	// one message
		WRITE_FLOAT(response->GetRadioHUDLength()); // length of icon's lifetime
		WRITE_STRING("radio_general"); // and this is hardcoded, but again, not important right now
		MessageEnd();
	}
#endif
}
