...
void CEntityParticleTrail::Spawn()
{
	...
	{
		SetContextThink( ZapThink, CURTIME + random->RandomFloat( 0.1f, 0.3f ), s_pZapContext ); 
	}
	*/
}
...
void CEntityParticleTrail::IncrementRefCount()
{
	if ( m_nRefCount == 0 )
	{
		SetContextThink( NULL, CURTIME, s_pRetireContext );
	}
	++m_nRefCount;
}
...
void CEntityParticleTrail::DecrementRefCount()
{
	...
	if ( m_nRefCount == 0 )
	{
		FollowEntity( NULL );
		g_pNotify->ClearEntity( this );
		SetContextThink( &CEntityParticleTrail::SUB_Remove, CURTIME + m_Info.m_flLifetime, s_pRetireContext );
	}
}
