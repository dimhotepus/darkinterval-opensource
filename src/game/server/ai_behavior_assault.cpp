...
void CAI_AssaultBehavior::SetAssaultPoint( CAssaultPoint *pAssaultPoint )
{
	...
	pAssaultPoint->m_flTimeLastUsed = CURTIME;
}
...
void CAI_AssaultBehavior::StartTask( const Task_t *pTask )
{
	...
	case TASK_ASSAULT_DEFER_SCHEDULE_SELECTION:
		m_flTimeDeferScheduleSelection = CURTIME + pTask->flTaskData;
		TaskComplete();
		break;
...
void CAI_AssaultBehavior::RunTask( const Task_t *pTask )
{
	...
	case TASK_WAIT_FOR_MOVEMENT:
		...
		if ( ( ( !GetOuter()->DidChooseEnemy() && CURTIME - GetOuter()->GetTimeEnemyAcquired() > 1 ) || !GetOuter()->GetEnemy() ) )
		{
...
bool CAI_AssaultBehavior::CanSelectSchedule()
{
	...
	if ( m_flTimeDeferScheduleSelection > CURTIME )
		return false;
...
#ifdef DARKINTERVAL // this is believed to have been a typo in the original code
...
bool CAI_AssaultBehavior::IsAllowedToDivert( void )
{
	if ( m_hAssaultPoint && m_hAssaultPoint->m_bAllowDiversion )
	{
		...
		{
			if ( m_flLastSawAnEnemyAt && ((CURTIME - m_flLastSawAnEnemyAt) < ASSAULT_DIVERSION_TIME) )
				return true;
		}
	}
