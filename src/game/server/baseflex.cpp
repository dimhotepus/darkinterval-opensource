...
#ifdef DARKINTERVAL // default to on, why not
ConVar	ai_expression_optimization( "ai_expression_optimization", "1", FCVAR_NONE, "Disable npc background expressions when you can't see them." );
#else
			...
			if (ai_expression_optimization.GetBool())
			{
				if (scene->IsBackground())
				{
					...
					if (gpGlobals->frametime > ai_expression_frametime.GetFloat())
					{
						...
						info->m_flNext = CURTIME + RandomFloat( 0.7, 1.2 );
					}
					// only check occasionally
					else if (info->m_flNext <= CURTIME)
					{
...
bool CBaseFlex::PermitResponse( float response_length )
{
	...
	if ( CURTIME + response_length <= m_flAllowResponsesEndTime )
	{
		return true;
	}
...
bool CBaseFlex::IsSuppressedFlexAnimation( CSceneEventInfo *info )
{
	...
	if (info->m_pScene && info->m_pScene->IsBackground())
	{
		...
		return m_flLastFlexAnimationTime > CURTIME - GetAnimTimeInterval() * 1.5;
	}
	// keep track of last non-suppressable flex animation
	m_flLastFlexAnimationTime = CURTIME;
	return false;
}
...
void CFlexCycler::Think( void )
{
	...
	if (GetNumFlexControllers() > 2)
	{
		...
		else if ( pszExpression && (pszExpression[0] == '^') && (pszExpression[1] == '\0') ) // ^ for sine wave
		{
			for ( LocalFlexController_t i = LocalFlexController_t(0); i < GetNumFlexControllers(); i++ )
			{
				...
				float fFlexTime = i * (1.0f / (float)GetNumFlexControllers()) + CURTIME;
		...
		else if (m_flextime < CURTIME)
		{
			// m_flextime = CURTIME + 1.0; // RandomFloat( 0.1, 0.5 );
			m_flextime = CURTIME + random->RandomFloat( 0.3, 0.5 ) * (30.0 / GetNumFlexControllers());
		...
		if (flex_talk.GetInt())
		{
			if (m_speaktime < CURTIME)
			{
				...
				if (m_istalking)
				{
					m_looktime = CURTIME - 1.0;
					m_speaktime = CURTIME + random->RandomFloat( 0.5, 2.0 );
				}
				else
				{
					m_speaktime = CURTIME + random->RandomFloat( 1.0, 3.0 );
				}
			}
			...
			if (m_istalking)
			{
				m_flextime = CURTIME + random->RandomFloat( 0.0, 0.2 );
		...
		// blink
		if (m_blinktime < CURTIME)
		{
			Blink();
			m_blinktime = CURTIME + random->RandomFloat( 1.5, 4.5 );
		}
	}
	...
	if (pPlayer)
	{
		if (pPlayer->GetSmoothedVelocity().Length() != 0 && DotProduct( forward, pPlayer->EyePosition() - EyePosition()) > 0.5)
		{
			...
			m_looktime = CURTIME + random->RandomFloat(2.0,4.0);
		}
		else if (m_looktime < CURTIME)
		{
			if ((!m_istalking) && random->RandomInt( 0, 1 ) == 0)
			{
				...
				m_looktime = CURTIME + random->RandomFloat(0.3,1.0);

				if (m_blinktime - 0.5 < CURTIME)
				{
					Blink();
				}
			}
			...
			{
				m_lookTarget = pPlayer->EyePosition();
				m_looktime = CURTIME + random->RandomFloat(1.0,4.0);
			}
		}
