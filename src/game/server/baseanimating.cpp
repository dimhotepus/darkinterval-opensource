...
#ifdef DARKINTERVAL // for freeze ray functionality (itself incomplete)
...
#ifdef DARKINTERVAL // Ice, freezing mechanic ported from ASW
...
ConVar ai_sequence_debug( "ai_debug_sequence", "0" );
#else
...
//	DEFINE_FIELD( m_BoneSetupMutex, CThreadFastMutex ),
	...
	DEFINE_FIELD( m_live_ragdoll_ent, FIELD_EHANDLE),
#endif
...
#endif
#ifdef DARKINTERVAL // glowing outlines
...
#ifdef DARKINTERVAL	// DI NEW: meshBuilder based text strings on models
	DEFINE_KEYFIELD(m_nameplate_string, FIELD_STRING, "nameplate"),
	DEFINE_INPUTFUNC(FIELD_STRING, "SetNameplateText", InputSetNameplateText),
	DEFINE_INPUTFUNC(FIELD_STRING, "SetModelName", InputSetModelName), // changing model on the fly
#endif
#ifdef DARKINTERVAL	// DI NEW: Live Ragdolls Experiment (inspired by Dark Messiah)
	DEFINE_INPUTFUNC(FIELD_STRING, "SetLiveRagdollEnt", InputSetLiveRagdollEnt),
	DEFINE_INPUTFUNC(FIELD_VOID, "ClearLiveRagdollEnt", InputClearLiveRagdollEnt),
#endif
...
#ifdef DARKINTERVAL	// DI NEW: Live Ragdolls Experiment (inspired by Dark Messiah)
	SendPropEHandle( SENDINFO( m_live_ragdoll_ent ) ),
#endif
...
#ifdef DARKINTERVAL // DI NEW: meshBuilder based text strings on models
	SendPropStringT(SENDINFO(m_nameplate_string)),
#endif
END_SEND_TABLE()
...
CBaseAnimating::CBaseAnimating()
{
	...
	m_fBoneCacheFlags = 0;
#ifdef DARKINTERVAL
	m_remember_transmit_state_int = 0;

	m_nameplate_string = NULL_STRING; // DI NEW: meshBuilder based text strings on models
...
void CBaseAnimating::SetTransmit( CCheckTransmitInfo *pInfo, bool bAlways )
{
	...
	{
		m_live_ragdoll_ent->SetTransmit(pInfo, bAlways);
	}
#endif
}
...
{
	float flInterval;
	if (m_flAnimTime < CURTIME)
	{
		// estimate what it'll be this frame
		flInterval = clamp( CURTIME - m_flAnimTime, 0.f, MAX_ANIMTIME_INTERVAL );
	}
	else
	{
		// report actual
...
void CBaseAnimating::InvalidateBoneCacheIfOlderThan( float deltaTime )
{
	...
	if ( !pcache || !pcache->IsValid( CURTIME, deltaTime ) )
	{
		InvalidateBoneCache();
	}
}
...
void CBaseAnimating::StudioFrameAdvance()
{
	...
	float flInterval = CURTIME - m_flAnimTime;
	...
	{
		Msg("%5.2f : %s : %s : %5.3f\n", CURTIME, GetClassname(), GetSequenceName( GetSequence() ), GetCycle() );
	}
}
...
#ifdef DARKINTERVAL	// DI NEW: meshBuilder based text strings on models
void CBaseAnimating::InputSetNameplateText(inputdata_t &inputdata)
{
	m_nameplate_string = AllocPooledString("                "); // HACK: using empty 16 char string to clear it first
	m_nameplate_string = AllocPooledString(inputdata.value.String());
}
#endif
#ifdef DARKINTERVAL // changing model name on the fly
//-----------------------------------------------------------------------------
// Purpose: SetModelName input handler
//-----------------------------------------------------------------------------
void CBaseAnimating::InputSetModelName(inputdata_t &inputdata)
{
	char pModelName[512];
	Q_snprintf(pModelName, sizeof(pModelName), "models/%s", inputdata.value.String());
	Q_DefaultExtension(pModelName, ".mdl", sizeof(pModelName));
	MDLHandle_t h = mdlcache->FindMDL(pModelName);
	if (h == MDLHANDLE_INVALID)
	{
		Msg("That model path is incorrect!\nPath should be relevant to models\\ - do NOT include the 'models\\' part itself.\n");
		return;
	}

	PrecacheModel(pModelName);
	Msg("Changing entity '%s' model to '%s'; original model was %s\n", GetEntityName(), pModelName, GetModelName());
	SetModel(pModelName);
}
#endif
#ifdef DARKINTERVAL	// DI NEW: Live Ragdolls Experiment (inspired by Dark Messiah)
void CBaseAnimating::InputSetLiveRagdollEnt(inputdata_t &inputdata)
{
	CBaseEntity *pInputEnt = gEntList.FindEntityByName(NULL, inputdata.value.String(), NULL, inputdata.pActivator, inputdata.pCaller);
	CBaseAnimating *pliveragdollEnt = dynamic_cast<CBaseAnimating *>(pInputEnt);
	if (pliveragdollEnt != NULL)
	{
		m_live_ragdoll_ent = pliveragdollEnt;
	}
}

void CBaseAnimating::InputClearLiveRagdollEnt(inputdata_t &inputdata)
{
	if (m_live_ragdoll_ent)
	{
		m_live_ragdoll_ent = NULL;
...
bool CBaseAnimating::BecomeRagdollOnClient( const Vector &force )
{
	...
	if ( CanBecomeRagdoll() ) 
	{
		...
		if ( pFireChild )
		{
			...
			pFireChild->SetNextThink( CURTIME + 0.1f );
		}
		...
		else if ( pEvent->event == AE_SV_DUSTTRAIL )
		{
			...
			if (sscanf( pEvent->options, "%s %f %f", szAttachment, &flDuration, &flSize ) == 3)
			{
				...
				if( hDustTrail )
				{
					...
					hDustTrail->m_StopEmitTime = CURTIME + flDuration;
...
#ifdef DARKINTERVAL	// spams otherwise
...
{
	m_flIKGroundContactTime = CURTIME;
...
void CBaseAnimating::UpdateStepOrigin()
{
	...
	*/
	...
	if (m_flIKGroundContactTime > 0.2 && m_flIKGroundContactTime > CURTIME - 0.2)
	{
...
#ifdef DARKINTERVAL	// naming consistency
ConVar ai_debug_bones( "ai_debug_bones", "0", 0, "Shows that bones that are setup every think" );
#else
...
void CBaseAnimating::SetupBones( matrix3x4_t *pBoneToWorld, int boneMask )
{
	...
	if ( CanSkipAnimation() )
	{
		...
		// Msg( "%.03f : %s:%s not in pvs\n", CURTIME, GetClassname(), GetEntityName().ToCStr() );
	}
	else 
	{
		if ( m_pIk )
		{
			...
			m_iIKCounter++;
			m_pIk->Init( pStudioHdr, GetAbsAngles(), adjOrigin, CURTIME, m_iIKCounter, boneMask );
...
CBoneCache *CBaseAnimating::GetBoneCache( void )
{
	...
	if ( pcache )
	{
		if ( pcache->IsValid( CURTIME ) && (pcache->m_boneMask & boneMask) == boneMask && pcache->m_timeValid <= CURTIME)
		{
	...
	if ( pcache )
	{
		...
		pcache->UpdateBones( bonetoworld, pStudioHdr->numbones(), CURTIME );
	}
	else
	{
...
bool CBaseAnimating::TestCollision( const Ray_t &ray, unsigned int fContentsMask, trace_t& tr )
{
	...
	if ( GetModelScale() != 1.0f )
	{
		...
		if (!pPhysObject)
		{
			Warning("Couldn't get pPhysObject of %s (%s, model %s) to TestCollision!\n", GetDebugName(), GetClassName(), GetModelName());
			AssertMsg("Couldn't get pPhysObject of %s (%s, model %s) to TestCollision!\n", GetDebugName(), GetClassName(), GetModelName());
			UTIL_Remove(this); // DI REVISE: think on it in the future.
			return false;
		}
#endif
...
#ifdef DARKINTERVAL // DI NEW: meshBuilder based text strings on models
void CBaseAnimating::SetNameplateText(string_t newString)
{
	m_nameplate_string = AllocPooledString(newString.ToCStr());
}
#endif
...
void CBaseAnimating::GetSkeleton( CStudioHdr *pStudioHdr, Vector pos[], Quaternion q[], int boneMask )
{
	...
	boneSetup.AccumulatePose( pos, q, GetSequence(), GetCycle(), 1.0, CURTIME, m_pIk );

	if ( m_pIk )
	{
		CIKContext auto_ik;
		auto_ik.Init( pStudioHdr, GetAbsAngles(), GetAbsOrigin(), CURTIME, 0, boneMask );
		boneSetup.CalcAutoplaySequences( pos, q, CURTIME, &auto_ik );
	}
	else
	{
		boneSetup.CalcAutoplaySequences( pos, q, CURTIME, NULL );
	}
...
#endif
#ifdef DARKINTERVAL
void CBaseAnimating::Ignite( float flFlameLifetime, bool bNPCOnly, float flSize, bool bCalledByLevelDesigner, int flame_color)
#else
...
//		return;
	...
	//	if( flame_plasma ) //pFlame->Plasma
	}

	m_OnIgnite.FireOutput( this, this );
}
#ifdef DARKINTERVAL
void CBaseAnimating::IgniteLifetime( float flFlameLifetime, int flame_color)
#else
...
#endif
{
	if( !IsOnFire() )
#ifdef DARKINTERVAL
		Ignite( 5, false, 0.0f, true, flame_color );
#else
...
void CBaseAnimating::IgniteNumHitboxFires( int iNumHitBoxFires, int flame_color)
{
...
void CBaseAnimating::IgniteHitboxFireScale( float flHitboxFireScale, int flame_color)
{
#if !defined (CLASSIC_FIRE)
	if( !IsOnFire() )
#ifdef DARKINTERVAL
...
#ifdef GLOWS_ENABLE
void CBaseAnimating::InputAddGlowEffect(inputdata_t &inputdata)
{
	Vector vecColor;
	inputdata.value.Vector3D(vecColor);
	AddGlowEffect(vecColor.x, vecColor.y, vecColor.z);
}
void CBaseAnimating::InputRemoveGlowEffect(inputdata_t &inputdata)
{
	RemoveGlowEffect();
}
//-----------------------------------------------------------------------------
// Purpose: 
//-----------------------------------------------------------------------------
void CBaseAnimating::AddGlowEffect(float r, float g, float b)
{
	m_remember_transmit_state_int = GetTransmitState();
	SetTransmitState(FL_EDICT_ALWAYS);
	m_fGlowColorRed.Set(r);
	m_fGlowColorGreen.Set(g);
	m_fGlowColorBlue.Set(b);
...
{
	SetTransmitState(m_remember_transmit_state_int);
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
#ifdef DARKINTERVAL // Ice, ported freezing mechanic from ASW
void CBaseAnimating::InputFreeze(inputdata_t &inputdata)
{
	Freeze(m_flFrozen + 0.5f);
}

void CBaseAnimating::InputUnfreeze(inputdata_t &inputdata)
{
	Unfreeze();
}
		...
		if (pStudioHdr)
		{
			...
			if (set && set->numhitboxes > 0)
			{
				for (int i = 0; i < set->numhitboxes; ++i)
				{
					...
					trace_t tr; 
					if (IntersectRayWithOBB(*pFreezeRay, vecPosition, angAngles, pBox->bbmin, pBox->bbmax, 0.0f, &tr))
					{
