...
void AI_Response::Describe()
{
	...
	if ( m_szContext )
	{
		Msg( "Contexts to set '%s' on %s, ", m_szContext, m_bApplyContextToWorld ? "world" : "speaker" );
	}
...
#ifdef DARKINTERVAL
float AI_Response::GetRadioHUDStart(void) const
{
	if (m_Params.flags & AI_ResponseParams::RG_SHOWRADUIOHUD)
	{
		interval_t temp;
		m_Params.showradiohud.ToInterval(temp);
		return temp.start;
	}

	return 0.0f;
}
float AI_Response::GetRadioHUDLength(void) const
{
	if (m_Params.flags & AI_ResponseParams::RG_SHOWRADUIOHUD)
	{
		interval_t temp;
...
const char *SplitContext( const char *raw, char *key, int keylen, char *value, int valuelen, float *duration )
{
	...
	if ( !colon1 )
	{
		Msg( "SplitContext:  warning, ignoring context '%s', missing colon separator!\n", raw );
		*key = *value = 0;
		return NULL;
	}
