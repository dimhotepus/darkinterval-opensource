...
void CWeaponPortalgun::DoEffectBlast( bool bPortal2, int iPlacedBy, const Vector &ptStart, const Vector &ptFinalPos, const QAngle &qStartAngles, float fDelay )
{
	...
	fxData.m_flScale = CURTIME + fDelay;
...
float CWeaponPortalgun::FirePortal( bool bPortal2, Vector *pVector /*= 0*/, bool bTest /*= false*/ )
{
	...
	if ( !bTest )
	{
		...
		pPortal->SetContextThink( &CProp_Portal::DelayedPlacementThink, CURTIME + fDelay, s_pDelayedPlacementContext ); 
...
static void change_portalgun_linkage_id_f( const CCommand &args )
{
	...
	CHL2_Player *pPlayer = (CHL2_Player *)UTIL_GetCommandClient();
...
ConCommand change_portalgun_linkage_id( "change_portalgun_linkage_id", change_portalgun_linkage_id_f, "Changes the portal linkage ID for the portal gun held by the commanding player.", FCVAR_NONE);
#endif