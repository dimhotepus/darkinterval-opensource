...
ConVar sv_portal_new_velocity_check("sv_portal_new_velocity_check", "1", FCVAR_NONE);
...
BEGIN_DATADESC( CProp_Portal )
	...
	DEFINE_KEYFIELD(m_ParentWatchEntity, FIELD_STRING, "ParentWatchEntity"),
	...
	// Function Pointers
	DEFINE_THINKFUNC( FollowParentThink),
...
{
	m_ParentWatchEntity = NULL_STRING;
...
void CProp_Portal::Spawn( void )
{
	...
	if( m_ParentWatchEntity != NULL_STRING)
	SetContextThink(&CProp_Portal::FollowParentThink, CURTIME + 0.1f, s_pFollowParentContext);
}
...
void CProp_Portal::DelayedPlacementThink( void )
{
	...
	if( pPortalGun )
	{
		CHL2_Player *pFiringPlayer = dynamic_cast<CHL2_Player *>( pPortalGun->GetOwner() );
		if( pFiringPlayer )
		{
#if 0
	...
	SetContextThink( &CProp_Portal::TestRestingSurfaceThink, CURTIME + 0.1f, s_pTestRestingSurfaceContext );
}

void CProp_Portal::FollowParentThink(void)
{
//	Vector origin = GetAbsOrigin();
//	QAngle angles = GetAbsAngles();
//	origin.x += 0.5f;
//	Teleport(&origin, &angles, 0);

	CBaseEntity *watchEntity = gEntList.FindEntityByName(NULL, m_ParentWatchEntity);

	if( watchEntity)
	{
		// This does not change the portal's angles, only origin.
		// I decided it's best to have it this way because
		// we can be certain about its orientation - which can't
		// be said about e. g. func_tracktrain if we choose to 
		// follow it. 
		Teleport(&watchEntity->GetAbsOrigin(), &GetAbsAngles(), 0);
	}
	SetNextThink(CURTIME, s_pFollowParentContext);
}
...
{
	SetContextThink( &CProp_Portal::FizzleThink, CURTIME, s_pFizzleThink );
}
...
void CProp_Portal::TeleportTouchingEntity( CBaseEntity *pOther )
{
	...
	CHL2_Player *pOtherAsPlayer;

	
	if( bPlayer )
	{
		//NDebugOverlay::EntityBounds( pOther, 255, 0, 0, 128, 60.0f );
		pOtherAsPlayer = (CHL2_Player *)pOther;
	...
	{
		CHL2_Player *pHoldingPlayer = (CHL2_Player *)GetPlayerHoldingEntity( pOther );
...
void CProp_Portal::Touch( CBaseEntity *pOther )
{
	...
	if( !m_bActivated || (m_hLinkedPortal.Get() == NULL) )
	{
		...
		Assert( !pOther->IsPlayer() || (((CHL2_Player *)pOther)->m_hPortalEnvironment.Get() != this) );
	...
	if( vVelocityCheck != vec3_origin || vAngularImpulseCheck != vec3_origin )
	{
		...
		{
			if ( !FClassnameIs( pOther, "func_physbox" ) 
				&& !FClassnameIs(pOther, "func_brush" )
				&& !FClassnameIs(pOther, "func_tracktrain")
				&& !FClassnameIs( pOther, "simple_physics_brush" ) )	// except CPhysBox
			{
				Vector vForward;
				GetVectors( &vForward, NULL, NULL );
...
void CProp_Portal::StartTouch( CBaseEntity *pOther )
{
	...
	if( sv_portal_debug_touch.GetBool() )
	{
		DevMsg( "Portal %i StartTouch: %s : %f\n", ((m_bIsPortal2)?(2):(1)), pOther->GetClassname(), CURTIME );
	}
#if !defined ( DISABLE_DEBUG_HISTORY )
	if ( !IsMarkedForDeletion() )
	{
...
void CProp_Portal::EndTouch( CBaseEntity *pOther )
{
	...
	if( ShouldTeleportTouchingEntity( pOther ) ) //an object passed through the plane and all the way out of the touch box
		TeleportTouchingEntity( pOther );
			...
			(((CHL2_Player *)pOther)->m_Local.m_bInDuckJump) ) //while ducking
	{
...
{
	if( pEntity->IsPlayer() )
		((CHL2_Player *)pEntity)->m_hPortalEnvironment = this;
}

void CProp_Portal::PortalSimulator_ReleasedOwnershipOfEntity( CBaseEntity *pEntity )
{
	if( pEntity->IsPlayer() && (((CHL2_Player *)pEntity)->m_hPortalEnvironment.Get() == this) )
		((CHL2_Player *)pEntity)->m_hPortalEnvironment = NULL;
}
