...
void CFunc_LiquidPortal::Activate( void )
{
	...
	for( int i = m_hLeftToTeleportThisFill.Count(); --i >= 0; )
	{
		...
		{
			((CHL2_Player *)pEnt)->m_hSurroundingLiquidPortal = this;
		}
	}
}
...
{
	AssertMsg( m_fFillEndTime <= CURTIME, "Fill already in progress." );
	m_fFillStartTime = CURTIME;
	m_fFillEndTime = CURTIME + m_fFillTime;
...
void CFunc_LiquidPortal::InputAddActivatorToTeleportList( inputdata_t &inputdata )
{
	...
	if( inputdata.pActivator->IsPlayer() )
		((CHL2_Player *)inputdata.pActivator)->m_hSurroundingLiquidPortal = this;
}
...
void CFunc_LiquidPortal::InputRemoveActivatorFromTeleportList( inputdata_t &inputdata )
{
	...
	for( int i = m_hTeleportList.Count(); --i >= 0; )
	{
		if( m_hTeleportList[i].Get() == inputdata.pActivator )
		{
			...
			if( inputdata.pActivator->IsPlayer() && (((CHL2_Player *)inputdata.pActivator)->m_hSurroundingLiquidPortal.Get() == this) )
				((CHL2_Player *)inputdata.pActivator)->m_hSurroundingLiquidPortal = NULL;
...
void CFunc_LiquidPortal::TeleportImmersedEntity( CBaseEntity *pEntity )
{
	...
	if( pEntity->IsPlayer() )
	{
		CHL2_Player *pEntityAsPlayer = (CHL2_Player *)pEntity;
...
void CFunc_LiquidPortal::Think( void )
{
	if( m_bFillInProgress )
	{
		if( CURTIME < m_fFillEndTime )
		{
			float fInterp = ((CURTIME - m_fFillStartTime) / (m_fFillEndTime - m_fFillStartTime));
			Vector vMins, vMaxs;
