...
void CTriggerPortalCleanser::Touch( CBaseEntity *pOther )
{
	...
	if ( pOther->IsPlayer() )
	{
		CHL2_Player *pPlayer = ToHL2Player( pOther );
		...
		if ( pPlayer )
		{
			...
			if ( pPortalgun )
			{
				...
				if ( pPortalgun->CanFirePortal1() )
				{
					...
					if ( pPortal && pPortal->GetNextThink( s_pDelayedPlacementContext ) > CURTIME )
					{
						pPortal->SetContextThink( NULL, CURTIME, s_pDelayedPlacementContext ); 
	...
	if ( pBaseAnimating && !pBaseAnimating->IsDissolving() )
	{
		...
		{
			CHL2_Player *pPlayer = (CHL2_Player *)GetPlayerHoldingEntity( pBaseAnimating );
			if( pPlayer )
			{
		...
		{
			pDisolvingAnimating->Dissolve( "", CURTIME, false, ENTITY_DISSOLVE_NORMAL );
		}
