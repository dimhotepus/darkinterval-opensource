...
#ifdef DARKINTERVAL // fix the AI oversight where player can use a tiny prop to block their origin from enemy's LOS
//ConVar ai_ignore_player_held_props_for_weapon_los("ai_ignore_player_held_props_for_weapon_los", "1");
#endif
//-----------------------------------------------------------------------------
...
CBaseEntity* CBaseCombatWeapon::Respawn( void )
{
	...
	if ( pNewWeapon )
	{
		...
		pNewWeapon->SetNextThink( CURTIME + g_pGameRules->FlWeaponRespawnTime( this ) );
	}
	else
	{
...
bool CBaseCombatWeapon::WeaponLOSCondition( const Vector &ownerPos, const Vector &targetPos, bool bSetConditions )
{
	...
	trace_t tr;
#ifdef DARKINTERVAL	// DI NEW: extra trace that hits grates, then checks if they're BlocksLOS()
	UTIL_TraceLine(barrelPos, targetPos, MASK_SHOT_HULL, m_hOwner.Get(), COLLISION_GROUP_BREAKABLE_GLASS, &tr);
	if (tr.fraction != 1.0)
	{
		CBaseEntity	*pHitEnt = tr.m_pEnt;
		{
			if (pHitEnt->ClassMatches("prop_dynamic") && pHitEnt->BlocksLOS())
			{
				if (ai_debug_shoot_positions.GetBool())
				{
					NDebugOverlay::Line(barrelPos, targetPos, 255, 225, 25, false, 1.0);
				}
				if (!FClassnameIs(this, "weapon_oicw")) // OICW users (soldiers) don't suffer this penalty
				{
...
#ifdef DARKINTERVAL // fix the AI oversight where player can use a tiny prop to block their origin from enemy's LOS
	if (pEnemyPlayer /*&& pEnemyPlayer->GetUseEntity() == pHitEnt*/ && !pHitEnt->BlocksLOS())
	{
		return /*ai_ignore_player_held_props_for_weapon_los.GetBool()*/ true;
	}
#endif
...
void CBaseCombatWeapon::FallInit( void )
{
	...
	SetNextThink( CURTIME + 0.1f );
#ifdef DARKINTERVAL // todo - needed?
	AddGlowEffect(1, 0.1, 0.1);
#endif
}
