...
#ifdef DARKINTERVAL
	DEFINE_OUTPUT(m_OnMagnetAttachPassEntity, "OnAttachPassEntity"), // DI NEW - same as OnAttach, but passes along the attached entity as the activator
	DEFINE_OUTPUT(m_OnMagnetDetachPassEntity, "OnDetachPassEntity"), // DI NEW - same as OnDetach, but passes along the attached entity as the activator
#endif
	// Keys
	DEFINE_KEYFIELD( m_massScale, FIELD_FLOAT, "massScale" ),
	DEFINE_KEYFIELD( m_iszOverrideScript, FIELD_STRING, "overridescript" ),
...
#ifdef DARKINTERVAL
	m_OnMagnetAttachPassEntity.FireOutput(pOther, this);
#endif
	BaseClass::VPhysicsCollision( index, pEvent );
}
...
#ifdef DARKINTERVAL
			m_OnMagnetDetachPassEntity.FireOutput(m_MagnettedEntities[i].hEntity.Get(), this);
#endif
