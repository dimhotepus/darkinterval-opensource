...
#ifdef DARKINTERVAL
#define	MAX_NODE_LINK_DIST			256*16		// Maximum connection length between nodes as well as furthest
#else
...
#ifdef DARKINTERVAL
#define	MAX_AIR_NODE_LINK_DIST		512*16	// Maximum connection length between air nodes as well as furthest
#else
#define	MAX_AIR_NODE_LINK_DIST		120*12
#endif
...
public:
	...
	bool 			IsLocked() const			{ return ( m_flNextUseTime > CURTIME ); }
	void			Lock( float duration )		{ m_flNextUseTime = CURTIME + duration; }
	void			Unlock()					{ m_flNextUseTime = CURTIME; }
