	...
	for ( i = 0; i < TLK_CFRIENDS; i++ )
	{
		while ((pFriend = EnumFriends( pFriend, i, true )) != NULL )
		{
			...
			if ( pNPC->IsAlive() )
			{
				...
				{
#ifdef DARKINTERVAL // so that it works with wider list of reguarly used NPCs
					...
					{
#ifdef DARKINTERVAL // todo - investigate, this may not be working
						AI_CriteriaSet set;
						set.AppendCriteria("victim_class", (GetClassname()));
						pTalkNPC->ModifyOrAppendCriteria(set);
#endif
...
//-----------------------------------------------------------------------------
CBaseEntity *CNPCSimpleTalker::FindNamedEntity(const char *pszName, IEntityFindFilter *pFilter)
{
...
{
	if ( m_flNextIdleSpeechTime > CURTIME )
		return false;
...
{ 
...
#ifdef DARKINTERVAL // used to be disabled
	// if there is a friend nearby to speak to, play sentence, set friend's response time, return
	CBaseEntity *pFriend = FindNearestFriend(false);

	// 75% chance of talking to another citizen if one is available.
#ifdef DARKINTERVAL
	if (pFriend && !(pFriend->IsMoving()) ) // the 75% chance is now built into the criteria.
#else
...
#endif
	{
		if ( SpeakQuestionFriend( pFriend ) )
		{
...
#ifdef DARKINTERVAL // rewritten to work
			CNPCSimpleTalker *pTalkNPC = dynamic_cast<CNPCSimpleTalker *>(pFriend);
			if (pTalkNPC) // this'll work for vort on vort Q&A.
			{
				if (!pTalkNPC->HasSpawnFlags(SF_NPC_GAG) && !pTalkNPC->IsInAScript())
				{
			...
			else// this'll work for vort on citizen Q&A.
			{
				CAI_PlayerAlly *pTalkAlly = dynamic_cast<CAI_PlayerAlly *>(pFriend); 
				if (pTalkAlly && !pTalkAlly->HasSpawnFlags(SF_NPC_GAG) && !pTalkAlly->IsInAScript())
				{
					SetSpeechTarget(pTalkAlly);
					pTalkAlly->SetSpeechTarget(this);
					pTalkAlly->AnswerQuestion(this, -1, false); // todo - the citizens answer instantly without waiting for question to end.
					pTalkAlly->GetExpresser()->BlockSpeechUntil(GetExpresser()->GetTimeSpeechComplete());

					m_nSpeak++;
				}

				// Don't let anyone else butt in.
				DeferAllIdleSpeech(random->RandomFloat(TALKER_DEFER_IDLE_SPEAK_MIN, TALKER_DEFER_IDLE_SPEAK_MAX), this);
			}
#else
	...
	if ( pFriend )
	{
		...
		else
		{
			...
			m_flNextIdleSpeechTime = CURTIME + 3;
		}
...
{
#ifdef DARKINTERVAL
	return BaseClass::SelectNonCombatSpeechSchedule(); // let player ally code handle it all. It just works.
#else
	if ( !IsOkToSpeak() )
		return SCHED_NONE;
