...
{
#ifdef DARKINTERVAL
	if ( GetSink() && !GetSink()->UseSemaphore() )
#else
...
bool CAI_Expresser::SpeakDispatchResponse( AIConcept_t concept, AI_Response *result, IRecipientFilter *filter /* = NULL */ )
{
	...
	case RESPONSE_PRINT:
		{
#ifndef DARKINTERVAL // this can be desired without developer for either debugging or modded levels
	...
	if ( spoke )
	{
		m_flLastTimeAcceptedSpeak = CURTIME;
...
bool CAI_Expresser::Speak( AIConcept_t concept, const char *modifiers /*= NULL*/, char *pszOutResponseChosen /* = NULL*/, size_t bufsize /* = 0 */, IRecipientFilter *filter /* = NULL */ )
{
	...
	SpeechMsg( GetOuter(), "%s (%p) spoke %s (%f)\n", STRING(GetOuter()->GetEntityName()), GetOuter(), concept, CURTIME );
...
void CAI_Expresser::NoteSpeaking( float duration, float delay )
{
	...
	{
		// no duration :( 
		m_flStopTalkTime = CURTIME + 3;
		duration = 0;
	}
	else
	{
		m_flStopTalkTime = CURTIME + duration;
	}
...
void CAI_Expresser::ForceNotSpeaking( void )
{
	if ( IsSpeaking() )
	{
		m_flStopTalkTime = CURTIME;
		m_flStopTalkTimeWithoutDelay = CURTIME;
...
bool CAI_Expresser::IsSpeaking( void )
{
	...
	if ( m_flLastTimeAcceptedSpeak == CURTIME ) // only one speak accepted per think
		return true;

	return ( m_flStopTalkTime > CURTIME );
}
...
bool CAI_Expresser::CanSpeakConcept( AIConcept_t concept )
{
	...
	{
		if ( history->timeSpoken != -1 && ( CURTIME < history->timeSpoken + respeakDelay ) )
			return false;
	}
...
void CAI_ExpresserHost_NPC_DoModifyOrAppendCriteria( CAI_BaseNPC *pSpeaker, AI_CriteriaSet& set )
{
	...
	else
	{
		if ( pSpeaker->GetLastEnemyTime() == 0.0 )
			...
			set.AppendCriteria( "timesincecombat", UTIL_VarArgs( "%f", CURTIME - pSpeaker->GetLastEnemyTime() ) );
	}
