	...
	case CChoreoEvent::GENERIC:
		{
			if (stricmp( event->GetParameters(), "AI_BLINK") == 0)
			{
				...
				m_flBlinktime = CURTIME + MAX( flDuration, random->RandomFloat( 1.5, 4.5 ) ); 
			}
			...
			else if (stricmp( event->GetParameters(), "AI_IGNORECOLLISION") == 0)
			{
				...
				if (pTarget)
				{
					...
					info->m_flNext = CURTIME + remaining;
					return true;
				}
				else
				{
...
#ifdef DARKINTERVAL // handy debug info
	...
	case CChoreoEvent::GENERIC:
		{
			switch(info->m_nType)
			{
				case SCENE_AI_BLINK:
					{
						...
						float flDuration = (event->GetEndTime() - scene->GetTime());
						m_flBlinktime = MAX( m_flBlinktime, CURTIME + flDuration );
					}
					return true;
				...
				case SCENE_AI_RANDOMLOOK:
					{
						if (info->m_flNext < CURTIME)
						{
							info->m_flNext = CURTIME + PickLookTarget( m_syntheticLookQueue ) - 0.4;
							if (m_syntheticLookQueue.Count() > 0)
							{
								float flDuration = (event->GetEndTime() - scene->GetTime());
								int i = m_syntheticLookQueue.Count() - 1;
								m_syntheticLookQueue[i].m_flEndTime = MIN( m_syntheticLookQueue[i].m_flEndTime, CURTIME + flDuration );
				...
				case SCENE_AI_IGNORECOLLISION:
					if (info->m_hTarget && info->m_flNext < CURTIME)
					{
						float remaining = event->GetEndTime() - scene->GetTime();
...
bool CAI_BaseActor::RandomFaceFlex( CSceneEventInfo *info, CChoreoScene *scene, CChoreoEvent *event )
{
	if (info->m_flNext < CURTIME)
	{
		...
		if ( pSettinghdr )
		{
			info->m_flNext = CURTIME + random->RandomFloat( 0.3, 0.5 ) * (30.0 / pSettinghdr->numflexsettings);
...
#ifdef _DEBUG // DI makes it a debug-only feature
...
void CAI_BaseActor::UpdateHeadControl( const Vector &vHeadTarget, float flHeadInfluence )
{
	...
	if (iEyes <= 0 || iForward <= 0)
	{
		...
		DevWarning( "%s using model \"%s\" that doesn't have attachments related to head turning\n", GetClassname(), STRING( GetModelName() ) );
...
void CAI_BaseActor::ClearLookTarget( CBaseEntity *pTarget )
{
	...
	if ( iIndex != m_randomLookQueue.InvalidIndex() )
	{
		...
		m_flNextRandomLookTime = CURTIME + 1.0;
...
void CAI_BaseActor::MaintainLookTargets( float flInterval )
{
	...
	if (IsCurSchedule(SCHED_DI_FALL_IN_RAGDOLL_PUSH))
		return; //// DI NEW: Live Ragdolls Experiment, inspired by Dark Messiah - don't switch schedules if in special ragdoll sched
#endif
	int i;
...
#ifdef DARKINTERVAL
	if(ai_debug_looktargets.GetInt() > 0)
	...
	if (m_lookQueue.Count() != 0 || m_syntheticLookQueue.Count() != 0)
	{
		...
		{
			if (CURTIME < m_randomLookQueue[i].m_flEndTime - m_randomLookQueue[i].m_flRamp - 0.2)
			{
				m_randomLookQueue[i].m_flEndTime = CURTIME + m_randomLookQueue[i].m_flRamp + 0.2;
			}
		}
		m_flNextRandomLookTime = CURTIME + 1.0;
	}
	else if (CURTIME >= m_flNextRandomLookTime && GetState() != NPC_STATE_SCRIPT)
	{
		// Look at whatever!
		m_flNextRandomLookTime = CURTIME + PickLookTarget( m_randomLookQueue ) - 0.4;
	}
	...
	if ( m_flNextRandomExpressionTime && CURTIME > m_flNextRandomExpressionTime )
	{
	...
	if (!bFoundTarget && !ValidEyeTarget( GetViewtarget() ))
	{
		...
		// DevMsg("random view\n");
#ifdef DARKINTERVAL // DI npcs look a bit farther when choosing random look targets
		SetViewtarget( EyePosition() + HeadDirection3D() * 128 + right * random->RandomFloat(-128,128) + up * random->RandomFloat(-128,128) );
#else
	...
	// blink
	if (m_flBlinktime < CURTIME)
	{
		Blink();
#ifdef DARKINTERVAL // blink frequency tweak
		m_flBlinktime = CURTIME + random->RandomFloat( 0.5, 4.5 );
#else
		m_flBlinktime = CURTIME + random->RandomFloat(1.5, 4.5);
#endif
	}
...
void CAI_BaseActor::PlayExpressionForState( NPC_STATE state )
{
	...
	if ( pszExpression && *pszExpression )
	{
		...
		m_flNextRandomExpressionTime = CURTIME + flDuration;
		return;
	}
	else
	{
...
#ifdef DARKINTERVAL
// Let the hacks begin.
BEGIN_DATADESC(CAI_BaseActorNoBlend)
...
// Purpose: clear out latched state
//-----------------------------------------------------------------------------
void CAI_BaseActorNoBlend::StudioFrameAdvance()
{
	// clear out head and eye latched values
	m_fLatchedPositions &= ~(HUMANOID_LATCHED_ALL);
...
void CAI_BaseActorNoBlend::Precache()
{
	BaseClass::Precache();
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CAI_BaseActorNoBlend::IsServerSideFlexController(char const *szName)
{
	int c = ARRAYSIZE(g_ServerSideFlexControllers);
	for (int i = 0; i < c; ++i)
	{
...
void CAI_BaseActorNoBlend::SetModel(const char *szModelName)
{
	BaseClass::SetModel(szModelName);
...
bool CAI_BaseActorNoBlend::StartSceneEvent(CSceneEventInfo *info, CChoreoScene *scene, CChoreoEvent *event, CChoreoActor *actor, CBaseEntity *pTarget)
{
	Assert(info);
	Assert(info->m_pScene);
	Assert(info->m_pEvent);
...
bool CAI_BaseActorNoBlend::ProcessSceneEvent(CSceneEventInfo *info, CChoreoScene *scene, CChoreoEvent *event)
{
	Assert(info);
	Assert(info->m_pScene);
	Assert(info->m_pEvent);
...
bool CAI_BaseActorNoBlend::RandomFaceFlex(CSceneEventInfo *info, CChoreoScene *scene, CChoreoEvent *event)
{
	if (info->m_flNext < CURTIME)
	{
		const flexsettinghdr_t *pSettinghdr = (const flexsettinghdr_t *)FindSceneFile(event->GetParameters2());
...
bool CAI_BaseActorNoBlend::ClearSceneEvent(CSceneEventInfo *info, bool fastKill, bool canceled)
{
	Assert(info);
	Assert(info->m_pScene);
	Assert(info->m_pEvent);
...
bool CAI_BaseActorNoBlend::CheckSceneEventCompletion(CSceneEventInfo *info, float currenttime, CChoreoScene *scene, CChoreoEvent *event)
{
	Assert(info);
	Assert(info->m_pScene);
	Assert(info->m_pEvent);
...
// Purpose: clear out latched state
//-----------------------------------------------------------------------------
void CAI_BaseActorNoBlend::SetViewtarget(const Vector &viewtarget)
{
	// clear out eye latch
	m_fLatchedPositions &= ~HUMANOID_LATCHED_EYE;
...
// Purpose: Returns true position of the eyeballs
//-----------------------------------------------------------------------------
void CAI_BaseActorNoBlend::UpdateLatchedValues()
{
	if (!(m_fLatchedPositions & HUMANOID_LATCHED_HEAD))
	{
		// set head latch
...
// Purpose: Returns true position of the eyeballs
//-----------------------------------------------------------------------------
Vector CAI_BaseActorNoBlend::EyePosition()
{
	UpdateLatchedValues();
...
// Purpose: Returns true if target is in legal range of eye movement for the current head position
//-----------------------------------------------------------------------------
bool CAI_BaseActorNoBlend::ValidEyeTarget(const Vector &lookTargetPos)
{
	Vector vHeadDir = HeadDirection3D();
	Vector lookTargetDir = lookTargetPos - EyePosition();
	float flDist = VectorNormalize(lookTargetDir);
...
// Purpose: Returns true if target is in legal range of possible head movements
//-----------------------------------------------------------------------------
bool CAI_BaseActorNoBlend::ValidHeadTarget(const Vector &lookTargetPos)
{
	Vector vFacing = BodyDirection3D();
	Vector lookTargetDir = lookTargetPos - EyePosition();
	float flDist = VectorNormalize(lookTargetDir);
...
// Purpose: Returns how much to try to look at the target
//-----------------------------------------------------------------------------
float CAI_BaseActorNoBlend::HeadTargetValidity(const Vector &lookTargetPos)
{
	Vector vFacing = BodyDirection3D();
...
// Purpose: Integrate head turn over time
//-----------------------------------------------------------------------------
void CAI_BaseActorNoBlend::SetHeadDirection(const Vector &vTargetPos, float flInterval)
{
	Assert(0); // Actors shouldn't be calling this, it doesn't do anything
}

float CAI_BaseActorNoBlend::ClampWithBias(PoseParameter_t index, float value, float base)
{
	return EdgeLimitPoseParameter((int)index, value, base);
}
...
void CAI_BaseActorNoBlend::AccumulateIdealYaw(float flYaw, float flIntensity)
{
	float diff = AngleDiff(flYaw, GetLocalAngles().y);
	m_flAccumYawDelta += diff * flIntensity;
	m_flAccumYawScale += flIntensity;
...
bool CAI_BaseActorNoBlend::SetAccumulatedYawAndUpdate(void)
{
	if (m_flAccumYawScale > 0.0)
	{
		float diff = m_flAccumYawDelta / m_flAccumYawScale;
...
void CAI_BaseActorNoBlend::UpdateBodyControl()
{
	// FIXME: only during idle, or in response to accel/decel
	//Set( m_ParameterBodyTransY, Get( m_FlexweightMoveRightLeft ) );
	//Set( m_ParameterBodyTransX, Get( m_FlexweightMoveForwardBack ) );
...
void CAI_BaseActorNoBlend::UpdateHeadControl(const Vector &vHeadTarget, float flHeadInfluence)
{
	float flTarget;
	float flLimit;
...
// Output  :
//------------------------------------------------------------------------------
Vector CAI_BaseActorNoBlend::EyeDirection2D(void)
{
	Vector vEyeDirection = EyeDirection3D();
	vEyeDirection.z = 0;
...
// Output  :
//------------------------------------------------------------------------------
Vector CAI_BaseActorNoBlend::EyeDirection3D(void)
{
	UpdateLatchedValues();
...
// Output  :
//------------------------------------------------------------------------------
Vector CAI_BaseActorNoBlend::HeadDirection2D(void)
{
	Vector vHeadDirection = HeadDirection3D();
	vHeadDirection.z = 0;
	vHeadDirection.AsVector2D().NormalizeInPlace();
...
// Output  :
//------------------------------------------------------------------------------
Vector CAI_BaseActorNoBlend::HeadDirection3D()
{
	UpdateLatchedValues();
...
// Purpose:
//-----------------------------------------------------------------------------
bool CAI_BaseActorNoBlend::HasActiveLookTargets(void)
{
	return m_lookQueue.Count() != 0;
}
...
// Purpose: Clear any active look targets for the specified entity
//-----------------------------------------------------------------------------
void CAI_BaseActorNoBlend::ClearLookTarget(CBaseEntity *pTarget)
{
	int iIndex = m_lookQueue.Find(pTarget);
	if (iIndex != m_lookQueue.InvalidIndex())
	{
...
// Purpose: Look at other NPCs and clients from time to time
//-----------------------------------------------------------------------------
float CAI_BaseActorNoBlend::PickLookTarget(bool bExcludePlayers, float minTime, float maxTime)
{
	return PickLookTarget(m_randomLookQueue, bExcludePlayers, minTime, maxTime);
}

float CAI_BaseActorNoBlend::PickLookTarget(CAI_InterestTarget &queue, bool bExcludePlayers, float minTime, float maxTime)
{
	AILookTargetArgs_t args;
...
bool CAI_BaseActorNoBlend::PickTacticalLookTarget(AILookTargetArgs_t *pArgs)
{
	CBaseEntity *pEnemy = GetEnemy();
...
bool CAI_BaseActorNoBlend::PickRandomLookTarget(AILookTargetArgs_t *pArgs)
{
	bool bIsNavigating = (GetNavigator()->IsGoalActive() && GetNavigator()->IsGoalSet());
...
// All attempts to find a target have failed, so just make something up.
//-----------------------------------------------------------------------------
void CAI_BaseActorNoBlend::MakeRandomLookTarget(AILookTargetArgs_t *pArgs, float minTime, float maxTime)
{
	Vector forward, right, up;
	GetVectors(&forward, &right, &up);
...
void CAI_BaseActorNoBlend::StartTaskRangeAttack1(const Task_t *pTask)
{
	BaseClass::StartTaskRangeAttack1(pTask);
	if (GetEnemy())
	{
...
// Purpose: Set direction that the NPC is looking
//-----------------------------------------------------------------------------
void CAI_BaseActorNoBlend::AddLookTarget(CBaseEntity *pTarget, float flImportance, float flDuration, float flRamp)
{
	m_lookQueue.Add(pTarget, flImportance, flDuration, flRamp);
}

void CAI_BaseActorNoBlend::AddLookTarget(const Vector &vecPosition, float flImportance, float flDuration, float flRamp)
{
	m_lookQueue.Add(vecPosition, flImportance, flDuration, flRamp);
}
...
// Purpose: Maintain eye, head, body postures, etc.
//-----------------------------------------------------------------------------
void CAI_BaseActorNoBlend::MaintainLookTargets(float flInterval)
{
	int i;
...
void CAI_BaseActorNoBlend::PlayExpressionForState(NPC_STATE state)
{
	// If we have an override expression, use it above everything else
	if (m_iszExpressionOverride != NULL_STRING && state != NPC_STATE_DEAD)
	{
...
//			the state's expression loop.
//-----------------------------------------------------------------------------
const char *CAI_BaseActorNoBlend::SelectRandomExpressionForState(NPC_STATE state)
{
	return NULL;
}
...
void CAI_BaseActorNoBlend::OnStateChange(NPC_STATE OldState, NPC_STATE NewState)
{
	PlayExpressionForState(NewState);
...
float CAI_BaseActorNoBlend::SetExpression(const char *pszExpressionScene)
{
	if (!pszExpressionScene || !*pszExpressionScene)
	{
		ClearExpression();
...
void CAI_BaseActorNoBlend::ClearExpression()
{
	if (m_hExpressionSceneEnt != NULL)
	{
		StopScriptedScene(this, m_hExpressionSceneEnt);
...
const char *CAI_BaseActorNoBlend::GetExpression()
{
	return STRING(m_iszExpressionScene);
}
...
void CAI_BaseActorNoBlend::InputSetExpressionOverride(inputdata_t &inputdata)
{
	bool fHadOverride = (m_iszExpressionOverride != NULL_STRING);
	m_iszExpressionOverride = inputdata.value.StringID();
	if (m_iszExpressionOverride != NULL_STRING)
...
bool CAI_BaseActorNoBlend::UseSemaphore(void)
{
	if (m_bDontUseSemaphore)
		return false;
...
CAI_Expresser *CAI_BaseActorNoBlend::CreateExpresser()
{
	m_pExpresser = new CAI_Expresser(this);
	return m_pExpresser;
}
...
CAI_Expresser *CAI_BaseActorNoBlend::GetExpresser()
{
	return m_pExpresser;
}
...
bool CAI_BaseActorNoBlend::CreateComponents()
{
	if (!BaseClass::CreateComponents())
		return false;
