...
void CPhysicsHook::FrameUpdatePostEntityThink( ) 
{
	...
	// update the physics simulation, note we don't use gpGlobals->frametime, since that can be 30 msec or 15 msec
	// depending on whether IsSimulatingOnAlternateTicks is true or not
	if ( CBaseEntity::IsSimulatingOnAlternateTicks() )
	{
		m_isFinalTick = false;

#ifdef PORTAL_COMPILE //slight detour if we're the portal mod
...
void CCollisionEvent::UpdatePenetrateEvents( void )
{
	for ( int i = m_penetrateEvents.Count()-1; i >= 0; --i )
	{
		...
		else if ( CURTIME - m_penetrateEvents[i].timeStamp > 1.0 )
		{
...
int CCollisionEvent::ShouldSolvePenetration( IPhysicsObject *pObj0, IPhysicsObject *pObj1, void *pGameData0, void *pGameData1, float dt )
{
	...
	float eventTime = CURTIME - event.startTime;
	...
	if ( phys_dontprintint.GetBool() == false )
	{
		...
		if ( pEntity0 == pEntity1 )
		{
			...
			DevMsg(1, "***Inter-penetration on %s (%d & %d) (%.0f, %.0f)\n", pName1?pName1:"(null)", index0, index1, CURTIME, eventTime );
		}
		else
		{
			DevMsg(1, "***Inter-penetration between %s(%s) AND %s(%s) (%.0f, %.0f)\n", pName1?pName1:"(null)", pEntity0->GetDebugName(), pName2?pName2:"(null)", pEntity1->GetDebugName(), CURTIME, eventTime );
		}
	}
#endif
...
void CCollisionEvent::Friction( IPhysicsObject *pObject, float energy, int surfaceProps, int surfacePropsHit, IPhysicsCollisionData *pData )
{
	...
	if ( pEntity  )
	{
		...
		{
#ifdef DARKINTERVAL // DI is not SP and the old code didn't check
			if ( (pFriction->flLastEffectTime + 0.1f) > CURTIME)
#else
			// in MP mode play sound and effects once every 500 msecs,
			// no ongoing updates, takes too much bandwidth
			if ( ( pFriction->flLastEffectTime + 0.5f ) > CURTIME )
#endif
			{
...
#ifdef DARKINTERVAL
			float amplitude = impulse * (0.2 / (30.0f * VPHYSICS_LARGE_OBJECT_MASS));
			UTIL_ScreenShake( vecPos, amplitude, 0.5, 0.5, amplitude * 60, SHAKE_START );
#else
