...
static ConCommand ai_disable("ai_disable", CC_AI_Disable, "Bypasses all AI logic routines and puts all NPCs into their idle animations.  Can be used to get NPCs out of your way and to test effect of AI logic routines on frame rate", /*FCVAR_CHEAT*/ 0);
...
#ifdef DARKINTERVAL// DI NEW - water nodes
//------------------------------------------------------------------------------
// Purpose: Show node connections with lines
//------------------------------------------------------------------------------
void CC_AI_ShowSwimConnect(const CCommand &args)
{
	// Eventually this will be done by name when mulitple
	// networks are used, but for now have one big AINet
	//static char entName[256];	
	...
	g_pAINetworkManager->GetEditOps()->SetDebugBits("BigNet", bits_debugOverlayConnections);
	g_pAINetworkManager->GetEditOps()->SetDebugBits("BigNet", bits_debugOverlaySwimConnections);
	CBaseEntity::m_nDebugPlayer = UTIL_GetCommandClientIndex();

}
static ConCommand ai_show_connect_swim("ai_show_connect_swim", CC_AI_ShowSwimConnect, "Displays the allowed connections between each node for the currently selected hull type.  Hulls are color code as follows:\n\tGreen		- ground movement \n\tBlue		- jumping movement\n\tCyan		- flying movement\n\tMagenta	- climbing movement\n\tBlue		- swimming movement\n\tRed		- connection disabled", /*FCVAR_CHEAT*/ 0);
#endif
//------------------------------------------------------------------------------
...
static ConCommand ai_show_grid("ai_show_grid", CC_AI_ShowGrid, "Draw a grid on the floor where looking.", /*FCVAR_CHEAT*/ 0);
...
static ConCommand ai_step("ai_step", CC_AI_Step, "NPCs will freeze after completing their current task.  To complete the next task, use 'ai_step' again.  To resume processing normally use 'ai_resume'", /*FCVAR_CHEAT*/ 0);
...
static ConCommand ai_resume("ai_resume", CC_AI_Resume, "If NPC is stepping through tasks (see ai_step ) will resume normal processing.", /*FCVAR_CHEAT*/ 0);
...
static ConCommand ai_show_visibility("ai_show_visibility", CC_AI_ShowVisibility, "Toggles visibility display for the node that the player is looking at.  Nodes that are visible from the selected node will be drawn in red with yellow lines connecting to the selected node.  Nodes that are not visible from the selected node will be drawn in blue.", /*FCVAR_CHEAT*/ 0);
...
void CC_NPC_Create( const CCommand &args )
{
	...
	if (baseNPC)
	{
#ifdef DARKINTERVAL // DI NEW - special case for big hydras
		Vector forward;
		pPlayer->EyeVectors(&forward);
		
		if (baseNPC->ClassMatches("npc_hydra"))
		{
			Vector pforward;
			pPlayer->GetVectors(&pforward, NULL, NULL);
			Vector spawn = pPlayer->GetAbsOrigin() + pforward * 128;
			spawn.z += 16;
			char buf[64];
			Q_snprintf(buf, sizeof(buf), "%.1f %.1f %.1f", spawn.x, spawn.y, spawn.z);
			baseNPC->KeyValue("vecRoot", buf);
		}
#endif
		baseNPC->KeyValue( "additionalequipment", npc_create_equipment.GetString() );
		baseNPC->Precache();
#ifdef DARKINTERVAL // we rely a lot on server ragdolls
		baseNPC->AddSpawnFlags(SF_ALWAYS_SERVER_RAGDOLL);
#endif
		if ( args.ArgC() == 3 )
		{
...
#ifdef DARKINTERVAL // makes spawning scanners, manhacks easier
				tr.endpos.z += 32;
#endif
...
static ConCommand npc_create_aimed("npc_create_aimed", CC_NPC_Create_Aimed, "Creates an NPC aimed away from the player of the given type where the player is looking (if the given NPC can actually stand at that location).\n\tArguments:	{npc_class_name}", /*FCVAR_CHEAT*/ 0);
#ifdef DARKINTERVAL // opposite of "aimed"
//------------------------------------------------------------------------------
// Purpose: Create an NPC of the given type
//------------------------------------------------------------------------------
void CC_NPC_Create_Facing(const CCommand &args)
{
	MDLCACHE_CRITICAL_SECTION();
...
static ConCommand npc_create_facing("npc_create_facing", CC_NPC_Create_Facing, "Creates an NPC aimed toward the player of the given type where the player is looking (if the given NPC can actually stand at that location).\n\tArguments:	{npc_class_name}", 0);
#endif
//------------------------------------------------------------------------------
...
static ConCommand npc_destroy_unselected("npc_destroy_unselected", CC_NPC_DestroyUnselected, "Removes all NPCs from the universe that aren't currently selected", /*FCVAR_CHEAT*/ 0);
...
static ConCommand npc_squads("npc_squads", CC_NPC_Combat, "Obsolete.  Replaced by npc_combat", /*FCVAR_CHEAT*/ 0);
