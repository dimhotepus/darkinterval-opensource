...
bool CAI_Enemies::ShouldDiscardMemory( AI_EnemyInfo_t *pMemory )
{
	...
	if ( !pMemory->bUnforgettable &&
		 CURTIME > pMemory->timeLastSeen + m_flEnemyDiscardTime )
	{
		return true;
	}
...
void CAI_Enemies::RefreshMemories(void)
{
	...
	{	
		...
		{
			if ( CURTIME <= pMemory->timeLastSeen + m_flFreeKnowledgeDuration )
			{
			...
			if ( CURTIME <= pMemory->timeLastSeen )
			{
...
bool CAI_Enemies::UpdateMemory(CAI_Network* pAINet, CBaseEntity *pEnemy, const Vector &vPosition, float reactionDelay, bool firstHand )
{
	...
	if ( pMemory )
	{
		...
		if ( firstHand )
			pMemory->timeLastSeen = CURTIME;
		pMemory->bEludedMe = false;
		
		float deltaDist = (pMemory->vLastKnownLocation - vPosition).LengthSqr();
		
		if (deltaDist>DIST_TRIGGER_REACQUIRE_SQ || ( deltaDist>MIN_DIST_TIME_TRIGGER_REACQUIRE_SQ && ( CURTIME - pMemory->timeLastSeen ) > TIME_TRIGGER_REACQUIRE ) )
		{
			pMemory->timeLastReacquired = CURTIME;
		}
		...
		{
			pMemory->timeAtFirstHand = CURTIME;
		}
	...
	if ( firstHand )
	{
		pAddMemory->timeLastReacquired = pAddMemory->timeFirstSeen = pAddMemory->timeLastSeen = pAddMemory->timeAtFirstHand = CURTIME;
	}
	else
	{
		// Block free knowledge
		pAddMemory->timeLastReacquired = pAddMemory->timeFirstSeen = pAddMemory->timeLastSeen = ( CURTIME - (m_flFreeKnowledgeDuration + 0.01) );
		pAddMemory->timeAtFirstHand = AI_INVALID_TIME;
	}

	if ( reactionDelay > 0.0 )
		pAddMemory->timeValidEnemy = CURTIME + reactionDelay;
...
void CAI_Enemies::OnTookDamageFrom( CBaseEntity *pEnemy )
{
	...
	if ( pMemory )
		pMemory->timeLastReceivedDamageFrom = CURTIME;
}
...
bool CAI_Enemies::HasFreeKnowledgeOf( CBaseEntity *pEnemy )
{
	...
	if ( pMemory )
	{
		...
		return ( CURTIME < flFreeKnowledgeTime );
	}
