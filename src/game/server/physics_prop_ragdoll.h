...
public:
	...
	void InitRagdoll( const Vector &forceVector, int forceBone, const Vector &forcePos, 
		matrix3x4_t *pPrevBones, matrix3x4_t *pBoneToWorld, float dt, int collisionGroup, 
		bool activateRagdoll, bool bWakeRagdoll = true, 
#ifdef DARKINTERVAL // for gibbing corpses
		bool bBreakable = false, int iBreakableHealth = 100 
#endif
	);
...
#ifdef DARKINTERVAL // proxy for fading out fx on dying NPCs
	void		PostmortemProxyThink(void);
#endif
...
#ifdef DARKINTERVAL 
	void			InputSetUnragdollTarget(inputdata_t &inputdata);
	void			InputBeginUnragdoll(inputdata_t &inputdata);
	void			InputUnragdollToEntity(inputdata_t &inputdata);
#endif
	DECLARE_DATADESC();
...
private:
	...
	float				m_flFadeTime;
#ifdef DARKINTERVAL // for proxy
	CNetworkVar(float, m_hGoalValueForControlledVariable);
#endif
...
CBaseEntity *CreateServerRagdoll( CBaseAnimating *pAnimating, 
	int forceBone, 
	const CTakeDamageInfo &info, int collisionGroup, 
	bool bUseLRURetirement = false, Activity pDeathActivity = ACT_DIERAGDOLL, 
	bool bBreakable = false, int iBreakableHealth = 100, 
	bool bForLiveRagdoll = false );
#endif
