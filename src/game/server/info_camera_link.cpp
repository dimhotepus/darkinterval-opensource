...
CBaseEntity *CreateInfoCameraLink( CBaseEntity *pTarget, CPointCamera *pCamera )
{
	...
	if (pTarget == NULL)
	{
		Warning("Failed to create camera link - target was NULL!");
		return NULL;
	}
	if (pCamera == NULL)
	{
		Warning("Failed to create camera link - camera was NULL!");
		return NULL;
	}
#endif
	...
	{
#ifdef DARKINTERVAL
		Warning("Failed to create camera link between target %s and camera %s!\n", pTarget->GetDebugName(), pCamera->GetDebugName());
#endif
		return NULL;
	}
