...
BEGIN_SEND_TABLE_NOBASE( CPlayerLocalData, DT_Local )
	...
	SendPropInt		(SENDINFO(m_bWearingLongJump), 1, SPROP_UNSIGNED),
	SendPropBool	(SENDINFO(m_bParalyzed)),
	SendPropBool	(SENDINFO(m_bPlayingArcade) ),
	SendPropBool	(SENDINFO(m_bPlayingArcadeAsManhack)),
	SendPropBool	(SENDINFO(m_bLocatorVisible)),
	SendPropBool	(SENDINFO(m_bSuitlessHUDVisible)),
	SendPropFloat	(SENDINFO(m_WeaponProxyValueSlot01_float), 0, SPROP_NOSCALE),
	SendPropFloat	(SENDINFO(m_WeaponProxyValueSlot02_float), 0, SPROP_NOSCALE),
	SendPropFloat	(SENDINFO(m_WeaponProxyValueSlot03_float), 0, SPROP_NOSCALE),
	SendPropFloat	(SENDINFO(m_WeaponProxyValueSlot04_float), 0, SPROP_NOSCALE),
	SendPropFloat	(SENDINFO(m_WeaponProxyValueSlot05_float), 0, SPROP_NOSCALE),
	SendPropFloat	(SENDINFO(m_WeaponProxyValueSlot06_float), 0, SPROP_NOSCALE),
	SendPropFloat	(SENDINFO(m_WeaponProxyValueSlot07_float), 0, SPROP_NOSCALE),
	SendPropFloat	(SENDINFO(m_WeaponProxyValueSlot08_float), 0, SPROP_NOSCALE),
#endif
...
#ifdef DARKINTERVAL
	SendPropInt( SENDINFO_STRUCTELEM(m_skybox3d.disabled), 1, SPROP_UNSIGNED),
#endif
...
#ifdef DARKINTERVAL
	DEFINE_FIELD( disabled, FIELD_INTEGER), // whether or not the sky camera is the active one and set to draw 3d skybox
#endif
	DEFINE_EMBEDDED( fog ),
...
#ifdef DARKINTERVAL
	DEFINE_FIELD( m_bParalyzed, FIELD_BOOLEAN),
#endif
	DEFINE_FIELD( m_flStepSize, FIELD_FLOAT ),
	DEFINE_FIELD( m_bAllowAutoMovement, FIELD_BOOLEAN ),
#ifdef DARKINTERVAL
	DEFINE_FIELD( m_suppressing_weapons_bool, FIELD_BOOLEAN),
#endif
...
#ifdef DARKINTERVAL	// DI NEW
	DEFINE_FIELD(m_bWearingLongJump, FIELD_BOOLEAN),
	DEFINE_FIELD(m_bPlayingArcade, FIELD_BOOLEAN ),
	DEFINE_FIELD(m_bPlayingArcadeAsManhack, FIELD_BOOLEAN),
	DEFINE_FIELD(m_bLocatorVisible, FIELD_BOOLEAN),
	DEFINE_FIELD(m_bSuitlessHUDVisible, FIELD_BOOLEAN),
	DEFINE_FIELD(m_WeaponProxyValueSlot01_float, FIELD_FLOAT),
	DEFINE_FIELD(m_WeaponProxyValueSlot02_float, FIELD_FLOAT),
	DEFINE_FIELD(m_WeaponProxyValueSlot03_float, FIELD_FLOAT),
	DEFINE_FIELD(m_WeaponProxyValueSlot04_float, FIELD_FLOAT),
	DEFINE_FIELD(m_WeaponProxyValueSlot05_float, FIELD_FLOAT),
	DEFINE_FIELD(m_WeaponProxyValueSlot06_float, FIELD_FLOAT),
	DEFINE_FIELD(m_WeaponProxyValueSlot07_float, FIELD_FLOAT),
	DEFINE_FIELD(m_WeaponProxyValueSlot08_float, FIELD_FLOAT),
#endif
	// Why this isn't saved: entity trigger_vphysics_motion uses vPhysics to 
	//	touch the player, so if the trigger is Disabled via an input while 
	//	the player is inside it, the trigger won't get its EndTouch until 
	//	the player moves. Since touchlinks aren't saved and restored, 
	//	if the we save before EndTouch is called, it will never be called 
	//	after we load.
...
#ifdef DARKINTERVAL
	if (pSkyCamera && pSkyCamera->GetMoveParent() != NULL)
	{
		pl->m_Local.m_skybox3d.CopyFrom(pSkyCamera->m_skyboxData);
	//	DevMsg(2, "Player update: current sky camera %s with parent %s\n", pSkyCamera->GetEntityName(), pSkyCamera->GetMoveParent()->GetEntityName());
	}
#endif
