...
#ifdef DARKINTERVAL
//-----------------------------------------------------------------------------
// Purpose: the driver model entity that sits on the vehicle attachemnt and plays an idle animation
//-----------------------------------------------------------------------------
class CVehicleDriverDummy : public CBaseAnimating
{
	DECLARE_CLASS(CVehicleDriverDummy, CBaseAnimating);
	void Spawn();
};
#endif
//-----------------------------------------------------------------------------
...
#ifdef DARKINTERVAL
	void			InputGotoPathTrackViaNearestPath(inputdata_t &inputdata);
#endif
public:
...
#ifdef DARKINTERVAL
	// Our pilot model
	bool				m_customModel_bool; // whether or not to use default rollermine model or a specified one
	string_t			m_placementAttachment; // name of the attachment to lookup in the vehicle model and place ourselves on it
	string_t			m_placementSequence; // name of the sequence to play (looping) when we're sitting in the vehicle
	CVehicleDriverDummy*	m_driverDummyHandle;
#endif
};
