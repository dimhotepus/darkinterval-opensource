...
class CNPC_Hydra_Ken : public CAI_BaseNPC
{
	DECLARE_CLASS(CNPC_Hydra_Ken, CAI_BaseNPC);
	DECLARE_SERVERCLASS();
	DECLARE_DATADESC();
...
LINK_ENTITY_TO_CLASS(npc_hydra_ken, CNPC_Hydra_Ken);
...
//---------------------------------------------------------
IMPLEMENT_SERVERCLASS_ST(CNPC_Hydra_Ken, DT_NPC_Hydra_Ken)
...
void CNPC_Hydra_Ken::Precache()
{
	PrecacheModel("models/_Monsters/Xen/hydra.mdl");
	UTIL_PrecacheOther("hydra_ken_impale");
...
void CNPC_Hydra_Ken::Activate(void)
{
...
//-----------------------------------------------------------------------------
Class_T	CNPC_Hydra_Ken::Classify(void)
{
...
void CNPC_Hydra_Ken::Spawn()
{
	Precache();

	BaseClass::Spawn();

	SetModel("models/_Monsters/Xen/hydra.mdl");
...
void CNPC_Hydra_Ken::RunAI(void)
{
	CheckLength();
...
Vector CNPC_Hydra_Ken::TestPosition(float t)
{
	...
	return tmp;
#endif
	// m_vecHeadGoal = GetAbsOrigin( ) + Vector( sin( CURTIME * 0.3 ) * 15, cos( CURTIME * 0.4 ) * 150, sin( CURTIME * 0.2 ) * 50 + dt );
}
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_Hydra_Ken::CalcGoalForces()
{
	int i;
...
void CNPC_Hydra_Ken::Nudge(CBaseEntity *pOther, const Vector &vecContact, const Vector &vecSpeed)
{
...
void CNPC_Hydra_Ken::Stab(CBaseEntity *pOther, const Vector &vecSpeed, trace_t &tr)
{
...
void CNPC_Hydra_Ken::Kick(CBaseEntity *pHitEntity, const Vector &vecContact, const Vector &vecSpeed)
{
...
void CNPC_Hydra_Ken::Splash(const Vector &vecSplashPos)
{
...
void CNPC_Hydra_Ken::CheckLength()
{
	int i;
...
void CNPC_Hydra_Ken::AdjustLength()
{
	...
	else if (m_flCurrentLength > m_idealLength)
	{
		...
		else if (CURTIME - m_flLastAdjustmentTime > 1.0)
		{
			bShouldAdjust = true;
			// start to panic
			if (!GrowFromMostStretched())
	...
	if (!bAdjustFailed)
	{
		m_flLastAdjustmentTime = CURTIME;
...
bool CNPC_Hydra_Ken::ContractFromHead()
{
	if (m_body.Count() <= 2)
	{
		return false;
...
bool CNPC_Hydra_Ken::ContractBetweenStuckSegments()
{
	if (m_body.Count() <= 3)
		return false;
...
bool CNPC_Hydra_Ken::ContractFromRoot()
{
	if (m_body.Count() <= 3)
		return false;
...
int CNPC_Hydra_Ken::VirtualRoot()
{
	// first first stuck segment closest to head;
	int iStuckHead;
...
bool CNPC_Hydra_Ken::AddNodeBefore(int iNode)
{
	if (iNode < 1)
		return false;
...
bool CNPC_Hydra_Ken::AddNodeAfter(int iNode)
{
...
bool CNPC_Hydra_Ken::GrowFromVirtualRoot()
{
...
bool CNPC_Hydra_Ken::GrowFromMostStretched()
{
...
void CNPC_Hydra_Ken::CalcRelaxedLength()
{
	m_flRelaxedLength = m_idealSegmentLength * (m_body.Count() - 2) + HYDRA_OUTWARD_BIAS;
}


bool CNPC_Hydra_Ken::IsValidConnection(int iNode0, int iNode1)
{
	trace_t tr;
...
float CNPC_Hydra_Ken::MaxYawSpeed()
{
	return 0;
...
int CNPC_Hydra_Ken::TranslateSchedule(int scheduleType)
{
...
void CNPC_Hydra_Ken::HandleAnimEvent(animevent_t *pEvent)
{
...
void CNPC_Hydra_Ken::PrescheduleThink()
{
...
int CNPC_Hydra_Ken::SelectSchedule()
{
	switch (m_NPCState)
	{
	case NPC_STATE_IDLE:
...
void CNPC_Hydra_Ken::StartTask(const Task_t *pTask)
{
	switch (pTask->iTask)
	{
	...
	{
		m_flTaskEndTime = CURTIME + pTask->flTaskData;
...
void CNPC_Hydra_Ken::RunTask(const Task_t *pTask)
{
	switch (pTask->iTask)
	{
	case TASK_HYDRA_DEPLOY:
	...
	case TASK_HYDRA_PREP_STAB:
	{
		...
		float influence = 1.0 - (m_flTaskEndTime - CURTIME) / pTask->flTaskData;
		if (influence > 1)
			influence = 1.0;
		...
		for (i = i - 1; i > 1 && distBackFromHead < distToTarget; i--)
		{
			...
			{
			if ( CURTIME - m_last_attack_time > 4.0)
			{
			TaskComplete();
			}
			return;
...
Vector CNPC_Hydra_Ken::EyePosition()
{
...
const QAngle &CNPC_Hydra_Ken::EyeAngles()
{
	return GetAbsAngles();
}


Vector CNPC_Hydra_Ken::BodyTarget(const Vector &posSrc, bool bNoisy)
{
	int i;
...
void CNPC_Hydra_Ken::AimHeadInTravelDirection(float flInfluence)
{
...
//-----------------------------------------------------------------------------
class CHydraKenImpale : public CBaseAnimating
{
	DECLARE_CLASS(CHydraKenImpale, CBaseAnimating);
public:
	DECLARE_DATADESC();
	...
	IPhysicsConstraint *CreateConstraint(CNPC_Hydra_Ken *pHydra, IPhysicsObject *pTargetPhys, IPhysicsConstraintGroup *pGroup);
	static CHydraKenImpale *Create(CNPC_Hydra_Ken *pHydra, CBaseEntity *pObject2);

public:
	IPhysicsConstraint		*m_pConstraint;
	CHandle<CNPC_Hydra_Ken>		m_hHydra;
};

BEGIN_DATADESC(CHydraKenImpale)
...
LINK_ENTITY_TO_CLASS(hydra_ken_impale, CHydraKenImpale);

//-----------------------------------------------------------------------------
// Purpose: To by usable by the constraint system, this needs to have a phys model.
//-----------------------------------------------------------------------------
void CHydraKenImpale::Spawn(void)
{
	Precache();
...
// Purpose: 
//-----------------------------------------------------------------------------
void CHydraKenImpale::Precache(void)
{
...
//-----------------------------------------------------------------------------
void CHydraKenImpale::ImpaleThink(void)
{
	if (!m_hHydra)
	{
		// Remove ourselves.
...
//-----------------------------------------------------------------------------
IPhysicsConstraint *CHydraKenImpale::CreateConstraint(CNPC_Hydra_Ken *pHydra, IPhysicsObject *pTargetPhys, IPhysicsConstraintGroup *pGroup)
{
	m_hHydra = pHydra;
	...
	SetThink(&CHydraKenImpale::ImpaleThink);
...
//-----------------------------------------------------------------------------
CHydraKenImpale *CHydraKenImpale::Create(CNPC_Hydra_Ken *pHydra, CBaseEntity *pTarget)
{
	Vector vecOrigin;
	QAngle vecAngles;
	...
	CHydraKenImpale *pImpale = (CHydraKenImpale *)CBaseEntity::Create("hydra_ken_impale", vecOrigin, vecAngles);
	if (!pImpale)
		return NULL;
	...
	{
		DevMsg(" Error: Tried to hydra_ken_impale an entity without a physics model.\n");
		return NULL;
	}
...
void CNPC_Hydra_Ken::AttachStabbedEntity(CBaseAnimating *pAnimating, Vector vecForce, trace_t &tr)
{
	...
	CHydraKenImpale::Create(this, pRagdoll);
...
void CNPC_Hydra_Ken::UpdateStabbedEntity(void)
{
	/*
...
void CNPC_Hydra_Ken::DetachStabbedEntity(bool playSound)
{
	/*
...
void CNPC_Hydra_Ken::GetDesiredImpaledPosition(Vector *vecOrigin, QAngle *vecAngles)
{
...
//-----------------------------------------------------------------------------
//
// CNPC_Hydra_Ken Schedules
//
//-------------------------------------


AI_BEGIN_CUSTOM_NPC(npc_hydra_ken, CNPC_Hydra_Ken)
