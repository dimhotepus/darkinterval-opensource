//========= Dark Interval, 2021. ==============================================//
//
// Purpose: The recon trooper - fast medium range Combine scout
//
//=============================================================================//
...
class CNPC_CombineRecon : public CNPC_CombineS
{
	DECLARE_CLASS(CNPC_CombineRecon, CNPC_CombineS);
	DECLARE_DATADESC();
public:
	CNPC_CombineRecon();
	~CNPC_CombineRecon();
...
//	enum
//	{
//		SCHED_COMBINE_RECON_ESTABLISH_LINE_OF_FIRE = 122,
//		SCHED_COMBINE_RECON_SNIPER_FIRE,
//		NEXT_SCHEDULE,
//	};
};
...
CNPC_CombineRecon::CNPC_CombineRecon()
{
	m_hNPCFileInfo = LookupNPCInfoScript("npc_combine_recon");
}
CNPC_CombineRecon::~CNPC_CombineRecon()
{
}

void CNPC_CombineRecon::Precache()
{
...
void CNPC_CombineRecon::Spawn(void)
{
	Precache();
...
void CNPC_CombineRecon::RunAI(void)
{
	BaseClass::RunAI();
}

void CNPC_CombineRecon::DeathSound(const CTakeDamageInfo &info)
{
...
void CNPC_CombineRecon::PrescheduleThink(void)
{
	BaseClass::PrescheduleThink();
}

void CNPC_CombineRecon::BuildScheduleTestBits(void)
{
...
int CNPC_CombineRecon::SelectSchedule(void)
{
	return BaseClass::SelectSchedule();
}

void CNPC_CombineRecon::StartTask(const Task_t *pTask)
{
	switch (pTask->iTask)
	{
...
void CNPC_CombineRecon::RunTask(const Task_t *pTask)
{
	switch (pTask->iTask)
	{
	case TASK_RANGE_ATTACK1:
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
float CNPC_CombineRecon::GetHitgroupDamageMultiplier(int iHitGroup, const CTakeDamageInfo &info)
{
	switch (iHitGroup)
	{
	case HITGROUP_HEAD:
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_CombineRecon::HandleAnimEvent(animevent_t *pEvent)
{
	switch (pEvent->event)
	{
...
void CNPC_CombineRecon::OnChangeActivity(Activity eNewActivity)
{
...
void CNPC_CombineRecon::OnListened()
{
	BaseClass::OnListened();
}

void CNPC_CombineRecon::Event_Killed(const CTakeDamageInfo &info)
{
	BaseClass::Event_Killed(info);
}

bool CNPC_CombineRecon::IsLightDamage(const CTakeDamageInfo &info)
{
	return BaseClass::IsLightDamage(info);
}

bool CNPC_CombineRecon::IsHeavyDamage(const CTakeDamageInfo &info)
{
...
/*
int CNPC_CombineRecon::TranslateSchedule(int scheduleType)
{
switch (scheduleType)
{
...
Activity CNPC_CombineRecon::NPC_TranslateActivity(Activity eNewActivity)
{
...
BEGIN_DATADESC(CNPC_CombineRecon)
DEFINE_KEYFIELD(m_iUseMarch, FIELD_INTEGER, "usemarch"),
END_DATADESC()

LINK_ENTITY_TO_CLASS(npc_combine_recon, CNPC_CombineRecon);
PRECACHE_REGISTER(npc_combine_recon);