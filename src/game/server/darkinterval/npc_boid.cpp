...
#define		AFLOCK_TOO_CLOSE			256
#define		AFLOCK_TOO_FAR				1024

#define		AFLOCK_MODEL				"models/_Monsters/Xen/Xen_Flock_A.mdl"
#define		BOID_MODEL					"models/_Monsters/Xen/Xen_Flock_B.mdl"
//=========================================================
//=========================================================
class CNPC_FlockingFlyerFlock : public CAI_BaseNPC
{
	DECLARE_CLASS(CNPC_FlockingFlyerFlock, CAI_BaseNPC);
public:
...
class CNPC_FlockingFlyer : public CAI_BaseNPC
{
	DECLARE_CLASS(CNPC_FlockingFlyer, CAI_BaseNPC);
public:
	void Spawn(void);
	void Precache(void);
...
LINK_ENTITY_TO_CLASS(npc_boid, CNPC_FlockingFlyer);
LINK_ENTITY_TO_CLASS(npc_boid_flock, CNPC_FlockingFlyerFlock);
...
{	
	PrecacheModel(AFLOCK_MODEL);
	PrecacheModel(BOID_MODEL);
...
void CNPC_FlockingFlyerFlock::SpawnFlock(void)
{
	...
	{
		pBoid = CREATE_ENTITY(CNPC_FlockingFlyer, "npc_boid");
		...
		pBoid->SetNextThink(CURTIME + 0.05);
...
void CNPC_FlockingFlyer::SpawnCommonCode()
{
	...
	if (RandomInt(0, 2) == 0)
		...
		SetModel(AFLOCK_MODEL);

	UTIL_SetSize(this, Vector(-5, -5, 0), Vector(5, 5, 2));

	SetModelScale(RandomFloat(0.95, 1.15), 0.1f);
}
...
//	if (!FNullEnt(UTIL_FindClientInPVS(edict())) || DetectInSkybox())
	{
...
void CNPC_FlockingFlyer::FlockLeaderThink(void)
{
	...
	UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() - vUp * 16, MASK_SOLID, this, COLLISION_GROUP_NONE, &tr);
	...
	if (m_flFlockNextSoundTime < CURTIME)
	{
		//		MakeSound();
		m_flFlockNextSoundTime = CURTIME + random->RandomFloat(1, 3);
	}
...
bool CNPC_FlockingFlyer::FPathBlocked(void)
{
	...
	if (m_flFakeBlockedTime > CURTIME)
	{
		m_flLastBlockedTime = CURTIME;
		return TRUE;
	}
	...
	UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() + vForward * AFLOCK_CHECK_DIST, MASK_NPCSOLID, this, COLLISION_GROUP_NONE, &tr);
	...
	// extra wide checks
	UTIL_TraceLine(GetAbsOrigin() + vRight * 12, GetAbsOrigin() + vRight * 12 + vForward * AFLOCK_CHECK_DIST, MASK_NPCSOLID, this, COLLISION_GROUP_NONE, &tr);
	...
	if (!fBlocked && CURTIME - m_flLastBlockedTime > 6)
	{
		// not blocked, and it's been a few seconds since we've actually been blocked.
		m_flFakeBlockedTime = CURTIME + random->RandomInt(1, 3);
	}
...
void CNPC_FlockingFlyer::Event_Killed(const CTakeDamageInfo &info)
{
	...
	while (pSquad)
	{
		pSquad->m_flAlertTime = CURTIME + 15;
