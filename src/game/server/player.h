...
enum PlayerPhysFlag_e
{
	...
	// overwriting phys flags in the HL2 classes
};
...
public:
...
#ifdef DARKINTERVAL
	virtual void			Spawn( bool placeOnPlayerStart = true ); // for reviving player // todo - unfinished
#else
...
#ifdef DARKINTERVAL
	float					fPreframeSpeed;
	float					fPostframeSpeed;
#endif
...
#ifdef DARKINTERVAL
	virtual Vector			Weapon_TrueShootPosition_Front( );
	virtual Vector			Weapon_TrueShootPosition_Back( );
#endif
...
#ifdef DARKINTERVAL
	virtual int				ToggleSprintIsOn(void) { return false; }
	virtual void			ToggleSprintTurnOn(void) { };
	virtual void			ToggleSprintTurnOff(void) { };
#endif
	void					UpdatePlayerSound ( void );
	virtual void			UpdateStepSound( surfacedata_t *psurface, const Vector &vecOrigin, const Vector &vecVelocity );
#ifdef DARKINTERVAL
	virtual void			PlayJumpSound(bool bLongJump = false);
#endif
...
#ifdef DARKINTERVAL
	// DI footprints test 
	virtual void PlacePlayerFootprint();
	bool m_IsFootprintOnLeft = true;
#endif
	...
	float					GetTimeSinceLastUserCommand( void ) { return ( !IsConnected() || IsFakeClient() || IsBot() ) ? 0.f : CURTIME - m_flLastUserCommandTime; }
#ifndef DARKINTERVAL // DI is singleplayer, so it ditched teams
	// Team Handling
...
public:
	...
	virtual void EquipLongJump(bool bPlayEffects = true);
	virtual void RemoveLongJump(void);
	bool	IsLongJumpEquipped() const { return m_Local.m_bWearingLongJump; }
	bool	IsPlayingArcade() const { return m_Local.m_bPlayingArcade; }
	bool	IsPlayingArcadeAsManhack() const { return m_Local.m_bPlayingArcadeAsManhack; }
	float	WeaponProxyValueSlot01() const { return m_Local.m_WeaponProxyValueSlot01_float; }
	float	WeaponProxyValueSlot02() const { return m_Local.m_WeaponProxyValueSlot02_float; }
	float	WeaponProxyValueSlot03() const { return m_Local.m_WeaponProxyValueSlot03_float; }
	float	WeaponProxyValueSlot04() const { return m_Local.m_WeaponProxyValueSlot04_float; }
	float	WeaponProxyValueSlot05() const { return m_Local.m_WeaponProxyValueSlot05_float; }
	float	WeaponProxyValueSlot06() const { return m_Local.m_WeaponProxyValueSlot06_float; }
	float	WeaponProxyValueSlot07() const { return m_Local.m_WeaponProxyValueSlot07_float; }
	float	WeaponProxyValueSlot08() const { return m_Local.m_WeaponProxyValueSlot08_float; }
#endif
	...
	// talk control
	void	NotePlayerTalked() { m_fLastPlayerTalkTime = CURTIME; }
...
#ifdef DARKINTERVAL
	int						m_nParalysisLasting;	// track recoverable paralysis damage taken
	int						m_nParalysisGone;		// track paralysis damage restored
#endif
...
#ifdef DARKINTERVAL
	int						m_iClientVehicleIntegrity;	// the vehicle (APC, Digger) state currently known by the client.  If this changes, send a new
#endif
	// Autoaim data
	QAngle					m_vecAutoAim;
...
#ifdef DARKINTERVAL
	// vehicle status
	CNetworkVarForDerived(int, m_iVehicleIntegrity);
#endif
protected:
