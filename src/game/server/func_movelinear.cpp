...
#ifdef DARKINTERVAL // unfinished prototype for movable expensive water
//IMPLEMENT_SERVERCLASS_ST(CFuncMoveLinear, DT_FuncWaterAnalog)
//END_SEND_TABLE()
#endif
...
#ifdef DARKINTERVAL	// VDC parenting fix
	DEFINE_FIELD( m_vecParentReference, FIELD_VECTOR),
#endif
...
void CFuncMoveLinear::Spawn( void )
{
	...
	// VDC parenting fix
	m_vecParentReference = m_vecFinalDest;
#else
...
void CFuncMoveLinear::SetParent(CBaseEntity *pParentEntity, int iAttachment)
{
	...
	{
		m_vecParentReference = ((m_vecParentReference - oldLocal) + GetLocalOrigin());
		m_vecPosition1 = m_vecParentReference - (m_vecMoveDir * m_flMoveDistance * m_flStartPosition);
		m_vecPosition2 = m_vecPosition1 + (m_vecMoveDir * m_flMoveDistance);
		m_vecFinalDest = m_vecParentReference - m_vecFinalDest;
	}
}
#endif
//------------------------------------------------------------------------------
