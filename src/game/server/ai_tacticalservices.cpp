...
#ifdef DARKINTERVAL
ConVar ai_debug_tactical_nodes("ai_debug_tactical_nodes", "0");
#define ShouldDebugLos( node ) ai_debug_tactical_nodes.GetBool()
#else
...
bool CAI_TacticalServices::FindLateralLos( const Vector &vecThreat, Vector *pResult )
{
	...
	if(  !bLookingForEnemy || GetOuter()->HasCondition(COND_SEE_ENEMY) || GetOuter()->HasCondition(COND_HAVE_ENEMY_LOS) || 
		 GetOuter()->GetTimeScheduleStarted() == CURTIME ) // Conditions get nuked before tasks run, assume should try
	{
