...
int CAI_Hint::DrawDebugTextOverlays(void) 
{
	...
	if (m_debugOverlays & OVERLAY_TEXT_BIT) 
	{
		...
		text_offset++;
		Q_snprintf(tempstr,sizeof(tempstr),"delay %f", MAX( 0.0f, m_flNextUseTime - CURTIME ) ) ;
		EntityText(text_offset,tempstr,0);
		text_offset++;
...
#ifdef DARKINTERVAL // todo - is this used? it is commented out in the original code
	{	HINT_HEADCRAB_BURROW_POINT, "Headcrabs use it to hide after crawling out of the cannister"	},
#endif
