...
void CAI_FearBehavior::StartTask( const Task_t *pTask )
{
	...
	case TASK_FEAR_WAIT_FOR_SAFETY:
		m_flTimeToSafety = CURTIME + BEHAVIOR_FEAR_SAFETY_TIME;
		break;
			...
			case 0:// Find the hint node
				{
					...
					if( pHint == NULL )
					{
						...
						m_flDeferUntil = CURTIME + 3.0f;// Don't bang the hell out of this behavior. If we don't find a node, take a short break and run regular AI.
					}
					else
					{
...
void CAI_FearBehavior::GatherConditions()
{
	...
	{
		m_flTimePlayerLastVisible = CURTIME;
	}
	else
	{
		if( CURTIME - m_flTimePlayerLastVisible >= 2.0f )
		{
			SetCondition(COND_FEAR_SEPARATED_FROM_PLAYER);
		}
	}
