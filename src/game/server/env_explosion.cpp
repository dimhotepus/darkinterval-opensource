...
#include "decals.h"
#include "ieffects.h"
#ifdef DARKINTERVAL // shake ropes in the radius
#include "rope.h"
#endif
...
#ifdef DARKINTERVAL // shake ropes in the radius
ConVar g_explosion_shake_ropes("g_explosion_shake_ropes", "1");
#endif
...
{ 
	...
	SetNextThink( CURTIME + 0.3 );

	// Only do these effects if we're not submerged
#ifdef DARKINTERVAL // the original code had it accidentally swapped around
...
#ifdef DARKINTERVAL // shake ropes in the radius // DI todo - finalise
	if ( HasSpawnFlags(SF_ENVEXPLOSION_SHAKE_ROPES) )
	{
		if ( g_explosion_shake_ropes.GetBool() )
		{
			CRopeKeyframe::ShakeRopes(vecExplodeOrigin, m_iMagnitude * 3.0f, m_iMagnitude * 1.0f);
		}
	}
#endif
}
...
int CEnvExplosion::DrawDebugTextOverlays( void ) 
{
	...
	if (m_debugOverlays & OVERLAY_TEXT_BIT) 
	{
		...
		if (m_hEntityIgnore)
		{
			Q_snprintf(tempstr, sizeof(tempstr), "    limited to: %i", m_hEntityIgnore->GetDebugName());
			EntityText(text_offset, tempstr, 0);
			text_offset++;
		}
#endif
