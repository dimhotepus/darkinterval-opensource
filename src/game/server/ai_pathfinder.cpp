...
Navigation_t MoveBitsToNavType( int fBits )
{
	...
	case bits_CAP_MOVE_JUMP:
		return NAV_JUMP;
#ifdef DARKINTERVAL
	case bits_CAP_MOVE_SWIM: // DI NEW - water nodes
		return NAV_SWIM;
#endif
	default:
	// This will only happen if more than one bit is set
#ifdef DARKINTERVAL	// UNDONE: Antlions and others use both ground and jump, for example.
		return NAV_GROUND;
#else
		Assert(0);
		return NAV_NONE;
...
bool CAI_Pathfinder::IsLinkStillStale(int moveType, CAI_Link *nodeLink)
{
	...
	if ( CURTIME < nodeLink->m_timeStaleExpires )
		return true;

	// NPC should only check one stale link per think
	if (CURTIME == m_flLastStaleLinkCheckTime)
	{
		return true;
	}
	else
	{
		m_flLastStaleLinkCheckTime = CURTIME;
	}
	...
	nodeLink->m_timeStaleExpires = CURTIME + 1.0;
...
#ifdef DARKINTERVAL
	else if (pNPC->CapabilitiesGet() & bits_CAP_MOVE_SWIM) // DI NEW - water nodes
	{
		return bits_BUILD_SWIM | bits_BUILD_GIVEWAY;
	}
#endif
	return 0;
}
...
//-----------------------------------------------------------------------------
// Purpose: Attempts to build a swim route between vStart
//			and vEnd, ignoring entity pTarget the the given tolerance
// Input  :
// Output : Returns a route if sucessful or NULL if no ground route was possible
//-----------------------------------------------------------------------------
#ifdef DARKINTERVAL // DI NEW - water nodes
AI_Waypoint_t *CAI_Pathfinder::BuildSwimRoute(const Vector &vStart, const Vector &vEnd,
	const CBaseEntity *pTarget, int endFlags, int nodeID, int buildFlags, float flYaw, float goalTolerance)
{
	return BuildComplexRoute(NAV_SWIM, vStart, vEnd, pTarget,
		endFlags, nodeID, buildFlags, flYaw, goalTolerance, MAX_LOCAL_NAV_DIST_FLY[UseStrongOptimizations()]);
}
#endif
//-----------------------------------------------------------------------------
...
#ifdef DARKINTERVAL	// DI NEW - water nodes
	if (buildFlags & bits_BUILD_SWIM)
	{
		AI_Waypoint_t *swimRoute = BuildSwimRoute(vStart, vEnd, pTarget, endFlags, nodeID, buildFlags, flYaw, goalTolerance);
...
#ifndef DARKINTERVAL
		|| (CapabilitiesGet() & bits_CAP_MOVE_SWIM)
#endif
		) // DI NEW - water nodes
	{
...
#ifdef DARKINTERVAL
	else if (CapabilitiesGet() & bits_CAP_MOVE_SWIM) // DI NEW - water nodes
	{
		buildFlags = (bits_BUILD_SWIM | bits_BUILD_GIVEWAY | bits_BUILD_TRIANG);
	}
#endif
...
#ifdef DARKINTERVAL	// DI NEW - water nodes
	if (moveTypes & bits_CAP_MOVE_SWIM)
	{
		if (CheckStaleNavTypeRoute(NAV_SWIM, vStart, vEnd))
			return true;
	}
#endif
...
#ifdef DARKINTERVAL
		| bits_BUILD_SWIM ) // DI NEW - water nodes
#endif
		),
		m_pRoute( NULL )
	{
...
#ifdef DARKINTERVAL
		COMPILE_TIME_ASSERT( bits_BUILD_GROUND == (int)bits_CAP_MOVE_GROUND 
			&& bits_BUILD_FLY == (int)bits_CAP_MOVE_FLY 
			&& bits_BUILD_JUMP == (int)bits_CAP_MOVE_JUMP 
			&& bits_BUILD_CLIMB == (int)bits_CAP_MOVE_CLIMB 
			/*&& bits_BUILD_SWIM == (int)bits_CAP_MOVE_SWIM*/ // DI NEW - water nodes
		);
#else
		COMPILE_TIME_ASSERT(bits_BUILD_GROUND == ( int )bits_CAP_MOVE_GROUND
			&& bits_BUILD_FLY == ( int )bits_CAP_MOVE_FLY
