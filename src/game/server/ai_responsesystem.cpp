...
#ifdef DARKINTERVAL
		if (!Q_stricmp(token, "showradiohud"))
		{
			ParseToken();
			rp->flags |= AI_ResponseParams::RG_SHOWRADUIOHUD;
			rp->showradiohud.FromInterval(ReadInterval(token));
			continue;
		}
#endif
...
public:
	...
	virtual const char *GetScriptFile( void ) 
	{
#ifdef DARKINTERVAL // DI moves them to a different folder so there's no chance of cross-interference from HL2/Episodes
		return "scripts/responsesystem/response_rules.txt";
#else
