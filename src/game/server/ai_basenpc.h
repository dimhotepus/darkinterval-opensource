...
#ifdef DARKINTERVAL
#define SF_ALWAYS_SERVER_RAGDOLL		( 1 << 15 )	// Ragdoll stuff. DI add
#endif
...
#ifdef DARKINTERVAL
	// Optional distance optimisation - determined by individual classes
	inline bool			ThisNPCInPVS(void)
	{
		return (UTIL_FindClientInPVS(edict()) != NULL) || (UTIL_ClientPVSIsExpanded() && UTIL_FindClientInVisibilityPVS(edict()));
	}
	void				SetNextThinkByDistance(float minRndTime = 1.0f, float maxRndTime = 3.0f, float A = 400, float B = 5000, float C = 1.0f, float D = 5.0f );
#endif
...
private:
	...
	CHandle<CRagdollProp> m_live_ragdoll_handle; // the fake double that we create to pose as our live ragdoll
	float m_flUnragdollTime; // this timer will tell the NPC that it's time to blend the ragdoll into the model and stand up;
	float m_flStartRagdollTime; // this timer keeps track of when we first entered ragdoll;
	int m_rememberSolidFlags;
	int m_rememberMoveType;

	// prototyping barnacle rescue
public:
	// Base class doesn't allow it. 
	// Override with true for anyone that should be rescuable 
	// (they'll need to handle g_interactionBarnacleVictimReleased, too!)
	virtual bool CanBeRescuedFromBarnacle(void) { return false; }
public:
	float m_flExtraRagdollTime; // this float is added to duration of lying on the ground, so we can control how long we want them to be ragdoll (-1 for indefinite)

	void InputFallInRagdoll(inputdata_t &inputdata); // by default the extra time is 5 seconds when called via this input.
	void InputFallInRagdollForSeconds(inputdata_t &inputdata); // this input needs float value. -1 for indefinite.
	void InputExitRagdoll(inputdata_t &inputdata); // this resets that extra time to 0.
	void InputEnableLiveRagdoll(inputdata_t &inputdata); // this adds bits_CAP_LIVE_RAGDOLL
	void InputDisableLiveRagdoll(inputdata_t &inputdata); // this removes bits_CAP_LIVE_RAGDOLL
	COutputEHANDLE m_OnFallInRagdoll;
	COutputEvent m_OnExitRagdoll;
		
	inline void CAI_BaseNPC::RestoreNPCAfterRagdoll(void)
	{
...
#ifdef DARKINTERVAL
	Activity			GetCurrentActivity(void) { return GetActivity(); }
#endif
...
#ifdef DARKINTERVAL // we want some critters to be immune to player flares. Go with default off, and specify otherwise
	virtual	bool		AllowedToIgniteByFlare(void) { return false; }
#endif
protected:
...
#ifdef DARKINTERVAL
	virtual void		PainSound(const CTakeDamageInfo &info) { m_flNextPainSoundTime = CURTIME + 1.0f; return; };
#else
...
#ifdef DARKINTERVAL
	void				InputCapabilitiesAdd(inputdata_t &inputdata);
	void				InputCapabilitiesRemove(inputdata_t &inputdata);
	void				InputSetTarget(inputdata_t &inputdata);
	void				InputClearTarget(inputdata_t &inputdata);
#endif
private:
...
public:
	...
	void				DelayMoveStart( float delay )	{ m_flMoveWaitFinished = CURTIME + delay; }
...
#ifdef DARKINTERVAL
	int SwimMove(const Vector& vecPosition, unsigned int mask); // DI NEW - water nodes
#endif
...
#ifdef DARKINTERVAL
	void InputSetStateAlert(inputdata_t &inputdata) { SetState(NPC_STATE_ALERT); }
	void InputSetStateCombat(inputdata_t &inputdata) { SetState(NPC_STATE_COMBAT); }
	void InputSetStateIdle(inputdata_t &inputdata) { SetState(NPC_STATE_IDLE); }
#endif
...
public:
	...
	void				AddSceneLock( float flDuration = 0.2f ) { m_flSceneTime = MAX( CURTIME + flDuration, m_flSceneTime ); };
	void				ClearSceneLock( float flDuration = 0.2f ) { m_flSceneTime = CURTIME + flDuration; };
	bool				IsInLockedScene( void ) { return m_flSceneTime > CURTIME; };
	float				m_flSceneTime;
...
public:
	...
	void				SetLastAttackTime( float time)	{ m_last_attack_time = time; }

	float				GetLastAttackTime() const { return m_last_attack_time; }
...
protected:
	...
	float				m_last_attack_time;			// Last time that I attacked my current enemy
...
#ifdef DARKINTERVAL
	virtual bool		FindPhysicsCoverInRadius(CBaseEntity *pEnemy, const Vector &goalPos, Vector *pResult/*, CBaseEntity *pTarget*/);
#endif
...
#ifdef DARKINTERVAL
	virtual float		IgniteDamageThreshold(void) { return m_ignite_damage_threshold_float; };
private: 
	float				m_ignite_damage_threshold_float = 20.0f;
public:
#endif
	...
	void InputSetHintGroupLimitNav(inputdata_t &inputdata);
#endif
	//---------------------------------
...
public:
	...
	COutputEHANDLE		m_OnFoundEnemyLOSAndViewCone;
	COutputEHANDLE		m_OnFoundPlayerLOSAndViewCone;
#endif
...
#ifdef DARKINTERVAL // ported freezing code from ASW, see also physics prop statue
...
public:
	...
	void				StartPingEffect( void ) { m_flTimePingEffect = CURTIME + 2.0f; DispatchUpdateTransmitState(); }
#ifdef DARKINTERVAL
	// DARK INTERVAL NPC PARSING TEST
	const				FileNPCInfo_t	&GetNPCScriptData(void) const;
