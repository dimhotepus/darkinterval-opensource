...
{ 
	...
	AssertMsg(0, "Invalid call to CurWaypointPos() at %.1f %.1f %.1f", m_goalPos.x, m_goalPos.y, m_goalPos.z);
...
{ 
	...
	AssertMsg(0, "Invalid call to NextWaypointPos() at %.1f %.1f %.1f", m_goalPos.x, m_goalPos.y, m_goalPos.z);
	return gm_InvalidWaypoint.GetPos();
}
