...
void CPointDevShotCamera::Spawn( void )
{
	...
	SetNextThink( CURTIME + DEVSHOT_INITIAL_WAIT + (g_iDevShotCameraCount * DEVSHOT_INTERVAL) );
...
#ifdef DARKINTERVAL // DI doesn't care about cheats
...
void CPointDevShotCamera::DevShotThink_TakeShot( void )
{
	...
	SetNextThink( CURTIME + (DEVSHOT_INTERVAL - 1) );
}
...
#ifdef DARKINTERVAL
			pkvMapCameras->deleteThis(); // memory leak fix from vdc
#endif
			if ( !g_iDevShotCameraCount )
			{
