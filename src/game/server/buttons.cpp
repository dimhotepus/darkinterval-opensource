...
#ifdef DARKINTERVAL // fix for non precached and inaudible sparking buttons
...
{ 
	...
	if ( HasSpawnFlags( SF_BUTTON_SPARK_IF_OFF ) )// this button should spark in OFF state
	{
		...
		SetNextThink( CURTIME + 0.5f );// no hurry, make sure everything else spawns
	}
...
{
	SetThink ( &CBaseButton::ButtonSpark );
	SetNextThink( CURTIME + 0.1 + random->RandomFloat ( 0, 1.5 ) );// spark again at random interval
...
bool CBaseButton::OnUseLocked( CBaseEntity *pActivator )
{
	...
	if ( CURTIME > m_flUseLockedTime )
	{
		m_OnUseLocked.FireOutput( pActivator, this );
		m_flUseLockedTime = CURTIME + 0.5;
		return true;
	}
...
void CBaseButton::ButtonBackHome( void )
{
	...
	{
		SetThink ( &CBaseButton::ButtonSpark );
		SetNextThink( CURTIME + 0.5f );// no hurry
	}
}
...
#ifdef DARKINTERVAL
	DEFINE_FIELD( m_rotationchild_entity, FIELD_EHANDLE),
#endif
	// Function Pointers
...
#ifdef DARKINTERVAL
	DEFINE_INPUTFUNC( FIELD_VOID, "DisableAutoReturn", InputDisableAutoReturn),
	DEFINE_INPUTFUNC( FIELD_VOID, "EnableAutoReturn", InputEnableAutoReturn),
#endif
	// Outputs
...
void CMomentaryRotButton::Spawn( void )
{
	...
	m_bDisabled = false;
#ifdef DARKINTERVAL
	m_rotationchild_entity = GetNextTarget();
#endif
}
...
#ifdef DARKINTERVAL
//------------------------------------------------------------------------------
// Purpose: Clears auto return spawnflag.
//------------------------------------------------------------------------------
void CMomentaryRotButton::InputDisableAutoReturn(inputdata_t &inputdata)
{
	if( !HasSpawnFlags(SF_BUTTON_TOGGLE))
		AddSpawnFlags(SF_BUTTON_TOGGLE);
}
//------------------------------------------------------------------------------
// Purpose: Adds auto return spawnflag.
//------------------------------------------------------------------------------
void CMomentaryRotButton::InputEnableAutoReturn(inputdata_t &inputdata)
{
	if (HasSpawnFlags(SF_BUTTON_TOGGLE))
		RemoveSpawnFlags(SF_BUTTON_TOGGLE);
}
#endif
//-----------------------------------------------------------------------------
// Purpose: Locks the button. If locked, the button will play the locked sound
...
void CMomentaryRotButton::Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	if ( !m_lastUsed )
	{
		...
		m_OnPressed.FireOutput( m_hActivator, this );
#ifdef DARKINTERVAL
		UpdateTarget(GetPos(GetLocalAngles()), this);
#endif
	}
...
void CMomentaryRotButton::ReturnMoveDone( void )
{
	...
	{
		//
		// Got back to the start or got locked in place, stop spinning.
		//
		SetLocalAngularVelocity( vec3_angle );
		SetLocalAngles( m_start );
...
#ifdef DARKINTERVAL
	if (m_rotationchild_entity != NULL)
	{
		m_rotationchild_entity->SetLocalAngles(GetLocalAngles());
	}
#endif
	SetNextThink( CURTIME );
}
