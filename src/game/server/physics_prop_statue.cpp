...
void CStatueProp::EnableAutoShatter(const CTakeDamageInfo &dmgInfo)
{
	m_ShatterDamageInfo = dmgInfo;

	SetThink(&CStatueProp::AutoShatterThink);
	SetNextThink(CURTIME + 2.0f);
}

void CStatueProp::AutoShatterThink()
{
...
void CStatueProp::BreakThink(void)
{
...
bool CStatueProp::CreateVPhysicsFromHitBoxes( CBaseAnimating *pInitBaseAnimating )
{
	...
	color32 colorRender = pInitBaseAnimating->GetRenderColor();
	SetRenderColor( colorRender.r, colorRender.g, colorRender.b );
	SetRenderColorA(colorRender.a);
	...
	int nMaterialIndex = physprops->GetSurfaceIndex( "metal" );	// use ice material
...
CBaseEntity *CreateServerStatue( CBaseAnimating *pAnimating, int collisionGroup, const CTakeDamageInfo &dmgInfo, bool bAutoShatter)
{
...
CBaseEntity *CreateServerStatueFromOBBs( const CUtlVector<outer_collision_obb_t> &vecSphereOrigins, CBaseAnimating *pAnimating )
{
	...
	if ( pStatue )
	{
		...
		pStatue->SetNextThink( CURTIME + 1.0f );
	}
