...
#ifdef DARKINTERVAL
void CBoneFollowerManager::UpdateBoneFollowers(CBaseAnimating *pParentEntity, Vector &replaceBonePos, QAngle &replaceBoneAng, float updateinterval)
{
	if (m_iNumBones)
	{
		matrix3x4_t boneToWorld;
		...
		for (int i = 0; i < m_iNumBones; i++)
		{
			...
			m_physBones[i].hFollower->UpdateFollower(replaceBonePos, replaceBoneAng, updateinterval);
		}
	}
}
#endif
