...
#ifdef DARKINTERVAL	// DI NEW: Does this vehicle allow to pick up items by touching them? 
	// (airboat in HL2 does, but APC in DI does not - have to get out and grab them)
	virtual bool			VehicleCanPickupItems(void) { return true; }
#endif
...
#ifdef DARKINTERVAL
	virtual void			NPC_Thruster(void) = 0; // for npc drivers to be able to use APC thrusters to upright themselves
#endif
	// Weapon handling
