...
void CPhysForce::ForceOn( void )
{
	...
	if ( m_forceTime )
	{
		SetNextThink( CURTIME + m_forceTime );
...
void CPhysMotor::TurnOn( void )
{
	...
	if ( pPhys )
	{
		...
		m_lastTime = CURTIME - TICK_INTERVAL;
		Think();
	}
}
...
void CPhysMotor::Think( void )
{
	...
	m_motor.m_speed = UTIL_Approach( m_flSpeed, m_motor.m_speed, m_angularAcceleration*(CURTIME-m_lastTime) );
	m_lastTime = CURTIME;
