...
#ifdef DARKINTERVAL
#include "entitylist.h"
// DARK INTERVAL NPC PARSING TEST
...
#ifdef DARKINTERVAL
ConVar	ai_frametime_limit("ai_frametime_limit", "30", FCVAR_NONE, "frametime limit for min efficiency AIE_NORMAL (in sec's).");
#else
...
#ifdef DARKINTERVAL // separate additional input when death caused by player
			if (pCulprit)
			{
				if (pCulprit->IsPlayer())
				{
					m_OnDeathByPlayer.FireOutput(pCulprit, this);
				}
			}
#endif
		}
...
#ifdef DARKINTERVAL
ConVar g_ai_use_deathposes("g_ai_use_deathposes", "1");

bool CAI_BaseNPC::ShouldPickADeathPose(void)
{
	return g_ai_use_deathposes.GetBool();
}
#if 0
...
void CAI_BaseNPC::Event_Killed( const CTakeDamageInfo &info )
{
	...
	// Untie our ragdoll handle from us when we die in invisible live-ragdoll state.
	// Dying like that should only happen if we took damage from the ragdoll handle.
	// If we don't do this, it'll get removed along with us.
	if (m_live_ragdoll_ent)
	{
	//	Msg("cleaning up live ragdoll ent\n");
		m_live_ragdoll_ent = NULL;
	}
	if (m_live_ragdoll_handle)
	{
	//	Msg("cleaning up live ragdoll handle\n");
		m_live_ragdoll_handle->SetRenderMode(kRenderNormal); // it was hidden on creation, unhide it.
		m_live_ragdoll_handle->SetDamageEntity(NULL);
	...
	// Select a death pose to extrapolate the ragdoll's velocity.
...
int CAI_BaseNPC::OnTakeDamage_Alive( const CTakeDamageInfo &info )
{
	...
	if ( info.GetAttacker()->GetFlags() & (FL_NPC | FL_CLIENT) )
	{
		...
		if ((CURTIME - m_flLastDamageTime) < 1.0)
		{
		...
		if ( info.GetAttacker() && info.GetAttacker()->IsPlayer() )
			m_flLastPlayerDamageTime = CURTIME;
...
{
#ifndef DARKINTERVAL // don't care about this bit, unused
...
#ifdef DARKINTERVAL
	// DI NEW: adds physgun push away for all NPCs, EXCEPT vital ally characters.
	// Ignore if we're in a dynamic scripted sequence
	if (subInfo.GetDamageType() & DMG_PHYSGUN 
		&& !IsRunningDynamicInteraction()
		&& Classify() != CLASS_PLAYER_ALLY_VITAL )
	{
		Vector	puntDir = (info.GetDamageForce() * 100.0f);

		subInfo.SetDamage(Min(5.0f, m_iMaxHealth / 15.0f));
		...
		if (info.GetDamage() >= GetHealth())
		{
			...
			// affected.
			subInfo.SetDamageForce(info.GetDamageForce() * 3000.0f);
		}
...
void CAI_BaseNPC::OnListened()
{
	...
	static int conditionsToClear[] =
	{
		...
		COND_HEAR_MOVE_AWAY,
#ifdef DARKINTERVAL
		COND_HEAR_HIDE_POS, // e. g. a point inside smoke grenade, good idea to hide in
#endif
		COND_NO_HEAR_DANGER,
		COND_SMELL,
	};
	...
	while ( pCurrentSound )
	{
		...
		if ( pCurrentSound->FIsSound() )
		{
			...
			{
				case SOUND_DANGER:			
					if( CURTIME > m_flIgnoreDangerSoundsUntil)
						condition = COND_HEAR_DANGER;			
					break;
#ifdef DARKINTERVAL
				case SOUND_HIDE_POSITION:	condition = COND_HEAR_HIDE_POS;			break;
#endif
...
{
	if (m_flNextEyeLookTime < CURTIME)
	{
		...
		if (pBestEntity)
		{
			m_flNextEyeLookTime = CURTIME + random->RandomInt(1,5);
...
void CAI_BaseNPC::MaintainLookTargets ( float flInterval )
{
	...
	if (m_flNextEyeLookTime < CURTIME)
	{
		...
		*/
		...
		m_flNextEyeLookTime = CURTIME + 0.5; // random->RandomInt(1,5);
	}
...
void CAI_BaseNPC::PerformMovement()
{
	...
	float flInterval = ( m_flTimeLastMovement != FLT_MAX ) ? CURTIME - m_flTimeLastMovement : 0.1;

	m_pNavigator->Move( ROUND_TO_TICKS( flInterval ) );
	m_flTimeLastMovement = CURTIME;
...
bool CAI_BaseNPC::PreThink( void )
{
	...
	{
		if ( CURTIME >= g_AINextDisabledMessageTime && !IsInCommentaryMode() )
		{
			g_AINextDisabledMessageTime = CURTIME + 0.5f;
...
void CAI_BaseNPC::RunAnimation( void )
{
	...
	if ( m_NPCState != NPC_STATE_SCRIPT && m_NPCState != NPC_STATE_DEAD && m_Activity == ACT_IDLE && IsActivityFinished() )
	{
		...
		if ( iSequence != ACTIVITY_NOT_AVAILABLE )
		{
			...
			// Basically everywhere else in the AI code this variable gets set to whatever our sequence is.
...
void CAI_BaseNPC::UpdateEfficiency( bool bInPVS )
{
	...
	if ( CURTIME - GetLastAttackTime() < .15 )
	{
		SetEfficiency( AIE_NORMAL );
		return;
	}
	...
	if ( m_bForceConditionsGather || 
		 CURTIME - GetLastAttackTime() < .2 ||
		 CURTIME - m_flLastDamageTime < .2 ||
...
#ifdef DARKINTERVAL
			if (pCurrentSound)	iSound = pCurrentSound->NextSound();
			else break;
#else
...
bool CAI_BaseNPC::PreNPCThink()
{
	...
	if ( bUseThinkLimits && VCRGetMode() == VCR_Disabled )
	{
		...
		else
		{
			...
			{
				float timeSinceLastRealThink = CURTIME - m_flLastRealThinkTime;
...
void CAI_BaseNPC::NPCThink( void )
{
	...
	if ( GetEfficiency() < AIE_DORMANT && GetSleepState() == AISS_AWAKE )
	{
		...
		if ( g_pAINetworkManager && g_pAINetworkManager->IsInitialized() )
		{
			...
			if ( PreThink() )
			{
				if ( m_flNextDecisionTime <= CURTIME )
				{
					bRanDecision = true;
...
#ifdef DARKINTERVAL // Ice, ported freezing code from ASW
	if (m_flFrozen > 0.0f && m_flFrozen < 1.0f)
	{
		SetPlaybackRate(GetPlaybackRate() * (1 - m_flFrozen));
	}
#endif
	if ( m_bUsingStandardThinkTime )
	{
		...
		if ( bRanDecision )
		{
			m_flNextDecisionTime = CURTIME + g_DecisionIntervals[GetEfficiency()];
		}

		if ( GetMoveEfficiency() == AIME_NORMAL || GetEfficiency() == AIE_NORMAL )
		{
			SetNextThink( CURTIME + .1 );
		}
		else
		{
			SetNextThink( CURTIME + .2 );
		}
	}
	else
	{
...
#ifdef DARKINTERVAL
//=========================================================
// CAI_BaseNPC - SetNextThinkByDistance - optimise thinking
// frequency based on distance to the player
//=========================================================
void CAI_BaseNPC::SetNextThinkByDistance(float minRndTime, float maxRndTime, float A, float B, float C, float D)
{
	CBasePlayer *pPlayer = AI_GetSinglePlayer();
	if (pPlayer == NULL)
	{
		SetNextThink(CURTIME + random->RandomFloat(0.5f, 1.0f));
		return;
	}

	float flDistToPlayerSqr = (GetAbsOrigin() - pPlayer->GetAbsOrigin()).LengthSqr();
	float scale = RemapValClamped(flDistToPlayerSqr, A, B, C, D);
	float time = random->RandomFloat(minRndTime, maxRndTime);
...
void CAI_BaseNPC::SetState( NPC_STATE State )
{
	...
	if ( State != m_NPCState )
	{
		m_flLastStateChangeTime = CURTIME;
	}
...
void CAI_BaseNPC::GatherConditions( void )
{
	...
	if( CURTIME > m_flTimePingEffect && m_flTimePingEffect > 0.0f )
	{
...
NPC_STATE CAI_BaseNPC::SelectIdleIdealState()
{
	...
	if ( HasCondition(COND_LIGHT_DAMAGE) || 
	   ...
	   (!GetEnemy() && CURTIME - GetEnemies()->LastTimeSeen( AI_UNKNOWN_ENEMY ) < TIME_CARE_ABOUT_DAMAGE ) )
	{
		Vector vecEnemyLKP;
...
//			 the future
// UNDONE: actually rate the weapons based on their strength
// Input  :
// Output :
//-----------------------------------------------------------------------------
bool CAI_BaseNPC::Weapon_IsBetterAvailable()
{
	...
	if( ShouldLookForBetterWeapon() )
	{
		if( GetActiveWeapon() )
		{
			m_flNextWeaponSearchTime = CURTIME + 2;
		}
		else
		{
			if( IsInPlayerSquad() )
			{
				...
				m_flNextWeaponSearchTime = CURTIME + 1;
			}
			else
			{
				m_flNextWeaponSearchTime = CURTIME + 2;
...
void CAI_BaseNPC::GatherEnemyConditions( CBaseEntity *pEnemy )
{
	...
	// ---------------------------
	if ( HasCondition( COND_NEW_ENEMY ) || GetSenses()->GetTimeLastUpdate( GetEnemy() ) == CURTIME )
	{
		...
		else
		{
			...
			if ( bSensesDidSee )
			{
				...
				SetCondition( COND_SEE_ENEMY );
#ifdef DARKINTERVAL
				// DI NEW - extra outputs regarding enemies found *in viewcone*
				if (GetEnemy()->IsPlayer())
				{
					m_OnFoundPlayerLOSAndViewCone.Set(hEnemy, this, this);
					m_OnFoundEnemyLOSAndViewCone.Set(hEnemy, this, this);
				}
				else
				{
					m_OnFoundEnemyLOSAndViewCone.Set(hEnemy, this, this);
			...
			if (!HasMemory( bits_MEMORY_HAD_LOS ))
			{
				AI_PROFILE_SCOPE(CAI_BaseNPC_GatherEnemyConditions_Outputs);
				// Send output event
#ifndef DARKINTERVAL // moved it upward
				EHANDLE hEnemy;
				hEnemy.Set(GetEnemy());
#endif
				if (GetEnemy()->IsPlayer())
	...
	//-----------------------------------------------------------------------
	if (CURTIME - GetEnemyLastTimeSeen() > 8)
	{
		//-----------------------------------------------------------------------
...
void CAI_BaseNPC::ResolveActivityToSequence(Activity NewActivity, int &iSequence, Activity &translatedActivity, Activity &weaponActivity)
{
	...
	else
	{
		...
		if ( iSequence == ACTIVITY_NOT_AVAILABLE )
		{
			...
			if ( ( pLastWarn != this && lastWarnActivity != translatedActivity ) || CURTIME - timeLastWarn > 5.0 )
			{
...
#ifndef DARKINTERVAL // no need for convar in DI because on by default
...
{
#ifndef DARKINTERVAL // obviously, NPCs are allowed in DI
	...
	// Only give weapon i allowed to have one
	if (CapabilitiesGet() & bits_CAP_USE_WEAPONS)
...
int	CAI_BaseNPC::HolsterWeapon( void )
{
	...
	if (iLayer != -1)
	{
		...
		m_ShotRegulator.FireNoEarlierThan( CURTIME + flDuration + 0.5 );
...
{
#ifdef DARKINTERVAL
	if (GetActiveWeapon() != NULL && !CanHolsterWeapon())
	{
		...
		//Force the NPC to recalculate its arrival activity since it'll most likely be wrong now that we don't have a weapon out.
...
void CAI_BaseNPC::InputUnholsterWeapon( inputdata_t &inputdata )
{
	...
	{
		int iHolsterGesture = FindGestureLayer(ACT_ARM);
		if (iHolsterGesture == -1) // no animation. Unholster manually, immediately
		{
			if (GetActiveWeapon())
			{
...
void CAI_BaseNPC::StartNPC( void )
{
	...
	if ( gm_flTimeLastSpawn != CURTIME )
	{
		gm_nSpawnedThisFrame = 0;
		gm_flTimeLastSpawn = CURTIME;
	}
	...
	SetNextThink( CURTIME + nextThinkTimes[gm_nSpawnedThisFrame % 20] );
...
void CAI_BaseNPC::RememberUnreachable(CBaseEntity *pEntity, float duration )
{
	...
	for (int i=m_UnreachableEnts.Size()-1;i>=0;i--)
	{
		...
		{
			m_UnreachableEnts[i].fExpireTime	 = CURTIME + NPC_UNREACHABLE_TIMEOUT;
	...
	m_UnreachableEnts[nNewIndex].fExpireTime	 = CURTIME + NPC_UNREACHABLE_TIMEOUT;
...
bool CAI_BaseNPC::IsValidEnemy( CBaseEntity *pEnemy )
{
	...
	if ( pEnemyNPC && pEnemyNPC->CanBeAnEnemyOf( this ) == false )
		return false;
#ifdef DARKINTERVAL
	if (IsOnFire()) return false; // As general rule, NPCs on fire should start ignoring everything
#endif
...
CBaseEntity *CAI_BaseNPC::BestEnemy( void )
{
	...
	for( AI_EnemyInfo_t *pEMemory = GetEnemies()->GetFirst(&iter); pEMemory != NULL; pEMemory = GetEnemies()->GetNext(&iter) )
	{
		...
		if ( m_bIgnoreUnseenEnemies )
		{
			...
			if ( pEMemory->timeLastSeen < CURTIME - TIME_CONSIDER_ENEMY_UNSEEN )
			{
		...
		if ( pEMemory->timeValidEnemy > CURTIME )
		{
...
#ifdef DARKINTERVAL
			else if( pEvent->event == AE_NPC_OPEN_DOOR_AWAY || pEvent->event == AE_NPC_OPEN_DOOR_TOWARDS_LEFT || pEvent->event == AE_NPC_OPEN_DOOR_TOWARDS_RIGHT)
			{
#if 1
#if 0
				CBasePropDoor *pDoor = (CBasePropDoor *)(CBaseEntity *)GetNavigator()->GetPath()->GetCurWaypoint()->GetEHandleData();
...
#else
				// check to see if there really is a door in front of me. 
				trace_t tr;
				Vector vTrace;
				GetVectors(&vTrace, NULL, NULL);
				AI_TraceLine(WorldSpaceCenter(), WorldSpaceCenter() + vTrace * 128, MASK_NPCSOLID, this, COLLISION_GROUP_NONE, &tr);
				if (tr.fraction != 1.0f)
				{
					if (tr.m_pEnt)
					{
						CBaseEntity *entToEnt = dynamic_cast<CBaseEntity*>(tr.m_pEnt);
						if (entToEnt)
						{
							CBasePropDoor *pDoor = dynamic_cast<CBasePropDoor*>(entToEnt);
							if (pDoor != NULL)
							{
								OpenPropDoorNow(pDoor);
							}
...
void CAI_BaseNPC::DrawDebugGeometryOverlays(void)
{
	...
	// ----------------------------------------------
	if ((m_debugOverlays & OVERLAY_NPC_FOCUS_BIT) && developer.GetInt() == 2)
	{
		if (GetEnemy() != NULL)
		{
...
int CAI_BaseNPC::DrawDebugTextOverlays(void)
{
	...
	if (m_debugOverlays & OVERLAY_TEXT_BIT)
	{
...
#ifdef DARKINTERVAL
		static const char *pMoveNames[] = { "None", "Ground", "Jump", "Fly", "Climb", "Swim" };
#else
		...
		{
			Q_snprintf(tempstr, sizeof(tempstr), "Enemy: %s (%s), ", GetEnemy()->GetEntityName(), GetEnemy()->GetClassname());
			EntityText(text_offset, tempstr, 0);
			NDebugOverlay::Cross3D(GetEnemy()->WorldSpaceCenter() + Vector(0, 0, 32), 8, 200, 80, 40, true, 0.1f);
			text_offset++;
		}

		// --------------
		// Print Combat Confidence
		// --------------
		//if (m_NPCState == NPC_STATE_COMBAT && GetEnemy())
		{
			Q_snprintf(tempstr, sizeof(tempstr), "Combat Confidence: %i, ", CombatConfidence());
			EntityText(text_offset, tempstr, 0);
			text_offset++;
		}
#endif
		...
		if (GetActiveWeapon())
		{
			Q_snprintf(tempstr, sizeof(tempstr), "Weapon:%s", GetActiveWeapon()->GetClassName());
			EntityText(text_offset, tempstr, 0);
			text_offset++;
			...
			if (GetActiveWeapon()->UsesClipsForAmmo1())
			{
				// only clip1, because NPCs only fire primary ammo (for now).
				Q_snprintf(tempstr, sizeof(tempstr), "Ammo (primary):%i/%i", GetActiveWeapon()->m_iClip1, GetActiveWeapon()->GetMaxClip1());
				EntityText(text_offset, tempstr, 0);
				text_offset++;
			}
			else
			{
				// only clip1, because NPCs only fire primary ammo (for now).
				Q_snprintf(tempstr, sizeof(tempstr), "Ammo (primary):%i", GetActiveWeapon()->m_iClip1);
				EntityText(text_offset, tempstr, 0);
				text_offset++;
			}
		}
...
void CAI_BaseNPC::ReportAIState( void )
{
	...
	if ( IsMoving() )
	{
		...
		if ( m_flMoveWaitFinished > CURTIME )
			DevMsg( ": Stopped for %.2f. ", m_flMoveWaitFinished - CURTIME );
...
float CAI_BaseNPC::GetSpreadBias( CBaseCombatWeapon *pWeapon, CBaseEntity *pTarget )
{
	...
	if ( pEnemyInfo )
	{
		...
		if ( timeToFocus > 0.0 )
		{
			float timeSinceValidEnemy = CURTIME - pEnemyInfo->timeValidEnemy;
			if ( timeSinceValidEnemy < 0.0f )
			{
				timeSinceValidEnemy = 0.0f;
			}
			float timeSinceReacquire = CURTIME - pEnemyInfo->timeLastReacquired;
...
bool CAI_BaseNPC::ShouldChooseNewEnemy()
{
	...
	if ( pEnemy )
	{
		...
		if ( m_FailChooseEnemyTimer.Expired() )
		{
			...
			if ( HasCondition( COND_TASK_FAILED ) || 
				 ( pInfo && ( pInfo->timeAtFirstHand == AI_INVALID_TIME || CURTIME - pInfo->timeLastSeen > 10 ) ) )
			{
				return true;
			}
		}

		if ( pInfo && pInfo->timeValidEnemy < CURTIME )
		{
...
{
	m_flSoundWaitTime = CURTIME + flSoundLength + random->RandomFloat(1.5, 2.0);
	m_nSoundPriority = soundPriority;

	if (m_pSquad)
	{
		m_pSquad->JustMadeSound( soundPriority, CURTIME + flSoundLength + random->RandomFloat(1.5, 2.0) );
	}
}
...
BEGIN_DATADESC( CAI_BaseNPC )
...
#ifdef DARKINTERVAL
	DEFINE_FIELD( m_ignite_damage_threshold_float, FIELD_FLOAT),
#endif
	...
	DEFINE_FIELD(m_flUnragdollTime, FIELD_TIME),
	DEFINE_FIELD(m_flStartRagdollTime, FIELD_TIME),
	DEFINE_FIELD(m_flExtraRagdollTime, FIELD_FLOAT),
	DEFINE_FIELD(m_rememberSolidFlags, FIELD_INTEGER),
	DEFINE_FIELD(m_rememberMoveType, FIELD_INTEGER),
	DEFINE_INPUTFUNC( FIELD_VOID, "FallInRagdoll", InputFallInRagdoll),
	DEFINE_INPUTFUNC( FIELD_FLOAT, "FallInRagdollForSeconds", InputFallInRagdollForSeconds),
	DEFINE_INPUTFUNC( FIELD_VOID, "ExitRagdoll", InputExitRagdoll),
	DEFINE_INPUTFUNC(FIELD_VOID, "EnableLiveRagdoll", InputEnableLiveRagdoll),
	DEFINE_INPUTFUNC(FIELD_VOID, "DisableLiveRagdoll", InputDisableLiveRagdoll),
	DEFINE_OUTPUT(m_OnFallInRagdoll, "OnFallInRagdoll"),
	DEFINE_OUTPUT(m_OnExitRagdoll, "OnExitRagdoll"),
#endif
	// Satisfy classcheck
	...
	DEFINE_OUTPUT( m_OnFoundEnemyLOSAndViewCone,	"OnFoundEnemyLOSAndViewCone"),
	DEFINE_OUTPUT( m_OnFoundPlayerLOSAndViewCone,	"OnFoundPlayerLOSAndViewCone"),
#endif
...
#ifdef DARKINTERVAL
	// DI NEW
	DEFINE_INPUTFUNC(FIELD_INTEGER, "CapabilitiesAdd", InputCapabilitiesAdd),
	DEFINE_INPUTFUNC(FIELD_INTEGER, "CapabilitiesRemove", InputCapabilitiesRemove),
	...
	DEFINE_INPUTFUNC(FIELD_STRING, "SetHintGroupLimitNav", InputSetHintGroupLimitNav),

	DEFINE_INPUTFUNC(FIELD_VOID, "SetStateAlert", InputSetStateAlert),
	DEFINE_INPUTFUNC(FIELD_VOID, "SetStateCombat", InputSetStateCombat),
	DEFINE_INPUTFUNC(FIELD_VOID, "SetStateIdle", InputSetStateIdle),
#endif
	// Function pointers
...
#else // Ice, ported freezing code from ASW
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
ConVar g_frozen_npcs_autoshatter("g_frozen_npcs_autoshatter", "0");
void CAI_BaseNPC::Freeze(float flFreezeAmount, CBaseEntity *pFreezer, Ray_t *pFreezeRay)
{
	...
	else
	{
		...
		if(ShouldBecomeStatue())
		{
			...
			if (IsAlive())
			{
				CreateServerStatue(this, COLLISION_GROUP_NONE, CTakeDamageInfo(pFreezer, pFreezer, 1000.0, DMG_GENERIC | DMG_REMOVENORAGDOLL | DMG_PREVENT_PHYSICS_FORCE), g_frozen_npcs_autoshatter.GetBool());
...
#ifdef DARKINTERVAL
// DARK INTERVAL NPC PARSING PROTOTYPE
const FileNPCInfo_t &CAI_BaseNPC::GetNPCScriptData(void) const
{
    ...
    m_bDeferredNavigation( false )
{
	...
	// DI NEW: Live Ragdolls Experiment (inspired by Dark Messiah)
	m_live_ragdoll_handle = NULL;
	m_flUnragdollTime = m_flStartRagdollTime = 0;
	m_flExtraRagdollTime = 0.1; // this is a bit of a hack. -1 is used for indefinite and 0 is used for "exit right now".
	m_rememberSolidFlags = 0;
	m_rememberMoveType = 0;

	// DARK INTERVAL NPC PARSING TEST
	m_hNPCFileInfo = GetInvalidNPCInfoHandle();
#endif
}
...
void CAI_BaseNPC::InputIgnoreDangerSounds( inputdata_t &inputdata )
{
	...
	m_flIgnoreDangerSoundsUntil = CURTIME + flDelay;
}
...
// DI NEW
//-----------------------------------------------------------------------------
void CAI_BaseNPC::InputSetHintGroup(inputdata_t &inputdata)
{
	const char *pszGroup = inputdata.value.String();
	SetHintGroup(MAKE_STRING(pszGroup));
}

void CAI_BaseNPC::InputSetHintGroupLimitNav(inputdata_t &inputdata)
{
	const char *pszGroup = inputdata.value.String();
	SetHintGroup(MAKE_STRING(pszGroup), true);
}
#endif
//-----------------------------------------------------------------------------
// Purpose: 
...
bool CAI_BaseNPC::CineCleanup()
{
	...
	if ( IsRunningDynamicInteraction() )
	{
		...
		if ( m_hInteractionPartner )
		{
			...
			//Msg("%s(%s) enabled collisions with %s(%s) at %0.2f\n", GetClassname(), GetDebugName(), m_hInteractionPartner->GetClassName(), m_hInteractionPartner->GetDebugName(), CURTIME );
		}
...
{	
	...
	if (HaveSequenceForActivity(opendata.eActivity) && !IsInAScript())
	{
		if (GetIdealActivity() != opendata.eActivity)
		{
			pDoor->SetNavIgnore(0.5f);
			SetIdealActivity(opendata.eActivity);
			// Wait for the door to finish opening before trying to move through the doorway.
			m_flMoveWaitFinished = CURTIME + SequenceDuration() + pDoor->GetOpenInterval();
		}
	}
	else	
#endif
...
void CAI_BaseNPC::OpenPropDoorNow( CBasePropDoor *pDoor )
{
	...
	// Wait for the door to finish opening before trying to move through the doorway.
	m_flMoveWaitFinished = CURTIME + pDoor->GetOpenInterval();
}
...
// DI NEW - water nodes
//-----------------------------------------------------------------------------
int CAI_BaseNPC::SwimMove(const Vector& pfPosition, unsigned int mask)
{
	Vector		oldorg, neworg;
	trace_t		trace;
...
#ifdef DARKINTERVAL
	if (GetFlags() & (FL_SWIM)) // DI NEW - water nodes
	{
		return SwimMove(vecPosition, mask);
	}
#endif
...
#ifdef DARKINTERVAL
		|| GetNavType() == NAV_SWIM ) // DI NEW - water nodes
#else
		)
#endif
	{
...
float CAI_BaseNPC::SetWait( float minWait, float maxWait )
{
	...
	if ( maxWait == 0.0 )
	{
		m_flWaitFinished = CURTIME + ( 0.1 * minThinks );
	}
	else
	{
		...
		m_flWaitFinished = CURTIME + ( 0.1 * random->RandomInt( minThinks, maxThinks ) );
	}
...
{
	return ( CURTIME >= m_flWaitFinished );
}
...
{ 
	...
	return ( m_flNextDodgeTime <= CURTIME );
}
...
void CAI_BaseNPC::StartScriptedNPCInteraction( CAI_BaseNPC *pOtherNPC, ScriptedNPCInteraction_t *pInteraction, Vector vecOtherOrigin, QAngle angOtherAngles )
{
	...
	if ( pOtherNPC )
	{
		...
		//Msg("%s(%s) disabled collisions with %s(%s) at %0.2f\n", GetClassname(), GetDebugName(), pOtherNPC->GetClassName(), pOtherNPC->GetDebugName(), CURTIME );
	...
	// Setup next attempt
	pInteraction->flNextAttemptTime = CURTIME + pInteraction->flDelay + RandomFloat(-2,2);
...
void CAI_BaseNPC::CheckForScriptedNPCInteractions( void )
{
	...
	for ( int i = 0; i < m_ScriptedInteractions.Count(); i++ )
	{
		...
		if ( pInteraction->flNextAttemptTime > CURTIME )
			continue;
...
void CAI_BaseNPC::CheckForcedNPCInteractions( void )
{
	...
	if ( !bAbort )
	{
		if ( !InteractionCouldStart( pNPC, pInteraction, vecOrigin, angAngles ) )
		{
			if ( ( CURTIME > m_flForcedInteractionTimeout ) && ( m_iInteractionState == NPCINT_MOVING_TO_MARK ) )
			{
				bAbort = true;
			}
			else
...
void CAI_BaseNPC::StartForcedInteraction( CAI_BaseNPC *pNPC, int iInteraction )
{
	...
	m_flForcedInteractionTimeout = CURTIME + 8.0f;
...
void CAI_BaseNPC::ModifyOrAppendCriteria( AI_CriteriaSet& set )
{
	...
	{
		set.AppendCriteria( "timesinceseenplayer", UTIL_VarArgs( "%f", CURTIME - m_flLastSawPlayerTime ) );
	}
	else
	{
...
#ifdef DARKINTERVAL
//const char *CAI_BaseNPC::GetModelName(int /*viewmodelindex = 0 -- this is ignored in the base class here*/) const
//{
//	return GetNPCScriptData().szModel;
//}

void CAI_BaseNPC::InputFallInRagdoll(inputdata_t &inputdata)
{
	m_flExtraRagdollTime = 5;
	ClearSchedule("forced to go to ragdoll via input");
	SetSchedule(SCHED_DI_FALL_IN_RAGDOLL_PUSH);
}

void CAI_BaseNPC::InputFallInRagdollForSeconds(inputdata_t &inputdata)
{
	m_flExtraRagdollTime = inputdata.value.Float();
	ClearSchedule("forced to go to ragdoll via input");
	SetSchedule(SCHED_DI_FALL_IN_RAGDOLL_PUSH);
}

void CAI_BaseNPC::InputExitRagdoll(inputdata_t &inputdata)
{
	m_flExtraRagdollTime = 0;
}

void CAI_BaseNPC::InputEnableLiveRagdoll(inputdata_t &inputdata)
{
	CapabilitiesAdd(bits_CAP_LIVE_RAGDOLL);
}

void CAI_BaseNPC::InputDisableLiveRagdoll(inputdata_t &inputdata)
{
	CapabilitiesRemove(bits_CAP_LIVE_RAGDOLL);
}

void CAI_BaseNPC::InputCapabilitiesAdd(inputdata_t &inputdata)
{
	CapabilitiesAdd(inputdata.value.Int());
}

void CAI_BaseNPC::InputCapabilitiesRemove(inputdata_t &inputdata)
{
...
{
	if (/*MAKE_STRING*/(inputdata.value.StringID()) != NULL_STRING)
	{
		CBaseEntity *find = gEntList.FindEntityByName(NULL, inputdata.value.StringID());
		if (find)
		{
			SetTarget(find);
		//	NDebugOverlay::Cross3D(find->WorldSpaceCenter(), 8, 255, 0, 0, true, 5);
		}
		else
			Warning("Could not find target entity %s for %s!\n", inputdata.value.StringID(), GetDebugName());
	}
}

void CAI_BaseNPC::InputClearTarget(inputdata_t &inputdata)
{
	SetTarget(NULL);
}
#endif